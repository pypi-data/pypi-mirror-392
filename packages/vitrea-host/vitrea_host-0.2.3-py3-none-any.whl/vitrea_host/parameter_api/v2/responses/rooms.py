...
from vitrea_host.parameter_api.v2.commands.rooms import GetRoomParams
from vitrea_host.models.database import RoomModel
...
class RoomNumbersParser(BaseParameterResponseParser):
    ...
    async def parse_response(self):
        ...
        for room in self.rooms_list:
            await self.send_callback(GetRoomParams(room))
...
class RoomParamsParser(BaseParameterResponseParser):
    ...
    async def parse_response(self):
        ...
        return [RoomModel(id=room_id, name=room_name, floor_id=floor_id)]
