from vitrea_host.utils import const
...
class VitreaDatabaseModel:
    ...
    def get_all_ids(self):
        return {
            "keys": [{"key_id": key.id, "keypad_id": key.keypad_id} for key in self.keys],
            "air_conditioners": [{"ac_id": ac.id} for ac in self.air_conditioners],
        }
        
    def get_state_activated_ids(self):
        result = {
            "keys": [{"key_id": key.id, "keypad_id": key.keypad_id} for key in self.keys if key.type in const.POLLABLE_KEY_TYPES],
            "air_conditioners": [{"ac_id": ac.id} for ac in self.air_conditioners],
        }
        return result