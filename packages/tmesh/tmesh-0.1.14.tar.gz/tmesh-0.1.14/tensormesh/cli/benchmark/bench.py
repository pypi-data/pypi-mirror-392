...
class WorkloadConfig: 
    ...
    def _find_model(endpoint: str, api_key: str) -> str:
        # 1. Try OpenAI SDK path (works for vLLM, OpenAI-compatible servers)
        try:
            client = OpenAI(base_url=endpoint, api_key=api_key)
            models = client.models.list()
            data = models.data
            print("successfully queried models from OpenAI SDK")
        except Exception:
            print("falling back to raw HTTP GET")
            # 2. Fallback: raw HTTP GET (required for Together, etc.)
            try:
                resp = requests.get(
                    f"{endpoint}/models",
                ...
                raise RuntimeError(
                    f"Failed to query /models from endpoint '{endpoint}': {e}"
                )
