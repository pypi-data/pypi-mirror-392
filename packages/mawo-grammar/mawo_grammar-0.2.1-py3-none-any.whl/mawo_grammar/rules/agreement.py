...
class AdjNounAgreementRule(Rule):
    ...
    def check(self, text: str, morphology: Any = None) -> RuleResult:
        ...
        """
        # TEMPORARY: This rule produces too many false positives with markdown line breaks
        # Problem: "программного обе\nспечения" → split() → ["программного", "обе", "спечения"]
        # Rule tries to check "обе спечения" which are FRAGMENTS, not real words!
        #
        # This is a fundamental design issue with .split() tokenization on markdown text.
        # Proper fix requires:
        # 1. Better tokenization that preserves word boundaries across \n
        # 2. Or smarter fragment detection
        # 3. Or using original text positions instead of split tokens
        #
        # For now, disable to avoid flooding with false positives
        ...
        # Tokenize and analyze (preserve whitespace/newlines)
        ...
        # Find adjective-noun pairs (check adjacent words)
        for i in range(len(tokens) - 1):
            adj_token = tokens[i].strip(",.!?;:\"'()[]{}«»")
            noun_token = tokens[i + 1].strip(",.!?;:\"'()[]{}«»")

            # Skip empty tokens
            if not adj_token or not noun_token:
                continue

            # Skip if tokens are too short (likely punctuation)
            if len(adj_token) < 2 or len(noun_token) < 2:
                continue

            # Skip if tokens contain Latin characters (English text)
            # Example: "Contributor Covenant" - skip English names
            # Check for Latin/English letters (ord < 0x400 = before Cyrillic range)
            has_latin_adj = any(ord(c) < 0x400 and c.isalpha() for c in adj_token)
            has_latin_noun = any(ord(c) < 0x400 and c.isalpha() for c in noun_token)
            if has_latin_adj or has_latin_noun:
                continue  # Skip English/Latin text
            ...
            # CRITICAL: Skip word fragments from line breaks!
            # Problem: "программного обе\nспечения" → ["обе", "спечения"]
            #
            # Conservative approach: Skip pairs where EITHER word is very short
            # This catches line-break fragments while allowing real short words
            # in clear contexts
            if len(adj_token) < 4 or len(noun_token) < 5:
                # At least one word is short - might be fragment
                # Be extra conservative: skip if EITHER has low score
                adj_score = getattr(adj_parses[0], "score", 1.0)
                noun_score = getattr(noun_parses[0], "score", 1.0)

                if adj_score < 0.8 or noun_score < 0.8:
                    continue  # Likely fragment, skip

            # CRITICAL FIX: Try to find matching adjective-noun pair among ALL parses
            # Don't just take the first parse (adj_parses[0]) - it may be wrong!
            #
            # Example: "нашем сообществе"
            # - "нашем" can be: masc/prep OR neut/prep
            # - "сообществе" is: neut/prep
            # - We need to find the neut/prep parse of "нашем", not masc/prep!

            adj_parse, noun_parse = self._find_matching_parse(adj_parses, noun_parses)
            if adj_parse is None or noun_parse is None:
                # No adjective-noun pair found
                continue
            ...
            # Verify it's actually an adjective-noun pair
            if adj_tag.POS not in ("ADJF", "ADJS", "PRTF") or noun_tag.POS not in ("NOUN", "NPRO"):
                continue
    ...
    def _find_matching_parse(
        self, adj_parses: list[Any], noun_parses: list[Any]
    ) -> tuple[Any | None, Any | None]:
        """Find matching adjective and noun parses.

        Checks all possible parse combinations to find one where:
        1. Adjective is ADJF/ADJS/PRTF
        2. Noun is NOUN/NPRO
        3. They agree in gender, case, and number

        Returns:
            Tuple of (adj_parse, noun_parse) if matching pair found, (None, None) otherwise
        """
        # First, try to find ANY pair where POS matches adjective-noun pattern
        adj_candidates = []
        noun_candidates = []

        for adj_p in adj_parses:
            if not hasattr(adj_p, "tag") or not hasattr(adj_p.tag, "POS"):
                continue
            if adj_p.tag.POS in ("ADJF", "ADJS", "PRTF"):
                adj_candidates.append(adj_p)

        for noun_p in noun_parses:
            if not hasattr(noun_p, "tag") or not hasattr(noun_p.tag, "POS"):
                continue
            if noun_p.tag.POS in ("NOUN", "NPRO"):
                noun_candidates.append(noun_p)

        # No adjective-noun candidates found
        if not adj_candidates or not noun_candidates:
            return (None, None)

        # Now try to find a MATCHING pair (agreeing in gender/case/number)
        for adj_p in adj_candidates:
            for noun_p in noun_candidates:
                adj_tag = adj_p.tag
                noun_tag = noun_p.tag
        ...
        # No matching pair found - they don't agree
        # Return first candidates to report the error
        return (adj_candidates[0], noun_candidates[0])
