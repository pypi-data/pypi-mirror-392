...
```python
from core import DataFakeGenerator
...
DataFakeGenerator(schema).many(100).export(
    table_name="users",
    output_dir="output"
)
```
...
| Template           | `template`           | Concatenate values from several columns into one                                     | `{first_name} {last_name}`, `User-{id}`       |
...
# Template concatenation
{
    ...
    "group": "advanced",
    "options": {
        "format": "{{first_name}} {{last_name}} ({{employee_id}})"
    }
}
...
DataFakeGenerator(schema).many(5000).export("orders", "exports", formats=["csv", "json", "excel"])
```
...
DataFakeGenerator(schema).many(1000).export("patients", "medical_data")
```
...
DataFakeGenerator(schema).many(10000).export("transactions", "finance")
```
...
DataFakeGenerator(schema).many(2000).export("bookings", "travel_data")
```
...
DataFakeGenerator(schema).many(500).export("servers", "monitoring")
```
...
DataFakeGenerator(schema).many(3000).export("employees", "hr_data")
```
...
DataFakeGenerator(schema).many(5000).export("crypto_trades", "blockchain")
```
...
DataFakeGenerator(schema).many(1500).export("students", "education_data")
```
...
DataFakeGenerator(schema).many(10000).export("iot_sensors", "sensor_data")
```
...
DataFakeGenerator(schema).many(2000).export("properties", "real_estate")
```
...
```
core/
├── datafake_generator.py  # Main entry point
├── base_generator.py      # Core generation logic
...
### 2. Leverage Templates for Complex Fields
...
# Combine multiple fields into one
{
    ...
    "group": "advanced",
    "options": {
        "format": "{street_address}, {city}, {state} {postal_code}"
    }
}
```

### 3. Use Distributions for Realistic Data
...
### 4. Custom Lists for Domain-Specific Values
...
### 5. Sequential IDs with Custom Patterns
...
**DataFakeGenerator** - Making test data generation simple and powerful
