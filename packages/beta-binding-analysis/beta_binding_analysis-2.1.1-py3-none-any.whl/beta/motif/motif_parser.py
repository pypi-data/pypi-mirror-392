...
class MotifParser:
    ...
    def ParserTable(self, tfile):
        ...
        for each in headList:
            if each not in self.all_list:
                Info(
                    "WARNING: column name <%s> is not a node, it will input but can't output. use 'MP_Object.all_list' to get formal format."
                    % each
                )
                # return 1
    ...
    def SearchMotif(self, **attrs):
        ...
        for i in list(attrs.keys()):
            if i not in self.all_list:
                Info(
                    "Wrong input attr:%s, select attr from:\n: %s"
                    % (i, List2Str(self.attr_list + self.tag_list, ","))
                )
                return None
        ...
        for attr in list(attrs.items()):
            ...
            for i in list(sub_motifs.motifs.items()):
                ...
                elif attr[1].upper() in (SEP.join(i[1][attr[0]])).upper().replace("::", SEP).split(
                    SEP
                ):
                    temp_dict[i[0]] = i[1].copy()
    ...
    def ToXml(self, xmlfile, xsl=False, sortkey=""):
        ...
        if xmlString[0].find("xml") != -1:
            outf.write(
                r'<?xml-stylesheet type="text/xsl" href="%s.xsl"?>' % os.path.split(xmlfile_)[-1]
                + "\n"
            )
...
<table cellspacing="1" border="1" bordercolor="#FF9900">
    ...
    def _Similarity(self, motifid1, motifid2, metric="Bayesian"):
        ...
        if len(self.motifs[motifid1]["pssm"]) == 1 and len(self.motifs[motifid2]["pssm"]) == 1:
            ...
            similarity_score, offset, antisense, flag_merge = bayesian_motif_comp.BLiC_score(
                m1.pssm, m2.pssm
            )
    ...
    def _ParserTable(self, tfile):
        ...
        for each in headList:
            if each not in self.all_list:
                print(
                    "WARNING: column name '%s' is not a node, use 'MP_Object.attr_list' to get formal format."
                    % each
                )
                # return 1
