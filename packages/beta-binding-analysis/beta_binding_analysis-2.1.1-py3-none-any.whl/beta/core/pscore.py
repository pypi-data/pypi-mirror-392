    ...
    def __init__(self, options):
        ...
        if options.genome == "hg19":
            if options.boundarylimit == False:
                Info(
                    "You do not like filter peak by CFCT boundary, it will be filtered only by the distance"
                )
                self.boundaryfile = ""
            else:
        ...
        if options.genome == "hg38":
            if options.boundarylimit == False:
                ...
                if not options.boundaryfile:
                    Info(
                        "We didn't provide a CTCF boundary file in hg38 version, please give the boundary file via --bf"
                    )
                    sys.exit(1)
                else:
                    self.boundaryfile = options.boundaryfile
        ...
        if options.genome == "mm10":
            if options.boundarylimit == False:
                ...
                if not options.boundaryfile:
                    Info(
                        "We didn't provide a CTCF boundary file in mm10 version, please give the boundary file via --bf"
                    )
                    sys.exit(1)
                else:
                    self.boundaryfile = options.boundaryfile
        ...
        if options.genome == "hg18":
            if options.boundarylimit == False:
                ...
                if not options.boundaryfile:
                    Info(
                        "We didn't provide a CTCF boundary file in hg18 version, please give the boundary file via --bf"
                    )
                    sys.exit(1)
                else:
                    self.boundaryfile = options.boundaryfile
        ...
        if options.genome == "hg" or options.genome == "mm" or (not options.genome):
            ...
            if options.boundarylimit != False:
                ...
                Info(
                    "We don not provide a CTCF boundary file for %s, the peak will be filtered only by the distance"
                    % options.genome
                )
                self.boundaryfile = ""
        ...
        for igene in self.geneInfo:
            ...
            for i in boundarys:
                if int(i[1]) > int(gTTS):
                    downstreams.append(
                        int(i[1])
                    )  # boundary tss > gene tts is the upstream boundarys
                if int(i[2]) < int(gTSS):
                    upstreams.append(
                        int(i[2])
                    )  # boundary tts < gene tss is the downstream boundarys
                else:
                    continue
            upstreams.sort()
            downstreams.sort()
            ...
            try:
                ...
                peaks = []
            peaksInDistance = [
                abs((t[1] + t[2]) / 2 - gTSS) * 1.0 / distance
                for t in peaks
                if abs((t[1] + t[2]) / 2 - gTSS) < distance
                and not t[2] < uplimit
                and not t[1] > downlimit
            ]  # peak pvalue<1e-5, and distance < distance
            ...
            peakswithinDistance = [
                ...
                for t in peaks
                if abs((t[1] + t[2]) / 2 - gTSS) < distance
                and not (t[2] < uplimit or t[1] > downlimit)
            ]
