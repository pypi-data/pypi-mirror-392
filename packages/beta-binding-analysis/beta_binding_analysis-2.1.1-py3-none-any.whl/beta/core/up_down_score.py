...
def prepare_optparser():
    ...
    optparser = OptionParser(
        version="%prog 0.5", description=description, usage=usage, add_help_option=False
    )
    ...
    optparser.add_option(
        ...
        action="append",
        type="string",
        help="Gene set to see the association with the genomic coordinates given through -b. Multiple gene sets can be given by repeatedly using this option (e.g. -g geneset1 -g geneset2 -g geneset3). Refseq accession numbers or official gene symbols can be used.",
    )
    optparser.add_option(
        "--bg",
        dest="bg",
        type="string",
        help="the background genes file you want to set as a control, make it as a gene list file as the gset you input.Default = ALL",
        default="ALL",
    )
    optparser.add_option(
        "--name",
    ...
    optparser.add_option(
        ...
        action="append",
        type="string",
        help="Label for each gene set. Likewise, multiple gene set labels can be given by repeatedly using this option (e.g. -l label1 -l label2 -l label3). If labels are not given, 'gene set' will be used by default.",
        default=None,
    )
    optparser.add_option(
        "--gname2",
        dest="name2",
        action="store_true",
        help="If this switch is on, gene or transcript IDs in files given through -g will be considered as official gene symbols.",
        default=False,
    )
    return optparser
...
def read_score_file(scorefile):
    ...
    symbolscores = (
        {}
    )  # a refseq ID or gene symbols may have several locations, hence they have several scores
    ...
    symbol_score = sorted(
        list(symbolscores.items()), key=lambda symbolscores: symbolscores[1], reverse=True
    )
    # [('ad',2),('ab',2),('cd',3)]
...
def write_R_code(bgscore, genesets_score, label, name):
    ...
    rscript += "downlimit <- %f\n" % minscore
    rscript += (
        'plot(x, cbg, type="l", lty=2, lwd=2, col="black", xlab="Rank of genes based on Regulatory Potential Score (from high to low)", ylab="Cumulative Fraction of Genes%%", main="%s Activating/Repressive Function Prediction", xlim=c(downlimit, uplimit), ylim=c(0, 100))\n'
        % name
    )
    rscript += "\n"
