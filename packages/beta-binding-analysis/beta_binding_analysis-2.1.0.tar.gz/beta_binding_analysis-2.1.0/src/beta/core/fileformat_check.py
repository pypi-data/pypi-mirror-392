...
class checkfileformat:
    ...
    def bed_format(self, check_3col_bed=True):
        fname = (
            self.peakfile
        )  # the peakfile format should be at least 5 column, if it is 3 column, we need to do some conversion
                ...
                if is_5col_bed(last_line):
                    ...
                    if check_3col_bed:
                        ...
                        if is_3col_bed(last_line):
                            ...
                            Info(
                                "Right Format should look like:\t%s"
                                % ("chr1\t567577\t567578\tMACS_peak_1\t119.00")
                            )
                            Info(
                                "Or the depreciate 3-column format like this:\t%s"
                                % ("chr1\t567577\t567578")
                            )
                        return False
                    else:
    ...
    def check_expr(self):
        ...
        with open(exprefile, "r") as expref:
            ...
            if not first_line.startswith("#"):
                ...
                Info(
                    "Take the first line with Differential Information as an example: %s"
                    % second_line
                )
                ...
                if self.gene == False:
                    if not is_refseq(refseq) or not is_value(tscore) or not is_value(fdr):
                        Info(
                            "BETA cannot recognize the refseq gene ID, status value(logFC) or FDR. Please give the exact column numbers of the refseq, logFC, and fdr like: 1,2,6 for LIMMA; 2,10,13 for Cufdiff; and 1,2,3 for BETA specific format."
                        )
                        sys.exit()
                    else:
                ...
                if self.gene == True:
                    if not is_genesymbol(refseq) or not is_value(tscore) or not is_value(fdr):
                        Info(
                            "BETA cannot recognize the official gene symbol, status value(logFC) or FDR. Please give the exact column numbers of the genesymbol, logFC, and fdr like: 1,2,6 for LIMMA; 2,10,13 for Cufdiff; and 1,2,3 for BETA specific format."
                        )
                        sys.exit()
                    else:
                        Info("Differential Expression file format successful passed")
