...
def opt_validate_basic(optparser):
    ...
    if not options.genome and not options.reference:
        optparser.print_help()
        Info(
            "ERROR: Genome (hg38, hg19, hg18, mm10, mm9) or the refseq geneinfo from UCSC is required."
        )
        sys.exit(1)

    if not os.path.isfile(options.peakfile):
        Info(
            "ERROR: Cannot find peak file, a tab-delimit peak file must be given through -p (--peakfile)."
        )
        sys.exit(1)
    if not os.path.isfile(options.exprefile):
        Info(
            "ERROR: Cannot find differential expression file, a tab-delimit differential expression file must be given through -e (--diff_expr)."
        )
        sys.exit(1)
    if options.reference and not os.path.isfile(options.reference):
        Info(
            "ERROR: Cannot find the refseq gene info, a tab-dilimit 6 column (refseqID, chroms, strand, txstart, txend, genesymbol) file required via -r --reference"
        )
        sys.exit(1)
    if options.boundaryfile and not os.path.isfile(options.boundaryfile):
        Info(
            "ERROR: Cannot find the bounaryfile, a tab-dilimit bed file (at least 3 columns) is required"
        )
        sys.exit(1)
    if (
        options.genome != "hg19"
        and options.genome != "mm9"
        and options.genome != "hg38"
        and options.genome != "mm10"
        and options.genome != "hg18"
        and options.genome
    ):
        Info(
            "ERROR: Input a correct genome: hg38, hg19, hg18, mm10, mm9 (or provide reference files via -r parameter)"
        )
        sys.exit(1)
    if not options.genome and options.boundarylimit == True and not options.boundaryfile:
        Info(
            "ERROR: Do you really want to use CTCF boundary to filter peaks? If so, please inpit the boundary file via --bf, else, please don not use --bl"
        )
        sys.exit(1)
    if not options.kind or (
        options.kind != "LIM"
        and options.kind != "CUF"
        and options.kind != "BSF"
        and options.kind != "O"
    ):
        Info(
            "ERROR: Please input the right kind of your expression file,  LIM for LIMMA standard format. CUF for CUFDIFF standard format, BSF for BETA specific format and O for other formats"
        )
        sys.exit(1)
    if options.kind == "O" and not options.expreinfo:
        Info(
            "ERROR: The differntial expression infomation is required when the it is other formats"
        )
    if options.method != "score" and options.method != "distance":
        Info(
            "ERROR: please choose the correct method to do the CR/TF function prediction, score for regulatory potential and distance for the distance to the proximal binding peak"
        )
        sys.exit(1)
    if not options.name:
        options.name = "NA"
...
def opt_validate_super(optparser):
    ...
    options = optparser.parse_args()
    if (
        not options.peakfile
        or not options.exprefile
        or not options.kind
        or not options.genomesequence
    ):
        optparser.print_help()
        sys.exit(1)
    if (
        not options.genome or options.genome == "mm" or options.genome == "hg"
    ...
    if not os.path.isfile(options.genomesequence):
        Info(
            "ERROR: Cannot find genome sequece data, a fasta format genome sequence data is requested"
        )
        sys.exit(1)
    if options.reference and not os.path.isfile(options.reference):
        Info(
    ...
    if (
        ...
        and options.genome != "mm10"
        and options.genome != "hg18"
        and options.genome != "hg"
        and options.genome != "mm"
        and options.genome
    ):
        Info(
            "ERROR: Input a correct genome: hg38, hg19, hg18, mm10, mm9, hg, mm (or provide reference files via -r parameter)"
        )
        sys.exit(1)
    if not options.genome and options.boundarylimit == True and not options.boundaryfile:
        Info(
    ...
    if options.motifnumber < 0:
        Info(
            "--mn error, please set a number bigger than 0, 0~1 as the pvalue and >1 as the number to get the motif output in html file"
        )
...
def opt_validate_noexpre(optparser):
    ...
    if not options.genome and not options.reference:
        optparser.print_help()
        Info(
            "ERROR: Genome(hg19, mm9, hg38, mm10, hg18) or the refseq geneinfo from UCSC is request."
        )
        sys.exit(1)
    ...
    if (
        ...
        Info(
            "ERROR: Input a right genome, hg19, mm9, hg38, mm10, hg18 or input reference files via other parameter"
        )
        sys.exit(1)
    if not options.genome and options.boundarylimit == True and not options.boundaryfile:
        Info(
