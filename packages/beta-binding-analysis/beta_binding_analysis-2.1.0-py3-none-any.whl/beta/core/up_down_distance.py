...
def readfile(peakfile):
    ...
    for line in peakf:
        ...
        try:
            peaklist[line[0]].append(
                line
            )  # peaklist = {chr1:[[start,end,name,score],[],[]...], chr2:[[],[],[]...]...}
        except KeyError:
        ...
        if line.startswith("#") or line == "\n" or not line:
            ...
            if line[1] in chroms:
                info = [
                    ...
                    line[2],
                    line[5],
                ]  # ['chr1', 'NM_032291', '66999824', '67210768', '+', 'SGIP1']
...
def write_R_code(alldist, geneset_dists, label, name):
    ...
    rscript += (
        'plot(x, cbg, type="l", lty=2, lwd=2, col="black", xlab="Rank of genes based on log(10) distance to nearest peak", ylab="Cumulative Fraction of Genes%%", main="%s Activating/Repressive Function Prediction", xlim=c(0, uplimit), ylim=c(0, 100))\n'
        % name
    )
    rscript += "\n"
    rscript += "l <- c()\n"
