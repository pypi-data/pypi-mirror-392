...
# Script: tasks-move-to-lane.sh
# Purpose: Move tasks between workflow lanes (planned/doing/review/done)
...
if [[ "$SHOW_HELP" == true ]]; then
    show_script_help "$(basename "$0")" \
        "Move tasks between workflow lanes (planned/doing/review/done)"
    exit $EXIT_SUCCESS
fi
