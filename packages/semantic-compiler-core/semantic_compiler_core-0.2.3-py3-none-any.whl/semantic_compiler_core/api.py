...
from semantic_compiler.sir_model import DecisionPipeline, sir_from_dict
from semantic_compiler.sir_to_rlang import sir_to_rlang as _sir_to_rlang_impl
...
def compile_sir_to_proof(sir: Dict[str, Any], input_value: Any) -> Dict[str, Any]:
    """
    ...
    # Step 2: RLang → ProofBundle
    proof_result = run_with_proof(rlang_source, input_value)
    
    # Step 3: Normalize SIR dict
    if isinstance(sir, dict):
        from semantic_compiler.sir_model import sir_from_dict, sir_to_dict
        sir_obj = sir_from_dict(sir)
        sir_dict = sir_to_dict(sir_obj)
    elif isinstance(sir, DecisionPipeline):
        from semantic_compiler.sir_model import sir_to_dict
