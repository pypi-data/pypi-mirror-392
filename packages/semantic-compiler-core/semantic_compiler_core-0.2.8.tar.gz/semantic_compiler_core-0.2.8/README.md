...
4. **RLang Runtime** (`rlang_runtime.py`): Wraps the `rlang-compiler` PyPI package to execute RLang code and generate proof bundles. Handles IR canonicalization and hash extraction. Automatically generates function registry from SIR SetOutput steps.

5. **Public API** (`api.py`): Clean, stable API surface:
   - `compile_sir_to_proof()`: One-shot SIR → ProofBundle compilation (auto-generates fn_registry)
   - `sir_to_rlang()`: SIR → RLang source code
   - `run_with_proof()`: RLang → ProofBundle execution (auto-generates fn_registry from SIR if provided)
...
- **Automatic function registry**: No manual fn_registry needed - automatically generated from SIR SetOutput steps
...
# Note: fn_registry is automatically generated from SIR SetOutput steps
# Pass sir parameter to enable auto-generation, or provide fn_registry explicitly
result = run_with_proof(rlang_source, input_value=15, sir=sir)
