...
class UserDiscover:
    ...
    def enumerate_via_author_id_endpoint(self, author_id: int, space=1):
        ...
        if response.status_code == 200:
            ...
            if name_from_title:
                ptprinthelper.ptprint(f"ID: {author_id}{' '*max_length}   → {url}{' '*max_length} →   {name_from_title}", "VULN", condition=not self.args.json, indent=4, clear_to_eol=True)
