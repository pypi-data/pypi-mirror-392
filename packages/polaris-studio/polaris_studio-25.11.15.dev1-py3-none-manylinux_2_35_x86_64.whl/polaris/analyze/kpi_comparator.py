...
class KpiComparator:
    ...
    def plot_trip_length_distributions(self, max_dist=None, modes=None, types=None, use_imperial=True, **kwargs):
        ...
        combinations = zip(combinations, ["-", "--", "-.", ":"] * (len(combinations) // 4 + 1))
        ...
        for run_id in df.run_id.unique():
            for (mode, type), line_style in combinations:
                ...
                print(label)
                ax.plot( df_.distance, df_.trips, label=label, linestyle=line_style)
    ...
    def plot_freight_distance_distribution_by_mode(self, mode_filter=None, **kwargs):
        ...
        ax.set(
            title="Distance Distribution by Freight Mode",
            xlabel="Distance bin (kilometer)",
            ylabel="Trip count"
        )
    ...
    def plot_freight_shipment_count_share_by_mode(self, mode_filter=None, **kwargs):
        ...
        ax = plt.gca()
        sns.barplot(
            data=df.assign(share=df.share * 100), x="freight_mode", y="share", hue="freight_mode", palette="pastel"
        )
    ...
    def plot_trip_count_by_attributes(self, **kwargs):
        ...
        df_plot["x_label"] = df_plot["type"].astype(str) + "-" + \
                             df_plot["mode"].astype(str) + "-" + \
                             df_plot["purpose"].astype(str)
        if df_plot.empty:
            logging.warning("No trips found after aggregation.")
            return
        ...
        ax.set(
            title="Trip Count by Attributes",
            xlabel="Trip Attributes (Type-Mode-Purpose)",
            ylabel="Trip count"
        )
    ...
    def _plot_truck_vxt_by_mode_trade_type(self, mode_filter=None, metric_type="M_VMT", **kwargs):
        ...
        agg_df = df.groupby(["mode", "freight_mode", "trade_type"], as_index=False)[metric_type].sum()
        ...
        fig.figure.suptitle(f"{metric_label} breakdown by trade type, shipment mode and vehicle mode", y=1.05)

        KpiComparator._style_axes(fig.axes.flat, rotate_x_labels=45, fontsize=14, common_legends=True)
