...
class ResultKPIs:
    ...
    def metric_calibration_mode_share(self):
        ...
        # It may not be 1 because maybe the target file may have split "TRANSIT"
        # into "BUS" and "RAIL" but the simulated as implemented here only has
        # "TRANSIT"
        mode_choice_df[["simulated", "target"]] /= mode_choice_df.groupby(["type"])[["simulated", "target"]].sum()
    ...
    def metric_spatial_trips(self):
        ...
        with commit_and_close(ScenarioCompression.maybe_extract(self.inputs.demand_db)) as conn:
            sql = """
            SELECT person, start, end, origin, destination, mode, type, travel_distance from trip
        ...
        keep_cols = ["trips", "travel_distance", "duration"]
        trip_start_df = trip_df.groupby(["origin_zone", "mode", "type", "purpose", "start_period"])[keep_cols].sum()
        trip_start_df = trip_start_df / self.population_scale_factor
        trip_start_df = trip_start_df.reset_index()
        trip_start_df = trip_start_df.rename(columns={"origin_zone": "zone", "start_period": "period"})
        trip_start_df["trip_side"] = "origin"

        # The next part can possibly be removed because we alost always look at only the origin side
        trip_end_df = trip_df.groupby(["destination_zone", "mode", "type", "purpose", "end_period"])[keep_cols].sum()
        trip_end_df = trip_end_df / self.population_scale_factor
        trip_end_df = trip_end_df.reset_index()
        trip_end_df = trip_end_df.rename(columns={"destination_zone": "zone", "end_period": "period"})
    ...
    def metric_freight_distance_distribution(self):
        ...
        for m, t, p, fm in df[["mode", "type", "purpose", "freight_mode"]].drop_duplicates().values:
            ...
            y, x = np.histogram(df_.travel_distance.to_numpy() / 1000, bins=range(0, max_dist))
            dfs.append(pd.DataFrame({"mode": m, "type": t, "purpose": p, "freight_mode": fm, "distance": x[1:], "trips": y}))
