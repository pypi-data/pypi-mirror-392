...
class dctDwt(BaseWatermarkModel):
    """
    The DWT-DCT watermarking algorithm.
    ...
    def __init__(self, bits_len: int, img_size: int, modelname: str = "dctDwt", scales=[0, 36, 36]):
        """
        ...
        self.scales = scales
        self.model = EmbedMaxDct(watermarks=[], wmLen=bits_len, scales=scales, block=4)
        logger.info(f"dctDwt is already!")
    ...
    def embed(self, cover_list: List[Any], secrets: List[List]) -> Result:
        ...
        Args:
            ...
            secrets (List[List]): List of binary watermarks.
        ...
        for cover, secret in zip(cover_list, secrets):
            ...
            self.model = EmbedMaxDct(watermarks=secret, wmLen=len(secret), scales=self.scales, block=4)
