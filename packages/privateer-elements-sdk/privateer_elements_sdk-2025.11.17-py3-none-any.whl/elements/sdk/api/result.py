...
class APIResult:
    ...
    async def iter_arrow_batches(
        ...
        if start_time is not None or end_time is not None:
            start_str = _to_rfc3339(start_time) if start_time is not None else ".."
            end_str = _to_rfc3339(end_time) if end_time is not None else ".."
            params = {"datetime": f"{start_str}/{end_str}"}

        async with httpx.AsyncClient(timeout=self.__httpx_timeout) as client:
            try:
                async with client.stream("GET", feature_url, headers=headers, params=params) as response:
    ...
    async def download_parquet(self, collection_id: str,
                                 dest_path: str,
                                 start_time: Optional[object] = None,
                                 end_time: Optional[object] = None) -> int:
        """
        ...
        writer = None
        try:
            async for batch in self.iter_arrow_batches(collection_id, start_time=start_time, end_time=end_time):
                if batch is None:
                    continue
                if writer is None:
