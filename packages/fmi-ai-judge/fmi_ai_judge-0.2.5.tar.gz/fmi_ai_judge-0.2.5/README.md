...
**Output (checker-accepted):**

- Non-terminal: one line with the move (1-based):  
  `row col`
- Terminal:  
  `-1`
...
The checker first enforces **value optimality** (minimax outcome: +1 win / 0 draw / −1 loss).  
When multiple moves are value-optimal, it applies a **depth-aware tie-break**:

- **Win earlier** (if winning): prefer the move that leads to a win in fewer plies.
- **Lose later** (if losing): prefer the move that prolongs the loss.  
  *(Blocking an opponent’s immediate win is already implied by value optimality.)*

A move that is value-optimal but violates these preferences is graded **WA** with an explanatory note.
...
**Example (O to move; must win immediately at `3 2`):**
