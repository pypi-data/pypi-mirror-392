...
def _ensure_programgarden_finance_stub() -> None:
    ...
    class _DummyLS:
        ...
        def overseas_stock(self) -> "_DummyLS":  # pragma: no cover - no real network access
            return self
...
from programgarden_community.overseas_stock.strategy_conditions.macd_momentum_shift import (  # noqa: E402
    MACDSnapshot,
    StockMACDShift,
)
...
def test_stock_macd_shift_flags_recent_bullish_crossover() -> None:
    candles = _make_bullish_reversal_candles()
    strategy = StockMACDShift()
    ...
    assert response["condition_id"] == StockMACDShift.id
...
def test_stock_macd_shift_handles_recent_bearish_turn() -> None:
    candles = [
    ...
    strategy._load_candles = types.MethodType(_fake_load_candles, strategy)
    strategy._set_symbol({"symbol": "NVDA", "exchcd": "82"})
...
def test_stock_macd_shift_focus_direction_filters_crossovers() -> None:
    strategy = StockMACDShift(focus_direction="bearish")
    ...
    async def _fake_load_candles(self) -> List[Dict[str, float]]:
        return _make_bullish_reversal_candles()

    def _fake_calculate_macd(self, _candles: List[Dict[str, float]]) -> List[MACDSnapshot]:
        return snapshots

    strategy._load_candles = types.MethodType(_fake_load_candles, strategy)
    strategy._calculate_macd = types.MethodType(_fake_calculate_macd, strategy)
    strategy._set_symbol({"symbol": "AAPL", "exchcd": "82"})

    response = asyncio.run(strategy.execute())

    assert response["success"] is False
    assert response["data"]["recent_crossovers"] == []
    assert response["data"]["last_crossover"] == "none"
