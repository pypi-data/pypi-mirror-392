...
class BrowserTests(HTTPTests):
    ...
    def test_FileUpload(self):
        ...
        request.processInputs()
        filename = os.path.basename(request.form['upload'].filename)
        self.assertTrue(filename.endswith('notepad.exe'),
            "Expected filename to end with 'notepad.exe', got: %r" %
            filename)
    ...
    def testForm(self):
        request = self._createRequest()
        publish(request)
        self.assertEqual(request.form, {u'a':u'5', u'b':6})
...
class APITests(BaseTestIPublicationRequest, BaseTestIApplicationRequest,
    BaseTestIPublisherRequest, unittest.TestCase):
