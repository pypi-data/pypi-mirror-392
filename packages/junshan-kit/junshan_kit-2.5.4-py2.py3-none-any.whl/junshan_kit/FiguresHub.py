...
from junshan_kit import ParametersHub, kit
...
def final_plot(model_name, info_dict, cols = 3, svae_path = None):
    # matplotlib settings
    ...
    for data_name, info in info_dict.items():
        for optimizer_name, info_opt in info["optimizer"].items():
            ...
            pkl_path = f'Results_SPBM/seed_{info["seed"]}/{model_name}/{data_name}/{optimizer_name}/train_{info["train_test"][0]}_test_{info["train_test"][1]}/Batch_size_{info["batch_size"]}/epoch_{info["epochs"]}/{info_opt["ID"]}/Results_{ParametersHub.model_abbr(model_name)}_{data_name}_{optimizer_name}.pkl'
    ...
    for idx, (data_name, info) in enumerate(draw_data.items()):
        ...
        for optimizer_name, metric_list in info.items():
            ax.plot(metric_list, label=optimizer_name)
