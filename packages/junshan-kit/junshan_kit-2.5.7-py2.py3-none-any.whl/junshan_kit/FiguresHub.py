...
def final_plot(model_name, info_dict, Exp_name = "SPBM", cols = 3, svae_path = None):
    # matplotlib settings
    ...
    for data_name, info in info_dict.items():
        for optimizer_name, info_opt in info["optimizer"].items():
            ...
            pkl_path = f'Results_{Exp_name}/seed_{info["seed"]}/{model_name}/{data_name}/{optimizer_name}/train_{info["train_test"][0]}_test_{info["train_test"][1]}/Batch_size_{info["batch_size"]}/epoch_{info["epochs"]}/{info_opt["ID"]}/Results_{ParametersHub.model_abbr(model_name)}_{data_name}_{optimizer_name}.pkl'
