...
{Fore.rgb(55,191,78)}HFL:{Fore.rgb(55,130,191)}{lineTotal()}{Fore.light_red}{Fore.light_green}{Back.grey_15}'''
    ...
    cmd_len={len(cmd)}{Style.reset}'''
                        ...
                        hw_in_use=bool(db.detectGetOrSet("HW_IN_USE",False,setValue=False,literal=False))
                        ml_in_use=bool(db.detectGetOrSet("ML_IN_USE",False,setValue=False,literal=False))
                        if not hw_in_use:
                            if cmd.startswith(ml_delim) and not cmd.endswith(ml_delim):
                                ml_in_use=bool(db.detectGetOrSet("ML_IN_USE",True,setValue=True,literal=False))
                                msg=f'''
        ...
        '''
                                ...
                                buffer=[]
                                ml_in_use=bool(db.detectGetOrSet("ML_IN_USE",False,setValue=True,literal=False))
        ...
        CMD's are not final until ended with {Fore.magenta}{ml_delim}{Style.reset}""")
                        ...
                        #multiline end#
                        ignore_hw_delim=bool(db.detectGetOrSet('IgnoreHW_DELIM',False,setValue=False,literal=False))
                        hw_delim=str(detectGetOrSet('HW_DELIM',str(Path('#hw#'))))
                        if not ml_in_use:
                            if not ignore_hw_delim:
            ...
            CMD's are not final until ended with {Fore.magenta}{hw_delim}{Style.reset}""")
                                ...
                                #print(cmd,len(hw_delim))
                                if cmd.startswith(hw_delim):
                                    cmd=cmd[len(hw_delim):]
                                #print(cmd)
                                if cmd.endswith(hw_delim):
                                    cmd=cmd[:len(hw_delim)*-1]
                                #print(cmd)
...
with some options:
    ...
    4.) [PBUC,PurchasedBaseUnitCharge] Charge by the Unit purchased 1 Reem of 500 Paper = 1, I need 1.001 of 1, then charge for 2, but don't give the remainder to the customer; ensure the customer is aware of the Purchased Base Unit Charge'''
        ...
        degress fahrenheite - degF
                        ...
                        elif cmd.lower() in ['ignore hw delim','ignr hw delim','ighwdlm','ig hw dlm']:
                            ignore_hw_delim=bool(db.detectGetOrSet('IgnoreHW_DELIM',False,setValue=False))
                            print(f"Ignore HW Delimiter Was {Fore.orange_red_1}{ignore_hw_delim}{Style.reset}")
                            ignore_hw_delim=bool(db.detectGetOrSet('IgnoreHW_DELIM',not ignore_hw_delim,setValue=True))
                            print(f"Ignore HW Delimiter is Now {Fore.orange_red_1}{ignore_hw_delim}{Style.reset}")
