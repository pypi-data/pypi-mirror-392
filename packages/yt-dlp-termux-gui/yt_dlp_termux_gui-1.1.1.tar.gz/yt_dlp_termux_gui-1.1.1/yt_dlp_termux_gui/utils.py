...
def ensure_curl_impersonate(force: bool = False):
    ...
    try:
        os.rmdir(f"{install_dir}/.git")
        os.remove(f"{install_dir}/README.md")
        run_cli_command_simple(["cp", "-rf", ".", constants.TERMUX_PREFIX], cwd=install_dir)
        os.rmdir(install_dir)
    except Exception as e:
