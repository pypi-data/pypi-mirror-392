...
def handle_commands(commands: list[str], options: list[str]):
    ...
    if commands.count("deps"):
        ...
        print(
            "\n".join([
                "\nPlease make sure the plugins 'Termux:GUI' and 'Termux:Widget' were installed the same way as Termux (ignore if already installed):",
        ...
        if ensure_termux_widget():
            logging.info("Script for Termux:Widget has been created.")
        else:
            logging.info("Script for Termux:Widget already exists.")
