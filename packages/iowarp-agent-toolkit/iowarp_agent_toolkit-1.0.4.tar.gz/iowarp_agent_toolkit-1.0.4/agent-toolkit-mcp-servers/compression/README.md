...
```bash
uvx iowarp-agent-toolkit mcp-server compression
```
...
```json
{
  "mcpServers": {
    "compression-mcp": {
      "command": "uvx",
      "args": ["iowarp-agent-toolkit", "mcp-server", "compression"]
    }
  }
}
```
...
```sh
claude mcp add compression-mcp -- uvx iowarp-agent-toolkit mcp-server compression
```
