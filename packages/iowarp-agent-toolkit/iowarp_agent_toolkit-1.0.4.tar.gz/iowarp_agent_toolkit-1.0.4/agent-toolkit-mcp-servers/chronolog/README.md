...
```bash
uvx iowarp-agent-toolkit mcp-server chronolog
```
...
```json
{
  "mcpServers": {
    "chronolog-mcp": {
      "command": "uvx",
      "args": ["iowarp-agent-toolkit", "mcp-server", "chronolog"]
    }
  }
}
```
...
```sh
claude mcp add chronolog-mcp -- uvx iowarp-agent-toolkit mcp-server chronolog
```
