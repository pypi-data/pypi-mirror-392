...
```bash
uvx iowarp-agent-toolkit mcp-server parallel-sort
```
...
```json
{
  "mcpServers": {
    "parallel-sort-mcp": {
      "command": "uvx",
      "args": ["iowarp-agent-toolkit", "mcp-server", "parallel-sort"]
    }
  }
}
```
...
```sh
claude mcp add parallel-sort-mcp -- uvx iowarp-agent-toolkit mcp-server parallel-sort
```
