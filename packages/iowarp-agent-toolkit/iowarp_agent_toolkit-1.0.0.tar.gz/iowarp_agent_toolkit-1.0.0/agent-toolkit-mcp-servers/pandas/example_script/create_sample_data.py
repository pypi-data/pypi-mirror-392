...
def create_sample_datasets():
    ...
    employee_data = {
        ...
        "hire_date": pd.date_range("2015-01-01", periods=n_employees, freq="D")[
            :n_employees
        ],
    }
    ...
    weather_data = {
        ...
        "temperature": (
            20
            + 15 * np.sin(2 * np.pi * day_of_year / 365)
    ...
    inventory_data = {
        ...
        "last_updated": pd.date_range("2023-12-01", periods=n_products, freq="H")[
            :n_products
        ],
    }
    ...
    inventory_df["selling_price"] = (
        inventory_df["cost_price"] * np.random.uniform(1.2, 2.5, n_products)
    ).round(2)
...
def main():
    ...
    print(
        f"Total files created: {sum(len(formats) for formats in saved_files.values())}"
    )
