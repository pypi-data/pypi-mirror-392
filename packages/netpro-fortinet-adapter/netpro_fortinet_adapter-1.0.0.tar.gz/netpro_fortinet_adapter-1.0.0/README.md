...
**Fortinet appliances/devices – currently FortiGate firewalls running FortiOS** –  
...
NetPro’s built-in adapter framework.
...
- Shared **base adapter**: `BaseFortigateAPIAdapter`
  ...
  - **Backward compatible with 7.0 → 7.4** through conditional endpoint maps,
    feature detection, and compatibility shims.
  - Inherits from NetPro Core’s `APIAdapterBase`
...
> but includes internal logic that automatically adapts to 7.0–7.4 API paths  
...
After installation, the adapter registers itself into NetPro’s global adapter registry.
