...
for cluster_name, cluster_model in cluster_algorithms.items():
    ...
    for sequence_composition in sequences_compositions:
        ...
        X_ordered, y_ordered = context_sorter.transform(X_full.copy(), y_full.copy())
        ################


        splitter_tt = PastFutureSplit(proportion=0.75, timestamp_column=timestamp_column)
        splitter_tt.fit(X_ordered, y_ordered)
               
        X_train_full, y_train_full, X_test, y_test = splitter_tt.transform(X_ordered, y_ordered)
        ...
        for sequence_length in sequence_lengths:
            for i in range(iterations):
                ...
                model_LSTM = LSTM(validation_data=validation_data, target_column=target_column, seq_length = sequence_length, numeric_features= [col for col in X_train.columns if (col != timestamp_column and col != 'GROUP')],context_feature=['GROUP'],random_seed=None,device=device)
