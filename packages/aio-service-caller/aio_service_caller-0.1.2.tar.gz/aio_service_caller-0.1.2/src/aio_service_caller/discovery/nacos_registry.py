...
class NacosServiceRegistry(IServiceRegistry):
    ...
    async def create(
        ...
        **kwargs
        ...
        # 创建命名服务客户端
        naming_service: NacosNamingService = await RegistryThreadLoop().get_instance().run(naming_service_coro)
        return cls(naming_service, group_name)
    ...
    async def register(
        ...
        try:
            ...
            response = await RegistryThreadLoop().get_instance().run(self._client.register_instance(param))
            if response:
    ...
    async def deregister(
        ...
        try:
            ...
            response = await RegistryThreadLoop().get_instance().run(response_coro)
    ...
    async def close(self):
        ...
        try:
            if self._client:
                await RegistryThreadLoop().get_instance().run(self._client.shutdown())
