...
# Wishlist
# - Check inputs for inconsistencies or errors
# - Check returns
# - Provide aliases, e.g., stoichiometry for stoich
...
class ReactionSystem:
    ...
    Args:
        ...
        stoich: Reaction stoichiometry. Reactants get negative coefficients,
            whereas products get positive coefficients.
    ...
    def __init__(self, component_ids, component_names, axes,
                 reactions, stoich, kinetics, inequality,
    ...
    def rate(self, x):
        ...
        if not np.issubdtype(x.dtype, np.number):
            raise TypeError('x must be an array with integer or float elements')
        if min(x.shape) < 2:
    ...
    def plot_rate_field(
        self,
        n_vec=51, n_vec_h=None, n_vec_v=None, inequality_tol=1e-6,
        ...
        Args:
            ...
            font: Main font used for axis labels, etc. Defaults to 12.
        ...
        for i in range(np.size(xx, 0)):
            for j in range(np.size(yy, 1)):
                ...
                if np.size(self.inequality_status([0, 0])) == 1:
                    if np.any(inequalities != 0):
                        if inequalities[i, j] > -inequality_tol:
                            uu[i, j] = np.nan
                            vv[i, j] = np.nan
                else:
                    for k in range(np.size(self.inequality_status([0, 0]))):
                        if inequalities[k, i, j] > 0:
                            uu[i, j] = np.nan
                            vv[i, j] = np.nan
        ...
        ax.quiver(X, Y, un, vn, color='#888888', angles='xy',
