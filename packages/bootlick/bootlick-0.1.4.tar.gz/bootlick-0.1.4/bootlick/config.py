    ...
    def _load_config(self, config_file):
        ...
        with open(config_file, "r") as file:
            try:
                ...
                get_bootlick_logger().log_info(f"Configuration loaded from {config_file}", {
                    "config_file": config_file
                })
            except Exception as e:
                get_bootlick_logger().log_error(f"Failed to load configuration", e, {
                    "config_file": config_file
                })
                raise
...
def load_config(default_config_path, force_reload=False):
    ...
    if _config_instance is None or force_reload:
        ...
        get_bootlick_logger().log_warning(f"Config loaded from file {_config_instance.config_path}", {
            "config_path": _config_instance.config_path
        })
    else:
        get_bootlick_logger().log_info("Configuration is already loaded. Skipping reload.")
