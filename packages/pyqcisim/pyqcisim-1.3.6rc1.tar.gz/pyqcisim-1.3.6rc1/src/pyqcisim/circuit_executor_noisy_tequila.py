...
class CircuitExecutorTequilaNoise(CircuitExecutorTequila):
    def list_num(self, num):
        return [int(i) for i in list(bin(num)[2:].zfill(len(self._names)))]

    def execute(self, instructions, mode, num_shots, noise_config):
        ...
        q.add_noise(noise_config[0], noise_config[1], noise_config[2])
        ...
        if mode == "final_result":
            ...
            result["quantum"] = (self._names, q.mat())
            return result

        if mode == "state_vector":
            return (self._names, q.mat())
        if mode == "probability":
            msmt_qubit_indices = [i for i in range(len(self._names))]
            p0_for_each_qubit = [
                q.get_prob(msmt_qubit_indices, self.list_num(i))
                for i in range(2 ** len(self._names))
            ]
