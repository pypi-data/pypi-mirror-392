...
def get_ms_pointing(ms_path: str) -> SkyCoord:
    """
    Return the MS pointing centre as a SkyCoord (ICRS),
    read from FIELD::PHASE_DIR (stored in radians).
    """
    # Both '::FIELD' and '/FIELD' work with casacore; keep your style.
    with tables.table(f"{ms_path}/FIELD", ack=False) as t:
        # PHASE_DIR shape: (nfield, ntime, 2) or (nfield, 2)
        pd = t.getcol("PHASE_DIR")[0]

    # Normalize shape and extract RA/Dec in radians
    if pd.ndim == 1:           # (2,)
        ra_rad, dec_rad = pd[0], pd[1]
    else:                      # (ntime, 2)
        ra_rad, dec_rad = pd[0, 0], pd[0, 1]

    return SkyCoord(ra=ra_rad * u.rad, dec=dec_rad * u.rad, frame="icrs")


def get_mean_altitude_for_ms(ms_path: str, location) -> float:
    """
    Return the mean altitude (deg) at the MS midpoint time for its pointing centre.
    TIME in MS is in seconds; convert to MJD days for astropy Time.
    """
    src = get_ms_pointing(ms_path)

    # Read first/last TIME (seconds since MJD 0) and take midpoint
    with tables.table(ms_path, ack=False) as ct:
        t0 = ct.getcell("TIME", 0)
        tn = ct.getcell("TIME", ct.nrows() - 1)

    t_mid_mjd = 0.5 * (t0 + tn) / 86400.0
    obstime = Time(t_mid_mjd, format="mjd", scale="utc")

    frame = AltAz(obstime=obstime, location=location)
    return float(src.transform_to(frame).alt.deg)
