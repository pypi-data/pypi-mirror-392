...
class Trajectory:
    ...
    def get_cellboundary_library(self,frames=None,indcells=None,secretion_rates=None,cell_states=None,state_ids=None,surface_fmask_channels=[0],surface_states_baseid=None,border_resolution=1.0,vdist_scale=.3,visual=False,save_h5=False,overwrite=False,**border_property_args):
        """
        ...
        **border_property_args
        ...
        if state_labels is None:
            state_labels=[None]*(np.max(state_ids))+1
            for istate in range((np.max(state_ids))+1):
                state_labels[istate]=f'state {istate}'
        global_index=[]
        states=[]
        pts=[]
        dx_prevs=[]
