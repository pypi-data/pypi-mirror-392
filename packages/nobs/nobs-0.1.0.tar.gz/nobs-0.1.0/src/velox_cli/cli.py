...
from velox_cli.models import Job, NetworkApp, Project, Subscriber, VeloxClient, VeloxServer
from velox_cli.runners import default_runner
...
async def process_queue(name: str) -> None:
    ...
    from velox_cli.models import QueueMessage
...
@cli.command()
@async_
async def build_image(ref: str | None) -> None:
    from pathlib import Path 
    import sys
    ...
    platform = "linux/amd64"
    reg_url = f"scw.rg.com/{project.name}"


    "rg.fr-par.scw.cloud/aligned-registry-test/velox:latest"

    _ = default_runner([
        "docker", "build", "Dockerfile", f"-t={reg_url}", f"--platform={platform}"
    ])
