...
if hasattr(np.lib, "_format_impl"):
    # numpy>=2.3.0
...
1. start: 开始位置
2. end: 结束位置
3. update_timestamp: 修改时间，用来判读是否要读取，用来解决end不变的情况
4. offset: 数据区开始位置，方便其他语言快速定位并写入
5. magic: 魔术数，用来判断是否`NPYT`格式文件
...
if hasattr(np.lib, "_format_impl"):
    # numpy>=2.3.0
    np.lib._format_impl.descr_to_dtype = _descr_to_dtype
# 函数替换
...
def write_footer(fp, dtype: np.dtype, shape: tuple, start: int, end: int, offset: int) -> None:
    ...
    # 写入尾巴
    fp.write(np.array([int(start), int(end), time.time_ns(), offset, _MAGIC_NUMBER_], dtype=np.uint64).tobytes())
