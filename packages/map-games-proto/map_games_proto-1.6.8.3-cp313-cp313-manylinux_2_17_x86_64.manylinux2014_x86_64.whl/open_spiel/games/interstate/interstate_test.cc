...
void BasicTicTacToeTests() {
  testing::LoadGameTest("interstate");
  testing::NoChanceOutcomesTest(*LoadGame("interstate"));
  testing::RandomSimTest(*LoadGame("interstate"), 100);
}

void TestStateStruct() {
  auto game = LoadGame("interstate");
  auto state = game->NewInitialState();
  InterstateState* ttt_state = static_cast<InterstateState*>(state.get());
  ...
  SPIEL_CHECK_EQ(nlohmann::json::parse(state_json).dump(),
                 InterstateStateStruct(state_json).ToJson());
}
...
int main(int argc, char** argv) {
  open_spiel::interstate::BasicTicTacToeTests();
  open_spiel::interstate::TestStateStruct();
}
