...
```
    ...
    def test_pydantic_custom_validator_executed(self):
        """Pydantic custom validators are executed (not bypassed)."""
        ...
        valid_response = '<json>{"name": "Widget", "status": "active"}</json>'
        product, error = to_data(valid_response, Product)
        assert error is None
        assert product.status == "active"

        # Invalid status should fail with custom validator error
        invalid_response = '<json>{"name": "Widget", "status": "invalid"}</json>'
        product, error = to_data(invalid_response, Product)
        assert product is None
        assert error is not None
        assert "status" in error.lower()

    def test_msgspec_struct_validation(self):
        """msgspec Struct validation works correctly."""
        import msgspec
        ...
        valid_response = '<json>{"name": "Alice", "age": 30}</json>'
        user, error = to_data(valid_response, User)
        assert error is None
        ...
        invalid_response = '<json>{"name": "Bob", "age": "not-a-number"}</json>'
        user, error = to_data(invalid_response, User)
        assert user is None
        assert error is not None
