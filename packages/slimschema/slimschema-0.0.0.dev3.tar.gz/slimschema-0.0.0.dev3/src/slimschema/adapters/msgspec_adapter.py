...
def _msgspec_annotation_to_slimschema(annotation) -> tuple[str, str | None]:
    ...
    if origin is set:
        ...
        if args:
            inner = _msgspec_annotation_to_slimschema(args[0])
            return f"[{inner[0]}]", f"Set[{inner[1] or inner[0]}]"
        return "[]", "Set[str]"
    ...
    if origin is frozenset:
        ...
        if args:
            inner = _msgspec_annotation_to_slimschema(args[0])
            return f"[{inner[0]}]", f"FrozenSet[{inner[1] or inner[0]}]"
    ...
    if origin is tuple:
        ...
        if len(args) == 2 and args[1] is Ellipsis:
            inner = _msgspec_annotation_to_slimschema(args[0])
            ann_inner = inner[1] or inner[0]
            return f"[{inner[0]}]", f"Tuple[{ann_inner}, ...]"
        ...
        for arg in args:
            ...
            tuple_ann.append(inner[1] or inner[0])

        if tuple_parts:
            return f"[{tuple_parts[0]}]", f"Tuple[{', '.join(tuple_ann)}]"
