...
from slimschema.prompt import to_prompt, to_prompt_compact
...
class TestCompactPrompt:
    """Test to_prompt_compact() convenience function."""

    def test_compact_prompt_basic(self):
        """Compact prompt generates with defaults."""
        schema_str = "name: str\nage: int"

        prompt = to_prompt_compact(schema_str)

        assert "Follow this schema:" in prompt
        assert "<output>" in prompt
        assert "```json" in prompt

    def test_compact_prompt_with_errors(self):
        """Compact prompt supports errors parameter."""
        schema_str = "name: str"
        error_msg = "Missing field: name"

        prompt = to_prompt_compact(schema_str, errors=error_msg)

        assert error_msg in prompt

    def test_compact_prompt_custom_xml_tag(self):
        """Compact prompt allows custom xml_tag."""
        schema_str = "name: str"

        prompt = to_prompt_compact(schema_str, xml_tag="data")

        assert "<data>" in prompt

    def test_compact_prompt_custom_fence(self):
        """Compact prompt allows custom fence_language."""
        schema_str = "name: str"

        prompt = to_prompt_compact(schema_str, fence_language="yaml")
