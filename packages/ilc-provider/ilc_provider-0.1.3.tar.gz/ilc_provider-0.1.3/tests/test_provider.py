...
class TestLeague:
    ...
    def test_no_team_has_more_than_one_match_per_day(self):
        league = fake.league(team_count=8)
        team = random.choice(league.teams)
        dates = set()
        for match in league.matches(team=team):
            assert match.date not in dates
