...
class Circuit:
    ...
    ##  SECTION：----执行线路并返回错误率----
    def experiment(self,p_noise,p_measure, sample_number: int, method: str,predict_time=False,strength=50):
        ...
        if method=='bposd':
            decoder=stimbposd.bp_osd.BPOSD(model=dem, bp_method='min_sum', max_bp_iters=strength)
        elif method=='tesseract':
            config=tesseract.TesseractConfig(dem=dem, det_beam=strength)
