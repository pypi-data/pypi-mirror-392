...
230 PRINTLC$;ERAS$;CN$"Difficulty (1 easy, 10 hard)? ";:LINE INPUT IN$
240 PRINTCO$
250 TT=(VAL(IN$)/10):PRINTGRA$
260 IF TT>1ORTT<=0THEN230
270 PRINTERAS$:A=TT:IF RND(1)<0.5THENF=1
...
450 IF LEN(IN$)>2THENIN$=MID$(IN$,2):GOTO470
460 GOSUB2220:PRINTA$;CN$;"   MOVE";I;"? ";:GOSUB3070:PRINT:PRINT:IFIN$="" THEN1000
470 IF LEN(IN$)>1THEN520
480 IFIN$>="A" ANDIN$<="F" THENIN$="="+IN$
490 IFIN$>="S" ANDIN$<="X" THENIN$=IN$+"-"
500 IF LEN(IN$)>1THEN520
510 PRINT"BAD MOVE.":GOTO440
520 IF LEN(IN$)>3THEN510
530 IF MID$(IN$,2,1)="," THENIN$=LEFT$(IN$,1)+MID$(IN$,3):GOTO530
540 F$=LEFT$(IN$,1)
...
610 IFFM=25ANDB(25)=0THEN PRINT"YOU HAVE NO MEN ON THE BAR.":GOTO440
620 IFFM<TM THEN PRINT"WRONG DIRECTION.":GOTO440
630 IFB(25)<0ANDFM<>25THEN PRINT"YOU HAVE MEN ON THE BAR.":GOTO440
640 IFB(FM)>=0THEN PRINT"YOU HAVE NO MEN ON ";F$;".":GOTO440
650 IFTM=0THEN870
660 L=0:FORJ=1TOTU:IFUS(J) THENK=K+1:GOTO690
670 L=L+1:N=N+D(J):IFN=M THEN700
680 IFD(J)=M THENK=J:L=1:GOTO700
690 NEXTJ:PRINT"YOU CAN'T MOVE";M;".":GOTO440
700 IFB(TM)>1THEN PRINT"POINT ";T$;" IS BLOCKED.":GOTO440
710 IFL=1THEN GOSUB2880:US(K)=1:GOTO1200
...
850 I=I-1:GOTO1200
860 NEXTK
870 FORJ=7TO25:IFB(J)<0THEN PRINT"YOU CAN'T CHEAT!!":GOTO440
880 NEXTJ
890 TM=26:FORJ=1TOTU:IFUS(J)=0ANDFM=D(J) THEN GOSUB2880:US(J)=1:GOTO1200
900 NEXTJ
910 IFFM=6THEN950
920 FORJ=6TOFM+1STEP-1
930 IFB(J)<0THEN PRINT"YOU CAN'T BEAR OFF FROM ";F$;".":GOTO440
940 NEXTJ
950 K=0:D(K)=0:FORJ=1TOTU:IFUS(J)=0ANDD(J)>FM ANDD(J)>D(K) THENK=J
960 NEXTJ
970 IFK=0THEN PRINT"YOU CAN'T MOVE";FM;"=.":GOTO440
...
1040 NEXTJ:GOTO1210
1050 FORJ=24TO2 STEP-1
1060 IFB(J)>=0THEN1090
1070 IFJ-D(TU-1)>0THEN IFUS(TU-1)=0ANDB(J-D(TU-1))<2THEN1190
1080 IFJ-D(TU)>0THEN IFUS(TU)=0ANDB(J-D(TU))<2THEN1190
1090 NEXTJ
1100 FORJ=24TO7 STEP-1:IFB(J)<0THEN1210
1110 NEXTJ
...
1200 I=I+1:IFI<=TU THEN450
1210 NP=1:GOTO2660
1220 MO=0:B9=0
1230 XC=XC+Y
1240 K=0
1250 K=K+1:IFB(0)=0ORK>TU THEN1310
...
1300 GOTO1250
1310 K=0
1320 K=K+1:IFB(0)=0ORK>TU THEN1380
1330 IFB(D(K))<0ORUS(K)>0THEN1320
1340 US(K)=1:MO=MO+1
1350 FM=0:TM=D(K):GOSUB2880
1360 PRINT"OFF BAR TO ";MID$(AL$,TM,1)
1370 GOTO1320
1380 IFB(0)=0ANDMO<TU THEN1410
1390 IFMO<TU THEN FORI=1TO500:NEXT:PRINTA$;"CAN'T GET OFF BAR":FORI=1TO500:NEXT
1400 GOTO1990
1410 IFCM>6THEN1600
...
1570 IFTU=MO THEN1990
1580 NEXTJ
1590 GOTO1990
1600 IFTU=MO THEN1990
...
1730 PRINT MID$(AL$,J1,1);" AND ";MID$(AL$,J2,1):MO=MO+2:HB=HB-1
...
1830 G=0:IFBS>S1+S2 THENY=TM:TM=FM+D(Y):GOTO1860
1840 IFS2>S1 THENFM=F2:Y=2:TM=F2+D(Y):BM=M2:GOTO1860
1850 FM=F1:Y=1:TM=F1+D(Y):BM=M1
1860 IFBM<>4THEN1920
1870 IFF<>1THENF=1:GOTO1790
1880 IFTRY<TU+4THEN1970
1890 FORJ=1TO18:IFB(J)>0ORB9 THEN PRINTM$(4);:GOTO1990
1900 NEXTJ:B9=1:GOTO1420
1910 GOTO1970
1920 IFBM=1THENHB=HB-1
1930 MO=MO+1:US(Y)=1:IFHM<24-CM THENBM=3:A=0
1940 GOSUB2880
1950 PRINTM$(BM);MID$(AL$,TM,1);FR$;MID$(AL$,FM,1)
1960 IFMO=TU THEN1990
1970 Y=Y+1:IFY>TU THENY=1
1980 GOTO1780
1990 NP=-1
2000 GOTO2660
...
2060 PRINT TAB(39);ESC$;"p";"  A B C D E F  =  G H I J K L  "RVO$
2070 FORI=1TO8
2080 PRINT TAB(39);GR$"i `   `   `   iii `   `   `   i":NEXT
2090 FORI=1TO2
2100 PRINT TAB(39);GR$"i             iii             i"LC$:NEXT
2110 FORI=1TO8
2120 PRINT TAB(39);GR$; "i   `   `   ` iii   `   `   ` i"LC$:NEXT
2130 PRINT TAB(39);ESC$"p  X W V U T S     R Q P O N M  "RVO$
2140 GOTO3450
2150 PRINT RIGHT$(" "+STR$(13-I),2);"&";TAB(21);"&";MID$(STR$(I+12),2):RETURN
...
2200 B(I)=0:IFN<>0THEN FORJ=1TO ABS(N):GOSUB2880:NEXTJ
2210 NEXTI
2220 PRINTA$;
...
2300 FORI=K TOJ6:IFB(I)<0THENPP=PP+P(I-K)
...
2460 FORI=K TOJ:IFB(I)=-1THENSM=SM+P(I-K)*(25-I)
2470 NEXTI:CS=CS-CW+(1-PZ)*SM
2480 RETURN
2490 BS=-999:BM=4:FM=0:TM=0:TRY=TRY+1
2500 FORL=24TO1 STEP-1:IFB(L)<1THEN2650
2510 RL=D(Y):R=L+RL:IFR>24THEN2650
2520 IFR<F ANDB(R)<>-1THEN2650
...
2860 IF LEFT$(X$,1)="Y" THEN220
2870 PRINTCN$:END
2880 PT=FM:GOSUB2980:P=SGN(B(FM)):GOSUB2980:B(FM)=B(FM)-P
2890 IFFM=25ORFM=0THEN PRINTGR$"i";LC$:GOTO2910
2900 IFFM<26THEN PRINTGR$;MID$(" `",2+((1ANDPT)=1OR ABS(B(PT))>8),1);LC$
2910 IFB(TM)<>-P THEN2940
2920 PT=-25*(P>0):B(PT)=B(PT)-P:GOSUB2980:B(TM)=0
2930 IFP=-1THEN PRINTDA$(3):ELSE PRINTDA$(1)
2940 B(TM)=B(TM)+P:PT=TM:IFTM<26THEN GOSUB2980:PRINTDA$(2+P)
2950 IFMV<=-1THENMV=0
2960 PRINTA$:MV=MV+1
2970 RETURN
2980 IFPT>25THEN RETURN
...
3080 PRINTA$;CO$;
3090 RETURN
3100 SI=INP(&O351):LN=SI AND&O376:OUT&O351,LN
3110 GOSUB2220
3120 PRINTA$;"  ";"Press any key to roll."
3130 REM
3140 GOSUB3350
3150 IF INP(&O350)=13THEN3130
...
3420 PRINT TAB(2*J+7);"} "+DI$(I+T(J),D(J))+" |  ";:NEXTJ,I
3430 PRINT CHR$(27);"G";:PRINT
3440 RETURN
3450 PRINTH$;E$+"F";:PRINT TAB(9);"   {{{{{    {{{{{    "
3460 FORI=1TO3:PRINT TAB(9);"}     |  }     |"
3470 NEXTI
3480 PRINT TAB(10)"zzzzz    zzzzz"
3490 PRINTLC$
3500 RETURN
