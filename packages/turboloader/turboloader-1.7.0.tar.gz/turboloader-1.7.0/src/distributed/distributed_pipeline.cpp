...
DistributedPipeline::DistributedPipeline(const DistributedConfig& config)
    ...
    UnifiedPipelineConfig pipeline_config;
    pipeline_config.data_path = config_.data_path;
    pipeline_config.batch_size = config_.batch_size;
    pipeline_config.num_workers = config_.num_workers;
    pipeline_config.queue_size = config_.queue_size;
    pipeline_config.shuffle = config_.shuffle;

    pipeline_ = std::make_unique<UnifiedPipeline>(pipeline_config);
...
UnifiedBatch DistributedPipeline::next_batch() {
    if (!pipeline_) {
        return UnifiedBatch();
    }
