...
def test_move_from_current_to_new_worktree() -> None:
    ...
    with pure_workstack_env(runner) as env:
        source_wt = env.cwd
        target_wt = env.workstacks_root / env.cwd.name / "target-wt"
...
def test_move_with_branch_flag_auto_detect() -> None:
    ...
    runner = CliRunner()
    with pure_workstack_env(runner) as env:
        source_wt = env.workstacks_root / env.cwd.name / "old-wt"
        ...
        test_ctx = env.build_context(git_ops=git_ops, existing_paths={source_wt})
...
def test_move_with_worktree_flag() -> None:
    ...
    runner = CliRunner()
    with pure_workstack_env(runner) as env:
        source_wt = env.workstacks_root / env.cwd.name / "source-wt"
...
def test_move_swap_between_two_worktrees() -> None:
    ...
    runner = CliRunner()
    with pure_workstack_env(runner) as env:
        source_wt = env.workstacks_root / env.cwd.name / "wt1"
        target_wt = env.workstacks_root / env.cwd.name / "wt2"
        ...
        test_ctx = env.build_context(git_ops=git_ops, existing_paths={source_wt, target_wt})
...
def test_move_error_source_and_target_same() -> None:
    ...
    runner = CliRunner()
    with pure_workstack_env(runner) as env:
        wt = env.workstacks_root / env.cwd.name / "same-wt"
        ...
        test_ctx = env.build_context(git_ops=git_ops, existing_paths={wt})
...
def test_move_to_existing_worktree_in_detached_head() -> None:
    ...
    runner = CliRunner()
    with pure_workstack_env(runner) as env:
        source_wt = env.workstacks_root / env.cwd.name / "source"
        target_wt = env.workstacks_root / env.cwd.name / "target"
...
def test_move_to_root() -> None:
    ...
    runner = CliRunner()
    with pure_workstack_env(runner) as env:
        source_wt = env.workstacks_root / env.cwd.name / "feature-wt"
...
def test_move_to_root_with_explicit_current() -> None:
    ...
    with pure_workstack_env(runner) as env:
        ...
        test_ctx = env.build_context(git_ops=git_ops, cwd=source_wt, existing_paths={source_wt})
