...
from maleo.schemas.mixins.filter import ListOfDateFilters, convert as convert_filter
...
from maleo.schemas.mixins.sort import ListOfSortColumns, convert as convert_sort
...
from maleo.schemas.pagination import Limit
...
from maleo.types.uuid import OptUUID, OptListOfUUIDs
...
class CreateParameter(Name[str]):
    pass


class BaseInsertData(UUID[OptUUID]):
    uuid: Annotated[OptUUID, Field(None, description="UUID")]
    key: Annotated[str, Field(..., description="Key")]


class InsertData(Name, BaseInsertData):
    pass
...
class DeleteSingleParameter(BaseDeleteSingleParameter[ClientIdentifier]):
    ...
    @classmethod
    def new(
        cls, identifier_type: Literal[IdentifierType.UUID], identifier_value: PythonUUID
    ) -> "DeleteSingleParameter": ...
    @overload
    @classmethod
    def new(
