...
from universal_mcp.agentr import AgentrRegistry, AgentBuilderRegistry
...
class CodeActPlaybookAgent(BaseAgent):
    def __init__(
        ...
        agent_builder_registry: AgentBuilderRegistry | None = None,
        ...
        **kwargs,
        ...
        self.agent_builder_model_instance = load_chat_model(
            "anthropic:claude-sonnet-4-5-20250929", thinking=False, disable_streaming=True, tags=("quiet",), temperature=0.3
        )
        self.registry = registry
        self.agent_builder_registry = agent_builder_registry
        self.agent = self.agent_builder_registry.get_agent() if self.agent_builder_registry else None
        ...
        async def call_model(state: CodeActState) -> Command[Literal["execute_tools"]]:
            ...
            if isinstance(self.model_instance, ChatAnthropic):
                if isinstance(messages[-1].content, str):
                    ...
                    messages[-1] = last
            response = cast(AIMessage, await self.model_with_tools.with_retry().ainvoke(messages))
