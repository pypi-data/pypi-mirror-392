...
def _find_missing_orders(payments: dict, orders: set):
    missing_orders = []
    for order, payment in payments.items():
...
def reconcile(payments_dir: str):
    ...
    missing_orders = _find_missing_orders(
        payments, set(df["Nom de la commande"].unique())
    )
    missing_payment_df = pd.DataFrame(missing_orders)
    if not missing_payment_df.empty:
        missing_payment_df.to_excel("Commandeds-manquantes.xlsx", index=False)
        print(
            f"{len(missing_orders)} commandes ont un paiement associé mais sont absentes du tableau des ventes. "
            + "Elles sont sauvegardées dans 'Commandeds-manquantes.xlsx'"
        )
