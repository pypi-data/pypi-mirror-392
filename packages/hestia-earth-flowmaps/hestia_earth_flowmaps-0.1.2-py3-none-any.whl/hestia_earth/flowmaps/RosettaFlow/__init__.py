...
def _get_reverse_column(column: str):
    return next((v for k, v in _FLOW_REVERSE_COLUMNS if k == column), None)
...
class FlowMap:
    ...
    def map_flow(
        ...
        if check_reverse:
            ...
            if not results.empty:
                ...
                results["ConversionFactor"] = results["ConversionFactor"].apply(
                    lambda x: 1 / x
                )  # flip ratio

                results["MatchCondition"] = results["MatchCondition"].apply(
    ...
    def map_flow_query(
        ...
        results = self.all_flow_mappings.loc[
            self.all_flow_mappings["SourceFlowUUID"] == flow_query.SourceFlowUUID
        ]
        if not results.empty and flow_query.TargetListNames:
            results = results.loc[
                results["TargetListName"].isin(flow_query.TargetListNames)
            ]

        if not results.empty and flow_query.TargetFlowContexts:
            results = results.loc[
                results["TargetFlowContext"].isin(flow_query.TargetFlowContexts)
            ]
        ...
        if check_reverse:  # todo merge reverse results with normal results
            ...
            if not results.empty and flow_query.TargetListNames:
                results = results.loc[
                    results["SourceListName"].isin(flow_query.TargetListNames)
                ]
            ...
            if not results.empty:
                ...
                if not results.empty and flow_query.SourceFlowContexts:
                    if isinstance(flow_query.SourceFlowContexts, list):
                        results = results.loc[
                            results["TargetFlowContext"].isin(
                                flow_query.SourceFlowContexts
                            )
                        ]
                    else:
                        results = results.loc[
                            results["TargetFlowContext"]
                            == flow_query.SourceFlowContexts
                        ]
...
def rank_candidates(
    ...
    sorted_list = sorted(
        ...
        key=lambda x: (
            (
                order_prefer_given_flow_context(x, context_rank)
                if context_rank
                else order_prefer_emission_over_pesticide(x)
            ),
...
def reverse_flowmaps(flowmap_df, source_nomenclatures: List[str]):
    results = flowmap_df[
        ...
        & (flowmap_df["TargetListName"].isin(source_nomenclatures))
    ].copy()
