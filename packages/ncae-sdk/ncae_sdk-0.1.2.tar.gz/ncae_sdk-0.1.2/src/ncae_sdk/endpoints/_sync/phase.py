# @generated
# This file was automatically generated by the ncae-sdk code generator.
# DO NOT EDIT MANUALLY, ANY CHANGES WILL BE OVERWRITTEN.

from collections.abc import Iterator
from typing import Final, Optional

from typing_extensions import Unpack

from ncae_sdk._resource import ResourceId
from ncae_sdk._sync.endpoint import Endpoint
from ncae_sdk._util import map_sync
from ncae_sdk.resources._schema import (
    Phase,
    PhaseCreate,
    PhaseCreateModel,
    PhaseFilter,
    PhaseFilterModel,
    PhaseUpdate,
    PhaseUpdateModel,
)


class PhaseEndpoint(Endpoint):
    BASE_PATH: Final[str] = "automation/v1/phase"

    def list(self, **filters: Unpack[PhaseFilter]) -> Iterator[Phase]:
        results = self._list(self.BASE_PATH, PhaseFilterModel, filters)
        return map_sync(Phase.parse_api, results)

    def find(self, **filters: Unpack[PhaseFilter]) -> Optional[Phase]:
        result = self._get_by_filters(self.BASE_PATH, PhaseFilterModel, filters)
        return Phase.parse_api(result) if result else None

    def get(self, rid: ResourceId) -> Optional[Phase]:
        result = self._get_by_id(self.BASE_PATH, rid)
        return Phase.parse_api(result) if result else None

    def create(self, **payload: Unpack[PhaseCreate]) -> Phase:
        result = self._create(self.BASE_PATH, PhaseCreateModel, payload)
        return Phase.parse_api(result)

    def update(self, rid: ResourceId, /, **payload: Unpack[PhaseUpdate]) -> Phase:
        result = self._update(self.BASE_PATH, rid, PhaseUpdateModel, payload)
        return Phase.parse_api(result)

    def delete(self, rid: ResourceId) -> bool:
        return self._delete(self.BASE_PATH, rid)
