...
class CredentialEndpoint(Endpoint):
    BASE_PATH: Final[str] = "dashboard/v1/credential"

    def list(self, **filters: Unpack[CredentialFilter]) -> Iterator[Credential]:
        results = self._list(self.BASE_PATH, CredentialFilterModel, filters)
        return map_sync(Credential.parse_api, results)

    def find(self, **filters: Unpack[CredentialFilter]) -> Optional[Credential]:
        result = self._get_by_filters(self.BASE_PATH, CredentialFilterModel, filters)
        return Credential.parse_api(result) if result else None

    def get(self, rid: ResourceId) -> Optional[Credential]:
        result = self._get_by_id(self.BASE_PATH, rid)
        return Credential.parse_api(result) if result else None

    def create(self, **payload: Unpack[CredentialCreate]) -> Credential:
        result = self._create(self.BASE_PATH, CredentialCreateModel, payload, preprocessor=self._add_repeat_passwords)
        return Credential.parse_api(result)

    def update(self, rid: ResourceId, /, **payload: Unpack[CredentialUpdate]) -> Credential:
        result = self._update(
