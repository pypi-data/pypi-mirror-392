...
import src.library_core.library_main as core_main
...
def test_os_is_windows_or_linux(_, capsys, os, return_vals):
        ...
        with mock.patch('sys.argv', new_callable=mock.PropertyMock(return_value=('python', 'src/core'))):        
            if os == 'other_os' and not return_vals:
                with pytest.raises(SystemExit) as excinfo:
                    ...
                    return_val = core_main.do_start_run_checks(app_name, cwd_path.joinpath('src', 'core', '__main__.py'), return_value=return_vals)
                assert excinfo.value.code == core_main.UNKNOWN_OS                    
            else:
                app_name = cwd_path.parts[-1]
                return_val = core_main.do_start_run_checks(app_name, cwd_path.joinpath('src', 'core', '__main__.py'), return_value=return_vals)        
    ...
    if os == 'other_os':
        if return_vals:
            assert return_val[0] == core_main.UNKNOWN_OS and re.match(r'FATAL\..*windows or linux platforms.*', return_val[1]) != None
        else:
            assert 'FATAL' in err, f'err was |{err}|'
    elif return_vals:
        assert return_val == core_main.CHECKS_PASSED  
        ...
        with mock.patch('sys.version_info', version_info):
            if (version_found == 'real') or return_vals:
                ...
                with pytest.raises(SystemExit) as excinfo:
                    app_name = cwd_path.parts[-1]
                    return_val = core_main.do_start_run_checks(app_name, cwd_path.joinpath('src', 'core', '__main__.py'), return_value=return_vals)
                assert excinfo.value.code == core_main.PYTHON_VERSION_IS_TOO_LOW                            
    _, err = capsys.readouterr()
    if return_vals:
        if version_found == 'real':
            assert return_val == core_main.CHECKS_PASSED
        else:
            assert return_val[0] == core_main.PYTHON_VERSION_IS_TOO_LOW and re.match( r'FATAL\..*Python 10 or higher.*', return_val[1]) != None 
    else:
        if version_found == 'real':
...
def test_main_stops_if_not_run_from_top_directory(_, mock_cwd, good_path, capsys, return_vals):
    ...
    if good_path:
        ...
        app_name = cwd_path.parts[-1]
        return_value = core_main.do_start_run_checks(app_name, mock_cwd.return_value.joinpath('src', 'core', '__main__.py'), return_value=return_vals)
        assert return_value ==  core_main.CHECKS_PASSED
        _, err = capsys.readouterr()
        assert err == ''
    else:
        ...
        if return_vals:
            app_name = cwd_path.parts[-1]
            return_value = core_main.do_start_run_checks(app_name, mock_cwd.return_value, return_value=return_vals)        
            _, err = capsys.readouterr()
            assert return_value[0] == core_main.NOT_RUN_FROM_PROJECT_ROOT
            assert re.match( r'FATAL\..*top-level directory.*', return_value[1]) != None
            assert err == ''
        else:
            with pytest.raises(SystemExit) as excinfo:
                app_name = cwd_path.parts[-1]
                core_main.do_start_run_checks(app_name, mock_cwd.return_value, return_value=return_vals)        
            _, err = capsys.readouterr()
            assert re.match( r'FATAL\..*top-level directory.*', err) != None
            assert excinfo.value.code == core_main.NOT_RUN_FROM_PROJECT_ROOT
...
def test_main_path_is_relative_to_cwd(_, mock_resolve, good_path, capsys, return_vals):
    ...
    if good_path:
        ...
        app_name = cwd_path.parts[-1]
        return_value = core_main.do_start_run_checks(app_name, mock_resolve.return_value)
        assert return_value ==  core_main.CHECKS_PASSED
        _, err = capsys.readouterr()
        assert err == ''
    else:
        ...
        if return_vals:
            app_name = cwd_path.parts[-1]
            return_val = core_main.do_start_run_checks(app_name, mock_resolve.return_value, return_value=return_vals)        
            _, err = capsys.readouterr()
            assert re.match(pattern_str, return_val[1], flags=re.IGNORECASE) != None
            assert return_val[0] == core_main.CALLER_FILE_NOT_UNDER_CWD
        else:
            with pytest.raises(SystemExit) as excinfo:
                app_name = cwd_path.parts[-1]
                core_main.do_start_run_checks(app_name, mock_resolve.return_value, return_value=return_vals)        
            _, err = capsys.readouterr()
            assert re.match(pattern_str, err, flags=re.IGNORECASE) != None
            assert excinfo.value.code == core_main.CALLER_FILE_NOT_UNDER_CWD
...
def test_main_path_relative_to_cwd_is_in_expected_package(_, good_path, capsys, return_vals):
    ...
    if good_path:
        app_name = cwd_path.parts[-1]
        return_value = core_main.do_start_run_checks(app_name, root_dir_path.joinpath('src', 'core', '__main__.py'), return_value=return_vals)
        assert return_value ==  core_main.CHECKS_PASSED
        _, err = capsys.readouterr()
        assert err == ''
    else:
        ...
        if return_vals:
            app_name = cwd_path.parts[-1]
            return_val = core_main.do_start_run_checks(app_name, caller_file_path, return_value=return_vals)
            _, err = capsys.readouterr()
            assert re.match(pattern_str, return_val[1], flags=re.IGNORECASE) != None
            assert return_val[0] == core_main.CALLER_FILE_NOT_IN_EXPECTED_PACKAGE
        else:
            with pytest.raises(SystemExit) as excinfo:
                app_name = cwd_path.parts[-1]
                core_main.do_start_run_checks(app_name, caller_file_path, return_value=return_vals)
            _, err = capsys.readouterr()
            assert re.match(pattern_str, err, flags=re.IGNORECASE) != None
            assert excinfo.value.code == core_main.CALLER_FILE_NOT_IN_EXPECTED_PACKAGE
...
def test_EnhancedStandardFormatter_prints_only_path_from_cwd():
    ...
    # configure_logging.logger.info('some bubbles')
    logger = logging.getLogger('test')
    logger.info('some bubbles')
    ...
    # sys.pytl.log(f'result.groups()[0] |{result.groups()[0]}| last_part_of_path |{last_part_of_path}|')
...
def test_do_start_run_checks_can_return_values_rather_than_exiting(_):
    ...
    with mock.patch('sys.version_info', version_info):
        return_value = core_main.do_start_run_checks(app_name, cwd_path.joinpath('src', 'core', '__main__.py'), return_value=True)
    assert return_value[0] == core_main.PYTHON_VERSION_IS_TOO_LOW     
...
def test_do_start_run_checks_checks_package_name_param(_, package_name, return_vals, capsys):
    ...
    if package_name == 'string_name':
        return_val = core_main.do_start_run_checks(app_name, caller_file_path, package_name=package_name, return_value=return_vals)
        assert return_val == core_main.CHECKS_PASSED
        _, err = capsys.readouterr()
        assert err == ''
    elif return_vals:
        return_val = core_main.do_start_run_checks(app_name, caller_file_path, package_name=package_name, return_value=return_vals)
        assert return_val[0] == core_main.BAD_PARAMS
        assert re.match(fr'FATAL\..*Bad param.*', return_val[1]) != None, f'return_val[1] |{return_val[1]}|'
        _, err = capsys.readouterr()
        assert err == ''
    else:
        with pytest.raises(SystemExit) as excinfo:
            core_main.do_start_run_checks(app_name, caller_file_path, package_name=package_name, return_value=return_vals)
        assert excinfo.value.code == core_main.BAD_PARAMS
        _, err = capsys.readouterr()
...
def test_do_start_run_checks_checks_caller_file_path_param(file_path, return_vals, capsys):
    ...
    if file_path == 'real':
        ...
        with mock.patch('pathlib.Path.is_file', return_value=True):
            return_val = core_main.do_start_run_checks(app_name, file_path, return_value=return_vals)
        assert return_val == core_main.CHECKS_PASSED
        _, err = capsys.readouterr()
        assert err == ''
    elif return_vals:
        ...
        with pytest.raises(SystemExit) as excinfo:
            core_main.do_start_run_checks(app_name, file_path, return_value=return_vals)
        assert excinfo.value.code == core_main.BAD_PARAMS
        _, err = capsys.readouterr()
        assert re.match(fr'FATAL\..*Bad param.*', err) != None, f'err |{err}|'
...
def test_do_start_run_checks_checks_app_name_param(app_name_param, return_vals, capsys):
    ...
    if app_name_param == 'real':
        ...
        return_val = core_main.do_start_run_checks(app_name_param, caller_file_path, return_value=return_vals)
        assert return_val[0] == core_main.BAD_PARAMS
        assert re.match(fr'FATAL\..*Bad param.*', return_val[1]) != None, f'return_val[1] |{return_val[1]}|'
        _, err = capsys.readouterr()
        assert err == ''
        ...
        with pytest.raises(SystemExit) as excinfo:
            core_main.do_start_run_checks(app_name_param, caller_file_path, return_value=return_vals)
        assert excinfo.value.code == core_main.BAD_PARAMS
        _, err = capsys.readouterr()
        assert re.match(fr'FATAL\..*Bad param.*', err) != None, f'err |{err}|'
...
def test_do_start_run_checks_checks_that_caller_file_path_is_file(caller_file_path, return_vals, capsys, tmpdir):
    ...
    if caller_file_path == 'file':
        ...
        return_val = core_main.do_start_run_checks(app_name, caller_file_path, package_name='library_core', return_value=return_vals)
        assert return_val == core_main.CHECKS_PASSED
        _, err = capsys.readouterr()
        assert err == ''
    elif return_vals:
        return_val = core_main.do_start_run_checks(app_name, bad_caller_file_path, return_value=return_vals)
        assert return_val[0] == core_main.BAD_PARAMS
        assert re.match(fr'FATAL\..*Bad param.*', return_val[1]) != None, f'return_val[1] |{return_val[1]}|'
        _, err = capsys.readouterr()
        assert err == ''
        ...
        with pytest.raises(SystemExit) as excinfo:
            core_main.do_start_run_checks(app_name, bad_caller_file_path, return_value=return_vals)
        assert excinfo.value.code == core_main.BAD_PARAMS
        _, err = capsys.readouterr()
        assert re.match(fr'FATAL\..*Bad param.*', err) != None, f'err |{err}|'
...
def test_version_check_message_if_lib_version_is_too_low(version_too_low, tmpdir):
    ...
    # 2024-12-07 this has changed completely: core_main.lib_module_version_check must now read in file version.toml in the root dir
    ...
    with mock.patch('pathlib.Path.cwd', return_value=tmpdir_path):
        ...
        assert core_main.lib_module_version_check(min_lib_version) == (not version_too_low)
...
def test_main_stops_if_not_run_in_virtual_environment(capsys, in_ve, return_vals):
    ...
    if in_ve:
        ...
        assert error_code == core_main.NOT_IN_VIRTUAL_ENVIRONMENT
