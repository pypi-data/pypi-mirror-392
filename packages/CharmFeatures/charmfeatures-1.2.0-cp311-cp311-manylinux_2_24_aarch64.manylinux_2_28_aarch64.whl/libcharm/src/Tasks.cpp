...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
/* Written by:                                                                   */
/*      Ilya G. Goldberg <igg [at] iggtec [dot] com>                             */
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
#include <stdexcept>
#include <string>
#include <iostream>
...
const char *ComputationTask::typeLabels (size_t type_idx) {
	...
	if (! (type_idx > 0 && type_idx < (sizeof(type_labels) / sizeof (type_labels[0]))) ) 
		throw std::invalid_argument ("Attempt to get out of range TaskType label");
...
bool ComputationTaskInstances::add (const ComputationTask *task) {
	...
	if (LC::verbosity > 4) std::cout << "Registering " << task->typeLabel () << " '" << task->name << "'" << std::endl;
...
// Add a FeatureGroup to a plan.
// A FeatureGroup is a feature sub-vector resulting from one specific feature algorithm and upstream transforms
// It is called a group, since it is usually more than a single feature value.
void FeatureComputationPlan::add (const FeatureGroup *fg) {
	...
	if (isFinal) throw std::logic_error("Attempt to call FeatureComputationPlan::add() to a finalized plan");
	if (!fg) throw std::invalid_argument("Attempt to call FeatureComputationPlan::add() with NULL feature group");
	if (FG_offset_map.find (fg->name) != FG_offset_map.end())
		throw std::invalid_argument("Calling FeatureComputationPlan::add() with duplicate FeatureGroup");
...
void FeaturePlanExecutor::execute_node (const ComputationTaskNode *exec_node) {
	...
	if (!IM_in)
		throw std::invalid_argument ("Attempt to execute a FeatureComputationPlan node with a NULL source ImageMatrix");

	if (LC::verbosity > 5) std::cout << "** executing node '" << exec_node->name << "' with " << exec_node->num_dependent_nodes << " total dependents. IM_in=" << IM_in;
	switch (task->type) {
		case ComputationTask::ImageTransformTask: {
			const ImageTransform *IT_task = dynamic_cast<const ImageTransform *>(exec_node->task);
			if (!IT_task)
				throw std::invalid_argument ("Attempt to cast task as a (const ImageTransform *) failed.");
			// The ImageMatrix cache is keyed by node_key
			if (IM_map.find(exec_node->node_key) != IM_map.end())
				throw std::logic_error ("Attempt to execute a transform which is already cached.");

			AnonSharedImageMatrix *IM_out = new AnonSharedImageMatrix;
			if (LC::verbosity > 5) std::cout << " ImageTransform task '" << IT_task->name << "'" << std::endl;

			IT_task->execute (*IM_in, *IM_out); // Child
			IM_map[exec_node->node_key] = IM_out;
			if (LC::verbosity > 7) std::cout << " IM_map size: " << IM_map.size() << std::endl;
		} break;
		...
		case ComputationTask::FeatureAlgorithmTask: {
			...
			if (!FA_task)
				throw std::invalid_argument ("Attempt to cast task as a (const FeatureAlgorithm *) failed.");
			// The name of a node with a FeatureAlgorithmTask is the FeatureGroup name
			size_t offset = plan->getFGoffset (exec_node->name);
			if (offset == (size_t)-1)
				throw std::logic_error ("execute_node() called with an unknown offset for the Feature Group");

			if (LC::verbosity > 5) std::cout << " FeatureAlgorithm task '" << FA_task->name << "' (@ " << offset << ", " << FA_task->n_features << " features)" << std::endl;
			size_t mat_offset = (plan->n_features * current_feature_mat_row) + offset;
			// construct a vector for the result
			FA_task->execute (*IM_in, &feature_mat[mat_offset]);
		} break;
		
		default:
			throw std::logic_error ("Attempt to execute a node with an undefined task type");
		break;
	}
}

// FIXME: this can go into the base class (?) if its not specialized for task types
void FeaturePlanExecutor::finish_node_execution (const ComputationTaskNode *exec_node) {
	if (LC::verbosity > 6) std::cout << "finished '" << exec_node->name << "'" << std::endl;
...
void FeaturePlanExecutor::run (const ImageMatrix *source_mat, double *feature_mat_in, size_t dest_row) {
	...
	if (LC::verbosity > 5) std::cout << "Finished running execution plan '" << plan->name << "'" << std::endl;

	// dump out algorithm run times
	if (LC::verbosity > 4) dump_runtimes();
}

void ForkingFeaturePlanExecutor::execute_node (const ComputationTaskNode *exec_node) {
	if( exec_node != plan->root ) FeaturePlanExecutor::execute_node(exec_node);
	if (LC::verbosity > 5) std::cout << "** Completed node '" << exec_node->name << "' with " << exec_node->num_dependent_nodes << " dependents." << "\n";
	for (size_t i = 0; i < exec_node->dependent_tasks.size(); i++) {
		if (LC::verbosity > 5) std::cout << "\t** Forking node '" << exec_node->dependent_tasks[i]->name << std::endl;
		pid_t cpid = fork();
		if (cpid == 0) execute_node(exec_node->dependent_tasks[i]); // Child
	}
	wait_all_forks();
	if( exec_node != plan->root ) 
		_Exit(EXIT_SUCCESS); // don't call destructors in the child
}


void ForkingFeaturePlanExecutor::run (const ImageMatrix *source_mat, double *feature_mat_in, size_t dest_row) {
	reset();

	// set up an anoymous shared mmap for child processes to write results into
	size_t shm_size = plan->n_features * sizeof(double);
	void *shm_addr = mmap(NULL, shm_size, PROT_READ | PROT_WRITE,
		MAP_SHARED | MAP_ANONYMOUS, -1, 0
	);

	if (shm_addr == MAP_FAILED)
		throw std::runtime_error ("anonymous mmap failed in ForkingFeaturePlanExecutor::run()");
	feature_mat = (double *) shm_addr;

	// Setup input & output
	IM_map["root"] = source_mat;
	// We've allocated memory just for this row, so the dest_row in it is 0
	current_feature_mat_row = 0;

	// Actually execute
	execute_node (plan->root);

	wait_all_forks();

	std::copy (feature_mat, feature_mat + plan->n_features, feature_mat_in+(dest_row * plan->n_features));

	munmap (shm_addr, shm_size);
	
	// Store the external feature array pointer to preserve legacy behaviors.
	feature_mat = &feature_mat_in[0];
	// The caches get cleaned up in reset() above, or in the destructor
	if (LC::verbosity > 5) std::cout << "Finished running execution plan with '" << plan->name << "'" << std::endl;
...
ComputationPlanExecutor::~ComputationPlanExecutor () {
	if (LC::verbosity > 7) std::cout << "~ComputationPlanExecutor()" << std::endl;
	reset();
}

FeaturePlanExecutor::~FeaturePlanExecutor () {
	if (LC::verbosity > 7) std::cout << "~FeaturePlanExecutor()" << std::endl;
	reset();
}

ForkingFeaturePlanExecutor::~ForkingFeaturePlanExecutor () {
	if (LC::verbosity > 7) std::cout << "~ForkingFeaturePlanExecutor()" << std::endl;
	reset();
}


void ComputationPlanExecutor::reset () {
	if (LC::verbosity > 7) std::cout << "ComputationPlanExecutor::reset()" << std::endl;
	node_times.clear();
	alg_runs.clear();
	executable_nodes.clear();
	executing_nodes.clear();
};

void FeaturePlanExecutor::reset () {
	if (LC::verbosity > 7) std::cout << "FeaturePlanExecutor::reset(); cleaning up " << IM_map.size() << " images in IM_map cache" << std::endl;
	// Call the parent class's reset
	ComputationPlanExecutor::reset();
	...
	for(IM_map_it = IM_map.begin(); IM_map_it != IM_map.end(); IM_map_it++) {
		if (LC::verbosity > 7) std::cout << "FeaturePlanExecutor::reset(): deleting IM for node_key=" << IM_map_it->first << std::endl;
...
void ForkingFeaturePlanExecutor::reset () {
	if (LC::verbosity > 7) std::cout << "ForkingFeaturePlanExecutor::reset()" << std::endl;
	FeaturePlanExecutor::reset();
	node_pids.clear();
}
