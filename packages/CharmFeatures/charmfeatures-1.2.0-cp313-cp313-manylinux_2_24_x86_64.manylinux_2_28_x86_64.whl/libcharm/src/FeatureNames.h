...
// This is to avoid the "static initialization order fiasco" (see http://www.parashift.com/c++-faq/static-init-order-on-first-use-members.html)
	typedef std::unordered_map<std::string, const Channel *> cnm_t;
	static cnm_t &channels_map ();

	typedef std::unordered_map<std::string, const ImageTransform *> tnm_t;
	static tnm_t &transforms_map ();

	typedef std::unordered_map<std::string, const FeatureAlgorithm *> fam_t;
	static fam_t &feature_algorithms_map ();

	typedef std::unordered_map<std::string, const FeatureGroup *> fgnm_t;
	static fgnm_t &feature_groups_map ();

	typedef std::unordered_map<std::string, const FeatureInfo *> fnm_t;
	static fnm_t &feature_names_map ();

	typedef std::unordered_map<std::string, std::string> ofnm_t;
