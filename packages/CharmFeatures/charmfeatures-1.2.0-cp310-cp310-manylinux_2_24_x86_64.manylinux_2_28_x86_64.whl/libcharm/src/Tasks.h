...
#define CURRENT_FEATURE_VERSION 5
...
class ComputationTaskNode {
	public:
		...
		// FIXME: This is a bug. The name should be the task->name or "root", i.e. name of algorithm
		// and the node_key should be unique in the tree (like the FG name, for e.g.)
		// As is, name is FG name, and node_key is another unique reshuffling of transforms and feature algs.
		std::string name;
		std::string node_key;
		...
		ComputationTaskNode(ComputationTaskNode *source_node_in, const ComputationTask *task_in,
			const std::string &name_in = "", const std::string &key_in = "")
		{
			if (task_in && !source_node_in)
				throw std::invalid_argument ("Attempt to create a ComputationTaskNode with an undefined source but defined task");
			if (!task_in && source_node_in)
				throw std::invalid_argument ("Attempt to create a ComputationTaskNode with an undefined task but defined source");
...
class ComputationPlan {
		...
		const ComputationTaskNode *add_get_node (const std::string &node_key, const ComputationTaskNode *source_node,
			const ComputationTask *task, const std::string &name_in = "") {
				...
				if (nodemap_it != nodemap.end()) {
					...
					return (nodemap[node_key] = new ComputationTaskNode(const_cast<ComputationTaskNode *>(source_node),
						task, name_in, node_key
					));
				}
		}
	...
	protected:
		...
		typedef std::unordered_map<std::string, const ComputationTaskNode *> nodemap_t;
		nodemap_t nodemap;
	private:
...
// FeaturePlanExecutor : ComputationPlanExecutor
//     has IM_cache (IM_map)
// ForkingFeaturePlanExecutor : FeaturePlanExecutor
//     has concurrency specialization (execute_node(), finish_node_execution())
//     As implemented, concurrency is done by first executing the parent node,
//       then forking the execution of every dependent node.
//
...
class ComputationPlanExecutor {
	public:
		...
		typedef std::unordered_map<std::string, double> alg_times_t;
		alg_times_t alg_times, node_times;
		typedef std::unordered_map<std::string, int> alg_runs_t;
		alg_runs_t alg_runs;
		...
		virtual void finish_node_execution (const ComputationTaskNode *exec_node) {
			...
			if (exec_node->task) {
				...
				alg_times[exec_node->task->name] += (
					(tim.tv_sec+(tim.tv_usec/1000000.0)) - node_times[exec_node->node_key]
				);
				alg_runs[exec_node->task->name]++;
			}
		}
	...
	protected:
		...
		typedef std::unordered_map<std::string, const ComputationTaskNode *> executing_nodes_t;
		...
		// This also starts timers for the node
		virtual void execute_node (const ComputationTaskNode *exec_node) {
			if (executing_nodes.find(exec_node->node_key) != executing_nodes.end())
				throw std::logic_error ("Attempt to execute a node which is already executing.");
			executing_nodes.insert ( std::pair<std::string, const ComputationTaskNode *>(exec_node->node_key, exec_node) );

			if (exec_node->task) {
				struct timeval tim;
				gettimeofday(&tim, NULL);
				// Note that name is the algorithm name, repeated multiple times in the tree
				// while node_key is unique in the tree.
				// FIXME: This is probably a bug. The node name should be the task name, but it is the FG name.
				//        The node_key shoul dbe the FG name, but it is some other representation of the FG name.
				if (alg_times.find(exec_node->task->name) == alg_times.end()) {
					alg_times[exec_node->task->name] = 0.0;
					alg_runs[exec_node->task->name] = 0;
				}
				node_times[exec_node->node_key] = tim.tv_sec+(tim.tv_usec/1000000.0);
			}
		}
		// This resets the object for the next call to run()
		// run() calls reset(), so this is protected.  The destructor also calls reset()
		virtual void reset ();
		virtual void dump_runtimes () {
			// dump out algorithm run times
			alg_times_t::iterator alg_times_it;
			for(alg_times_it = alg_times.begin(); alg_times_it != alg_times.end(); alg_times_it++) {
				std::cout << alg_times_it->first << " : " << alg_runs[alg_times_it->first] << " @ "
					<< alg_times_it->second << std::endl;
			}
		}
	private:
...
class FeatureComputationPlan : public ComputationPlan {
	...
	private:
		...
		typedef std::unordered_map<std::string, size_t> FG_offset_map_t;
		FG_offset_map_t FG_offset_map;
		// offset_FG_map keys are feature numbers/offsets, values are FG pointers
		typedef std::unordered_map<size_t, const FeatureGroup *> offset_FG_map_t;
		...
		typedef std::unordered_map<size_t, const std::string *> offset_FN_map_t;
		...
		typedef std::unordered_map<std::string, const ComputationTaskNode *> FG_node_map_t;
		FG_node_map_t FG_node_map;

};

class FeaturePlanExecutor : public ComputationPlanExecutor {
	public:
		...
		virtual void finish_node_execution (const ComputationTaskNode *exec_node);
		virtual void run (const ImageMatrix *source_mat, double *feature_mat_in, size_t dest_row);
		...
		virtual void run () {}
		virtual ~FeaturePlanExecutor ();
		// sub-classes must set their own plan.  Relying on the parent class to do this doesn't work.
		FeaturePlanExecutor (const FeatureComputationPlan *plan_in) : ComputationPlanExecutor (plan_in) {
			plan = plan_in;
			feature_mat = NULL;
	...
	protected:
		...
		typedef std::unordered_map<std::string, const ImageMatrix *> IM_map_t;
		IM_map_t IM_map;
...
// This concurrent executor calls the parent's execute_node to execute a node, then it forks
// for each dependency, and executes the dependent node in the child with its derived execute_node.
// Finally, the parent waits for all the forked childern to finish.
// Considerations:
// Provision has to be made for putting the computed features into shared memory (an mmapped temp file)
// It is not necessary to share the transformed image matrixes because they are always dependencies
//   and never propagated to the parents.
class ForkingFeaturePlanExecutor : public FeaturePlanExecutor {
	public:
		virtual void run (const ImageMatrix *source_mat, double *feature_mat_in, size_t dest_row);
		ForkingFeaturePlanExecutor (const FeatureComputationPlan *plan_in) : FeaturePlanExecutor (plan_in) {}
		virtual ~ForkingFeaturePlanExecutor ();
	protected:
		// The node_pids is a map of node pointers keyed on PIDs that are executing the node
		typedef std::unordered_map<pid_t, const ComputationTaskNode *> node_pids_t;
		node_pids_t node_pids;
		virtual void execute_node (const ComputationTaskNode *exec_node);
		virtual void reset ();
