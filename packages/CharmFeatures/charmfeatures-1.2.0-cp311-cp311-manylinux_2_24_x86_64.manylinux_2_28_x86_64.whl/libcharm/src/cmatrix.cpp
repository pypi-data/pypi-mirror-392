...
/* From https://stackoverflow.com/questions/279729/how-to-wait-until-all-child-processes-called-by-fork-complete */
void wait_all_forks() {
	while (true) {
		int status;
		...
		if (spp == 3 && bits > 8) {
			...
			for (y = 0; y < height; y++) {
				for (x = 0; x < width; x++) {
					...
					pix_plane (y, x) = stats.add (RGB2GRAY (R_matrix.ReadablePixels()(y,x), G_matrix.ReadablePixels()(y,x), B_matrix.ReadablePixels()(y,x)));
					clr_plane (y, x) = RGB2HSV(rgb);
				}
			}
		}
...
// 
// If you call this method, you are taking over memory management of the data pointer.
// You must dispose/deallocate the ptr outside of this code-base, and you must call remap_pix_plane(NULL,0,0)
// after you are finished with the data ptr, and before the object falls out of scope.
// There will be  double free if you do not call remap_pix_plane(NULL,0,0)
...
void ImageMatrix::allocate (unsigned int w, unsigned int h) {
	...
	if ((unsigned int) _pix_plane.cols() != w || (unsigned int)_pix_plane.rows() != h) {
		if (!_is_reallocatable)
			throw std::logic_error ("Reallocating a non-reallocatable pixel matrix");
		...
		if (LC::verbosity > 7 && _pix_plane.data()) fprintf (stdout, "deallocating grayscale %p\n",(void *)_pix_plane.data());
		//std::cout <<  "ImageMatrix::allocate(): deallocating grayscale pix_plane pointer=" << (void *)_pix_plane.data() << std::endl;
		if (_pix_plane.data()) _pix_allocator.deallocate (_pix_plane.data(), _pix_plane.size());
		remap_pix_plane (_pix_allocator.allocate (w * h), w, h);
		if (LC::verbosity > 7 && _pix_plane.data()) fprintf (stdout, "allocated grayscale %p (%d,%d)\n",(void *)_pix_plane.data(), w, h);
	...
	if ( ColorMode == cmGRAY || (_pix_plane.data() && ((unsigned int)_clr_plane.cols() != w || (unsigned int)_clr_plane.rows() != h)) ) {
		if (!_is_reallocatable)
			throw std::logic_error ("Reallocating a non-reallocatable pixel matrix");
		if (LC::verbosity > 7 && _clr_plane.data()) fprintf (stdout, "  deallocating color %p\n",(void *)_clr_plane.data());
		if (_clr_plane.data()) _HSV_allocator.deallocate (_clr_plane.data(), _clr_plane.size());
	...
	if (ColorMode != cmGRAY && ! (_clr_plane.data()) ) {
		...
		// These throw exceptions, which we don't catch (catch in main?)
		// FIXME: We could check for shrinkage and simply remap instead of allocating.
		remap_clr_plane (_HSV_allocator.allocate (w * h), w, h);
		if (LC::verbosity > 7 && _clr_plane.data()) fprintf (stdout, "  allocated color %p (%d,%d)\n",(void *)_clr_plane.data(), w, h);
	}
}
...
void ImageMatrix::Downsample (const ImageMatrix &matrix_IN, double x_ratio, double y_ratio) {
	...
	for (new_y = 0; new_y < old_height; new_y++) {
		...
		while (x < old_width) {
			...
			if (ColorMode != cmGRAY) {
				hsv.h = (int8_t)(sum_h/(dx));
				hsv.s = (int8_t)(sum_s/(dx));
				hsv.v = (int8_t)(sum_v/(dx));
	...
	for (new_x = 0; new_x < new_width; new_x++) {
		...
		while (y < old_height) {
			...
			if (new_x < new_width && new_y < new_height) {
				...
				if (ColorMode != cmGRAY) {
					hsv.h = (int8_t)(sum_h/(dy));
					hsv.s = (int8_t)(sum_s/(dy));
					hsv.v = (int8_t)(sum_v/(dy));
...
double ImageMatrix::fft2 (const ImageMatrix &matrix_IN) {
	...
	p = fftw_plan_dft_r2c_2d(width,height,in,out, FFTW_ESTIMATE); // FFTW_ESTIMATE: deterministic
	...
	unsigned long idx;
 	for (x=0;x<width;x++) {
 		for (y=0;y<half_height;y++) {
 			idx = half_height*x+y;
 			out_plane (y,x) = stats.add ( sqrt(
 				pow( out[idx][0],2 ) + pow( out[idx][1],2 )
