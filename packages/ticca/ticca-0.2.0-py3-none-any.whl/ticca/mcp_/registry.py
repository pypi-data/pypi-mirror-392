...
class ServerRegistry:
    ...
    validation, and persistent storage to ~/.ticca/mcp_registry.json.
    ...
    def __init__(self, storage_path: Optional[str] = None):
        ...
        Args:
            storage_path: Optional custom path for registry storage.
                         Defaults to ~/.ticca/mcp_registry.json
        """
        if storage_path is None:
            home_dir = Path.home()
            ticca_dir = home_dir / ".ticca"
            ticca_dir.mkdir(exist_ok=True)
            self._storage_path = ticca_dir / "mcp_registry.json"
        else:
