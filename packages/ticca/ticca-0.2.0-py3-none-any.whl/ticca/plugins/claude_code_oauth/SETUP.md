...
1. Ensure the plugin files live under `ticca/plugins/claude_code_oauth/`
...
5. Tokens are stored locally at `~/.ticca/claude_code_oauth.json`
...
- Models appear in `~/.ticca/extra_models.json` with the `claude-code-` prefix
...
```bash
python ticca/plugins/claude_code_oauth/test_plugin.py
```
