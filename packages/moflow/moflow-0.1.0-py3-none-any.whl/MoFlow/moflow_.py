...
def get_loss(c, u, s, param, indices, 
             direction=None, margin=None, rna_only=False, warm=False, test=False,
             ):
    ...
    if rna_only:
        ...
        param = [alpha_c, alpha, beta, gamma] #TODO
...
class DNN_layer(nn.Module):
    ...
    def __init__(self, rna_only=False, 
                 direction_='complete', margin=None):
        super().__init__()
    ...
    def forward(self, c, u, s, indices, epoch=None, test=False):
        if self.rna_only:
            ...
            loss, velo, param, states, direction = get_loss(
                c, u, s, param, indices, self.direction, self.margin, self.rna_only, warm,test)
            vc, vu, vs = velo
            alpha_c, alpha, beta, gamma = param
    ...
    def load(self, model_name=None):
        ...
        if not self.rna_only:
            ...
            "/model/multimodel/"
            if self.direction == 'on':
                self.param.load_state_dict(torch.load(net_path + 'param_on.pt'))
            elif self.direction == 'off':
                self.param.load_state_dict(torch.load(net_path + 'param_off.pt'))
            else:
                if model_name=='circle':
                    self.param.load_state_dict(torch.load(net_path + 'param_circle.pt'))
                elif model_name=='branch':
                    self.param.load_state_dict(torch.load(net_path + 'param_branch.pt'))
            self.state_on.load_state_dict(torch.load(net_path + 'state_on.pt'))
            self.state_off1.load_state_dict(torch.load(net_path + 'state_off1.pt'))
            self.state_off2.load_state_dict(torch.load(net_path + 'state_off2.pt'))
            self.state_complete1.load_state_dict(torch.load(net_path + 'state_complete1.pt'))
            self.state_complete2.load_state_dict(torch.load(net_path + 'state_complete2.pt'))
...
class DNN_module(nn.Module):
    ...
    def velocity_calculate(self, 
        ...
        loss, vc, vu, vs, alpha_c, alpha, beta, gamma, states = self.module(
            c, u, s, embedding1, embedding2, indices, time, epoch, test)
...
class ltModule(pl.LightningModule):
    ...
    def load(self, model_name=None):
        self.backbone.module.load(model_name)   # load network
    ...
    def training_step(self, batch, batch_idx):
        ...
        cs, us, ss, cmaxs, umaxs, smaxs, stdcs, stdus, stdss, embedding1s, embedding2s, gene_names, times, _, _ = batch
        c, u, s, stdc, stdu, stds, embedding1, embedding2, gene_name, time = cs[0], us[0], ss[0], stdcs[0], stdus[0], stdss[0], embedding1s[0], embedding2s[0], gene_names[0], times[0]
        
        cost, vc, vu, vs, alpha_c, alpha, beta, gamma, states = self.backbone.velocity_calculate( \
                c, u, s, time, stdc, stdu, stds, embedding1, embedding2, self.current_epoch)
...
class getItem(Dataset): 
    ...
    '''
    def __init__(self, c, u, s, time, embedding, cluster,
                 rna_only=False,
    ...
    def __getitem__(self, idx):
        ...
        stdc = np.std(self.c[data]).astype(np.float32)
        stdu = np.std(self.u[data]).astype(np.float32)
        stds = np.std(self.s[data]).astype(np.float32)
        ...
        return c, u, s, cmax, umax, smax, stdc, stdu, stds, embedding1, embedding2, gene_name, time, cluster, self.local_std
...
class feedData(pl.LightningDataModule):
    ...
    def __init__(self, c, u, s, time, embedding, cluster,
        ...
        self.fit_dataset = getItem(c, u, s, time, embedding, cluster,
                                   rna_only=rna_only,
        ...
        self.predict_dataset = getItem(c, u, s, time, embedding, cluster,
                                       rna_only=rna_only, 
                                       data_fit=data_fit, data_predict=data_predict,
                                       gene_name=gene_name,
...
def build_datamodule(c, u, s, time, embedding, cluster,
                     rna_only=False,
                     speed_up=True,
                     norm_data=True,
    ...
    if speed_up:
        ...
        feed_data = feedData(c, u, s, time, embedding, cluster,
                             rna_only=rna_only, 
...
class MOFlow():
    def __init__(self,
        ...
        try:
            self.time = adata.obs[time_key].values
        except:
            print('time is not found! Set to 0')
            self.time = np.zeros(adata.n_obs)
        try:
            self.cluster = adata.obs[cluster_key].cat.codes.values 
        except:
            print('cluster is not found! Set to 0')
            self.cluster = np.zeros(adata.n_obs)
            self.time = np.zeros(adata.n_obs)
        try:
    ...
    def _train_thread(self,
        ...
        try:
            ...
            c, u, s, cmax, umax, smax, stdc, stdu, stds, embedding1, embedding2, gene_name, time, cluster, local_std = selected_data.predict_dataset.__getitem__(0)
            ...
            model_ = select_initial_net(c, u, s, gene_name, self.rna_only, embedding, local_std, time, cluster)
            ...
            backbone = DNN_module(DNN_layer(self.rna_only, direction_, margin_),
    ...
    def velocity(self, adata, 
        ...
        datamodule=build_datamodule(self.c, self.u, self.s, self.time, self.embedding, self.cluster,
...
def select_initial_net(
    c, u, s, gene_name, rna_only, embedding, local_std, time=None, cluster=None, fit_slope=True):
    '''
    ...
    if rna_only:
        ...
        if np.std(time)>0:
            df = pd.DataFrame({'u': u, 's': s,
                          't': time, 'group': cluster})
            df_ = df.groupby('group').median()

            x = df_['t']
            u = df_['u']
            s = df_['s']

            poly = np.polyfit(x, s, 1)
            a = poly[0]
            b = poly[1]
        ...
        if len(ss_u) < 10 or len(u_non_zero) < 10:
            ...
            if partial is True:
                if np.max(time) >0:
                    time_last = time>=np.max(time)-1
                    time_first = time==0
                    
                    u_last = np.mean(u[time_last])
                    u_first = np.mean([time_first])
                    s_last = np.mean(s[time_last])
                    s_first = np.mean(s[time_first])
