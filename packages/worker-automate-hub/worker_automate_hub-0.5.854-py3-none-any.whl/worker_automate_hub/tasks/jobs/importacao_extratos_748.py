    ...
    try:
        ...
        for idx, sel in enumerate(selecionados, 1):
            ...
            try:
                ...
                # Conecta na janela do Conciliador
                app_conc = Application(backend="win32").connect(
                    class_name="TFrmConciliadorBancario2", timeout=120
                )
                wnd = app_conc.window(class_name="TFrmConciliadorBancario2")

                # Busca todos os campos numéricos
                ...
                if len(campos) < 5:
                    ...
                    if valor_0 == valor_4 and valor_0 != "":
                        console.print(
                            "[green]Valores iguais! Clicando no botão 'Conciliar'...[/green]"
                        )
                        pos = pyautogui.locateCenterOnScreen(
                        ...
                        if pos:
                            ...
                            IMAGEM_SUCESSO = (
                                fr"{ASSETS_BASE_PATH}\conciliados_sucesso.png"
                            )
                            ...
                            console.print(
                                "[cyan]Aguardando imagem de sucesso aparecer...[/cyan]"
                            )
                            ...
                            while time.time() - inicio < timeout:
                                ...
                                if pos:
                                    console.print(
                                        "[green]Imagem 'conciliados_sucesso' encontrada![/green]"
                                    )
                                    try:
                                        app_info = Application(
                                            backend="win32"
                                        ).connect(title="Information", found_index=0)
                                        main_info = app_info["Informação"]
                                        main_info.child_window(
                                            title="OK", found_index=0
                                        ).click()
                                        console.print(
                        ...
                        console.print("[yellow]Valores diferentes.[/yellow]")
