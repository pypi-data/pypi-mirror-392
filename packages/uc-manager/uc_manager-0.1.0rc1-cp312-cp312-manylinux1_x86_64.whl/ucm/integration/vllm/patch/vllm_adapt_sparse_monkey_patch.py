...
def _apply_sparse_patch() -> None:
    """Patch sparse attention to add UCM sparse support."""
    _patch_attention_layer()
    _patch_shared_storage_connector()
    _patch_mla_common()
    _patch_kv_cache_manager()
    # _patch_SchedulerOutput() # Already patched in the begging
    _patch_scheduler()
    _patch_block_table()
...
def _patch_attention_layer() -> None:
    ...
    try:
        ...
        layer.maybe_execute_sparse_attention_begin = maybe_execute_sparse_attention_begin
        ...
        layer.maybe_execute_sparse_attention_finished = maybe_execute_sparse_attention_finished
        ...
        logger.warning("Could not patch unified attention with output - module not found")
...
def _patch_shared_storage_connector() -> None:
    ...
    try:
        ...
        from vllm.distributed.kv_transfer.kv_connector.v1 import shared_storage_connector
        @dataclass
        ...
        shared_storage_connector.SharedStorageConnectorMetadata = SharedStorageConnectorMetadata
    except ImportError:
...
def _patch_mla_common() -> None:
    ...
    try:
        ...
        from vllm.v1.attention.backends.mla.common import MLACommonMetadata, MLACommonImpl
...
def _patch_kv_cache_manager() -> None:
    ...
    try:
        ...
        def patched_allocate_slots(
            ...
            return original_allocate_slots(self,
                                           request,
                                           num_new_tokens,
...
def _patch_SchedulerOutput() -> None:
...
def _patch_scheduler() -> None:
    """Patch scheduler to add UCM sparse support."""
    try:
        import time
        ...
        def init_ucm_sparse(self):
            ...
            if self.vllm_config.kv_transfer_config is not None:
                if "ucm_sparse_config" in self.vllm_config.kv_transfer_config.kv_connector_extra_config:
                    ensure_ucm_sparse_initialized(self.vllm_config, role=UcmSparseRole.SCHEDULER)
        ...
        logger.warning("Could not patch add request - module not found")
...
def _patch_gpu_model_runner() -> None:
    ...
    try:
        ...
        GPUModelRunner.maybe_execute_ucm_sparse_finished = maybe_execute_ucm_sparse_finished
        GPUModelRunner.ucm_sparse_request_finished_in_worker = ucm_sparse_request_finished_in_worker    
...
def _patch_gpu_worker() -> None:
    ...
    try:
        ...
        original_init_worker_distributed_environment = gpu_worker.init_worker_distributed_environment
        def patched_init_worker_distributed_environment(
            ...
            original_init_worker_distributed_environment(vllm_config, rank, distributed_init_method, local_rank, backend)
            ensure_ucm_sparse_initialized(vllm_config)
        gpu_worker.init_worker_distributed_environment = patched_init_worker_distributed_environment
    except ImportError:
