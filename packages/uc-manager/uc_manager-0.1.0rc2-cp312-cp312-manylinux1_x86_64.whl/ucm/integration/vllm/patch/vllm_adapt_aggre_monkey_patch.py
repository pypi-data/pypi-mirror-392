...
# ==================== Aggregation Patch Functions ====================
def _apply_aggre_patch() -> None:
    """Apply vllm-adapt-aggre.patch changes."""
    try:
        # Patch 1: outputs.py - add invalid_block_ids field
        _patch_outputs_invalid_block_ids()

        # Patch 2: base.py - add get_block_ids_with_load_errors
        _patch_base_get_block_ids_with_load_errors()
        
        # Patch 3: multi_connector.py - add get_block_ids_with_load_errors
        _patch_multi_connector_get_block_ids_with_load_errors()
        
        # Patch 4: block_pool.py - fix num_cached_blocks comparison
        _patch_block_pool_num_cached_blocks()
        
        # Patch 5: sched/output.py - add num_output_tokens field
        # _patch_CachedRequestData()
        
        # Patch 6: scheduler.py - add make_cached_request_data method
        _patch_scheduler()

        # Patch 7: single_type_kv_cache_manager.py - fix cache_blocks method
        _patch_single_type_kv_cache_manager_cache_blocks()

        # Patch 8: multiproc_executor.py
        _patch_multiproc_executor()

        # Patch 9: gpu_input_batch.py
        _patch_input_batch()

        # Patch 10: gpu_model_runner.py
        _patch_gpu_model_runner()

        # Patch 11: gpu_worker.py
        _patch_gpu_worker_execute_model()

        logger.debug("Applied aggre patch")
    except Exception as e:
        logger.error(f"Failed to apply aggre patch: {e}", exc_info=True)
        raise

# ==================== vllm/v1/outputs.py ====================
def _patch_outputs_invalid_block_ids() -> None:
...
def _patch_base_get_block_ids_with_load_errors() -> None:
    ...
    try:
        ...
        logger.warning("Could not patch Base.get_block_ids_with_load_errors - module not found")

# ==================== vllm/distributed/kv_transfer/kv_connector/v1/multi_connector.py ====================
def _patch_multi_connector_get_block_ids_with_load_errors() -> None:
    """Patch MultiConnector to add get_block_ids_with_load_errors."""
    try:
        ...
        logger.warning("Could not patch MultiConnector.get_block_ids_with_load_errors - module not found")

# ==================== vllm/v1/core/block_pool.py ====================
def _patch_block_pool_num_cached_blocks() -> None:
    ...
    try:
        ...
        def patched_cache_full_blocks(
            ...
            if num_cached_blocks >= num_full_blocks:
                return
            return original_cache_full_blocks(self,
                                              request,
                                              blocks, 
                                              block_hashes,
                                              num_cached_blocks,
...
def _patch_CachedRequestData() -> None:
...
def _patch_single_type_kv_cache_manager_cache_blocks() -> None:
    ...
    try:
        ...
        logger.warning("Could not patch SingleTypeKVCacheManager.cache_blocks - module not found")
    ...
    try:
        ...
        class KVOutputAggregator:
            ...
            def aggregate(self, outputs: list[ModelRunnerOutput], output_rank: int = 0) -> ModelRunnerOutput:
                ...
                def update_finished_list(req_ids: Optional[dict[str, list[str]]], remaining_count_dict: dict[str, int], finished_list: dict[str, list[str]]) -> None:
                ...
                for output in outputs:
                    ...
                    update_finished_list(output.finished_dumping, self._dump_remaining_count, finished_dumping)
                    if hasattr(output, 'invalid_block_ids') and output.invalid_block_ids:
                    ...
                    def callback(fut):
                        ...
                        if all(outputs):
                            result_future.set_result(
                                self.aggregate(cast(list[ModelRunnerOutput], outputs), output_rank)
                            )
                    return callback
        ...
        logger.warning("Could not patch MultiprocExecutor.execute_model - module not found")
    ...
    try:
        ...
        def __init__(self, *args, **kwargs):
            if not hasattr(self, 'is_token_ids'):
                max_num_reqs = kwargs.get('max_num_reqs', getattr(self, 'max_num_reqs', 10))
                max_model_len = kwargs.get('max_model_len', getattr(self, 'max_model_len', 512))
...
def _patch_gpu_model_runner() -> None:
    """Patch GPUModelRunner to add update_states method."""
    try:
        import copy
        ...
        logger.warning("Could not patch GPUModelRunner - module not found: {e}")
