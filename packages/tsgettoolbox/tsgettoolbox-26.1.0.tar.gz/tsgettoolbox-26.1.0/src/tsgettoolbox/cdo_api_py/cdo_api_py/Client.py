...
class BaseClient:
    ...
    def _test_auth(self):
        ...
        return (False, r) if r.status_code == 400 else (True, r)
...
class Client(BaseClient):
    ...
    def get(
        ...
        """
        if "limit" not in kwargs and self.default_limit is not None:
            kwargs["limit"] = self.default_limit
        if "units" not in kwargs and self.default_units is not None:
    ...
    def get_data_by_station(
        ...
        **kwargs,
        ...
        return data_df if return_dataframe else list(data_df.to_dict("index").values())
