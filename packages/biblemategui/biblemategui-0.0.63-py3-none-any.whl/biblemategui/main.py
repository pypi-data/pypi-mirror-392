...
def get_tooltip_data(word):
    ...
    with apsw.Connection(db) as connn:
        query = "SELECT * FROM morphology WHERE WordID=?"
        cursor = connn.cursor()
        cursor.execute(query, (word,))
        result = cursor.fetchone()

    """CREATE TABLE morphology (WordID INT, ClauseID INT, Book INT, Chapter INT, Verse INT, Word TEXT, LexicalEntry TEXT, MorphologyCode TEXT, Morphology TEXT, Lexeme TEXT, Transliteration TEXT, Pronunciation TEXT, Interlinear TEXT, Translation TEXT, Gloss TEXT)"""
    ...
    if result:
        ...
        return {'description': interlinear, 'links': ""}
    return None
