...
class CreditGrant(sgqlc.types.Type):
    __schema__ = schema
    __field_names__ = ('additional_meta_data', 'amount', 'comment', 'consumed_amount', 'cost', 'created_at', 'currency_id', 'customer_id', 'display_name', 'effective_at', 'expire_at', 'grant_id', 'grant_type', 'id', 'invoice_id', 'latest_invoice', 'payment_collection', 'priority', 'resource_id', 'status', 'updated_at', 'voided_at')
    additional_meta_data = sgqlc.types.Field(JSON, graphql_name='additionalMetaData')
    amount = sgqlc.types.Field(sgqlc.types.non_null(Float), graphql_name='amount')
    comment = sgqlc.types.Field(String, graphql_name='comment')
