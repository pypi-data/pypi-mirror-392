"""Extremly basic Markdown to reveal.js conversion tool."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("md", help="Markdown input file", type=Path)
    parser.add_argument(
        "-o", "--output", dest="html", help="HTML output file", type=Path
    )
    ...
    if not args.html:
        args.html = Path(str(args.md).replace(".md", ".html"))
    ...
    if not args.title:
        args.title = re.sub(".md$", "", str(args.md))

    return args


REVEAL_JS_VERSION = "4.1.0"
REVEAL_DIRNAME = f"reveal.js-{REVEAL_JS_VERSION}"
...
def download_reveal(root: Path) -> None:
    """If missing, download revealjs in root."""
    reveal_path = root / REVEAL_DIRNAME
    if reveal_path.exists():
        return
    tarball = io.BytesIO(requests.get(REVEAL_ARCHIVE, timeout=30).content)
...
def main() -> None:
    ...
    args.html.write_text(
        tpl.render(title=args.title, slides=sections, revealjs_url=REVEAL_DIRNAME),
        encoding="UTF-8",
    )
