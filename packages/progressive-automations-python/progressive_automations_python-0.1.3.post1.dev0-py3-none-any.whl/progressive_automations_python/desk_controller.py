...
def check_duty_cycle_status_before_execution() -> dict:
    ...
    movements_possible = (
        int(remaining_capacity / max_single_movement) if remaining_capacity > 0 else 0
    )

    # Display status
    print(
        f"Current usage: {current_usage:.1f}s / {DUTY_CYCLE_MAX_ON_TIME}s ({percentage_used:.1f}%)"
    )
...
def move_to_height(target_height: float) -> dict:
    ...
    if not (LOWEST_HEIGHT <= target_height <= HIGHEST_HEIGHT):
        raise ValueError(
            f"Target height {target_height}'' is out of range [{LOWEST_HEIGHT}-{HIGHEST_HEIGHT}]."
        )
    ...
    if state["last_position"] is None:
        cleanup_gpio()
        raise ValueError(
            "No last known position in state file. Initialize position first."
        )
    ...
    check_result = check_movement_against_duty_cycle(
    ...
    print(
        f"Duty cycle OK: {check_result['current_usage']:.1f}s + {check_result['estimated_duration']:.1f}s <= {DUTY_CYCLE_MAX_ON_TIME}s"
    )
    ...
    print(
        f"Duty cycle usage: {duty_status['current_usage']:.1f}s / {duty_status['max_usage']}s ({duty_status['percentage_used']:.1f}%)"
    )
