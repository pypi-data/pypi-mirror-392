...
from progressive_automations_python.config import DOWN_RATE, UP_RATE
...
def simple_movement_flow(target_height: float):
    ...
    logger.info(
        f"Initial duty cycle: {initial_status['current_usage']:.1f}s / {initial_status['window_period']}s used"
    )
    ...
    if not check_result["allowed"]:
        ...
        logger.warning(
            f"⏳ Duty cycle capacity insufficient - will wait {wait_time:.1f}s before movement"
        )
    ...
    # Log usage
    capacity_used = (
        initial_status["remaining_capacity"] - final_status["remaining_capacity"]
    )
    ...
    return {
        **result,
        ...
        "wait_time": (
            check_result.get("wait_time_needed", 0)
            if not check_result["allowed"]
            else 0
        ),
    }
...
def deploy_duty_cycle_monitoring(
    deployment_name: str = "duty-cycle-monitor", schedule_cron: str = None
):
    ...
    if schedule_cron:
        ...
        print(
            f"To run: prefect deployment run 'scheduled-duty-cycle-check/{deployment_name}'"
        )
    return deployment_name
