...
def record_usage_period(
    state: Dict[str, Any], start_time: float, end_time: float, duration: float
) -> Dict[str, Any]:
...
def check_movement_against_duty_cycle(
    target_height: float,
    ...
    if distance == 0:
        return {
            ...
            "remaining_capacity": DUTY_CYCLE_MAX_ON_TIME
            - get_current_duty_cycle_usage(state),
...
def show_duty_cycle_status():
    ...
    print(
        f"  Window period: {status['window_period']}s ({status['window_period']/60:.0f} minutes)"
    )
