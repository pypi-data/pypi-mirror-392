...
def create_deployments(work_pool_name: str = "desk-lifter-pool"):
    ...
    print(
        f"   run_deployment('simple-movement-flow/move-to-position', parameters={{'target_height': 30.0}}, timeout=0)"
    )
    print(f"3. Or from CLI:")
    print(
        f"   prefect deployment run 'simple-movement-flow/move-to-position' --param target_height=30.0"
    )
