...
def load_movement_configs(config_file: str = "movement_configs.json") -> list:
    ...
    enabled_movements = [
        m for m in config.get("movements", []) if m.get("enabled", True)
    ]
...
def validate_movement_config(movement: dict) -> dict:
    ...
    print(
        f"✅ Movement {movement_id} validated: {check_result['estimated_duration']:.1f}s, {check_result['movement_type']}"
    )
    return check_result
...
def execute_movement_config(movement: dict) -> dict:
    ...
    print(
        f"Executing configured movement {movement_id}: {movement.get('description', '')}"
    )
    ...
    if result["success"]:
        print(
            f"✅ Movement {movement_id} completed: {result['duration']:.1f}s, final height: {result['end_height']}\""
        )
    else:
...
def execute_custom_movements(config_file: str = "movement_configs.json") -> dict:
    ...
    for movement in movements:
        ...
        try:
            ...
            results.append(
                {"movement_id": movement_id, "success": False, "error": str(e)}
            )
