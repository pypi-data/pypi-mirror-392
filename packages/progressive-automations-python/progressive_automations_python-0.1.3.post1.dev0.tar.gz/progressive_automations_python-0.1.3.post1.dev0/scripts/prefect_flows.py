...
from duty_cycle import get_duty_cycle_status, load_state, show_duty_cycle_status
...
def duty_cycle_status_task():
    ...
    try:
        ...
        logger.info(
            f"  Usage: {status['current_usage']:.1f}s / 120s ({status['percentage_used']:.1f}%)"
        )
...
def execute_custom_movements_task(config_file: str = "movement_configs.json"):
    ...
    try:
        ...
        if result["success"]:
            logger.info(
                f"✅ All movements completed successfully ({result['successful']}/{result['total_movements']})"
            )
        else:
            logger.info(
                f"⚠️ Movements completed with some failures ({result['failed']}/{result['total_movements']} failed)"
            )
