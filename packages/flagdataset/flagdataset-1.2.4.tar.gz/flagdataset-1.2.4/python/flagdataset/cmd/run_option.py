...
def _download_with_ihttpd_runtime(cmd_args):
    try:
        try:
            ...
            print("请安装运行环境: pip install -U baai-flagdataset-rs")
            return
        ...
        multi_download(use_path, presign, network, cmd_args.bandwidth, jobs_down)
        ...
        try:
            ...
            while True:
                ...
                meta_next_path = meta_path / f"{cmd_args.dataset}_{offset+limit}.bin"
                if meta_next_path.exists():
                    offset = offset + limit
                    meta_push(f"{cmd_args.dataset}_{offset}.bin")
                    continue
...
def _download_with_ks3util_runtime(cmd_args):
    ...
    try:
        ...
        download_share(dataset, save_path, prefix, key, network=config.network, jobs_down=jobs_down, debug=debug)
