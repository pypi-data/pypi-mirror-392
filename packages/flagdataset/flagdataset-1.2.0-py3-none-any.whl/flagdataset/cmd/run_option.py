...
def runcmd_option():
    ...
    root_parser.add_argument('-b', '--bandwidth-down', type=int, default=100, help='带宽')
...
def _download_with_ihttpd_runtime(cmd_args):
    try:
        ...
        multi_download(use_path, presign, network, 200,  jobs_down)
...
def _download_with_ks3util_runtime(cmd_args):
    ...
    save_path = pathlib.Path(cmd_args.save_path).__str__()
    network = config.network
