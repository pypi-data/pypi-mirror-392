...
class RequestJob(Job):
    ...
    json: dict = {}
    headers: dict = {}
    cache_ttl: int | None = None  # None = use default, 0 = no cache, >0 = custom TTL, -1 = infinite
    ...
    def _is_cache_valid(self, cache_entry: CacheEntry, api_cache: 'APICache') -> bool:
        ...
        # None or -1 = infinite cache (always valid)
        if ttl is None or ttl == -1:
            return True

        # 0 = no cache (never valid)
        if ttl == 0:
            return False

        # Positive value = check age
    ...
    def execute(self, resources: dict[str, Resource | APICache], **kwargs):
