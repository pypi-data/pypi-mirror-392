...
class CodeChangedHandler(FileSystemEventHandler):
    ...
    def __init__(self, config, wait_seconds = 10):
        self.config = config
    ...
    def config_replace(self, value, version):
        if "$REGISTRY_IP" in value:
            if not sys.argv[1]:
                raise Exception("Need to pass local net ip of this machine as first parameter!")
            value = value.replace("$REGISTRY_IP", sys.argv[1])
    ...
    def run(self):
        ...
        version = f"0.0.{datetime.now().strftime("%m%d%H%S")}"
...
def main():
    ...
    event_handler = CodeChangedHandler(dog_settings)
    observer = Observer()
