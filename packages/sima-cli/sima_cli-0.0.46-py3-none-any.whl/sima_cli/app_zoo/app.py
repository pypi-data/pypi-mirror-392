...
def _list_external_apps(ver):
    ...
    while True:
        ...
        yaml_url = f"{base_url}/{entry['yaml_file']}"
        ...
        action = _prompt_user_action(selected_app)
        if action == "Download":
            click.echo(f"⬇️ Downloading YAML for '{selected_app}' ...")
            download_file_from_url(yaml_url, dest_folder=selected_app, internal=False)
        elif action == "Exit":
            break
...
def describe_app(internal, ver, model_name):
    if internal:
        ...
        click.echo("❌ External direct describe not supported. Use list_apps instead.")
...
def download_app(internal, ver, model_name):
    if internal:
        ...
        click.echo("❌ External ZIP download not supported.")
