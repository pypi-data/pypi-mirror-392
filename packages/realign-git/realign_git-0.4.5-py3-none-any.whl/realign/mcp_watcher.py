...
class DialogueWatcher:
    ...
    async def _check_if_turn_complete(self, session_file: Path) -> bool:
        ...
        A complete turn has:
        1. A user message (actual user input, not just tool results)
        2. An assistant message with stop_reason (indicating response finished)
        """
        try:
            ...
            for line in check_lines:
                ...
                # Detect user turn (actual user input, not tool results)
                if msg_type == "user":
                    content = message.get("content", [])
                    if isinstance(content, str):
                        has_user_turn = True
                    elif isinstance(content, list) and content:
                        # Check if it's not just tool results
                        has_non_tool_result = any(
                            isinstance(item, dict) and item.get("type") != "tool_result"
                            for item in content
                        )
                        if has_non_tool_result or isinstance(content[0], str):
                            has_user_turn = True

                # Detect assistant completion (stop_reason field)
                elif msg_type == "assistant" and message.get("role") == "assistant":
                    if message.get("stop_reason"):
                        has_stop_reason = True

            return has_user_turn and has_stop_reason
