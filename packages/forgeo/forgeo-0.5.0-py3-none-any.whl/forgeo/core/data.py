...
class DipDirMeasurements:
    ...
    def __attrs_post_init__(self):
        if np.any(self.dips < 0) or np.any(self.dips > 90):
            raise ValueError("expecting a positive angle in degree for dips")
        if np.any(self.dirs < 0) or np.any(self.dirs > 360):
            raise ValueError("expecting a positive angle in degree for dirs")
        if self.reverse_polarities is not None and len(self.reverse_polarities) != len(
            self.values
        ):
            raise ValueError("expecting the same number of dips and polarities")
...
class Orientations:
    ...
    def __attrs_post_init__(self):
        if (
            ...
            raise ValueError("expecting locations and normals with the same dimension")
        if len(self.locations) != len(self.normals):
            raise ValueError("expecting the same number of locations and normals")
...
class ItemData:
    ...
    def __attrs_post_init__(self):
        if not (self.observations or self.orientations or self.orientations_only):
            raise ValueError("items must have observations or orientations data")
        if self.observations:
            if (
                ...
                raise ValueError(
                    "expecting observations and orientations with the same dimension"
                )
    ...
    def all_orientations(self) -> Orientations:
        ...
        if nb_arrays == 0:
            ...
            return Orientations(
                Locations(
                    np.vstack([data[0].locations.values, data[1].locations.values])
                ),
                UnitNormals(
                    np.vstack([data[0].normals.values, data[1].normals.values])
                ),
            )
...
class InterpolationMethod:
    ...
    RASTER = "raster"
    _AUTHORIZED_METHODS = [POTENTIAL, ELEVATION_KRIGING]
    _DEFAULT = POTENTIAL

    @classmethod
    def list_available_methods(cls):
        # Returns a copy to forbid modification
        return [method for method in cls._AUTHORIZED_METHODS]
...
class Interpolator:
    ...
    def references_are_resolved(self):
        if self.dataset:
            if any(not isinstance(item, Item) for item in self.dataset):
                return False
        return True
...
class Neighborhood:
    ...
    def __init__(
        ...
        *,
        ...
        nb_max_neighbors: int = None,
        nb_min_neighbors: int = None,
        nb_angular_sectors: int = None,
        nb_max_neighbors_per_sector: int = None,
    ):
        kwargs = {**locals()}
