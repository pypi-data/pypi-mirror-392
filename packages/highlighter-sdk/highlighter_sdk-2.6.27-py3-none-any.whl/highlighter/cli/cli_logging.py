...
TERSE_LOG_FORMAT = "%(asctime)s [%(levelname).3s] %(message)s   |> %(name)s:%(lineno)d"
...
class ColourFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if stream_id is not None:
            stream_prefix = format_stream_info(stream_id, capability_name)
        ...
        tail = f"|> {record.name}:{record.lineno}"
...
def format_stream_info(stream_id, capability_name=None):
    ...
    Returns:
        Formatted string with colors: "<Stream <id>|Capability>"

    Example:
        format_stream_info("7", "VideoDataSource")
        # Returns colored: "Stream 7 [VideoDataSource]"
    """
