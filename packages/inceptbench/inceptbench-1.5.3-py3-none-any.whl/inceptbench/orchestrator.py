...
def _item_requires_localization(item, default_language: Optional[str] = None) -> bool:
    ...
    return localization_info.language not in ["en", "english"]
...
class UniversalEvaluationRequest(BaseModel):
    ...
    def _determine_submodules(self) -> List[str]:
        ...
        # Localization evaluator - run for localized (non-English) content
    ...
    def _is_localized(self) -> bool:
        """
        Determine if content is localized (non-English) by checking:
        1. The language parameter
        2. The skill.language field in questions, content, or articles
        3. Actual content language detection (if language field not available)
        """
        # Check if language/locale parameter is non-English
        request_locale = self.locale or self.language
        if request_locale:
            lang, _, _ = _parse_locale(request_locale)
        ...
        if self.generated_questions:
            for question in self.generated_questions:
                localization_info = _determine_item_language(question, request_locale)
                if localization_info.language not in ["en", "english"]:
                    return True

        # Check content items
        if self.generated_content:
            for content in self.generated_content:
                localization_info = _determine_item_language(content, request_locale)
                if localization_info.language not in ["en", "english"]:
                    return True
        ...
        if self.generated_articles:
            for article in self.generated_articles:
                localization_info = _determine_item_language(article, request_locale)
                if localization_info.language not in ["en", "english"]:
                    return True
...
def _run_localization_evaluator_task(item_idx: int, item, default_language: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def _async_task():
        ...
        try:
            ...
            inferred_language = localization_info.inferred
            
            # If English, skip localization evaluation (shouldn't happen, but safety check)
            if target_language in ["en", "english"] and not inferred_language:
                logger.debug(f"Item {item_idx} is English, skipping localization evaluation")
                return {
                    'question_idx': item_idx,
                    'result': None,
                    'skip_reason': 'english_content'
                }
...
def universal_unified_benchmark(request: UniversalEvaluationRequest) -> UniversalEvaluationResponse:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        # Works for both questions and text content (only if localized/non-English)
    ...
    for question_id, evaluation in evaluations.items():
        ...
        if evaluation.localization_evaluator:
            ...
            scores_to_combine.append(localization_score)
