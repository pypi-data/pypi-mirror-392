...
@overload
def __note_to_degree(key: _IterableLike[str]) -> jnp.ndarray:
    ...
...
@overload
def __simplify_note(key: _IterableLike[str], additional_acc: str = ..., unicode: bool = ...) -> jnp.ndarray:
    ...


@overload
def __simplify_note(key: Union[str, _IterableLike[str], Iterable[str]], additional_acc: str = ...,
                    unicode: bool = ...) -> Union[str, jnp.ndarray]:
    ...
...
def __mode_to_key(signature: str, unicode: bool = True) -> str:
    ...
    if match.group('scale') or not match.group("mode"):
        ...
        signature = __simplify_note(match.group("tonic").upper() + match.group('accidental'), unicode=unicode) + (
...
def key_to_degrees(key: str) -> jnp.ndarray:
    ...
    """
    notes = dict(
        maj=jnp.array([0, 2, 4, 5, 7, 9, 11]), min=jnp.array([0, 2, 3, 5, 7, 8, 10])
    )
    ...
    if match.group('mode') or not match.group('scale'):
        ...
        return jnp.roll(key_to_degrees(equiv), -offset)
