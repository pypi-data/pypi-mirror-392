    ...
    *,
    ...
    if isinstance(intervals, str):
        if intervals == "equal":
            ...
            ratios = 2.0 ** (
                (tuning + jnp.arange(0, bins_per_octave, dtype=float)) / bins_per_octave
            )
    ...
    n_octaves = jnp.ceil(n_bins / bins_per_octave)
    all_ratios = jnp.multiply.outer(2.0 ** jnp.arange(n_octaves), ratios).flatten()[
        :n_bins
    ]

    if sort:
        all_ratios = jnp.sort(all_ratios)
...
def pythagorean_intervals(
    *, bins_per_octave: int = 12, sort: bool = True, return_factors: bool = False
    ...
    pow2: jnp.ndarray
    log_ratios, pow2 = jnp.modf(pow3 * jnp.log2(3))
...
def __harmonic_distance(logs, a, b):
    ...
    a_num = jnp.maximum(a, 0)
    # denominator = negative exponents
    a_den = a_num - a

    b_num = jnp.maximum(b, 0)
    b_den = b_num - b
    ...
    gcd = jnp.minimum(a_num, b_num) - jnp.maximum(a_den, b_den)

    # Rounding this to 6 decimals to avoid floating point weirdness
    return jnp.around(logs.dot(a + b - 2 * gcd), 6)
...
def plimit_intervals(
    *,
    ...
    pows = jnp.array(list(intervals), dtype=float)

    log_ratios: jnp.ndarray
    pow2: jnp.ndarray
    log_ratios, pow2 = jnp.modf(pows.dot(logs))
