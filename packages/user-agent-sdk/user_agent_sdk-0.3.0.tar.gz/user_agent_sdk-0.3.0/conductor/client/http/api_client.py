...
class ApiClient(object):
    ...
    def __call_api_no_retry(
        ...
        # auth setting
        auth_headers = None
        if self.configuration.authentication_settings is not None and resource_path != self.configuration.authentication_settings.auth_url:
    ...
    def __get_new_token(self) -> str:
        try:
            ...
            response = requests.post(
                self.configuration.authentication_settings.auth_url,
                data={
                    'grant_type': 'client_credentials',
                    'client_id': self.configuration.authentication_settings.key_id,
                    'client_secret': self.configuration.authentication_settings.key_secret
                },
                headers={
            ...
            if expires_in:
                good_expires_in = int(expires_in) - 30  # Subtract 30 seconds for safety
                self.configuration.auth_token_ttl_msec = good_expires_in * 1000
            return jwt.get("access_token")
        except Exception as e:
            logger.error(f'Failed to get new token, reason: {e}')
            return None
