...
class CameraStreamer:
    ...
    def __init__(
        ...
        # Add high-performance configuration for Redis
        if self.server_type == "redis":
            self.stream_config.update({
                'pool_max_connections': 50,  # Connection pooling for high throughput
                'enable_batching': True,      # Enable message batching
                'batch_size': 50,             # Batch 50 frames before flushing
                'batch_timeout': 0.1          # Flush every 100ms if batch not full
            })
    ...
    def _process_and_send_frame(
        ...
        # Encode frame (ZERO-COPY optimized)
