...
from codec_cub.common import TWO, extract, first_token, unescape_string as unescaped_str
...
class _NixParser:
    ...
    def _build(self) -> pp.ParserElement:
        ...
        pp.ParserElement.set_default_whitespace_chars(f" {ch.TAB}{ch.CARRIAGE_RETURN}{ch.NEWLINE}")
        ...
        def unescape_string(tokens: list[str]) -> list[Any]:
            ...
            text: str = extract(first_token(tokens))
        ...
        def to_bool(tokens: list) -> bool:
            ...
            return parse_bool(first_token(tokens))
        ...
        def to_attrset(tokens: list[Any]) -> dict[str, Any]:
            ...
            result: dict[str, Any] = {}
            actual_pairs: list[Any] | pp.ParseResults = first_token(tokens) if _is_nested(tokens) else tokens
