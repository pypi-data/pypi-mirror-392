...
def test_unescape_string_invalid_escape_sequences(bad_sequence: str) -> None:
    with pytest.raises(
        ValueError, match=rf"Invalid escape sequence: \\{bad_sequence}"
    ):
...
def test_first_token_nested_structure() -> None:
    nested = [[1, 2], {"key": "value"}]
    assert first_token(nested) is nested[0]


def test_first_token_tuple() -> None:
