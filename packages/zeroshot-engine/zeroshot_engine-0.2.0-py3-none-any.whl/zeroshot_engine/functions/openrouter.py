...
def setup_openrouter_api_key():
    """Set up the OpenRouter API key for use with the demo."""
    # Check for OpenRouter API key (in environment variables first)
                    ...
                    for line in lines:
                        if line.strip().startswith("OPENROUTER_API_KEY="):
                            ...
                            if api_key:
                                ...
                                print("✅ Found OpenRouter API key in .env file")
                                break
    ...
    if not api_key:
        ...
        print("--------------------------------")
        print(
            "• Your OpenRouter API key is stored locally only in the .env file or temporary environment variable"
        )
        print("• The key is never transmitted to any third-party servers")
        print(
            "• API calls are made to OpenRouter's API which routes to various AI providers\n"
        )
        print(
        ...
        print("\n❌ No OpenRouter API key found in environment or .env file.")
        print(
            "⚠️  Note: Using the OpenRouter API will incur charges to your OpenRouter account\n"
        )

        print("Please provide your OpenRouter API key to continue.")
