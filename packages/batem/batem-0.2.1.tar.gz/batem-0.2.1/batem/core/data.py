...
_RESULTS_FOLDER = Setup.folder_path('results')
_DATA_FOLDER = Setup.folder_path('data')
...
class ParameterSet(VariableSet):
    ...
    def load(self, file_name: str = "best_parameters"):
        ...
        :type file_name: str
        """
        data_json_path = _DATA_FOLDER / f"{file_name}.json"
        if data_json_path.exists():
            with open(_DATA_FOLDER / file_name, 'r', encoding='utf-8') as file:
...
class IndependentVariableSet(VariableSet):
            ...
            for variable_name in self.site_weather_data.variable_names:
                if variable_name not in all_deleted_variables and variable_name != "epochtimems":
                    ...
                    if len(values) != len(self._dataframe):
                        print(f"Attempting to align weather data for '{variable_name}' with measurement timestamps...")
    ...
    def _align_weather_data(self, weather_values: list[float], variable_name: str) -> list[float] | None:
        ...
        try:
            ...
            print(f"Successfully aligned weather data for '{variable_name}': {len(weather_values)} -> {len(aligned_values)}")
