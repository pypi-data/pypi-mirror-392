...
### Added
- Comprehensive documentation for Pristy pivot format and `__dag_param` internal metadata
- Validation warnings for missing or incomplete `__dag_param` in `SaveNodeToSqliteOperator`
- Test coverage for `__dag_param` validation scenarios
- 
### Changed
- **BREAKING**: Operators must now store `isFile`/`isFolder` in `__dag_param` instead of pivot format
- Refactored `SaveNodeToSqliteOperator.execute()` to reduce cognitive complexity
- Optimized SQL operations in `SaveNodeToSqliteOperator` using UPSERT pattern

## [0.8.0] - 2025-01-17

### Changed
- **BREAKING**: SQLite schema now uses `isFile INTEGER` and `isFolder INTEGER` instead of `node_type TEXT`
- Changed file size extraction from `source.size` to `content.sizeInBytes` to match Alfresco API structure
- Uses Alfresco native `isFile`/`isFolder` boolean fields instead of deriving from `type` field (`cm:content`, `cm:folder`)
- Fixed Airflow 3.1+ deprecation warnings for Variable import in `transform_file.py`, `push_node_to_kafka.py`, `update_node_path.py`
- Changed from `airflow.models.Variable` to `airflow.sdk.Variable` for Airflow 3.1+ compatibility

## [0.7.7] - 2025-01-14

### Fixed
- Fixed `TypeError: expected str, bytes or os.PathLike object, got 'PlainXComArg'` in `SaveNodeToSqliteOperator`
- Supports both direct string paths and dict outputs (extracts `sqlite_path` key from dict)
