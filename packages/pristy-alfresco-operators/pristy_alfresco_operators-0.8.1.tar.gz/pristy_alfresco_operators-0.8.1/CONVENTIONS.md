...
### Pristy Pivot Format Compliance

All operators that transform Alfresco nodes **MUST** produce output conforming to the Pristy InjectorNode format defined in the [Pristy Injector documentation](https://gitlab.com/pristy-oss/pristy-core/-/blob/develop/docs/PristyInjector.md).

**Allowed pivot fields:**
- `name`, `type`, `path`, `properties`, `source`, `dateCreated`, `owner`, etc. (see InjectorNode schema)

**Forbidden in pivot:**
- Custom fields like `isFile`, `isFolder`, `uuid`, etc.
- Internal processing metadata

#### Internal Metadata Storage

Use the `__dag_param` field for DAG-internal metadata that should **NOT** be sent to Pristy Injector:

```python
# ✅ CORRECT - Metadata in __dag_param
pivot_node = create_base_node(node)
pivot_node["__dag_param"] = {
    "parent_uuid": folder_uuid,
    "isFile": node.get("isFile", False),
    "isFolder": node.get("isFolder", False),
}

# ❌ INCORRECT - Custom fields in pivot
pivot_node = create_base_node(node)
pivot_node["isFile"] = True  # Violates InjectorNode spec!
```

#### Reading Internal Metadata

Always read DAG metadata from `__dag_param`:

```python
# ✅ CORRECT
isFile = node.get("__dag_param", {}).get("isFile", False)

# ❌ INCORRECT
isFile = node.get("isFile", False)  # Expects pivot violation
```

#### Validation Checklist

Before sending nodes to Kafka or Pristy Injector API:
1. Ensure no `__dag_param` field is present (strip it if needed)
2. Validate against InjectorNode JSON Schema
3. Verify no custom fields were added to pivot
