...
class SaveNodeToSqliteOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        # Si c'est un dict (output complet), extraire la clé sqlite_path
        ...
        self.log.info(
            f"Saving {len(nodes_resolved)} nodes to SQLite database: {sqlite_path_resolved}"
        )
        ...
        try:
            with sqlite3.connect(sqlite_path_resolved) as conn:
                ...
                for node in nodes_resolved:
                    ...
                    # Extract node metadata from Alfresco API
                    isFile = 1 if node.get("isFile", False) else 0
                    isFolder = 1 if node.get("isFolder", False) else 0
