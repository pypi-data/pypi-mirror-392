...
class ProverAgent:
    ...
    def write_proof_to_latex(self, theorem: Theorem, proof_latex: str) -> bool:
        ...
        if not theorem.tex_file or not theorem.tex_file.exists():
            print(f"  ✗ Cannot write proof: source file not found")
            return False

        print(f"\n📝 Writing proof to {theorem.tex_file.name}...")

        try:
            with open(theorem.tex_file, 'r') as f:
                lines = f.readlines()
    ...
    def _call_claude(self, prompt: str, timeout: int = 180) -> str:
        """Call Claude for proof generation/verification"""
        try:
...
Generate 5-15 detailed steps."""
        ...
        response = self._call_claude(prompt, timeout=240)
