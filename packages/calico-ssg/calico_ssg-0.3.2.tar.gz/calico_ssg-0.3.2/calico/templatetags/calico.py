...
def translations(page):
    return [(f'/{pre}/' if pre else '/', lang) for pre, lang in calico_setting('LANGUAGES') if pre != page.language]
...
class SourceSet(TemplateTag):
    ...
    def render(self, image_path, divide_by=1, **kwargs):
        tn = Thumbnailer(image_path, divide_by=divide_by)
        return {'original': tn.max_res or tn.original_url, 'sizes': tn.sizes, 'srcset': tn.srcset, **kwargs}
