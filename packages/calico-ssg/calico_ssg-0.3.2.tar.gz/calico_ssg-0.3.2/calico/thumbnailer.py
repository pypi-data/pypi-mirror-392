...
class Thumbnailer:
    def __init__(self, filename, divide_by=1):
        ...
        self.thumbs = []
        for _name, size in calico_setting('THUMBS').items():
            if size is None:
            ...
            url = os.path.join(self.original_url.rsplit('/', 1)[0], calico_setting('THUMBS_SUBDIR'), target_filename)
            if size is None:
                self.max_res = url
            else:
    ...
    def sizes(self):
        return ', '.join(
            [f'() {t.adjusted_size[0]:.0f}' for t in self.thumbs]
            + [
    ...
    @property
    def srcset(self):
        return ', '.join([f'{static(t.url)} {t.adjusted_size[0]:.0f}w' for t in self.thumbs])
