...
class RefactoredPublicDomainValidator:
    ...
    async def is_likely_public_domain_with_details(
        ...
        self.audit_logger.log_pd_determination(
            item, result, method="heuristic_analysis"
        )
    ...
    def assess_use_risk(
        ...
        if author_death_year:
            ...
            if (
                years_since_death
                >= self.country_law_manager.copyright_terms.get("worldwide", 70) - 10
    ...
    def export_configuration_to_json(
        self, config_dir: Union[str, Path, None] = None
    ) -> Dict[str, str]:
        """
        ...
        heuristic_indicators = (
            self.heuristic_analyzer.indicators
            if hasattr(self.heuristic_analyzer, "indicators")
            else HeuristicIndicators([], [], [], [], [])
        )

        heuristic_file = config_dir / "heuristic_indicators.json"
        heuristic_data = {
            "heuristic_indicators": {
                "title_pd_indicators": getattr(
                    heuristic_indicators, "title_pd_indicators", []
                ),
                "content_pd_indicators": getattr(
                    heuristic_indicators, "content_pd_indicators", []
                ),
                "historical_authors": getattr(
                    heuristic_indicators, "historical_authors", []
                ),
                "time_period_indicators": getattr(
                    heuristic_indicators, "time_period_indicators", []
                ),
                "genre_pd_indicators": getattr(
                    heuristic_indicators, "genre_pd_indicators", []
                ),
            },
            "generated_at": datetime.now().isoformat(),
            "version": "1.0",
