...
def _decision_node_to_dict(node):
    ...
    return {
        ...
        "result": node.result,
        "is_pd": node.is_pd,
        "children": [_decision_node_to_dict(child) for child in node.children] if node.children else []
    }
...
def load_country_special_rules() -> dict[str, dict]:
    ...
    if config_path.exists():
        ...
        return {
            "US": {
                "publication_before_1928": {"threshold_year": 1928, "is_public_domain": True, "explanation": "Published before 1928 in the US (now in public domain)"},
                "publication_1923_1977": {"publication_term": 95, "explanation": "Published 1923-1977 in the US (95 years from publication)"},
                "corporate_works": {"term": 95, "explanation": "Corporate authorship in the US (95 years from publication or 120 years from creation, whichever is shorter)"},
                "government_works": {"is_public_domain": True, "explanation": "US government works are not copyrighted"}
            },
            "EU_WARTIME": {
            ...
            "GOVERNMENT_WORKS": {
                "US": {"is_public_domain": True, "explanation": "US government works are not copyrighted"},
                "UK": {"term": 50, "publication_only": True, "explanation": "Crown copyright in UK expires 50 years after publication"},
                "CA": {"term": 50, "publication_only": True, "explanation": "Crown copyright in Canada expires 50 years after publication"},
                "AU": {"term": 50, "publication_only": True, "explanation": "Government works in Australia expire 50 years after publication"},
                "IN": {"term": 60, "publication_only": True, "explanation": "Government works in India expire 60 years after publication"}
            },
            "ANONYMOUS_WORKS": {
                "US": {"term": 95, "explanation": "Anonymous works in US: 95 years from publication or 120 years from creation, whichever is shorter"},
                "UK": {"term": 70, "explanation": "Anonymous works in UK: 70 years after publication"},
                "EU": {"term": 70, "explanation": "Anonymous works in EU: 70 years after publication"},
                "default": {"term": 70, "explanation": "Anonymous works: 70 years after publication"}
            },
            "CORPORATE_WORKS": {
                "US": {"term": 95, "or_creation": 120, "explanation": "Corporate authorship in US: 95 years from publication or 120 years from creation"},
                "UK": {"term": 70, "explanation": "Corporate authorship in UK: 70 years after publication"},
                "default": {"term": 70, "explanation": "Corporate authorship: 70 years after publication"}
            }
        }
...
class PublicDomainDecisionTree:
    ...
    def assess_public_domain_status(
        ...
        if publication_year is not None:
            ...
            if author_death_year is not None:
                ...
                result["explanation"].append("Cannot determine public domain status without publication year or author death year")
    ...
    def _assess_pre_1928_publication(self, result: dict, publication_year: int, country: str) -> dict:
    ...
    def _assess_post_1977_publication(
        ...
        if work_type == "individual":
            ...
            if author_death_year:
                pd_year = author_death_year + 70 + 1  # +1 since copyright lasts for the year of death too
                result["is_public_domain"] = pd_year <= datetime.now().year
                result["pd_year"] = pd_year
                result["years_until_pd"] = max(0, pd_year - datetime.now().year)
                result["explanation"].append(
    ...
    def _assess_non_us_publication(
        ...
        copyright_term = COUNTRY_COPYRIGHT_TERMS.get(country.upper(), COUNTRY_COPYRIGHT_TERMS.get(nationality.upper(), 70))

        if work_type == "individual" and author_death_year:
            # Use life + copyright_term rule
            pd_year = author_death_year + copyright_term + 1  # +1 since copyright lasts for the year of death too
            result["is_public_domain"] = pd_year <= datetime.now().year
            result["pd_year"] = pd_year
            result["years_until_pd"] = max(0, pd_year - datetime.now().year)
            result["explanation"].append(
...
class PublicDomainValidator:
    ...
    def __init__(self) -> None:
        ...
        self.title_pd_indicators = HEURISTIC_INDICATORS.get("title_pd_indicators", [
            "public domain",
            "public domain,",
            "public domain.",
            "pd-",
        ...
        self.content_pd_indicators = HEURISTIC_INDICATORS.get("content_pd_indicators", [
            "public domain",
            "copyright expired",
            "pd-",
            "pd ",
        ...
        self.historical_authors = HEURISTIC_INDICATORS.get("historical_authors", [
            "shakespeare",
            "darwin",
            "marx",
            "lincoln",
        ...
        self.time_period_indicators = HEURISTIC_INDICATORS.get("time_period_indicators", [
            "19th century",
            "18th century",
            "17th century",
            "16th century",
        ...
        self.genre_pd_indicators = HEURISTIC_INDICATORS.get("genre_pd_indicators", [
            "ancient greek",
            "ancient roman",
            "classic",
            "by publication",
    ...
    async def is_likely_public_domain_with_details(
        ...
        title_check_node = DecisionNode(description=f"Check title for PD indicators: '{item.title}'", result=None)
        root_node.children.append(title_check_node)
        if self._check_title_for_pd_indicators(item.title):
            explanations.append(f"Title '{item.title}' contains public domain indicators")
            confidence = max(confidence, 90)
            title_check_node.result = "Found PD indicators"
            title_check_node.is_pd = True
            result = {"is_public_domain": True, "explanation": explanations, "confidence": confidence, "decision_tree": _decision_node_to_dict(root_node)}
            self.log_pd_determination(item, result, method="title_indicators")
            self._validation_cache.set(title=item.title, result=result, content=item.content, snippet=item.snippet)
            return result
        else:
        ...
        content_check_node = DecisionNode(description="Check content for PD indicators", result=None)
        ...
        heuristic_check_node = DecisionNode(description="Apply heuristic checks", result=None)
        ...
        if self._apply_heuristic_checks(item.title, content_text):
            ...
            if any(author in title_lower for author in self.historical_authors):
                matching_authors = [author for author in self.historical_authors if author in title_lower]
                explanation = f"Work associated with historical author(s): {', '.join(matching_authors[:3])}"
                heuristic_check_node.children.append(DecisionNode(description="Historical author check", result=explanation, is_pd=True))
                explanations.append(explanation)
                confidence = max(confidence, 80)
            elif any(period in combined_text for period in self.time_period_indicators):
                matching_periods = [period for period in self.time_period_indicators if period in combined_text]
                explanation = f"Work associated with historical time period: {', '.join(matching_periods[:3])}"
                heuristic_check_node.children.append(DecisionNode(description="Time period check", result=explanation, is_pd=True))
                explanations.append(explanation)
                confidence = max(confidence, 75)
            elif any(genre in combined_text for genre in self.genre_pd_indicators):
                matching_genres = [genre for genre in self.genre_pd_indicators if genre in combined_text]
                explanation = f"Work belongs to public domain genre: {', '.join(matching_genres[:3])}"
                heuristic_check_node.children.append(DecisionNode(description="Genre check", result=explanation, is_pd=True))
                explanations.append(explanation)
        ...
        final_node = DecisionNode(description="Final Result", result="No public domain indicators found", is_pd=False)
        ...
        result = {"is_public_domain": False, "explanation": explanations, "confidence": confidence, "decision_tree": _decision_node_to_dict(root_node)}
        self.log_pd_determination(item, result, method="no_indicators_found")
        self._validation_cache.set(title=item.title, result=result, content=item.content, snippet=item.snippet)
        return result

    def _generate_dot_from_tree(self, node, dot_lines: list[str], parent_id: str | None = None) -> str:
    ...
    def _check_content_for_pd_indicators(self, content: str) -> bool:
        ...
        return any(indicator in content_lower for indicator in self.content_pd_indicators)
    ...
    def _apply_publication_rules(self, publication_year: int, country: str, current_year: int, pd_info: dict[str, Any]) -> dict[str, Any]:
        """Apply country-specific publication-based rules to determine PD status."""
        country_upper = country.upper()
        copyright_term = COUNTRY_COPYRIGHT_TERMS.get(country_upper, COUNTRY_COPYRIGHT_TERMS["worldwide"])
        ...
        if country_upper == "US":
            ...
            # Check if it's a pre-1928 publication
            pre_1928_rule = us_rules.get("publication_before_1928")
            if pre_1928_rule and publication_year < pre_1928_rule["threshold_year"]:
                pd_info["is_public_domain"] = True
                pd_info["pd_year"] = publication_year
                pd_info["copyright_term"] = pre_1928_rule["explanation"]
                pd_info["explanation"].append(pre_1928_rule["explanation"])
                return pd_info
            
            # Check 1923-1977 rule
            pub_rule_1923_1977 = us_rules.get("publication_1923_1977")
            if pub_rule_1923_1977 and 1923 <= publication_year <= 1977:
                pd_year = publication_year + pub_rule_1923_1977["publication_term"]
                pd_info["is_public_domain"] = pd_year <= current_year
                pd_info["pd_year"] = pd_year
                pd_info["years_until_pd"] = max(0, pd_year - current_year) if not pd_info["is_public_domain"] else 0
                pd_info["copyright_term"] = pub_rule_1923_1977["explanation"]
                pd_info["explanation"].append(pub_rule_1923_1977["explanation"])
                return pd_info
        ...
        if (current_year - publication_year) > copyright_term:
            pd_info["is_public_domain"] = True
            pd_info["pd_year"] = publication_year
            pd_info["copyright_term"] = f"Published more than {copyright_term} years ago ({country})"
            pd_info["explanation"].append(f"Published in {publication_year}, which is more than {copyright_term} years ago (copyright term in {country})")
        else:
            ...
            pd_info["copyright_term"] = f"Published and protected for {copyright_term} years after publication ({country})"
            pd_info["explanation"].append(f"Published in {publication_year}, copyright expires in {remaining_years} years (in {pd_info['pd_year']})")
    ...
    def calculate_pd_status_from_copyright_info(
            ...
            if gov_rules:
                if gov_rules.get("is_public_domain"):
                    ...
                    if gov_rules.get("publication_only") and publication_year:
                        ...
                        pd_info["pd_year"] = pd_year
                        pd_info["years_until_pd"] = max(0, pd_year - current_year) if not pd_info["is_public_domain"] else 0
                        pd_info["copyright_term"] = f"Government work: {gov_rules['explanation']}"
                        pd_info["expiry_type"] = "publication_based_government_work"
                        pd_info["primary_expiry_basis"] = f"publication_in_{publication_year}_plus_{term}_years_government"
                        pd_info["explanation"].append(gov_rules["explanation"])
                        return pd_info
        ...
        if work_type == "corporate":
            corp_rules = COUNTRY_SPECIAL_RULES["CORPORATE_WORKS"].get(country_upper) or COUNTRY_SPECIAL_RULES["CORPORATE_WORKS"]["default"]
            copyright_term = corp_rules["term"]
            pd_info["explanation"].append(corp_rules["explanation"])
        elif work_type == "anonymous":
            anon_rules = COUNTRY_SPECIAL_RULES["ANONYMOUS_WORKS"].get(country_upper) or COUNTRY_SPECIAL_RULES["ANONYMOUS_WORKS"]["default"]
        ...
        # For US works published before 1923, they should be in PD regardless of author death date
        if country_upper == "US" and publication_year and publication_year < 1923:
            pd_info["is_public_domain"] = True
            pd_info["pd_year"] = publication_year
            pd_info["copyright_term"] = "Published before 1923 in the US (now in public domain)"
            pd_info["expiry_type"] = "publication_based"
            pd_info["primary_expiry_basis"] = f"published_before_1923_in_US"
            pd_info["explanation"].append("Published before 1923 in the US (now in public domain)")
            return pd_info

        # For US works published 1923-1977, we need to consider both publication-based and life-based terms
        # and take the earlier expiration date
        elif country_upper == "US" and author_death_year and publication_year and 1923 <= publication_year <= 1977:
            ...
            pub_rule = COUNTRY_SPECIAL_RULES.get("US", {}).get("publication_1923_1977", {})
            pub_term = pub_rule.get("publication_term", 95)  # Default to 95 years if not specified
            ...
            pd_info["years_until_pd"] = max(0, actual_pd_year - current_year) if not pd_info["is_public_domain"] else 0
            ...
            if actual_pd_year == life_based_pd_year and actual_pd_year == publication_based_pd_year:
                ...
                pd_info["expiry_type"] = "publication_based"
                pd_info["primary_expiry_basis"] = f"publication_in_{publication_year}_plus_{pub_term}_years"
            else:
                pd_info["expiry_type"] = "author_death_based"
                pd_info["primary_expiry_basis"] = f"author_death_in_{author_death_year}_plus_{copyright_term}_years"
            ...
            pd_info["explanation"].append(f"US 1923-1977 rule: {pub_explanation}; {life_explanation}. Earlier date applied: {actual_pd_year} - {'Public domain' if pd_info['is_public_domain'] else 'Still under copyright'}. Expiry type: {pd_info['expiry_type']}")

            # Check for wartime extensions in certain EU countries (though this is US-only, keeping for completeness)
            if country_upper in COUNTRY_SPECIAL_RULES["EU_WARTIME"]["extension_countries"]:
                # For specific historical contexts, copyright may be extended
                extended_pd_year = author_death_year + copyright_term + COUNTRY_SPECIAL_RULES["EU_WARTIME"]["extension_years"] + 1
                if current_year < extended_pd_year and current_year - author_death_year <= 100:  # WWII era author
                    pd_info["is_public_domain"] = False
                    ...
                    pd_info["explanation"].append(COUNTRY_SPECIAL_RULES["EU_WARTIME"]["explanation"])
                    pd_info["copyright_term"] = f"life + {copyright_term} years + wartime extension ({COUNTRY_SPECIAL_RULES['EU_WARTIME']['extension_years']} years)"
            ...
            pd_info["pd_year"] = pd_year
            pd_info["years_until_pd"] = max(0, pd_year - current_year) if not pd_info["is_public_domain"] else 0
            pd_info["copyright_term"] = f"life + {copyright_term} years ({country_upper})"
            pd_info["expiry_type"] = "author_death_based"
            pd_info["primary_expiry_basis"] = f"author_death_in_{author_death_year}_plus_{copyright_term}_years"
            pd_info["explanation"].append(f"Author died in {author_death_year}, copyright expires {pd_year} ({copyright_term} years after death)")
            ...
            if country_upper == "US":
                ...
                # Check for US works published 1923-1977 (95 years from publication)
                    ...
                    pd_info["pd_year"] = pd_year
                    pd_info["years_until_pd"] = max(0, pd_year - current_year) if not pd_info["is_public_domain"] else 0
                    pd_info["copyright_term"] = f"Published 1923-1977 in US: 95 years from publication"
                    pd_info["expiry_type"] = "publication_based"
                    pd_info["primary_expiry_basis"] = f"publication_in_{publication_year}_plus_95_years"
                    pd_info["explanation"].append(f"US 1923-1977 rule: Published in {publication_year}, copyright expires {pd_year} (95 years from publication). {'Public domain' if pd_info['is_public_domain'] else 'Still under copyright'}.")
                    return pd_info

            # Handle other country-specific publication-based rules
            # Set default expiry type for publication-based works
            pd_info["expiry_type"] = "publication_based"
            pd_info["primary_expiry_basis"] = f"publication_in_{publication_year}_plus_{copyright_term}_years"
            pd_info = self._apply_publication_rules(publication_year, country, current_year, pd_info)

            # Add explanation for publication-based PD
            if pd_info["is_public_domain"]:
                pd_info["explanation"].append(f"Published in {publication_year}, now past copyright term of {copyright_term} years")
    ...
    def assess_use_risk(
        ...
        if author_death_year:
            ...
            if years_since_death >= COUNTRY_COPYRIGHT_TERMS.get("worldwide", 70) - 10 and years_since_death <= COUNTRY_COPYRIGHT_TERMS.get("worldwide", 70) + 10:
    ...
    def get_educational_resources(self, category: str | None = None) -> dict[str, Any]:
        ...
        if category:
            ...
            filtered_resources = [
                resource for resource in index_data.get("resources", [])
    ...
    def analyze_audio_video_copyright(
        ...
        if creation_year:
            ...
            if original_pd_result["is_public_domain"]:
                ...
                result["recommendations"].append("Original work is in public domain, but consider sampling rights for derivative works.")
        ...
        if sampling_info:
            ...
            if sampling_info.get("sampled_from_year"):
                ...
                if sample_pd_result["is_public_domain"]:
                    ...
                    result["sampling_analysis"]["sample_pd_explanation"] = "Sample source is in public domain"
                else:
                    result["sampling_analysis"]["sample_pd_status"] = False
                    result["sampling_analysis"]["sample_pd_explanation"] = f"Sample source still under copyright until {sample_pd_result.get('pd_year')}"
        ...
        if result["is_original_pd"] and (not sampling_info or result["sampling_analysis"].get("sample_pd_status")):
            result["risk_level"] = "very_low"
            result["recommendations"].append("Work and samples appear to be in public domain. Safe for use.")
        elif result["is_original_pd"]:
            result["risk_level"] = "low_to_medium"
            result["recommendations"].append("Original work is public domain, but sampled elements may require permission.")
            ...
            result["risk_level"] = "high"  # The original work from 2020 is still copyrighted
            result["recommendations"].append("New work is not public domain but sample source is. Permission needed for original work.")
        else:
            result["risk_level"] = "high"
            result["recommendations"].append("Original work is not in public domain. Significant copyright risk exists.")
    ...
    def _analyze_sampling_rights(self, sampling_info: dict[str, Any]) -> dict[str, Any]:
        ...
        if sampled_year:
            ...
            years_since_sample = datetime.now().year - sampled_year if sampled_year else 0
        ...
        if sample_length and sample_length > 10:  # More than 10 seconds might require licensing
            analysis["licensing_requirements"].append(
    ...
    def _evaluate_fair_use_factors(self, intended_use: str, has_sampling: bool) -> list[str]:
        ...
        if has_sampling:
            considerations.append("Use of sampling involves creative, protected elements")
        
        # Amount used
        if has_sampling:
            considerations.append("Amount used depends on sample length and recognizability")
        
        # Effect on market
        if intended_use == "commercial":
            considerations.append("Commercial use may harm the market for the original work")
    ...
    def analyze_software_source_pd(
        ...
        if author_death_year:
            ...
            if author_pd_result["is_public_domain"] and not result["is_pd"]:
                result["is_pd"] = True  # If author is PD and no restrictive license exists
        
        # Determine risk level and recommendations
        if result["is_pd"]:
            result["risk_level"] = "very_low"
            if license_type and "public domain" in license_analysis.get("explanation", "").lower():
                result["recommendations"].append(f"Licensed under {license_type} - explicitly placed in public domain")
            else:
                result["recommendations"].append("Software appears to be in public domain")
        elif license_type and license_analysis.get("is_permissive", False):
            result["risk_level"] = "low"
            result["recommendations"].append(f"Licensed under permissive license ({license_type}) - use according to terms")
        else:
            result["risk_level"] = "high"
            if not license_type:
                result["recommendations"].append("No explicit license found - assume copyrighted, seek permission")
            else:
                result["recommendations"].append(f"Licensed under {license_type} - use only according to license terms")
    ...
    def _analyze_software_license(self, license_type: str) -> dict[str, Any]:
        ...
        if any(pd_license in license_lower for pd_license in ["public domain", "cc0", "unlicense"]):
            return {
        ...
        elif "gpl" in license_lower or "agpl" in license_lower or "lgpl" in license_lower:
            return {
                "license_type": license_type,
                "is_pd": False,
                "is_permissive": False,
        ...
        elif "apache" in license_lower or "bsd" in license_lower or "mit" in license_lower:
            return {
                "license_type": license_type,
                "is_pd": False,
                "is_permissive": True,
    ...
    def analyze_database_compilation_rights(
        ...
        result = {
            ...
            "copyright_analysis": {},
            "content_analysis": {"contents_sample": database_contents[:5] if database_contents else [] if database_contents else []},
            "recommendations": [],
            "risk_level": "unknown",
            "full_analysis": {}
        }
        ...
        database_rights_expired = result["database_rights_analysis"].get("is_public_domain", False)
        
        if copyright_expired and database_rights_expired:
            result["risk_level"] = "very_low"
            result["recommendations"].append("Both copyright and database rights have expired. Data is in public domain.")
        elif copyright_expired and not result["database_rights_analysis"].get("has_rights", False):
            result["risk_level"] = "very_low"
            result["recommendations"].append("Copyright expired and no database rights apply. Data is in public domain.")
        elif copyright_expired and not database_rights_expired:
            result["risk_level"] = "low"
            remaining_db_years = result["database_rights_analysis"].get("years_until_pd", 0)
            jurisdiction_name = result["database_rights_analysis"].get("jurisdiction", jurisdiction)
            ...
            result["risk_level"] = "high"
            result["recommendations"].append("Copyright still applies. Database rights may have expired but copyright remains.")
        else:
            # Both copyright and database rights may still apply
            result["risk_level"] = "high" 
            db_explanation = result["database_rights_analysis"].get("explanation", "unknown database rights situation")
            result["recommendations"].append(f"Both copyright and potential database rights still apply. {db_explanation}")
    ...
    def _analyze_jurisdiction_database_rights(
        ...
        eu_database_jurisdictions = [
            "EU", "DE", "FR", "UK", "NL", "ES", "IT", "PL", "BE", "AT", 
            "SE", "DK", "FI", "NO", "CH", "LU", "IE", "PT", "GR", "CY", 
            "MT", "LV", "LT", "EE", "CZ", "SK", "SI", "HR", "RO", "BG"
        ]
        
        if jur_upper in eu_database_jurisdictions:
            return self._analyze_eu_database_rights(creation_year, jurisdiction, substantial_investment)
        ...
        # UK: Has database rights (similar to EU) but different rules
            ...
            return self._analyze_uk_database_rights(creation_year, substantial_investment)
        
        # Canada: Has database rights for certain compilations
        elif jur_upper in ["CA"]:
            return self._analyze_canadian_database_rights(creation_year, substantial_investment)
        
        # Australia: Has compilation rights
        elif jur_upper in ["AU"]:
            return self._analyze_australian_database_rights(creation_year, substantial_investment)
        
        # Brazil: Has database rights
        elif jur_upper in ["BR"]:
            return self._analyze_brazilian_database_rights(creation_year, substantial_investment)
        
        # China: Has database rights
        elif jur_upper in ["CN"]:
            return self._analyze_chinese_database_rights(creation_year, substantial_investment)
        
        # Japan: Has database rights
        elif jur_upper in ["JP"]:
            return self._analyze_japanese_database_rights(creation_year, substantial_investment) 
    ...
    def _analyze_eu_database_rights(self, creation_year: int | None, jurisdiction: str, substantial_investment: bool = False) -> dict[str, Any]:
        ...
        rights_expire_year = creation_year + protection_period  # Rights expire at end of this year
        ...
        if not substantial_investment and has_rights:
            ...
            explanation_addition = " (note: rights may not apply without substantial investment claim)"
        else:
    ...
    def _analyze_uk_database_rights(self, creation_year: int | None, substantial_investment: bool = False) -> dict[str, Any]:
    ...
    def _analyze_canadian_database_rights(self, creation_year: int | None, substantial_investment: bool = False) -> dict[str, Any]:
    ...
    def _analyze_australian_database_rights(self, creation_year: int | None, substantial_investment: bool = False) -> dict[str, Any]:
    ...
    def _analyze_brazilian_database_rights(self, creation_year: int | None, substantial_investment: bool = False) -> dict[str, Any]:
    ...
    def _analyze_chinese_database_rights(self, creation_year: int | None, substantial_investment: bool = False) -> dict[str, Any]:
    ...
    def _analyze_japanese_database_rights(self, creation_year: int | None, substantial_investment: bool = False) -> dict[str, Any]:
    ...
    def _generate_rights_breakdown(
        ...
        breakdown = {
            ...
            "copyright_status": {
                ...
                "explanation": copyright_analysis.get("copyright_term", "Not specified")
            },
            ...
            "investment_factor": {
                ...
                "affects_rights": database_analysis.get("substantial_investment_applied", False)
            },
            "license_factor": {
                "is_licensed_dataset": is_licensed_dataset,
    ...
    def _get_combined_database_risk_level(self, copyright_pd: bool, database_pd: bool) -> str:
    ...
    def _analyze_other_database_rights(self, creation_year: int | None, jurisdiction: str, substantial_investment: bool = False) -> dict[str, Any]:
    ...
    def analyze_performance_neighboring_rights(
        ...
        if perf_pd and rec_pd:
            result["risk_level"] = "very_low"
            result["recommendations"].append("Both performance and recording rights have expired. Safe for use.")
        elif perf_pd or rec_pd:
            result["risk_level"] = "low"
            result["recommendations"].append("One of performance/recording rights has expired. Consider remaining rights.")
        else:
            result["risk_level"] = "high"
            result["recommend_level"] = "Seek permission for both performance and recording rights."
        
        return result

    def _analyze_performance_rights(self, performance_year: int, jurisdiction: str) -> dict[str, Any]:
    ...
    def _analyze_recording_rights(self, recording_year: int, jurisdiction: str) -> dict[str, Any]:
    ...
    def log_pd_determination(self, content_item: ContentItem, result: dict[str, Any], method: str = "unknown") -> str:
        """
    ...
    def get_historical_copyright_status(self, work_identifier: str) -> list[dict[str, Any]]:
        """
    ...
    def get_work_copyright_timeline(self, work_identifier: str) -> dict[str, Any]:
        ...
        return {
            ...
            "current_status": sorted_history[-1]["to_status"] if sorted_history else None,
            "first_status": sorted_history[0]["from_status"] if sorted_history else None
        }
    ...
    def get_copyright_law_at_date(
        ...
        if applicable_changes:
            ...
            latest_change = sorted(applicable_changes, key=lambda x: x.get("effective_date", ""))[-1]
            effective_law["current_terms"] = latest_change.get("terms", COUNTRY_COPYRIGHT_TERMS.get(country, 70))
            effective_law["source_change"] = latest_change
            effective_law["law_in_effect"] = latest_change.get("description", "Law in effect")
    ...
    def create_historical_analysis_report(
        ...
        for year in range(start_year, end_year + 1):
            ...
            standard_law = self.get_copyright_law_at_date(country, year_date, "standard")
            db_law = self.get_copyright_law_at_date(country, year_date, "database_rights") if include_database_rights else None
            ...
            analysis_by_year[str(year)] = {
                ...
                "standard_change": standard_law["source_change"]["description"] if standard_law["source_change"] else "No change",
                "database_change": db_law["source_change"]["description"] if db_law and db_law["source_change"] else "No change"
            }
    ...
    def get_tracked_works_summary(self) -> dict[str, Any]:
        ...
        for work_id, history in self.historical_copyright_data.items():
            ...
            work_changes = [h for h in history if h.get("type") != "copyright_law_update"]
            if work_changes:
                works_with_history[work_id] = {
                    "change_count": len(work_changes),
                    "first_change": work_changes[0]["timestamp"] if work_changes else None,
                    "last_change": work_changes[-1]["timestamp"] if work_changes else None
                }
    ...
    def export_configuration_to_json(self, config_dir: str | Path | None = None) -> dict[str, str]:
        """
    ...
    def get_copyright_office_api_client(self, jurisdiction: str) -> dict[str, str]:
        ...
        return api_endpoints.get(jurisdiction, {
            "api_url": "https://www.wipo.int",
            "documentation_url": "https://www.wipo.int/portal/en/index.html", 
            "search_url": "https://www.wipo.int/goldstandard/en/",
    ...
    def fetch_copyright_law_updates(self, jurisdiction: str = "worldwide") -> list[dict[str, Any]]:
        """
    ...
    def subscribe_to_legal_feeds(self, feed_urls: list[str], callback: callable = None) -> dict[str, bool]:
        """
        ...
        if feedparser is None:
            ...
            for url in feed_urls:
                results[url] = False
            print("Warning: feedparser library not available. Please install it with 'pip install feedparser'")
            return results
        ...
        for url in feed_urls:
            try:
                ...
                is_valid = bool(feed.get('entries', [])) or bool(feed.get('feed', {}).get('title'))
    ...
    def update_country_law_from_official_sources(self, country_code: str) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if country_code.upper() in ["US", "UK", "EU", "DE", "JP"]:
                ...
                result["updates_applied"] = [{
                    "type": "copyright_term",
                    "field": "general_term",
                    "old_value": COUNTRY_COPYRIGHT_TERMS.get(country_code.upper(), 70),
                    "new_value": 70,  # Updated value from official source
                    "source": api_info["source"] if "source" in api_info else api_info["description"]
                }]
                
                # Update the actual country terms if changed
                if result["updates_applied"][0]["old_value"] != result["updates_applied"][0]["new_value"]:
                    COUNTRY_COPYRIGHT_TERMS[country_code.upper()] = result["updates_applied"][0]["new_value"]
                ...
                result["message"] = f"Official source updates not configured for {country_code}"
    ...
    def get_world_ip_resources(self) -> dict[str, Any]:
        ...
        wipo_resources = {
            ...
            "global_ip_statistics": {
                ...
                "access": "public",
                "data_types": ["patents", "trademarks", "industrial_designs", "copyright"]
            },
            "treaty_data": {
                "url": "https://www.wipo.int/treaties/en/",
                "description": "International IP treaty information",
                "access": "public", 
                "data_types": ["copyright_treaties", "patent_treaties", "trademark_treaties"]
            },
    ...
    def check_work_registration_status(self, work_identifier: str, jurisdiction: str = "US") -> dict[str, Any]:
        """
...
async def validate_public_domain_with_explanation(
    ...
    content_to_check = item.snippet or item.content
    if content_to_check and validator._check_content_for_pd_indicators(content_to_check):
