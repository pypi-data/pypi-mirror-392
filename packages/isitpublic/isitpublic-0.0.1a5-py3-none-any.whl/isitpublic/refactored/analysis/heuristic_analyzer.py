...
class HeuristicAnalyzer:
    ...
    def _load_heuristic_indicators(self) -> HeuristicIndicators:
        ...
        return HeuristicIndicators(
            title_pd_indicators=indicators_data.get("title_pd_indicators", [
                "public domain", "public domain,", "public domain.", "pd-", "pd ",
                "pd-", "copyright expired", "copyright-expired", "expired copyright"
            ]),
            content_pd_indicators=indicators_data.get("content_pd_indicators", [
                "public domain", "copyright expired", "pd-", "pd ", "copyright-expired",
                "expired copyright", "released into public domain", "released to public domain",
                "freely available", "copyright has expired"
            ]),
            historical_authors=indicators_data.get("historical_authors", [
                "shakespeare", "darwin", "marx", "lincoln", "twain", "dickens",
                "tolstoy", "dostoevsky", "goethe", "shelley", "byron", "chaucer",
                "cervantes", "plato", "aristotle", "socrates", "homer", "virgil",
                "greek", "roman", "biblical", "bible", "declaration of independence",
                "constitution", "charter", "ancient", "medieval", "classical"
            ]),
            time_period_indicators=indicators_data.get("time_period_indicators", [
                "19th century", "18th century", "17th century", "16th century",
                "15th century", "ancient", "medieval", "early manuscript",
                "historical document", "folk tale", "traditional", "anonymous",
                "unknown author", "by author", "by death", "author died", "died before"
            ]),
            genre_pd_indicators=indicators_data.get("genre_pd_indicators", [
                "ancient greek", "ancient roman", "classic", "by publication",
                "biblical", "bible", "religious text", "sacred text", "mythology",
                "fairy tale", "folk tale", "traditional story", "legend", "myth"
            ])
        )
    ...
    def analyze(self, item: ContentItem) -> Dict[str, Any]:
        ...
        if self._check_title_for_pd_indicators(item.title):
            add_explanation(result, f"Title '{item.title}' contains public domain indicators")
        ...
        if self._apply_heuristic_checks(item.title, content_text):
            ...
            if any(author in title_lower for author in self.indicators.historical_authors):
                matching_authors = [author for author in self.indicators.historical_authors if author in title_lower]
                explanation = f"Work associated with historical author(s): {', '.join(matching_authors[:3])}"
                heuristic_check_node.children.append(
                    DecisionNode(description="Historical author check", result=explanation, is_pd=True)
                )
                add_explanation(result, explanation)
                set_confidence(result, max(result["confidence"], 80))
                found_matches = True

            if any(period in combined_text for period in self.indicators.time_period_indicators):
                matching_periods = [period for period in self.indicators.time_period_indicators if period in combined_text]
                explanation = f"Work associated with historical time period: {', '.join(matching_periods[:3])}"
                heuristic_check_node.children.append(
                    DecisionNode(description="Time period check", result=explanation, is_pd=True)
                )
                add_explanation(result, explanation)
                set_confidence(result, max(result["confidence"], 75))
                found_matches = True

            if any(genre in combined_text for genre in self.indicators.genre_pd_indicators):
                matching_genres = [genre for genre in self.indicators.genre_pd_indicators if genre in combined_text]
                explanation = f"Work belongs to public domain genre: {', '.join(matching_genres[:3])}"
                heuristic_check_node.children.append(
                    DecisionNode(description="Genre check", result=explanation, is_pd=True)
                )
                add_explanation(result, explanation)
    ...
    def _check_title_for_pd_indicators(self, title: str) -> bool:
        ...
        return any(indicator in title_lower for indicator in self.indicators.title_pd_indicators)

    def _check_content_for_pd_indicators(self, content: str) -> bool:
        """Check if content contains public domain indicators."""
        content_lower = content.lower()
        return any(indicator in content_lower for indicator in self.indicators.content_pd_indicators)
    ...
    def _apply_heuristic_checks(self, title: str, content: str) -> bool:
        ...
        return bool(any(genre in combined_text for genre in self.indicators.genre_pd_indicators))