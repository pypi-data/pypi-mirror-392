...
def register_replica_with_node_recycler(node_recycler: Optional[DeploymentHandle] = None) -> Optional[str]:
    ...
    try:
        return ray.get(handle.register_replica.remote(replica_id, node_ip, actor_name))
    except Exception:
        return None
...
def report_unhealthy_replica(error: Optional[str] = None,
    ...
    try:
        return ray.get(handle.report_failure.remote(replica_id, node_ip, error, actor_name))
    except Exception:
        return None
