...
def register_replica_with_node_recycler(node_recycler: Optional[DeploymentHandle] = None) -> Optional[str]:
    """Register the current replica with the NodeRecycler so it can receive failure notices."""
    context = _gather_replica_context()
    if not context:
        raise RuntimeError("Replica context unavailable while registering with NodeRecycler.")
    handle = _resolve_node_recycler_handle(node_recycler)
    if handle is None:
        raise RuntimeError("NodeRecycler handle unavailable while registering replica.")
    replica_id, node_ip, actor_name = context
    return handle.register_replica.remote(replica_id, node_ip, actor_name).result()


def report_unhealthy_replica(error: Optional[str] = None,
                             node_recycler: Optional[DeploymentHandle] = None) -> Optional[str]:
    """Synchronously report the current replica as unhealthy to the NodeRecycler."""
    handle, replica_data = _resolve_failure_report_context(node_recycler)
    if not handle or replica_data is None:
        raise RuntimeError("Unable to resolve NodeRecycler context while reporting unhealthy replica.")
    replica_id, node_ip, actor_name = replica_data
    return handle.report_failure.remote(replica_id, node_ip, error, actor_name).result()
