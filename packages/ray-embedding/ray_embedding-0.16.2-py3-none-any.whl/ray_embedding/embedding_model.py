...
from ray_embedding.utils import (register_replica_with_node_recycler, report_unhealthy_replica)
...
class EmbeddingModel:
    ...
    def wait_for_cuda(self, wait: int = 10):
        ...
        if error_message:
            ...
            asyncio.get_event_loop().create_task(self._report_unhealthy_async(error_message))
    ...
    async def check_health(self):
        ...
        if error_message:
            ...
            ack = await report_unhealthy_replica(error=error_message, node_recycler=self.node_recycler)
    ...
    async def _register_with_node_recycler(self):
        """Register this replica with the NodeRecycler and surface failures."""
        ack = await register_replica_with_node_recycler(self.node_recycler)
        if not ack or "unavailable" in ack.lower():
            self.logger.error(f"Failed to register embedding replica with NodeRecycler: {ack}")
            return
        self.logger.info(ack)

    async def _report_unhealthy_async(self, error_message: str):
