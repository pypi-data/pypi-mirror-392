...
class EmbeddingModel(RecyclableMixin):
    ...
    def __init__(self, model: str, served_model_name: Optional[str] = None,
        ...
        """Initialize the embedding model replica and NodeRecycler coordination."""
    ...
    def _check_cuda_health(self) -> Optional[str]:
        ...
        self._flush_cuda_cache_if_needed(mem_info.total)

        return None

    def _flush_cuda_cache_if_needed(self, total_memory: int):
        """Flush CUDA cache when reserved memory exceeds the configured threshold."""
        if self.cuda_memory_flush_threshold is None:
            return

        reserved = torch.cuda.memory_reserved()  # bytes currently reserved by CUDA cache
        threshold_bytes = self.cuda_memory_flush_threshold * total_memory

        if reserved > threshold_bytes:
            # flush only when cache exceeds the percentage threshold
            self.logger.info(
                f"Flushing CUDA cache because it has exceeded {self.cuda_memory_flush_threshold} ({threshold_bytes} bytes)"
            )
