...
- A `NodeRecycler` supervisor that tracks failure reports from replicas, exposes a pull-based health lookup so colocated replicas/routers can detect failed nodes, and recycles the underlying worker node after quarantining it for one recycle cycle.
...
4. When a node is marked unhealthy, the NodeRecycler waits one recycle cycle for graceful shutdown, then issues an SSH stop to the Ray worker’s Docker container. Ray will automatically restart the stopped worker.

## Failure & Recycling Workflow

When a replica hits a CUDA error (or any fatal condition), the recycler quarantines the node so clients experience deterministic errors while Ray’s autoscaler replaces the failed node.
