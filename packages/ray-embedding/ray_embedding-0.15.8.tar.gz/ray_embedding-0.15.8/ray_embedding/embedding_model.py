...
from ray_embedding.utils import (register_replica_with_node_recycler,
                                 report_unhealthy_replica, report_unhealthy_replica_async)
...
class EmbeddingModel:
    def __init__(self, model: str, served_model_name: Optional[str] = None,
                 ...
                 node_recycler: Optional[DeploymentHandle] = None):
    ...
    def wait_for_cuda(self, wait: int = 10):
        ...
        if error_message:
            ...
            ack = report_unhealthy_replica(error=error_message, node_recycler=self.node_recycler)
            if ack and "unavailable" not in ack:
    ...
    async def check_health(self):
        ...
        if error_message:
            ...
            ack = await report_unhealthy_replica_async(error=error_message, node_recycler=self.node_recycler)
            if ack and "unavailable" not in ack:
                self.logger.info(ack)
            raise RuntimeError(error_message)
    ...
    async def mark_failed_node(self, node_ip: Optional[str] = None, reason: Optional[str] = None):
        """Record that this replica is on a failed node so future health checks error."""
        message = reason or "Embedding replica is colocated with a failed node."
        self._failed_node_error = message
        node_suffix = f" on {node_ip}" if node_ip else ""
        self.logger.error(f"Embedding replica marked unhealthy{node_suffix}: {message}")

    async def _register_with_node_recycler(self, retries: int = 5, delay_seconds: float = 0.5):
        """Attempt to register this replica with the NodeRecycler, retrying if context is unavailable."""
        if not self.node_recycler:
            self.logger.debug("NodeRecycler handle not provided; skipping registration.")
            return
        for attempt in range(1, retries + 1):
            ack = register_replica_with_node_recycler(self.node_recycler)
            if ack and "unavailable" not in ack.lower():
                self.logger.info(ack)
                return
            self.logger.warning(f"NodeRecycler registration attempt {attempt} failed: {ack}")
            await asyncio.sleep(delay_seconds)
        self.logger.error("Exhausted NodeRecycler registration attempts for embedding replica.")
