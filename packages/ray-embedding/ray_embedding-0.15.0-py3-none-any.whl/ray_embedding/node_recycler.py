...
class NodeRecycler:
    ...
    def register_replica(self, replica_id: str, node_ip: str, actor_name: Optional[str] = None):
        ...
        self.logger.debug(f"Registered replica {replica_id} ({actor_name}) on {node_ip}")
    ...
    def _notify_actor_of_failure(self, actor_name: str, node_ip: str, reason: str):
        ...
        handle = self._get_actor_handle(actor_name)
        if handle is None:
            self.logger.debug(f"Actor {actor_name} not found for failed node notice on {node_ip}")
            return
        try:
