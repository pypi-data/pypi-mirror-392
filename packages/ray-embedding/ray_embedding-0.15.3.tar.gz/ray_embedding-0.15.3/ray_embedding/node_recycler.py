...
class NodeRecycler:
    def __init__(
        ...
        self._node_registry: Dict[str, Set[str]] = {}
    ...
    def register_replica(self, replica_id: str, node_ip: str, actor_name: Optional[str] = None):
        ...
        if previous_node and previous_node != node_ip:
            ...
            if actors:
                actors.discard(actor_name)
                if not actors:
                    self._node_registry.pop(previous_node, None)

        self._actor_nodes[actor_name] = node_ip
        self._node_registry.setdefault(node_ip, set()).add(actor_name)
        self.logger.info(f"Registered replica {replica_id} ({actor_name}) on {node_ip}")
        node_entry = self._nodes_marked_for_recycle.get(node_ip)
        if node_entry:
            reason = node_entry.get("reason") or "Replica colocated with failed node."
            self._notify_actor_of_failure(actor_name, node_ip, reason)
    ...
    def report_failure(self, replica_id: str, node_ip: str, error: Optional[str] = None,
        ...
        self.logger.warning(f"Replica {replica_id} on {node_ip} marked for recycling: {error}")
    ...
    async def _gracefully_terminate_replicas(self, node_ip: str):
        """Ask every unhealthy replica on a node to terminate before recycling the node."""
        replicas = [
    ...
    def _notify_node_failure(self, node_ip: str, reason: str, source_actor: Optional[str] = None):
        ...
        actor_names = list(self._node_registry.get(node_ip, set()))
        if not actor_names:
            return
    ...
    def _notify_actor_of_failure(self, actor_name: str, node_ip: str, reason: str):
        ...
        handle = self._get_actor_handle(actor_name)
        if handle is None:
            self.logger.warning(f"Actor {actor_name} not found for failed node notice on {node_ip}")
            return
        try:
    ...
    def _get_actor_handle(self, actor_name: str) -> Optional[ActorHandle]:
        """Best-effort lookup for a Serve actor handle by name."""
        try:
            return ray.get_actor(actor_name, namespace=self.serve_namespace)
        except ValueError:
            self.logger.warning(f"Actor {actor_name} not found in namespace {self.serve_namespace}")
            return None
        except Exception as exc:
    ...
    def _clear_node(self, node_ip: str):
        ...
        actors = self._node_registry.pop(node_ip, set())
