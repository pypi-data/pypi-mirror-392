...
class EmbeddingModel:
    def __init__(self, model: str, served_model_name: Optional[str] = None,
        ...
        try:
            register_replica_with_node_recycler(self.node_recycler)
            self.logger.info("Registered embedding model replica with NodeRecycler for failure notifications.")
    ...
    def wait_for_cuda(self, wait: int = 10):
        ...
        if error_message:
            ...
            report_unhealthy_replica(error=error_message, node_recycler=self.node_recycler)
    ...
    async def check_health(self):
        ...
        if error_message:
            ...
            await report_unhealthy_replica_async(error=error_message, node_recycler=self.node_recycler)
            raise RuntimeError(error_message)
