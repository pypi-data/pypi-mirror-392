...
class EmbeddingModel:
    def __init__(self, model: str, served_model_name: Optional[str] = None,
        ...
        self._register_with_node_recycler()
    ...
    def _register_with_node_recycler(self):
        """Register this replica with the NodeRecycler and surface failures."""
        ack = register_replica_with_node_recycler(self.node_recycler)
        if not ack or "unavailable" in ack.lower():
            raise RuntimeError(f"Failed to register embedding replica with NodeRecycler: {ack}")
        self.logger.info(ack)
