...
class ModelRouter:
    def __init__(self, deployed_models: Dict[str, DeployedModel], path_prefix: List[str],
                 max_concurrency: Optional[int] = 32, node_recycler: Optional[DeploymentHandle] = None):
    ...
    async def mark_failed_node(self, node_ip: Optional[str] = None, reason: Optional[str] = None):
        """Record that this router replica is on a failed node so health checks fail."""
        message = reason or "Model router replica is colocated with a failed node."
        self._failed_node_error = message
        node_suffix = f" on {node_ip}" if node_ip else ""
        self.logger.error(f"Router replica marked unhealthy{node_suffix}: {message}")

    async def _register_with_node_recycler(self, retries: int = 5, delay_seconds: float = 0.5):
        """Attempt to register the router replica with the NodeRecycler, retrying if context is unavailable."""
        if not self.node_recycler:
            self.logger.debug("NodeRecycler handle not provided; skipping router registration.")
            return
        for attempt in range(1, retries + 1):
            ...
            if ack and "unavailable" not in ack.lower():
                ...
                return
            self.logger.warning(f"NodeRecycler registration attempt {attempt} for router failed: {ack}")
            await asyncio.sleep(delay_seconds)
        self.logger.error("Exhausted NodeRecycler registration attempts for router replica.")
