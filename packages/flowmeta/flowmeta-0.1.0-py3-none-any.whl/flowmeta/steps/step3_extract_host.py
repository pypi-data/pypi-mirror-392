"""Step 3: Extract host reads from BAM files"""
...
def _initialize_step_logger(output_dir):
    ...
    _LOG_FILE_PATH = os.path.join(output_dir, f"flowmeta-step3-samtools-{date_str}.log")
    header = "=" * 80
    try:
        with open(_LOG_FILE_PATH, "a", encoding="utf-8") as fp:
...
def extract_host_reads_from_bam(sample_id, bam_dir, host_dir, mode="mapped_anypair",
    ...
    if (not force) and _sample_outputs_exist(sample_id, host_dir):
...
def run_extract_host(bam_dir, host_dir, mode="mapped_anypair",
    ...
    log_info("=" * 60)
    print_colorful_message(
        f"STEP 3: Extract Host Reads from BAM (samtools {samtools_version}, pigz {pigz_version})",
        "cyan",
    )
    log_info("=" * 60)
    if _LOG_FILE_PATH:
    ...
    log_success(f"STEP 3 completed: {success_count}/{len(sample_ids)} samples processed")
