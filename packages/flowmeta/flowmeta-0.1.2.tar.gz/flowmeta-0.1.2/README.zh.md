...
通过 `--step N` 可以仅运行某一阶段（默认 `--step 1`，即全流程）。进入每个 Step 前，CLI 会打印“这一步要做什么？预计多少样本可用”，并说明当前 `--force` 状态，便于判断是否需要重新生成结果。

| Step | 目的 | 进入时统计的样本/文件 |
| --- | --- | --- |
| 1 | fastp 质控与修剪。 | `01-raw` 中符合 `suffix1` 的 FASTQ（单双端皆可）。 |
| 2 | fastp 结果完整性验证。 | `02-qc` 下的 `.task.complete` 或 `_fastp.json`。 |
| 3 | Bowtie2 去宿主并生成 BAM/FASTQ。 | `02-qc` 中的 `.task.complete`。 |
| 4 | 去宿主 FASTQ 完整性检查。 | `03-hr` 中 `_host_remove_R1.fastq.gz`。 |
