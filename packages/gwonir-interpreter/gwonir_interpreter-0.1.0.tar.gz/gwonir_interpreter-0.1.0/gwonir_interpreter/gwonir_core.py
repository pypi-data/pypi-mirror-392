# gwonir_core.py (IF-ELSE 로직 최종 안정화 버전)
import re
import random
...
def execute_print(line):
    # 이제 세미콜론이 포함된 명령어는 한 줄에 하나만 옵니다.
            ...
            while current_index < len(final_commands):
                ...
                if cleaned_line.startswith('권일이가') and cleaned_line.endswith('이면'):
                    ...
                    if evaluate_condition(cleaned_line):
                        # 조건 참: 다음 명령어(IF 본문) 실행
                        next_index = current_index + 1
                        
                        # (핵심 수정) IF 본문 실행 후, ELSE 블록을 건너뛰도록 점프 로직 추가
                        # 현재 IF 본문 실행 후, 다음 명령이 ELSE 블록의 시작(또 다른 IF)이라면
                        if current_index + 2 < len(final_commands) and \
                           final_commands[current_index + 2].startswith('권일이가') and \
                           final_commands[current_index + 2].endswith('이면'):
                            # IF 본문 실행 후, 다음 명령(IF 본문)이 실행된 '후' 점프하기 위해
                            # current_index + 1은 IF 본문, current_index + 2는 ELSE IF
                            # 이 경우, next_index를 current_index + 1로 설정하고, IF 본문 실행 후 점프하도록 처리
                            pass # 다음 루프에서 점프하기 위해 현재는 아무것도 하지 않음
                        
                    else:
                        # 조건 거짓: 다음 명령어(IF 본문) 건너뛰기
                        next_index = current_index + 2 # IF 본문 1줄 건너뛰기
                ...
                # --- 5. IF 본문 실행 후, ELSE 블록 점프 (핵심 수정) ---
                # 현재 명령어(current_index)가 IF 본문이었고, 다음 명령이 ELSE 블록이라면 건너뜁니다.
                
                # 이전 명령이 IF 문이었고, 그 IF 문이 참이어서 이 명령어가 실행되었을 경우
                if current_index > 0 and \
                   ...
                   if current_index + 1 < len(final_commands) and \
                      ...
                      # IF 본문 1줄 실행 후, 다음 2줄(ELSE IF + ELSE 본문)을 건너뛰기
                      next_index = current_index + 2 # 다음 ELSE IF 명령 건너뛰기
...
def main():
    # gwonir [파일 경로] 형식으로 인자를 받습니다.
    if len(sys.argv) < 2:
        print("사용법: gwonir [궈니르 파일 경로]")
        return
    
    file_path = sys.argv[1]
    # 기존에 정의한 run_gwonir_file 함수를 호출합니다.
