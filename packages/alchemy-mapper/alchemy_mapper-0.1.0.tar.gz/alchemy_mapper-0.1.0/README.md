...
Configure custom logger and log level:

```python
import logging

# Create custom logger
query_logger = logging.getLogger('my_app.database.queries')
query_logger.setLevel(logging.INFO)

# Use with mapper
mapper = AlchemyMapper(
    conn,
    custom_logger=query_logger,
...
3. **Stream large result sets**: Don't load everything into memory
   ...
   for batch in mapper.stream[User](sql, batch_size=1000):
       process_batch(batch)
...
5. **Separate read and write**: Use this only for reads, not writes
   ```python
   # Good - Read side
   users = read_mapper.query[list[UserDTO]](sql)
...
See `scaffold/tests/test_logging_and_sa2.py` and `scaffold/tests/test_nested_mapping.py` for comprehensive examples.

## Migration from DataclassQueryMapper

If you were using the previous version that supported dataclasses:
...
2. **Update class name**:
   ```python
   # Old
   mapper = DataclassQueryMapper(conn)
   
   # New
   mapper = AlchemyMapper(conn)
   ```

3. **Convert dataclasses to Pydantic models**:
   ```python
   # Old
   from dataclasses import dataclass
...
4. **Update exception handling**:
   ```python
   # Old
   from alchemy_mapper import PydanticMapperError
