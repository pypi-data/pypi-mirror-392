...
def build_contract(
    ...
    Returns:
        ...
        - metadata: Metadata (if include_metadata=True)
        - ownership: Ownership info (if include_ownership=True)
        - governance_rules: Governance rules (if include_governance=True)
...
def build_contract_from_store(
    ...
    metadata = None
    if include_metadata:
        metadata = store.get_metadata(schema_id, "schema")
    ...
    # Retrieve governance rules (from metadata if available)
    governance_rules = None
    if include_governance and metadata:
