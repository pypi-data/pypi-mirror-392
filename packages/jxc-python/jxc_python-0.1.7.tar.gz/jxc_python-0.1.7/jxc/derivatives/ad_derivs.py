...
def _ad_eval_polarized(name: str, p_stub, rho: Tuple, sigma: Optional[Tuple], lapl: Optional[Tuple], tau: Optional[Tuple], *, scale_with_density: bool = True) -> Dict[str, jnp.ndarray]:
  # Build EXC energy callable with composite support
  ...
  size = int(r0a.shape[0])
  results: list[Dict[str, jnp.ndarray]] = []
  r1a = jnp.asarray(r1, dtype=jnp.float64)
  for i in range(size):
    s_i = None if sigma is None else tuple(jnp.asarray(comp[i], dtype=jnp.float64) for comp in sigma)
    l_i = None if lapl is None else tuple(jnp.asarray(comp[i], dtype=jnp.float64) for comp in lapl)
    t_i = None if tau is None else tuple(jnp.asarray(comp[i], dtype=jnp.float64) for comp in tau)
    results.append(_ad_eval_polarized_point(energy_fn, fam, r0a[i], r1a[i], s_i, l_i, t_i))
  return _stack_results(results)
...
def _ad_eval_unpolarized(name: str, p_stub, r, s, l, t) -> Dict[str, jnp.ndarray]:
  ...
  size = int(r_arr.shape[0])
  out_list: list[Dict[str, jnp.ndarray]] = []
  for i in range(size):
    s_i = None if s is None else jnp.asarray(s[i], dtype=jnp.float64)
    l_i = None if l is None else jnp.asarray(l[i], dtype=jnp.float64)
    t_i = None if t is None else jnp.asarray(t[i], dtype=jnp.float64)
    out_list.append(_ad_eval_unpol_point(energy_fn, fam, r_arr[i], s_i, l_i, t_i))
  return _stack_results(out_list)
...
def _ad_eval_unpolarized_fxc(name: str, p_stub, r, s, l, t) -> Dict[str, jnp.ndarray]:
  ...
  stacked = _stack_results(outputs)
  return {k: jnp.asarray(v, dtype=jnp.float64) for k, v in stacked.items()}
...
def _ad_eval_polarized_fxc(name: str, p_stub, rho: Tuple, sigma: Optional[Tuple], lapl: Optional[Tuple], tau: Optional[Tuple]) -> Dict[str, jnp.ndarray]:
  ...
  sigma_arrays = tuple(_normalize(comp) for comp in sigma_raw)
  lapl_arrays = tuple(_normalize(comp) for comp in lapl_raw)
  tau_arrays = tuple(_normalize(comp) for comp in tau_raw)
  ...
  def _pick(comp, idx: Optional[int] = None):
    if comp is None:
      return jnp.asarray(0.0, dtype=jnp.float64)
    if idx is None:
      return comp
    return jnp.asarray(comp[idx], dtype=jnp.float64)

  if r0a.ndim == 0:
    args = (
      r0a,
      r1a,
      _pick(sigma_arrays[0]),
      _pick(sigma_arrays[1]),
      _pick(sigma_arrays[2]),
      _pick(lapl_arrays[0]),
      _pick(lapl_arrays[1]),
      _pick(tau_arrays[0]),
      _pick(tau_arrays[1]),
  ...
  for i in range(size):
    args = (
      r0a[i],
      r1a[i],
      _pick(sigma_arrays[0], i),
      _pick(sigma_arrays[1], i),
      _pick(sigma_arrays[2], i),
      _pick(lapl_arrays[0], i),
      _pick(lapl_arrays[1], i),
      _pick(tau_arrays[0], i),
      _pick(tau_arrays[1], i),
    )
    outputs.append(_ad_eval_pol_fxc_point(energy_fn, has_sigma, has_lapl, has_tau, args))
  stacked = _stack_results(outputs)
  stacked = _flatten_polarized_fxc(stacked)
  return {k: jnp.asarray(v, dtype=jnp.float64) for k, v in stacked.items()}
...
def _ad_eval_unpolarized_density_order(
  ...
  if r_arr.ndim == 0:
    ...
    arr = jnp.asarray(value, dtype=jnp.float64).reshape(1, 1)
    return {key: arr}

  outputs = []
  size = int(r_arr.shape[0])
  for i in range(size):
    sigma_val = _as_scalar(None if s is None else s[i])
    lapl_val = _as_scalar(None if l is None else l[i])
    tau_val = _as_scalar(None if t is None else t[i])
    def fn(rho_val, sigma_val=sigma_val, lapl_val=lapl_val, tau_val=tau_val):
      return energy_fn(rho_val, sigma_val, lapl_val, tau_val)
    value = _scalar_density_derivative(fn, order, jnp.asarray(r_arr[i], dtype=jnp.float64))
    outputs.append(value)
  stacked = jnp.stack([jnp.asarray(v, dtype=jnp.float64) for v in outputs], axis=0)
...
def _ad_eval_polarized_density_order(
  ...
  def _pick(comp_tuple, idx: Optional[int] = None):
    if comp_tuple is None:
      return jnp.asarray(0.0, dtype=jnp.float64)
    if idx is None:
      return comp_tuple
    return jnp.asarray(comp_tuple[idx], dtype=jnp.float64)

  def _eval_point(args: Tuple) -> jnp.ndarray:
    rho0, rho1, sig0, sig1, sig2, lap0, lap1, tau0, tau1 = args
    def scalar_fn(r_vec):
      return energy_fn(r_vec[0], r_vec[1], sig0, sig1, sig2, lap0, lap1, tau0, tau1)
    tensor = _spin_density_derivative(scalar_fn, order, jnp.stack((rho0, rho1)))
    return _flatten_spin_tensor(tensor, order)
  ...
  if r0a.ndim == 0:
    ...
    values = _eval_point(args)
    return {key: jnp.asarray(values, dtype=jnp.float64).reshape(1, -1)}
  ...
  for i in range(size):
    ...
    outputs.append(_eval_point(args))
  stacked = jnp.stack([jnp.asarray(v, dtype=jnp.float64) for v in outputs], axis=0)
  return {key: stacked}
