...
def create_database_if_not_exists() -> bool:
    ...
    try:
        ...
        logger.info(f"Connecting to PostgreSQL server at {db_url.host}:{db_url.port}...")
        logger.info(f"Using credentials: {db_url.username}@{db_url.host}")
        ...
        # Create the database
        logger.info(f"Creating database '{db_url.database}'...")
...
def check_database_connection() -> bool:
    ...
    try:
        ...
        logger.info(f"Testing connection to database '{target_db_url.database}'...")
        ...
        with engine.connect() as connection:
            ...
            if row:
                version = row[0]
                logger.info(f"Successfully connected to database '{target_db_url.database}'")
        ...
        logger.error(f"Failed to connect to database '{target_db_url.database}': {e}")
        return False
    except SQLAlchemyError as e:
        logger.error(f"SQLAlchemy error testing connection: {e}")
        return False
    except Exception as e:
