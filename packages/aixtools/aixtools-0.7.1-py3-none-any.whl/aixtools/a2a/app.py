...
def create_handler_with_lifespan(
    ...
    async def lifespan(app: Starlette) -> AsyncIterator[dict]:
        async with httpx.AsyncClient(timeout=httpx.Timeout(push_timeout_seconds)) as client:
            handler._push_sender = BasePushNotificationSender(
                httpx_client=client,
                config_store=push_config_store,
            )
            handler._push_config_store = push_config_store
            yield {}

    return handler, lifespan


def build_a2a_starlette_app(  # noqa: PLR0913
    ...
    Args:
        ...
        task_store: Store for managing tasks (defaults to InMemoryTaskStore)
        push_config_store: Store for push notification configurations (defaults to InMemoryPushNotificationConfigStore)
    ...
    push_config_store = push_config_store or InMemoryPushNotificationConfigStore()
