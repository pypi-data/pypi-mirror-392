...
class VaultConfigError(RuntimeError):
    """Raised when VAULT_TOKEN is accessed but Crypto file is missing."""


class _LazyVaultToken:
    def __init__(self):
    ...
    def _get(self) -> str | None:
        ...
        if not key_path.is_file():
            logging.error("VAULT_TOKEN was accessed, but the crypto key file is missing: %s.", key_path)
        ...
        if not vault_token:
            logging.warning("VAULT_TOKEN environment variable not set .")

        self._token = crypto.decrypt(vault_token)
        self._loaded = True
        return self._token
    ...
    def __repr__(self) -> str:
        # Show the underlying token repr (or you could hide it if you prefer)
        return repr(self._get())

    def __getattr__(self, name):
        # Delegate string methods (startswith, upper, etc.)
        value = self._get()
        if value is None:
            raise AttributeError(f"VAULT_TOKEN is None, cannot use string method {name!r}")
