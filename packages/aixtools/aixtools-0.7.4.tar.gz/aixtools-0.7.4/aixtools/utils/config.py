...
from aixtools.utils.crypto import AesGcmCrypto
...
class VaultTokenCache:  # pylint: disable=too-few-public-methods
    ...
    VAULT_TOKEN_LOADED: bool = False


def _load_vault_token() -> str:
    if VaultTokenCache.VAULT_TOKEN_LOADED:
        if VaultTokenCache.VAULT_TOKEN is None:
            raise VaultConfigError("VAULT_TOKEN environment variable is empty or decryption returned None.")
        return VaultTokenCache.VAULT_TOKEN

    key_path = Path(AesGcmCrypto.CRYPTO_KEY_PATH)
    if not key_path.is_file():
        logging.error(
            "VAULT_TOKEN was accessed, but the crypto key file is missing: %s.",
            key_path,
        )
        raise VaultConfigError(f"VAULT_TOKEN was accessed, but the crypto key file is missing: {key_path}.")

    crypto = AesGcmCrypto.create()
    token = crypto.decrypt(get_variable_env("VAULT_TOKEN", allow_empty=True))
    VaultTokenCache.VAULT_TOKEN = token
    VaultTokenCache.VAULT_TOKEN_LOADED = True

    if VaultTokenCache.VAULT_TOKEN is None:
        logging.error("VAULT_TOKEN environment variable is missing or empty.")
        raise VaultConfigError("VAULT_TOKEN environment variable is missing or empty or decryption returned None.")
