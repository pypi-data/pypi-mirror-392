...
# ------------------------------------------------------------
# Convenience wrapper – your generate_map function as a helper
# ------------------------------------------------------------

def generate_map(theta_offset, phi_offset, filename, *, accel: str = "gpu"):
    eng = ThicknessMap(
        dem_path="DEM_20m.npz",
        angle_deg=90 - 14.6 + theta_offset,  # zenith tilt
        angle_window_deg=17,
        fine_step_mrad=0.2,
        det_azimuth_deg=phi_offset,         # detector faces North
        dem_azimuth_deg=0.0,                # DEM ENU-aligned; adjust if rotated
        flip_x=False,
        flip_y=True,                        # if your raw DEM Y needs flipping
        log_level=logging.INFO,
        ds_coarse_scale=0.1,
    )

    eng.compute_thickness(
        calculate_sigma=False,
        accelerator=accel,
        result_step_mrad=1.0,
        profile=True,
    )
    eng.plot()
    eng.export_npz(
        filename,
        export_order="xy",
        extra_meta=dict(accelerator=accel, note="L only + J-binned"),
    )
    return eng
