...
def default_energy_edges(E_min: float = 10.0, E_max: float = 4000.0, nbins: int = 100) -> np.ndarray:
    """
    Convenience helper: log-spaced energy *edges* in GeV.
...
def calculate(
    ...
    *,
    ...
    energies : array_like
        1D array of monotonically increasing energy **bin edges** in GeV,
        shape (N+1,). The code will integrate over these bins using the
        internal `differential_flux` model.
    window_size : float
    ...
    with prof.section("loop:build_all_params"):
        ...
        if energies.ndim != 1 or energies.size < 2:
            raise ValueError(
                "energies must be a 1D array of bin edges with length >= 2 "
                "(e.g. from default_energy_edges)."
            )
        # bin widths and midpoints
        dE = energies[1:] - energies[:-1]
        energies_mid = 0.5 * (energies[1:] + energies[:-1])

        params = None
        for i, energy in enumerate(energies_mid):
            # integrated flux over this bin: Phi(E) * dE
