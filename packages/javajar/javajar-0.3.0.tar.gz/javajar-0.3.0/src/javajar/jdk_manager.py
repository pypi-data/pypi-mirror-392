...
def _parse_version_spec(version: str) -> Tuple[int, bool] | None:
    ...
    try:
        ...
        logger.error("jdk version字符串转换为int异常.version:{},is_plus:{}", version, is_plus)
        return None
...
def install_jdk(version: str, distro: str = "temurin") -> JdkInfo:
    ...
    if version_dir.exists():
        ...
        if jhome and is_valid_jdk_home(jhome):
            logger.info(f"已经存在jdk,直接返回.jhome:{jhome}")
...
def ensure_jdk(version: str, distro: str = "temurin") -> JdkInfo:
    ...
    """
    logger.info(f"检查JDK是否存在 version:{version},distro:{distro}")
    # 1) JAVA_HOME
    ...
    if jhome_env:
        ...
        if is_valid_jdk_home(jhome):
            ...
            if v is not None and _version_matches_spec(v, version):
                logger.info(f"JAVA_HOME存在,版本匹配,不用下载.jhome:{jhome}")
    ...
    if path_javac:
        ...
        if v is not None and _version_matches_spec(v, version):
            ...
            logger.info(f"java在path存在,版本匹配,不用下载.jhome:{Path('java')}")
    ...
    if jhome and is_valid_jdk_home(jhome):
        logger.info(f"java在安装目录存在,不用下载.jhome:{jhome}")
        return JdkInfo(java_exec=jhome / "bin" / JAVA_NAME, java_home=jhome, source="INSTALLED")
...
def java_list() -> list[str]:
    ...
    # .ylk_javajar
    for subDir in Path(INSTALL_BASE).iterdir():
