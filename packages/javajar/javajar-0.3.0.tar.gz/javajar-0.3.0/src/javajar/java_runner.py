...
def run_jar(
    ...
    if maven:
        try:
            release_repo = release_repo or ["http://repo1.maven.org/maven2"]
            snapshot_repo = snapshot_repo or ["http://repo1.maven.org/maven2"]
    ...
    logger.info(f"要执行的jar文件路径: {str(jar_path)}")
    ...
    logger.info(f"找到了匹配的java执行路径,请求java_version:{java_version},找到的java路径:{jdk_info.java_exec}")
    ...
    try:
        # Execute the command
        logger.info(f"执行java命令: {' '.join(cmd)}")
