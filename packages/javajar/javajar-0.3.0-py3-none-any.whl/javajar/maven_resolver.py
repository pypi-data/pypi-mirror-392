...
def download(url: str, dest: Path, verbose: bool = False) -> bool:
    ...
    try:
        ...
        logger.info(f"maven依赖下载成功:{dest}")
        return True
    except Exception as e:
        logger.exception(f"下载maven依赖异常.url={url},dest={dest}")
        return False
...
def resolve_from_maven(
    ...
    """
    logger.info(
        "从maven仓库解析jar包, coord={}, release_repos={}, snapshot_repos={}",
    ...
    if dest.exists() and not is_snapshot:
        logger.debug(f"本地找到maven的jar: {dest}")
        return dest
