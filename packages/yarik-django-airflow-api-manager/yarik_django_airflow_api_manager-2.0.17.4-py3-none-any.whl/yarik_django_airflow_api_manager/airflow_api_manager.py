...
def catch_airflow_errors[**P, R](func: Callable[Concatenate[P], R | None]) -> Callable[Concatenate[P], R | None]:
...
class AirflowManager:
    ...
    def get_dag(self) -> Dag | None:
        ...
        return Dag(**(get_request(urlunparse(url.components)).json()))
    ...
    def get_dags_dict(
        ...
        url.path += "dags"
        return get_request(urlunparse(url.components)).json()
    ...
    def get_dags(
        ...
        if tags is None:
            tags = []
        dct = self.get_dags_dict(tags, dag_id_pattern, only_active, limit, offset, fields)
        if not dct:
            return None
        return Dags(**dct)
    ...
    @catch_airflow_errors
    def get_dag_runs(self, states: list[DagState] | None = None) -> list[DagRun] | None:
    ...
    def get_dag_runs_batch(
        ...
        return DagRuns(**(post_request(urlunparse(url.components), payload=payload).json()))

    @catch_airflow_errors
    def get_last_dag_run(self, username: str | None = None) -> DagRun | None:
        """Получить список запусков дага.
    ...
    @catch_airflow_errors
    def get_current_dag_run(self) -> DagRun | None:
    ...
    @catch_airflow_errors
    def trigger(self, conf: dict[str, Any] | None = None) -> DagRun | None:
        """Запустить даг.
    ...
    def stop(self) -> DagRun | None:
        ...
        return DagRun(**(patch_request(urlunparse(url.components), payload=payload).json()))

    @catch_airflow_errors
    def clear(self) -> DagRun | None:
        ...
        return DagRun(**(post_request(urlunparse(url.components), payload=payload).json()))

    @catch_airflow_errors
    def get_task_instances(self) -> TaskInstances | None:
    ...
    @catch_airflow_errors
    def get_task_instance(self, task_id: str) -> TaskInstance | None:
        """Получить все экземпляры задач текущего запуска дага.
    ...
    @catch_airflow_errors
    def get_logs(self, task_id: str, try_num: int, continuation_token: str | None = None) -> Logs | None:
