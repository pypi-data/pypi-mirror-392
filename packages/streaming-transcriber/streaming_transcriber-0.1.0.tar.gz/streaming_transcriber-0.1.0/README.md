...
`streaming-transcriber` 提供一个异步流式语音转文本的 Python 库，基于 Google Cloud Speech-to-Text 官方示例进行了抽象。该库专注于以下目标：
...
```bash
pip install streaming-transcriber
```
...
```python
import asyncio
from streaming_transcriber import StreamingTranscriber, TranscriptionConfig
...
构建完成后，会在 `dist/` 目录下生成：
- `streaming-transcriber-X.X.X.tar.gz` (源码分发包)
- `streaming_transcriber-X.X.X-py3-none-any.whl` (wheel 包)
...
# 验证安装
pip install --upgrade --force-reinstall dist/streaming_transcriber-*.whl
```
