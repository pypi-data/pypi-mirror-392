...
def all_pairs[T1, T2](xs: FSharpList[T1], ys: FSharpList[T2]) -> FSharpList[tuple[T1, T2]]:
    ...
    def _arrow70(x: T1 | None = None, xs: Any = xs, ys: Any = ys) -> None:
        def _arrow69(y: T2 | None = None) -> None:
            nonlocal node

            def _arrow68(__unit: None = None) -> FSharpList[tuple[T1, T2]]:
    ...
    def loop(
        ...
        while True:
            ...
            if i <= int32.ZERO:
                ...
                def _arrow71(__unit: None = None, i: Any = i, acc: Any = acc, xs_1: Any = xs_1) -> FSharpList[T]:
    ...
    def loop(
        ...
        while True:
            ...
            if FSharpList__get_IsEmpty(xs_1):
                ...
                def _arrow72(__unit: None = None, acc: Any = acc, xs_1: Any = xs_1) -> FSharpList[T]:
                    t: FSharpList[T] = FSharpList(FSharpList__get_Head(xs_1), None)
                    acc.tail_ = t
                    return t
    ...
    def loop(
        ...
        while True:
            ...
            if i <= int32.ZERO:
                ...
                def _arrow73(__unit: None = None, i: Any = i, acc: Any = acc, xs_1: Any = xs_1) -> FSharpList[T]:
                    t: FSharpList[T] = FSharpList(FSharpList__get_Head(xs_1), None)
                    acc.tail_ = t
                    return t
...
def insert_at[T](index: int32, y: T, xs: FSharpList[T]) -> FSharpList[T]:
    ...
    def _arrow74(__unit: None = None, index: Any = index, y: Any = y, xs: Any = xs) -> FSharpList[T]:
        raise Exception((SR_indexOutOfBounds + "\\nParameter name: ") + "index")

    return reverse(
        result if is_done else (FSharpList_Cons_305B8EAC(y, result) if ((i + int32.ONE) == index) else _arrow74())
    )
...
def insert_many_at[T](index: int32, ys: IEnumerable_1[T], xs: FSharpList[T]) -> FSharpList[T]:
    ...
    def _arrow75(__unit: None = None, index: Any = index, ys: Any = ys, xs: Any = xs) -> FSharpList[T]:
        raise Exception((SR_indexOutOfBounds + "\\nParameter name: ") + "index")

    return reverse(result if is_done else (append(ys_1, result) if ((i + int32.ONE) == index) else _arrow75()))
