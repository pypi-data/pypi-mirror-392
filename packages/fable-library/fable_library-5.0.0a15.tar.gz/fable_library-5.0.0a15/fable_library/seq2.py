...
def distinct[T](xs: IEnumerable_1[T], comparer: IEqualityComparer_1[Any]) -> IEnumerable_1[T]:
    def _arrow76(__unit: None = None, xs: Any = xs, comparer: Any = comparer) -> IEnumerable_1[T]:
    ...
    def _arrow82(
        ...
        with get_enumerator(xs) as enumerator:
            while enumerator.System_Collections_IEnumerator_MoveNext():
                ...
                def _arrow79(__unit: None = None) -> int32:
                    return out_arg

                def _arrow80(v: int32) -> None:
                    nonlocal out_arg
                    out_arg = v

                match_value = (try_get_value(dict_1, key, FSharpRef(_arrow79, _arrow80)), out_arg)
        ...
        def _arrow81(key_1: KEY | None = None) -> tuple[KEY, int32]:
            return (key_1, get_item_from_dict(dict_1, key_1))

        return map(_arrow81, keys)
    ...
    def _arrow86(
        ...
        with get_enumerator(xs) as enumerator:
            while enumerator.System_Collections_IEnumerator_MoveNext():
                ...
                def _arrow83(__unit: None = None) -> list[T]:
                    return out_arg

                def _arrow84(v: list[T]) -> None:
                    nonlocal out_arg
                    out_arg = v
        ...
        def _arrow85(key_1: KEY | None = None) -> tuple[KEY, IEnumerable_1[T]]:
            return (key_1, get_item_from_dict(dict_1, key_1))
