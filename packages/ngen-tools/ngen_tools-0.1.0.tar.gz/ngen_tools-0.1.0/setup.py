"""Setup configuration for ngen package."""
...
class PostInstallCommand(install):
    ...
    def run(self):
        ...
        if self.install_lib:
            package_dir = Path(self.install_lib) / "ngen" / "scripts"
        else:
            # Fallback: try to find in installed packages
            try:
                import ngen
                package_dir = Path(ngen.__file__).parent / "scripts"
            except ImportError:
                # During build, use source directory
                package_dir = Path(__file__).parent / "ngen" / "scripts"
