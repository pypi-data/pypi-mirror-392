        ...
        try:
            ...
            for i in range(self.base_index):
                ...
                if "Ok" in res:
                    ...
                    self.risk_control_manager.update_position(i, res["Ok"])
                    for pos in res['Ok']:
            ...
            # 配置 self.total_account_stat 的余额
            if not self.total_account_stat.start_balance:
                init_available_balance, init_balance = self.get_total_init_balance()
                self.total_account_stat.start_balance = init_balance
                self.total_account_stat.max_balance = init_balance
                self.total_account_stat.now_balance = init_balance
                self.total_account_stat.today_balance = init_balance
            ...
            for i, symbols in enumerate(self.symbols):
                for symbol in symbols:
                    ...
                    if "Ok" in res:
                        ...
                        # 计算最大持仓量占总体资金的百分比，更新 position_ratio
                        self.symbol_position_ratio[symbol] = min(self.symbol_position_ratio[symbol], int(
                            res["Ok"]["long_notional"] / leverage) / self.total_account_stat.start_balance)
                        self.max_position[0][symbol] = res["Ok"]
        ...
        subs.append(
            {
                "sub": {
                    "SubscribeTimer": {
                        ...
                        "name": "update_exchange_balance",  # 每 60 秒进行一次交易所余额更新
                    }
                }
            },
        )
