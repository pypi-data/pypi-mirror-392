...
# 2. 修改 max_account_leverage 和 max_symbol_leverage 为 0.01，来检测异常的杠杆使用
# 3. 敞口过大。
...
max_account_leverage = 0.01,                            # 全部账户最大持仓杠杆约束
max_symbol_leverage = 0.01,                             # symbol 最大持仓杠杆约束
abnormal_position_exposure_threshold = 0.001,            # 持仓数量的敞口暴露异常阈值, 0.05 表示 5% 
...
class Strategy(base_strategy.BaseStrategy):
        ...
        subs.append(
            {
                "sub": {
                    "SubscribeTimer": {
                        ...
                        "name": "update_exchange_balance",  # 每 3 秒进行一次交易所余额更新
                    }
                }
            },
        )
    ...
    def on_position(self, account_id, position):
        self.pm.update_positions(account_id, position)
    ...
    def on_order(self, account_id, order):
        ...
        self.pm.update_position_by_order(account_id, input_order)
    ...
    def on_timer_subscribe(self, timer_name):
        if timer_name == "global_risk_check":
            ...
            self.trader.log(f"仓位的信息: {self.pm.positions}", level="INFO", color="blue")
...
# ============================= 创建仓位函数 ====================================
    def create_position(self, index, symbol):
        ...
        side = "Buy"
        pos_side = "Long"
        if symbol not in self.bbo[index]:
            return False
