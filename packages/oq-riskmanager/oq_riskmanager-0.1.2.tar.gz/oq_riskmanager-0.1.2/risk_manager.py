...
class RiskManager:
    ...
    def check_global_risk(self) -> RiskSignal:
        ...
        if balance_ratio_check and zero_check and unbalance_check \
           and drawdown_check and profit_threshold_check and exposure_check['exposure'] \
           and notional_exposure_check['exposure'] and single_leg_check and leverage_check_passed \
           and commission_check_result and liquidation_price_check['Info']:
            rs = RiskSignal()
            self._last_signal = rs.to_dict()
            return rs
        else:
            ...
            if not profit_threshold_check:
                ...
                self._last_signal = rs.to_dict()
            if not exposure_check['exposure']:
                rs.add(
                ...
                self._last_signal = rs.to_dict()
            if not notional_exposure_check['exposure']:
                rs.add(
                    scope=RiskScope.SYMBOL,
                    action=RiskAction.CLOSE_POSITION,
            ...
            if not commission_check_result:
                ...
                self._last_signal = rs.to_dict()
            if not liquidation_price_check['Info']:
                rs.add(
                    scope=RiskScope.SYMBOL,
                    action=RiskAction.CLOSE_POSITION,
    ...
    def _cache_on_event(self, method_name: str, args: tuple, kwargs: dict) -> None:
        ...
        if method_name == "on_bbo":
            ...
            index = self.ex_map[exchange]
            self.fundings[index][funding.get('symbol', '')] = funding
