...
def available_balance_ratio_check(
    ...
    # 检查余额是否为0，避免除零错误
    if taker_balance <= 1e-3 or maker_balance <= 1e-3:
        return False

    if (taker_balance != 0 and taker_available_balance / taker_balance <= available_balance_warning_threshold) \
        or (maker_balance != 0 and maker_available_balance / maker_balance <= available_balance_warning_threshold):
        return False
...
def balance_zero_check(balances: List[Dict[str, Any]]) -> bool:
    ...
    if taker_available_balance <= 1e-3 or maker_available_balance <= 1e-3:
        return False
...
def balance_unbalance_check(
    ...
    if (maker_available_balance != 0 and taker_available_balance / maker_available_balance > available_balance_unbalance_threshold) \
        or (taker_available_balance != 0 and maker_available_balance / taker_available_balance > available_balance_unbalance_threshold):
        return False
