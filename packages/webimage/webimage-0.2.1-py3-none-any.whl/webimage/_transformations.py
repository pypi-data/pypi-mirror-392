...
from ._properties import ImageProperties, ImageSize
...
class Mage:
    ...
    Attributes:
        file: The path to the image file to be transformed.
        output_dir: The directory where the transformed image will be saved.
        img_format: The format of the output image (e.g., "webp", "jpg").
        prefix_name: A string to be prefixed to the output file name.
        suffix_name: A string to be suffixed to the output file name.
    """
    ...
    def transform(
        ...
        **kwargs,
        ...
        Args:
            ...
            operation: The transformation operation to apply. Defaults to "scale". Options include:
    ...
    def _process_image(
        ...
        **kwargs,
        ...
        with Image.open(file_path) as img:
            ...
            match operation:
                case "scale":
                    ImageOps.scale(img, size[0] / img.width, **kwargs).save(self._output(size[0]), **kwargs)
                case "contain":
                    ImageOps.contain(img, size, **kwargs).save(self._output(size[0]), **kwargs)
                case "cover":
                    ImageOps.cover(img, size, **kwargs).save(self._output(size[0]), **kwargs)
                case "fit":
                    ImageOps.fit(img, size, **kwargs).save(self._output(size[0]), **kwargs)
                case "pad":
                    ImageOps.pad(img, size, **kwargs).save(self._output(size[0]), **kwargs)
                case "thumbnail":
                    img.thumbnail(size, **kwargs)
