...
class DatabaseController:
    ...
    def create_data(self, dataset_id, pipeline_stage, result_type, result_data):
        try:
            with self.conn.cursor() as cursor:
                ...
                cursor.execute(insert_sql, (dataset_id, pipeline_stage, result_type, Jsonb(result_data)))
                self.conn.commit()
        ...
        try:
            with self.conn.cursor() as cursor:
                update_sql = SQL('''
                    ...
                    WHERE "dataset_id" = %s AND "pipeline_stage" = %s AND "result_type" = %s
                ''').format(Identifier(self.table_name))
                cursor.execute(update_sql, (Jsonb(new_result_data), dataset_id, pipeline_stage, result_type))
                self.conn.commit()
        except Exception as e:
    ...
    def save_large_dataframe(self, dataset_id, pipeline_stage, result_type, result_dataframe, storage_folder):
        ...
        try:
            ...
            self.create_data(dataset_id, pipeline_stage, result_type, result_data)
        except Exception as e:
            print(f"Error saving large dataframe: {e}")
        
    def load_large_dataframe(self, dataset_id, pipeline_stage, result_type):
        """Load large dataframe from Parquet file using metadata from DB"""
        try:
            result_records = self.read_data(dataset_id, pipeline_stage, result_type)
