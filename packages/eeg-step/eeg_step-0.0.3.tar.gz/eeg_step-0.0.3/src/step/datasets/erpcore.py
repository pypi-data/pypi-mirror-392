...
FILE_TYPE_DICT = {"eeg.set": "raw_files", "events.tsv": "log_files"}
...
def _write_erpcore_manifest():
    ...
    for component, osf_id in OSF_IDS.items():
        base_url = (
            f"https://files.de-1.osf.io/v1/resources/{osf_id}/providers/osfstorage/"
        )
        ...
        participants = df["name"].str.split(r"_|\.").str[0]
        ...
        local_paths = df["materialized"].str.replace(
            f"/{component} Raw Data BIDS-Compatible/", f"erpcore/{component}/"
        )
...
def _find_bids_remote_path(base_url):
    ...
    with urlopen(base_url) as url:
        ...
        bids_dir = [
            f for f in files if "Raw Data BIDS-Compatible" in f["attributes"]["name"]
        ][0]
...
def _list_files(base_url, suffix, exclude_dirs=None):
    ...
    with urlopen(f"{base_url}/{suffix}") as url:
        ...
        files = [file for file in files if file["attributes"]["kind"] == "file"]
