...
# Import validation function with fallback
try:
...
def detect_contaminated_payer_names(crosswalk):
    ...
    Contaminated names are identified by:
    - Names that look like payer IDs (suggesting misassignment)
    - Names containing address-like patterns (commas, long strings, address indicators)
    - Names that are too long to be valid payer names (typically >50 chars suggests address)
    ...
    # Address-like patterns that indicate contamination (exclude ',' - checked separately for multiple commas)
    address_indicators = ['ST ', 'STREET', 'APT', 'FL ', 'FLORIDA', 'CITY', 'STATE', 'ZIP']
    
    # Get validation function (with fallback)
    # Use a local variable name to avoid shadowing if we need to define a fallback function
    _validate_payer_id = is_valid_payer_id_format
    if _validate_payer_id is None:
        # Fallback validation function (define with different name to avoid shadowing)
        def _validate_payer_id(value):
            if not value or not isinstance(value, str):
                return False
            value = value.strip()
            return (3 <= len(value) <= 10) and value.isalnum()
    ...
    for payer_id, details in crosswalk['payer_id'].items():
        ...
        # Check if name looks like a payer ID (suggests misassignment)
        if _validate_payer_id(name):
            reason = "Name appears to be a payer ID format (suggests misassignment)"
        
        # Check if name contains address-like patterns
        if reason is None:
            name_upper = name.upper()
            for indicator in address_indicators:
                if indicator in name_upper:
                    reason = "Contains address indicator: '{}'".format(indicator)
                    break
        
        # Check if name is suspiciously long (addresses are typically long)
        if reason is None and len(name) > 50:
            reason = "Name is unusually long ({} chars) - may be address data".format(len(name))
        
        # Check if name contains multiple commas (address format)
        if reason is None and name.count(',') >= 2:
            reason = "Contains multiple commas (typical of address format)"
