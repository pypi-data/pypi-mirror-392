...
def load_cache(csv_dir):
    ...
    try:
        ...
        if not os.path.exists(path):
            ...
            return _empty_cache()
        _log("Loading cache from '{}'".format(path), level="INFO")
        ...
        with io.open(path, 'r', encoding='utf-8') as f:
            ...
            _log("Cache loaded: {} patients in cache".format(patient_count), level="INFO")
            if patient_count == 0:
...
def put_entry(csv_dir, patient_id, dob, member_id, payer_id, code, 
    ...
    try:
        ...
        if not _is_valid_insurance_code(code_norm):
            _log("Cache put_entry SKIP: invalid code format '{}' for patient_id='{}' (description or invalid format, not caching)".format(
                code_norm[:50], _normalize_str(patient_id)), level="INFO")
            return
