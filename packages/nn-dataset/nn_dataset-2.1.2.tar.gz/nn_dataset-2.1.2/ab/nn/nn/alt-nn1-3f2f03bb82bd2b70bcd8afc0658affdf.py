...
class BagNetBottleneck(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, bottleneck_factor=4):
        ...
        self.conv2 = self.conv_block(mid_channels, 256, kernel_size, stride)
        self.conv3 = self.conv1x1_block(256, out_channels, activation=False)
...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(
            nn.Conv2d(channel_number, 128, kernel_size=5, stride=3, padding=2),                       
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
