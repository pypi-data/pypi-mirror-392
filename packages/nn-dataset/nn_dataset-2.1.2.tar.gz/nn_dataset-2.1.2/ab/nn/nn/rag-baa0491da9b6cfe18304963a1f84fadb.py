# Auto-generated single-file for ReparamLargeKernelConv
...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device) -> None:
        ...
        self.reparam_large_kernel_conv = ReparamLargeKernelConv(in_chs=64, out_chs=128, kernel_size=7, stride=1, group_size=1, small_kernel=3, use_se=True, act_layer=nn.ReLU, inference_mode=False)
    ...
    def forward(self, x):
        x = self.features(x)
        x = self.reparam_large_kernel_conv(x)
        x = x.mean(dim=(2, 3))
        x = self.classifier(x)
        return x
