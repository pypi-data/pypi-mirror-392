...
class DecoderRNN(nn.Module):
    def __init__(self, embed_size: int, hidden_size: int, vocab_size: int, drop_prob: float = 0.0, num_layers: int = 1):
        ...
        self.feat_norm = nn.LayerNorm(embed_size)
    ...
    def forward(self, inputs: torch.Tensor, hidden: tuple, encoder_features: torch.Tensor):
        ...
        h, c = hidden
        ef = self.feat_norm(encoder_features)
        outputs = []
        for t in range(T):
            word_t = embeds[:, t, :]
            step_in = torch.cat([word_t, ef], dim=1)
    ...
    def generate(self, encoder_features: torch.Tensor, hidden: tuple, max_len: int = 50, start_token: int = 1, end_token: int = 2):
        ...
        for _ in range(max_len):
            step_logits, hidden = self.forward(cur.unsqueeze(1), hidden, ef)
...
class Net(nn.Module):
    ...
    @staticmethod
    def _normalize_captions(captions: torch.Tensor, batch_size: int = None) -> torch.Tensor:
        if captions is None:
            return None
        if captions.ndim == 3:
            captions = captions[:, 0, :]
        ...
        if batch_size is not None and captions.size(0) != batch_size:
            captions = captions.expand(batch_size, -1)
    ...
    def forward(self, images, captions=None, hidden_state=None):
        ...
        captions = self._normalize_captions(captions, B)
    ...
    def train_setup(self, prm):
        ...
        self.optimizer = torch.optim.SGD(self.parameters(), lr=lr, momentum=momentum, nesterov=True, weight_decay=1e-4)
    ...
    def learn(self, train_data):
        ...
        for images, captions in train_data:
            ...
            caps_for_loss = self._normalize_captions(captions, images.size(0))
