...
def supported_hyperparameters():
    return {'lr', 'momentum', 'num_blocks', 'growth_rate'}
...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.model = model_class(self.channel_number, self.class_number, num_blocks=5, growth_rate=32)

        self.learning_rate = prm['lr']
        self.momentum = prm['momentum']
        self.num_blocks = prm['num_blocks'] if 'num_blocks' in prm else 5
        self.growth_rate = prm['growth_rate'] if 'growth_rate' in prm else 32
