...
class BagNetBottleneck(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, bottleneck_factor=4):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(mid_channels, mid_channels, kernel_size=kernel_size, stride=stride, padding=(kernel_size-1)//2, bias=False),
            nn.BatchNorm2d(mid_channels),
            nn.ReLU(inplace=True)
        )
...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.in_size = in_shape[2]
        self.num_classes = out_shape[0]
        self.bottleneck_factor = 4
        self.bottleneck_stages = 3
        self.dropout = prm['dropout']
        self.learning_rate = prm['lr']
        self.momentum = prm['momentum']

        self.features = nn.Sequential(
            nn.Conv2d(in_shape[1], 64, kernel_size=5, stride=3, padding=2),
            nn.BatchNorm2d(64),
            BagNetUnit(64, 64, kernel_size=3, stride=1),
            BagNetUnit(64, 64, kernel_size=3, stride=2),
            BagNetUnit(64, 64, kernel_size=3, stride=2)
        )

        in_channels = 64
        for i in range(3):
            stage_channels = self.channels[i]
            stage = nn.Sequential()
