...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        conv1_kernel = random.randint(3, 5)
        conv1_stride = random.randint(1, 2)
        conv1_padding = random.randint(0, 2)
        maxpool1_kernel = random.randint(1, 3)

        layers += [
            nn.Conv2d(in_channels, 32, kernel_size=conv1_kernel,
                      stride=conv1_stride, padding=conv1_padding),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=maxpool1_kernel, stride=2),
        ]
        in_channels = 32
