...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        layers += [
            nn.Conv2d(in_channels, 128, kernel_size=2, padding=2),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),
        ]
        in_channels = 128
        ...
        layers += [
            nn.Conv2d(in_channels, 440, kernel_size=1, padding=1),
            nn.ReLU(inplace=True),
        ]
        in_channels = 440
        ...
        layers += [
            nn.Conv2d(in_channels, 256, kernel_size=1, padding=1),
            nn.ReLU(inplace=True),
        ]
        in_channels = 256
        ...
        layers += [
            nn.Conv2d(in_channels, 192, kernel_size=1, padding=1),
            nn.ReLU(inplace=True),
