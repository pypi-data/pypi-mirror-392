# Auto-generated single-file for DMoNPooling
...
# ---- torch_geometric.nn.dense.mincut_pool._rank3_trace ----
def _rank3_trace(x: Tensor) -> Tensor:
    return torch.einsum('ijj->i', x)

# ---- torch_geometric.nn.dense.dmon_pool.EPS ----
EPS = 1e-15
...
class DMoNPooling(torch.nn.Module):
    ...
    def __init__(self, channels: Union[int, List[int]], k: int,
        ...
        self.mlp = self._build_mlp(channels + [k])
    ...
    def __repr__(self) -> str:
        return (f'{self.__class__.__name__}({self.mlp[0].in_features}, '
                f'num_clusters={self.mlp[-1].out_features})')
...
class Net(torch.nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device) -> None:
        ...
        self.dmon_pool = DMoNPooling(channels=32, k=8, dropout=0.1)
    ...
    def forward(self, x):
        ...
        adj = torch.eye(H * W, device=x.device).unsqueeze(0).expand(B, -1, -1)
        s, out, out_adj, spectral_loss, ortho_loss, cluster_loss = self.dmon_pool(x_flat, adj)
        x = out.mean(dim=1)
        x = self.avgpool(x.unsqueeze(-1).unsqueeze(-1))
        x = x.flatten(1)
