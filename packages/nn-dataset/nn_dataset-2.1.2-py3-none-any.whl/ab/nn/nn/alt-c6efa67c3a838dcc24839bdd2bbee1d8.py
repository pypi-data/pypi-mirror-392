...
class Net(nn.Module):
    ...
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        layers += [
            nn.Conv2d(in_channels, conv1_filters, kernel_size=conv1_kernel,
                      stride=conv1_stride, padding=2),
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Dropout(p=prm['dropout']),
            nn.Linear(fc1_neurons, fc1_neurons),
            nn.ReLU(inplace=True),
            nn.Linear(fc1_neurons, out_shape[0]),
        )
