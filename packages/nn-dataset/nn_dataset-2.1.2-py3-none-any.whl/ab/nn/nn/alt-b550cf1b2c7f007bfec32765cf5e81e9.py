...
class Net(nn.Module):
    def train_setup(self, prm):
        ...
        params = ['lr', 'momentum', 'dropout']
        selected = random.sample(params, k=2)
        
                                                  
        for p in selected:
            if p == 'lr':
                new_val = random.uniform(0.0001, 0.1)
            elif p == 'momentum':
                new_val = random.uniform(0.5, 0.99)
            elif p == 'dropout':
                new_val = random.uniform(0.1, 0.5)
            prm[p] = new_val
