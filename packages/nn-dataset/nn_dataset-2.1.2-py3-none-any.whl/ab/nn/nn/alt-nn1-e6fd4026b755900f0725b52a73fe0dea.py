...
class PSPBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.gelu(self.bottleneck(torch.cat([x] + pooled_features, dim=1)))
...
class ICNetClassification(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.psp_block(x)
        x = self.head_block(x)
        x = F.gelu(F.adaptive_avg_pool2d(x, (1, 1)))
        x = torch.flatten(x, 1)
        x = self.classifier(x)
        return x
...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.act_type = prm.get('act_type', 'gelu')                           
    ...
    def train_setup(self, prm):
        ...
        if self.act_type == 'swish':
            self.optimizer = optim.SGD(
                ...
                activation_fn=lambda x: x * torch.sigmoid(x) if x is not None else x
            )
        else:                            
            self.optimizer = optim.SGD(
                self.parameters(),
