...
class Net(nn.Module):
    def train_setup(self, prm):
        ...
        params_to_modify = random.sample(supported_hyperparameters(), k=random.randint(2, len(supported_hyperparameters())))
        
                                         
        for param_name in params_to_modify:
            if param_name == 'lr':
                new_value = random.uniform(0.001, 0.1)
            elif param_name == 'momentum':
                new_value = random.uniform(0.5, 0.99)
            elif param_name == 'dropout':
                new_value = random.uniform(0.2, 0.5)
