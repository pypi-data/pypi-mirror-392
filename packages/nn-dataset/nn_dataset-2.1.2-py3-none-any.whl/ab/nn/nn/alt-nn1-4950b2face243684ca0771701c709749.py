...
class BagNetBottleneck(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, bottleneck_factor=4):
        ...
        self.conv1 = self.conv1x1_block(in_channels, 32)
                                                   
        self.conv2 = self.conv_block(32, 32, kernel_size, stride)
        self.conv3 = self.conv1x1_block(32, out_channels, activation=False)
...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device: torch.device) -> None:
        ...
        self.features = nn.Sequential(
            nn.Conv2d(channel_number, 64, kernel_size=5, stride=2, padding=3, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
