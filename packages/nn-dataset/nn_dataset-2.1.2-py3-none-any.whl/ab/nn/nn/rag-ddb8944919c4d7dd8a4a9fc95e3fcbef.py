# Auto-generated single-file for Block35
...
class Block35(nn.Module):
    def __init__(self, scale=1.0, conv_block=None):
        ...
        self.branch0 = conv_block(32, 8, kernel_size=1, stride=1)

        self.branch1 = nn.Sequential(
            conv_block(32, 8, kernel_size=1, stride=1),
        ...
        self.conv2d = nn.Conv2d(24, 32, kernel_size=1, stride=1)
        self.act = nn.ReLU()
...
class Net(torch.nn.Module):
    ...
    def build_features(self):
        ...
        self.block35 = Block35(scale=0.35)
    ...
    def forward(self, x):
        ...
        B, C, H, W = x.shape
        if H > 8 or W > 8:
            x_down = torch.nn.functional.adaptive_avg_pool2d(x, (8, 8))
            x_down = torch.nn.functional.interpolate(x_down, size=(H, W), mode='bilinear', align_corners=False)
            x = x + x_down * 0.35
        else:
            x = self.block35(x)
