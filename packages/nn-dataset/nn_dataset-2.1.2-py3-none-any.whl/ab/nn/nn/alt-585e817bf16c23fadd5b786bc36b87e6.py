...
class Conv(nn.Module):
    def __init__(self, inp, oup, k=3, s=1, p=None, g=1, d=1, act=True):
        super().__init__()
...
class Residual(nn.Module):
    def __init__(self, inp, g=1, k=(3, 3), e=0.3):
        super().__init__()
...
class CSPBlock(torch.nn.Module):
    def __init__(self, in_ch, out_ch):
        super().__init__()
        self.conv1 = Conv(in_ch, out_ch // 3)
        self.conv2 = Conv(in_ch, out_ch // 3)
        self.conv3 = Conv(2 * (out_ch // 3), out_ch)
        self.res_m = torch.nn.Sequential(Residual(out_ch // 3, e=1.0),
                                         Residual(out_ch // 3, e=1.0))
