# Auto-generated single-file for TPStransform
...
class Net(nn.Module):
    def __init__(self, in_shape: tuple, out_shape: tuple, prm: dict, device) -> None:
        ...
        self.tps_transform = TPStransform(
            output_image_size=(8, 8),
            num_control_points=20,
            margins=[0.05, 0.05]
        )
        self.classifier = nn.Linear(64 * 8 * 8, self.num_classes)
    ...
    def forward(self, x):
        ...
        B, C, H, W = x.shape
        source_control_points = torch.randn(B, 20, 2, device=x.device) * 0.1 + 0.5
        source_control_points = torch.clamp(source_control_points, 0, 1)
        
        x = self.tps_transform(x, source_control_points)
        x = x.view(x.size(0), -1)
        x = self.classifier(x)
        return x
