...
class ParseConfig:
    ...
    def __init__(self, provider: Literal["textin", "mineru", "paddle"] = "textin", **kwargs):
        ...
        Args:
            provider: 解析提供者，默认为 "textin"
            **kwargs: 其他动态配置字段，会根据不同的 provider 有不同的配置项
        """
...
def create_pipeline_from_config(config: Dict[str, Any]) -> Pipeline:
    ...
    if 'parse_config' in config and config['parse_config']:
        parse_cfg = config['parse_config']
        # 提取 provider，其他字段作为动态配置传递
        parse_cfg_copy = dict(parse_cfg)  # 复制字典，避免修改原配置
        provider = parse_cfg_copy.pop('provider', 'textin')  # 提取并移除 provider
        parse_config = ParseConfig(provider=provider, **parse_cfg_copy)  # 剩余字段作为动态配置
    ...
    if 'chunk_config' in config and config['chunk_config']:
        ...
        chunk_config = ChunkConfig(
            ...
            overlap_all=chunk_cfg.get('overlap_all', False)
        )
