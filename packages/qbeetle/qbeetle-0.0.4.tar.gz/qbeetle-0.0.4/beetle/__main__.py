...
def _main():
    """
    Main entry point for the `beetle` command line script.

    We init logging here instead of in beetle.cmdline.main(...) because the latter
    can be called by projects using beetle, and it's bad practice for libraries to
