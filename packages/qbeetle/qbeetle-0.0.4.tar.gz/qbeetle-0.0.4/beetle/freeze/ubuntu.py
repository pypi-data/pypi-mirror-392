...
def freeze_ubuntu(debug=False, tool=FreezeTool.pyinstaller):
    if tool == FreezeTool.pyinstaller:
        freeze_ubuntu_pyinstaller(debug=debug)
    else:
        freeze_ubuntu_nuitka(debug=debug)


def freeze_ubuntu_nuitka(debug=False):
    freeze_linux(debug, tool=FreezeTool.nuitka)


def freeze_ubuntu_pyinstaller(debug=False):
