...
from beetle.resources import get_icons
...
def freeze_mac(debug=False, tool=FreezeTool.pyinstaller):
    if tool == FreezeTool.pyinstaller:
        freeze_mac_pyinstaller(debug=debug)
    else:
        freeze_mac_nuitka(debug=debug)


def freeze_mac_nuitka(debug=False):
    if not exists(path('target/Icon.icns')):
        _generate_iconset()
        run(['iconutil', '-c', 'icns', path('target/Icon.iconset')], check=True)
    args = []
    args.extend(['--macos-app-icon=' + path('target/Icon.icns'), ])
    run_nuitka(args, debug)


def freeze_mac_pyinstaller(debug=False):
    if not exists(path('target/Icon.icns')):
        _generate_iconset()
        run(['iconutil', '-c', 'icns', path('target/Icon.iconset')], check=True)
    args = []
