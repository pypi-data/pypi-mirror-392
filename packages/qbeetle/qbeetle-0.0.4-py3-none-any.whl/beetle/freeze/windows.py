...
from beetle.freeze import run_nuitka, run_pyinstaller, _generate_medias, FreezeTool
...
def freeze_windows(debug=False, tool=FreezeTool.pyinstaller):
    if tool == FreezeTool.pyinstaller:
        freeze_windows_pyinstaller(debug=debug)
    else:
        freeze_windows_nuitka(debug=debug)


def freeze_windows_nuitka(debug=False):
    ...
    if not debug:
        args.extend(["--windows-disable-console", ])
    args.extend(['--windows-icon-from-ico=' + path(os.path.join(PROJECTINFO.icons_dir, 'Icon.ico')), ])
    args.extend(["--mingw64",])
    args.extend([
        f"--file-description={app_name}",
        f"--copyright=\"© {author}. All rights reserved.\"",
        f"--trademarks=\"© {author}. All rights reserved.\"",
    ])
...
def freeze_windows_pyinstaller(debug=False):
    args = []
    ...
    args.extend(['--icon', path(os.path.join(PROJECTINFO.icons_dir, 'Icon.ico'))])
