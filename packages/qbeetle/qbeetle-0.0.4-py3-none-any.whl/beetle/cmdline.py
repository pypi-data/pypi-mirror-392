...
from beetle._state import COMMANDS
...
def main(project_dir=None):
    """
    This function is executed when you run `beetle ...` on the command line. You
    can call this function from your own build script to run beetle as if it were
    ...
    if project_dir is None:
        project_dir = getcwd()
    try:
        beetle.init(project_dir)
        # Load built-in commands:
        from beetle import builtin_commands
        from beetle.builtin_commands import _docker
        from beetle.builtin_commands import gpg
        # from beetle.builtin_commands import _account
        from beetle.builtin_commands import _licensing
        ...
        # Don't print a stack trace for BeetleError, just their message:
        _LOG.error(str(e))
        sys.exit(-1)
...
def command(f):
    """
    Use this as a decorator to define custom beetle commands. For an example, see:
        https://build-system.fman.io/manual/#custom-commands
    """
...
def _get_cmdline_parser():
    # Were we invoked with `python -m beetle`?
    is_python_m_beetle = splitext(basename(sys.argv[0]))[0] == '__main__'
    if is_python_m_beetle:
        prog = '%s -m beetle' % basename(sys.executable)
    else:
        prog = None
    parser = ArgumentParser(prog=prog, description='beetle')
