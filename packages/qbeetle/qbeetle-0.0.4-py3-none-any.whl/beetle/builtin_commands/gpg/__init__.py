...
from beetle.builtin_commands._docker import _run_docker
from beetle.builtin_commands._util import prompt_for_value, update_json, \
...
def gengpgkey():
    require_project_existing()
    if exists(_DEST_DIR):
        raise BeetleError('The %s folder already exists. Aborting.' % _DEST_DIR)
    try:
    ...
    update_json(path(PROJECTINFO.base_json), {'gpg_key': key, 'gpg_name': name})
    update_json(path(PROJECTINFO.secret_json), {'gpg_pass': passphrase})
    _LOG.info(
        'Done. Created %s and ...%s. Also updated %s and ...secret.json with '
        'the values you provided.', pubkey_dest, _PRIVKEY_NAME, PROJECTINFO.base_json
    )


gengpgkey.__doc__ = _("Generate a GPG key for Linux code signing")
