...
def pack_license_key(data, privkey_args):
    ...
    fields "n", "e", "d", "p" and "q". You can generate it with beetle's command
    `init_licensing`.
...
class _Licensing:
    ...
    application context, in such a way that the beetle user does not have to worry
...
def unpack_license_key(key_str, pubkey_args):
    ...
    fields "n" and "e". It can be generated with beetle's command `init_licensing`.
    """
    try:
