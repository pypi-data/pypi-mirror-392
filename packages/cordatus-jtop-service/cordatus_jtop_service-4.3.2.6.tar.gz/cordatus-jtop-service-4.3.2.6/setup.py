...
class JTOPInstallCommand(install):
    ...
    def run(self):
        # Skip custom installation when building wheel (WHEEL_BUILD env var set by build system)
        if os.environ.get('WHEEL_BUILD') or '--no-user-cfg' in sys.argv:
            # Just run the default installation (for wheel building)
            install.run(self)
        else:
            # Run the uninstaller before to copy all scripts
...
setup(
    ...
    description="Interactive system-monitor and process viewer for all NVIDIA Jetson [Orin, Xavier, Nano, TX] series - Cordatus Edition",
    license='AGPL-3.0',
    ...
    # Keep directory structure: jetson_stats/services/ and jetson_stats/scripts/
    data_files=[
        ('jetson_stats/services', ['services/jtop.service']),
        ('jetson_stats/scripts', ['scripts/jtop_env.sh']),
    ],
    # Install extra scripts - DISABLED for PyPI distribution, use cordatus-jtop-setup instead
    ...
    entry_points={'console_scripts': [
        ...
        'cordatus-jtop-setup = jtop.setup_service:main',
    ]},
)
# EOF
