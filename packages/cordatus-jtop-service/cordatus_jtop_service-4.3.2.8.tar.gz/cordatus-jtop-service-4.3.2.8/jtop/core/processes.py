...
class ProcessService(object):
    ...
    def get_process_info(self, pid, gpu_mem_usage, process_name, uptime):
        try:
            ...
            # Process klasörü yoksa direkt dön
            if not os.path.isdir(proc_dir):
                return []

            # ---- STAT READ ----
            try:
                stat = cat(os.path.join(proc_dir, 'stat')).split()
            except Exception:
                logger.debug(f"Cannot read stat for PID {pid}")
                return []

            # ---- LOGINUID READ ----
            try:
                uid_path = os.path.join(proc_dir, 'loginuid')
                uid = int(cat(uid_path))
            except Exception:
                # loginuid yoksa "nobody"
                uid = 65534
            ...
            # ---- STATM READ ----
            try:
                mem_raw = cat(os.path.join(proc_dir, 'statm')).split()
