...
class TestValidateZoteroConfig:
    ...
    def test_validate_missing_ids(self):
        ...
        with pytest.raises(ValueError, match="Either Zotero user ID or group ID is required"):
            config.validate_zotero_config(args)
    ...
    @patch.dict(os.environ, {'ZOTERO_API_KEY': 'env_key', 'ZOTERO_GROUP_ID': 'env_group'})
