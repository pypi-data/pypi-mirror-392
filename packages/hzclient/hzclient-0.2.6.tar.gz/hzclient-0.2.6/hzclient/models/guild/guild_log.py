...
class GuildLog(_Base):
  ...
  def chats(self) -> List[GuildLogEntry]:
    """All chat entries sorted by timestamp ascending."""
    return [e for e in self.all() if e.is_chat]
  ...
  def append_entry(self, payload: Mapping[str, Any]) -> GuildLogEntry:
    """
    Append a single guild log entry (e.g. from `guild_chat_message`)
    """
    entry = GuildLogEntry.model_validate(payload)
