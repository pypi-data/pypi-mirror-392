...
def compare_with_rdkit(
    ...
    if rdkit_graph is None:
        ...
        atoms = [(reference_graph.nodes[i]['symbol'], reference_graph.nodes[i]['position']) 
    ...
    charge = reference_graph.graph.get('total_charge', 0)
    out.append(f"# RDKit graph: {rdkit_graph.number_of_nodes()} atoms, {rdkit_graph.number_of_edges()} bonds (charge={charge})")
    ...
    if ascii:
        ...
        ascii_txt = graph_to_ascii(rdkit_graph, scale=ascii_scale, include_h=ascii_include_h, reference_layout=layout)
    ...
    out.append(f"# Bond differences: only_in_native={len(only_ref):,}   only_in_rdkit={len(only_rdkit):,}   bond_order_diffs={len(bo_diffs):,}")
