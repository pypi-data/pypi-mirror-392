...
from .cloud_api import save_custom_prompt, save_personality_message, run_flow, run_flow_stream, update_vault_metadata, delete_vault_metadata, update_vault_mapping, get_vault_mapping, get_user_vault_data, update_user_vault_data
...
class Vault:
    ...
    def delete_items(self, item_ids: List[int], trees: int = 10) -> None:
        ...
        def download_and_upload(old_id, new_id):
            ...
            # Upload with retry to ensure it persists
    ...
    def _get_mapping_for_vault(self, vault):
        ...
        mapping = get_vault_mapping(self.user, self.api, vault)
        
        if not mapping:
            gcs_mapping = self.cloud_manager.download_old_gcs_mapping(vault)
            if gcs_mapping and len(gcs_mapping) > 0:
                update_vault_mapping(self.user, self.api, vault, gcs_mapping)
                mapping = gcs_mapping
    ...
    def process_batch(self, batch_text_chunks, never_stop, loop_timeout):
        ...
        while True:
            try:
                ...
                return [record.embedding for record in embeddings.data]
            except Exception as e:
