...
from .cloud_api import call_proj,get_init_data, get_vault_metadata, update_vault_metadata, get_vault_mapping, update_vault_mapping, get_vaults_list, update_vaults_list, save_custom_prompt, save_personality_message, get_user_vault_data, TokenManager, login_with_api, get_access_token
...
class CloudManager:
    def __init__(self, user: str, api_key: str, vault: str):
        ...
        self.api = api_key
        self.vault = vault
        # Token manager for this instance
    ...
    def download_old_gcs_mapping(self, vault_name):
        try:
            ...
            return self._normalize_item_mapping(mapping)
    ...
    def get_mapping(self):
        ...
        # If mapping is empty, try to get it from old storage
        if not self.item_mapping or len(self.item_mapping) == 0:
            ...
            if gcs_mapping and len(gcs_mapping) > 0:
                update_vault_mapping(self.user, self.api, self.vault, gcs_mapping)
