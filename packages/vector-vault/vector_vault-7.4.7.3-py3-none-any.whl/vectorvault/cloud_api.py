...
# These create a temporary CloudAPI instance for each call (legacy support)
# New code should use CloudAPI class directly through CloudManager
# ============================================================================

def get_init_data(user, api_key, vault, max_retries=1):
    """Legacy wrapper - prefer using CloudAPI instance"""
    api = CloudAPI(user, api_key)
    return api.get_init_data(vault, max_retries)

def get_vault_metadata(user, api_key, max_retries=3):
    """Legacy wrapper - prefer using CloudAPI instance"""
    api = CloudAPI(user, api_key)
    return api.get_vault_metadata(max_retries)
...
def update_vault_metadata(user, api_key, vault, total_items=None, last_update=None, last_use=None, total_use=None, max_retries=3):
    """Legacy wrapper - prefer using CloudAPI instance"""
    api = CloudAPI(user, api_key)
    return api.update_vault_metadata(vault, total_items, last_update, last_use, total_use, max_retries)

def get_vault_mapping(user, api_key, vault, max_retries=3):
    """Legacy wrapper - prefer using CloudAPI instance"""
    api = CloudAPI(user, api_key)
    return api.get_vault_mapping(vault, max_retries)

def update_vault_mapping(user, api_key, vault, mapping, max_retries=3):
    """Legacy wrapper - prefer using CloudAPI instance"""
    api = CloudAPI(user, api_key)
    return api.update_vault_mapping(vault, mapping, max_retries)

def get_vaults_list(user, api_key, max_retries=3):
    """Legacy wrapper - prefer using CloudAPI instance"""
    api = CloudAPI(user, api_key)
    return api.get_vaults_list(max_retries)

def update_vaults_list(user, api_key, vaults, max_retries=3):
    """Legacy wrapper - prefer using CloudAPI instance"""
    api = CloudAPI(user, api_key)
    return api.update_vaults_list(vaults, max_retries)

def save_custom_prompt(user, api_key, vault, prompt, context=True, max_retries=3):
    """Legacy wrapper - prefer using CloudAPI instance"""
    api = CloudAPI(user, api_key)
    return api.save_custom_prompt(vault, prompt, context, max_retries)

def save_personality_message(user, api_key, vault, personality_message, max_retries=3):
    """Legacy wrapper - prefer using CloudAPI instance"""
    api = CloudAPI(user, api_key)
    return api.save_personality_message(vault, personality_message, max_retries)

def get_user_vault_data(user, api_key, max_retries=3):
    """Legacy wrapper - prefer using CloudAPI instance"""
    api = CloudAPI(user, api_key)
    return api.get_user_vault_data(max_retries)

def update_user_vault_data(user, api_key, vault_data, max_retries=3):
    """Legacy wrapper - prefer using CloudAPI instance"""
    api = CloudAPI(user, api_key)
    return api.update_user_vault_data(vault_data, max_retries)

def delete_vault_metadata(user, api_key, vault, max_retries=3):
    """Legacy wrapper - prefer using CloudAPI instance"""
    api = CloudAPI(user, api_key)
    return api.delete_vault_metadata(vault, max_retries)

def run_flow(user, api_key, flow_name, message, history='', invoke_method=None, 
            internal_vars=None, image_url=None, max_retries=5, **kwargs):
    """Legacy wrapper - prefer using CloudAPI instance"""
    api = CloudAPI(user, api_key)
    return api.run_flow(flow_name, message, history, invoke_method, internal_vars, image_url, max_retries, **kwargs)

def run_flow_stream(user, api_key, flow_name, message, history='', invoke_method=None, 
                   internal_vars=None, image_url=None, max_retries=5, **kwargs):
    """Legacy wrapper - prefer using CloudAPI instance"""
    api = CloudAPI(user, api_key)
    return api.run_flow_stream(flow_name, message, history, invoke_method, internal_vars, image_url, max_retries, **kwargs)


def call_proj():
    """Get the GCP project name (standalone utility)"""
    base = 'vector'
    encoded_str = base.encode()
    mid = ''
    for i, letter in enumerate(encoded_str):
