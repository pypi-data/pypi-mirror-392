...
def get_rfc_data(ctx: Context) -> RFCIndexData:
    """Lazy load RFC index data"""
    server_ctx = ctx.request_context.lifespan_context
    if server_ctx.get("data") is None:
        index_path = os.path.join(CACHE_DIR, "rfc-index.txt")
        download_rfc_index(CACHE_DIR)
        server_ctx["data"] = parse_rfc_index(index_path)
        logger.debug(f"data loaded: {server_ctx['data']}")
    return server_ctx["data"]
...
def list_ietf_docs_number(ctx: Context) -> int:
    ...
    """
    data = get_rfc_data(ctx)
    logger.debug(f"doc count:{data.docs_count}")
...
def search_ietf_rfc_by_keyword(keyword: str, ctx: Context) -> List[Dict[str, str]]:
    ...
    for number, title in data.rfc_titles.items():
...
def serve():
    import sys
    logger.info(f"Starting mcp-server-ietf. Cache directory: {CACHE_DIR}")
