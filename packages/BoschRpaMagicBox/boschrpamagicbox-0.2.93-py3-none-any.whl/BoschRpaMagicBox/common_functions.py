...
class WebSapCommonFunctions:
    ...
    def select_sap_layout(self, layout_name: str, shortcut_list: list):
        ...
        try:
            self.wait_element_presence_by_css_selector("span[aria-label='Starting at current line']")
            sleep(1)
            current_line_element = self.browser.find_element("span[aria-label='Starting at current line'][aria-checked='true']")
            self.move_to_and_click_element('', current_line_element)
            sleep(1)
        except:
            print(f'[WARNING]: Starting at current line option not found!\n{traceback.format_exc()}')
