...
def completion_command() -> str:
    py = sys.executable
    cmd = f"complete -o default -C '{py} -m myqueue.complete' mq"
    return cmd


def suggest_completions() -> None:
    """Suggest adding BASH tab-completion command."""
    venv = os.environ.get('VIRTUAL_ENV')
    if not venv:
        return

    suggested = Path(venv) / 'etc/myqueue/completion-suggested'
    if suggested.is_file():
        return

    cmd = completion_command()
    activate = Path(venv) / 'bin/activate'
    print('Tab-completion can be enabled by appending the following line:\n')
    print(f'  {cmd}\n')
    print('to your activate script:\n')
    print(f'  {activate}\n')
