...
from urllib.request import urlopen, Request, build_opener, ProxyHandler, HTTPHandler, HTTPSHandler
...
def _upload_file_to_oss(file_path: str) -> str:
    ...
    try:
        ...
        req = Request(UPLOAD_API_URL, data=body, headers=headers)

        disable_proxies = os.getenv("MCP_DISABLE_PROXIES") == "1"
        if disable_proxies:
            opener = build_opener(HTTPHandler(), HTTPSHandler(), ProxyHandler({}))
        else:
...
def make_id_photo(
    ...
    # 准备调用：远程 URL 直接使用；本地路径先上传以便返回可访问 URL
