...
n_Univ_NIBRS_orig<-read_csv(paste0(output_weighting_data_folder,'weights_MSA_cal_srs_altcombs_col.csv'),
                            guess_max=1e6)
...
#Update (05NOV2021): Adding guess_max argument
SF <- read_csv(paste0(output_weighting_data_folder,"SF_postMSA_cal_srs_altcombs_col.csv"),
               guess_max=1e6) %>%
...
#Stratum Totals
#Update (05NOV2021): Adding guess_max argument
SF_tots<-read_csv(paste0(output_weighting_data_folder,"SF_postMSA_cal_srs_altcombs_col.csv"),
...
ucr_SRS <- ucr_SRS_orig %>%
  ...
  left_join(SF_tots[,c("ORI_universe","county","MSA_NAME_COUNTY","PARENT_POP_GROUP_DESC2")],
            by=c("ORI_universe","county","MSA_NAME_COUNTY")) %>%
...
weight_state_lists_n_df<-data.frame(matrix(unlist(weight_state_lists_n), 
                                           nrow=length(weight_state_lists_n), 
                                           byrow=T))
...
dummyDatA<-
  read.csv(file.path(output_weighting_tableshell_folder,"TableAMSADummy.csv"))%>%
...
str1 <- file.path(output_weighting_populated_folder,
                  'TableA_MSA_Populated_SRS_AltCombs_Collapsed')
...
filein <- file.path(output_weighting_tableshell_folder,"TableA_MSA_Shell.xlsx")
