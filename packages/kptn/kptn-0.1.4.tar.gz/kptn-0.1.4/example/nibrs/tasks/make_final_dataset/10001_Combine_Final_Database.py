...
if __name__ == "__main__":
    ...
    final_folder = inputPipelineDir / "final-estimates"
    final_folder_out = outputPipelineDir / "final-estimates"
    final_folder_out.mkdir(exist_ok=True)

    for file in [
        "Indicator_Tables",
        "Indicator_Tables_flag_non_0_est_no_prb",
        ...
        """files_to_merge = [
            f
            for f in [f"{file}_{i}.csv" for i in list(range(1, 109))]
            if f in all_files
        ]
        write_to_out_file(
    ...
    # write this last merged file needed by validation scripts to sample estimates
    validation_der_files = os.listdir(
        outputPipelineDir / "validation_inputs" / "der_variable_name"
    )
    validation_der_files = [f for f in validation_der_files if str(f).endswith(".csv")]
    write_to_out_file(
        in_file_path=outputPipelineDir / "validation_inputs" / "der_variable_name",
        out_file_path=outputPipelineDir / "validation_inputs",
        file_name="der_variable_name_Merged.csv.gz",
        file_list=validation_der_files,
    )
