...
statusStep2 <- file.path(output_copula_data_folder,
                         paste0("Table_",temp.table,"_Imputation_Step2_Summary_Perm_",temp.perm,"_ColNum_",temp.colnum,"_",temp.subsetSuffix,"_Rates_",temp.stratVar,"_",temp.stratLvl,".csv")) %>%
  fread()
...
if (nrow(statusStep2)>0 & !any(statusStep2$failInd==TRUE)){
  ...
  statusStep3 <- sapply(1:length(columns),function(temp.colnum2){#15#14
    ...
    #06Jun2025: keep only the variables needed for this column
    temp3b <- data.frame(temp3) %>% 
      select(ORI,county,sourceInd,
             matches(str_c("_",columns[temp.colnum2],"$")))
    tempEnv2 <- environment()
    temp.statusStep3 <- sapply(1:2,function(temp.tier){#1
      if (temp.tier==1){
        temp.status <- getRatAdj(dat=temp3b,
                                 tier=1,
      ...
      #print(colnames(temp3b))
      dropCols <- intersect(colnames(temp3),colnames(temp3b)) %>%
        str_subset("^(ORI|county|sourceInd)$",negate=TRUE)
      temp3 <- data.frame(temp3) %>%
        select(-all_of(dropCols)) %>%
        full_join(temp3b,by=c("ORI","county","sourceInd"))
      list2env(list("temp3"=temp3),tempEnv)
      return(temp.status)
    },simplify=FALSE) %>% 
