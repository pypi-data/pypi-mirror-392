...
assign_row <- function(data){
  ...
  row = fcase(
    section == 1,  1,
    der_property_seized_any %in% c(1),  2, #Property seized
    der_property_seized_categories_short %in% c(1:5),  der_property_seized_categories_short + 2,
    der_property_seized_any %in% c(0),  8 #Property not seized
...
row == 1,  'Total',
row == 2,  'Property description: Property seized',
row == 3,  'Property seized: Drugs/narcotics',
row == 4,  'Property seized: Drug/narcotic equipment',
row == 5,  'Property seized: Chemicals',
row == 6,  'Property seized: Money',
row == 7,  'Property seized: Firearms',
row == 8,  'Property description: Property not seized'
...
column == 1,  'Completed Drug/Narcotic Violation'
  ...
  full_table = "TableDM3 - Property Type",
...
trim_upcase(estimate_type) %in% c('PERCENTAGE') & row == 2,  'Incident Level', #Property description: Property seized
trim_upcase(estimate_type) %in% c('PERCENTAGE') & row == 3,  'Incident Level subset to property seized', #Property seized: Drugs/narcotics
trim_upcase(estimate_type) %in% c('PERCENTAGE') & row == 4,  'Incident Level subset to property seized', #Property seized: Drug/narcotic equipment
trim_upcase(estimate_type) %in% c('PERCENTAGE') & row == 5,  'Incident Level subset to property seized', #Property seized: Chemicals
trim_upcase(estimate_type) %in% c('PERCENTAGE') & row == 6,  'Incident Level subset to property seized', #Property seized: Money
trim_upcase(estimate_type) %in% c('PERCENTAGE') & row == 7,  'Incident Level subset to property seized', #Property seized: Firearms
trim_upcase(estimate_type) %in% c('PERCENTAGE') & row == 8,  'Incident Level' #Property description: Property not seized
...
trim_upcase(estimate_type) %in% c('RATE') & row == 2,  DER_NA_CODE_STRING, #Property description: Property seized
trim_upcase(estimate_type) %in% c('RATE') & row == 3,  DER_NA_CODE_STRING, #Property seized: Drugs/narcotics
trim_upcase(estimate_type) %in% c('RATE') & row == 4,  DER_NA_CODE_STRING, #Property seized: Drug/narcotic equipment
trim_upcase(estimate_type) %in% c('RATE') & row == 5,  DER_NA_CODE_STRING, #Property seized: Chemicals
trim_upcase(estimate_type) %in% c('RATE') & row == 6,  DER_NA_CODE_STRING, #Property seized: Money
trim_upcase(estimate_type) %in% c('RATE') & row == 7,  DER_NA_CODE_STRING, #Property seized: Firearms
trim_upcase(estimate_type) %in% c('RATE') & row == 8,  DER_NA_CODE_STRING #Property description: Property not seized
...
generate_est <- function(maindata, subsetvareq1, column_number){
  ...
  #Property seized
  s2 <- agg_percent_by_incident_id(leftdata = main_filter, rightdata = agg_property_seized_any, var=der_property_seized_any, section=2)

  #Total Property seized
  der_property_seized_denom <- s2[[1]] %>%
    filter(der_property_seized_any == 1) %>% #Yes response
    select(final_count) %>%
    as.double()

  #Property seized - categories
  s3 <- agg_percent_by_incident_id_CAA(leftdata = main_filter, rightdata = agg_property_seized_categories_short, var=der_property_seized_categories_short, section=2, denom=der_property_seized_denom)
  ...
  final_reporting_database <-
    ...
    mutate(final_count = case_when(is.na(final_count) ~ 0,
           ...
           count    = case_when(row %in% c(1:8) ~ final_count,
                                      TRUE ~ DER_NA_CODE),
