...
log_info("Running 28_TablGV1a_part3_finalize.R")
...
#Create missing percentage if missing
raw_test_vars <- which(trim_upcase(colnames(final_reporting_database2)) == "PERCENTAGE")

#Create variable if missing
if(is_empty(raw_test_vars)) {
  final_reporting_database2 <- final_reporting_database2 %>%
    mutate(percentage = NA_real_)
}
...
final_ori_data2 <- final_ori_data %>%
  ...
  write.csv0(gzfile(paste0(final_path,"Table ", DER_TABLE_NAME, " ORI_PERSON.csv.gz")))
