...
source("../Table3b/util.R")

#####everything between main and setting constants
log_info("Running 34_Table3bunclear_part1_prepare_datasets.R")

full_table = "Table3bunclear-Person Victims-Rates"
...
#Subset to uncleared incidents

#Clearance
agg_clearance_cat             	<- fread(paste0(der_file_path, "agg_clearance_cat.csv.gz"))


log_debug("Before subset to uncleared incidents")
main <- merge(main, agg_clearance_cat[,.(incident_id, der_clearance_cat)],
              by = c("incident_id"), all.x = TRUE)
log_dim(main)

#Filter to uncleared incidents
main <- main[der_clearance_cat==1,]

log_debug("After subset to uncleared incidents")
log_dim(main)
