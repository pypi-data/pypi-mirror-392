...
file <- paste0("../TableShells/", "Indicator_Table_Shells Youth Homicide - YT2.xlsx")

# Load the existing workbook
wb <- loadWorkbook(file = file)
table_sheet = "TableYT2-Person Offenders"
...
for(i in 1:length(LIST_FINAL_DATA2)){
  ...
  if(i == length(LIST_FINAL_DATA2)) {
    #Save to list
    LIST_FINAL_DATA2[[i]] <- LIST_FINAL_DATA[[i]][1:CONST_NUMBER_OF_FILES_IN_LAST_LOOP] %>% reduce(merge, by="ori", all.x=TRUE)
  } else{
    #Save to list
    LIST_FINAL_DATA2[[i]] <- LIST_FINAL_DATA[[i]] %>% reduce(merge, by="ori", all.x=TRUE)
  }
...
final_ori_data2 <- LIST_FINAL_DATA2 %>%
  reduce(merge, by="ori", all.x=TRUE)
