...
log_info("Running 12_Table4a_part3_finalize.R")
...
gc(rm(list=c("weight_dataset","pop_data","pop_data_NATIONAL")))
...
log_debug("About to merge final data for excel...")
log_debug(system("free -mh", intern = FALSE))

final_data <- reduce(final_data_list, merge, by=c("section","row"), all.x=TRUE)

gc(rm(list=c("final_data_list")))
...
start_row = c(
  2, 4, 8, 19, 23, 30, 52, 57, 61, 67, 76, 83, 87)
...
log_debug("Finished excel. Starting reporting db...")
log_debug(system("free -mh", intern = FALSE))
...
gc(rm(list=c("final_reporting_database")))
...
log_debug("Finished reporting db. On to ORI file...")
log_debug(system("free -mh", intern = FALSE))
