...
log_info("Running 17_TableDM1_part1_prepare_datasets.R")
...
collist <- c(
    "der_drug_narcotic_equipment"
)

#Need to create new indicator for table after merging
# raw_main <- main %>%
#   left_join(agg_drug_narcotic_equipment_cat, by="incident_id") %>%
#   mutate(der_drug_narcotic_equipment = fcase(der_drug_narcotic_equipment_cat %in% c(1:3),  1, default = 0))
raw_main <- merge(raw_main, agg_drug_narcotic_equipment_cat, by="incident_id", all.x = TRUE)
raw_main <- raw_main[,der_drug_narcotic_equipment := fcase(der_drug_narcotic_equipment_cat %in% c(1:3), 1, default = 0)]
...
log_debug("After merging on weight_dataset")
log_debug(system("free -mh", intern = FALSE))
