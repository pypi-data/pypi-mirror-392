  ...
  if(nrow(raw_percentage_3) > 0) {
    ...
    raw_percentage_3_dt[,
      raw_denominator := fcase(
        #Total Incident Counts per Activity
        #Total Incident Counts per Drug Category
        ...
        row == 32, CREATE_PERCENTAGE_DENOMINATOR(c(32:37), column), #Agency indicator: City
        row == 33, CREATE_PERCENTAGE_DENOMINATOR(c(32:37), column), #Agency indicator: County
        row == 34, CREATE_PERCENTAGE_DENOMINATOR(c(32:37), column), #Agency indicator: University or college
        row == 35, CREATE_PERCENTAGE_DENOMINATOR(c(32:37), column), #Agency indicator: State police
        row == 36, CREATE_PERCENTAGE_DENOMINATOR(c(32:37), column), #Agency indicator: Other state agencies
        row == 37, CREATE_PERCENTAGE_DENOMINATOR(c(32:37), column), #Agency indicator: Tribal agencies
        #Total: Total
        row == 39, CREATE_PERCENTAGE_DENOMINATOR(c(39:42), column), #MSA: MSA Counties
        row == 40, CREATE_PERCENTAGE_DENOMINATOR(c(39:42), column), #MSA: Outside MSA
        row == 41, CREATE_PERCENTAGE_DENOMINATOR(c(39:42), column), #MSA: Non-MSA Counties
        row == 42, CREATE_PERCENTAGE_DENOMINATOR(c(39:42), column), #MSA: Missing
        row == 43, CREATE_PERCENTAGE_DENOMINATOR(c(1), column), #Location type 2: Residence/hotel
        row == 44, CREATE_PERCENTAGE_DENOMINATOR(c(1), column), #Location type 2: Transportation hub/outdoor public locations
        row == 45, CREATE_PERCENTAGE_DENOMINATOR(c(1), column), #Location type 2: Schools, daycares, and universities
        row == 46, CREATE_PERCENTAGE_DENOMINATOR(c(1), column), #Location type 2: Retail/financial/other commercial establishment
        row == 47, CREATE_PERCENTAGE_DENOMINATOR(c(1), column), #Location type 2: Restaurant/bar/sports or entertainment venue
        row == 48, CREATE_PERCENTAGE_DENOMINATOR(c(1), column), #Location type 2: Religious buildings
        row == 49, CREATE_PERCENTAGE_DENOMINATOR(c(1), column), #Location type 2: Government/public buildings
        row == 50, CREATE_PERCENTAGE_DENOMINATOR(c(1), column), #Location type 2: Jail/prison
        row == 51, CREATE_PERCENTAGE_DENOMINATOR(c(1), column), #Location type 2: Shelter-mission/homeless
        row == 52, CREATE_PERCENTAGE_DENOMINATOR(c(1), column), #Location type 2: Drug Store/Doctor Office/Hospital
        row == 53, CREATE_PERCENTAGE_DENOMINATOR(c(1), column) #Location type 2: Other/unknown location
      ),
