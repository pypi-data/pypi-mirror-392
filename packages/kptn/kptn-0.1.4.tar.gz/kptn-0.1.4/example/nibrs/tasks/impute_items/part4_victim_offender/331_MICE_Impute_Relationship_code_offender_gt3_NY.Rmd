---
title: "331_MICE_Impute_Relationship_code_offender_gt3"
...
NIBRS_NY <- data.frame(fread_logging(paste0(inputmainpath, "01_", input_state, "_recoded_for_imputation.csv.gz")))

#For NY, stacked data from MA as well
NIBRS_MA <- data.frame(fread_logging(paste0(inputmainpath, "01_", "MA", "_recoded_for_imputation.csv.gz")))

NIBRS <- bind_rows(NIBRS_NY, NIBRS_MA)

log_dim(NIBRS)
log_dim(NIBRS_NY)
log_dim(NIBRS_MA)

checkfunction(NIBRS, state_abbr)
...
processimputeddata <- function(type, state){
  ...
  #Read the file
  raw1 <- data.frame(fread_logging(paste0(inputmainpath, "17_", state, "_", type, "_imputed_final_flag.csv.gz"))) %>%
    select(
           #Keep ID variables
           incident_id,	!!inseq,
...
#NY
imp_victim_NY <- processimputeddata(type="victim", state="NY")
imp_offender_NY <- processimputeddata(type="offender", state="NY")
imp_arrestee_NY <- processimputeddata(type="arrestee", state="NY") %>%
  rename(arrestee_seq_num2 = arrestee_seq_num)

#MA
imp_victim_MA <- processimputeddata(type="victim", state="MA")
imp_offender_MA <- processimputeddata(type="offender", state="MA")
imp_arrestee_MA <- processimputeddata(type="arrestee", state="MA") %>%
  rename(arrestee_seq_num2 = arrestee_seq_num)

#Stack the data
imp_victim <- bind_rows(imp_victim_NY, imp_victim_MA)
imp_offender <- bind_rows(imp_offender_NY, imp_offender_MA)
imp_arrestee <- bind_rows(imp_arrestee_NY, imp_arrestee_MA)
...
NIBRS <- NIBRS %>% mutate(
  der_relationship_raw = fcase(
	...
	NIBRS %>% write_csv_logging(paste0(mainpath, "04_beforeimp_relationship_code_gt3_", input_state, ".csv.gz"), na="") 																									  
...
NIBRS %>%
  ...
  #Filter to NY agencies
  filter(trim_upcase(state_abbr) == "NY") %>%
	write_csv_logging(paste0(mainpath, "04_imputed_relationship_code_offender_gt3_", input_state, ".csv.gz"), na="" )

}else{

write_xlsx_logging(md2,paste0(miscpathout ,"mdpattern_relationship_code_", input_state, "_offender_gt3.xlsx"))
...
write_xlsx(as.data.frame(predQuick1), paste0(miscpathout, "predQuick_der_relationship_",input_state,"_offender_gt3.xlsx"))
...
write_xlsx_logging(as.data.frame(meth2), paste0(miscpathout, "meth_relationship_code_",input_state,"_offender_gt3.xlsx"))

imp = mice(NIBRS, pred=predQuick1, meth=meth2, m=1, seed=643220, maxit=5)
