...
NIBRS <- read_csv_logging(paste0(inputmainpath, "04_", input_state, "_offender_for_imputation.csv.gz"))

#Drop ethnicity
NIBRS <- NIBRS %>%
  select(-ethnicity_code_offender2) %>%
  #Drop Victim type ID since there are multiple victims
  select(-victim_type_code)
...
NIBRS %>%
    ...
    write_csv_logging(paste0(mainpath, "05_", input_state, "_offender_imputed.csv.gz"), na="" )

NIBRS %>%    
    write_csv_logging(paste0(mainpath, "05_", input_state, "_offender_mice.csv.gz"), na="" )	

    log_debug("Imputation NOT needed.")
}else{
    log_debug("Imputation IS needed.")
write_xlsx_logging(md2, paste0(miscpathout ,"05_mdpattern2_", input_state, ".xlsx"))
...
predQuick1[,c("incident_id","offender_seq_num")] <- 0 #Will not use variables as predictor by making 0

write_xlsx_logging(as.data.frame(predQuick1), paste0(miscpathout, "05_predQuick1_", input_state, ".xlsx"))
...
check0 <- NIBRS[1:100,]
write_xlsx_logging(check0,paste0(miscpathout, "05_check0_", input_state, ".xlsx"))

check1 <- NIBRS1[1:100,]
write_xlsx_logging(check1, paste0(miscpathout, "05_check1_", input_state, ".xlsx"))
...
Sex_offd_imputed %>% write_csv_logging(paste0(mainpath, "05_", input_state, "_offender_imputed.csv.gz"), na="" )
