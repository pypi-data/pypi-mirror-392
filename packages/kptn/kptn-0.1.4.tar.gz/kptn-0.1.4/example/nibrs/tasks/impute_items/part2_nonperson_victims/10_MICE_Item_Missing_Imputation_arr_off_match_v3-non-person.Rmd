...
NIBRS <- logical_edits_frame %>%
  ...
  #Drop relationship_id since there may be multiple victims
...
log_debug("Checking dimensions before createoffenserecodeindicator")
log_dim(NIBRS)
NIBRS <- createoffenserecodeindicator(data=NIBRS, type="offender", deduptype="victim")
log_debug("Checking dimensions after createoffenserecodeindicator")
log_dim(NIBRS)
...
log_debug("Checking dimensions of NIBRS after merging back to NIBRS")
log_dim(NIBRS)
summary(NIBRS)
...
NIBRS %>%
    ...
    write_csv_logging(paste0(mainpath, "10_", input_state, "_imputed_arrestee_offender_match.csv.gz"), na="" )

NIBRS %>%
    write_csv_logging(paste0(mainpath, "10_", input_state, "_mice_arrestee_offender_match.csv.gz"), na="" )  
...
write_xlsx_logging(md2,paste0(miscPathout ,"10_mdpattern2_", input_state, ".xlsx"))
...
write_xlsx_logging(as.data.frame(predQuick1), paste0(miscPathout, "10_predQuick1_", input_state, ".xlsx"))
write_xlsx_logging(as.data.frame(meth1),      paste0(miscPathout ,"10_meth1", input_state, ".xlsx"))
write_xlsx_logging(as.data.frame(meth2),      paste0(miscPathout ,"10_meth2", input_state, ".xlsx"))
...
check0 <- NIBRS[1:100,]
write_xlsx_logging(check0,paste0(miscPathout, "10_check0_", input_state, ".xlsx"))

check1 <- NIBRS1[1:100,]
write_xlsx_logging(check1, paste0(miscPathout, "10_check1_", input_state, ".xlsx"))
