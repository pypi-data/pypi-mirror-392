...
#Note (19Apr2024): Using reduced file (much smaller / faster to read in)
pop_raw <- paste0(external_path,"/",file_locs[[toString(year)]]$cbi_summary_county_reduced) %>%
  read_xlsx()
...
flag0LEAs <- SF_merge_stacked %>%
  ...
  mutate(groupImp=case_when(
    ...
    POPULATION_GROUP_DESC_UNIV=="Cities from 100,000 thru 249,999" ~ 2,
    #Cities from 50,000 - 99,999
    POPULATION_GROUP_DESC_UNIV=="Cities from 50,000 thru 99,999" ~ 3,
    #Cities from 25,000 - 49,999
    POPULATION_GROUP_DESC_UNIV=="Cities from 25,000 thru 49,999" ~ 4,
    #Cities from 10,000 - 24,999
    POPULATION_GROUP_DESC_UNIV=="Cities from 10,000 thru 24,999" ~ 5,
    #Cities from 2,500 - 9,999
    POPULATION_GROUP_DESC_UNIV=="Cities from 2,500 thru 9,999" ~ 6,
    #Cities under 2,500
    POPULATION_GROUP_DESC_UNIV=="Cities under 2,500" ~ 7,
    #Non-MSA counties and non-MSA State Police
    POPULATION_GROUP_DESC_UNIV=="Non-MSA counties 100,000 or over" ~ 8,
    POPULATION_GROUP_DESC_UNIV=="Non-MSA counties from 25,000 thru 99,999" ~ 8,
    POPULATION_GROUP_DESC_UNIV=="Non-MSA counties from 10,000 thru 24,999" ~ 8,
    POPULATION_GROUP_DESC_UNIV=="Non-MSA counties under 10,000" ~ 8,
    POPULATION_GROUP_DESC_UNIV=="Non-MSA State Police" ~ 8,
    #MSA counties and MSA State Police
    POPULATION_GROUP_DESC_UNIV=="MSA counties 100,000 or over" ~ 9,
    POPULATION_GROUP_DESC_UNIV=="MSA counties from 25,000 thru 99,999" ~ 9,
    POPULATION_GROUP_DESC_UNIV=="MSA counties from 10,000 thru 24,999" ~ 9,
    POPULATION_GROUP_DESC_UNIV=="MSA counties under 10,000" ~ 9,
    POPULATION_GROUP_DESC_UNIV=="MSA State Police" ~ 9,
    TRUE ~ 0
  )) %>%
  split(.,.$groupImp)
...
#12Mar2024: including METRO_DIVISION_COUNTY
#19Apr2024: swapping out POP1 for popResidAgcyCounty_cbi and adding propMult ... in addition, adding ORI (alongside LEGACY_ORI)
...
SF <- SF%>%
  ...
  #18Jun2024: no longer use universe file's MSA/JD/FO/MD if missing/NA on population file
  # mutate(MSA_NAME_COUNTY=ifelse(MSA_NAME_COUNTY %in% c("Not Specified","Not Available") & (is.na(COUNTY_NAME_CENSUS)|COUNTY_NAME_CENSUS=="NOT SPECIFIED") & !is.na(MSA_UNIV),MSA_UNIV,MSA_NAME_COUNTY),
         ...
         # METRO_DIVISION_COUNTY=ifelse(METRO_DIVISION_COUNTY %in% c("Not Specified","Not Available") & (is.na(COUNTY_NAME_CENSUS)|COUNTY_NAME_CENSUS=="NOT SPECIFIED") & !is.na(METRO_DIVISION_UNIV),METRO_DIVISION_UNIV,METRO_DIVISION_COUNTY)) %>%
  ...
  #19Apr2024: use popResidAgcyCounty_cbi instead of POP1
  #mutate(POP1=ifelse(is.na(POP1) & POPULATION_UNIV==0,0,POP1)) %>%
  mutate(popResidAgcyCounty_cbi=ifelse(is.na(popResidAgcyCounty_cbi) & POPULATION_UNIV==0,0,popResidAgcyCounty_cbi)) %>%
  #19Apr2024: we're switching away from using propPOP1 and towards using propMult
  ...
  #19Apr2024: just to be safe let's assign propMult to 1 if missing (unsure if this ever is the case)
  mutate(propMult=ifelse(is.nan(propMult)|is.na(propMult),1,propMult)) %>%
  #ungroup() %>%
  # mutate(across(matches("^totcrime.*_imp"),
  # ~.x*propPOP1),
  # POPULATION_UNIV=POPULATION_UNIV*propPOP1)
  mutate(across(matches("^totcrime.*_imp"),
                ~.x*propMult),
         POPULATION_UNIV=POPULATION_UNIV*propMult)


#######################
# output data for next program in sequence

fwrite_wrapper(SF, paste0(output_weighting_data_folder,"SF_srs.csv"))

log_info("Finished 02_Weights_Data_Setup_SRS.R\n\n")
