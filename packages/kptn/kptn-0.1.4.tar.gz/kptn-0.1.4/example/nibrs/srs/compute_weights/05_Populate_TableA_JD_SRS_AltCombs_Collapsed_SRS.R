...
log_info("Running 05_Populate_TableA_JD_SRS_AltCombs_Collapsed_SRS.R")
...
n_Univ_NIBRS_orig<-read_csv(paste0(output_weighting_data_folder,'weights_jd_cal_srs_altcombs_col_srs.csv'),
                            guess_max=1e6)
...
#Update (05NOV2021): Adding guess_max argument
SF <- read_csv(paste0(output_weighting_data_folder,"SF_postJD_cal_srs_altcombs_col_srs.csv"),
               guess_max=1e6) %>%
...
#Stratum Totals
#Update (05NOV2021): Adding guess_max argument
SF_tots<-read_csv(paste0(output_weighting_data_folder,"SF_postJD_cal_srs_altcombs_col_srs.csv"),
...
ucr_SRS <- ucr_SRS_orig %>%
  ...
  left_join(SF_tots[,c("ORI_universe","county","JUDICIAL_DISTRICT_NAME","PARENT_POP_GROUP_DESC_UNIV2")],by=c("ORI_universe","county","JUDICIAL_DISTRICT_NAME")) %>%
  inner_join(select(n_Univ_NIBRS_orig,"ORI_universe",county,JDWgt,JUDICIAL_DISTRICT_NAME),by=c("ORI_universe","county","JUDICIAL_DISTRICT_NAME")) %>%
  mutate(one=1)
...
dummyDatA<-
  read.csv(file.path(output_weighting_tableshell_folder,"TableAJDDummySRS.csv"))%>%
...
str1 <- file.path(output_weighting_populated_folder,'TableA_JD_Populated_SRS_AltCombs_Collapsed_SRS')

filein <- file.path(output_weighting_tableshell_folder,"TableA_JD_Shell_SRS.xlsx")
...
log_info("Finished 05_Populate_TableA_JD_SRS_AltCombs_Collapsed_SRS.R")