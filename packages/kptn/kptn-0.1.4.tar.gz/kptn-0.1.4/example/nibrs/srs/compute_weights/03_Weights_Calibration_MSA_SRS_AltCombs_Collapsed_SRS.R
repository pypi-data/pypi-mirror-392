...
srK <- 30 #This is the k (e.g., 30) to use for self-reporting agencies
...
#Update (13AUG2025): Automatically handling assignments of metro divisions and 
#                      self-reporting agencies (previously, manually tracked).
#                    Removing use of gencalib() with only overall crime if 
...
log_info("Running 03_Weights_Calibration_MSA_SRS_AltCombs_Collapsed_SRS.R")

#Note (23Jun2023): Reading in new crossing worksheet
...
SF <- paste0(input_weighting_data_folder,"SF_postJD_cal_srs_altcombs_col_srs.csv") %>%
...
SF2 <- SF %>%
  ...
  mutate(wgtGpMSA=case_when(POPULATION_GROUP_DESC_UNIV=="Cities 1,000,000 or over" ~ 1,
  ...
  #Note (29Apr2024): Drop all calibration variables that already exist (e.g., FO and JD)
...
#06Jun2024: implementing self-reporting treatment:
...
srEligible <- SF2 %>%
  ...
  mutate(mean_totcrime_imp_msa=mean(totcrime_imp),
         POP_ELIG=sum(POPULATION_UNIV),
         POP_RESP=sum(POPULATION_UNIV*resp_ind_srs),
...
#Note (12Mar2024): we now need to incorporate metro division into the groups
...
#Note (06Jun2024): Updating to reflect addition of the self-representing weighting group
#Note (13Aug2025): Automatically tracking lists of metro divisions
if (year >= 2022){
  mdList <- SF2 %>% 
  ...
  wgtGpMSADescs2 <- c("Not self-representing - Not Select Metro Division - Nonzero population agencies",
                      ...
                      expand.grid(str_c("Not self-representing - ",mdList," - "),
                        ...
                        pull(string),
                      "Self-representing")
  #13Aug2025: this will help assign wgtGpMSA2 for cases in select metro divisions...
  mdCW <- SF2 %>%
    ungroup() %>%
...
#Note (12Mar2024): Also need to now worry about respecting metro divisions when collapsing
crossings_col <- ratio %>%
...
#12Mar2024: no longer update weighting groups here
#wgtGpsMSA2 <- 1:3
...
SF2_skips <- SF2 %>%
  ...
  mutate(MSAWgt=ifelse(resp_ind_srs==1,1,NA_real_))
SF2_skipsB <- SF2 %>%
...
SF2_wgts2_test <- sapply(msaGps2,function(i){#Loop over weight groupings
  ...
  SF_temp_MSA <- SF2 %>%
    subset(MSA_NAME_COUNTY==i & resp_ind_srs==1)
  ...
  SF2_wgts_MSA <- sapply(temp.wgtGps,function(j){#Loop over weight groupingstemp.#wgtGps,function(j){#Loop over weight groupings
    ...
    #Note (09Jan2023): Removing totcrime_imp requirement
    SF_temp <- SF_temp_MSA %>%
      subset(wgtGpMSA==j & resp_ind_srs==1)
    if (nrow(SF_temp)>0){
  ...
  if (nrow(SF2_wgts_MSA)>0){
    SF2_wgts_MSA <- SF2_wgts_MSA %>%
      ...
      left_join(SF_temp_MSA,by=c("ORI","county")) #by=colnames(SF_temp_MSA))
  }
...
#   mutate(MSAWgt_nVar0_comb1=ifelse(resp_ind_srs==1 & rowSums(!is.na(select(.,matches("MSAWgt_nVar\\d"))),na.rm=TRUE)==0,
...
combAll_table <- bind_rows(comb10_table_test,
  ...
  subset(!is.na(n)) %>% #13Mar2024: combination convergence has gotten large (e.g., ~200 MB for 2022 rerun) -> only keep weighting groups that actually appear
...
log_debug("Writing excel workbook SRS_Variable_Combination_Convergence_by_MSA_X_Weighting_Group_AltCombs_Collapsed_Automatic_SRS.xlsx")
workbook <- paste0(output_weighting_data_folder,
                   "SRS_Variable_Combination_Convergence_by_MSA_X_Weighting_Group_AltCombs_Collapsed_Automatic_SRS.xlsx")
wb <- createWorkbook()
...
SF2 <- SF2 %>%
  arrange(ORI) %>%
  left_join(msaCrosses2) %>% #23Jun2023: joining to cross list
...
#06Jun2024: accounting for addition of new self-reporting weighting group throughout this section
if (nrow(ctrlVars)==SF2 %>% select(MSA_NAME_COUNTY,wgtGpMSA) %>% subset(duplicated(.)==FALSE) %>% nrow()){
  ...
  SF2_wgts2 <- sapply(msaGps2,function(i){#Loop over weight groupings - just gp 1 for test
    ...
    SF_temp_MSA <- SF_MSA %>%
      subset(resp_ind_srs==1)
    #temp_MSA_code <- SF2 %>% subset(MSA_NAME_COUNTY==i) %>% getElement("MSA_CODE") %>% unique()
    ...
    #13Aug2025: automatically handle creation of temp_wgtGpX_code variables
    if (year >= 2022){
    ...
    ctrlTtlsM_MSA <- srs_MSA_control_totals %>%
      subset(MSA_NAME_COUNTY==i) %>%
      left_join(msaCrosses2) %>% #23Jun2023: merging on cross number
      mutate(wgtGpMSA2=msaCrossNum) %>%
      arrange(wgtGpMSA) %>%
    ...
    #13Aug2025: specifying reshape2::melt() (and dcast())
    ...
    ctrlVars2_MSA <- ctrlVarsM2_MSA %>%
      data.frame() %>%
      #Note (10Jan2023): Removing totcrime_imp requirement
      mutate(ORI=SF_MSA %>% arrange(ORI) %>% subset(MSA_NAME_COUNTY==i & resp_ind_srs==1) %>% pull(ORI),
             county=SF_MSA %>% arrange(ORI) %>% subset(MSA_NAME_COUNTY==i & resp_ind_srs==1) %>% pull(county),
             wgtGpMSA2=SF_MSA %>% arrange(ORI) %>% subset(MSA_NAME_COUNTY==i & resp_ind_srs==1) %>% pull(wgtGpMSA2))
    ...
    #13Aug2025: automatically handle updating ctrlVars2_MSA for remaining groups
    if (year >= 2022){
      for (j in 4:max(wgtGpsMSA2)){
    ...
    SF_temp_wgts2 <- SF2_wgts2_test %>%
      subset(resp_ind_srs==1 & MSA_NAME_COUNTY==i) %>%
    ...
    SF2_wgts_MSA <- sapply(temp.wgtGps,function(j){#Loop over weight groupings
      ...
      #13Aug2025: automatically handle remaining wgt groups' temp_wgtGp_code
      if (year >= 2022){
      ...
      if (nrow(SF_temp)>0){
        ...
        temp.nElig <- SF2 %>%
          subset(MSA_NAME_COUNTY==i & wgtGpMSA==j) %>%
          nrow()
        #06Jun2024: round weight before checking for weights <1
    ...
    bind_rows()%>%
    #13Mar2024: reducing number of variables to only ID variables plus new variables (gWgt, MSAWgt, and calibration variables
  ...
  new_weights[,c("ORI_universe","county","MSA_NAME_COUNTY","wgtGpMSA","wgtGpMSADesc",
    ...
    fwrite_wrapper(paste0(output_weighting_data_folder,'weights_MSA_cal_srs_altcombs_col_srs.csv'))
  ...
  oldSF <- fread(paste0(input_weighting_data_folder,"SF_postJD_cal_srs_altcombs_col_srs.csv"))%>%
    #Adding just in case already on file...
    #29Apr2024: comment out the line below
    #select(-matches(paste0("(wgtGpMSA|",calVars,")")))%>%
    mutate(totcrime_imp=totcrime_violent_imp+totcrime_property_imp) %>%
    mutate(totcrime_agg_rob_imp=totcrime_aggAssault_imp+totcrime_rob_imp)
  #Note (23Jun2023): Binding together SF2 and SF2_skips/SF2_skipsB/SF2_skipsC in 1st left join
  newSF <- oldSF %>%
  ...
  fwrite_wrapper(newSF,paste0(output_weighting_data_folder,"SF_postMSA_cal_srs_altcombs_col_srs.csv"))
  
  #Note (JDB 06Jul2022): Export weight check results
  #write_csv(results_MSA,
  fwrite_wrapper(results_MSA,
                 paste0(output_weighting_data_folder,"weights_MSA_cal_srs_altcombs_col_checks_srs.csv"))
  #Track list of MSAs
  msa_all <- SF %>%
  ...
  fwrite_wrapper(msa_all,
                 paste0(output_weighting_data_folder,"msa_all_srs.csv"))
  
} else {
  stop("No calibration model converged for 1+ (MSA X weighting group) crossing")
}
log_info("Finished 03_Weights_Calibration_MSA_SRS_AltCombs_Collapsed_SRS.R\n\n")
