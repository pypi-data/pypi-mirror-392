...
#19Apr2024: implementing self-reporting (SR) approach
...
log_info("Running 03_Weights_Calibration_JD_SRS_AltCombs_Collapsed_SRS.R")
...
SF <- paste0(input_weighting_data_folder,"SF_postFO_cal_srs_altcombs_col_srs.csv") %>%
...
SF2 <- SF %>%
  ...
  mutate(wgtGpJD=case_when(POPULATION_GROUP_DESC_UNIV=="Cities 1,000,000 or over" ~ 1,
  ...
  #Note (29Apr2024): Drop all calibration variables that already exist (e.g., FO)
...
#19Apr2024: implementing self-reporting treatment:
...
#22Apr2024: actually, we're switching from looking at the means at the agency level within each state to the means at the (agency X county) level within each JD
srEligible <- SF2 %>%
...
#22Apr2024: updating lvls to reflect self-representing LEAs
wgtGpsJD2 <- 1:4
wgtGpJDDescs2 <- c("Not self-representing - Nonzero population agencies",
                   "Not self-representing - Zero population agencies",
...
SF2_skips <- SF2 %>%
  ...
  mutate(JDWgt=ifelse(resp_ind_srs==1,1,NA_real_))
...
SF2_wgts2_test <- sapply(jdGps2,function(i){#Loop over weight groupings
  ...
  SF_temp_jd <- SF2 %>%
    subset(JUDICIAL_DISTRICT_NAME==i & resp_ind_srs==1)
  ...
  SF2_wgts_jd <- sapply(temp.wgtGps,function(j){#Loop over weight groupingstemp.#wgtGps,function(j){#Loop over weight groupings
    ...
    #Note (09Jan2023): Removing totcrime_imp requirement
    SF_temp <- SF_temp_jd %>%
      subset(wgtGpJD==j & resp_ind_srs==1)
    if (nrow(SF_temp)>0){
    ...
    bind_rows()
  #13Mar2024: getting errors when merging in certain JDs... modifying merge (only keep certain variables from SF2_wgts_JD)
  if (nrow(SF2_wgts_jd)>0){
    SF2_wgts_jd <- SF2_wgts_jd %>%
      select(ORI,county,matches("^JDWgt_")) %>% #13Mar2024: added today
...
#   mutate(JDWgt_nVar0_comb1=ifelse(resp_ind_srs==1 & rowSums(!is.na(select(.,matches("JDWgt_nVar\\d"))),na.rm=TRUE)==0,
...
log_debug("Writing excel workbook SRS_Variable_Combination_Convergence_by_JD_X_Weighting_Group_AltCombs_Collapsed_Automatic_SRS.xlsx")
workbook <- paste0(output_weighting_data_folder,
                   "SRS_Variable_Combination_Convergence_by_JD_X_Weighting_Group_AltCombs_Collapsed_Automatic_SRS.xlsx")
wb <- createWorkbook()
...
SF2 <- SF2 %>%
  arrange(ORI) %>%
  left_join(jdCrosses2) %>% #22Jun2023: joining to cross list
...
if (nrow(ctrlVars)==SF2 %>% select(JUDICIAL_DISTRICT_NAME,wgtGpJD) %>% subset(duplicated(.)==FALSE) %>% nrow()){
  ...
  SF2_wgts2 <- sapply(jdGps2,function(i){#Loop over weight groupings - just gp 1 for test
    ...
    SF_temp_jd <- SF_jd %>%
      subset(resp_ind_srs==1)
    #temp_jd_code <- SF2 %>% subset(JUDICIAL_DISTRICT_NAME==i) %>% getElement("JD_CODE") %>% unique()
    ...
    ctrlTtlsM_jd <- srs_jd_control_totals %>%
      subset(JUDICIAL_DISTRICT_NAME==i) %>%
      left_join(jdCrosses2) %>% #22Jun2023: merging on cross number
      mutate(wgtGpJD2=jdCrossNum) %>%
      arrange(wgtGpJD) %>%
    ...
    ctrlVars2_jd <- ctrlVarsM2_jd %>%
      data.frame() %>%
      #Note (10Jan2023): Removing totcrime_imp requirement
      mutate(ORI=SF_jd %>% arrange(ORI) %>% subset(JUDICIAL_DISTRICT_NAME==i & resp_ind_srs==1) %>% pull(ORI),
             county=SF_jd %>% arrange(ORI) %>% subset(JUDICIAL_DISTRICT_NAME==i & resp_ind_srs==1) %>% pull(county),
             wgtGpJD2=SF_jd %>% arrange(ORI) %>% subset(JUDICIAL_DISTRICT_NAME==i & resp_ind_srs==1) %>% pull(wgtGpJD2)) 
    ...
    SF_temp_wgts2 <- SF2_wgts2_test %>%
      subset(resp_ind_srs==1 & JUDICIAL_DISTRICT_NAME==i) %>%
    ...
    SF2_wgts_jd <- sapply(temp.wgtGps,function(j){#Loop over weight groupings
      ...
      if (nrow(SF_temp)>0){
        ...
        #Update (13Aug2024): added min and max to checks
    ...
    bind_rows()%>%
    #13Mar2024: issues during merge... reducing number of variables to only ID variables plus new variables (gWgt, JDWgt, and calibration variables
    select(ORI,county,gWgt,JDWgt,matches("V\\d+_\\w")) %>%
    #full_join(SF2,by=colnames(SF2))
    #29Apr2024: dropping existing calibration variables (e.g., from FO)
    full_join(SF2 %>%
                select(-matches("^V\\d+_\\w")),
              by=c("ORI","county"))
  ...
  #Note (19Apr2024): Also stacking SR agencies
  new_weights <- SF2_wgts2 %>%
    bind_rows(SF2_skips,
              SF2_skipSR)
  
  ### export for others to start writing functions to analyze bias, MSE, etc.
  new_weights[,c("ORI_universe","wgtGpJD","wgtGpJDDesc","county","JUDICIAL_DISTRICT_NAME",
    ...
    fwrite_wrapper(paste0(output_weighting_data_folder,'weights_jd_cal_srs_altcombs_col_srs.csv'))
  ...
  oldSF <- fread(paste0(input_weighting_data_folder,"SF_postFO_cal_srs_altcombs_col_srs.csv"))%>%
    #Adding just in case already on file...
    #29Apr2024: comment out the line below
    #select(-matches(paste0("(wgtGpJD|",calVars,")")))%>%
    mutate(totcrime_imp=totcrime_violent_imp+totcrime_property_imp) %>%
    mutate(totcrime_agg_rob_imp=totcrime_aggAssault_imp+totcrime_rob_imp)
  #Note (23Jun2023): Binding together SF2 and SF2_skips in 1st left join
  #Note (29Apr2024): Including self-representing LEAs...
  newSF <- oldSF %>%
  ...
  fwrite_wrapper(newSF,paste0(output_weighting_data_folder,"SF_postJD_cal_srs_altcombs_col_srs.csv"))
  
  #Note (JDB 06Jul2022): Export weight check results
  #write_csv(results_jd,
  fwrite_wrapper(results_jd,
                 paste0(output_weighting_data_folder,"weights_jd_cal_srs_altcombs_col_checks_srs.csv"))
  ...
  fwrite_wrapper(jd_all,
                 paste0(output_weighting_data_folder,"jd_all_srs.csv"))
  
} else {
  stop("No calibration model converged for 1+ (judicial district X weighting group) crossing")
}
log_info("Finished 03_Weights_Calibration_JD_SRS_AltCombs_Collapsed_SRS.R\n\n")
