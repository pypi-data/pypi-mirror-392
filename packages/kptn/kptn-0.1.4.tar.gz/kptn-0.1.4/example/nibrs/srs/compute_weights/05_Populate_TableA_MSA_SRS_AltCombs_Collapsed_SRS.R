#########################################################################
#Program      : 05_Populate_TableA_MSA
...
log_info("Running 05_Populate_TableA_MSA_SRS_AltCombs_Collapsed_SRS.R")
...
n_Univ_NIBRS_orig<-read_csv(paste0(output_weighting_data_folder,'weights_MSA_cal_srs_altcombs_col_srs.csv'),
                            guess_max=1e6)

NumStates<-n_Univ_NIBRS_orig %>% subset(!is.na(MSAWgt)) %>% pull(MSA_NAME_COUNTY) %>% unique() %>% length()

#SRS
#Update (27OCT2021): Use variables on SF instead of recreating based on cleanframe + srs2016_2020_smoothed
#Update (05NOV2021): Adding guess_max argument
SF <- read_csv(paste0(output_weighting_data_folder,"SF_postMSA_cal_srs_altcombs_col_srs.csv"),
               guess_max=1e6) %>%
  subset(!is.na(MSA_NAME_COUNTY))


ucr_SRS_orig <- SF %>%
  select(ORI_universe,county,MSA_NAME_COUNTY,matches("totcrime.*_imp"))
...
#Stratum Totals
#Update (05NOV2021): Adding guess_max argument
SF_tots<-read_csv(paste0(output_weighting_data_folder,"SF_postMSA_cal_srs_altcombs_col_srs.csv"),
                  col_types = cols_only("wgtGpMSA"=col_integer(),
                                        "county"=col_character(),
                                        "MSA_NAME_COUNTY"=col_character(),
...
SF_tots_state_<-SF_tots %>%
  group_by(wgtGpMSA,MSA_NAME_COUNTY) %>%
  summarise(Nh=n())

SF_tots_state<-as.data.frame(SF_tots_state_) %>%
  mutate(wgtGpMSA_new = row_number())


SF_tots_all_fin<-inner_join(SF_tots_all,SF_tots,by=c("wgtGpMSA"))

SF_tots_all_state_fin_<-inner_join(SF_tots_state,SF_tots,by=c("wgtGpMSA","MSA_NAME_COUNTY"))
SF_tots_all_state_fin_<-SF_tots_all_state_fin_ %>%
  rename(wgtGpMSA_old=wgtGpMSA)


SF_tots_all_state_fin<-SF_tots_all_state_fin_%>%rename(wgtGpMSA=wgtGpMSA_new)
...
#  inner_join(SF_tots[,c("ORI_universe","wgtGpMSA","Nh")],by=c("ORI_universe")) %>%
...
ucr_SRS <- ucr_SRS_orig %>%
  inner_join(SF_tots_all_state_fin[,c("ORI_universe","county","MSA_NAME_COUNTY","wgtGpMSA","Nh")],
             by=c("ORI_universe","county","MSA_NAME_COUNTY")) %>%
  left_join(SF_tots[,c("ORI_universe","county","MSA_NAME_COUNTY","PARENT_POP_GROUP_DESC_UNIV2")],
            by=c("ORI_universe","county","MSA_NAME_COUNTY")) %>%
  inner_join(select(n_Univ_NIBRS_orig,ORI_universe,county,MSAWgt,MSA_NAME_COUNTY),
             by=c("ORI_universe","county","MSA_NAME_COUNTY")) %>%
  mutate(one=1)
...
#####################################################################
weight_state_lists<-(colnames(n_Univ_NIBRS_orig) %>%str_subset("MSAWgt") %>%as.list()) #List of weights, update as necessary


weight_state_lists_n<-list("MSAWgt")
...
msaall <- SF %>% select(MSA_NAME_COUNTY) %>% unique() %>% arrange(MSA_NAME_COUNTY)
...
dummyDatA<-
  read.csv(file.path(output_weighting_tableshell_folder,"TableAMSADummySRS.csv"))%>%
...
####################Table A states#################################
compute_msaTotals_TabA <- function(wtvar,indata,indicator){
  respWgts <- indata %>%
    rename(wt=all_of(wtvar)) %>%
    subset(!is.na(totcrime_imp)) %>%
    subset(!is.na(wt)) %>% #Switch to indicator?
    svydesign(ids= ~1,data=.,strata=.$wgtGpMSA,weights=.$wt,fpc=.$Nh)
  
  df<-data.frame(genAppA_msa(respWgts))
  
  state_tot<-gather(data=df,key,value,starts_with("tot"),-MSA_NAME_COUNTY)%>%mutate(RowValue = as.numeric(row_number()))%>% select(-contains("se."))
  state_setot<-gather(data=df,key,value,starts_with("se"),-MSA_NAME_COUNTY)%>%mutate(RowValue = as.numeric(row_number()))%>% select(-contains("tot"))
  
  
  state_all0<-merge(state_tot,state_setot,by=c("MSA_NAME_COUNTY","RowValue"))%>%rename(Total=value.x)%>%rename(SE=value.y)%>% mutate(state_name=MSA_NAME_COUNTY)
...
msaEstimates_TabA_SRS <-(
  compute_msaTotals_TabA(wtvar="one",indata=ucr_SRS,indicator="inNIBRS") %>%
    .[[1]])
...
msaEstimates_TabA_nibrsReps <- (
  sapply(weight_state_lists,compute_msaTotals_TabA,indata=ucr_SRS,indicator="inNIBRS"))


######
#Merge estimates for SRS + NIBRS reporters
list_of_tablesA_msas <- (
  sapply(1:length(msaEstimates_TabA_nibrsReps),
         function(subTab){
           inner_join(x=msaEstimates_TabA_nibrsReps[[subTab]],
                      y=msaEstimates_TabA_SRS,
...
names(list_of_tablesA_msas) <- weight_state_lists

##For summary of Relative Bias Table###
summary_tableA_msas<-list_of_tablesA_msas %>% bind_rows(.id="weightVar") %>%
...
summary_tableA_msaspt1<-summary_tableA_msas
...
str1 <- file.path(output_weighting_populated_folder,'TableA_MSA_Populated_SRS_AltCombs_Collapsed_SRS')

filein <- file.path(output_weighting_tableshell_folder,"TableA_MSA_Shell_SRS.xlsx")



# Load the existing workbook
wbTabAMSA <- loadWorkbook(file =filein )
...
writeData(wbTabAMSA, sheet = "Summary_MSA_TabA_Ex1", x = summary_tableAKeep, startCol = 3, startRow = 8, colNames = FALSE,keepNA=FALSE)

saveWorkbook(wbTabAMSA, file=paste0(str1, format(Sys.time(),'_%Y%m%d'), '.xlsx'), overwrite = TRUE)
...
write_files_xlsx(listfile_name = list_of_tablesA_msas,
...
log_info("Finished 05_Populate_TableA_MSA_SRS_AltCombs_Collapsed_SRS.R")