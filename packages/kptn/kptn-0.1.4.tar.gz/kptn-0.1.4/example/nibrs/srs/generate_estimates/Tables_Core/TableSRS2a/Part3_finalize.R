...
load(paste0(in_file_path,"/SRS2a_prep_env.RData"))
...
log_info("Running 02_TableSRS2a_part3_finalize.R")
...
for(i in 1:maximum_column){
  ...
  #Get the data object
  raw_object <- readRDS(paste0(out_file_path,"/data_SRS2a_",i,".rds"))
...
listofvars_percent <- final_data %>% select(starts_with("percent")) %>% colnames()
print(listofvars_percent)

final_data2 <- NA_to_0_count(data=final_data, list=listofvars_count)
final_data3 <- NA_to_0_percent(data=final_data2, list=listofvars_percent, keepNA=1)
...
file <- paste0("../tableshells/", "SRS_Indicator_Table_Shells.xlsx")
...
fileout <- paste0(final_path, "Indicator_Table_SRS2a.xlsx")
...
final_reporting_database2 %>%
  ...
  write.csvSpace(paste0(final_path,"Table ", DER_TABLE_NAME, "_Reporting_Database.csv"))
