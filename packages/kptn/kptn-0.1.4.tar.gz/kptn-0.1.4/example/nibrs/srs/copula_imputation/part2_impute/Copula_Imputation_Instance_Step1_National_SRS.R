...
#Note (18Dec2024): Creating national SRS pipeline version (e.g., for 30 year run). Heavily based off original SRS version.
nstacks <- 10
...
temp.uid <- paste0(temp.table,"_",temp.perm,"_",temp.colnum,"_",temp.subsetSuffix,"_Rates_",temp.stratVar,"_",temp.stratLvl,"_National_SRS") #Unique ID (table X perm X col set # X subset X "Rates" X strat var X strat lvl) for temp files
#Get current permutation
#10Oct2023: due to growing file size of permutation file, select only the columns we care about
temp.permDatCols <- paste0(filepathin_initial, "POP_TOTALS_PERM_", year, "_SRS.csv") %>%
  read_csv(n_max=0) %>%
  colnames()
...
temp.permDat <- paste0(filepathin_initial, "POP_TOTALS_PERM_", year, "_SRS.csv") %>%
...
temp.rowSpecs <- read_csv("../data/Indicator_Table_Row_Specs_National_SRS.csv") %>%
...
if (temp.demographics==0 & as.numeric(temp.perm)>1000){
  ...
  temp.allVars <-fread(file=file.path(output_copula_data_folder,
  ...
  tableDat <- fread(file=file.path(output_copula_data_folder,
                                   paste0("Table_",temp.table,"_ORI_all_national_SRS.csv")),
  ...
  if (length(temp.ttlVars)==0){
    ...
    tableDat <- tableDat %>%
      mutate(across(all_of(temp.ttlVars),.fns = ~ifelse(sourceInd=="SRS Reporter",0,NA_real_)))
  }
  ...
  temp.step2Tier2Rows <- sapply(1:length(temp.step2Tier2Sects),function(i){
    colnames(tableDat) %>%
      str_subset(paste0("^t_",temp.table,"_",temp.step2Tier2Sects[i],"_",temp.step2Tier2Rows[i],"_1$")) %>%
      str_extract(paste0("(?<=t_",temp.table,"_\\d{1,2}_)\\d+(?=_\\d+)"))
  },simplify=FALSE)
  ...
  if (nrow(temp)==0){
    ...
    #if (!any(sourceInd$sourceInd=="SRS Non-reporter")){
    if (!any(temp$sourceInd=="SRS Non-reporter")){
      log_debug(paste0("No SRS Non-reporting LEAs in ",as.character(temp.permDesc),". Skip steps 1-3."))
      log_debug(system("free -mh", intern = FALSE))
      temp2 <- temp
      ...
      sapply(1:nrow(temp.step1Specs),function(i){
        ...
        if (length(temp.step1TtlVars)>=1 & is.na(temp.special)){
          ...
          sapply(1:length(temp.step1TtlVars),function(i){
            ...
            temp <- temp %>%
              mutate(!!temp.step1TtlVar := ifelse(sourceInd=="SRS Reporter",
          ...
          temp <- temp %>%
            mutate(across(.cols=matches(temp.step1TtlVars),.fns=function(x){ifelse(sourceInd=="SRS Reporter",x,p*rowSums(dplyr::select(.,all_of(temp.step1Vars)),na.rm=TRUE))}))
        } else {
          statusStep1 <- rbindlist(list(statusStep1,
                                        getStep1Imp(temp,
                                                    table=temp.table,
...
outName <- paste0("Table_",temp.table,"_Imputation_Step1_Summary_Perm_",temp.perm,"_ColNum_",temp.colnum,"_",temp.subsetSuffix,"_Rates_",temp.stratVar,"_",temp.stratLvl,"_National_SRS")
...
outName <- paste0("Table_",temp.table,"_Imputation_Step2_Summary_Perm_",temp.perm,"_ColNum_",temp.colnum,"_",temp.subsetSuffix,"_Rates_",temp.stratVar,"_",temp.stratLvl,"_National_SRS")
...
outName <- paste0("Table_",temp.table,"_Ratio_Adjustment_Summary_Perm_",temp.perm,"_ColNum_",temp.colnum,"_",temp.subsetSuffix,"_Rates_",temp.stratVar,"_",temp.stratLvl,"_National_SRS")
