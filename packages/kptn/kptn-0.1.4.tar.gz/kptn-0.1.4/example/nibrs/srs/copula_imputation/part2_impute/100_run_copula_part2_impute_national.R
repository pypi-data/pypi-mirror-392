...
temp.perm <- 1
temp.colnum <- 1
temp.stratLvl <- Sys.getenv("STRAT_VAR") %>% as.numeric() #list(1:8)
temp.demographics <- 0
temp.stratVar <- "PARENT_POP_GROUP_CODE2"

log_info("Current step: 1")
...
source("Copula_Imputation_Instance_Step1_National_SRS.R",local=TRUE,echo=TRUE)
source("Copula_Imputation_Instance_Step2_Alt_National_SRS.R",local=TRUE,echo=TRUE)
source("Copula_Imputation_Instance_Step3_Alt_National_SRS.R",local=TRUE,echo=TRUE)
source("Copula_Imputation_Instance_Step4_National_SRS.R",local=TRUE,echo=TRUE)

#Stack a table X permutation after all column sets and subsets
# run after all the subsets
