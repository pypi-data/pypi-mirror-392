---
title: '3-Implement SRS Rule Plus Block_LR'
author: "Philip Lee"
date: "April 7, 2023"
output:
  html_document:
...
#Read in the external current year NIBRS block imputation data
raw_nibrs_block_imputed <- fread(paste0(block_imp_path, "NIBRS_INCIDENT_PLUS_IMPUTED_BLOCK.csv.gz"))

#Read in the incident file
raw_main_inc0 <- readfiles(inpath=filepathin, inpatternfile="^raw_all_incidents_\\w+.csv.gz") %>%
  #Need to drop the agencies variables since we will be using the ones from the block imputation 
...
tbd_nm_victim_circum <- readfiles(inpath=filepathin, inpatternfile="^agg_neg_manslaughter_victim_circumstance_\\w+.csv.gz")

#Get if weapon involved is motor vehicle
#tbd_nm_victim_mv     <- fread(paste0(filepathin, "agg_nm_mv_victim_", CONST_STATE ,".csv.gz") )
tbd_nm_victim_mv <- readfiles(inpath=filepathin, inpatternfile="^agg_nm_mv_victim_\\w+.csv.gz")
...
#Drop the records
raw_main_inc01 <- raw_main_inc1 %>%
  filter(der_drop_record == 0)

#Check the dim
log_dim(raw_main_inc01)
log_dim(raw_main_inc1)

#Do a left join on the block imputed file
raw_main_inc <- raw_nibrs_block_imputed %>%
  left_join(raw_main_inc01, by="incident_id")


#Check the dimensions
log_dim(raw_main_inc)
log_dim(raw_nibrs_block_imputed)
log_dim(raw_main_inc01)

#Delete datasets no longer needed
rm(raw_nibrs_block_imputed, raw_main_inc0, raw_main_inc01)
...
#tbd_murder_counts <- fread(paste0(CONST_OUTPUT, "agg_victim_murder_person_counts_", CONST_STATE ,"_.csv.gz") )
tbd_murder_counts <- readfiles(inpath=filepathin, inpatternfile="^agg_victim_murder_person_counts_\\w+.csv.gz")
...
#tbd_nm_counts <- fread(paste0(CONST_OUTPUT, "agg_victim_nm_person_counts_", CONST_STATE ,"_.csv.gz") )
tbd_nm_counts <- readfiles(inpath=filepathin, inpatternfile="^agg_victim_nm_person_counts_\\w+.csv.gz")
...
# .	"11A" [Rape]
# -	Classify:
# .	Get the offenses of rape code:  11A. Subset to those with an associated victim with sex  F. 
# 1.	If none of the offense victims had a sex of F, this incident is skipped. 
# 2.	For the remaining offenses with a F victim, if any were completed, the counts for this incident will falls in the "rape by force" bucket, otherwise it's the "attempted rape by force." 
# -	Count: The number of unique victims tied to either 11A (looking at all the offenses in the incident). F victims are included in this count.



#Filter to incidents with rape
# tbd_1 <- remaining_2 %>%
#   filter(inc_der_rape == 1) %>%
#   #Fix need to deduplicate the incident_id due to multiple offenses
#   group_by(ori, incident_id) %>%
#   mutate(raw_first = row_number()) %>%
#   ungroup() %>%
...
#Read in the legacy rape count extracts
#tbd_rape_counts <- fread(paste0(CONST_OUTPUT, "agg_victim_rape_gender_completed_person_counts_", CONST_STATE ,"_.csv.gz") )
tbd_rape_counts <- readfiles(inpath=filepathin, inpatternfile="^agg_victim_legacy_rape_gender_completed_person_counts_\\w+.csv.gz")
...
#Edited for legacy rape to include female victims only
...
# tbd_robbery_weapons_counts <- fread(paste0(CONST_OUTPUT, "agg_robbery_weapons_", CONST_STATE ,"_.csv.gz") )

tbd_robbery_weapons_counts <- readfiles(inpath=filepathin, inpatternfile="^agg_robbery_weapons_\\w+.csv.gz")
...
# tbd_assault_weapons_counts <- fread(paste0(CONST_OUTPUT, "agg_agg_assault_weapon_victim_count_", CONST_STATE ,"_.csv.gz") )

tbd_assault_weapons_counts <- readfiles(inpath=filepathin, inpatternfile="^agg_agg_assault_weapon_victim_count_\\w+.csv.gz")
...
#Read in the assault count extracts
#tbd_assault_weapons_counts <- fread(paste0(CONST_OUTPUT, "agg_assault_weapon_victim_count_", CONST_STATE ,"_.csv.gz") )

tbd_assault_weapons_counts <- readfiles(inpath=filepathin, inpatternfile="^agg_assault_weapon_victim_count_\\w+.csv.gz")
...
#tbd_inc_burlglary_completed_force_num_entry <- fread(paste0(CONST_OUTPUT, "agg_inc_burlglary_completed_force_num_entry_", CONST_STATE ,"_.csv.gz") )
tbd_inc_burlglary_completed_force_num_entry <- readfiles(inpath=filepathin, inpatternfile="^agg_inc_burlglary_completed_force_num_entry_\\w+.csv.gz")
...
#tbd_mvt_variables <- fread(paste0(CONST_OUTPUT, "agg_mvt_variables_", CONST_STATE ,"_.csv.gz") )
tbd_mvt_variables <- readfiles(inpath=filepathin, inpatternfile="^agg_mvt_variables_\\w+.csv.gz")

glimpse(tbd_mvt_variables)

#Read in the agg_inc_mvt_att_comp_flags file
tbd_mvt_inc_variables <- readfiles(inpath=filepathin, inpatternfile="^agg_inc_mvt_att_comp_flags_\\w+.csv.gz")
...
merge_final <- reduce(mget(ls(pattern="final_")), bind_rows) %>%
  ...
  select(ori, incident_id, incident_month, incident_year, der_offense,counts)
...
#Need to aggregate at the ori, level
...
merge_final5 %>%
  write_csv(paste0(filepathout, "SRS_NIBRS_BLOCK_IMP_ONLY_LR.csv"))
