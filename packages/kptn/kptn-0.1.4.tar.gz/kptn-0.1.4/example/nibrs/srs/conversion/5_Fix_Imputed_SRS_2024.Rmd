...
raw_srs <- fread(paste0(filepathout, "SRS_Original_Combined.csv"))

#Read in the combined imputed SRS file
raw_srs_imputed <- fread(paste0(filepathout, "SRS_NIBRS_BLOCK_IMP_ONLY.csv"))

#Read in the SRS's Return A for the late-starters
raw_srs_returna <- file.path(input_files_path, paste0("UCR_SRS_", CONST_YEAR, "_clean_reta_mm_selected_vars.csv")) %>%
  fread() %>% 
  select(ORI_UNIV, ORI,
        #Get the mm flag
        ends_with("_mm_flag"),
...
final_list <- map_dfr(CONST_LIST_STATES, ~ {
  ...
  tbd_merge2 <- tbd_merge %>%
    ...
    mutate(
      ...
      max_value = fcase(
        #Handle situation for LAPD in 2024, use the SRS's Return A values
        ORI == "CA0194200", srs_returna,
...
#Output to share
final_list %>%
  write_csv(paste0(filepathout, "SRS_NIBRS_BLOCK_IMP_ONLY_MAX.csv"), na = "0")
