...
CONST_YEAR <- Sys.getenv("DATA_YEAR") %>% as.numeric()
...
########Read in the block imputed dataset###################################

if(CONST_YEAR <= 2014 ){
  log_debug("Reading in the Legacy Rape Version of the SRS")
  raw_main <- fread(paste0(CONST_INPUT, "SRS_Imputed_LR.csv"))
}else{
  log_debug("Reading in the Revised Rape Version of the SRS")
  raw_main <- fread(paste0(CONST_INPUT, "SRS_Imputed.csv"))
}
...
raw_main4 %>% 
    write_csv(gzfile(paste0(CONST_OUTPUT,"/cleaned_recoded_SRS.csv.gz")), na="") 
