...
load(paste0(in_file_path,"/SRS1araw_prep_env.RData"))
...
log_info("Running 03_TableSRS1araw_part3_finalize.R")
...
for(i in 1:maximum_column){
  ...
  #Get the data object
  raw_object <- readRDS(paste0(out_file_path,"/data_SRS1araw_",i,".rds"))
...
listofvars_rate <- final_data %>% select(starts_with("rate")) %>% colnames()
print(listofvars_rate)

final_data2 <- NA_to_0_count(data=final_data, list=listofvars_count)
final_data3 <- NA_to_0_percent(data=final_data2, list=listofvars_rate, keepNA=1)
...
for(i in 1:output_column){
  ...
  output_list_vars[[list_counter]] <- paste0("rate_", i)
  list_counter = list_counter + 1
...
fileout <- paste0(final_path, "Indicator_Table_SRS1araw.xlsx")
