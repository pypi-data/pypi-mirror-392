---
title: '103_TableSRS1araw_Variance'
author: "Philip Lee"
...
DER_TABLE_NAME_FOR_PRB_BLANK = "SRS1a"
#############################################################################

log_debug("Program Start")
#Read in the main datasets to be used for the table

#Per request need to use SRS1a with the block imputed data over the raw data
raw_main0 <- read_csv(file=gzfile(paste0(final_path, "Table ", DER_TABLE_NAME_FOR_PRB_BLANK, " ORI.csv.gz")))
log_debug("Read in ORI file")

#Will need to keep using the raw version of the database														
...
#Create a dataset to rename the t_ variables from SRS1a to SRS1araw
raw_rename_data <- colnames(raw_main0) %>%
  as_tibble() %>%
  mutate(
    der_keep = str_detect(string = value, pattern = DER_TABLE_NAME_FOR_PRB_BLANK)
  ) %>%
  #Filter when true
  filter(der_keep == TRUE) %>%
  #rename the variable
  rename(original_name  = value) %>%
  #Create the new name 
  mutate(
    new_name = str_replace_all(string = original_name, pattern = DER_TABLE_NAME_FOR_PRB_BLANK, replacement=DER_TABLE_NAME)
  )

#Loop thru and rename the dataset
raw_main <- raw_main0

for(i in 1:nrow(raw_rename_data)){
  
  #Get the current name
  tbd_original_name = raw_rename_data[i, "original_name"] %>% pull() %>% rlang:::parse_expr()
  
  #Get the new name
  tbd_new_name = raw_rename_data[i, "new_name"] %>% pull() %>% rlang:::parse_expr()
  
  #Rename the variables
  raw_main <- raw_main %>%
    rename(
      !!tbd_new_name := !!tbd_original_name
    )
  
  #Delete the tbd objects
  rm(tbd_original_name, tbd_new_name)
  invisible(gc())

}

#Delete objects not needed
rm(raw_main0, raw_rename_data)
invisible(gc())

#Need to create dataset raw_main for the remaining program																   

#Merge on the information from the weight dataset
main0 <- merge_on_weights_variance(raw_main)

log_dim(raw_main)
log_dim(main0)

#Need to create the main dataset with some tweaks
#main01 <- main0 %>%
  #Need to rename the weight variable
#  rename(tbd_weight = weight) %>%
#  mutate(
#    tbd_weight_gt0 = fcase(tbd_weight > 0, 1,
#                        is.na(tbd_weight), NA_real_,
#                        default = 0),
#    weight = fcase(
      #If the weight for the geographic permutation is greater than 0, then use the population proportion variable, propMult
#      tbd_weight_gt0 == 1, propMult
#    )    											   
#) 
...
#Need to rename the weight variable
 ...
 mutate(
   tbd_weight_gt0 = fcase(tbd_weight > 0, 1,
                       is.na(tbd_weight), NA_real_,
                       default = 0),
   weight = tbd_weight_gt0
   )				   

#Check the recodes
main01 %>% checkfunction(weight, tbd_weight, tbd_weight_gt0)
#main01 %>% checkfunction(weight, tbd_weight_gt0, propMult)


#Create the main data object
main <- main01 %>%
  select(-tbd_weight, -tbd_weight_gt0)
...
#Delete raw_main
rm(raw_main, main0, main01)


invisible(gc())

#Read in the Percent Relative Bias file - for the original table but need to NA since we don't need confidence intervals for the raw data
main_prb <- readRDS(paste0(input_copula_prb_folder, "/Relative_Bias_Estimates_", DER_TABLE_NAME_FOR_PRB_BLANK, "_", DER_CURRENT_PERMUTATION_NUM, ".rds")) %>%
  #Need to blank out the estimates
  mutate(
    percent_relative_bias = NA_real_,
    PRB_ACTUAL = NA_real_,
    der_all_counts = NA_real_,
    
    #Update the table variable for merging
...
final_main_reporting_db <- main_reporting_db3 %>%
  left_join(final_estimate2, by=c("table", "section", "row", "column", "estimate_type_num"))  %>%
     ...
     mutate(estimate = case_when(is.na(estimate) ~  0,
                                 TRUE ~ estimate),
     #For this table (i.e. raw data - make sure that the analysis_weight_name has a value of one to indicate unweighted counts)
            analysis_weight_name = "one")
