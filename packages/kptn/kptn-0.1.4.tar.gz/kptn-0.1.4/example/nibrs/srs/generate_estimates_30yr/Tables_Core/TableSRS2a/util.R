...
assign_row <- function(data){
  ...
  row = fcase(
    der_region %in% c(1:4), der_region
...
assign_labels <- function(data){
  ...
  estimate_domain = fcase(
    ...
    row == 1, 'NIBRS Indicator Percentage: Northeast',
    row == 2, 'NIBRS Indicator Percentage: Midwest',
    row == 3, 'NIBRS Indicator Percentage: South',
    row == 4, 'NIBRS Indicator Percentage: West'
  ...
  full_table = "TableSRS2a-SRS Regional Percentages",
  ...
  returndata <- indata %>%
    mutate(
      estimate_type_detail_percentage = fcase(
        ...
        trim_upcase(estimate_type) %in% c('PERCENTAGE') & row == 1, 'Offense Level', #NIBRS Indicator Percentage: Northeast
        trim_upcase(estimate_type) %in% c('PERCENTAGE') & row == 2, 'Offense Level', #NIBRS Indicator Percentage: Midwest
        trim_upcase(estimate_type) %in% c('PERCENTAGE') & row == 3, 'Offense Level', #NIBRS Indicator Percentage: South
        trim_upcase(estimate_type) %in% c('PERCENTAGE') & row == 4, 'Offense Level' #NIBRS Indicator Percentage: West
  ...
  returndata <- indata %>%
    mutate(
      estimate_type_detail_rate = fcase(
        ...
        trim_upcase(estimate_type) %in% c('RATE') & row == 1,  'Offense rate per 100,000 persons', #NIBRS Indicator Percentage: Northeast
        trim_upcase(estimate_type) %in% c('RATE') & row == 2,  'Offense rate per 100,000 persons', #NIBRS Indicator Percentage: Midwest
        trim_upcase(estimate_type) %in% c('RATE') & row == 3,  'Offense rate per 100,000 persons', #NIBRS Indicator Percentage: South
        trim_upcase(estimate_type) %in% c('RATE') & row == 4,  'Offense rate per 100,000 persons' #NIBRS Indicator Percentage: West
...
generate_est <- function(maindata, subsetvareq1, column_number){
  ...
  s1[[1]] <- maindata %>%
    ...
    ungroup() %>%
    mutate(
      percent = (final_count/sum(final_count, na.rm=TRUE)*100),
      section = 1)
  
  #For ORI need regional counts
  s1[[2]] <- maindata %>%
    group_by(ori, der_region) %>%
    summarise(final_count = sum(!!infiltervar, na.rm=TRUE)) %>%
    ungroup() %>%
    mutate(section = 1)  
