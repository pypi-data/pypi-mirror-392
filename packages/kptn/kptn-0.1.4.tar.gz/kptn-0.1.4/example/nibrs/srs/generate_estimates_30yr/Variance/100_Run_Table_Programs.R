...
#For this run use the following file 
  num_der_oris <- fread(paste0(outputPipelineDir, "/srs/weighting/Data/POP_TOTALS_PERM_", CONST_YEAR, "_FINAL_NATIONAL_SRS.csv") ) %>% filter(PERMUTATION_NUMBER == DER_CURRENT_PERMUTATION_NUM) %>%
  select(NUMBER_OF_ELIGIBLE_ORIS) %>%
  pull()


source("../POP_Total_code_assignment.R")
subset_variance <- fread(paste0(der_file_path, DER_ORI_VARIANCE_FILE)) %>% filter(!!DER_PERMUTATION_SUBSET_SYMBOL)
