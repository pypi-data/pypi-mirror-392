...
log_info("Running 03_TableSRS1araw_part1_prepare_datasets.R")
...
main <- fread(
    file = paste0(der_file_path, "raw_recoded_SRS.csv.gz"),
    select = c(
        "ori",
        "weight",
        collist
...
DER_TABLE_NAME = "SRS1araw"
DER_MAXIMUM_ROW = 1
#############################################################################


log_debug(system("free -mh", intern = FALSE))
save.image(file=paste0(out_file_path,"/SRS1araw_prep_env.RData"))
