...
pop_data_NATIONAL <- read_csv(paste0(inputPipelineDir,"/srs/weighting/Data/", "POP_TOTALS_PERM_", CONST_YEAR, "_FINAL_NATIONAL_SRS.csv")) %>% filter(PERMUTATION_NUMBER == 1)
...
pop_data <- read_csv(paste0(inputPipelineDir,"/srs/weighting/Data/", "POP_TOTALS_PERM_", CONST_YEAR, "_FINAL_NATIONAL_SRS.csv")) %>% filter(PERMUTATION_NUMBER == DER_CURRENT_PERMUTATION_NUM)
...
#New in the POP_TOTAL_PERM_YEAR_SRS_FINAL.csv file, there is a variable called DER_ORI_CALIBRATION_MODEL_PROCESS that has 
...
agg_percent_by_incident_id <- function(leftdata, rightdata, var, sectionnum){
...
agg_percent_by_incident_id_CAA <- function(leftdata, rightdata, var, sectionnum, denom){
...
agg_percent_victim <- function(leftdata, rightdata, var, sectionnum, mergeby){
...
agg_percent_CAA_victim <- function(leftdata, rightdata, var, sectionnum, mergeby, denom){
...
agg_percent_CAA_victim_rate <- function(leftdata, rightdata, var, sectionnum, mergeby, denom){
...
agg_percent_arrestee <- function(leftdata, rightdata, var, sectionnum, mergeby){
...
agg_percent_CAA_arrestee <- function(leftdata, rightdata, var, sectionnum, mergeby, denom){
...
agg_percent_CAA_arrestee_rate <- function(leftdata, rightdata, var, sectionnum, mergeby, denom){
...
agg_percent <- function(leftdata, rightdata, var, sectionnum, mergeby){
...
agg_percent_CAA <- function(leftdata, rightdata, var, sectionnum, mergeby, denom){
...
agg_percent_add_weight <- function(leftdata, rightdata, var, sectionnum, mergeby, addweight){
...
agg_percent_CAA_add_weight <- function(leftdata, rightdata, var, sectionnum, mergeby, denom, addweight){
...
clean_main <- function(returndata){
  ...
  log_debug(system("free -mh", intern = FALSE))
  #Need to handle block imputation first
  raw_block_imputation <- read_csv(gzfile(paste0(block_imp_path, "NIBRS_INCIDENT_PLUS_IMPUTED_BLOCK.csv.gz")))
  ...
  #Read in the current weight dataset
  raw_weight <- read_csv(gzfile(paste0(der_file_path, DER_ORI_WEIGHT_FILE))) %>%
    #Get selected variables needed - Note ori is the NIBRS CDE variable
    select(ori, !!DER_WEIGHT_VARIABLE_SYMBOL, PARENT_POP_GROUP_CODE,	AGENCY_TYPE_NAME,	REGION_CODE,	STATE_ABBR,	der_national)
...
merge_on_weights_variance <- function(indata){
  ...
  raw_variance <- read_csv(gzfile(paste0(der_file_path, DER_ORI_VARIANCE_FILE)))
  ...
  raw_variance <- raw_variance %>%
    ...
    #select(ori, county, !!DER_WEIGHT_VARIABLE_SYMBOL, !!DER_ORI_WEIGHT_GROUP_SYMBOL, !!DER_ORI_WEIGHT_DESCRIPT_GROUP_SYMBOL, propMult, matches("^V\\d+_\\w+")) %>%
...
#        {raw_method_2 <- ext.calibrated(data=raw_data_frame_subset, ids=~ori_county, strata=DER_ORI_WEIGHT_GROUP_FORMULA, weights=~simpwgt, fpc=~newfpc, weights.cal= ~weight, calmodel= DER_ORI_CALIBRATION_MODEL_FORMULA)  
        {raw_method_2 <- ext.calibrated(data=raw_data_frame_subset, ids=~ori, strata=DER_ORI_WEIGHT_GROUP_FORMULA, weights=~simpwgt, fpc=~newfpc, weights.cal= ~weight, calmodel= DER_ORI_CALIBRATION_MODEL_FORMULA)          
...
#          raw_method_2 <- e.svydesign(   data=raw_data_frame_subset, ids=~ori_county, strata=DER_ORI_WEIGHT_GROUP_FORMULA, weights=~weight, fpc=~newfpc)
          raw_method_2 <- e.svydesign(   data=raw_data_frame_subset, ids=~ori, strata=DER_ORI_WEIGHT_GROUP_FORMULA, weights=~weight, fpc=~newfpc)          
          #Return the object
          return(raw_method_2)
...
CREATE_PERCENTAGE_DENOMINATOR <- function(indata, inrow, incolumn){
...
PERCENTAGE_SE_FUNCTION <- function(indata, invar_denom, intotalse, inprb){
  ...
  for(i in 1:nrow(invar_denom)){
    ...
    innewvar        <- invar_denom[i,"variable"] %>% pull() %>% paste0(.,"_den") %>% rlang:::parse_expr()
    #Get the variables of the denominator
    indemovarstring <- invar_denom[i,"raw_denominator"] %>% pull() %>% strsplit(split=", ") %>% unlist()
    
    #Save the results to the list.  
    raw_summarise_list[[i]] <- indata %>%
      #Need to keep the ori != 0
      #filter(trimws(ori, which="both") != "0") %>%
      #Need to keep the ori that is not missing
      filter( !is.na(trimws(ori, which="both"))) %>%
      
      mutate(!!innewvar := rowSums(.[,indemovarstring], na.rm=TRUE)) %>%
#      select(ori, county, !!innewvar)
       select(ori, !!innewvar)
        
    #Clear the object
    rm(innewvar, indemovarstring)
    invisible(gc())
...
#    raw_method_unweighted <- e.svydesign(   data=raw_data_frame_subset, ids=~ori_county, strata=DER_ORI_WEIGHT_GROUP_FORMULA, weights=~der_ori_prop, fpc=~newfpc)
    raw_method_unweighted <- e.svydesign(   data=raw_data_frame_subset, ids=~ori, strata=DER_ORI_WEIGHT_GROUP_FORMULA, weights=~der_ori_prop, fpc=~newfpc)
  ...
  for(i in 1:nrow(invar_denom)){
    ...
    #Save the results to the list.  
    raw_summarise_prb_list[[i]] <- raw_prb_trans %>%
      mutate(raw_ALL_bias_num = !!invar,
             raw_ALL_bias_den = rowSums(.[,indemovarstring], na.rm=TRUE),
             variable = invarstring
      ) %>%
      select(variable, raw_ALL_bias_num, raw_ALL_bias_den)
    
    
    #Clear the object
    rm(invar, indemovarstring, invarstring)
    invisible(gc())
...
TOTAL_COUNT_FUNCTION <- function(indata, invar){
  ...
  #Want to sum up the unweighted counts for comparison to raw indicator tables
  invarsymbol <- invar %>% rlang:::parse_expr()
  
  returndata <- indata %>%
    #filter to those who have weights
    filter(weight > 0) %>%
    #Need to add code to handle the pseudo-ori level
    group_by(ori) %>%
    mutate(der_ori_county_count = n() ) %>%
    ...
    mutate(raw_agency_counts = case_when(!!invarsymbol > 0 ~ 1,
                                         TRUE ~ 0
    )) %>%
    summarise(unweighted_counts = sum(der_ori_prop * !!invarsymbol, na.rm = TRUE),
              agency_counts = sum(raw_agency_counts, na.rm = TRUE)
    ) %>%
    mutate(variable = str_match(string=invar, pattern=DER_TABLE_PATTERN_STRING)[,1],
           table    = str_match(string=invar, pattern=DER_TABLE_PATTERN_STRING)[,2],
           section  = str_match(string=invar, pattern=DER_TABLE_PATTERN_STRING)[,3] %>% as.numeric(),
           row      = str_match(string=invar, pattern=DER_TABLE_PATTERN_STRING)[,4] %>% as.numeric(),
           column   = str_match(string=invar, pattern=DER_TABLE_PATTERN_STRING)[,5] %>% as.numeric())
...
combinedoris <- function(incombined, inadditional, incombinednum, inadditionalnum){
  ...
  return(raw_data_combined_ori %>% select(-ends_with(inadditionalnum %>% as.character() )))
}
