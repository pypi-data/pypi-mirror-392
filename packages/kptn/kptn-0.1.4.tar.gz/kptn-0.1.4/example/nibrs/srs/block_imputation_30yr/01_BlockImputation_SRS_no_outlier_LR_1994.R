...
#####################For 1994###################################################
if(year == "1994"){
  log_debug("Running 1994 code edits")
  
  srs4_2 <- srs4 %>% 
    #Need to collapse Montana (i.e. STATE_ID_UNIV = 30) with Wyoming (i.e. STATE_ID_UNIV = 56)
    #Change STATE_ID_UNIV to numeric
    mutate(
      STATE_ID_UNIV = as.double(STATE_ID_UNIV)
    ) %>%
    mutate(
      STATE_ID_UNIV = fcase(
        #Collapse Montana with Wyoming
        STATE_ID_UNIV == 30, 56,
        #Collapse Minnesota with Iowa
        STATE_ID_UNIV == 27, 16,
        #Otherwise leave as is
        !is.na(STATE_ID_UNIV), as.double(STATE_ID_UNIV)
      )
    ) %>%
    mutate_at(c("suburb_flg", "rpt_type", "agn_type", "agn_status", "STATE_ID_UNIV", "REGION_CODE_UNIV", "POP0", "coveringAgency"), as.factor)

  #Check recodes
  srs4_2 %>% checkfunction(STATE_ID_UNIV)
