...
checkfunction(don_m3,agn_type)
checkfunction(agnCONST_YEAR_2_DIGIT_2,agn_type)

# determine if both full dataset and donor file have same levels of agn_type
compare_agn_type <- agnCONST_YEAR_2_DIGIT_2 %>%
                distinct(agn_type) %>%
                anti_join(distinct(don_m3,agn_type), by = "agn_type")

# collapse agn_type categories if full dataset and donor file do not have same levels
# currently code accounts for missing "Tribal" and missing "OTher" instances
# others may need to be added
if(nrow(compare_agn_type) == 1){
  agnCONST_YEAR_2_DIGIT_2 <- agnCONST_YEAR_2_DIGIT_2 %>%
    mutate(agn_type = case_when(compare_agn_type$agn_type==7 & agn_type %in% c("7","4","5") ~ "4",
                                compare_agn_type$agn_type==4 & agn_type %in% c("4","5") ~ "4",
                                .default = agn_type))
...
origid   <- agnCONST_YEAR_2_DIGIT_2 %>% 
  select(ori, NR_flg, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11,c12,low_p1:low_p12)

#Make missings monthly counts to -1
origid <- origid %>% 
  mutate_at(.vars=c("c1","c2","c3","c4","c5","c6","c7","c8","c9","c10","c11","c12"), ~replace(., is.na(.), -1))


#New edit:  Need to make sure that the donors incident-offense counts are higher than the recipent's low outlier counts
donid_m3 <- dat_m3_final %>% 
  select(ori, ori.don, Lowoutlier, 
         #Get the donor variables
         ends_with(".i")
         ) %>%
  #Need to create the imputed totals c#.i variables
  mutate( c1.i  = rowSums(select(., matches("^jan\\w+\\.i$"))),
          c2.i  = rowSums(select(., matches("^feb\\w+\\.i$"))),
          c3.i  = rowSums(select(., matches("^mar\\w+\\.i$"))),
          c4.i  = rowSums(select(., matches("^apr\\w+\\.i$"))),
          c5.i  = rowSums(select(., matches("^may\\w+\\.i$"))),
          c6.i  = rowSums(select(., matches("^jun\\w+\\.i$"))),
          c7.i  = rowSums(select(., matches("^jul\\w+\\.i$"))),
          c8.i  = rowSums(select(., matches("^aug\\w+\\.i$"))),                                           
          c9.i  = rowSums(select(., matches("^sep\\w+\\.i$"))),
          c10.i = rowSums(select(., matches("^oct\\w+\\.i$"))),
          c11.i = rowSums(select(., matches("^nov\\w+\\.i$"))),                                                       
          c12.i = rowSums(select(., matches("^dec\\w+\\.i$"))))
  
#Check the recodes
donid_m3 %>% checkfunction(c1.i, !!!(donid_m3[1,] %>% select(matches("^jan\\w+\\.i$")) %>% colnames() %>% rlang:::parse_exprs()))
donid_m3 %>% checkfunction(c2.i, !!!(donid_m3[1,] %>% select(matches("^feb\\w+\\.i$")) %>% colnames() %>% rlang:::parse_exprs()))
donid_m3 %>% checkfunction(c3.i, !!!(donid_m3[1,] %>% select(matches("^mar\\w+\\.i$")) %>% colnames() %>% rlang:::parse_exprs()))
donid_m3 %>% checkfunction(c4.i, !!!(donid_m3[1,] %>% select(matches("^apr\\w+\\.i$")) %>% colnames() %>% rlang:::parse_exprs()))
donid_m3 %>% checkfunction(c5.i, !!!(donid_m3[1,] %>% select(matches("^may\\w+\\.i$")) %>% colnames() %>% rlang:::parse_exprs()))
donid_m3 %>% checkfunction(c6.i, !!!(donid_m3[1,] %>% select(matches("^jun\\w+\\.i$")) %>% colnames() %>% rlang:::parse_exprs()))
donid_m3 %>% checkfunction(c7.i, !!!(donid_m3[1,] %>% select(matches("^jul\\w+\\.i$")) %>% colnames() %>% rlang:::parse_exprs()))
donid_m3 %>% checkfunction(c8.i, !!!(donid_m3[1,] %>% select(matches("^aug\\w+\\.i$")) %>% colnames() %>% rlang:::parse_exprs()))
donid_m3 %>% checkfunction(c9.i, !!!(donid_m3[1,] %>% select(matches("^sep\\w+\\.i$")) %>% colnames() %>% rlang:::parse_exprs()))
donid_m3 %>% checkfunction(c10.i, !!!(donid_m3[1,] %>% select(matches("^oct\\w+\\.i$")) %>% colnames() %>% rlang:::parse_exprs()))
donid_m3 %>% checkfunction(c11.i, !!!(donid_m3[1,] %>% select(matches("^nov\\w+\\.i$")) %>% colnames() %>% rlang:::parse_exprs()))
donid_m3 %>% checkfunction(c12.i, !!!(donid_m3[1,] %>% select(matches("^dec\\w+\\.i$")) %>% colnames() %>% rlang:::parse_exprs()))



#Need to join the data
donorids <- origid %>% 
  left_join(donid_m3, by=c("ori")) 
...
#Using donorids, need to create the flags
...
donorid2 <- donorids %>% 
  mutate(
    ...
    sep=fcase(c9==-1,1,
              low_p9==1 & (c9 < c9.i),2),
    oct=fcase(c10==-1,1,
              low_p10==1 & (c10 < c10.i),2),
    nov=fcase(c11==-1,1,
              low_p11==1 & (c11 < c11.i),2),
    dec=fcase(c12==-1,1,
              low_p12==1 & (c12 < c12.i),2))


#Check the recodes
donorid2 %>% checkfunction(jan, low_p1, c1, c1.i)
donorid2 %>% checkfunction(feb, low_p2, c2, c2.i)
donorid2 %>% checkfunction(mar, low_p3, c3, c3.i)
donorid2 %>% checkfunction(apr, low_p4, c4, c4.i)
donorid2 %>% checkfunction(may, low_p5, c5, c5.i)
donorid2 %>% checkfunction(jun, low_p6, c6, c6.i)
donorid2 %>% checkfunction(jul, low_p7, c7, c7.i)
donorid2 %>% checkfunction(aug, low_p8, c8, c8.i)
donorid2 %>% checkfunction(sep, low_p9, c9, c9.i)
donorid2 %>% checkfunction(oct, low_p10, c10, c10.i)
donorid2 %>% checkfunction(nov, low_p11, c11, c11.i)
donorid2 %>% checkfunction(dec, low_p12, c12, c12.i)

#Next need to filter to records with non-response flag = 1 or Lowoutier
donorids3 <- donorid2 %>% 
  filter(NR_flg==1 | Lowoutlier>0)

#Check the dimension
log_dim(donorids3)
log_dim(donorid2)

#Need to keep selected records
donorids4 <- donorids3 %>% 
  select(ori, ori.don, jan, feb, mar, apr, may, jun, jul, aug, sep, oct, nov, dec)

#Note it is possible with the latest edits that some month variables will be all missing
donorids5 <- donorids4 %>%
  mutate(
    der_keep = fcase(
      is.na(jan) & is.na(feb) & is.na(mar) & 
      is.na(apr) & is.na(may) & is.na(jun) & 
      is.na(jul) & is.na(aug) & is.na(sep) & 
      is.na(oct) & is.na(nov) & is.na(dec), 0,
      default = 1
    )
  )

#Check recodes
donorids5 %>% checkfunction(der_keep, jan, feb, mar, apr, may, jun, jul, aug, sep, oct, nov, dec)
...
#Output the dataset
donorids6 %>% 
  write_csv(paste0(outblockimp, "donorids.csv"), na="")
