  ...
  if(nrow(raw_percentage_3) > 0) {
    ...
    raw_percentage_3_dt[,
      raw_denominator := fcase(
        #Total arrests
        row == 2, CREATE_PERCENTAGE_DENOMINATOR(c(2:3), column), #Arrestee sex: Male
        row == 3, CREATE_PERCENTAGE_DENOMINATOR(c(2:3), column) #Arrestee sex: Female
      ),
