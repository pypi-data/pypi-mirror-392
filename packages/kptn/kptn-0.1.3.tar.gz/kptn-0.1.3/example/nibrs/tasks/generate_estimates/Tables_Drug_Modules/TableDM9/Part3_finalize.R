...
log_info("Running 25_TableDM9_part3_finalize.R")
...
for(i in 1:maximum_column){
  ...
  #Get the data object
  raw_object <- readRDS(paste0(in_file_path,"/data_",DER_TABLE_NAME,"_",i,".rds"))
...
final_data3 <- NA_to_0_percent(data=final_data2, list=listofvars_percentage, keepNA=c(1,2))
...
start_row = c(
  3, 5, 8, 17, 20, 27, 49, 54, 58, 61, 63, 70, 74)


# Specify the filename of existing workbook(Do not have file open)
file <- paste0("../TableShells/", "Additional_Drug_Indicator_Table_Shells.xlsx")
table_sheet =  "TableDM9-Drug Count By Arrestee"
...
fileout <- paste0(final_path, "Additional_Drug_Indicator_Table_",DER_TABLE_NAME,".xlsx")
