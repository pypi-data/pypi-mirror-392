...
main <- fread(file=paste0(der_file_path, main_file))

log_debug("About to kick off clean_main_part_1")
log_debug(system("free -mh", intern = FALSE))

#Filter to eligible agencies
if(main_file == "recoded_all_recoded_arrestee_groupb_arrest_code.csv.gz"){
  #Run a special version for the group b arrestee
  TEMP_main <- main %>% clean_main_part_1_group_b_arrestee()
}else{
  TEMP_main <- main %>% clean_main_part_1()
}

log_debug(paste0("Writing TEMP_cleaned_",main_file))
log_debug(system("free -mh", intern = FALSE))

fwrite_wrapper(TEMP_main, paste0(der_file_path, "TEMP_cleaned_", main_file), na = "")
