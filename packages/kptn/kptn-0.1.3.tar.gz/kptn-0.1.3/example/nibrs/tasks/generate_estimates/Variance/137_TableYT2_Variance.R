  ...
  if(nrow(raw_percentage_3) > 0) {
    ...
    raw_percentage_3_dt[,
      raw_denominator := fcase(
        #Number of Victimizations performed by offenders aged 12 or older, or missing age, or from unknown offender incidents
        row == 2, CREATE_PERCENTAGE_DENOMINATOR(c(1), column), #Victimization performed by any offenders in an incident by age: 12-17
        row == 3, CREATE_PERCENTAGE_DENOMINATOR(c(1), column), #Victimization performed by any offenders in an incident by age: 18 or older
        row == 4, CREATE_PERCENTAGE_DENOMINATOR(c(1), column), #Victimization performed by any offenders in an incident by age: Known offenders and missing age
        row == 5, CREATE_PERCENTAGE_DENOMINATOR(c(1), column) #Victimization performed by any offenders in an incident by age: Unknown offender incidents
      ),
