...
statusStep1 <- file.path(output_copula_data_folder,
                         paste0("Table_",temp.table,"_Imputation_Step1_Summary_Perm_",temp.perm,"_ColNum_",temp.colnum,"_",temp.subsetSuffix,"_Rates_",temp.stratVar,"_",temp.stratLvl,".csv")) %>%
  fread()
...
if (nrow(statusStep1)>0 & statusStep1 %>% subset(failInd==TRUE) %>% nrow() == 0){
  ...
  statusStep2 <- sapply(1:length(columns),function(temp.colnum2){#14
    ...
    #06Jun2025: drop unnecessary t_ variables for this column
    dropVars <- colnames(temp2) %>%
      str_subset("^t_") %>%
      str_subset(str_c("_",columns[temp.colnum2],"$"),negate=TRUE)
    temp2b <- data.frame(temp2) %>% 
      select(-all_of(dropVars))
    tempEnv2 <- environment()
    temp.statusStep2 <- sapply(1:2,function(temp.tier){
      if (temp.tier==1){
        temp.status <- getStep2Imp(dat=temp2b,
                                   tier=1,
        ...
        temp.status <- getStep2Imp(temp2b,
                                   tier=2,
      ...
      dropCols <- intersect(colnames(temp2),colnames(temp2b)) %>%
        str_subset("^(ORI|county)$",negate=TRUE)
      temp2 <- data.frame(temp2) %>%
        select(-all_of(dropCols)) %>%
        full_join(temp2b,by=c("ORI","county"))
      list2env(list("temp2"=temp2),tempEnv)
      return(temp.status)
    },simplify=FALSE) %>% 
