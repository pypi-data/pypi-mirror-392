#########################################################################
#Program      : 05_Populate_TableA_State
#Description  : Generate Table A within Appendix for state-level
#
...
#Notes:
#########################################################################
#Update (26AUG2021): Replaced all instances of 'stratum_f' with 'wgtGpState'
...
log_info("Running 05_Populate_TableA_State.R")
#####################################################################
# Table A
...
n_Univ_NIBRS_orig<-read_csv_logging(paste0(output_weighting_data_folder,'weights_state.csv'),
                            guess_max=1e6)

NumStates<-(dim(distinct(subset(n_Univ_NIBRS_orig,!is.na(StateWgt)) %>%  mutate(state_abbr=substr(ORI_universe,1,2)),state_abbr,.keep_all = TRUE))[1])
...
SF_tots<-read_csv_logging(paste0(output_weighting_data_folder,"SF_postSP.csv"),
                  col_types = cols_only("wgtGpState"=col_integer(),
...
SF_tots_state_<-SF_tots %>%
  group_by(wgtGpState,STATE_ABBR) %>%
  summarise(Nh=n())

SF_tots_state<-as.data.frame(SF_tots_state_) %>%
  mutate(wgtGpState_new = row_number())


SF_tots_all_fin<-inner_join(SF_tots_all,SF_tots,by=c("wgtGpState"))

SF_tots_all_state_fin_<-inner_join(SF_tots_state,SF_tots,by=c("wgtGpState","STATE_ABBR"))
SF_tots_all_state_fin_<-SF_tots_all_state_fin_ %>%
  rename(wgtGpState_old=wgtGpState)


SF_tots_all_state_fin<-SF_tots_all_state_fin_%>%rename(wgtGpState=wgtGpState_new)
...
#  inner_join(SF_tots[,c("ORI_universe","wgtGpState","Nh")],by=c("ORI_universe")) %>%
...
ucr_SRS <- ucr_SRS_orig %>%
  inner_join(SF_tots_all_state_fin[,c("ORI_universe","wgtGpState","Nh")],by=c("ORI_universe")) %>%
  left_join(SF_tots[,c("ORI_universe","PARENT_POP_GROUP_DESC2")],by=c("ORI_universe")) %>%
  inner_join(select(n_Univ_NIBRS_orig,"ORI_universe",StateWgt),by=c("ORI_universe")) %>%
  mutate(one=1)
...
#####################################################################
weight_state_lists<-(colnames(n_Univ_NIBRS_orig) %>%str_subset("StateWgt") %>%as.list()) #List of weights, update as necessary


weight_state_lists_n<-list("StateWgt")



weight_state_lists_n_df<-data.frame(matrix(unlist(weight_state_lists_n), nrow=length(weight_state_lists_n), byrow=T))
...
#state_name_abb<-subset(state_codes,jurisdiction_type=="state"|jurisdiction_type=="district")
state_name_abb <- fread(file.path('Data',"state_codes.csv"))
state_name_abb$nid <- seq.int(nrow(state_name_abb))
...
dummyDatA<-
  read_dot_csv_logging(file.path(output_weighting_tableshell_folder,"TableAStateDummy.csv"))%>%
...
####################Table A states#################################
compute_stateTotals_TabA <- function(wtvar,indata,indicator){
  log_debug("Running function compute_stateTotals_TabA")
  respWgts <- indata %>%
    rename(wt=all_of(wtvar)) %>%
    subset(!is.na(totcrime_imp)) %>%
    subset(!is.na(wt)) %>% #Switch to indicator?
    svydesign(ids= ~1,data=.,strata=.$wgtGpState,weights=.$wt,fpc=.$Nh)

  df<-data.frame(genAppA_state(respWgts))

  state_tot<-gather(data=df,key,value,starts_with("tot"),-state_abbr)%>%mutate(RowValue = as.numeric(row_number()))%>% select(-contains("se."))
  state_setot<-gather(data=df,key,value,starts_with("se"),-state_abbr)%>%mutate(RowValue = as.numeric(row_number()))%>% select(-contains("tot"))


  state_all0<-merge(state_tot,state_setot,by=c("state_abbr","RowValue"))%>%rename(Total=value.x)%>%rename(SE=value.y)
  ...
  state_all2<-merge(state_all1,state_name_abb,by=c("state_abbr"))
  state_all<-left_join(dummyDatA,state_all2,by=c("state_name","state_abbr","CrimeOrder"))

  state_all <- state_all[with(state_all,order(state_name,state_abbr,CrimeOrder)),]

  return(list(data.frame(state_all)))
}

#Generate UCR overall
stateEstimates_TabA_SRS <-(
  compute_stateTotals_TabA(wtvar="one",indata=ucr_SRS,indicator="inNIBRS") %>%
    .[[1]])
...
stateEstimates_TabA_nibrsReps <- (
  sapply(weight_state_lists,compute_stateTotals_TabA,indata=ucr_SRS,indicator="inNIBRS"))


######
#Merge estimates for SRS + NIBRS reporters
list_of_tablesA_states <- (
  sapply(1:length(stateEstimates_TabA_nibrsReps),
         function(subTab){
           inner_join(x=stateEstimates_TabA_nibrsReps[[subTab]],
                      y=stateEstimates_TabA_SRS,
                      by=c("state_abbr","CrimeOrder"),
                      suffix=c(".Reps",".SRS")) %>%
...
names(list_of_tablesA_states) <- weight_state_lists

##For summary of Relative Bias Table###
summary_tableA_states<-list_of_tablesA_states %>% bind_rows(.id="weightVar") %>%
  ...
  reshape2::melt(id.vars=c("weightVar","RowValue2","state_abbr","CrimeOrder"),measure.vars=c("BR.Repsn")) %>%
  reshape2::dcast(formula=state_abbr+RowValue2+weightVar~paste0(variable,CrimeOrder)) %>%
  # {merge(state_name_abb[c("state_abbr","nid")],.,by=c("state_abbr"))} %>%
  right_join(state_name_abb %>% select(state_abbr, nid)) %>%
  arrange(nid,weightVar)

summary_tableA_statespt1<-summary_tableA_states
...
str1 <- file.path(output_weighting_populated_folder,'TableA_State_Populated')
...
filein <- file.path(output_weighting_tableshell_folder,"TableA_State_Shell.xlsx")
...
log_debug("Writing sheet Summary_State_TabA_Ex1")
...
writeData(wbTabAState, sheet = "Summary_State_TabA_Ex1", x = summary_tableAKeep, startCol = 3, startRow = 8, colNames = FALSE,keepNA=FALSE)
...
write_files_xlsx(listfile_name = list_of_tablesA_states,
                 ...
                 inRow=8,
                 typeTab=9999)
log_info("Finished 05_Populate_TableA_State.R\n\n")