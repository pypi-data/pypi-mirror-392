...
log_info("Running 03_Weights_Calibration_FO_SRS_AltCombs_Collapsed.R")
...
SF <- paste0(input_weighting_data_folder,"SF_county.csv") %>%
...
SF2 <- SF %>%
  ...
  mutate(wgtGpFO=case_when(POPULATION_GROUP_DESC=="Cities 1,000,000 or over" ~ 1,
...
srs_FO_control_totals <- SF2 %>%
  ...
  dplyr::summarize(across(all_of(crimeVarsWgt),.fns=list("sum"=sum),
                          .names="{fn}_{col}",
                          na.rm=TRUE),
                   .groups="drop")
...
SF2_skips <- SF2 %>%
  ...
  mutate(FOWgt=ifelse(resp_ind_m3==1,1,NA_real_))
...
SF2_wgts2_test <- sapply(foGps2,function(i){#Loop over weight groupings
  ...
  SF_temp_FO <- SF2 %>%
    subset(FIELD_OFFICE_NAME==i & resp_ind_m3==1)
  ...
  SF2_wgts_FO <- sapply(temp.wgtGps,function(j){#Loop over weight groupingstemp.#wgtGps,function(j){#Loop over weight groupings
    ...
    #Note (09Jan2023): Removing totcrime_imp requirement
    SF_temp <- SF_temp_FO %>%
      subset(wgtGpFO==j & resp_ind_m3==1)
    if (nrow(SF_temp)>0){
  ...
  #02May2024: getting errors when merging in certain FOs... modifying merge (only keep certain variables from SF2_wgts_FO)
  if (nrow(SF2_wgts_FO)>0){
    SF2_wgts_FO <- SF2_wgts_FO %>%
	  select(ORI,county,matches("^FOWgt_")) %>% #02May2024: added today
...
combs_table_FO_gps <- function(indat,crimeVarsWgt,crimeVarsWgtAll,crimeVarsWgtRest,inWgtGps,wgtVar,wgtGpVar,wgtGpDescVar,suffix="",nInWgtGps=length(inWgtGps),nVars=length(crimeVarsWgtRest)){
  ...
  temp.out <- temp.out[rep(seq_len(nrow(temp.out)), each = nInGps), ]  %>%
    bind_cols(inGps) %>%
    mutate(!!wgtGpDescVar:=factor(eval(as.symbol(wgtGpVar)),
                                  levels=wgtGpsFO2,labels=wgtGpFODescs2))
  print(temp.out)
  #Note (09Jan2023): Removing totcrime_imp requirement
  temp.converge <- indat %>%
    subset(resp_ind_m3==1) %>%
  ...
  out <- temp.out %>% 
    ...
    left_join(ratio %>% 
                select(FIELD_OFFICE_NAME,wgtGpFO,N,n,sum_totcrime_imp_all,sum_totcrime_imp_NIBRS)) %>%
    list()
  colnames(out) %>% print()
...
#   mutate(FOWgt_nVar0_comb1=ifelse(resp_ind_m3==1 & 
#                                     rowSums(!is.na(select(.,matches("FOWgt_nVar\\d"))),
#                                             na.rm=TRUE)==0,
...
log_debug("Writing excel workbook SRS_Variable_Combination_Convergence_by_FO_X_Weighting_Group_AltCombs_Collapsed_Automatic.xlsx")
workbook <- paste0(output_weighting_data_folder,
                   "SRS_Variable_Combination_Convergence_by_FO_X_Weighting_Group_AltCombs_Collapsed_Automatic.xlsx")
wb <- createWorkbook()
...
if (nrow(ctrlVars)==SF2 %>% select(FIELD_OFFICE_NAME,wgtGpFO) %>% subset(duplicated(.)==FALSE) %>% nrow()){
  ...
  SF2_wgts2 <- sapply(foGps2,function(i){#Loop over weight groupings - just gp 1 for test
    ...
    ctrlTtlsM_FO <- srs_FO_control_totals %>%
      subset(FIELD_OFFICE_NAME==i) %>%
      left_join(foCrosses2) %>% #20Jun2023: merging on cross number
      mutate(wgtGpFO2=foCrossNum) %>%
      arrange(wgtGpFO) %>%
    ...
    ctrlVars2_FO <- ctrlVarsM2_FO %>%
      data.frame() %>%
      #Note (10Jan2023): Removing totcrime_imp requirement
      mutate(ORI=SF_FO %>% arrange(ORI) %>% subset(FIELD_OFFICE_NAME==i & resp_ind_m3==1) %>% pull(ORI),
             county=SF_FO %>% arrange(ORI) %>% subset(FIELD_OFFICE_NAME==i & resp_ind_m3==1) %>% pull(county),
             wgtGpFO2=SF_FO %>% arrange(ORI) %>% subset(FIELD_OFFICE_NAME==i & resp_ind_m3==1) %>% pull(wgtGpFO2))
    ...
    SF_temp_wgts2 <- SF2_wgts2_test %>%
      subset(resp_ind_m3==1 & FIELD_OFFICE_NAME==i) %>%
    ...
    SF2_wgts_FO <- sapply(temp.wgtGps,function(j){#Loop over weight groupings
      print("##############")
      print(paste0("Field Office: ",i,". Weight group: ",j))
    ...
    bind_rows()%>%
	#02May2024: issues during merge... reducing number of variables to only ID variables plus new variables (gWgt, FOWgt, and calibration variables)
  ...
  new_weights[,c("ORI_universe","LEGACY_ORI","county","FIELD_OFFICE_NAME","wgtGpFO","wgtGpFODesc",
    ...
    fwrite_wrapper(paste0(output_weighting_data_folder,'weights_FO_cal_srs_altcombs_col.csv'))
  ...
  oldSF <- fread(paste0(input_weighting_data_folder,"SF_county.csv"))%>%
    #Adding just in case already on file...
  ...
  fwrite_wrapper(newSF,paste0(output_weighting_data_folder,
                           "SF_postFO_cal_srs_altcombs_col.csv"))
  
  #Note (JDB 06Jul2022): Export weight check results
  #write_csv(results_FO,
  fwrite_wrapper(results_FO,
         paste0(output_weighting_data_folder,
                     "weights_FO_cal_srs_altcombs_col_checks.csv"))
  #Track list of FOs
  fo_all <- SF %>%
  ...
  fwrite_wrapper(fo_all,
         paste0(output_weighting_data_folder,"fo_all.csv"))
  
} else {
  stop("No calibration model converged for 1+ (field office X weighting group) crossing")
}
log_info("Finished 03_Weights_Calibration_FO_SRS_AltCombs_Collapsed.R\n\n")
