#########################################################################
#Program      : 05_Populate_TableA_Tribal
#Description  : Generate Table A within Appendix for tribal LEAs
#
...
log_info("Running 05_Populate_TableA_Tribal.R")
#####################################################################
# Table A
...
n_Univ_NIBRS_orig<-read_csv_logging(paste0(output_weighting_data_folder,'weights_tribal.csv'),
                            guess_max=1e6)

NumStates<-(dim(distinct(subset(n_Univ_NIBRS_orig,!is.na(TribalWgt)) %>%  mutate(state_abbr=substr(ORI_universe,1,2)),state_abbr,.keep_all = TRUE))[1])
...
#Stratum Totals
#Update (05NOV2021): Adding guess_max argument
#Update (08FEB2022): Subset to tribal LEAs
...
ucr_SRS <- ucr_SRS_orig %>%
  ...
  inner_join(select(n_Univ_NIBRS_orig,"ORI_universe",TribalWgt),by=c("ORI_universe")) %>%
  mutate(one=1)
...
#####################################################################
weight_natl_lists_n<-(colnames(n_Univ_NIBRS_orig) %>%str_subset("TribalWgt") %>%as.list()) #List of weights, update as necessary
...
dummyDatA<-
  read_dot_csv_logging(file.path(output_weighting_tableshell_folder,"TableATribalDummy.csv"))%>%
         ...
         function(subTab){
           inner_join(x=natlEstimates_TabA_nibrsReps[[subTab]] %>%
             ...
             mutate(RSE.Reps=round(SE.Reps/Total.Reps*100,2),
                    #Update (08FEB2022): looks like issue with BR.Reps below being character... wait until after BRGT5 to convert to character
                    ...
                    BRGT5=ifelse(BR.Repsn!=9999,ifelse(abs(BR.Repsn)>5,"*",""),""),
                    BR.Reps=ifelse(BR.Repsn!=9999,BR.Repsn,"NC")
             ) %>%
             list()
         })
...
str1 <- file.path(output_weighting_populated_folder,'TableA_Tribal_Populated')

filein <- file.path(output_weighting_tableshell_folder,"TableA_Tribal_Shell.xlsx")


# Load the existing workbook
wbTabATribal <- loadWorkbook(file =filein )
...
writeData(wbTabATribal, sheet = "Summary_Tribal_TabA_Ex1", x = summary_tableAKeep, startCol = 3, startRow = 8, colNames = FALSE,keepNA=FALSE)

saveWorkbook(wbTabATribal, file=paste0(str1, format(Sys.time(),'_%Y%m%d'), '.xlsx'), overwrite = TRUE)
...
write_files_xlsx(listfile_name = list_of_tablesA_natl,
                 ...
                 inRow=8,
                 typeTab=9999)
log_info("Finished 05_Populate_TableA_Tribal.R\n\n")