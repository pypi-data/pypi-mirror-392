#########################################################################
#Program      : 05_Populate_TableA_National
#Description  : Generate Table A within Appendix for national-level
#
...
log_info("Running 05_Populate_TableA_National.R")
#####################################################################
# Table A
...
n_Univ_NIBRS_orig<-read_csv_logging(paste0(output_weighting_data_folder,'weights_national.csv'),
                            guess_max=1e6)
...
dummyDatA<-
  read_dot_csv_logging(file.path(output_weighting_tableshell_folder,"TableANationalDummy.csv"))%>%
...
str1 <- file.path(output_weighting_populated_folder,'TableA_National_Populated')

filein <- file.path(output_weighting_tableshell_folder,"TableA_National_Shell.xlsx")
...
writeData(wbTabAState, sheet = "Summary_National_TabA_Ex1", x = summary_tableAKeep, startCol = 3, startRow = 8, colNames = FALSE,keepNA=FALSE)
...
log_info("Finished 05_Populate_TableA_National.R\n\n")