#########################################################################
#Program      : 04_TableShell_Region
#Description  : Create table shell for Table A within Appendix for region-level
#
...
log_info("Running 04_TableShell_Region.R")
##Get Weight Descriptions
regWgt <-"RegionWgt" #(Main) regional-level weight variable
...
if (nrow(weight_descrip0)<1){
  ...
  if (!regWgt %in% weight_descrip0$Variable.Name){
    ...
    log_debug("Regional Weight not in weight_descrip0")
...
state_name_abb_fin_all<-subset(state_name_abb_fin_all,state_name!="NATIONAL LEVEL ESTIMATES"  & Variable.Name %in% regWgt)
...
write_dot_csv_logging(file=file.path(output_weighting_tableshell_folder,"TableARegionDummy.csv"),state_name_abb_fin_SummTabA1)




######################################
# Table A shell Region
######################################
...
fileoutTabA<-file.path(output_weighting_tableshell_folder,"TableA_Region_Shell.xlsx")
...
removeCellMerge(wbShell_TabA_Region,"Summary_Region_TabA_Ex1", cols = 3:15, rows = 6:6)
...
#Header
writeData(wbShell_TabA_Region,sheet="Summary_Region_TabA_Ex1", x=RelBiasCtype,withFilter = F,colNames = FALSE, startRow =6, startCol=3)
mergeCells(wbShell_TabA_Region, "Summary_Region_TabA_Ex1", cols = 3:15, rows = 6:6)

#Crime Type
writeData(wbShell_TabA_Region,sheet="Summary_Region_TabA_Ex1", x=hcolNames_TabA,withFilter = F,colNames = FALSE, startRow =7,startCol=3)

#State by Weight
writeData(wbShell_TabA_Region,sheet="Summary_Region_TabA_Ex1", x=state_name_abb_fin_all[,c("state_name_char", "Description") ],withFilter = F,colNames = FALSE, startRow =8,startCol=1)
...
setColWidths(wbShell_TabA_Region, sheet = "Summary_Region_TabA_Ex1", cols = 2:2,widths =50)
setColWidths(wbShell_TabA_Region, sheet = "Summary_Region_TabA_Ex1", cols = 3:16,widths =30)
...
for(i in 1:length(weight_lists)){
  ...
  #Header
  writeData(wbShell_TabA_Region,sheet=weight_lists[[R]], x=hcolNames_TabA1[,1],withFilter = F,colNames = FALSE, startRow =6, startCol=2, headerStyle=headerStyle )
  writeData(wbShell_TabA_Region,sheet=weight_lists[[R]], x=hcolNames_TabA1[,2],withFilter = F,colNames = FALSE, startRow =6, startCol=3, headerStyle=headerStyle)
  mergeCells(wbShell_TabA_Region, weight_lists[[R]], cols=3:5, rows=6:6)
  writeData(wbShell_TabA_Region,sheet=weight_lists[[R]], x=hcolNames_TabA1[,3],withFilter = F,colNames = FALSE, startRow =6,startCol=6, headerStyle=headerStyle)
  writeData(wbShell_TabA_Region,sheet=weight_lists[[R]], x=hcolNames_TabA1[,4],withFilter = F,colNames = FALSE, startRow =6,startCol=7, headerStyle=headerStyle)
  mergeCells(wbShell_TabA_Region, weight_lists[[R]], cols=7:8, rows=6:6)

  #Header2
  writeData(wbShell_TabA_Region,sheet=weight_lists[[R]], x=hcolNames_TabA2,withFilter = F,colNames = FALSE, startRow =7, startCol=2,headerStyle=headerStyle)


  #Data Labels
  #if (str_detect(weight_lists[[R]],regWgt)){
  #  keepRows <- 1:13
  #} else if (str_detect(weight_lists[[R]],regWgt)){
  #  keepRows <- 14:nrow(state_name_abb_fin_SummTabA)
  #}
  keepRows <- 1:nrow(state_name_abb_fin_SummTabA)

  writeData(wbShell_TabA_Region,sheet=weight_lists[[R]], x=state_name_abb_fin_SummTabA[keepRows,],withFilter = F,colNames = FALSE, startRow =8, headerStyle=headerStyle)

  addStyle(wbShell_TabA_Region,weight_lists[[R]],mystyleA2,rows =6:6 ,cols = 2:8,gridExpand = T) #Header
  addStyle(wbShell_TabA_Region,weight_lists[[R]],mystyleA2,rows =7:(7+length(keepRows)) ,cols = 1:8,gridExpand = T) #Header

  setColWidths(wbShell_TabA_Region, weight_lists[[R]], cols = 1:8,widths = c(40,30,20,20,20,30,10,10))


}
saveWorkbook(wbShell_TabA_Region,file =fileoutTabA ,overwrite = T)
log_info("Finished 04_TableShell_Region.R\n\n")