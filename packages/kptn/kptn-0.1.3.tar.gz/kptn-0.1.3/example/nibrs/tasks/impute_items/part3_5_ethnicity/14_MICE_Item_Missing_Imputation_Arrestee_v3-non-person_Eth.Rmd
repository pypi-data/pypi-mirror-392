...
raw_mice <- map_dfr(tbd_list_states, ~{
  ...
  #Read in the dataset
  raw_data <- read_csv(paste0(mainpath, "14_", .x, "_arrestee_mice_final.csv.gz")) %>%
...
raw_data <- map_dfr(tbd_list_states, ~{
  ...
  #Read in the dataset
  raw_data <- fread(paste0(mainpath, "13_", .x, "_arrestee_for_imputation.csv.gz")) %>%
...
log_debug("Imputation NOT needed")
  ...
  walk(tbd_list_states, ~{
    ...
    NIBRS_OUT %>%
      ...
      write_csv_logging(paste0(mainpath, "14_", .x, "_arrestee_imputed_final_hisp.csv.gz"), na="" )
      
    NIBRS_OUT %>%  
      #Filter by state
      filter(tbd_state == .x) %>%	
      select(-tbd_state) %>%
      write_csv_logging(paste0(mainpath, "14_", .x, "_arrestee_mice_final_hisp.csv.gz"), na="" )  
...
write_xlsx_logging(md2,paste0(miscPathout ,"14_mdpattern2_", input_state, "_hisp.xlsx"))
...
write_xlsx_logging(as.data.frame(predQuick1), paste0(miscPathout, "14_predQuick1_", input_state, "_hisp.xlsx"))
...
check0 <- NIBRS[1:100,]
write_xlsx_logging(check0,paste0(miscPathout, "14_check0_", input_state, "_hisp.xlsx"))

check1 <- NIBRS1[1:100,]
write_xlsx_logging(check1, paste0(miscPathout, "14_check1_", input_state, "_hisp.xlsx"))
