...
raw_mice <- map_dfr(tbd_list_states, ~{
  ...
  #Read in the dataset
  raw_data <- read_csv(paste0(mainpath, "07_", .x, "_victim_mice_final.csv.gz")) %>%
...
raw_data <- map_dfr(tbd_list_states, ~{
  ...
  #Read in the dataset
  raw_data <- fread(paste0(mainpath, "06_", .x, "_victim_for_imputation.csv.gz")) %>%
...
if(CONST_CHECK_1 == TRUE) {
  ...
  walk(tbd_list_states, ~{
    ...
    NIBRS_OUT %>%
        ...
        write_csv_logging(paste0(mainpath, "07_", .x, "_victim_imputed_final_hisp.csv.gz"), na="" )
    	
    NIBRS_OUT %>%   
	  #Filter by state
      filter(tbd_state == .x) %>%
      select(-tbd_state) %>%
      write_csv_logging(paste0(mainpath, "07_", .x, "_victim_mice_final_hisp.csv.gz"), na="" )
...
write_xlsx_logging(md2,paste0(miscPathout ,"07_mdpattern2_", input_state, "_hisp.xlsx"))
...
write_xlsx_logging(as.data.frame(predQuick1), paste0(miscPathout, "07_predQuick1_", input_state, "_hisp.xlsx"))
...
check0 <- NIBRS[1:100,]
write_xlsx_logging(check0,paste0(miscPathout, "07_check0_", input_state, "_hisp.xlsx"))

check1 <- NIBRS1[1:100,]
write_xlsx_logging(check1, paste0(miscPathout, "07_check1_", input_state, "_hisp.xlsx"))
