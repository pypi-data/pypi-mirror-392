...
raw_mice <- map_dfr(tbd_list_states, ~{
  ...
  #Read in the dataset
  raw_data <- read_csv(paste0(mainpath, "10_", .x, "_mice_arrestee_offender_match.csv.gz")) %>%
...
# CONST_CHECK_1 <- raw_numbercheck == raw_numberrows
# print(CONST_CHECK_1)

#Check if variables are non-missing

CONST_CHECK_2 <- (any(is.na(NIBRS$ethnicity_code_offender2)) == FALSE &
...
if(CONST_CHECK_2 == TRUE) {
  ...
  walk(tbd_list_states, ~{
    ...
    NIBRS_OUT %>%
        ...
        write_csv_logging(paste0(mainpath, "10_", .x, "_imputed_arrestee_offender_match_hisp.csv.gz"), na="" )
      
    NIBRS_OUT %>%
      #Filter by state
      filter(tbd_state == .x) %>%
      select(-tbd_state) %>%
        write_csv_logging(paste0(mainpath, "10_", .x, "_mice_arrestee_offender_match_hisp.csv.gz"), na="" )      
...
write_xlsx_logging(md2,paste0(miscPathout ,"10_mdpattern2_", input_state, "_hisp.xlsx"))
...
#Update the variables to impute
# meth2[c("ethnicity_code_offender2",
#         "ethnicity_code_arrestee2")] <- c(rep("pmm",2))

#Need to drop arrestee
meth2[c("ethnicity_code_offender2")] <- c(rep("pmm",1))
...
write_xlsx_logging(as.data.frame(predQuick1), paste0(miscPathout, "10_predQuick1_", input_state, "_hisp.xlsx"))
write_xlsx_logging(as.data.frame(meth1),      paste0(miscPathout ,"10_meth1", input_state, "_hisp.xlsx"))
write_xlsx_logging(as.data.frame(meth2),      paste0(miscPathout ,"10_meth2", input_state, "_hisp.xlsx"))
...
check0 <- NIBRS[1:100,]
write_xlsx_logging(check0,paste0(miscPathout, "10_check0_", input_state, "_hisp.xlsx"))

check1 <- NIBRS1[1:100,]
write_xlsx_logging(check1, paste0(miscPathout, "10_check1_", input_state, "_hisp.xlsx"))
