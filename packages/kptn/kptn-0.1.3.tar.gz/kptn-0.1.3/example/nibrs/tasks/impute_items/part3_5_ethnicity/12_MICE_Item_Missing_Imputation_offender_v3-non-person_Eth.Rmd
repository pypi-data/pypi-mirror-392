...
raw_mice <- map_dfr(tbd_list_states, ~{
  ...
  #Read in the dataset
  raw_data <- read_csv(paste0(mainpath, "12_", .x, "_offender_mice.csv.gz")) %>%
...
raw_data <- map_dfr(tbd_list_states, ~{
  ...
  #Read in the dataset
  raw_data <- fread(paste0(mainpath, "11_", .x, "_offender_for_imputation.csv.gz")) %>%
...
raw_mice2 <- raw_mice %>%
  ...
  select(-tbd_state) %>%
  left_join(raw_data, by=c("incident_id", "offender_seq_num"))
...
if(CONST_CHECK_1 == TRUE) {
  ...
  walk(tbd_list_states, ~{
    ...
    NIBRS_OUT %>%
        ...
        write_csv_logging(paste0(mainpath, "12_", .x, "_offender_imputed_hisp.csv.gz"), na="" )
    	
    NIBRS_OUT %>%    
      #Filter by state
      filter(tbd_state == .x) %>%	
      select(-tbd_state) %>%
      write_csv_logging(paste0(mainpath, "12_", .x, "_offender_mice_hisp.csv.gz"), na="" )    
...
write_xlsx_logging(md2,paste0(miscPathout ,"12_mdpattern2_", input_state, "_hisp.xlsx"))
...
write_xlsx_logging(as.data.frame(predQuick1), paste0(miscPathout, "12_predQuick1_", input_state, "_hisp.xlsx"))
...
check0 <- NIBRS[1:100,]
write_xlsx_logging(check0,paste0(miscPathout, "12_check0_", input_state, "_hisp.xlsx"))

check1 <- NIBRS1[1:100,]
write_xlsx_logging(check1, paste0(miscPathout, "12_check1_", input_state, "_hisp.xlsx"))

NIBRS1 <- as.data.frame(NIBRS1)

#Check to make sure it is non-missing
NIBRS1 %>% checkfunction(ethnicity_code_offender2)
