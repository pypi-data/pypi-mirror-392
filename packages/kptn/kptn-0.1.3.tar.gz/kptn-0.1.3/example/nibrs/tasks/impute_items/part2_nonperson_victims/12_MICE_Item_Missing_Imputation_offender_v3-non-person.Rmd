...
NIBRS <- read_csv_logging(paste0(inputmainpath, "11_", input_state, "_offender_for_imputation.csv.gz"))

log_debug("Checking initial dimensions of NIBRS")
log_dim(NIBRS)

#Drop ethnicity
NIBRS <- NIBRS %>%
  # Drop victim_type_id since there could be multiple type of victims
  select(-c(ethnicity_code_offender2, victim_type_code))
...
NIBRS$offender_seq_num	<- as.factor(NIBRS$offender_seq_num	)
...
#NIBRS$ethnicity_code_offender2	<- as.factor(NIBRS$ethnicity_code_offender2	)
...
NIBRS %>%
    ...
    write_csv_logging(paste0(mainpath, "12_", input_state, "_offender_imputed.csv.gz"), na="" )

NIBRS %>%    
    write_csv_logging(paste0(mainpath, "12_", input_state, "_offender_mice.csv.gz"), na="" )

}else{
log_debug("Imputation IS needed.")

write_xlsx_logging(md2,paste0(miscPathout ,"12_mdpattern2_", input_state, ".xlsx"))
...
predQuick1[,c("incident_id","offender_seq_num")] <- 0 #Will not use variables as predicator by making 0

write_xlsx_logging(as.data.frame(predQuick1), paste0(miscPathout, "12_predQuick1_", input_state, ".xlsx"))
...
check0 <- NIBRS[1:100,]
write_xlsx_logging(check0,paste0(miscPathout, "12_check0_", input_state, ".xlsx"))

check1 <- NIBRS1[1:100,]
write_xlsx_logging(check1, paste0(miscPathout, "12_check1_", input_state, ".xlsx"))

NIBRS1 <- as.data.frame(NIBRS1)

Sex_offd_imputed <- NIBRS1  %>% select(incident_id,offender_seq_num, sex_code_offender2,race_code_offender2,age_num_offender2) #Let try a tibble
glimpse(Sex_offd_imputed)

Sex_offd_imputed %>% write_csv_logging(paste0(mainpath, "12_", input_state, "_offender_imputed.csv.gz"), na="" )
