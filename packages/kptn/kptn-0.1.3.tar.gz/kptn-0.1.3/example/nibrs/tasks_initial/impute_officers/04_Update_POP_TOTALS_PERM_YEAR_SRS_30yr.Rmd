---
title: "04_Update_POP_TOTALS_PERM_YEAR_SRS"
author: "Philip Lee"
date: "March 22, 2023"
output:
  html_document:
...
#Delete the tbd data data
rm(list=ls(pattern="tbd_"))
invisible(gc())

#Next need to read in the RETA MM file
raw_reta_mm <- fread(paste0(CONST_OUTPUT, "UCR_SRS_", CONST_YEAR, "_clean_reta_mm_selected_vars.csv"))

#Capitalize all variables
colnames(raw_reta_mm) <- toupper(colnames(raw_reta_mm))

#Keep only the variables we want
raw_reta_mm2 <- raw_reta_mm %>%
  select(ORI, ends_with("_MM_FLAG"))

#Do the merge
tbd_good_1 <- raw_universe4 %>%
  inner_join(raw_reta_mm2, by = c("LEGACY_ORI" = "ORI") )

tbd_anti_1 <- raw_universe4 %>%
  anti_join(raw_reta_mm2, by = c("LEGACY_ORI" = "ORI") )
...
tbd_good_2 <- tbd_anti_1 %>%
  inner_join(raw_reta_mm2, by = c("ORI" = "ORI") )

tbd_anti_2 <- tbd_anti_1 %>%
  anti_join(raw_reta_mm2, by = c("ORI" = "ORI") )
...
#Final dataset is called main
raw_main_2 <- bind_rows(
  tbd_good_1 %>% mutate(der_in_mm = 1), 
  tbd_good_2 %>% mutate(der_in_mm = 1), 
  tbd_anti_2 %>% mutate(der_in_mm = 0))

#See the dimension
log_dim(raw_universe4)
log_dim(raw_main_2)

#Delete the tbd_ and reta_mm dataset
rm(list=c(ls(pattern="tbd_"), ls(pattern="reta_mm")))
invisible(gc())

#Get the MM variables
CONST_MM_VARS <- raw_main_2 %>%
  head(1) %>%
...
#Next create the monthly report variables
main <- raw_main_2 %>%
  mutate(
  ...
  rowwise() %>%
  mutate(
    DER_IND_MM_FLAGS = sum( !!!(paste0("DER_IND_", CONST_MM_VARS  ) %>% rlang:::parse_exprs()), na.rm = TRUE ),
    DER_SRS_REPORTER_FLAG = fcase(
      DER_IND_MM_FLAGS %in% c(0:2) , 0,
      DER_IND_MM_FLAGS %in% c(3:12) , 1
    )
  ) %>%
  ungroup()

#Check the recodes
main %>% checkfunction(DER_IND_JAN_MM_FLAG, JAN_MM_FLAG)
main %>% checkfunction(DER_IND_FEB_MM_FLAG, FEB_MM_FLAG)
main %>% checkfunction(DER_IND_MAR_MM_FLAG, MAR_MM_FLAG)
main %>% checkfunction(DER_IND_APR_MM_FLAG, APR_MM_FLAG)
main %>% checkfunction(DER_IND_MAY_MM_FLAG, MAY_MM_FLAG)
main %>% checkfunction(DER_IND_JUN_MM_FLAG, JUN_MM_FLAG)
main %>% checkfunction(DER_IND_JUL_MM_FLAG, JUL_MM_FLAG)
main %>% checkfunction(DER_IND_AUG_MM_FLAG, AUG_MM_FLAG)
main %>% checkfunction(DER_IND_SEP_MM_FLAG, SEP_MM_FLAG)
main %>% checkfunction(DER_IND_OCT_MM_FLAG, OCT_MM_FLAG)
main %>% checkfunction(DER_IND_NOV_MM_FLAG, NOV_MM_FLAG)
main %>% checkfunction(DER_IND_DEC_MM_FLAG, DEC_MM_FLAG)

main %>% checkfunction(DER_IND_MM_FLAGS, !!!(paste0("DER_IND_", CONST_MM_VARS  ) %>% rlang:::parse_exprs()) )
main %>% checkfunction(DER_IND_MM_FLAGS, DER_SRS_REPORTER_FLAG)
...
#Create the for_counts data
for_counts <- main

#Read in the POP_TOTALS_PERM_YEAR file to update the officer variables
raw_population <- read_csv(file.path(external_path,file_locs[[CONST_YEAR]]$population_srs))
...
final <- map2_dfr(.x=main_population$PERMUTATION_NUMBER,
  ...
  final_2 <- tbd_2 %>% 
    filter(DER_SRS_REPORTER_FLAG == 1) %>%
...
#Output to share
raw_population3 %>%
  write_csv(paste0(CONST_OUTPUT, "POP_TOTALS_PERM_", CONST_YEAR, "_SRS.csv"))
