---
title: '3-Combined Original SRS_Partial'
author: "Philip Lee"
date: "April 7, 2023"
output:
  html_document:
...
readfiles <- function(inpath, inpatternfile){
  ...
  returndata <- map_dfr(raw_list_files,  ~{
      ...
      raw_main <- fread(paste0(inpath, raw_current_file)) %>%
        #Declare the type for certain variables
        mutate(
...
raw_main <- readfiles(inpath=filepathin, inpatternfile=paste0("^Partial_SRS_", CONST_YEAR, "_\\w{2}\\.csv")) %>%
  mutate(
    year = CONST_YEAR
  )
...
raw_main2 %>%
  select(ori, legacy_ori,
         !!!(MACRO_V_VARS %>% rlang:::parse_exprs()),
         everything()
         ) %>%
  write_csv(paste0(filepathout, "Partial_SRS_Original_Combined_", CONST_YEAR ,".csv"))
