...
offense_recode <- function(data){
  ...
  #Add on the two human trafficking
  der_human_trafficking_sex = fcase(
    trim_upcase(offense_code) %in% c("64A"), 1, #64A	Human Trafficking, Commercial Sex Acts
    default = 0),

  der_human_trafficking_labor = fcase(
    trim_upcase(offense_code) %in% c("64B"), 1, #64B	Human Trafficking, Involuntary Servitude
    default = 0) 
...
# filter queries by state and year
state_year_where <- paste0("EXTRACT(YEAR FROM incident_date) = ", CONST_YEAR," AND agencies.state_abbr = '", CONST_STATE,"'")
...
query1 <- paste0("
 SELECT 
  ...
  lkup_nibrs_offense.name AS location_name,
...
df1_1 %>% checkfunction(der_human_trafficking_sex, crime_against, offense_code, offense_name)
df1_1 %>% checkfunction(der_human_trafficking_labor, crime_against, offense_code, offense_name)
...
df1_1 %>% checkfunction(keep_attempt_completed_offense, attempt_complete_flag, attempt_complete_code)
...
df1_2 %>% checkfunction(keep_attempt_completed_offense, attempt_complete_flag, attempt_complete_code)
...
#Next count the number of victims for human_trafficking_sex
df1_2_4 <- df1_2_1 %>%
  filter(trim_upcase(offense_code) %in% c("64A")) %>% #64A	Human Trafficking, Commercial Sex Acts
  #Make sure we have unique person
  count(incident_id, victim_id) %>%
  #Count number of victims at incident level
  group_by(incident_id) %>%
...
#Output the human_trafficking_sex output
df1_2_4 %>% 
  write_csv(gzfile(paste0(filepathout,"agg_victim_human_trafficking_sex_person_counts_", CONST_STATE ,".csv.gz")), na="")


#Next count the number of victims for human_trafficking_labor
df1_2_5 <- df1_2_1 %>%
  filter(trim_upcase(offense_code) %in% c("64B")) %>% #64B	Human Trafficking, Involuntary Servitude
  #Make sure we have unique person
  count(incident_id, victim_id) %>%
  #Count number of victims at incident level
  group_by(incident_id) %>%
...
#Output the human_trafficking_sex output
df1_2_5 %>% 
  write_csv(gzfile(paste0(filepathout,"agg_victim_human_trafficking_labor_person_counts_", CONST_STATE ,".csv.gz")), na="")
