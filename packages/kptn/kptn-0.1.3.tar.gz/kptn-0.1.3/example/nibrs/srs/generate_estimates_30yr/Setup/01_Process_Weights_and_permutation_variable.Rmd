...
univ_ori <-file.path(input_files_path, paste0("ref_agency_", CONST_YEAR, ".csv")) %>%
  fread() %>% select(ORI, LEGACY_ORI, REGION_CODE, STATE_ABBR, POPULATION_GROUP_DESC, POPULATION_GROUP_ID)
...
raw_main_cal <- fread(paste0(weight_path, "Agency_Level_Calibration_Variable_File_SRS.csv"))  # DCH 12.12.24 adjusted for new name of file
...
raw_main_cal2 <- raw_main_cal %>%
  ...
  select(
    ...
    # DCH 12.12.24 comment out calls to variables that are not available due to switch from pseudo-ORI to ORI
    # county,
...
# DCH 12.12.24 
# ignoring the smaller geographies above, simply set raw_weight to the incoming national weight file
raw_weight <- fread(paste0(weight_path, "weights_national_srs.csv"))  
...
#Single record do not need county and more than one record will use county as criteria
...
#                          )
final_weight <- raw_final_weight_original_only1_final
...
final2 <- final_weight %>%
 # inner_join(univ_ori, by=c("ORI_universe" = "LEGACY_ORI")) %>%
   inner_join(univ_ori, by=c("ORI_universe" = "ORI")) %>%
  create_der_national() %>%
