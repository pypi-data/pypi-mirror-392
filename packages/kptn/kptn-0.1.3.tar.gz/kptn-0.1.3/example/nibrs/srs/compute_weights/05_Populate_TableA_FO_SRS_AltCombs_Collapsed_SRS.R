#########################################################################
#Program      : 05_Populate_TableA_FO
#Description  : Generate Table A within Appendix for field-office-level
#
...
log_info("Running 05_Populate_TableA_FO_SRS_AltCombs_Collapsed_SRS.R")
...
n_Univ_NIBRS_orig<-read_csv(paste0(output_weighting_data_folder,'weights_FO_cal_srs_altcombs_col_srs.csv'),
                            guess_max=1e6)

NumStates<-n_Univ_NIBRS_orig %>% subset(!is.na(FOWgt)) %>% pull(FIELD_OFFICE_NAME) %>% unique() %>% length()

#SRS
#Update (27OCT2021): Use variables on SF instead of recreating based on cleanframe + srs2016_2020_smoothed
#Update (05NOV2021): Adding guess_max argument
SF <- read_csv(paste0(output_weighting_data_folder,"SF_postFO_cal_srs_altcombs_col_srs.csv"),
               guess_max=1e6) %>%
  subset(!is.na(FIELD_OFFICE_NAME))


ucr_SRS_orig <- SF %>%
  select(ORI_universe,county,FIELD_OFFICE_NAME,matches("totcrime.*_imp"))
...
#Stratum Totals
#Update (05NOV2021): Adding guess_max argument
SF_tots<-read_csv(paste0(output_weighting_data_folder,"SF_postFO_cal_srs_altcombs_col_srs.csv"),
                  col_types = cols_only("wgtGpFO"=col_integer(),
                                        "county"=col_character(),
                                        "FIELD_OFFICE_NAME"=col_character(),
                  ...
                  guess_max=1e6) %>%
  mutate(PARENT_POP_GROUP_DESC_UNIV2=case_when(PARENT_POP_GROUP_DESC_UNIV %in% c("Cities under 2,500","Cities from 2,500 thru 9,999") ~ "Cities under 10,000",
                                               TRUE ~ PARENT_POP_GROUP_DESC_UNIV))
...
SF_tots_state_<-SF_tots %>%
  group_by(wgtGpFO,FIELD_OFFICE_NAME) %>%
  summarise(Nh=n())

SF_tots_state<-as.data.frame(SF_tots_state_) %>%
  mutate(wgtGpFO_new = row_number())


SF_tots_all_fin<-inner_join(SF_tots_all,SF_tots,by=c("wgtGpFO"))

SF_tots_all_state_fin_<-inner_join(SF_tots_state,SF_tots,by=c("wgtGpFO","FIELD_OFFICE_NAME"))
SF_tots_all_state_fin_<-SF_tots_all_state_fin_ %>%
  rename(wgtGpFO_old=wgtGpFO)


SF_tots_all_state_fin<-SF_tots_all_state_fin_%>%rename(wgtGpFO=wgtGpFO_new)
...
#  inner_join(SF_tots[,c("ORI_universe","wgtGpFO","Nh")],by=c("ORI_universe")) %>%
...
ucr_SRS <- ucr_SRS_orig %>%
  inner_join(SF_tots_all_state_fin[,c("ORI_universe","county","FIELD_OFFICE_NAME","wgtGpFO","Nh")],
             by=c("ORI_universe","county","FIELD_OFFICE_NAME")) %>%
  left_join(SF_tots[,c("ORI_universe","county","FIELD_OFFICE_NAME","PARENT_POP_GROUP_DESC_UNIV2")],
            by=c("ORI_universe","county","FIELD_OFFICE_NAME")) %>%
  inner_join(select(n_Univ_NIBRS_orig,ORI_universe,county,FOWgt,FIELD_OFFICE_NAME),
             by=c("ORI_universe","county","FIELD_OFFICE_NAME")) %>%
  mutate(one=1)
...
#####################################################################
weight_state_lists<-(colnames(n_Univ_NIBRS_orig) %>%str_subset("FOWgt") %>%as.list()) #List of weights, update as necessary


weight_state_lists_n<-list("FOWgt")
...
foall <- SF %>% select(FIELD_OFFICE_NAME) %>% unique() %>% arrange(FIELD_OFFICE_NAME)
...
dummyDatA<-
  read.csv(file.path(output_weighting_tableshell_folder,"TableAFODummySRS.csv"))%>%
...
####################Table A states#################################
compute_foTotals_TabA <- function(wtvar,indata,indicator){
  respWgts <- indata %>%
    rename(wt=all_of(wtvar)) %>%
    subset(!is.na(totcrime_imp)) %>%
    subset(!is.na(wt)) %>% #Switch to indicator?
    svydesign(ids= ~1,data=.,strata=.$wgtGpFO,weights=.$wt,fpc=.$Nh)
  
  df<-data.frame(genAppA_fo(respWgts))
  
  state_tot<-gather(data=df,key,value,starts_with("tot"),-FIELD_OFFICE_NAME)%>%mutate(RowValue = as.numeric(row_number()))%>% select(-contains("se."))
  state_setot<-gather(data=df,key,value,starts_with("se"),-FIELD_OFFICE_NAME)%>%mutate(RowValue = as.numeric(row_number()))%>% select(-contains("tot"))
  
  
  state_all0<-merge(state_tot,state_setot,by=c("FIELD_OFFICE_NAME","RowValue"))%>%rename(Total=value.x)%>%rename(SE=value.y)%>% mutate(state_name=FIELD_OFFICE_NAME)
...
foEstimates_TabA_SRS <-(
  compute_foTotals_TabA(wtvar="one",indata=ucr_SRS,indicator="inNIBRS") %>%
    .[[1]])
...
foEstimates_TabA_nibrsReps <- (
  sapply(weight_state_lists,compute_foTotals_TabA,indata=ucr_SRS,indicator="inNIBRS"))


######
#Merge estimates for SRS + NIBRS reporters
list_of_tablesA_fos <- (
  sapply(1:length(foEstimates_TabA_nibrsReps),
         function(subTab){
           inner_join(x=foEstimates_TabA_nibrsReps[[subTab]],
                      y=foEstimates_TabA_SRS,
...
names(list_of_tablesA_fos) <- weight_state_lists

##For summary of Relative Bias Table###
summary_tableA_fos<-list_of_tablesA_fos %>% bind_rows(.id="weightVar") %>%
...
summary_tableA_fospt1<-summary_tableA_fos
...
str1 <- file.path(output_weighting_populated_folder,'TableA_FO_Populated_SRS_AltCombs_Collapsed_SRS')

filein <- file.path(output_weighting_tableshell_folder,"TableA_FO_Shell_SRS.xlsx")



# Load the existing workbook
wbTabAFO <- loadWorkbook(file =filein )
...
writeData(wbTabAFO, sheet = "Summary_FO_TabA_Ex1", x = summary_tableAKeep, startCol = 3, startRow = 8, colNames = FALSE,keepNA=FALSE)

saveWorkbook(wbTabAFO, file=paste0(str1, format(Sys.time(),'_%Y%m%d'), '.xlsx'), overwrite = TRUE)
...
#TypeTab; 9999 code if includes field-office-level table, 8888 code for overall table
...
write_files_xlsx(listfile_name = list_of_tablesA_fos,
...
log_info("Finished 05_Populate_TableA_FO_SRS_AltCombs_Collapsed_SRS.R")