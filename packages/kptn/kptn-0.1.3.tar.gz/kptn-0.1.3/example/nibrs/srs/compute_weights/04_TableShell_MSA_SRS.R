...
if (nrow(weight_descrip0_all)<1){
  print("MSA Weight not in weight_descrip0_all")
  weight_descrip0_all <- weight_descrip0_all %>%
    ...
    arrange(Weight.Number) %>%
    mutate(Weight.Number = row_number())
  print("Added dummy description to weight_descrip0_all")
}

#Get State Names and Abbreviations
rel <- data.frame(state_name=msaall$MSA_NAME_COUNTY) %>%
  mutate(nid=1:nrow(.))

state_name_abb <- rel
curr_state<-c(rel$state_name)
...
state_name_abb_fin_all<-subset(state_name_abb_fin_all,state_name!="NATIONAL LEVEL ESTIMATES"  & Variable.Name %in% msaWgt)
...
write.csv(file=file.path(output_weighting_tableshell_folder,"TableAMSADummySRS.csv"),
          x=state_name_abb_fin_SummTabA1)
...
fileoutTabA<-file.path(output_weighting_tableshell_folder,"TableA_MSA_Shell_SRS.xlsx")
...
wbShell_TabA_MSA<- createWorkbook()



######Table A shell State -Summary######

addWorksheet(wbShell_TabA_MSA,"Summary_MSA_TabA_Ex1",gridLines = T) #comment out after creating

removeCellMerge(wbShell_TabA_MSA,"Summary_MSA_TabA_Ex1", cols = 3:15, rows = 6:6)


#Table Title
writeData(wbShell_TabA_MSA,sheet="Summary_MSA_TabA_Ex1", x=tableASummTitle,withFilter = F,colNames = FALSE, startRow =1, startCol=1, headerStyle=headerStyle )



#Header
writeData(wbShell_TabA_MSA,sheet="Summary_MSA_TabA_Ex1", x=RelBiasCtype,withFilter = F,colNames = FALSE, startRow =6, startCol=3)
mergeCells(wbShell_TabA_MSA, "Summary_MSA_TabA_Ex1", cols = 3:15, rows = 6:6)

#Crime Type
writeData(wbShell_TabA_MSA,sheet="Summary_MSA_TabA_Ex1", x=hcolNames_TabA,withFilter = F,colNames = FALSE, startRow =7,startCol=3)

#State by Weight
writeData(wbShell_TabA_MSA,sheet="Summary_MSA_TabA_Ex1", x=state_name_abb_fin_all[,c("state_name_char", "Description") ],withFilter = F,colNames = FALSE, startRow =8,startCol=1)


addStyle(wbShell_TabA_MSA,sheet = "Summary_MSA_TabA_Ex1",mystyle,rows =6:7 ,cols = 1:16,gridExpand = T) #Headers
addStyle(wbShell_TabA_MSA,sheet = "Summary_MSA_TabA_Ex1",mystyle,rows =8:(7+nrow(msaall)),cols = 1:1,gridExpand = T) #State Names
addStyle(wbShell_TabA_MSA,sheet = "Summary_MSA_TabA_Ex1",mystyleL,rows =8:(7+nrow(msaall)) ,cols = 2:16,gridExpand = T) #Weight Descriptions

setColWidths(wbShell_TabA_MSA, sheet = "Summary_MSA_TabA_Ex1", cols = 1:1,widths =30)
setColWidths(wbShell_TabA_MSA, sheet = "Summary_MSA_TabA_Ex1", cols = 2:2,widths =50)
setColWidths(wbShell_TabA_MSA, sheet = "Summary_MSA_TabA_Ex1", cols = 3:16,widths =30)
...
for(i in 1:length(weight_lists)){
  ...
  addWorksheet(wbShell_TabA_MSA,weight_lists[R],gridLines = T)
  
  removeCellMerge(wbShell_TabA_MSA, weight_lists[[R]], cols = 2:4, rows = 7:7) # removes any intersecting merges
  removeCellMerge(wbShell_TabA_MSA, weight_lists[[R]], cols = 6:7, rows = 7:7)
  
  #Weight Description
  writeData(wbShell_TabA_MSA,sheet=weight_lists[[R]], x=weight_list_descrip[[R]],withFilter = F,colNames = FALSE, startRow =1, startCol=1, headerStyle=headerStyle )
  
  #Footnotes
  writeData(wbShell_TabA_MSA,sheet=weight_lists[[R]], x=footnote,withFilter = F,colNames = FALSE, startRow =3, startCol=1)
  
  #Header
  writeData(wbShell_TabA_MSA,sheet=weight_lists[[R]], x=hcolNames_TabA1[,1],withFilter = F,colNames = FALSE, startRow =6, startCol=2, headerStyle=headerStyle )
  writeData(wbShell_TabA_MSA,sheet=weight_lists[[R]], x=hcolNames_TabA1[,2],withFilter = F,colNames = FALSE, startRow =6, startCol=3, headerStyle=headerStyle)
  mergeCells(wbShell_TabA_MSA, weight_lists[[R]], cols=3:5, rows=6:6)
  writeData(wbShell_TabA_MSA,sheet=weight_lists[[R]], x=hcolNames_TabA1[,3],withFilter = F,colNames = FALSE, startRow =6,startCol=6, headerStyle=headerStyle)
  writeData(wbShell_TabA_MSA,sheet=weight_lists[[R]], x=hcolNames_TabA1[,4],withFilter = F,colNames = FALSE, startRow =6,startCol=7, headerStyle=headerStyle)
  mergeCells(wbShell_TabA_MSA, weight_lists[[R]], cols=7:8, rows=6:6)
  
  #Header2
  writeData(wbShell_TabA_MSA,sheet=weight_lists[[R]], x=hcolNames_TabA2,withFilter = F,colNames = FALSE, startRow =7, startCol=2,headerStyle=headerStyle)
  ...
  writeData(wbShell_TabA_MSA,sheet=weight_lists[[R]], x=state_name_abb_fin_SummTabA[keepRows,],withFilter = F,colNames = FALSE, startRow =8, headerStyle=headerStyle)
  
  addStyle(wbShell_TabA_MSA,weight_lists[[R]],mystyleA2,rows =6:6 ,cols = 2:8,gridExpand = T) #Header
  addStyle(wbShell_TabA_MSA,weight_lists[[R]],mystyleA2,rows =7:(7+length(keepRows)) ,cols = 1:8,gridExpand = T) #Header
  
  setColWidths(wbShell_TabA_MSA, weight_lists[[R]], cols = 1:8,widths = c(40,30,20,20,20,30,10,10))
  
  
}
saveWorkbook(wbShell_TabA_MSA,file =fileoutTabA ,overwrite = T)
