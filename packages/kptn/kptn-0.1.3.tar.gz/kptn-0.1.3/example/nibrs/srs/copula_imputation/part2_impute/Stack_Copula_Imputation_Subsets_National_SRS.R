...
#Note (18Dec2024): Creating national SRS pipeline version. Heavily basing off original SRS equivalent.
library(data.table)
...
temp.rowSpecs <- fread("../data/Indicator_Table_Row_Specs_National_SRS.csv") %>%
  ...
  sapply(temp.stratVars,function(temp.stratVar){
    sapply(temp.stratLvls,function(temp.stratLvl){
      ...
      if (file.exists(file.path(output_copula_data_folder,
                                paste0("Table_",temp.table,"_Final_Agency_File_Perm_",temp.perm,"_ColNum_",temp.colnum,"_Nonzero_Pop_Rates_",temp.stratVar,"_",temp.stratLvl,"_National_SRS.csv")))){
        ...
        if (nrow(finalA)>0){
          finalA <- finalA %>%
			...
			#20Mar2023: ensuring county is character
          ...
          sapply(1:length(temp.step2Sects),function(j){
            ...
            temp.vars <- colnames(finalA) %>% str_subset(paste0("^t_",temp.table,"_",temp.sect,"_",temp.rows))
            sapply(temp.vars,function(temp.var){
              temp.col <- temp.var %>%
                str_extract("(?<=_)\\d+$") %>%
                as.numeric()
              temp.step1Var <- colnames(finalA) %>% str_subset(paste0("t_",temp.table,"_\\d+_",temp.step1RowsRegEx,"_",temp.col))
              if(any(finalA %>% select(all_of(temp.var)) %>% is.na())){
                log_debug(paste0("Fixing ",temp.var," for finalA column set ",temp.colnum))
                if(nrow(finalA %>% subset(sourceInd=="SRS Reporter" & eval(as.symbol(temp.step1Var))>0))>0){
                  temp.constVal <- finalA %>% 
                    subset(sourceInd=="SRS Reporter" & eval(as.symbol(temp.step1Var))>0) %>% 
                    getElement(temp.var) %>%
                    mean()
                } else {
                  temp.constVal <- 0
                ...
                finalA <- finalA %>%
                  mutate(!!temp.var := ifelse(sourceInd=="SRS Non-reporter" & eval(as.symbol(temp.step1Var))>0,
                                              min(temp.constVal,eval(as.symbol(temp.step1Var))),
                                              eval(as.symbol(temp.var))))%>%
                  mutate(!!temp.var := ifelse(sourceInd=="SRS Non-reporter" & eval(as.symbol(temp.step1Var))==0,
                                              0,
                                              eval(as.symbol(temp.var))))
                list2env(list("finalA"=finalA),envir=temp.envLoop)
              }
      ...
      if (file.exists(file.path(output_copula_data_folder,
                                paste0("Table_",temp.table,"_Final_Agency_File_Perm_",temp.perm,"_ColNum_",temp.colnum,"_Missing_Demo_Rates_",temp.stratVar,"_",temp.stratLvl,"_National_SRS.csv")))){
        ...
        if (nrow(finalB)>0){
          ...
          sapply(1:length(temp.step2Sects),function(j){
            ...
            temp.sect <- temp.step2Sects[j]
            temp.rows <- temp.step2Rows[j]
            temp.vars <- colnames(finalB) %>% str_subset(paste0("t_",temp.table,"_",temp.sect,"_",temp.rows))
            sapply(temp.vars,function(temp.var){
              temp.col <- temp.var %>%
                str_extract("(?<=_)\\d+$") %>%
                as.numeric()
              temp.step1Var <- colnames(finalB) %>% str_subset(paste0("t_",temp.table,"_\\d+_",temp.step1RowsRegEx,"_",temp.col))
              if(any(finalB %>% select(all_of(temp.var)) %>% is.na())){
                log_debug(paste0("Fixing ",temp.var," for finalB column set ",temp.colnum))
                if(nrow(finalB %>% subset(sourceInd=="SRS Reporter" & eval(as.symbol(temp.step1Var))>0))>0){
                  temp.constVal <- finalB %>% 
                    subset(sourceInd=="SRS Reporter" & eval(as.symbol(temp.step1Var))>0) %>% 
                    getElement(temp.var) %>%
                    mean()
                } else {
                ...
                finalB <- finalB %>%
                  ...
                  mutate(!!temp.var := ifelse(sourceInd=="SRS Non-reporter" & eval(as.symbol(temp.step1Var))==0,
                                              0,
                                              eval(as.symbol(temp.var))))
                list2env(list("finalB"=finalB),envir=temp.envLoop)
              }
              return(NULL)
            })
      ...
      if (file.exists(file.path(output_copula_data_folder,
                                paste0("Table_",temp.table,"_Final_Agency_File_Perm_",temp.perm,"_ColNum_",temp.colnum,"_Zero_Pop_Rates_",temp.stratVar,"_",temp.stratLvl,"_National_SRS.csv")))){
        ...
        if (nrow(finalC)>0){
          ...
          sapply(1:length(temp.step2Sects),function(j){
            ...
            temp.sect <- temp.step2Sects[j]
            temp.rows <- temp.step2Rows[j]
            temp.vars <- colnames(finalC) %>% str_subset(paste0("t_",temp.table,"_",temp.sect,"_",temp.rows))
            sapply(temp.vars,function(temp.var){
              temp.col <- temp.var %>%
                str_extract("(?<=_)\\d+$") %>%
                as.numeric()
              temp.step1Var <- colnames(finalC) %>% str_subset(paste0("t_",temp.table,"_\\d+_",temp.step1RowsRegEx,"_",temp.col))
              if(any(finalC %>% select(all_of(temp.var)) %>% is.na())){
                log_debug(paste0("Fixing ",temp.var," for finalC column set ",temp.colnum))
                if(nrow(finalC %>% subset(sourceInd=="SRS Reporter" & eval(as.symbol(temp.step1Var))>0))>0){
                  temp.constVal <- finalC %>% 
                    subset(sourceInd=="SRS Reporter" & eval(as.symbol(temp.step1Var))>0) %>% 
                    getElement(temp.var) %>%
                    mean()
                } else {
                ...
                finalC <- finalC %>%
                  ...
                  mutate(!!temp.var := ifelse(sourceInd=="SRS Non-reporter" & eval(as.symbol(temp.step1Var))==0,
                                              0,
                                              eval(as.symbol(temp.var))))
                list2env(list("finalC"=finalC),envir=temp.envLoop)
              }
              return(NULL)
            })
...
final <- data.frame(final)
nNR <- final %>% subset(sourceInd=="SRS Non-reporter") %>% nrow()
if (nNR>0 & nrow(temp.tier4Type96Specs)>0){
  nstacks <- 10
  ...
  if (temp.special=="X"){
    final <- sapply(subsets,function(temp.subset){
      ...
      temp.final <- final %>%
        ...
        mutate(den=ifelse(sourceInd=="SRS Reporter",
      ...
      temp.nNR <- temp.final %>% subset(sourceInd=="SRS Non-reporter") %>% nrow()
      if (temp.nNR>0){
        ...
        temp.final <- temp.final %>%
          mutate(across(.cols=matches(temp.type96Vars),
                        .fns=function(x){
                          ifelse(sourceInd=="SRS Non-reporter",
...
if (nNR>0){
  ...
  #Create the tier 3 variables for non-reporting LEAs
  ...
  sapply(columns,function(i){#Loop over columns
    ...
    if (nrow(temp.tier3Type98Specs)>0){
      ...
      if (names(temp.popGpList)[1] %in% colnames(final)){
        final <- final %>%
          mutate(!!names(temp.popGpList)[1] := ifelse(sourceInd=="SRS Non-reporter",
      ...
      if (names(temp.popGpList)[2] %in% colnames(final)){
        final <- final %>%
          mutate(!!names(temp.popGpList)[2] := ifelse(sourceInd=="SRS Non-reporter",
      ...
      if (names(temp.popGpList)[3] %in% colnames(final)){
        final <- final %>%
          mutate(!!names(temp.popGpList)[3] := ifelse(sourceInd=="SRS Non-reporter",
      ...
      if (names(temp.popGpList)[4] %in% colnames(final)){
        final <- final %>%
          mutate(!!names(temp.popGpList)[4] := ifelse(sourceInd=="SRS Non-reporter",
      ...
      if (names(temp.popGpList)[5] %in% colnames(final)){
        final <- final %>%
          mutate(!!names(temp.popGpList)[5] := ifelse(sourceInd=="SRS Non-reporter",
      ...
      # !!names(temp.popGpList)[6] := ifelse(sourceInd=="SRS Non-reporter",
    ...
    if (nrow(temp.tier3Type99Specs)>0){
      ...
      if (!!names(temp.agcyIndList)[1] %in% colnames(final)){
        final <- final %>%
          mutate(!!names(temp.agcyIndList)[1] := ifelse(sourceInd=="SRS Non-reporter",
      ...
      if (!!names(temp.agcyIndList)[2] %in% colnames(final)){
        final <- final %>%
          mutate(!!names(temp.agcyIndList)[2] := ifelse(sourceInd=="SRS Non-reporter",
      ...
      if (!!names(temp.agcyIndList)[3] %in% colnames(final)){
        final <- final %>%
          mutate(!!names(temp.agcyIndList)[3] := ifelse(sourceInd=="SRS Non-reporter",
      ...
      if (!!names(temp.agcyIndList)[4] %in% colnames(final)){
        final <- final %>%
          mutate(!!names(temp.agcyIndList)[4] := ifelse(sourceInd=="SRS Non-reporter",
      ...
      if (names(temp.agcyIndList)[5] %in% colnames(final)){
        final <- final %>%
          mutate(!!names(temp.agcyIndList)[5] := ifelse(sourceInd=="SRS Non-reporter",
      ...
      if (names(temp.agcyIndList)[6] %in% colnames(final)){
        final <- final %>%
          mutate(!!names(temp.agcyIndList)[6] := ifelse(sourceInd=="SRS Non-reporter",
...
outName <- paste0("Table_",temp.table,"_Final_Agency_File_Perm_",temp.perm,"_Rates_",temp.stratVar,"_National_SRS")
