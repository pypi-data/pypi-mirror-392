...
#	select(ORI, ori, !!DER_WEIGHT_VARIABLE_SYMBOL, !!DER_ORI_WEIGHT_GROUP_SYMBOL, !!DER_ORI_WEIGHT_DESCRIPT_GROUP_SYMBOL, county) %>%
  select(ORI, ori, !!DER_WEIGHT_VARIABLE_SYMBOL, !!DER_ORI_WEIGHT_GROUP_SYMBOL, !!DER_ORI_WEIGHT_DESCRIPT_GROUP_SYMBOL) %>%
...
pop_data <- read_csv(paste0(pop_file_path, "POP_TOTALS_PERM_", CONST_YEAR, "_FINAL_NATIONAL_SRS.csv"))

#Need to get list of csv output to compute PRB
copula_file_name = paste0("Table_",table,"_Final_Agency_File_Perm_",copula_perm,"_Rates_PARENT_POP_GROUP_CODE2_National_SRS.csv")
...
#DCH[11.25.24] - suggested P.Lee update to handle the agency level file and not pseuodo-ori file.
#main_copula <- merge_on_weights_variance_new_for_bias(raw_main2, incounty=1)
main_copula <- merge_on_weights_variance_new_for_bias(raw_main2, incounty=0)
