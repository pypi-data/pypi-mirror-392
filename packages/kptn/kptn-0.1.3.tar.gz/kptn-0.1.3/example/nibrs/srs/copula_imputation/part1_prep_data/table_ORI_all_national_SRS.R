...
#Note (18Dec2024): Creating SRS national pipeline version (e.g., for 30 year run). Heavily based off original SRS version.
...
#                         paste0("Table ",temp.table," ORI.csv.gz")) %>%
  #read_csv(n_max=0) %>%
temp.allVars <- fread(file=file.path(input_estimate_folder,
                                     paste0("Table ",temp.table," ORI.csv.gz")),
					  nrows=0) %>%
  colnames()
...
table_ORI_raw <- fread(file=file.path(input_estimate_folder,
                                      paste0("Table ",temp.table," ORI.csv.gz")))
...
univ_raw <- fread(file=file.path(input_files_folder, 
                                 paste0("ref_agency_", year, ".csv")))

#NOTE: Setup as external file
#Note (18Dec2024): We are not using the external population file for the SRS national run (since those files don't exist for earlier years)
#                  Instead, we are just going to rely on the population from the universe file
...
#                        guess_max=1e6)
oriMappings <- fread(file=paste0(input_extract_folder,"ORI_VARIANCE.csv.gz"))

#Add ORI to table_ORI
table_ORI <- oriMappings %>% 
#18Dec2024: changing from filtering on JDWgt (original SRS) to NationalWgt (national SRS)
...
table_ORI_all <- table_ORI_all %>%
  #18Dec2024: since we're not using the pop file for the SRS national run, let's just create dummy variables
  #merge.data.table(dplyr::select(allLEAsCBISummary,ORI,county,popResidAgcyCounty_cbi,matches("^pct"),propMult),by="ORI") %>%
  mutate(county="000",
         popResidAgcyCounty_cbi=POPULATION,
		 propMult=1) %>%
...
table_ORI_all <- table_ORI_all %>%
  data.table() %>%
  #18Dec2024: use the NationalWgt, unlike the original SRS version, which used JDWgt
  #           also, drop and remove join on county
  merge.data.table(dplyr::select(oriMappings,ORI,weight=NationalWgt),#,county
                   by=c("ORI"))#,"county"))
...
table_ORI_all <- table_ORI_all %>%
  ...
  #18Dec2024: just set all of the demographic variables to missing
  mutate(popResidAgcyCounty_ageLT5_cbi=NA_real_,
         popResidAgcyCounty_age5to14_cbi=NA_real_,
         popResidAgcyCounty_age15_cbi=NA_real_,
         popResidAgcyCounty_age16_cbi=NA_real_,
         popResidAgcyCounty_age17_cbi=NA_real_,
         popResidAgcyCounty_age18to24_cbi=NA_real_,
         popResidAgcyCounty_age25to34_cbi=NA_real_,
         popResidAgcyCounty_age35to64_cbi=NA_real_,
         popResidAgcyCounty_ageGTE65_cbi=NA_real_,
         popResidAgcyCounty_sexMale_cbi=NA_real_,
         popResidAgcyCounty_sexFemale_cbi=NA_real_,
         popResidAgcyCounty_raceWhite_cbi=NA_real_,
         popResidAgcyCounty_raceBlack_cbi=NA_real_,
         popResidAgcyCounty_raceAIAN_cbi=NA_real_,
         popResidAgcyCounty_raceAsian_cbi=NA_real_,
         popResidAgcyCounty_raceNHPI_cbi=NA_real_,
         #Note (JDB 30MAR2023): Adding poverty vars
         popResidAgcyCounty_incomeRatioLT1_cbi=NA_real_,
         popResidAgcyCounty_incomeRatio1to2_cbi=NA_real_,
         popResidAgcyCounty_incomeRatioGTE2_cbi=NA_real_,
		 #Note (JDB 09MAY2023): Adding 5-11, 12-14, LT 12, 12-17, LT 18, GTE 18
		 popResidAgcyCounty_age5to11_cbi=NA_real_,
		 popResidAgcyCounty_age12to14_cbi=NA_real_,
		 popResidAgcyCounty_ageLT12_cbi=NA_real_,
		 popResidAgcyCounty_age12to17_cbi=NA_real_,
		 popResidAgcyCounty_ageLT18_cbi=NA_real_,
		 popResidAgcyCounty_ageGTE18_cbi=NA_real_) %>%
...
fwrite_wrapper(table_ORI_all,
       file.path(output_copula_data_folder,
                 paste0("Table_",temp.table,"_ORI_all_national_SRS.csv")))