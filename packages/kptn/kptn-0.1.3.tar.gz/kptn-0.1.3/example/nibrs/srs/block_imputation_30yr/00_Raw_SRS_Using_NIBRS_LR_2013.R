...
raw_converted_srs <- read_csv(paste0(filepathin, "SRS_Original_Combined_LR.csv"))
...
for(i in 1:nrow(CROSSWALK_RAPE)){
  ...
  #Get the values from revised rape if legacy rape is 0
  tbd_srs1_1 <- tbd_srs1_1 %>%
    mutate(
      
      !!(tbd_legacy_var_sym) := fcase(
        
        #If legacy rape is 0 and revised rape is greater than 0, then use value from revised rape
        !!(tbd_legacy_var_sym) == 0 & !!(tbd_revised_var_sym) > 0, !!(tbd_revised_var_sym),
        #Otherwise use legacy
        !is.na(!!(tbd_legacy_var_sym)),  !!(tbd_legacy_var_sym)
      )
    )
...
#Output to the share
srs3 %>%
  write_csv(paste0(block_imputation_output, "Raw_SRS_Using_Converted_LR.csv"), na="")
