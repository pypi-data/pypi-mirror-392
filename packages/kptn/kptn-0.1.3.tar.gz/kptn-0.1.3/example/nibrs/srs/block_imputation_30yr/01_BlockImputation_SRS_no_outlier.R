...
tbd_srs <- tbd_srs1 %>%
  #Need to drop the legacy rape variables
  select(
    !!!(paste0("-", CONST_SRS_TOTAL_LEGACY_RAPE)) %>% rlang:::parse_exprs(),
    !!!(paste0("-", CONST_SRS_FORC_LEGACY_RAPE)) %>% rlang:::parse_exprs(),
    !!!(paste0("-", CONST_SRS_ATTEMPTED_LEGACY_RAPE)) %>% rlang:::parse_exprs()
  )
