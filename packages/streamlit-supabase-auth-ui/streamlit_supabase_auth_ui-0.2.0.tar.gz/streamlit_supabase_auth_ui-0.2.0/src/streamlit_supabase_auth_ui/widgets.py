        ...
        if st.session_state['LOGGED_IN'] == False:
            ...
            with del_login.form("Login Form"):
                ...
                if login_submit_button == True:
                    ...
                    elif authenticate_user_check is None:
                        st.error("You are already logged in in another session! Please log out and log in again.")
                    else:
                        st.session_state['LOGGED_IN'] = True
    ...
    def logout_widget(self) -> None:
        ...
        if st.session_state['LOGGED_IN'] == True:
            ...
            if logout_click_check == True:
                u = self.cookies['__streamlit_login_signup_ui_username__'] 
                supabase.table("user_authentication").update(json={"last_logout": time.time()}).eq("username", u).execute()
