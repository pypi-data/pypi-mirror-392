    ...
    for _, row in df_iter.iterrows():
        ...
        # Process bolus insulin events
            ...
            pumphistory.append({
                "timestamp": ts,
                "_type": "Bolus",
                "amount": row['INSNMBOL'],  # Actual insulin delivered (U)
                "programmed": row['INSNMBOL'],  # Insulin programmed to deliver (U)
            ...
            if row['INSSTYPE'] == 'extended':
                # First append normal bolus amount
                pumphistory.append({
                    "timestamp": ts,
                    "_type": "Bolus",
                    "amount": row['INSNMBOL'],  # Extended bolus amount (U)
                    "programmed": row['INSNMBOL'],
                    "unabsorbed": 0,
                    "duration": 0  # Simplified: treating as instantaneous
                })
                # And then the extended bolus amount
                pumphistory.append({
                    "timestamp": ts,
                    "_type": "Bolus",
                    "amount": row['INSEXBOL'],  # Extended bolus amount (U)
                    "programmed": row['INSEXBOL'],
                    "unabsorbed": 0,
                    "duration": 0  # Simplified: treating as instantaneous
                })
