...
class Commander:
    ...
    def start_worker(self, feed_name: str, interval: int = 5) -> Dict[str, int]:
        """
...
class Record:
    """
    Professional MongoDB document wrapper with state tracking and caller-IP
    encoding for any state changes.

    State format (new):
        "T|<timestamp>|<ip>"
        "F|<timestamp>|<ip>"

    Backwards compatibility: older states without an IP ("T|<timestamp>") are
    still decoded correctly; the `ip` field will be None in that case.

    Document Structure example:
        {
            "_id": ...,
            "stats": {"scraping": "T|20241115143022|192.168.1.50"},
            "meta": {"scraping": {"url": "...", "items": 42}}
        }
    """
    ...
    def _encode_state(self, success: bool, ip: Optional[str] = None) -> str:
        """Encode state as 'T|<timestamp>|<ip>' or 'F|<timestamp>|<ip>'.

        If `ip` is not provided, a best-effort local IP is used. Callers in
        networked environments should pass the caller's IP explicitly.
        """
    ...
    def _decode_state(self, encoded: str, state_name: str) -> Optional[Dict[str, Any]]:
        """Decode both old (2-part) and new (3-part) encodings.
        ...
        try:
            ...
            return {
                'success': flag == 'T',
                'ts': ts,
                'timestamp': self._parse_timestamp(ts) if ts else None,
                'ip': ip,
                'meta': self._doc.get('meta', {}).get(state_name, {})
            }
        except Exception as e:
    ...
    def mark_done(self, state: str, ip: Optional[str] = None, **meta) -> bool:
        """Mark a state as successful. Optionally pass the caller's IP and
        arbitrary meta which will be stored under `meta.<state>`.
        """
        encoded = self._encode_state(True, ip)
    ...
    def mark_fail(self, state: str, ip: Optional[str] = None, **meta) -> bool:
        """Mark a state as failed. Optionally pass the caller's IP and
        arbitrary meta which will be stored under `meta.<state>`.
        """
        encoded = self._encode_state(False, ip)
        update_doc = {f'stats.{state}': encoded}
        if meta:
            self._doc['meta'][state] = meta
            update_doc[f'meta.{state}'] = meta
    ...
    def mark_reset(self, state: str) -> bool:
        """Reset (remove) a state's stats and meta.

        This action unsets both stats.<state> and meta.<state> in MongoDB and
        updates the in-memory document accordingly.
        """
        try:
            if state in self._doc.get('stats', {}):
                self._doc['stats'][state] = None
            if state in self._doc.get('meta', {}):
    ...
    def has_state(self, state: str) -> bool:
        return state in self._doc.get('stats', {}) and self._doc['stats'][state] is not None
    ...
    def get_all_states(self) -> Dict[str, Optional[Dict[str, Any]]]:
        return {
            ...
            for state, value in self._doc.get('stats', {}).items()
        }
