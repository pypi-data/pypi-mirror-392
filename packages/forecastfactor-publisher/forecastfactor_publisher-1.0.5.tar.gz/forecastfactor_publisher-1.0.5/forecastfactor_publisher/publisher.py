...
def forecastfactor_publish(
    ...
    Parameters:
        ...
        demand_supply (Dict): Dictionary with keys 'shortTerm' and 'longTerm', each containing a dictionary with 'demand' and 'supply' keys. Each value is a list of [x, y] coordinate pairs. Format: {"shortTerm": {"demand": [[x, y], [x, y], ...], "supply": [[x, y], [x, y], ...]}, "longTerm": {"demand": [[x, y], [x, y], ...], "supply": [[x, y], [x, y], ...]}}.
    ...
    if "demand" not in demand_supply["shortTerm"] or "supply" not in demand_supply["shortTerm"]:
        return {"status": "error", "message": "Invalid format: 'demandSupply.shortTerm' must contain both 'demand' and 'supply' keys."}
    if not isinstance(demand_supply["shortTerm"]["demand"], list) or not isinstance(demand_supply["shortTerm"]["supply"], list):
        return {"status": "error", "message": "Invalid format: 'demandSupply.shortTerm' values must be lists."}
    if not all(isinstance(item, (list, tuple)) and len(item) == 2 for item in demand_supply["shortTerm"]["demand"]):
        return {"status": "error", "message": "Invalid format: 'demandSupply.shortTerm.demand' values must be lists of [x, y] pairs."}
    if not all(isinstance(item, (list, tuple)) and len(item) == 2 for item in demand_supply["shortTerm"]["supply"]):
        return {"status": "error", "message": "Invalid format: 'demandSupply.shortTerm.supply' values must be lists of [x, y] pairs."}
