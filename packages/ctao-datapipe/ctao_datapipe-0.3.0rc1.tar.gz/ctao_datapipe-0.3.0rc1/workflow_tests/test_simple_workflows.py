...
def test_dl0_dl1_camcalib(tmp_path):
    ...
    dl0_filename = str(get_dataset_path("gamma_prod6_preliminary.simtel.zst"))
    dl1_filename = "events_with_camcalib.dl1.h5"
    values = tmp_path / "values.cfg"
    calibpipe_camcalib_single_chunk = str(
        ...
        yaml.dump(
            {
                ...
                "ProcessorTool": {
                    "monitoring_source_list": ["HDF5MonitoringSource"],
                    "HDF5MonitoringSource": {
                        "input_files": [calibpipe_camcalib_single_chunk]
                    },
                },
            },
            f,
    ...
    with TableLoader(out_path) as loader:
        ...
        tel_events = loader.read_telescope_events()
        assert len(tel_events) == max_events
        assert "tel_id" in tel_events.colnames
        assert "hillas_width" in tel_events.colnames
