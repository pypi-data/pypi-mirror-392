...
from whitemagic.parallel.pools import ThreadingTier, PoolConfig
...
class AdaptiveThreadingController:
    ...
    def avg_cpu_usage(self) -> float:
        ...
        if not self._metrics_history:
            return 0.0
        return sum(m.cpu_percent for m in self._metrics_history) / len(self._metrics_history)
    ...
    def avg_memory_usage(self) -> float:
        ...
        if not self._metrics_history:
            return 0.0
        return sum(m.memory_percent for m in self._metrics_history) / len(self._metrics_history)
