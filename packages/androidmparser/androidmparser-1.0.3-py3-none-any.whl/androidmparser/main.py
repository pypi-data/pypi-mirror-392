...
if __name__ == "__main__":
    DEFAULT_PAYLOAD = os.path.join(get_data_dir(), "payloads_default.json")
    if not os.path.exists(DEFAULT_PAYLOAD):
        # Копируем из ресурсов проекта
        import shutil

        source = os.path.join(os.path.dirname(__file__), "payloads_default.json")
        if os.path.exists(source):
            shutil.copy(source, DEFAULT_PAYLOAD)
    main()
