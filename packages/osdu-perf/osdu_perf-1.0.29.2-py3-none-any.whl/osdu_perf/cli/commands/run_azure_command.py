...
class AzureLoadTestCommand(Command):
    ...
    def _execute_load_test(self, runner, config):
        ...
        try:
            self.logger.info("STEP 4 Azure load test wont be able to read environment variable once in a while hence running a dummy test first...")
            temp_name = "dummy_" + config['execution_display_name'][6:]
            execution_result = runner.run_test(
                test_name=config['test_name'],
                display_name=temp_name
            )
        except Exception as e:
            self.logger.warning(f"STEP 4 Initial test run check failed: {e}")
        ...
        try:
            self.logger.info(f"STEP 4 Stop the dummy tests runs {temp_name}")
            runner.stop_test_run(temp_name)
        except Exception as e:
            self.logger.warning(f"STEP 4 Failed to stop dummy test run: {e} {temp_name}")
