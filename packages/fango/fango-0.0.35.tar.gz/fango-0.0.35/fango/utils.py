...
from fango.generics import MethodT, ModelT
...
def get_choices_as_data(choices_class: type[IntegerChoices | TextChoices]) -> list[ChoicesItem]:
    """
...
def get_orm_object_or_404(django_model: type[ModelT], pk: int) -> ModelT:
    try:
...
async def aget_orm_object_or_404(django_model: type[ModelT], pk: int) -> ModelT:
    try:
...
def parse_str_as_json_type(value: Any) -> Any:
    """
    Get real typed value from sting value
...
def is_valid_date(string, fmt="%Y-%m-%d"):
    try:
...
def is_valid_iso_datetime(string, fmt="%Y-%m-%dT%H:%M:%S.%fZ"):
    try:
        datetime.strptime(string, fmt)
        return True
    except ValueError:
...
def get_related_model_through_lookup(queryset, lookup_path):
    model = queryset.model

    for field_name in lookup_path.split("__"):
        field = model._meta.get_field(field_name)
        if not hasattr(field, "related_model") or not field.related_model:
            raise ValueError(f"Поле '{field_name}' не является связью в модели {model.__name__}")
