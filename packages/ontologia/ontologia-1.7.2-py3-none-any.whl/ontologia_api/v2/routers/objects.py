...
def _ogm_upsert_object(*args, **kwargs) -> ObjectReadResponse:
    ...
    if not model_class:
        raise HTTPException(status_code=400, detail=f"OGM model not found for {object_type_api_name}")
...
def _ogm_get_object(*args, **kwargs) -> ObjectReadResponse:
    ...
    if len(args) == 2 or len(args) == 3:
        ...
        valid_at = args[2] if len(args) == 3 else None
    else:
        service = kwargs.get("service", "default")
        instance = kwargs.get("instance", "runtime")
        object_type_api_name = kwargs["object_type_api_name"]
...
def _map_where_to_filters(where: list[WhereCondition]):
    ...
    def op_map(op: str, value):
        ...
        if op in ("contains", "startswith", "endswith"):
            pattern = (
                f"%{value}%" if op == "contains" else f"{value}%" if op == "startswith" else f"%{value}"
            )
...
def _ogm_search_objects(
    ...
    from ontologia.application.instances_service import ObjectSearchRequest as DomainSearchRequest, SearchOrder
...
def upsert_object_instance(
    ...
    ontologyApiName = request.path_params.get("ontologyApiName") or "default"
    instance_name = "runtime"
    
    # Dual-mode: OGM or Service-based
    if use_ogm and OGM_AVAILABLE:
        try:
            return _ogm_upsert_object(ontologyApiName, instance_name, objectTypeApiName, pk, body.properties)
        except Exception:
    ...
    return ObjectReadResponse(
        rid=getattr(r, "rid", f"obj-{objectTypeApiName}-{getattr(r, 'pk_value', pk)}"),
        objectTypeApiName=objectTypeApiName,
        pkValue=getattr(r, "pk_value", pk),
...
def get_object_instance(
    ...
    if use_ogm and OGM_AVAILABLE:
        try:
            return _ogm_get_object(ontologyApiName, instance_name, objectTypeApiName, pk, validAt)
        except Exception:
            # Fall back to service-based if OGM fails
            pass
...
def delete_object_instance(
    ...
    if use_ogm and OGM_AVAILABLE:
        try:
            ok = _ogm_delete_object(ontologyApiName, instance_name, objectTypeApiName, pk)
            if not ok:
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Object not found")
            return None
        except Exception:
...
def list_objects(
    ...
    ontologyApiName = request.path_params.get("ontologyApiName")
    ...
    if use_ogm and OGM_AVAILABLE:
        try:
            return _ogm_list_objects(
                ontologyApiName or "default", "runtime", objectTypeApiName, limit, offset, validAt
            )
        except Exception:
            pass
    return query_service.list_objects(
    ...
    if use_ogm and OGM_AVAILABLE:
        try:
            return _ogm_search_objects(
                ontologyApiName or "default", "runtime", objectTypeApiName, body
            )
        except Exception:
            pass
...
def aggregate_objects(
    ...
    # Service-based approach (original logic)
    return query_service.aggregate_objects(objectTypeApiName, body)
...
def get_linked_objects(
    ...
    limit: int = Query(default=100, ge=1, le=1000),
    offset: int = Query(default=0, ge=0),
    direction: str = Query(default="outgoing", description="Traversal direction: outgoing or incoming"),
    validAt: datetime | None = Query(
        default=None,
        description="Timestamp used for bitemporal validity filtering",
    ),
    ...
    if use_ogm and OGM_AVAILABLE:
        try:
            ontology = get_ogm_ontology(ontologyApiName or "default", "runtime")
            if not ontology:
            ...
            with ontology.get_session() as session:  # type: ignore[union-attr]
                ...
                for edge in resp.linked_objects:
                    ...
                    dto = ios.get_object(
                        ontologyApiName or "default", "runtime", edge.target_object_type_api_name, target_pk
                    )
                    objs.append(
                        ObjectReadResponse(
                            rid=getattr(dto, "rid", f"obj-{edge.target_object_type_api_name}-{target_pk}"),
                            objectTypeApiName=edge.target_object_type_api_name,
                            pkValue=target_pk,
                            properties=dict(dto.properties or {}),
                        )
                    )
                return ObjectListResponse(objects=objs)
        except Exception:
            pass
