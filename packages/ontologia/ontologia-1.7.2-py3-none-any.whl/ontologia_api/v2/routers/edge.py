...
async def list_node_keys(
    ...
    try:
        record_delivered(session, [it.id for it in rows])
    except Exception:
...
async def enqueue_command(
    ontologyApiName: str, cmd: _EnqueueCommand, session: Session = Depends(get_session)
):
    ...
    # TODO: Extract principal from request headers when needed
    principal = None
