...
def upsert_edge_node(session: Session, *, node_id: str, public_key: str | None, capabilities: dict[str, Any] | None, software: dict[str, Any] | None, hw: dict[str, Any] | None) -> EdgeNode:
...
def ensure_message_not_seen(session: Session, *, msg_id: str, node_id: str | None, ttl: int | None) -> bool:
...
def get_public_key(session: Session, node_id: str) -> str | None:
    ...
    stmt = select(EdgeNodeKey).where(EdgeNodeKey.node_id == node_id, EdgeNodeKey.active == True).limit(1)  # noqa: E712
...
def add_edge_key(session: Session, node_id: str, public_key: str, *, make_active: bool = False) -> EdgeNodeKey:
    ...
    if make_active:
        ...
        for row in session.exec(select(EdgeNodeKey).where(EdgeNodeKey.node_id == node_id, EdgeNodeKey.active == True)).all():  # noqa: E712
            row.active = False
            session.add(row)
        record.active = True
    session.add(record)
...
def list_edge_keys(session: Session, node_id: str) -> list[EdgeNodeKey]:
    return session.exec(select(EdgeNodeKey).where(EdgeNodeKey.node_id == node_id)).all()
