            ...
            if params:
                ...
                if isinstance(params, dict):
                    ...
                    for i in range(len(query_parts) - 1):
                        # Find the parameter name that corresponds to this ?
                        # This is a simple approach - we assume the order matches
                        param_key = list(params.keys())[i] if i < len(params) else None
                        if param_key:
                            param_list.append(params[param_key])
