...
class SQLMetamodelRepository:
    ...
    def create_resource(
        ...
        # Prepare the data for insertion with correct column names
        data = {
            "rid": str(uuid.uuid4()),  # Generate a new UUID for the rid
            "resource_type": resource_type,
            "service": service,
            "instance": instance,
            "created_at": datetime.datetime.now(datetime.UTC),  # Add current timestamp
            "updated_at": datetime.datetime.now(datetime.UTC),  # Add current timestamp
        }

        # Add display_name if the column exists (it might be named differently)
        ...
        # Add optional fields if they exist in kwargs and in the table
        optional_fields = {"description", "metadata"}
        for field in optional_fields:
            if field in kwargs and field in columns:
                data[field] = kwargs[field]

        # Use SQLAlchemy Core to insert the record directly
        ...
        # Query the record to get a proper model instance
        resource = self.session.get(ResourceModel, inserted_id)
