...
class ObjectInstanceRepositoryAdapter(ObjectInstanceRepository):
    ...
    def _resolve_object_type_rid(self, object_type_rid: str) -> tuple[str, str, str]:
        ...
        # Attempt to read service/instance from Resource table using the session if available
        try:
    ...
    def save_object_instance(self, obj: ObjectInstance) -> ObjectInstance:
        print(f"DEBUG: Adapter save_object_instance - session ID: {id(self._sql_repo.session)}")
        print(f"DEBUG: Adapter save_object_instance - session new before: {len(self._sql_repo.session.new)}")
        result = self._sql_repo.save_object_instance(obj)
        print(f"DEBUG: Adapter save_object_instance - session new after: {len(self._sql_repo.session.new)}")
        return result
    ...
    def delete_object_instance(
        ...
        service, instance, api_name = self._resolve_object_type_rid(object_type_rid)
        return self._sql_repo.delete_object_instance(service, instance, api_name, pk_value)
