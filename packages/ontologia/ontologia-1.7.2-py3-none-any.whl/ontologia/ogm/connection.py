...
class CoreServiceProvider:
    def __init__(self, session: Session, event_bus: DomainEventBus | None = None):
        print(f"DEBUG: CoreServiceProvider.__init__ session ID: {id(session)}")
        print(f"DEBUG: CoreServiceProvider.__init__ session object: {session}")
        self.session = session
        print(f"DEBUG: CoreServiceProvider stored session ID: {id(self.session)}")
        print(f"DEBUG: CoreServiceProvider stored session object: {self.session}")
    ...
    def instances_repository(self) -> ObjectInstanceRepositoryAdapter:
        if self._instances_repo is None:
            print(f"DEBUG: CoreServiceProvider instances_repository session ID: {id(self.session)}")
...
class Ontology:
    ...
    def model(self, model_class):
        ...
        if (model_class not in _model_registry and 
            hasattr(model_class, "__primary_key__") and 
            hasattr(model_class, "__object_type_api_name__")):
