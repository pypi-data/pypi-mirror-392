    ...
    async def create_object(self, object_type: str, data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            response = await self._client.post(
                self._build_url(f"objects/{object_type}"), headers=self._headers, params=params, json=data
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    async def update_object(
        ...
        try:
            ...
            response = await self._client.put(
                self._build_url(f"objects/{object_type}/{pk}"), headers=self._headers, params=params, json=data
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
...
class LocalSession(ClientSession):
    ...
    def __init__(self, connection_string: str, *, use_ogm: bool = False, ogm_module: str | None = None):
    ...
    def _get_services(self) -> dict[str, Any]:
        ...
        if self._services is None:
            try:
                ...
                from ontologia.application import (
                    InstancesService,
                    LinkedObjectsService,
                    MetamodelService,
                )
                from ontologia.infrastructure.persistence.sql import (
                    SQLLinkedObjectRepository,  # Updated from SQLLinkRepository
                ...
                with self.session_factory() as session:
                    meta_repo = SQLMetamodelRepository(session)
                    instances_repo = ObjectInstanceRepositoryAdapter(
                        SQLObjectInstanceRepository(session), meta_repo
                    )
                    linked_repo = LinkedObjectRepositoryAdapter(
                        SQLLinkedObjectRepository(session), meta_repo
                    )

                    self._services = {
                        "metamodel": MetamodelService(meta_repo),
                        "instances": InstancesService(instances_repo, meta_repo),  # type: ignore[arg-type]
                        "linked": LinkedObjectsService(linked_repo, meta_repo),  # type: ignore[arg-type]
                    }
            ...
            if getattr(self, "_use_ogm", False):
                ...
                if model is not None and hasattr(model, "_db"):
                    ...
                    req = ObjectSearchRequest(
                        filters=[
                            SearchFilter(field=k, operator=("in" if isinstance(v, (list, tuple, set)) else "eq"), value=(list(v) if isinstance(v, (list, tuple, set)) else v))
                    ...
                    with db.get_session() as session:
                        ...
                        resp = provider.instances_service().search_objects(service, instance, object_type, req)
...
def create_session(
    *,
    ...
    if host:
        return RemoteSession(host=host, token=token, timeout=timeout, ontology=ontology, use_ogm=use_ogm)
    else:
        return LocalSession(connection_string=connection_string or "", use_ogm=use_ogm, ogm_module=ogm_module)


def create_remote_client(
    host: str, token: str | None = None, timeout: float = 30.0, ontology: str = "default", *, use_ogm: bool = False
) -> RemoteSession:
    """
...
def create_local_client(connection_string: str, ontology: str = "default", *, use_ogm: bool = False, ogm_module: str | None = None) -> LocalSession:
    """
