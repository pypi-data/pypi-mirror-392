...
class OGMQuery:
    ...
    async def all(self) -> list[dict[str, Any]]:
        ...
        if hasattr(self._session, "_client") and hasattr(self._session, "_headers"):
            ...
            where = [
                {"property": f, "op": self._normalize_op(op), "value": v} for f, op, v in self._filters
            ]
            order_by = [
                {"property": f, "direction": d} for f, d in self._order_by
            ]
            payload = {
                "where": where,
                "orderBy": order_by,
            ...
            resp = await self._session._client.post(url, headers=self._session._headers, params=params, json=payload)
            resp.raise_for_status()
            data = resp.json()
            return [obj.get("properties", {}) | {"pk": obj.get("pkValue")} for obj in data.get("objects", [])]
            ...
            try:
                ...
                if model is not None and hasattr(model, "_db"):
                    ...
                    req = ObjectSearchRequest(
                        filters=[SearchFilter(field=f, operator=self._normalize_op(op), value=v) for f, op, v in self._filters],
                    ...
                    with db.get_session() as session:
                        ...
                        resp = provider.instances_service().search_objects(service, instance, self._type, req)
