...
class ObjectModel(BaseModel, metaclass=ObjectModelMeta):  # type: ignore[misc]
    ...
    def get(cls, pk: str | int) -> ObjectModel:
        ...
        with session_ctx as session:
            ...
            try:
                ...
                resp = svc.get_object(service, instance, cls.object_type_api_name(), str(pk), valid_at=datetime.now(UTC))
    ...
    def transaction(cls) -> _TransactionContextManager:
        ...
        return _TransactionContextManager(sess)


class _TransactionContextManager:
    """Context manager that sets the thread-local session for the duration of the transaction."""
    ...
    def __exit__(self, exc_type: Any, exc: Any, tb: Any) -> None:
        try:
            ...
            self.session.close()
            # Restore previous thread-local session
