...
class ElasticsearchRepository:
    ...
    def index_object(self, object_data: dict[str, Any]) -> bool:
        """
        ...
        """
        try:
            object_type = object_data.get("objectTypeApiName")
            if not object_type:
            ...
            self.client.index(
                ...
                id=object_data.get("rid"),
                body=doc,
            )

            self.logger.debug(f"Indexed object: {object_data.get('rid')}")
            return True
    ...
    def search_by_text(
        ...
        Returns:
            List of pkValue strings matching the search
        """
        try:
            ...
            es_query: dict[str, Any] = {
                ...
                "size": limit,
                "from": offset,
                "_source": ["pkValue", "objectTypeApiName"],  # Only return these fields
            }
    ...
    def delete_object(self, object_type: str, pk_value: str) -> bool:
        ...
        try:
            ...
            # First find the document by pkValue
            search_result = self.client.search(
                index=index_name,
                body={"query": {"term": {"pkValue": pk_value}}, "_source": ["_id"], "size": 1},
            )
            ...
            self.logger.info(f"Deleted object {pk_value} from index {index_name}")
            return True
