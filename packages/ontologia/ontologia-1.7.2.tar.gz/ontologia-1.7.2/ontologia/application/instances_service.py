...
class InstancesService:
    ...
    def upsert_object(
        ...
        print(f"DEBUG: Domain upsert_object called with service={service}, instance={instance}")
        ...
        try:
            ...
            if object_type is None:
                print(f"DEBUG: Object type not found, creating new one")
                # Create object type if it doesn't exist
                from ontologia.application.metamodel_service import MetamodelService, ObjectTypePutRequest
                metamodel_service = MetamodelService(self.metamodel_repository.session)
                req = ObjectTypePutRequest(
                    api_name=object_type_api_name,
                    display_name=object_type_api_name.title(),
                    properties={
                        "id": {"dataType": "string", "required": True},
                    },
                )
                object_type = metamodel_service.upsert_object_type(object_type_api_name, req)
                print(f"DEBUG: Created new object type: {object_type}")
            if not object_type:
                raise HTTPException(
            ...
            print(f"DEBUG: Existing object found: {existing is not None}")
            ...
            if existing:
                ...
                print(f"DEBUG: Created ObjectInstance with object_type_rid={saved.object_type_rid}, pk={saved.pk_value}")
                logger.info(
                ...
                # Ensure a Resource row exists for the ObjectInstance so subsequent
                # queries using Resource-joined scope find the row.
                try:
                    res = self.metamodel_repository.create_resource(
                        ObjectInstance.__resource_type__,
                        service,
                        instance,
                        f"{object_type.display_name} {pk_value}",
                    )
                ...
                print(f"DEBUG: Session state after save: new={len(self.instances_repository._sql_repo.session.new)}, dirty={len(self.instances_repository._sql_repo.session.dirty)}")
                logger.info(
            ...
            # Commit the session to persist the object
    ...
    def delete_object(
        ...
        deleted = self.instances_repository.delete_object_instance(object_type.rid, pk_value)
        if not deleted:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Object '{object_type_api_name}:{pk_value}' not found",
...
class ObjectInstanceQueryService:
    """
    Service for object instance query operations (read, search, list).

    Handles read operations for object instances with optimized querying,
