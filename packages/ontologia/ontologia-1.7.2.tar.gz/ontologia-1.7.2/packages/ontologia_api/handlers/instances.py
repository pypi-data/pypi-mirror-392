        ...
        if os.getenv("TESTING") in {"1", "true", "True"}:
            ...
            for item in candidates:
                try:
                    ...
                    continue
            # Fallback: app.state hint (allows tests to set instance directly)
            try:
                state_obj = getattr(request.app.state, "instance_query_override", None)
                if state_obj is not None and hasattr(state_obj, "get_object"):
                    try:
                        globals()["last_query_override"] = state_obj
                    except Exception:
                        pass
