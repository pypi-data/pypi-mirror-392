...
def find_due_retries(session: Session, *, limit: int = 100) -> List[CommandReceipt]:
    ...
    rows = session.exec(
        ...
        .where(CommandReceipt.delivered_at.is_not(None))
    ).all()
...
def list_commands(session: Session, *, node_id: str | None = None, status: str | None = None, limit: int = 100):
    stmt = select(CommandReceipt).order_by(CommandReceipt.enqueued_at.desc()).limit(limit)
    if node_id:
