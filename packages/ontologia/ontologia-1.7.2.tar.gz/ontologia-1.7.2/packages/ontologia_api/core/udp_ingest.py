...
class _UDPHandler(asyncio.DatagramProtocol):
    ...
    async def _process(self, payload: dict[str, Any]) -> None:
        ...
        if ptype in {"EVENT", "HELLO"}:
            ok_rl = await self._rate_limit(f"udp:{ptype.lower()}:{node_id}", prefix="UDP", default_capacity=120, default_window=60)
            if not ok_rl:
                return
...
async def start_udp_server() -> asyncio.Transport | None:
    cfg = load_udp_config()
    if not cfg.enabled:
        return None
