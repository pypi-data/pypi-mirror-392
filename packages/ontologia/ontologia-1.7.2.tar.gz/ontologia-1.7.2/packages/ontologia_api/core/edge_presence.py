...
class PresenceBackend:
    async def upsert(self, node_id: str, payload: dict[str, Any], *, ttl: int | None = None) -> None:  # pragma: no cover - interface
...
class _RedisPresence(PresenceBackend):
    ...
    async def upsert(self, node_id: str, payload: dict[str, Any], *, ttl: int | None = None) -> None:
        expiry = ttl if ttl and ttl > 0 else 120
        key = f"{self._key}:{node_id}"
        data = json.dumps({"node_id": node_id, "payload": payload, "updated_at": datetime.now(UTC).isoformat()})
