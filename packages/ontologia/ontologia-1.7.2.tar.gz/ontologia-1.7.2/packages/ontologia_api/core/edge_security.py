...
class SignatureVerifier:
    ...
    async def ensure_key(self, node_id: str, public_key: str) -> None:
        ...
        if existing and existing != public_key:
            logger.warning(
                "Public key mismatch for node_id=%s (keeping existing)", node_id
            )
            return
        if not existing:
            await self._registry.set_key(node_id, public_key)

    async def verify(self, node_id: str, message_bytes: bytes, signature_b64: str, *, algo: str | None = None) -> bool:
        ...
        if algo_to_use in {"ed25519", "eddsa"}:
            ...
            try:
                ...
                logger.error("PyNaCl required for raw/base64 Ed25519 keys. Provide PEM or install pynacl.")
                return False
