...
class TestElasticsearchIntegration:
    ...
    def test_index_and_search_object(self, elasticsearch_repo):
        ...
        results = elasticsearch_repo.search_objects(
            query="John", object_type="test_person", limit=10
        )

        assert len(results) > 0
        assert any(result["primary_key_value"] == "person_123" for result in results)
    ...
    def test_update_object(self, elasticsearch_repo):
        ...
        # Verify update
        results = elasticsearch_repo.search_objects(
            query="Jane", object_type="test_person", limit=10
        )

        updated_obj = next((r for r in results if r["primary_key_value"] == "person_456"), None)
        assert updated_obj is not None
        assert updated_obj["properties"]["age"] == 26
    ...
    def test_delete_object(self, elasticsearch_repo):
        ...
        # Delete object
        success = elasticsearch_repo.delete_object(
            object_type="test_person", primary_key_value="person_789"
        )
        assert success
        ...
        # Verify deletion
        results = elasticsearch_repo.search_objects(
            query="Bob", object_type="test_person", limit=10
        )

        assert not any(r["primary_key_value"] == "person_789" for r in results)
    ...
    def test_json_extraction(self, duckdb_repo):
        ...
        results = duckdb_repo.execute_query(
            ...
            SELECT 
                ...
                JSON_EXTRACT_DOUBLE(properties, '$.age') as age,
                JSON_EXTRACT_DOUBLE(properties, '$.score') as score
            FROM test_json 
            ORDER BY id
            """
        )
...
class TestCrossServiceIntegration:
    ...
    def test_elasticsearch_duckdb_sync(self, elasticsearch_repo, duckdb_repo):
        ...
        es_results = elasticsearch_repo.search_objects(
            query="User", object_type="test_person", limit=10
        )

        assert len(es_results) == 2
        assert all(result["object_type"] == "test_person" for result in es_results)
