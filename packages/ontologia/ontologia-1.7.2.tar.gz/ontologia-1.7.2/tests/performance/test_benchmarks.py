...
class TestGraphTraversalPerformance:
    ...
    def test_sql_vs_graph_traversal_performance(self, benchmark, mock_linked_objects_service):
        """Benchmark SQL vs graph traversal performance comparison."""

        # Mock SQL traversal
        def sql_traversal():
            # Simulate SQL-based traversal
            time.sleep(0.001)  # Simulate database query time
            return [{"target_id": i, "depth": 1} for i in range(10)]

        # Mock graph traversal
        def graph_traversal():
            # Simulate graph-based traversal (should be faster)
        ...
        assert len(sql_result) == len(graph_result) == 10
