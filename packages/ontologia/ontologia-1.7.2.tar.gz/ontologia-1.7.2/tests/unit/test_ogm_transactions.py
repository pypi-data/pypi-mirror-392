...
from ontologia.ogm import Ontology, ObjectModel, LinkModel
from ontologia.ogm.errors import NotFound
...
class TestTransactionBasics:
    ...
    def test_transaction_commit(self, setup_base_schema, transaction_models):
        ...
        TestAccount, TestTransaction = transaction_models
        
        # Apply schema
        setup_base_schema.apply_schema([TestAccount])
        
        # Create account within transaction
        with TestAccount.transaction() as session:
            account = TestAccount(id="acc1", balance=100.0, owner="Alice")
            account.save(session=session)
        
        # Verify account exists after commit
        retrieved_account = TestAccount.get("acc1")
    ...
    def test_transaction_rollback(self, setup_base_schema, transaction_models):
        ...
        initial_account = TestAccount(id="acc1", balance=50.0, owner="Alice")
        initial_account.save()
        
        # Attempt transaction that fails
        try:
            with TestAccount.transaction() as session:
                account = TestAccount(id="acc2", balance=100.0, owner="Bob")
                account.save(session=session)
        ...
        with pytest.raises(NotFound):
            TestAccount.get("acc2")
        # Verify initial account is still there
        assert TestAccount.get("acc1") is not None
    ...
    def test_transaction_with_multiple_operations(self, setup_base_schema, transaction_models):
        ...
        # Apply schema
        setup_base_schema.apply_schema([TestAccount, TestTransaction])
        
        # Create multiple objects in single transaction
        with TestAccount.transaction() as session:
            account1 = TestAccount(id="acc1", balance=100.0, owner="Alice")
            account2 = TestAccount(id="acc2", balance=200.0, owner="Bob")
        ...
        assert TestAccount.get("acc1") is not None
        assert TestAccount.get("acc2") is not None
        assert TestTransaction.get("tx1") is not None
...
class TestNestedTransactions:
    ...
    def test_nested_transaction_commit(self, setup_base_schema, transaction_models):
        ...
        with TestAccount.transaction() as outer_session:
            outer_account = TestAccount(id="acc1", balance=100.0, owner="Alice")
            outer_account.save(session=outer_session)
            
            with TestAccount.transaction() as inner_session:
                inner_account = TestAccount(id="acc2", balance=200.0, owner="Bob")
    ...
    def test_nested_transaction_rollback(self, setup_base_schema, transaction_models):
        ...
        try:
            TestAccount.get("acc2")
            assert False, "acc2 should not exist after inner transaction rollback"
        except NotFound:
            pass  # Expected - inner transaction rolled back
    ...
    def test_outer_transaction_rollback(self, setup_base_schema, transaction_models):
        ...
        try:
            TestAccount.get("acc1")
            assert False, "acc1 should not exist after outer transaction rollback"
        except NotFound:
            pass  # Expected
...
class TestConcurrentTransactions:
    ...
    def test_concurrent_writes(self, setup_base_schema, transaction_models):
        ...
        def create_account(account_id, balance, owner):
            with TestAccount.transaction() as session:
                account = TestAccount(id=account_id, balance=balance, owner=owner)
                account.save(session=session)
        ...
        for i in range(10):
            account = TestAccount.get(f"acc{i}")
    ...
    def test_concurrent_reads_and_writes(self, setup_base_schema, transaction_models):
        ...
        # Create initial account
        initial_account = TestAccount(id="acc1", balance=1000.0, owner="Alice")
        initial_account.save()
        
        def read_account(account_id):
            return TestAccount.get(account_id)
        
        def update_account(account_id, new_balance):
            account = TestAccount.get(account_id)
            if account:
        ...
        # Verify final state
        final_account = TestAccount.get("acc1")
...
class TestTransactionErrorHandling:
    ...
    def test_constraint_violation_rollback(self, setup_base_schema, transaction_models):
        ...
        try:
            with TestAccount.transaction() as session:
                account2 = TestAccount(id="acc1", balance=200.0, owner="Bob")
                account2.save(session=session)
        except Exception:
            pass  # Expected constraint violation
        
        # Verify original account is unchanged
        original_account = TestAccount.get("acc1")
    ...
    def test_session_isolation(self, setup_base_schema, transaction_models):
        ...
        # Create initial account
        initial_account = TestAccount(id="acc1", balance=100.0, owner="Alice")
        initial_account.save()
        
        # Start transaction but don't commit
        with TestAccount.transaction() as session:
            # Modify account within transaction
            account = TestAccount.get("acc1")
            if account:
                account.balance = 200.0
                account.save(session=session)
            
            # Check from outside transaction (should see old value)
            outside_account = TestAccount.get("acc1")
...
class TestTransactionPerformance:
    ...
    def test_transaction_overhead(self, setup_base_schema, transaction_models, benchmark):
        ...
        def create_with_transaction():
            with TestAccount.transaction() as session:
                account = TestAccount(id="perf_acc", balance=100.0, owner="Perf")
                account.save(session=session)
        
        def create_without_transaction():
            account = TestAccount(id="perf_acc_no_tx", balance=100.0, owner="Perf")
            account.save()
...
class TestTransactionIntegration:
    ...
    def test_transaction_with_schema_changes(self, setup_base_schema, transaction_models):
        ...
        setup_base_schema.apply_schema([TestTransaction])
        
        # Create more data in new transaction
        with TestTransaction.transaction() as session:
            transaction = TestTransaction(
                id="tx1", 
                amount=50.0, 
                from_account="acc1", 
