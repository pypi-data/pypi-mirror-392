...
def test_ogm_query_local_mode():
    ...
    with tempfile.NamedTemporaryFile(suffix=".db", delete=True) as tf:
        ...
        client = OntologyClient(connection_string=conn, use_ogm=True, ogm_module="ontology_definitions.models")
        results = asyncio.run(client.ogm_query("company").where("name", "eq", "Acme").limit(10).all())
