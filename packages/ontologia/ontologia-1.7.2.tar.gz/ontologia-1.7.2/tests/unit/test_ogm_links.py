...
class TestLinkModelSchema:
    ...
    def test_schema_with_links(self, setup_base_schema, relationship_models):
        ...
        # Apply schema
        result = setup_base_schema.apply_schema([TestCompany, TestEmployee], dry_run=True)
...
class TestLinkModelOperations:
    ...
    def test_link_model_access_pattern(self, setup_base_schema, relationship_models):
        ...
        link_model = TestEmployee.company
        assert link_model.api_name == 'works_for'
        assert link_model.target_model == TestCompany
...
class TestLinkModelCardinality:
    ...
    def test_one_to_many_relationship(self, setup_base_schema, relationship_models):
        ...
        employees = []
        for i in range(3):
            emp = TestEmployee(id=f"emp{i}", name=f"Employee{i}")
            emp.save()
            employees.append(emp)
        
        # Verify all employees exist
        for emp in employees:
            assert TestEmployee.get(emp.id) is not None
    ...
    def test_many_to_many_relationship(self, setup_base_schema, complex_relationship_models):
        ...
        TestUser, TestGroup, TestMembership = complex_relationship_models
        
        # Apply schema
        setup_base_schema.apply_schema([TestUser, TestGroup, TestMembership])
        ...
        group1 = TestGroup(id="group1", name="Admins")
        group2 = TestGroup(id="group2", name="Developers")
        group1.save()
        group2.save()
        
        # Create memberships
        membership1 = TestMembership(id="mem1", role="admin", user=user1, group=group1)
        membership2 = TestMembership(id="mem2", role="member", user=user2, group=group2)
        membership1.save()
        membership2.save()
        ...
        assert TestGroup.get("group1") is not None
        assert TestGroup.get("group2") is not None
        assert TestMembership.get("mem1") is not None
        assert TestMembership.get("mem2") is not None
...
class TestLinkModelErrorHandling:
    ...
    def test_invalid_target_model(self, ontology):
        ...
        class TestSource(ObjectModel):
            ...
            model_config = {'arbitrary_types_allowed': True}
            id: str
            invalid_link: ClassVar[LinkModel['NonExistentModel']] = LinkModel('invalid_link')
    ...
    def test_missing_inverse_relationship(self, setup_base_schema, relationship_models):
        ...
        company = TestCompany(id="company1", name="Test Company")
        company.save()
        
        # The inverse 'employees' doesn't exist on TestCompany
        # This should not prevent schema application
        company = TestCompany.get("company1")
        if company:
...
class TestLinkModelPerformance:
    ...
    def test_many_relationships_performance(self, setup_base_schema, relationship_models):
        ...
        retrieved_count = 0
        for emp in employees:
            if TestEmployee.get(emp.id):
                retrieved_count += 1
        
        assert retrieved_count == 100
    
    @pytest.mark.benchmark
    def test_link_model_creation_performance(self, setup_base_schema, relationship_models, benchmark):
        """Benchmark LinkModel creation performance."""
        TestCompany, TestEmployee, TestProject = relationship_models
        
        result = benchmark(setup_base_schema.apply_schema, [TestCompany, TestEmployee], dry_run=True)
        assert result is not None
...
class TestLinkModelIntegration:
    ...
    def test_links_with_queries(self, setup_base_schema, relationship_models):
        ...
        # Create test data
        companies = [
            TestCompany(id="c1", name="ACME"),
            TestCompany(id="c2", name="TechCorp")
        ]
        for company in companies:
            company.save()
        
        employees = [
            TestEmployee(id="e1", name="Alice"),
            TestEmployee(id="e2", name="Bob"),
            TestEmployee(id="e3", name="Charlie")
        ]
        for employee in employees:
            employee.save()
        
        # Query all objects
        all_companies = list(TestCompany.query())
        all_employees = list(TestEmployee.query())
