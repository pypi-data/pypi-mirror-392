...
def test_merge_entities_action(session: Session):
    ...
    print(f"DEBUG: ObjectType created")
    ...
    resources = session.exec(select(Resource).where(Resource.rid.in_([ot.rid for ot in existing_types]))).all()
    ...
    existing_types_after = session.exec(select(ObjectType).where(ObjectType.api_name == "employee")).all()
    print(f"DEBUG: Found {len(existing_types_after)} employee types in database after link creation")
    ...
    print(f"DEBUG: Test session ID: {id(session)}")
    print(f"DEBUG: MetamodelService repository session is same as test session: {id(mm.repository.session) == id(session)}")
    ...
    print(f"DEBUG: InstancesService metamodel repo session ID: {id(instances._domain.metamodel_repository.session)}")
    print(f"DEBUG: ActionsService session vs InstancesService metamodel repo session: {id(svc.session) == id(instances._domain.metamodel_repository.session)}")
    ...
    print(f"DEBUG: domain has instances_repository: {hasattr(instances._domain, 'instances_repository')}")
    print(f"DEBUG: Test session vs domain repo session: {id(session) == id(instances._domain.instances_repository._sql_repo.session)}")
    ...
    print(f"DEBUG: Test session vs domain metamodel repo session: {id(session) == id(instances._domain.metamodel_repository.session)}")
    instances.upsert_object(
        "employee",
        "emp1",
        body={"properties": {"name": "Alice", "email": "alice@example.com"}},
    )
    print(f"DEBUG: Created emp1")
    ...
    for obj in session.identity_map.values():
        ...
        print(f"DEBUG: emp1 NOT found in identity map")
    ...
    print(f"DEBUG: Session is active: {session.is_active}")
    try:
        print(f"DEBUG: Session state before emp2: new={len(session.new)}, dirty={len(session.dirty)}")
        print(f"DEBUG: Session is active before emp2: {session.is_active}")
        print(f"DEBUG: About to create emp2...")
        ...
        print(f"DEBUG: Created emp2")
        print(f"DEBUG: Session state after emp2: new={len(session.new)}, dirty={len(session.dirty)}")
        ...
        print(f"DEBUG: Session new contents after emp2:")
        for obj in session.new:
            print(f"  New obj: pk={obj.pk_value}")
        print(f"DEBUG: Session dirty contents after emp2:")
    ...
    try:
        ...
        print(f"DEBUG: Created colleague")
    except Exception as e:
    ...
    for obj in all_objs:
        print(f"  Object: pk={obj.pk_value}, has_service={hasattr(obj, 'service')}, has_instance={hasattr(obj, 'instance')}, valid_to={obj.valid_to}")
    ...
    source_objs = session.exec(select(ObjectInstance).where(
    ...
    target_objs = session.exec(select(ObjectInstance).where(
    ...
    for obj in target_objs:
        print(f"  Target: rid={obj.rid}, data={obj.data}, valid_from={obj.valid_from}, valid_to={obj.valid_to}")
