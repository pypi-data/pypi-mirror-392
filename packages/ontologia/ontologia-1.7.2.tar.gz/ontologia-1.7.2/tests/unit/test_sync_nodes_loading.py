...
class _StubDF:
    ...
    def rename(self, mapping):
        new_cols = [mapping.get(c, c) for c in self.columns]  # type: ignore[index]
        return _StubDF(new_cols)

    def select(self, cols):
        filtered_cols = [c for c in self.columns if c in cols]
        df = _StubDF(filtered_cols or self.columns)
        df.rows = [{col: row.get(col) for col in filtered_cols} for row in self.rows]  # type: ignore[index]
        return df
...
class _Unified:
    ...
    def columns(self):
        if not self._rows:
            return []
        return list(self._rows[0].keys())  # type: ignore[index]
...
def test_sync_nodes_load_merges_into_unified_table(client: TestClient, session):
    ...
    assert "employee" in merge_calls[-1]  # type: ignore[index]
