...
class TestObjectModelBasics:
    ...
    def test_model_creation(self, sample_models):
        ...
        TestUser, TestCompany = sample_models
        
        # Verify model registration
        assert TestUser._meta()['object_type_api_name'] == 'test_user'
        assert TestUser._meta()['primary_key'] == 'id'
        assert TestCompany._meta()['object_type_api_name'] == 'test_company'
        
        # Verify model fields
        assert 'id' in TestUser.model_fields
        assert 'name' in TestUser.model_fields
        assert 'email' in TestUser.model_fields
    
    def test_model_instantiation(self, sample_models):
        """Test model instance creation."""
        TestUser, _ = sample_models
        
        user = TestUser(id="user1", name="Alice", email="alice@example.com")
        assert user.id == "user1"
        assert user.name == "Alice"
        assert user.email == "alice@example.com"
        
        # Test optional field
        user_no_email = TestUser(id="user2", name="Bob")
    ...
    def test_model_validation(self, sample_models):
        ...
        # Valid model
        user = TestUser(id="valid", name="Valid User")
        assert user.id == "valid"
        
        # Required field missing should raise validation error
        with pytest.raises(ValueError):
            TestUser(name="Missing ID")  # type: ignore[call-arg]
...
class TestObjectModelCRUD:
    ...
    def test_save_and_get(self, ontology, sample_models):
        ...
        retrieved_user = TestUser.get("user1")
    ...
    def test_delete(self, ontology, sample_models):
        ...
        # Verify user exists
        assert TestUser.get("user1") is not None
    ...
    def test_delete_nonexistent(self, sample_models):
        ...
        user = TestUser(id="nonexistent", name="Ghost")
...
class TestObjectModelQuery:
    ...
    def test_query_all(self, ontology, sample_models):
        ...
        users = [
            TestUser(id="user1", name="Alice"),
            TestUser(id="user2", name="Bob"),
            TestUser(id="user3", name="Charlie")
        ]
        for user in users:
            user.save()
        
        # Query all users
        all_users = list(TestUser.query())
    ...
    def test_query_with_filter(self, ontology, sample_models):
        ...
        TestUser(id="user1", name="Alice", email="alice@example.com").save()
        TestUser(id="user2", name="Bob", email="bob@gmail.com").save()
        TestUser(id="user3", name="Charlie").save()

        # Query users with email
        users_with_email = list(TestUser.query().filter("email", "is_not_null", None))
        assert len(users_with_email) == 2
        assert all(user.email is not None for user in users_with_email)
        
        # Query specific email domain
        gmail_users = list(TestUser.query().filter("email", "like", "%@gmail.com"))
    ...
    def test_query_ordering(self, ontology, sample_models):
        ...
        TestUser(id="user1", name="Charlie").save()
        TestUser(id="user2", name="Alice").save()
        TestUser(id="user3", name="Bob").save()

        # Query ordered by name
        ordered_users = list(TestUser.query().order_by("name", "asc"))
        assert [user.name for user in ordered_users] == ["Alice", "Bob", "Charlie"]

        # Query ordered by name descending
        desc_users = list(TestUser.query().order_by("name", "desc"))
    ...
    def test_query_limit(self, ontology, sample_models):
        ...
        # Create multiple users
        for i in range(5):
            TestUser(id=f"user{i}", name=f"User{i}").save()
        
        # Query with limit
        limited_users = list(TestUser.query().limit(3))
...
class TestObjectModelTransactions:
    ...
    def test_transaction_commit(self, ontology, sample_models):
        ...
        with TestUser.transaction() as session:
            user1 = TestUser(id="user1", name="Alice")
            user2 = TestUser(id="user2", name="Bob")
        ...
        assert TestUser.get("user1") is not None
        assert TestUser.get("user2") is not None
    ...
    def test_transaction_rollback(self, ontology, sample_models):
        ...
        # Create initial user
        TestUser(id="existing", name="Existing").save()
        ...
        assert TestUser.get("existing") is not None
    
    def test_nested_transactions(self, ontology, sample_models):
        """Test nested transaction behavior."""
        TestUser, _ = sample_models
        
        with TestUser.transaction() as outer_session:
            outer_user = TestUser(id="outer", name="Outer")
            outer_user.save(session=outer_session)
            
            with TestUser.transaction() as inner_session:
                inner_user = TestUser(id="inner", name="Inner")
                inner_user.save(session=inner_session)
        
        # Verify both are committed
        assert TestUser.get("outer") is not None
        assert TestUser.get("inner") is not None
...
class TestObjectModelPerformance:
    ...
    def test_query_performance(self, ontology, sample_models, benchmark):
        ...
        # Benchmark query
        result = benchmark(list, TestUser.query())
