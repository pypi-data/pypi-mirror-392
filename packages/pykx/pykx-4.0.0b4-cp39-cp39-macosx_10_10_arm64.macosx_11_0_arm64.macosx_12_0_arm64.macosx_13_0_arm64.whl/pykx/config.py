...
def _get_qhome():
    ...
    if kx_path.exists() and kx_path.is_dir():
        ...
        os.environ['QPATH'] = str(kx_path/'mod' if qpath is None else qpath+':'+str(kx_path/'mod'))
...
def _license_install(intro=None, return_value=False, license_check=False, license_error=None): # noqa:
    ...
    if continue_license in ('n', 'N'):
        ...
        if existing_license in ('N', 'n', ''):
            ...
            if personal:
                redirect = input(f'\nTo apply for your KDB-X Python license, navigate to {lic_url}.'
                ...
                redirect = input('\nTo apply for your KDB-X Python license, contact your '
