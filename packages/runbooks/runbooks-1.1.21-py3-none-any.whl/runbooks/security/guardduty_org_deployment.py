...
class GuardDutyOrgDeployment:
    ...
    def check_guardduty_status(self, accounts: List[Dict[str, Any]]) -> pd.DataFrame:
                ...
                try:
                    ...
                    if detectors_response['DetectorIds']:
                        ...
                        status.data_sources_enabled = {
                            ...
                            'MalwareProtection': data_sources.get('MalwareProtection', {}).get('ScanEc2InstanceWithFindings', {}).get('EbsVolumes', {}).get('Status') == 'ENABLED',
                        }
                    else:
        ...
        summary_table.add_row("Enabled", f"[green]{enabled_count}[/green]", f"{enabled_count/len(accounts)*100:.1f}%")
        summary_table.add_row("Disabled", f"[red]{disabled_count}[/red]", f"{disabled_count/len(accounts)*100:.1f}%")
        summary_table.add_row("Error/Access Denied", f"[yellow]{error_count}[/yellow]", f"{error_count/len(accounts)*100:.1f}%")
        summary_table.add_row("Skipped (Inactive)", f"[dim]{skipped_count}[/dim]", f"{skipped_count/len(accounts)*100:.1f}%")
    ...
    def validate_deployment(self, status_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        validation_table.add_row(
            ...
            STATUS_INDICATORS['success'] if validation_results['validation_status'] == 'COMPLIANT' else STATUS_INDICATORS['warning']
        )
            ...
            with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
                ...
                for status in report.account_statuses:
                    accounts_data.append({
                        ...
                        'S3 Logs Enabled': status.data_sources_enabled.get('S3Logs', False) if status.data_sources_enabled else False,
                        'Kubernetes Enabled': status.data_sources_enabled.get('Kubernetes', False) if status.data_sources_enabled else False,
                        'Malware Protection': status.data_sources_enabled.get('MalwareProtection', False) if status.data_sources_enabled else False,
...
def print_deployment_plan(status_df: pd.DataFrame, delegated_admin: str, auto_enable: bool):
    ...
    plan_table.add_row(
        ...
        "1",
        f"Account: {delegated_admin}"
    )
    plan_table.add_row(
        "Already Enabled (No Action)",
        str(len(enabled_accounts)),
        "GuardDuty detectors already active"
    )
    plan_table.add_row(
        "Enable GuardDuty",
        f"[yellow]{len(disabled_accounts)}[/yellow]",
        "Create detectors with data sources"
    )
    plan_table.add_row(
