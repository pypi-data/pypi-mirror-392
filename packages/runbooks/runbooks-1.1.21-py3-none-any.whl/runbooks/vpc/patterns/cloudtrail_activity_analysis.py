...
class ActivityAnalysisResult:
    ...
    last_access_timestamps: Dict[str, Optional[datetime]] = field(
        default_factory=dict
    )  # resource_id → last access
    access_counts: Dict[str, int] = field(default_factory=dict)  # resource_id → count
    idle_classifications: Dict[str, bool] = field(
        default_factory=dict
    )  # resource_id → is_idle
...
class CloudTrailActivityAnalyzer(ABC):
    ...
    def analyze_cloudtrail_activity(
        ...
        print_info(
            f"🔍 CloudTrail profile: {management_profile} (source: {profile_source})"
        )
        ...
        if lookback_days > 90:
            print_warning(
                f"⚠️  Lookback window {lookback_days} days exceeds CloudTrail retention (90 days)"
            )
            lookback_days = 90
        ...
        with Progress(
            ...
            task = progress.add_task(
                "Analyzing CloudTrail events...", total=len(resources)
            )
            ...
            for profile, profile_resources in resources_by_profile.items():
                try:
                        ...
                        for resource in batch:
                            ...
                            try:
                                ...
                                if events:
                                    ...
                                    last_access = max(
                                        event["EventTime"] for event in events
                                    )
                                    ...
                                    days_since_access = (
                                        datetime.now(last_access.tzinfo) - last_access
                                    ).days
                                    idle_classifications[resource_id] = (
                                        days_since_access > idle_threshold_days
                                    )
                                else:
                                ...
                                errors.append(
                                    f"CloudTrail query failed for {resource_id}: {error_code}"
                                )
                                last_access_timestamps[resource_id] = None
                                access_counts[resource_id] = 0
                                ...
                                errors.append(
                                    f"Unexpected error analyzing {resource_id}: {str(e)}"
                                )
                                last_access_timestamps[resource_id] = None
                                access_counts[resource_id] = 0
                                idle_classifications[resource_id] = True  # Assume idle on error
                    ...
                    if error_code == "AccessDeniedException":
                        print_error(
                            f"❌ Access denied to CloudTrail API (profile: {profile})"
                        )
                        print_warning(
                            "⚠️  Ensure profile has cloudtrail:LookupEvents permission"
                        )
                    else:
        ...
        print_success(
            f"Activity: {active_resources} ACTIVE, {idle_resources} IDLE{error_note}"
        )
    ...
    def _display_activity_table(
        ...
        sorted_resources = sorted(
            resource_data, key=lambda x: x["days_idle"], reverse=True
        )
        ...
        print_info(
            f"💡 'Never (>{lookback_days}d)' = No CloudTrail events in {lookback_days}-day lookback window"
        )
