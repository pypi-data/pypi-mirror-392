...
class VPCEnrichmentResult:
    ...
    vpc_resources: Dict[str, Dict] = field(
        default_factory=dict
    )  # vpc_id → resource counts
...
class VPCEnricher(ABC):
    ...
    def enrich_with_vpc_api(
        ...
        for resource in resources:
            ...
            if not profile:
                errors.append(
                    f"Resource {vpce_id or 'unknown'} missing profile attribute"
                )
                continue
                    ...
                    try:
                        ...
                        for region, vpce_resources in regions.items():
                            try:
                                ...
                                vpce_ids = [getattr(r, "vpce_id", None) or getattr(r, "id", None) for r in vpce_resources]
                                ...
                                try:
                                    ...
                                    if batch_error.response['Error']['Code'] == 'InvalidVpcEndpointId.NotFound':
                                        print_warning(f"⚠️  Batch query failed (1+ deleted VPCEs), retrying {len(vpce_ids)} individually...")

                                        for vpce_id in vpce_ids:
                                            try:
                                                individual_response = ec2_client.describe_vpc_endpoints(VpcEndpointIds=[vpce_id])
                                                ...
                                                if individual_error.response['Error']['Code'] == 'InvalidVpcEndpointId.NotFound':
                                                    errors.append(f"VPCE {vpce_id} not found in AWS (deleted but still in CSV)")
                                                else:
                                                    errors.append(f"Error fetching {vpce_id}: {individual_error.response['Error']['Code']}")
                                            finally:
                    ...
                    try:
                        ...
                        for region, vpc_ids in regions.items():
                            try:
                                ...
                                errors.append(
                                    f"Failed to describe VPCs in {region} (profile: {profile}): {error_code}"
                                )
                                # Advance progress even on error
                                progress.advance(task, advance=len(vpc_ids))
                            except Exception as e:
                                errors.append(
                                    f"Unexpected error in {region} (profile: {profile}): {str(e)}"
                                )
                                # Advance progress even on error
                                progress.advance(task, advance=len(vpc_ids))
        ...
        enriched_count = sum(
            1
            for resource in resources
            if getattr(resource, "vpc_id", None) in vpc_names
        )
        ...
        if discrepancy_count > 0:
            ...
            print_success(
                f"✅ Enriched {enriched_count} resources from {len(vpc_names)} VPCs"
            )
        ...
        if duplicate_vpces:
            print_info(f"📋 CSV Metadata Reconciliation: {len(duplicate_vpces)} VPCEs with multiple vpc_name entries detected\n")
        ...
        if vpc_names:
            self._display_vpc_table(
                vpc_names, vpc_cidrs, vpc_resources, resources, enriched_count
            )
    ...
    def _get_vpc_resource_counts(
        self, ec2_client, vpc_id: str, errors: List[str]
    ) -> Dict:
        """
        ...
        try:
            ...
            resource_counts["route_tables"] = len(
                route_tables_response.get("RouteTables", [])
            )
            ...
            resource_counts["internet_gateways"] = len(
                igws_response.get("InternetGateways", [])
            )
            ...
            resource_counts["nat_gateways"] = len(
                nat_gws_response.get("NatGateways", [])
            )

        except ClientError as e:
            errors.append(
                f"Failed to get resource counts for {vpc_id}: {e.response['Error']['Code']}"
            )
        except Exception as e:
            errors.append(
                f"Unexpected error getting resource counts for {vpc_id}: {str(e)}"
            )
