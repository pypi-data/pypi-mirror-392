...
def calculate_executive_metrics(scored_data: Dict[str, pd.DataFrame]) -> Dict:
    ...
    savings_percentage = (
        (annual_savings / annual_current * 100) if annual_current > 0 else 0.0
    )

    # Estimate implementation cost
    # Rule of thumb: $50K for projects >$200K annual savings, 25% of savings otherwise
    implementation_cost = (
        50000.0 if annual_savings > 200000 else annual_savings * 0.25
    )
    ...
    payback_months = (
        implementation_cost / total_savings_opportunity
        if total_savings_opportunity > 0
        else float('inf')
    )
...
def display_quarterly_table(quarterly: Dict) -> None:
    ...
    """
    table = Table(
        title="Quarterly Financial Projections (Year 1)",
        show_header=True,
        header_style="bold cyan"
    )
    ...
    for quarter, data in quarterly.items():
        table.add_row(
            quarter,
            data['phase'],
            format_cost(data['savings']),
            format_cost(data['cumulative']),
            data['realization']
        )
...
def display_detailed_quarterly_table(quarterly: Dict) -> None:
    ...
    """
    table = Table(
        title="Detailed Quarterly Implementation Plan",
        show_header=True,
        header_style="bold cyan"
    )
