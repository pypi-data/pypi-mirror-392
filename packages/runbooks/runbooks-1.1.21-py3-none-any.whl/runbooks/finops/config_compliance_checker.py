...
class ConfigComplianceChecker:
    ...
    def __init__(
        ...
        logger.info(
            f"AWS Config Compliance Checker initialized (profile={profile_name}, regions={len(self.regions)})"
        )
    ...
    async def _check_ebs_volume_inuse_compliance(
        self, region: str
    ) -> List[ConfigComplianceViolation]:
        """
            ...
            try:
                ...
                for rule_compliance in compliance_response.get("ComplianceByConfigRules", []):
                    ...
                    for eval_result in eval_results.get("EvaluationResults", []):
                        resource_id = eval_result["EvaluationResultIdentifier"]["EvaluationResultQualifier"].get("ResourceId")
    ...
    async def _check_eip_attached_compliance(
        self, region: str
    ) -> List[ConfigComplianceViolation]:
        """
    ...
    async def check_config_compliance(
        ...
        print_header(
            "AWS Config FinOps Integration", "Compliance-Driven Cost Optimization"
        )
        ...
        with create_progress_bar() as progress:
            task = progress.add_task(
                "[cyan]Checking Config compliance...", total=len(self.regions)
            )
...
• Execution Time: {results.execution_time_seconds:.2f}s
        ...
        if results.total_violations_detected > 0:
            ...
            sorted_violations = sorted(
                results.violations, key=lambda x: x.annual_cost_impact, reverse=True
            )[:20]
