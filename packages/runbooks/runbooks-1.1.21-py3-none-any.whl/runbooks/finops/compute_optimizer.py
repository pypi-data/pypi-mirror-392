        ...
        with create_progress_bar() as progress:
            ...
            while True:
                ...
                for rec in recommendations:
                    ...
                    # Apply idle threshold: Only include instances with CPU ≤1% (AWS standard)
                    if cpu_max > IDLE_CPU_THRESHOLD:
                        # Skip non-idle instances (optimized/overprovisioned/etc don't qualify as idle)
                        continue

                    # Override finding to 'Idle' (AWS doesn't have "Idle" value, they use "Optimized" etc)
                    finding = 'Idle'  # Standardize finding for <1% CPU instances

                    # Build recommendation record
                    idle_instances[instance_id] = {
                        'score': 60,  # Signal E1 weight
                        'finding': finding,  # Now always 'Idle' for qualifying instances
