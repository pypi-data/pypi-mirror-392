...
class ECSIdleSignal(str, Enum):
    """ECS idle/underutilization decommission signals (C1-C5)."""
...
class ECSActivityEnricher:
    ...
    def __init__(
        ...
        if self.output_controller.verbose:
            print_info(f"🔍 ECS Activity Enricher initialized: profile={self.operational_profile}, region={self.region}, lookback={self.lookback_days}d")
        else:
            self.logger.debug(f"ECS Activity Enricher initialized: profile={self.operational_profile}, region={self.region}, lookback={self.lookback_days}d")
    ...
    def analyze_cluster_activity(
        ...
        print_section(f"ECS Activity Analysis ({lookback}-day lookback)")
        ...
        with create_progress_bar(description="Analyzing ECS clusters") as progress:
            ...
            for cluster in clusters:
                try:
                    ...
                    self.logger.error(
                        f"Failed to analyze ECS cluster {cluster.get('clusterName')}: {e}",
                        exc_info=True
                    )
    ...
    def _get_ecs_clusters(
        self,
        cluster_arns: Optional[List[str]],
        region: str
    ) -> List[Dict]:
        """
    ...
    def _generate_idle_signals(
        ...
        if metrics.avg_cpu_utilization_90d < self.IDLE_UTILIZATION_THRESHOLD and \
           metrics.avg_memory_utilization_90d < self.IDLE_UTILIZATION_THRESHOLD:
    ...
    def display_analysis(self, analyses: List[ECSActivityAnalysis]) -> None:
        """
        ...
        table = create_table(
            ...
            columns=[
                {"name": "Cluster Name", "style": "cyan"},
                {"name": "CPU Util %", "style": "bright_yellow"},
                {"name": "Mem Util %", "style": "bright_yellow"},
                {"name": "Avg Tasks", "style": "white"},
    ...
    def _display_summary(self, analyses: List[ECSActivityAnalysis]) -> None:
        ...
        summary_text = (
            f"[bold]Total Clusters: {total}[/bold]\n"
            f"[bold green]Total Potential Savings: {format_cost(total_savings)}/year[/bold green]\n"
            f"[bold]Total Annual ECS Cost: {format_cost(total_cost)}[/bold]\n\n"
            f"Recommendations:\n"
        ...
        summary = create_panel(
            summary_text,
            title="ECS Idle Detection Summary",
            border_style="green"
        )
