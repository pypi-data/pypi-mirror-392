...
def display_ec2_cost_tree(
    ec2_data: List[Dict], console: Optional[Console] = None, top_n_per_region: int = 5
) -> None:
    """
    ...
    for account_id, account_data in sorted_accounts:
        ...
        account_node = root.add(
            f"🏢 Account: [yellow]{account_name}[/yellow] ({account_id}) | "
            f"[green]${account_cost:,.2f}/mo[/green]"
        )

        # Sort regions by cost (descending)
        regions = account_data["regions"]
        region_costs = {
            region: sum([inst.get("monthly_cost", 0) for inst in instances])
...
def display_cost_comparison_table(
    ec2_costs: Dict, workspaces_costs: Dict, console: Optional[Console] = None
) -> None:
    """
