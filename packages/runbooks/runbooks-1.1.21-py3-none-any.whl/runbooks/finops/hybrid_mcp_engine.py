    ...
    async def _query_mcp_cost(
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                self.logger.warning(
                    f"MCP query attempt {attempt}/{self.max_retries} failed for {resource_id}: {e}"
                )
    ...
    async def validate_batch(
        ...
        with create_progress_bar(description="Validating resources") as progress:
            task = progress.add_task(
                f"Validating {len(resources)} resources",
                total=len(resources)
            )
            ...
            validation_tasks = [
                validate_with_semaphore(resource)
        ...
        valid_results = [
            ...
            if isinstance(r, MCPValidationResult)
        ]
