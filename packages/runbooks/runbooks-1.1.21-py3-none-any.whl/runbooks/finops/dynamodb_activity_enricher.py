...
class DynamoDBIdleSignal(str, Enum):
    """DynamoDB idle/underutilization decommission signals (D1-D5)."""
...
# DynamoDB signal weights (0-100 scale, total: 100 points max)
DEFAULT_DYNAMODB_WEIGHTS = {
    'D1': 40,  # Low capacity utilization <5% (highest weight - capacity waste)
    'D2': 20,  # Idle Global Secondary Indexes
    'D3': 15,  # No Point-in-Time Recovery enabled
    'D4': 10,  # No DynamoDB Streams activity
    'D5': 15   # Low cost efficiency (provisioned + low usage)
}
...
class DynamoDBActivityEnricher:
    ...
    def __init__(
        ...
        if self.output_controller.verbose:
            print_info(f"🔍 DynamoDB Activity Enricher initialized: profile={self.operational_profile}, region={self.region}, lookback={self.lookback_days}d")
        else:
            self.logger.debug(f"DynamoDB Activity Enricher initialized: profile={self.operational_profile}, region={self.region}, lookback={self.lookback_days}d")
        ...
        with create_progress_bar(description="Analyzing DynamoDB tables") as progress:
            ...
            for table in tables:
                try:
                    ...
                    self.logger.error(
                        f"Failed to analyze DynamoDB table {table.get('TableName')}: {e}",
                        exc_info=True
                    )
    ...
    def _get_cloudwatch_metrics(
        ...
        read_30d = self._get_metric_average(
            table_name, 'ConsumedReadCapacityUnits',
            now - timedelta(days=30), now
        )
        read_60d = self._get_metric_average(
            table_name, 'ConsumedReadCapacityUnits',
            now - timedelta(days=60), now
        )

        # Query ConsumedWriteCapacityUnits
        write_30d = self._get_metric_average(
            table_name, 'ConsumedWriteCapacityUnits',
            now - timedelta(days=30), now
        )
        write_60d = self._get_metric_average(
            table_name, 'ConsumedWriteCapacityUnits',
            now - timedelta(days=60), now
        )
    ...
    def _count_idle_gsis(self, table: Dict, now: datetime) -> int:
        ...
        for gsi in gsis:
            ...
            consumed_read = self._get_gsi_metric_average(
                table_name, gsi_name, 'ConsumedReadCapacityUnits',
                now - timedelta(days=30), now
            )
            consumed_write = self._get_gsi_metric_average(
                table_name, gsi_name, 'ConsumedWriteCapacityUnits',
                now - timedelta(days=30), now
            )
    ...
    def _check_pitr_status(self, table_name: str) -> bool:
        ...
        try:
            ...
            status = response.get('ContinuousBackupsDescription', {}).get('PointInTimeRecoveryDescription', {}).get('PointInTimeRecoveryStatus')
    ...
    def _generate_idle_signals(
        ...
        """
        Generate D1-D5 idle signals and calculate decommission score.

        Signal generation rules with 0-100 scoring:
        - D1: read/write utilization < 5% → 40 points (HIGH confidence: 0.90)
        ...
        - D5: Low cost efficiency → 15 points (MEDIUM confidence: 0.70)
        ...
        Returns:
            Tuple of (signals list, decommission score 0-100)
        """
        signals: List[DynamoDBIdleSignal] = []
        score = 0

        # D1: Low capacity utilization (<5%) - 40 points
        if metrics.read_utilization_pct < self.IDLE_CAPACITY_THRESHOLD and \
           metrics.write_utilization_pct < self.IDLE_CAPACITY_THRESHOLD:
        ...
        # D5: Low cost efficiency (provisioned but low usage) - 15 points
    ...
    def display_analysis(self, analyses: List[DynamoDBActivityAnalysis]) -> None:
        ...
        console.print("├── Signal Legend: D1:Capacity <5% (40pts) | D2:Idle GSIs (20pts) | D3:No PITR (15pts)")
        console.print("│   D4:No Streams (10pts) | D5:Low cost efficiency (15pts)")
        console.print()
...
def create_dynamodb_activity_enricher(
    ...
    Example:
        ...
        >>> analyses = enricher.analyze_table_activity(...)
    """
    return DynamoDBActivityEnricher(
