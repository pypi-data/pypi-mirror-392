...
def calculate_roi_metrics(
    ...
    break_even_month = (
        int(payback_months) + 1 if payback_months != float('inf') else None
    )
...
def display_roi_table(scenario: Dict, roi_metrics: Dict) -> None:
    ...
    """
    table = Table(
        title=f"ROI Analysis - {scenario['name']} Scenario",
        show_header=True,
...
def compare_scenarios(scenarios: List[Dict]) -> None:
    ...
    recommendation_table = Table(
        title="Scenario Recommendations by Risk Tolerance",
        show_header=True,
        header_style="bold cyan"
    )
    ...
    recommendation_table.add_row(
        "Risk-Averse",
        "Conservative",
        "High confidence (95%), minimal disruption, quick wins"
    )

    recommendation_table.add_row(
        "Balanced",
        "Moderate",
        "Optimal risk/reward (80% confidence), phased approach"
    )

    recommendation_table.add_row(
        "Growth-Focused",
        "Aggressive",
        "Maximum savings potential with thorough validation"
    )
