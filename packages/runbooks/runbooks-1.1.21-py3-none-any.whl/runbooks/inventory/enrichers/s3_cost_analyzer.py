...
class S3StorageBreakdown:
    ...
    def __post_init__(self):
        ...
        self.total_gb = (
            self.standard + self.standard_ia + self.one_zone_ia +
            self.intelligent_tiering + self.glacier_ir +
            self.glacier + self.glacier_deep_archive
        )
...
class S3CostAnalyzer(CloudFoundationsBase):
    ...
    def run(self):
        ...
        raise NotImplementedError(
            "S3CostAnalyzer is a stateless enrichment utility. "
            "Use analyze_bucket_costs() method directly."
        )
    ...
    def _calculate_potential_savings(
        ...
        storage: S3StorageBreakdown,
        access_pattern: AccessPattern,
        signals: List[StorageOptimizationSignal]
    ) -> float:
        """
        ...
        if StorageOptimizationSignal.S2_INTELLIGENT_TIERING in signals:
            ...
            monthly_savings = it_savings_gb * (
                self.STORAGE_PRICING['STANDARD'] - self.STORAGE_PRICING['STANDARD_IA']
            )
        ...
        if StorageOptimizationSignal.S3_GLACIER_CANDIDATE in signals:
            ...
            monthly_savings = glacier_savings_gb * (
                self.STORAGE_PRICING['STANDARD'] - self.STORAGE_PRICING['GLACIER']
            )
            savings += monthly_savings * 12
        ...
        if StorageOptimizationSignal.S4_DEEP_ARCHIVE_CANDIDATE in signals:
            ...
            monthly_savings = da_savings_gb * (
                self.STORAGE_PRICING['STANDARD'] - self.STORAGE_PRICING['DEEP_ARCHIVE']
            )
            savings += monthly_savings * 12
