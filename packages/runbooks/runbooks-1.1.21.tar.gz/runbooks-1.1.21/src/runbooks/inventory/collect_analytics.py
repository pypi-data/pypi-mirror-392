...
def collect_analytics(
    ...
    try:
        ...
        if output or format != "table":
            output_path = output or f"data/outputs/analytics-discovered-{datetime.now().strftime('%Y%m%d-%H%M%S')}.{format}"
...
def _export_json(resources: List, output_path: str):
    ...
    with open(output_path, "w") as jsonfile:
        json.dump(
            {"discovery_timestamp": datetime.now().isoformat(), "total_resources": len(resources_dict), "resources": resources_dict},
            jsonfile,
            indent=2,
        )
