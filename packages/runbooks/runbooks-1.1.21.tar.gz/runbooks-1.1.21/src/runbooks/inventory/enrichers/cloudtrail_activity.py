...
class CloudTrailActivityEnricher:
    ...
    def analyze_resource_activity(
        ...
        with create_progress_bar(description="Analyzing CloudTrail events") as progress:
            task = progress.add_task(
                f"Analyzing {len(resource_ids)} resources",
            ...
            for resource_id in resource_ids:
                try:
                    ...
                    events_90d = self._query_cloudtrail_events(
                        resource_id,
                        start_time_90d,
                        end_time
                    )
                    ...
                    events_60d = [
                        ...
                        if e['EventTime'] >= start_time_60d
                    ]
                    events_30d = [
                        e for e in events_90d
                        if e['EventTime'] >= start_time_30d
                    ]

                    # Classify operations
                    classified_events = [
                        self._classify_operation(event)
                        for event in events_90d
                    ]
                    ...
                    read_only_pct = (
                        (read_only_count / total_events * 100)
                    ...
                    # Find last activity
                    last_activity = (
                        max(e['EventTime'] for e in events_90d)
                    ...
                    self.logger.error(
                        f"Failed to analyze CloudTrail activity for {resource_id}: {e}",
                        exc_info=True
                    )
    ...
    def _query_cloudtrail_events(
        ...
        try:
            ...
            if error_code == 'AccessDeniedException':
                self.logger.warning(
                    f"CloudTrail access denied for {resource_id} - check IAM permissions"
                )
            elif error_code == 'InvalidTimeRangeException':
                self.logger.error(
                    f"Invalid time range for CloudTrail query: {start_time} to {end_time}"
                )
            else:
    ...
    def _classify_operation(self, event: Dict) -> Dict[str, Any]:
        ...
        is_user_initiated = any(
            principal in username
            for principal in self.USER_INITIATED_PRINCIPALS
        )
        is_automated = any(
            principal in username
            for principal in self.AUTOMATED_PRINCIPALS
        )

        # Classify operation type
        is_read_only = any(
            event_name.startswith(pattern.rstrip('*'))
            for pattern in self.READ_ONLY_OPERATIONS
        )
        is_management = any(
            event_name.startswith(pattern.rstrip('*'))
            for pattern in self.MANAGEMENT_OPERATIONS
        )
    ...
    def _generate_signals(self, analysis_data: Dict) -> List[ActivitySignal]:
        ...
        mgmt_ops_60d = sum(
            1 for e in classified_events
            if e['is_management'] and e['event_name'] in [
                ev.get('EventName', '')
                for ev in analysis_data.get('events_60d', [])
            ]
        )
        ...
        user_ops_30d = sum(
            1 for e in classified_events
            if e['is_user_initiated'] and e['event_name'] in [
                ev.get('EventName', '')
                for ev in analysis_data.get('events_30d', [])
            ]
        )
    ...
    def _calculate_confidence(self, signals: List[ActivitySignal]) -> float:
        ...
        total_confidence = sum(
            signal_confidence.get(signal, 0.0)
    ...
    def display_analysis(self, analyses: List[CloudTrailActivityAnalysis]) -> None:
        ...
        for analysis in analyses:
            ...
            last_activity_str = (
                analysis.last_activity.strftime('%Y-%m-%d')
                if analysis.last_activity else 'Never'
            )
    ...
    def _display_summary(self, analyses: List[CloudTrailActivityAnalysis]) -> None:
        ...
        summary_table.add_row(
            ...
            f"[bright_red]{decommission_count}[/bright_red]"
        )
        summary_table.add_row(
            "Needs Investigation",
            f"[bright_yellow]{investigate_count}[/bright_yellow]"
        )
        summary_table.add_row(
            "Keep Resources",
            f"[bright_green]{keep_count}[/bright_green]"
        )
