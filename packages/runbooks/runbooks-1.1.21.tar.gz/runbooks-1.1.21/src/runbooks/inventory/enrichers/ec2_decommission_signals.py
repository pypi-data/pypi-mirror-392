...
class EC2DecommissionSignalEnricher(CloudFoundationsBase):
    ...
    def _check_network_throughput(
        ...
        try:
            ...
            network_in_avg_mbps = (sum(network_in_bytes) / len(network_in_bytes) / 1024 / 1024 * 8) if network_in_bytes else 0
            network_out_avg_mbps = (sum(network_out_bytes) / len(network_out_bytes) / 1024 / 1024 * 8) if network_out_bytes else 0
    ...
    def _check_load_balancer(
        ...
        try:
            ...
            return {
                'score': 0,
                'attached_to_elb': False,
                'target_groups': [],
                'triggered': False,
                'error': str(e)
            }

    def _check_instance_age(
        self,
        launch_time: str
    ) -> Dict:
        """
    ...
    def _assess_graviton_eligibility(
        self,
        row: pd.Series,
        signals: Dict
    ) -> Dict:
    ...
    def enrich_instances(
        ...
        with create_progress_bar() as progress:
            task = progress.add_task(
                "[cyan]Calculating E2-E7 signals...",
                total=len(df)
            )
    ...
    def _display_signal_summary(self, df: pd.DataFrame) -> None:
        ...
        try:
            ...
            for signal_id in ['E2', 'E3', 'E4', 'E5', 'E6', 'E7']:
                ...
                signal_stats.append([
                    signal_id,
                    str(self.SIGNAL_WEIGHTS[signal_id]),
                    str(triggered_count),
                    f"{triggered_pct:.1f}%"
                ])

            table = create_table(
                "E2-E7 Signal Summary",
                ["Signal", "Weight", "Triggered", "Percentage"],
                signal_stats
            )
            console.print(table)
