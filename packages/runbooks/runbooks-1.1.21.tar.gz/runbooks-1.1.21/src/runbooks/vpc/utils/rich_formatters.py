...
class VPCTableFormatter:
    ...
    def create_decision_matrix_table(self, analyses: List[VPCAnalysis]) -> Table:
        ...
        sorted_analyses = sorted(
            analyses,
            key=lambda a: a.cost_breakdown.total_monthly_cost,
            reverse=True
        )
    ...
    def print_summary_statistics(self, analyses: List[VPCAnalysis]) -> None:
        ...
        console.print(f"MUST DELETE: {len(must_delete)} VPCs ({format_cost(float(sum(a.cost_breakdown.total_monthly_cost for a in must_delete)))})")
        console.print(f"COULD DELETE: {len(could_delete)} VPCs ({format_cost(float(sum(a.cost_breakdown.total_monthly_cost for a in could_delete)))})")
        console.print(f"SHOULD NOT DELETE: {len(should_not)} VPCs ({format_cost(float(sum(a.cost_breakdown.total_monthly_cost for a in should_not)))})")
