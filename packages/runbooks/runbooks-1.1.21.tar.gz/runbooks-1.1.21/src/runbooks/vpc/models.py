...
class VPCCostBreakdown(BaseModel):
    ...
    nat_gateway_cost: Decimal = Field(
        ...
        description="NAT Gateway monthly cost ($32.85 per gateway)"
    )
    vpce_interface_cost: Decimal = Field(
        default=Decimal("0.00"),
        description="Interface VPCE monthly cost ($7.30 per endpoint)"
    )
    vpce_gateway_cost: Decimal = Field(
        default=Decimal("0.00"),
        description="Gateway VPCE monthly cost (always $0.00)"
    )
