    ...
    try:
        ...
        if output_format == 'table':
            table = create_table(
                ...
                columns=[
                    ["Subnet ID", "Name", "CIDR", "AZ", "Available IPs", "State"],
                    subnets_data
                ]
            )
            console.print(table)
    ...
    try:
        ...
        if output_format == 'table':
            table = create_table(
                ...
                columns=[
                    ["ENI ID", "Name", "VPC ID", "Private IP", "Status", "Region"],
                    idle_enis_data
                ]
            )
            console.print(table)
    ...
    try:
        ...
        if output_format == 'table':
            table = create_table(
                ...
                columns=[
                    ["NACL ID", "Name", "Default", "Entries", "Associations", "Status"],
                    nacl_data
                ]
            )
            console.print(table)
...
def flow_log_check(profile, vpc_id, region, output_format, output_file):
    ...
    try:
        ...
        response = ec2.describe_flow_logs(Filters=[
        ...
        if response['FlowLogs']:
            ...
            if output_format == 'table':
                table = create_table(
                    ...
                    columns=[
                        ["Flow Log ID", "Status", "Traffic Type", "Log Destination", "Created"],
                        flow_logs_data
                    ]
                )
                console.print(table)
...
def delete_unused_enis(profile, regions, dry_run, output_format, output_file):
    ...
    try:
        ...
        if output_format == 'table':
            if unused_enis:
                table = create_table(
                    title="Unused ENI Cleanup",
                    columns=[
                        ["ENI ID", "Name", "VPC ID", "Region", "Action"],
                        unused_enis
                    ]
                )
                console.print(table)
        ...
        if output_format == 'table':
            if unassociated_eips:
                table = create_table(
                    ...
                    columns=[
                        ["Allocation ID", "Public IP", "Monthly Cost", "Region", "Action"],
                        unassociated_eips
                    ]
                )
                console.print(table)
