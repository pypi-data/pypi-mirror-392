...
class CostExplorerEnricher(ABC):
    ...
    def _resolve_billing_profile(self, billing_profile: Optional[str] = None) -> str:
        ...
        if billing_profile is None:
            billing_profile = (
                ...
                os.getenv("AWS_BILLING_PROFILE") or
                VPCE_BILLING_PROFILE
            )
    ...
    def _get_ce_client(self, billing_profile: str):
        ...
        if not validation["valid"]:
            raise ValueError(
                f"Profile validation failed for '{billing_profile}': "
    ...
    def get_cost_by_period(
        ...
        **Alternative for VPC/VPCE Filtering**:
        ...
        try:
            ...
            if not usage_types:
                print_warning(
                    f"⚠️  No {usage_type_filter} USAGE_TYPE values found in Cost Explorer"
                )
                result = {
                    "period_total": 0.0,
                    "account_costs": {},
            ...
            account_scope = f"{len(account_ids)} account" + ("s" if len(account_ids) != 1 else "") if account_ids else "all accounts"
            print_info(f"📊 Cost Analysis Scope: {account_scope}, {len(usage_types)} {usage_type_filter} billing type" + ("s" if len(usage_types) != 1 else ""))
    ...
    def enrich_with_cost_explorer(
        ...
        **DEPRECATED**: Use get_cost_by_period(enrich_resources=True) instead.
        ...
        try:
                ...
                for result in response["ResultsByTime"]:
                    for group in result.get("Groups", []):
                        ...
                        if "VpcEndpoint" in usage_type:
                            cost = float(group["Metrics"]["UnblendedCost"]["Amount"])
                            last_month_costs_by_account[account_id] = last_month_costs_by_account.get(account_id, 0.0) + cost
            ...
            resources_by_account = self._get_resources_by_account()
            total_resources = sum(
                summary.endpoint_count for summary in resources_by_account.values()
            )
            ...
            for account_id, summary in resources_by_account.items():
                if account_id in last_month_costs_by_account:
                    ...
                    if hasattr(self, "_trailing_12_month_data") and self._trailing_12_month_data.get(
                        ...
                        actual_account_annual = self._trailing_12_month_data[
                            "account_annual_costs"
                        ].get(
                            account_id,
                    ...
                    print_warning(
                        f"⚠️  No {period['month_year']} billing data for account {account_id}, "
                        f"using calculated costs"
                    )
