...
class VPCOptimizationSettings(BaseSettings):
    ...
    aws_regions: List[str] = Field(
        default=["ap-southeast-2"],
        description="List of AWS regions for multi-region operations"
    )
    ...
    aws_billing_profile: Optional[str] = Field(
        default=None,
        description="AWS profile for billing/Cost Explorer access",
        validation_alias="AWS_BILLING_PROFILE"
    )
    aws_management_profile: Optional[str] = Field(
        default=None,
        description="AWS management account profile",
        validation_alias="AWS_MANAGEMENT_PROFILE"
    )
    aws_centralised_ops_profile: Optional[str] = Field(
        default=None,
        description="AWS centralized ops account profile",
        validation_alias="AWS_CENTRALISED_OPS_PROFILE"
    )
    ...
    nat_gateway_reduction_target: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="NAT Gateway reduction target (30% = 0.30)"
    )
    vpc_endpoint_reduction_target: float = Field(
        default=0.30,
        ge=0.0,
        le=1.0,
        description="VPC Endpoint reduction target (30% = 0.30)"
    )
    transit_gateway_reduction_target: float = Field(
        default=0.30,
        ge=0.0,
        le=1.0,
        description="Transit Gateway reduction target (30% = 0.30)"
    )
    cost_reduction_target: float = Field(
        default=0.30,
        ge=0.0,
        le=1.0,
        description="Overall cost reduction target (30% = 0.30)"
    )

    # Discovery Configuration
    auto_discover_resources: bool = Field(
        default=True,
        description="Enable AWS API resource discovery (no hardcoded counts)"
    )
    auto_discover_costs: bool = Field(
        default=True,
        description="Enable Cost Explorer cost discovery (no hardcoded pricing)"
    )
    ...
    default_analysis_days: int = Field(
        ...
        ge=1,
        le=90,
        description="Days for usage analysis"
    )
    forecast_days: int = Field(
        default=30,
        ge=1,
        le=365,
        description="Days for cost forecast"
    )
    ...
    enable_cost_approval_workflow: bool = Field(
        default=False,
        description="Enable enterprise cost approval workflow"
    )
    enable_mcp_validation: bool = Field(
        default=False,
        description="Enable MCP cross-validation"
    )
    cost_approval_threshold: float = Field(
        default=500.0,
        ge=0.0,
        description="Dollar threshold requiring approval"
    )
