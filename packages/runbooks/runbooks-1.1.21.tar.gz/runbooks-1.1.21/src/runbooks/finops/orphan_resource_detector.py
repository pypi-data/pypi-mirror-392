...
class OrphanResourceDetector:
    ...
    def __init__(
        ...
        logger.info(
            f"Orphan Resource Detector initialized (profile={profile_name}, regions={len(self.regions)})"
        )
    ...
    async def _detect_orphaned_ebs_volumes(
        self, region: str
    ) -> List[OrphanResourceMetrics]:
        """
    ...
    async def _detect_orphaned_elastic_ips(
        self, region: str
    ) -> List[OrphanResourceMetrics]:
        """
    ...
    async def _detect_orphaned_nat_gateways(
        self, region: str
    ) -> List[OrphanResourceMetrics]:
        """
    ...
    async def detect_orphaned_resources(
        ...
        with create_progress_bar() as progress:
            task = progress.add_task(
                "[cyan]Detecting orphaned resources...", total=len(self.regions)
            )
...
def detect_orphans(
    profile: str, regions: Tuple[str], resource_type: str, validate_with_config: bool
):
    """
