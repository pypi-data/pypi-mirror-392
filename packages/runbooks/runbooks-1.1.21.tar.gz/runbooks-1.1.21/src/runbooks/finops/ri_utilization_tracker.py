...
class RIUtilizationTracker:
    ...
    def track_utilization(self) -> List[RIUtilizationReport]:
        ...
        with create_progress_bar() as progress:
            task = progress.add_task(
                f"[cyan]Tracking RI utilization across {len(services)} services...",
                total=len(services)
            )
    ...
    def _track_service_utilization(
        self,
        service: str
    ) -> List[RIUtilizationReport]:
        """
        ...
        try:
            ...
            response = self.ce_client.get_reservation_utilization(
                ...
                Filter={
                    'Dimensions': {
                        'Key': 'SERVICE',
                        'Values': [self._get_service_name(service)]
                    }
                },
                GroupBy=[
            ...
            for time_period in response.get('UtilizationsByTime', []):
                for group in time_period.get('Groups', []):
                    report = self._parse_utilization_data(
                        service,
                        group,
                        time_period
                    )
    ...
    def generate_alerts(
        self,
        utilization_data: List[RIUtilizationReport]
    ) -> List[Alert]:
        """
    ...
    def _print_utilization_summary(
        self,
        reports: List[RIUtilizationReport]
    ) -> None:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Track Reserved Instance utilization with waste detection"
    )
    parser.add_argument(
        '--profile',
        required=True,
    ...
    parser.add_argument(
        ...
        type=float,
        default=90.0,
        help='Utilization threshold for alerts (default: 90%%)'
    )
    parser.add_argument(
        '--lookback-days',
        type=int,
