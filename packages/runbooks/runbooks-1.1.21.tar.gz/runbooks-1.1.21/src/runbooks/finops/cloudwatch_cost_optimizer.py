...
class CloudWatchCostOptimizer:
    ...
    async def _analyze_log_group(
        self, log_group_name: str, region: str, logs_client: Any
        ...
        try:
            ...
            recommended_retention = self._recommend_retention_policy(
                log_group_name, tags, retention_days
            )
    ...
    async def analyze_cloudwatch_costs(
        self, enable_mcp_validation: bool = False
    ) -> CloudWatchCostOptimizerResults:
        """
        ...
        with create_progress_bar() as progress:
            task = progress.add_task(
                "[cyan]Analyzing CloudWatch Log Groups...", total=len(self.regions)
            )
                ...
                try:
                    ...
                    for page in page_iterator:
                        for log_group in page.get("logGroups", []):
                            ...
                            metrics = await self._analyze_log_group(
                                log_group_name, region, logs_client
                            )
                    ...
                    print_success(
                        f"✓ {region}: {len(region_log_groups)} log groups analyzed"
                    )
        ...
        log_groups_with_infinite_retention = sum(
            1 for lg in all_log_group_metrics if lg.retention_in_days is None
        )
        log_groups_optimizable = sum(
            1 for lg in all_log_group_metrics if lg.cost_savings_annual > 0
        )
...
• Analysis Timestamp: {results.analysis_timestamp.strftime('%Y-%m-%d %H:%M:%S')}
        ...
        if results.log_groups_optimizable > 0:
            ...
            for lg in optimizable_log_groups:
                current_retention = (
                    f"{lg.retention_in_days}d" if lg.retention_in_days else "Infinite"
                )
