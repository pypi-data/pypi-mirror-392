...
class GravitonMigrationAnalyzer:
    ...
    def __init__(self, config: GravitonAnalysisConfig):
        ...
        logger.debug(f"Graviton analyzer initialized with profiles: "
    ...
    def _map_instance_types(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        mapping_table.add_row("Graviton Compatible", str(mapped_count),
                             f"{(mapped_count/len(df)*100):.1f}%")
        mapping_table.add_row("No Graviton Equivalent", str(unmapped_count),
                             f"{(unmapped_count/len(df)*100):.1f}%")
    ...
    def _check_ami_compatibility(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        with create_progress_bar() as progress:
            task = progress.add_task(
                "[cyan]Checking AMI architecture compatibility...",
    ...
    def _calculate_savings(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df.loc[eligible_mask, 'graviton_annual_savings'] = (
            df.loc[eligible_mask, 'graviton_monthly_savings'] * 12
        )
        df.loc[eligible_mask, 'graviton_monthly_cost'] = (
            df.loc[eligible_mask, 'monthly_cost'] * (1 - GRAVITON_COST_SAVINGS_PERCENT)
        )
    ...
    def _calculate_eligibility_scores(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        medium_score = ((df['graviton_eligibility_score'] >= 40) &
                       (df['graviton_eligibility_score'] < 70)).sum()
    ...
    def _generate_recommendations(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if recommend_count > 0:
            print_success(f"✅ {recommend_count} instances ready for Graviton migration (${recommend_savings:,.2f} annual)")
