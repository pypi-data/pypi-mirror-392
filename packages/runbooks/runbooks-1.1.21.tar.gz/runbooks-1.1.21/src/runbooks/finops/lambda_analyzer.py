...
class LambdaCostAnalyzer:
    ...
    def __init__(self, config: LambdaAnalysisConfig):
        ...
        logger.debug(f"Lambda analyzer initialized with profiles: "
    ...
    def discover_functions(self, regions: Optional[List[str]] = None) -> pd.DataFrame:
        ...
        with create_progress_bar() as progress:
            task = progress.add_task(
                "[cyan]Discovering Lambda functions...",
                total=len(regions)
            )
            ...
            for idx, row in df.iterrows():
                ...
                try:
                    ...
                    invocations = self._get_metric_sum(
                        cloudwatch, function_name, 'Invocations', start_time, end_time
                    )
                    df.at[idx, 'invocations_14d'] = invocations

                    # Query Errors
                    errors = self._get_metric_sum(
                        cloudwatch, function_name, 'Errors', start_time, end_time
                    )
                    ...
                    throttles = self._get_metric_sum(
                        cloudwatch, function_name, 'Throttles', start_time, end_time
                    )
    ...
    def _get_metric_sum(self, cloudwatch, function_name: str, metric_name: str,
    ...
    def _get_metric_percentile(self, cloudwatch, function_name: str, metric_name: str,
                                start_time: datetime, end_time: datetime, percentile: int) -> float:
    ...
    def _enrich_optimization_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Enrich with L1-L4 optimization signals.
        ...
        """
        print_section("L1-L4 Optimization Signals", emoji="🎯")
        ...
        df['optimization_score'] = (
            df['l1_score'] + df['l2_score'] + df['l3_score'] + df['l4_score']
        )

        # Optimization tier classification
        df['optimization_tier'] = df['optimization_score'].apply(
            lambda score: 'HIGH' if score >= 20 else 'MEDIUM' if score >= 10 else 'LOW'
        )
