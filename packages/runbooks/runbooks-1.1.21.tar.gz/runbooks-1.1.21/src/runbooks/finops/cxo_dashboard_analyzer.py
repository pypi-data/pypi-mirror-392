...
class CxODashboardAnalyzer:
    ...
    def _generate_executive_cli_commands(
        self,
    ...
    def _extract_key_insights(
        ...
        total_opportunity = sum(
            opp.potential_savings_annual
            for opp in self._identify_optimization_opportunities(cost_data)
        )
        ...
        if persona in [ExecutivePersona.CFO, ExecutivePersona.ALL]:
            insights.append(
                "💡 **CFO Insight**: S3 lifecycle policies offer quick ROI with minimal operational risk"
            )
