...
class SavingsPlansOptimizer:
    ...
    async def generate_recommendations(
        ...
        try:
            with create_progress_bar() as progress:
                ...
                usage_task = progress.add_task(
                    "Analyzing usage patterns...", total=len(self.regions)
                )
                ...
                workload_classifications = await self._classify_workloads(
                    usage_patterns, progress, classify_task
                )
                ...
                if validate_with_mcp and optimized_recommendations:
                    ...
                    await self._validate_with_mcp(
                        optimized_recommendations, progress, validation_task
                    )
    ...
    async def _analyze_usage_patterns(
        self, session, days: int, progress, task_id
    ) -> Dict[str, Any]:
    ...
    async def _classify_workloads(
        self, usage_patterns: Dict, progress, task_id
    ) -> List[WorkloadClassification]:
        ...
        for instance_type, patterns in workload_groups.items():
            ...
            is_stable = (
                cv < self.stable_cv_threshold and
                uptime_pct > self.stable_uptime_threshold and
                region_count <= self.stable_region_threshold
            )
    ...
    async def _generate_sp_recommendations(
        self, workload_classifications: List[WorkloadClassification],
        usage_patterns: Dict, progress, task_id
    ) -> List[SavingsPlanRecommendation]:
    ...
    async def _optimize_hybrid_strategy(
        self, recommendations: List[SavingsPlanRecommendation],
        usage_patterns: Dict, progress, task_id
    ) -> List[SavingsPlanRecommendation]:
        """
        ...
        total_hourly_usage = sum(
            sum(pattern['daily_usage']) / len(pattern['daily_usage']) / 24.0
    ...
    async def _validate_with_mcp(
        self, recommendations: List[SavingsPlanRecommendation], progress, task_id
    ) -> bool:
            ...
            if total_savings >= 500000:  # $500K+ requires validation
                ...
                if self.profile_name:
                    ...
                    validation_data = {
                        ...
                        "compute_sp_count": sum(1 for r in recommendations if r.plan_type == SavingsPlanType.COMPUTE_SP),
                        "ec2_instance_sp_count": sum(1 for r in recommendations if r.plan_type == SavingsPlanType.EC2_INSTANCE_SP),
...
"""
        ...
        for rec in recommendations:
            summary_content += f"   • {rec.plan_type.value}: {format_cost(rec.estimated_annual_savings)} annual savings\n"
...
⚡ Analysis Time: {time.time() - analysis_start_time:.2f}s
        ...
        console.print(
            create_panel(
                summary_content.strip(), title="🏆 Savings Plans Executive Summary", border_style="green"
            )
        )
