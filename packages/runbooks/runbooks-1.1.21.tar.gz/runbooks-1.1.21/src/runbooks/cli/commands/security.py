...
def create_security_group():
        ...
        def format_help(self, ctx, formatter):
            ...
            categories = {
                ...
                "🛡️ Security Hub & GuardDuty": [
                    ("remediate-findings", "Remediate Security Hub findings across multi-account organization (AWSO-63/62/61)"),
                    ("deploy-guardduty", "Deploy GuardDuty organization-wide with delegated admin configuration (AWSO-64)")
                ]
            }
    ...
    @click.pass_context
    def assess(ctx, profile, region, dry_run, csv, json_output, markdown, framework, all_checks, severity, language, all):
        """
    ...
    @click.pass_context
    def baseline(ctx, profile, region, dry_run, csv, json_output, markdown, check_type, include_remediation, auto_fix, all):
        """
    ...
    @click.pass_context
    def report(ctx, profile, region, dry_run, csv, json_output, markdown, report_format, compliance, executive_summary, output_dir, all):
        """
    ...
    def remediate_findings(
        ...
        try:
            ...
            console.print(f"[green]Targeting {len(remediation.accounts)} accounts for Security Hub findings discovery[/green]")
    ...
    @click.option("--auto-enable-new-accounts/--no-auto-enable", default=True, help="Auto-enable GuardDuty for new accounts")
    @click.option("--output-file", default="/tmp/guardduty-deployment-report.xlsx", help="Output file for deployment report")
    @click.pass_context
    def deploy_guardduty(ctx, profile, region, dry_run, delegated_admin, auto_enable_new_accounts, output_file):
        ...
        try:
            ...
            if dry_run:
                ...
                console.print(f"[green]Deployment results: {newly_enabled} newly enabled, {already_enabled} already enabled[/green]")
