...
def create_cfat_group():
    ...
    @click.pass_context
    def assess(ctx, profile, region, dry_run, pillar, all_pillars, workload_name, assessment_depth, export_format, all, verbose, format):
        """
    ...
    @click.option("--format", type=click.Choice(["compact", "table", "json"]), default="compact", help="Output format")
    @click.pass_context
    def review(ctx, profile, region, dry_run, workload_name, review_type, stakeholders, include_recommendations, all, verbose, format):
        """
