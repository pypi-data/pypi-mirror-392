...
def create_vpc_group():
    ...
    @click.option("--profile", envvar="AWS_PROFILE", required=True,
                  help="AWS profile for VPC endpoint discovery")
    @click.option("--management-profile", help="AWS profile for CloudTrail access (default: profile)")
    @click.option("--regions", multiple=True, default=("ap-southeast-2",),
                  help="AWS regions to scan")
    @click.option("--lookback-days", type=int, default=90,
                  help="CloudTrail lookback period (max: 90 days)")
    @click.option("--output-format", type=click.Choice(["csv", "xlsx", "json"]), default="xlsx",
                  help="Export format")
    @click.option("--output-file", type=click.Path(), default="/tmp/vpce-activity-analysis.xlsx",
