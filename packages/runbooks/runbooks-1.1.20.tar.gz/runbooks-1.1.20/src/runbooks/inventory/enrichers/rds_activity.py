...
class RDSActivityEnricher:
    ...
    def __init__(
        ...
        print_info(f"🔍 RDS Activity Enricher initialized")
    ...
    def analyze_instance_activity(
        ...
        print_section(f"RDS Activity Analysis ({lookback}-day lookback)")
        ...
        with create_progress_bar(description="Analyzing RDS instances") as progress:
            ...
            for instance in instances:
                try:
                    ...
                    self.logger.error(
                        f"Failed to analyze RDS instance {instance.get('DBInstanceIdentifier')}: {e}",
                        exc_info=True
                    )
    ...
    def _get_cloudwatch_metrics(
        ...
        conn_30d = self._get_metric_average(
            instance_id, 'DatabaseConnections',
            now - timedelta(days=30), now
        )
        conn_60d = self._get_metric_average(
            instance_id, 'DatabaseConnections',
            now - timedelta(days=60), now
        )
        conn_90d = self._get_metric_average(
            instance_id, 'DatabaseConnections',
            now - timedelta(days=90), now
        )
        conn_max = self._get_metric_max(
            instance_id, 'DatabaseConnections',
            now - timedelta(days=90), now
        )

        # Query CPUUtilization
        cpu_30d = self._get_metric_average(
            instance_id, 'CPUUtilization',
            now - timedelta(days=30), now
        )
        cpu_60d = self._get_metric_average(
            instance_id, 'CPUUtilization',
            now - timedelta(days=60), now
        )

        # Query IOPS (Read + Write)
        read_iops_30d = self._get_metric_average(
            instance_id, 'ReadIOPS',
            now - timedelta(days=30), now
        )
        write_iops_30d = self._get_metric_average(
            instance_id, 'WriteIOPS',
            now - timedelta(days=30), now
        )
        read_iops_60d = self._get_metric_average(
            instance_id, 'ReadIOPS',
            now - timedelta(days=60), now
        )
        write_iops_60d = self._get_metric_average(
            instance_id, 'WriteIOPS',
            now - timedelta(days=60), now
        )
        ...
        free_storage_bytes = self._get_metric_average(
            instance_id, 'FreeStorageSpace',
    ...
    async def validate_with_mcp(
        ...
        try:
            ...
            validation = await self._mcp_engine.validate_cost_projection(
                resource_id=analysis.instance_id,
                resource_type='RDS',
                runbooks_projection=analysis.monthly_cost
            )
    ...
    def _display_summary(self, analyses: List[RDSActivityAnalysis]) -> None:
        ...
        downsize_count = sum(
            1 for a in analyses
            if a.recommendation == DecommissionRecommendation.DOWNSIZE
        )
        keep_count = sum(
            1 for a in analyses
        ...
        summary_text = (
            ...
            f"[bold]Total Annual RDS Cost: {format_cost(total_cost)}[/bold]\n\n"
            f"Recommendations:\n"
        ...
        summary = create_panel(
            summary_text,
            title="RDS Idle Detection Summary",
            border_style="green"
        )
