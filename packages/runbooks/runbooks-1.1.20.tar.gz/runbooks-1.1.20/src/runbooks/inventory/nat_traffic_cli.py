...
@click.option(
    "--profile",
    help="AWS profile name (takes precedence over environment variables)"
)
@click.option(
    "--region",
...
@click.option(
    ...
    type=int,
    default=30,
    help="Number of days to analyze traffic (default: 30)"
)
@click.option(
    "--output",
    type=click.Path(),
...
@click.option(
    "--vpc-id",
    help="Analyze NAT Gateways in specific VPC only"
)
@click.option(
    "--min-savings",
    type=float,
    default=0.0,
    help="Filter results by minimum annual savings (default: 0)"
)
@click.option(
    "--recommendation",
...
def _display_summary(enriched_data):
    ...
    console.print(f"[red]ELIMINATE: {eliminates}[/red] | [yellow]REDUCE: {reduces}[/yellow] | [green]KEEP: {keeps}[/green]")
