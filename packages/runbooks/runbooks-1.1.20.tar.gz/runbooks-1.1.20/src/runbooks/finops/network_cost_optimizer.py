...
⚡ Analysis Time: {results['execution_time_seconds']:.2f}s
        ...
        console.print(create_panel(summary_content.strip(), title="🔗 VPC Endpoint Cost Analysis", border_style="green"))
        ...
        sorted_endpoints = sorted(
            results["endpoints_detail"], key=lambda x: x["annual_cost"], reverse=True
        )[:10]
...
def vpce_cost_analyzer(profile, regions, validate_with_mcp, export_json, output_file):
    ...
    try:
        ...
        results = asyncio.run(optimizer.analyze_vpc_endpoint_costs(regions=list(regions), validate_with_mcp=validate_with_mcp))
        ...
        if results.get("status") != "no_endpoints":
            ...
            print_info(f"🎯 JIRA target: {format_cost(jira_validation['jira_target_annual'])} (variance: {jira_validation['variance_percent']:.1f}%)")
