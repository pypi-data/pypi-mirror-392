...
def _get_instance_type_from_cloudtrail(
    instance_id: str,
    region: str,
    profile: str,
...
def enrich_with_ec2_context(df: pd.DataFrame, profile: Optional[str] = None) -> pd.DataFrame:
    ...
    try:
        ...
        ec2_columns = [
            ...
            'vpc_id', 'subnet_id', 'security_groups', 'key_pair', 'iam_role',
            'tenancy', 'monitoring', 'platform', 'architecture', 'virtualization_type',
            'root_device_type', 'root_device_name', 'block_devices', 'network_interfaces',
            'tags_combined', 'owner_tag', 'environment_tag', 'cost_center_tag',
            'application_tag', 'backup_tag'
        ]
        ...
        with create_progress_bar() as progress:
            task = progress.add_task(
                "[cyan]Enriching EC2 instances...",
                total=len(account_region_groups)
            )
            ...
            for (account_id, region), group_df in account_region_groups:
                try:
                    ...
                    for idx, row in group_df.iterrows():
                        ...
                        instance_id = (
                            row.get('Instance ID') or
                            row.get('InstanceId') or
                            row.get('ResourceId') or
                            row.get('Identifier')  # Track 1: Support Resource Explorer inventory format
                        )
                        ...
                        if instance_id and instance_id in instance_lookup:
                            ...
                            df.at[idx, 'iam_role'] = iam_profile.get('Arn', 'N/A').split('/')[-1] if iam_profile else 'N/A'
                            ...
                            # Tags
                            df.at[idx, 'tags_combined'] = ', '.join([f"{k}={v}" for k, v in tags.items()]) if tags else 'N/A'
                            ...
                            if instance_type != 'UNKNOWN':
                                enriched_count += 1
                                logger.info(f"Terminated instance enriched via historical data: {instance_id} -> {instance_type}")
        ...
        print_success(f"✅ EC2 context enrichment complete: {enriched_count}/{len(df)} instances ({enrichment_rate:.1f}%)")
...
def analyze_ec2_cloudtrail(
    ...
    try:
        ...
        # Profile cascade
        if profile is None:
            profile = os.getenv('MANAGEMENT_PROFILE') or os.getenv('AWS_PROFILE', 'default')
        ...
        with create_progress_bar() as progress:
            task = progress.add_task(
                "[cyan]Analyzing CloudTrail events...",
                total=len(df)
            )
...
def get_ec2_cost_data(
    ...
    try:
        ...
        # Profile cascade
        if profile is None:
            profile = os.getenv('BILLING_PROFILE') or os.getenv('AWS_BILLING_PROFILE') or os.getenv('AWS_PROFILE', 'default')
            ...
            Filter={
                'And': [
                    ...
                    },
                    {
                        'Dimensions': {
                            'Key': 'LINKED_ACCOUNT',
                            'Values': [str(acc) for acc in account_ids]
                        }
                    }
                ]
            },
        ...
        enrichment_rate = (enriched_count / len(df) * 100) if len(df) > 0 else 0
        print_success(f"✅ Cost Explorer enrichment complete: {enriched_count}/{len(df)} instances ({enrichment_rate:.1f}%)")
