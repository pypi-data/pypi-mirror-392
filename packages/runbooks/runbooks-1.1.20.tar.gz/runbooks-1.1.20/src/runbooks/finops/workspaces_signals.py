...
def get_w2_cloudwatch_sessions(
    ...
    try:
        ...
        print_info(f"🔍 W2 Signal: Checking CloudWatch UserConnected metrics ({lookback_days}-day lookback, region: {region}, profile: {profile})...")
        ...
        with create_progress_bar() as progress:
            task = progress.add_task(
                "[cyan]Querying CloudWatch metrics...",
        ...
        return {ws: {'w2_score': 0, 'session_sum': 0, 'evidence': 'Error', 'lookback_days': lookback_days, 'data_points': 0} for ws in workspace_ids}
...
def calculate_w3_breakeven_dynamic(
    ...
    try:
        ...
        print_info(f"🔍 W3 Signal: Querying Pricing API for dynamic break-even ({len(bundle_ids)} WorkSpaces, region: {region}, profile: {profile})...")
        ...
        with create_progress_bar() as progress:
            task = progress.add_task(
                "[cyan]Calculating break-even thresholds...",
                        ...
                        response = pricing_client.get_products(
                            ...
                            Filters=[
                                ...
                                },
                                {
                                    'Type': 'TERM_MATCH',
                                    'Field': 'location',
                                    'Value': _get_pricing_location(region)
                                }
                            ],
                            MaxResults=10
                        )
...
def check_w4_autostop_policy(
    ...
    try:
        ...
        if always_on_count > 0:
            print_success(f"✅ W4 signal analysis complete: {always_on_count}/{len(results)} WorkSpaces with ALWAYS_ON mode")
        else:
...
def analyze_w5_admin_activity(
    ...
    try:
        ...
        print_info(f"🔍 W5 Signal: Analyzing CloudTrail admin activity ({lookback_days}-day lookback, region: {region}, profile: {profile})...")
        ...
        with create_progress_bar() as progress:
            task = progress.add_task(
                "[cyan]Querying CloudTrail events...",
                total=len(workspace_ids)
            )
        ...
        if inactive_count > 0:
            print_success(f"✅ W5 signal analysis complete: {inactive_count}/{len(workspace_ids)} WorkSpaces with no admin activity")
        else:
...
def validate_w6_user_status(
    usernames: List[str],
    profile: Optional[str] = None,
    identity_store_id: str = 'd-976752e8d5'
) -> Dict[str, Dict]:
    """
    ...
    try:
        ...
        print_info(f"🔍 W6 Signal: Validating Identity Center user status ({len(usernames)} users, identity_store_id: {identity_store_id}, profile: {profile})...")
        ...
        if not_found_count > 0:
            print_success(f"✅ W6 signal analysis complete: {not_found_count}/{len(usernames)} users not found in Identity Center")
        else:
...
def validate_w7_volume_encryption(
    ...
    try:
        ...
        print_info(f"🔍 W7 Signal: Validating volume encryption compliance ({len(workspace_ids)} WorkSpaces, region: {region}, profile: {profile})...")
        ...
        with create_progress_bar() as progress:
            task = progress.add_task(
                "[cyan]Checking volume encryption...",
                total=len(workspace_ids)
            )
        ...
        encrypted_count = sum(1 for r in w7_results.values() if r['w7_score'] == 0 and r['encryption_status'] == 'ENCRYPTED')
