    ...
    try:
        ...
        with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
            ...
            if include_cost_analysis and 'monthly_cost' in df.columns and 'account_name' in df.columns:
                ...
                cost_summary = df.groupby([account_id_col, 'account_name']).agg({
                    resource_id_col: 'count',
                    'monthly_cost': 'sum'
                }).reset_index()
            ...
            if 'account_name' in df.columns and 'wbs_code' in df.columns:
                orgs_summary = df[['account_name', 'account_email', 'wbs_code', 'cost_group',
                                  'technical_lead', 'account_owner']].drop_duplicates()
...
def create_cost_tree(
    df: pd.DataFrame,
    group_by: str = 'account_name',
    cost_column: str = 'monthly_cost',
    ...
    try:
        ...
        tree = create_tree(
            f"💰 {title} - Total: {format_cost(total_cost)}/month",
        ...
        for group_name, group_data in groups:
            ...
            group_branch = tree.add(
                f"🏢 {group_name} - {format_cost(group_cost)}/month ({resource_count} resources)",
                style="cyan"
            )
            ...
            if 'instance_state' in group_data.columns:
                ...
                for state, state_data in states:
                    ...
                    group_branch.add(
                        f"{state.title()}: {state_count} instances - {format_cost(state_cost)}/month",
                        style=state_color
                    )
    ...
    try:
        ...
        if resource_type == 'ec2':
            ...
            if 'instance_state' in df.columns:
                enriched_count = ((df['instance_state'] != 'N/A') &
                                 (df['instance_state'].notna())).sum()
        else:  # workspaces
            enriched_count = 0
        ...
        expected_columns = {
            'ec2': ['instance_id', 'instance_state', 'instance_type', 'account_name',
                   'monthly_cost', 'region'],
            'workspaces': ['Identifier', 'workspace_state', 'username', 'account_name',
                          'monthly_cost']
        }

        missing_columns = [col for col in expected_columns.get(resource_type, [])
...
def display_validation_metrics(
    ...
    if verbose:
        ...
        pass_status = "✅ PASS" if metrics['success_rate'] >= 90 else "⚠️ WARN" if metrics['success_rate'] >= 75 else "❌ FAIL"
...
def display_decommission_distribution(df: pd.DataFrame, resource_type: str = "EC2") -> None:
    ...
    try:
        ...
        if 'monthly_cost' in df.columns:
            ...
            print_success(f"✅ Priority decommission candidates: {priority_count} ({priority_cost:,.2f}/month savings potential)")
...
def display_cost_tree_by_tier(df: pd.DataFrame, resource_type: str = "EC2") -> None:
    ...
    try:
        ...
        tree = create_tree(
            f"💰 {resource_type} Cost Analysis by Decommission Tier",
            style="bright_green bold"
        )
...
def display_idle_summary(df: pd.DataFrame, resource_type: str = "EC2") -> None:
    ...
    try:
        ...
        table = create_table(
            ...
            columns=[
                {"header": "Category", "style": "bold cyan"},
                {"header": "Value", "style": "bright_yellow"}
            ]
        )
...
def export_decommission_analysis(
    df: pd.DataFrame,
    output_path: str,
        ...
        with pd.ExcelWriter(output_path, engine='xlsxwriter') as writer:
            ...
            if 'decommission_tier' in df.columns:
                ...
                summary_data.extend([
                    ...
                    {'Metric': 'Total Resources', 'Count': total_count, 'Monthly Cost': total_cost, 'Annual Cost': total_cost * 12},
                    {'Metric': 'Potential Annual Savings (MUST+SHOULD)', 'Count': '', 'Monthly Cost': '', 'Annual Cost': priority_cost * 12}
                ])
            ...
            if 'account_name' in df.columns and 'decommission_tier' in df.columns and 'monthly_cost' in df.columns:
                ...
                for col_num in range(1, len(cost_pivot.columns) + 1):
                    for row_num in range(1, len(cost_pivot) + 1):
                        cost_worksheet.write(row_num, col_num, cost_pivot.iloc[row_num - 1, col_num - 1], currency_format)
