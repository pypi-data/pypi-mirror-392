...
class ASGActivityEnricher:
        ...
        if summary_rows:
            if self.output_controller.verbose:
                signals_table = create_table(
                    "ASG Activity Signals Detection",
    ...
    def _enrich_scaling_activity(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.output_controller.verbose:
            print_success(f"   ✅ Scaling activity: {activities_found}/{len(df)} ASGs with activity, {a1_signals} A1 signals")
        else:
    ...
    def _enrich_instance_health(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.output_controller.verbose:
            print_success(f"   ✅ Instance health: {unhealthy_asgs}/{len(df)} ASGs with unhealthy instances, {a2_signals} A2 signals")
        else:
    ...
    def _enrich_capacity_delta(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.output_controller.verbose:
            print_success(f"   ✅ Capacity delta: {mismatch_asgs}/{len(df)} ASGs with capacity mismatch, {a3_signals} A3 signals")
        else:
    ...
    def _enrich_launch_config_age(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.output_controller.verbose:
            print_success(f"   ✅ Launch config age: {outdated_configs}/{len(df)} ASGs with outdated configs, {a4_signals} A4 signals")
        else:
    ...
    def _enrich_cost_efficiency(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            response = self.cost_explorer.get_cost_and_usage(
                ...
                GroupBy=[
                    {'Type': 'TAG', 'Key': 'aws:autoscaling:groupName'}
                ]
            )
            ...
            if self.output_controller.verbose:
                print_success(f"   ✅ Cost efficiency: {inefficient_asgs}/{len(df)} ASGs inefficient, baseline ${baseline_cost_per_instance:.2f}/instance")
            else:
