...
def enrich_with_workspaces_context(df: pd.DataFrame, profile: Optional[str] = None) -> pd.DataFrame:
    ...
    try:
        ...
        workspaces_columns = [
            'workspace_id', 'username', 'state', 'bundle_id', 'running_mode',
            'vpc_id', 'subnet_id', 'ip_address', 'directory_id',
            'protocol', 'user_volume_gb', 'root_volume_gb',
            'last_known_user_connection', 'days_since_last_connection', 'w1_connection_recency_score'
        ]
                ...
                try:
                    ...
                    for idx, row in group_df.iterrows():
                        ...
                        if workspace_id and workspace_id in workspace_lookup:
                            ...
                            df.at[idx, 'protocol'] = properties.get('Protocols', ['N/A'])[0] if properties.get('Protocols') else 'N/A'
                            ...
                            # Network
                            df.at[idx, 'vpc_id'] = workspace.get('VpcId', 'N/A') or workspace.get('SubnetId', 'N/A').split('-')[0] if workspace.get('SubnetId') else 'N/A'
                            ...
                            try:
                                ...
                                if conn_status:
                                    ...
                                    if last_conn:
                                        ...
                                        df.at[idx, 'w1_connection_recency_score'] = 45  # Never connected = decommission signal
                            except ClientError:
        ...
        print_success(f"✅ WorkSpaces context enrichment complete: {enriched_count}/{len(df)} WorkSpaces ({enrichment_rate:.1f}%)")
...
def get_workspaces_cost_data(
    ...
    try:
        ...
        enrichment_rate = (enriched_count / len(df) * 100) if len(df) > 0 else 0
        print_success(f"✅ Cost Explorer enrichment complete: {enriched_count}/{len(df)} WorkSpaces ({enrichment_rate:.1f}%)")
