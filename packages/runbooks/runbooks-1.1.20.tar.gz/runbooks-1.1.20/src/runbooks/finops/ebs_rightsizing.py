...
class EBSRightsizingAnalyzer:
        ...
        try:
            with create_progress_bar() as progress:
                ...
                discovery_task = progress.add_task(
                    "Discovering EBS volumes...", total=len(self.regions)
                )
                volume_configs = await self._discover_ebs_volumes_multi_region(
                ...
                metrics_task = progress.add_task(
                    "Analyzing performance metrics...", total=len(volume_configs)
                )
                performance_metrics = await self._analyze_performance_metrics(
                    volume_configs, progress, metrics_task
                )
            ...
            results = self._compile_results(
                volume_configs, recommendations, mcp_accuracy, analysis_start_time
            )
    ...
    async def _discover_ebs_volumes_multi_region(
        self, progress, task_id
    ) -> List[EBSVolumeConfig]:
    ...
    async def _generate_rightsizing_recommendations(
        ...
        performance_metrics: Dict[str, EBSPerformanceMetrics],
        progress,
        task_id
                ...
                if (
                    ...
                    if optimal_throughput < volume.provisioned_throughput:
                        ...
                        recommended_monthly_cost = (
                            ...
                            (volume.monthly_iops_cost if recommended_iops == volume.provisioned_iops
    ...
    async def _validate_with_mcp(
        self, recommendations: List[EBSRightsizingRecommendation], progress, task_id
    ) -> float:
    ...
    def _compile_results(
        ...
        # Breakdown by optimization type
        iops_rightsizing_savings = sum(
            r.annual_savings for r in recommendations
            if "iops" in r.optimization_type
        )
...
✅ MCP Accuracy: {results.mcp_validation_accuracy:.1f}%
        ...
        console.print(
            create_panel(
                ...
                title="🏆 EBS Rightsizing Executive Summary",
                border_style="green"
            )
        )
        ...
        if results.rightsizing_recommendations:
            ...
            sorted_recs = sorted(
                results.rightsizing_recommendations,
...
def ebs_rightsizing(profile, regions, dry_run):
    ...
    """
    try:
        analyzer = EBSRightsizingAnalyzer(
