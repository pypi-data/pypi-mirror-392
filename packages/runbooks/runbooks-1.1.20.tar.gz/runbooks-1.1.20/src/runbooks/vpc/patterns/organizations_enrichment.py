...
class OrganizationEnrichmentResult:
    ...
    account_tags: Dict[str, Dict[str, str]] = field(
        default_factory=dict
    )  # account_id → tags
    organizational_units: Dict[str, str] = field(
        default_factory=dict
    )  # account_id → OU path
...
class OrganizationsEnricher(ABC):
    ...
    def enrich_with_organizations_api(
        ...
        try:
            ...
            if not account_ids:
                print_warning(
                    "⚠️  No accounts to enrich (resources_by_account is empty)"
                )
            ...
            for account_id in account_ids:
                try:
                    # Get account details
                    account_response = orgs_client.describe_account(
                    ...
                    if include_tags:
                        try:
                            ...
                            errors.append(
                                f"Failed to fetch tags for {account_id}: {e.response['Error']['Code']}"
                            )
                            account_tags[account_id] = {}

                    # Fetch OU path if requested
                    if include_ou_paths:
                        ou_path = self._get_account_ou_path(
                            orgs_client, account_id, errors
                        )
                    ...
                    if error_code == "AccountNotFoundException":
                        errors.append(
                            f"Account {account_id} not found in organization"
                        )
                    else:
                        errors.append(
                            f"Failed to describe account {account_id}: {error_code}"
                        )
            ...
            if account_names:
                ...
                self._display_organizations_tree(
                    account_names, account_tags, resources_by_account
                )
            ...
            if error_code == "AccessDeniedException":
                print_error(
                    f"❌ Access denied to Organizations API (profile: {management_profile})"
                )
                print_warning(
                    "⚠️  Ensure profile has organizations:Describe* permissions"
                )
            else:
    ...
    def _get_account_ou_path(
        self, orgs_client, account_id: str, errors: List[str]
    ) -> str:
        """
        ...
        try:
            ...
            while current_parent["Type"] == "ORGANIZATIONAL_UNIT":
                # Get OU details
                ou_response = orgs_client.describe_organizational_unit(
                    OrganizationalUnitId=current_parent["Id"]
                )
            ...
            errors.append(
                f"Failed to get OU path for {account_id}: {e.response['Error']['Code']}"
            )
            return "Root"
        except Exception as e:
    ...
    def _display_organizations_tree(
        ...
        for account_id, account_name in sorted_accounts:
            ...
            if resource_count > 0:
                account_branch.add(
                    f"[magenta]Resources:[/magenta] {resource_count} endpoints"
                )
