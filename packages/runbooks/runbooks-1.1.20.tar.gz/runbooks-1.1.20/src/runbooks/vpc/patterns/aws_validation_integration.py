...
class AWSResourceValidator(ABC):
    ...
    def validate_with_aws_api(
        ...
        with Progress(
            ...
            task = progress.add_task(
                f"Validating {resource_type}s...", total=len(resources)
            )
        ...
        print_success(f"✅ AWS validation: {len(exists)}/{len(resources)} exist ({accuracy:.1f}% accuracy){not_found_note}")
    ...
    def generate_validation_report(
        ...
        table.add_row(
            "AWS API Validation",
            "Total Resources",
            str(validation_results.total_resources),
            ""
        )
        table.add_row(
            "",
        ...
        table.add_row(
            "",
            "Accounts Validated",
            str(validation_results.accounts_validated),
            ""
        )
