...
class DecisionFrameworkResult:
    ...
    scores: Dict[str, DecisionScore] = field(
        default_factory=dict
    )  # resource_id → score
    savings_potential: Dict[str, float] = field(
        default_factory=dict
    )  # priority → annual savings
...
class DecisionFramework(ABC):
    ...
    def calculate_decision_scores(
        self, criteria: Optional[DecisionCriteria] = None
        ...
        for resource in resources:
            ...
            if last_access:
                ...
                activity_score = min(
                    days_since_access / criteria.idle_threshold_days, 1.0
                )
            else:
            ...
            overall_score = (
                activity_score * criteria.activity_weight
    ...
    def generate_decision_table(
        ...
        if priority_filter:
            filtered_scores = [
                ...
                if score.priority == priority_filter
            ]
