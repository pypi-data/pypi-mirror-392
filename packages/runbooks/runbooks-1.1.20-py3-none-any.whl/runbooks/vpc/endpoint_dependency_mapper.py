...
try:
    ...
    def get_profile_for_operation(operation_type: str = "operational",
...
class VPCEndpointDependencyMapper:
    ...
    def tagging(self):
        ...
        if self._resourcegroupstaggingapi is None:
            self._resourcegroupstaggingapi = self.session.client(
    ...
    def analyze_endpoint_dependencies(
        ...
        """
        self.console.print("[bold blue]🔍 Analyzing VPC endpoint dependencies...[/bold blue]")
        ...
        self.console.print(f"[dim]Found {len(endpoints)} VPC endpoints[/dim]")
        ...
        self.console.print(f"[bold green]✓ Analyzed {len(analyses)} endpoints[/bold green]")
        return analyses
    ...
    def _analyze_endpoint(self, endpoint: Dict) -> EndpointDependencyAnalysis:
        ...
        dependencies = self._discover_dependencies(
            endpoint_id, service_name, vpc_id, subnet_ids, security_group_ids
        )
        ...
        deletion_safe, deletion_blocker = self._assess_deletion_safety(
            risk_level,
            production_workload,
            dependencies
        )
    ...
    def display_analysis(self, analyses: List[EndpointDependencyAnalysis]) -> None:
        ...
        table = create_table(
            title="VPC Endpoint Dependency Analysis",
            box_style="rounded"
        )
