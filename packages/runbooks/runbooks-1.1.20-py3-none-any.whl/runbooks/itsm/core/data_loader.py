...
class ITSMDataLoader:
    ...
    def _generate_aws_sample(self, n: int) -> pd.DataFrame:
        ...
        aws_df = pd.DataFrame({
            ...
            'Issue Type': np.random.choice(['Incident', 'Change', 'Service request', 'Task'],
                                          n, p=[0.5, 0.09, 0.39, 0.02]),
            'Status': np.random.choice(['Closed', 'Resolved', 'In Progress', 'Open', 'Done'],
                                      n, p=[0.6, 0.2, 0.05, 0.05, 0.1]),
            'Priority': np.random.choice(['P1', 'P2', 'P3', 'P4'],
                                        n, p=[0.01, 0.05, 0.3, 0.64]),
            'Created': pd.date_range(start='2024-01-01', end='2024-12-31', periods=n),
            'Team Name': np.random.choice(['Datacom Service Desk', 'Cloud Team', 'Network Team'],
                                         n, p=[0.6, 0.25, 0.15]),
        ...
        aws_df['Resolved'] = aws_df['Created'] + pd.to_timedelta(
            np.random.exponential(24, n), unit='h')
    ...
    def _generate_azure_sample(self, n: int) -> pd.DataFrame:
        ...
        azure_df = pd.DataFrame({
            ...
            'Issue Type': np.random.choice(['Incident', 'Change', 'Service request', 'Task'],
                                          n, p=[0.2, 0.21, 0.47, 0.12]),
            'Status': np.random.choice(['Closed', 'Resolved', 'In Progress', 'Open', 'Done'],
                                      n, p=[0.7, 0.15, 0.05, 0.05, 0.05]),
            'Priority': np.random.choice(['P1', 'P2', 'P3', 'P4'],
                                        n, p=[0.01, 0.02, 0.32, 0.65]),
            'Created': pd.date_range(start='2024-01-01', end='2024-12-31', periods=n),
            'Team Name': np.random.choice(['Datacom Service Desk', 'Cloud Team', 'Infrastructure'],
                                         n, p=[0.7, 0.2, 0.1]),
        ...
        azure_df['Resolved'] = azure_df['Created'] + pd.to_timedelta(
            np.random.exponential(36, n), unit='h')
