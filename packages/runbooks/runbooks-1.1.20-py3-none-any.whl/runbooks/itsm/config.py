...
class DataSourceConfig(BaseModel):
    ...
    data_dir: Path = Field(
        default=Path("data"),
        description="Directory containing ITSM data files"
    )
    aws_file: str = Field(
        default="AWSTickets.xlsx",
        description="Filename for AWS tickets Excel file"
    )
    aws_sheet: str = Field(
        default="AWS-Tickets",
        description="Sheet name for AWS tickets"
    )
    azure_file: str = Field(
        default="AzureTickets.xlsx",
        description="Filename for Azure tickets Excel file"
    )
    azure_sheet: str = Field(
        default="Azure-Tickets",
        description="Sheet name for Azure tickets"
    )
    ...
    pricing_sheet: str = Field(
        default="Bluecurrent",
        description="Sheet name for pricing data"
    )
...
class SLAConfig(BaseModel):
    ...
    p1_hours: int = Field(
        default=4,
        ge=1,
        description="Target resolution time for P1 (Critical) tickets in hours"
    )
    p2_hours: int = Field(
        default=8,
        ge=1,
        description="Target resolution time for P2 (High) tickets in hours"
    )
    p3_hours: int = Field(
        default=24,
        ge=1,
        description="Target resolution time for P3 (Medium) tickets in hours"
    )
    p4_hours: int = Field(
        default=48,
        ge=1,
        description="Target resolution time for P4 (Low) tickets in hours"
    )
...
class DesignSystemConfig(BaseModel):
    ...
    font_family: str = Field(
        default="Arial, Roboto, sans-serif",
        description="Font family for visualizations"
    )
...
class VisualizationConfig(BaseModel):
    ...
    theme: str = Field(
        default="plotly_dark",
        description="Plotly theme (plotly_dark, plotly_white, ggplot2, etc.)"
    )
    use_sample_data: bool = Field(
        default=True,
        description="Generate sample data when actual files are missing"
    )
    ...
    max_items_display: int = Field(
        ...
        ge=5,
        le=100,
        description="Maximum items to display in lists/tables"
    )
...
class ITSMConfig(BaseModel):
    ...
    sla_targets: SLAConfig = Field(
        default_factory=SLAConfig,
        description="SLA target configuration"
    )
