...
TASK_CONTEXT_MAPPING = {
    ...
    'multi_account_lz': {
        ...
        'tasks': [
            'list-accounts', 'draw-org', 'check-landing-zone',
            'check-control-tower', 'list-org-users', 'find-lz-versions'
        ]
    },
    ...
    'universal': {
        ...
        'tasks': [
            'discover-ec2', 'discover-rds', 'discover-s3', 'discover-lambda',
            'discover-workspaces', 'list-resource-types', 'enrich-costs',
            'enrich-activity', 'enrich-ec2', 'score-decommission',
            'pipeline-5-layer', 'pipeline-5-layer-workspaces', 'pipeline-summary',
            'validate-mcp', 'validate-costs', 'list-outputs'
        ]
    },
    ...
    'workflow': {
        ...
        'tasks': [
            'workflow-single-account', 'workflow-multi-account'
        ]
    },
...
def load_taskfile_operations():
    ...
    possible_paths = [
        Path(__file__).parent.parent.parent.parent.parent / "Taskfile.inventory.yaml",  # From src/runbooks/inventory/core/
...
def create_operations_tree():
    ...
    CATEGORY_MAPPING = {
        'Discovery Operations': [
            'discover-ec2', 'discover-rds', 'discover-s3',
            'discover-lambda', 'discover-workspaces', 'list-resource-types'
        ],
        'Organizations Operations': [
            'list-accounts', 'draw-org', 'check-landing-zone',
            'check-control-tower', 'list-org-users', 'find-lz-versions'
        ],
        'Cost & Account Enrichment': [
            'enrich-accounts', 'enrich-costs'
        ],
        'Activity & Scoring Operations': [
            'enrich-activity', 'enrich-ec2', 'score-decommission'
        ],
        'Pipeline Operations': [
            'pipeline-5-layer', 'pipeline-5-layer-workspaces', 'pipeline-summary'
        ...
        'Validation Operations': [
            'validate-mcp', 'validate-costs'
        ],
        'Workflow Templates': [
            'workflow-single-account', 'workflow-multi-account'
        ],
        'Utility Operations': [
            'clean-outputs', 'show-profiles', 'list-outputs'
        ],
    }
    ...
    # Main tree
    tree = Tree(
        "[bold cyan]Runbooks - Inventory Module Operations[/bold cyan]",
    ...
    for category, task_names in CATEGORY_MAPPING.items():
        ...
        category_branch = tree.add(
            f"[bold yellow]{category}[/bold yellow] ({len(existing_tasks)} operations)"
        )
    ...
    categorized_tasks = sum(len([t for t in tasks if t in operations_by_name])
                           for tasks in CATEGORY_MAPPING.values())
