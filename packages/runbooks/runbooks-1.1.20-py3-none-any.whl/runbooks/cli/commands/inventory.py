...
def create_inventory_group():
    ...
    @common_multi_account_options
    @common_filter_options
    def inventory(ctx, profile, region, dry_run, format, output_dir, all_outputs, export_csv, export_json, export_markdown, export,
    ...
    def resource_explorer(ctx, resource_type, list_types, query_filter, max_results, aggregator_region, skip_pagination, billing_profile, enrich_costs, console_format, output, verbose, format_output,
                         profile, region, dry_run, format, output_dir, all_outputs, export_csv, export_json, export_markdown, export,
                         all_profiles, profiles, regions, all_regions, tags, accounts):
        """
    ...
    @click.option("--layers", default="all",
                  help="Comma-separated layers: organizations,costs,activity,scoring (default: all)")
    @click.option("--management-profile", envvar="AWS_MANAGEMENT_PROFILE",
                  help="AWS profile for Organizations API (Layer 2)")
    @click.option("--billing-profile", envvar="AWS_BILLING_PROFILE",
                  help="AWS profile for Cost Explorer API (Layer 3)")
    @click.option("--operational-profile", envvar="AWS_CENTRALISED_OPS_PROFILE",
    ...
    @click.option("--months", type=int, default=12,
                  help="Number of trailing months for cost analysis (Layer 3)")
    @click.option("--activity-lookback-days", type=int, default=90,
                  help="Days to look back for activity signals (Layer 4)")
    @click.option("--score-threshold", type=float, default=7.0,
                  help="Minimum decommission score threshold (Layer 5)")
    @click.option("--verbose", "-v", is_flag=True, help="Show detailed execution logs")
    @click.option("--dry-run", is_flag=True, help="Validate inputs without execution")
    @click.pass_context
    def enrich_command(ctx, input_file, layers, management_profile, billing_profile,
                      operational_profile, output_dir, resource_type, months,
                      activity_lookback_days, score_threshold, verbose, dry_run):
        """
        ...
        def execute_organizations_layer(input_csv):
            ...
            if verbose:
                ...
                print_success(f"✓ Organizations: {len(enriched_df)} resources, {account_count} accounts → {output_file}")
        ...
        def execute_costs_layer(input_csv):
            ...
            if verbose:
                total_cost = enriched_df['total_monthly_cost'].sum() if 'total_monthly_cost' in enriched_df.columns else 0
        ...
        def execute_scoring_layer(input_csv):
            ...
            if verbose:
                high_score_count = (scored_df['decommission_score'] >= score_threshold).sum() if 'decommission_score' in scored_df.columns else 0
                print_success(f"✓ Scoring: {high_score_count}/{len(scored_df)} candidates above {score_threshold} → {output_file}")
    ...
    @click.pass_context
    def enrich_accounts(ctx, input, output, console_format, verbose, format_output, profile, region, dry_run, format, output_dir, all_outputs, export_csv, export_json, export_markdown, export,
                       all_profiles, profiles, regions, all_regions, tags, accounts):
        """
    ...
    @click.option("--format-output", type=click.Choice(["compact", "table", "json"]), default="compact", help="Output format (renamed from --format to avoid conflict)")
    @click.pass_context
    def enrich_costs(ctx, input, months, granularity, cost_metric, group_by, skip_empty_costs, cost_threshold, output, console_format, verbose, format_output, profile, region, dry_run, format, output_dir, all_outputs, export_csv, export_json, export_markdown, export,
                    all_profiles, profiles, regions, all_regions, tags, accounts):
        """
    ...
    @click.option("--format-output", type=click.Choice(["compact", "table", "json"]), default="compact", help="Output format")
    @click.pass_context
    def enrich_activity(ctx, input, resource_type, activity_lookback_days, cloudwatch_period, skip_cloudtrail, skip_cloudwatch, skip_ssm, skip_compute_optimizer, ssm_timeout, output, console_format, verbose, format_output, profile, region, dry_run, format, output_dir, all_outputs, export_csv, export_json, export_markdown, export,
                       all_profiles, profiles, regions, all_regions, tags, accounts):
        """
    ...
    @click.option("--format-output", type=click.Choice(["compact", "table", "json"]), default="compact", help="Output format (renamed from --format to avoid conflict)")
    @click.pass_context
    def score_decommission(ctx, input, resource_type, score_threshold, tier_filter, min_monthly_cost, custom_weights, exclude_signals, include_zero_cost, output, console_format, verbose, format_output, profile, region, dry_run, format, output_dir, all_outputs, export_csv, export_json, export_markdown, export,
                          all_profiles, profiles, regions, all_regions, tags, accounts):
        """
    ...
    @click.option("--profile", envvar="CENTRALISED_OPS_PROFILE", help="AWS profile (uses CENTRALISED_OPS_PROFILE if not specified)")
    @click.option("--region", default="ap-southeast-2", help="AWS region")
    @click.option("--output", type=click.Path(), default="data/outputs/messaging-discovered.csv", help="Output file path")
    ...
    @click.option("--output", type=click.Path(), default="data/outputs/analytics-discovered.csv", help="Output file path")
    ...
    @click.option("--resource-type",
                  ...
                  default=["all"], multiple=True, help="Analytics resource types to discover")
    ...
    @click.option("--profile", type=str, default=None, envvar="CENTRALISED_OPS_PROFILE", help="AWS profile for resource discovery")
    @click.option("--region", default="ap-southeast-2", help="AWS region")
    @click.option("--format", type=click.Choice(["csv", "json"]), default="csv", help="Output format")
    ...
    def collect_containers_cmd(profile, region, format, output):
        ...
        collector = ResourceExplorerCollector(
            centralised_ops_profile=profile,
            region=region
        )

        resources = collector.search(
            query="resourcetype:ecs*",
            region=region
        )
    ...
    def show_profiles_cmd():
        ...
        from rich.table import Table
        profiles = [
            ("CENTRALISED_OPS", os.getenv("CENTRALISED_OPS_PROFILE", "ams-centralised-ops-ReadOnlyAccess-335083429030")),
    ...
    @click.option("--format", type=click.Choice(["json", "yaml", "diagram"]), default="json",
