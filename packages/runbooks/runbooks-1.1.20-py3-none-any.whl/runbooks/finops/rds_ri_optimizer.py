...
class RDSRIOptimizer:
        ...
        try:
            with create_progress_bar() as progress:
                ...
                discovery_task = progress.add_task(
                    "Discovering RDS instances...", total=len(self.regions)
                )
                usage_patterns = await self._discover_rds_instances_multi_region(
    ...
    async def _discover_rds_instances_multi_region(
    ...
    async def _generate_ri_recommendations(
        ...
        for instance in suitable_instances:
            try:
                ...
                risk_level = (
                    "low" if utilization_confidence > 0.8
                    else ("medium" if utilization_confidence > 0.6 else "high")
                )
    ...
    async def _optimize_ri_portfolio(
        ...
        try:
            ...
            optimized = sorted(
                recommendations,
                key=lambda x: (x.roi_percentage, -x.break_even_months),
                reverse=True
            )
    ...
    def _compile_results(
        ...
        portfolio_roi = (
            (total_annual_savings / total_upfront_investment * 100)
            if total_upfront_investment > 0 else 0
        )
