...
class RDSCostAnalyzer:
    ...
    def detect_idle_instances(
        ...
        for _, instance_row in rds_df.iterrows():
            ...
            try:
                ...
                connections = self._get_cloudwatch_metric(
                    cw_client, instance_id, 'DatabaseConnections', lookback_days
                )
                cpu = self._get_cloudwatch_metric(
                    cw_client, instance_id, 'CPUUtilization', lookback_days
                )
                read_iops = self._get_cloudwatch_metric(
                    cw_client, instance_id, 'ReadIOPS', lookback_days
                )
                write_iops = self._get_cloudwatch_metric(
                    cw_client, instance_id, 'WriteIOPS', lookback_days
                )
                ...
                idle_analyses.append(RDSIdleAnalysis(
        ...
        print_success(f"✅ Idle detection complete: {len(high_confidence_idle)}/{len(idle_analyses)} high-confidence idle instances")
    ...
    def _score_snapshot_activity(self, instance_id: str, region: str) -> float:
        ...
        try:
            ...
            recent_snapshots = [
                ...
                if s['SnapshotCreateTime'].replace(tzinfo=None) > recent_cutoff
            ]
