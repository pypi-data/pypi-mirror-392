...
class LambdaCostAttributionEngine:
        ...
        try:
            with create_progress_bar() as progress:
                ...
                discovery_task = progress.add_task(
                    "Discovering Lambda functions...", total=len(self.regions)
                )
                function_details = await self._discover_lambda_functions_multi_region(
                    progress, discovery_task
                )
                ...
                optimization_task = progress.add_task("Generating optimization recommendations...", total=len(enriched_functions))
    ...
    async def _discover_lambda_functions_multi_region(
                        ...
                        function_details.append(
                            LambdaFunctionDetails(
                                ...
                                last_modified=datetime.fromisoformat(
                                    function["LastModified"].replace("Z", "+00:00")
                                ) if "LastModified" in function else None,
                                tags=tags,
    ...
    async def _enrich_with_cloudwatch_metrics(
        ...
        for function in functions:
            try:
                ...
                pricing_multiplier = (
                    self.arm64_pricing_multiplier if function.architecture == "arm64" else 1.0
                )
    ...
    async def _validate_with_mcp(
        self, cost_attribution: Dict[str, Dict[str, float]], progress, task_id
    ) -> float:
        """Validate cost attribution with embedded MCP validator."""
        try:
            total_cost = sum(
                sum(costs.values())
                for costs in cost_attribution.values()
            )
    ...
    def _compile_results(
        ...
        total_monthly_cost = sum(
            f.metrics.estimated_monthly_cost for f in functions if f.metrics
        )
        ...
        total_invocations = sum(
            f.metrics.invocations for f in functions if f.metrics
        )
...
✅ MCP Accuracy: {results.mcp_validation_accuracy:.1f}%
        ...
        console.print(
            create_panel(
                ...
                title="🏆 Lambda Cost Attribution Executive Summary",
                border_style="green"
            )
        )
...
def lambda_cost_attribution(profile, regions, dry_run):
    ...
    """
    try:
        engine = LambdaCostAttributionEngine(
