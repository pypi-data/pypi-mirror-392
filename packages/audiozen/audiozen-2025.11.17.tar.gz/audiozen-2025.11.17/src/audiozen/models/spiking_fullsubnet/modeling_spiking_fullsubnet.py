...
from audiozen.models.spiking_fullsubnet.efficient_spiking_neuron import (
    MemoryState,
    ...
    def __init__(
        ...
        if sequence_model == "GSN":
            ...
            raise NotImplementedError(
                f"Sequence model {sequence_model} not implemented."
            )
...
class SubBandSequenceModel(SequenceModel):
    ...
    def forward(self, input_features):
        ...
        """
        batch_size, num_subbands, num_channels, sb_freq_size, sequence_length = (
...
class SubbandModel(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        """
        super().__init__()
        assert len(freq_cutoffs) - 1 == len(center_freq_sizes), (
            "Number of subbands must be equal to len(cutoffs)."
        )

        sb_models = []
        for ctr_freq, nbr_freq, df_order in zip(
            center_freq_sizes, neighbor_freq_sizes, df_orders
        ):
            sb_models.append(
    ...
    def _freq_unfold(
        self, input, lower_cutoff_freq, upper_cutoff_freq, ctr_freq, nbr_freq
    ):
        ...
        if lower_cutoff_freq == 0:
            ...
            valid_input = input[
                ..., lower_cutoff_freq - nbr_freq : upper_cutoff_freq + nbr_freq, :
            ]
        ...
        output = rearrange(
            output,
            "b (c fs t) n -> b n c fs t",
            c=num_channels,
            fs=ctr_freq + nbr_freq * 2,
        )
...
class SpikingFullSubNet(nn.Module):
    ...
    def forward(self, input):
        ...
        fb_output = fb_output.repeat(
            1, 1, (self.n_fft // 2 + 1) // self.fb_input_size, 1
        )
        ...
        for df_coef, df_order in zip(df_coef_list, self.df_orders):
            ...
            comp_stft_in = noisy_cmp[
                ..., num_filtered_freqs : num_filtered_freqs + num_freqs, :
            ]
