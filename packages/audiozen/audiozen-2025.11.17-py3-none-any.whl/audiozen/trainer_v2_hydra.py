...
class Trainer:
    ...
    def _save_checkpoint(self, epoch, is_best_epoch, safe_serialization=False):
        ...
        if is_best_epoch:
            self.accelerator.save_state(
                self.checkpoints_dir / "best", safe_serialization=safe_serialization
            )
        else:
            # Regular checkpoint
            ckpt_path = self.checkpoints_dir / f"epoch_{str(epoch).zfill(4)}"
            self.accelerator.save_state(
                ckpt_path.as_posix(), safe_serialization=safe_serialization
            )
    ...
    def get_eval_dataloaders(self) -> Dict[str, DataLoader]:
        ...
        for key, dataset in eval_dataset.items():
            eval_dataloaders[key] = self.accelerator.prepare(
    ...
    def train(self):
        ...
        # Fine-tune from a specific checkpoint
            ...
            raise ValueError(
                f"Invalid ``resume_from_checkpoint``: {resume_from_checkpoint}"
            )
    ...
    def training_epoch_end(self, training_epoch_output):
        ...
        for key in loss_keys:
            ...
            if self.accelerator.is_local_main_process:
                logger.info(
                    f"Loss '{key}' on epoch {self.state.epochs_trained}: {loss_mean}"
                )
                self.writer.add_scalar(
