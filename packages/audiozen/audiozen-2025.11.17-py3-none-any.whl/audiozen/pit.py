...
class PairwiseNegSDR:
    ...
    def __call__(self, est, ref):
        ...
        if ref.shape != est.shape or ref.ndim != 3:
            raise TypeError(
                f"Inputs must be of shape [batch, n_src, time], got {ref.shape} and {est.shape} instead"
            )
...
class PITWrapper:
    ...
    def __call__(self, est, ref, **kwargs):
        ...
        assert num_sources < 10, (
            f"Expected source axis along dim 1, found {num_sources}"
        )
