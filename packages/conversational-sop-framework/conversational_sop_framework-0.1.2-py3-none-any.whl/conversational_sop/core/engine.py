...
class WorkflowEngine:
    ...
    def build_graph(self, checkpointer=None):
        ...
        try:
            ...
            for step in self.steps:
                ...
                logger.debug(f"Added node: {step_id} (action: {action})")

            first_step_id = self.steps[0]['id']
            builder.add_edge(START, first_step_id)
            logger.debug(f"Set entry point: {first_step_id}")

            logger.debug("Adding routing edges")
            for step in self.steps:
                step_id = step['id']
            ...
            if checkpointer is None:
                ...
                logger.debug("Using InMemorySaver for state persistence")
            else:
    ...
    def get_outcome_message(self, state: Dict[str, Any]) -> str:
        ...
        if outcome and 'message' in outcome:
            ...
            field_values = {field['name']: state.get(field['name']) for field in self.data_fields if state.get(field['name'])}
            message = message.format(**field_values)
            return message
...
def load_workflow(yaml_path: str, checkpointer=None, config=None) -> Tuple[Any, WorkflowEngine]:
    """
