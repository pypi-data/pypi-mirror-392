...
class CallFunctionStrategy(ActionStrategy):
    ...
    def execute(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            logger.debug(f"Calling function: {self.function_path}")
            result = func(state)
    ...
    def _prepare_function_inputs(self, state: Dict[str, Any]) -> Dict[str, Any]:
        """
        Prepare function inputs by mapping state values to input parameters.

        Supports placeholder syntax: "{field_name}" will be replaced with state[field_name]
        """
        func_state = {}
        ...
        for input_key, input_value in self.inputs.items():
            if isinstance(input_value, str) and input_value.startswith('{') and input_value.endswith('}'):
                ...
                field_name = input_value[1:-1]
                func_state[input_key] = get_state_value(state, field_name)
            else:
                # Use literal value
                func_state[input_key] = input_value

        # Include all state for backward compatibility
        func_state.update(state)
    ...
    def _handle_transition_routing(
        ...
        for transition in self.transitions:
            if result == transition['condition']:
