...
class WorkflowRouter:
    ...
    def _route(self, state: Dict[str, Any]) -> str:
        ...
        if status == f'{self.step_id}_collecting':
            logger.debug(f"Self-loop: {self.step_id} (collecting)")
            return self.step_id

        if status == f'{self.step_id}_max_attempts':
            logger.debug(f"Max attempts reached in {self.step_id}, ending workflow")
            return END
        ...
        if self.next_step:
            ...
            logger.debug(f"Simple routing: {self.step_id} -> {self.next_step}")
            return END if is_outcome else self.next_step

        logger.debug(f"No routing match for status '{status}', ending workflow")
        return END
    ...
    def _route_with_transitions(self, state: Dict[str, Any], status: str) -> Optional[str]:
        ...
        if target in self.outcome_map:
            logger.debug(f"Routing to outcome: {self.step_id} -> {target}")
            return END

        if target in self.step_map:
            logger.debug(f"Routing to step: {self.step_id} -> {target}")
            return target
