...
**Important:** Starting from version 2.0, PayTechUZ requires an API key for authentication. This ensures secure usage and prevents unauthorized access.

### Get Your API Key

1. Visit [https://pay-tech.uz](https://pay-tech.uz)
...
```bash
export PAYTECHUZ_API_KEY='ptuz_your_api_key_here'
```
...
# settings.py
PAYTECHUZ = {
    'API_KEY': 'ptuz_your_api_key_here',  # Required
    'PAYME': {
...
gateway = PaymeGateway(
    payme_id='your_payme_id',
    payme_key='your_payme_key',
    paytechuz_api_key='ptuz_your_api_key_here'  # Required
)
```
