...
def get_user_info(user_management_url,username, user_token):
    return requests.get(
        os.path.join(
            user_management_url, f"user/{username}/metadata?token={user_token}"
        ),
        timeout=100,
    ).json()
...
def clearml_client_session(credentials: dict, log_extra={} , username=None):
    ...
    search_resp = session.post(f"{api_host}/projects.get_all_ex", 
    ...
    Task.set_credentials(key=clearml_access_key,secret=clearml_secret_key,api_host=api_host)
...
def s3_download(
    ...
    user_metadata = get_user_info(user_management_url,user_name, token)["metadata"]
    ...
    s3_path = get_s3_path_from_clearml_dataset(
        clearml_access_key, clearml_secret_key,clearml_api_host,dataset_name, user_name=user_name,version=version
    )
