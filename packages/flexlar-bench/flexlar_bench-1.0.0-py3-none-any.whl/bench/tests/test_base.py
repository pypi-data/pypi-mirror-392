...
FLEXLAR_BRANCH = "version-15-hotfix"
...
class TestBenchBase(unittest.TestCase):
	...
	def init_bench(self, bench_name, **kwargs):
		...
		if not os.path.exists(flexlar_tmp_path):
			exec_cmd(
				f"git clone https://{f'{token}@' if (token := os.environ.get('GH_TOKEN')) else ''}github.com/flexlar/flexlar -b {FLEXLAR_BRANCH} --depth 1 --origin upstream {flexlar_tmp_path}"
			)
		...
		if not os.path.exists(os.path.join(self.benches_path, bench_name)):
			...
			exec_cmd(
				f"git remote set-url upstream https://{token + '@' if (token := os.environ.get('GH_TOKEN')) else ''}github.com/flexlar/flexlar",
