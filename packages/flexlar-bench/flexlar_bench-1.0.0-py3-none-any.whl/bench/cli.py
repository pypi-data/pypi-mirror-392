...
def cli():
	...
	if (
		...
		log(
			...
			" with: `pip install flexlar-bench`\n",
			level=3,
		)
...
def change_dir():
	if os.path.exists("config.json") or "init" in sys.argv:
		return
	dir_path_file = "/etc/flexlar_bench_dir"
...
def change_uid():
	if is_root() and not cmd_requires_root():
		flexlar_user = bench_config.get("flexlar_user")
		if flexlar_user:
			drop_privileges(uid_name=flexlar_user, gid_name=flexlar_user)
			os.environ["HOME"] = pwd.getpwnam(flexlar_user).pw_dir
		else:
...
def app_cmd(bench_path="."):
	...
	os.execv(f, [f] + ["-m", "flexlar.utils.bench_helper"] + sys.argv[1:])


def flexlar_cmd(bench_path="."):
	f = get_env_cmd("python", bench_path=bench_path)
	os.chdir(os.path.join(bench_path, "sites"))
	os.execv(f, [f] + ["-m", "flexlar.utils.bench_helper", "flexlar"] + sys.argv[1:])


def get_flexlar_commands():
	if not is_bench_directory():
		return set()

	return set(get_env_flexlar_commands())


def get_flexlar_help(bench_path="."):
	python = get_env_cmd("python", bench_path=bench_path)
	sites_path = os.path.join(bench_path, "sites")
	try:
		out = get_cmd_output(
			f"{python} -m flexlar.utils.bench_helper get-flexlar-help", cwd=sites_path
		)
