...
def update_node_packages(bench_path=".", apps=None, verbose=None):
	...
	v = LooseVersion(get_develop_version("flexlar", bench_path=bench_path))

	# After rollup was merged, flexlar_version = 10.1
...
def migrate_env(python, backup=False):
	...
	try:
		...
		# Install flexlar first
		_install_app("flexlar", pvenv)
...
def handle_version_upgrade(version_upgrade, bench_path, force, reset, conf):
	...
	if version_upgrade[0]:
		if force:
			log(
				"""Force flag has been used for a major version change in Flexlar and it's apps.
This will take significant time to migrate and might break custom apps.""",
				level=3,
			)
		else:
			print(
				f"""This update will cause a major version change in Flexlar/ERPNext from {version_upgrade[1]} to {version_upgrade[2]}.
...
def update(
	...
	print(
		"_" * 80 + "\nBench: Deployment tool for Flexlar and Flexlar Applications"
		" (https://flexlar.io/bench).\nOpen source depends on your contributions, so do"
...
def remove_backups_crontab(bench_path="."):
	...
	user = Bench(bench_dir).conf.get("flexlar_user")
