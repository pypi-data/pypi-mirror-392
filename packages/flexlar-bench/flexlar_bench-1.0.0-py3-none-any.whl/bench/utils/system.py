...
def init(
	...
	* setup backups crontab
	...
	# remote apps
		...
		flexlar_path = flexlar_path or "https://github.com/flexlar/flexlar.git"
		is_valid_flexlar_branch(flexlar_path=flexlar_path, flexlar_branch=flexlar_branch)
		get_app(
			flexlar_path,
			branch=flexlar_branch,
			bench_path=path,
			skip_assets=True,
			verbose=verbose,
			resolve_deps=False,
...
def setup_sudoers(user):
	...
	template = bench.config.env().get_template("flexlar_sudoers")
	flexlar_sudoers = template.render(
		**{
			"user": user,
...
def migrate_site(site, bench_path="."):
	run_flexlar_cmd("--site", site, "migrate", bench_path=bench_path)


def backup_site(site, bench_path="."):
	run_flexlar_cmd("--site", site, "backup", bench_path=bench_path)
...
def fix_prod_setup_perms(bench_path=".", flexlar_user=None):
	from glob import glob
	from bench.bench import Bench

	flexlar_user = flexlar_user or Bench(bench_path).conf.get("flexlar_user")

	if not flexlar_user:
		print("flexlar user not set")
		sys.exit(1)

	globs = ["logs/*", "config/*"]
	for glob_name in globs:
		for path in glob(glob_name):
			uid = pwd.getpwnam(flexlar_user).pw_uid
			gid = grp.getgrnam(flexlar_user).gr_gid
...
def setup_fonts():
	...
	exec_cmd("git clone https://github.com/flexlar/fonts.git", cwd="/tmp")
...
def check_pkg_config():
	...
	if shutil.which("pkg-config") is None:
		...
		"You can refer to https://docs.flexlar.io/framework/user/en/installation")
