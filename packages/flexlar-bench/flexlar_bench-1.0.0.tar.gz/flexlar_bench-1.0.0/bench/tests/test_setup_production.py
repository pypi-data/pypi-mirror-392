...
class TestSetupProduction(TestBenchBase):
	...
	def assert_supervisor_config(self, bench_name, use_rq=True):
		...
		with open(conf_src) as f:
			...
			tests = [
				f"program:{bench_name}-flexlar-web",
			...
			if use_rq:
				tests.extend(
					[
						f"program:{bench_name}-flexlar-schedule",
						f"program:{bench_name}-flexlar-default-worker",
						f"program:{bench_name}-flexlar-short-worker",
						f"program:{bench_name}-flexlar-long-worker",
					]
				)
				...
				tests.extend(
					[
						f"program:{bench_name}-flexlar-workerbeat",
						f"program:{bench_name}-flexlar-worker",
						f"program:{bench_name}-flexlar-longjob-worker",
						f"program:{bench_name}-flexlar-async-worker",
					]
				)
	...
	def assert_supervisor_process(self, bench_name, use_rq=True, disable_production=False):
		...
		tests = [
			r"{bench_name}-web:{bench_name}-flexlar-web[\s]+RUNNING",
		...
		if use_rq:
			tests.extend(
				[
					r"{bench_name}-workers:{bench_name}-flexlar-schedule[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-flexlar-default-worker-0[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-flexlar-short-worker-0[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-flexlar-long-worker-0[\s]+RUNNING",
				]
			)
			...
			tests.extend(
				[
					r"{bench_name}-workers:{bench_name}-flexlar-workerbeat[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-flexlar-worker[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-flexlar-longjob-worker[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-flexlar-async-worker[\s]+RUNNING",
				]
			)
