...
def generate_systemd_config(
	...
	for i in range(number_of_workers):
		background_workers.append(
			get_bench_name(bench_path) + "-flexlar-default-worker@" + str(i + 1) + ".service"
		)

	for i in range(number_of_workers):
		background_workers.append(
			get_bench_name(bench_path) + "-flexlar-short-worker@" + str(i + 1) + ".service"
		)

	for i in range(number_of_workers):
		background_workers.append(
			get_bench_name(bench_path) + "-flexlar-long-worker@" + str(i + 1) + ".service"
		)
...
def setup_main_config(bench_info, bench_path):
	# Main config
	bench_template = bench.config.env().get_template("systemd/flexlar-bench.target")
...
def setup_workers_config(bench_info, bench_path):
	...
	bench_workers_target_template = bench.config.env().get_template(
		"systemd/flexlar-bench-workers.target"
	)
	bench_default_worker_template = bench.config.env().get_template(
		"systemd/flexlar-bench-flexlar-default-worker.service"
	)
	bench_short_worker_template = bench.config.env().get_template(
		"systemd/flexlar-bench-flexlar-short-worker.service"
	)
	bench_long_worker_template = bench.config.env().get_template(
		"systemd/flexlar-bench-flexlar-long-worker.service"
	)
	bench_schedule_worker_template = bench.config.env().get_template(
		"systemd/flexlar-bench-flexlar-schedule.service"
	)
	...
	bench_default_worker_config_path = os.path.join(
		...
		"config",
		"systemd",
		bench_info.get("bench_name") + "-flexlar-default-worker@.service",
	)
	bench_short_worker_config_path = os.path.join(
		bench_path,
		"config",
		"systemd",
		bench_info.get("bench_name") + "-flexlar-short-worker@.service",
	)
	bench_long_worker_config_path = os.path.join(
		bench_path,
		"config",
		"systemd",
		bench_info.get("bench_name") + "-flexlar-long-worker@.service",
	)
	bench_schedule_worker_config_path = os.path.join(
		bench_path,
		"config",
		"systemd",
		bench_info.get("bench_name") + "-flexlar-schedule.service",
	)
...
def setup_web_config(bench_info, bench_path):
	...
	bench_web_target_template = bench.config.env().get_template(
		"systemd/flexlar-bench-web.target"
	)
	bench_web_service_template = bench.config.env().get_template(
		"systemd/flexlar-bench-flexlar-web.service"
	)
	bench_node_socketio_template = bench.config.env().get_template(
		"systemd/flexlar-bench-node-socketio.service"
	)
	...
	bench_web_service_config_path = os.path.join(
		bench_path, "config", "systemd", bench_info.get("bench_name") + "-flexlar-web.service"
	)
...
def setup_redis_config(bench_info, bench_path):
	...
	bench_redis_target_template = bench.config.env().get_template(
		"systemd/flexlar-bench-redis.target"
	)
	bench_redis_cache_template = bench.config.env().get_template(
		"systemd/flexlar-bench-redis-cache.service"
	)
	bench_redis_queue_template = bench.config.env().get_template(
		"systemd/flexlar-bench-redis-queue.service"
	)
...
def get_unit_files(bench_path):
	...
	unit_files = [
		...
		[bench_name + "-flexlar-default-worker@", ".service"],
		[bench_name + "-flexlar-short-worker@", ".service"],
		[bench_name + "-flexlar-long-worker@", ".service"],
		[bench_name + "-flexlar-schedule", ".service"],
		[bench_name + "-flexlar-web", ".service"],
