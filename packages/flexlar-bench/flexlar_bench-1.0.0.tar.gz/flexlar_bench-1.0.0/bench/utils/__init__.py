...
def is_valid_flexlar_branch(flexlar_path: str, flexlar_branch: str):
	...
	if flexlar_branch:
		try:
			res = g.ls_remote("--heads", "--tags", flexlar_path.replace("https://github.com", f"https://{token}@github.com") if (token := os.environ.get("GH_TOKEN")) else flexlar_path, flexlar_branch)
			if not res:
...
def find_org(org_repo, using_cached: bool = False):
	...
	for org in ["flexlar", "erpnext"]:
		res = requests.head(f"https://api.github.com/repos/{org}/{org_repo}", headers={"Authorization": f"Bearer {token}"} if (token := os.environ.get("GH_TOKEN")) else {})
