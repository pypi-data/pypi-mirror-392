...
def get_tableau_auth_object(config: 'Config') -> 'Credentials':
    ...
    if not config.tableau:
        raise ValueError('No tableau configuration found in dmp_af config. Please specify tableau credentials.')
    ...
    raise ValueError(
        'No valid tableau authentication method found in dmp_af config. Please specify tableau credentials.'
    )
