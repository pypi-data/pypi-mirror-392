...
import pandas as pd
from pandas.io.formats.style import refactor_levels
...
class DataProcessor:
    ...
    def match_and_analyze_data(
        ...
        if historical_ref is not None:
            modeling_ref = pd.concat([matched_ref_df, historical_ref], axis=0, ignore_index=True)
            modeling_merged = pd.concat([matched_merged_df, historical_merged], axis=0, ignore_index=True)
            modeling_measure = pd.concat(
                [matched_measure_df, historical_measure], axis=0, ignore_index=True
            )

            model_result = modeler.fit(
                spectrum_data=modeling_ref,
            ...
            model_result = modeler.fit(
                spectrum_data=matched_ref_df,
                metric_data=matched_measure_df,
                old_predictions=matched_merged_df,
            )

        model_func = model_result.model_data
        pred_data = model_result.predict_unified(matched_ref_df, matched_merged_df)
        all_pred_data = model_result.predict_unified(ref_data, merged_data)
