...
from xfyunsdkcore.errors import FaceStatusError


class FaceStatusClient(HttpClient):
    """Client for 配合式活体检测 Face Status
    ...
    def _build_param(self, image_base64: str, image_format: str) -> Dict[str, Any]:
        """Build request parameters for Face Status."""
        try:
            param: Dict[str, Any] = {
                "header": {
                    "app_id": self.app_id,
            ...
            logger.debug(f"Face Status Request Parameters: {param}")
            return param
        except Exception as e:
            logger.error(f"Failed to build parameters: {e}")
            raise FaceStatusError(f"Failed to build parameters: {e}")
