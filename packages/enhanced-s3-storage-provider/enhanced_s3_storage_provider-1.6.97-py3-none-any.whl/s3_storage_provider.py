        ...
        if self._s3_client is None:
            with self._s3_client_lock:
                if self._s3_client is None:
                    ...
                    # Disable payload signing for better compatibility with Swift S3 API
                    from botocore.client import Config
                    boto_config = Config(
                        signature_version='s3v4',
                        max_pool_connections=self.max_pool_connections,
                        # Disable chunked encoding in requests for better OpenStack compatibility
                        s3={'payload_signing_enabled': False},
                        # Ensure proper HTTP adapter configuration
