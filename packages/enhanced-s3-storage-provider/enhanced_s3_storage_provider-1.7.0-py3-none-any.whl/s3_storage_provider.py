    ...
    def _get_s3_client(self):
        ...
        if self._s3_client is None:
            with self._s3_client_lock:
                if self._s3_client is None:
                    # Configure boto3 for OpenStack/OBS compatibility
                    # Disable payload signing and chunked encoding for Swift S3 API
                    from botocore.client import Config
                    boto_config = Config(
                        signature_version='s3v4',
                        max_pool_connections=self.max_pool_connections,
                        # Critical: Disable payload signing to prevent chunked encoding corruption
                        s3={
                            'payload_signing_enabled': False,
                            'addressing_style': 'path'  # Use path-style for OpenStack
                        },
                    ...
                    # Monkey-patch to force disable chunked transfer encoding
                    # This is necessary for OpenStack Swift compatibility
                    import botocore.endpoint
                    original_make_request = client._endpoint.make_request
                    
                    def patched_make_request(operation_model, request_dict, *args, **kwargs):
                        # Remove Transfer-Encoding: chunked header if present
                        if 'headers' in request_dict:
                            request_dict['headers'].pop('Transfer-Encoding', None)
                        return original_make_request(operation_model, request_dict, *args, **kwargs)
                    
                    client._endpoint.make_request = patched_make_request
...
def _is_chunked_encoded(data):
    """Detect if data starts with HTTP chunked transfer encoding markers.
    ...
    try:
        # Look for hex number followed by \r\n within first 16 bytes
        crlf_pos = data.find(b'\r\n')
        if crlf_pos == -1 or crlf_pos > 16:
            return False
        
        # Try to parse as hex
        hex_str = data[:crlf_pos].strip()
        if not hex_str:
            return False
        
        chunk_size = int(hex_str, 16)
        
        # Sanity check: chunk size should be reasonable (1 byte to 100MB)
        if chunk_size < 1 or chunk_size > 100 * 1024 * 1024:
            return False
        
        logger.debug("[CHUNKED_DETECT] Found potential chunk marker: %s (%d bytes)", hex_str.decode('ascii', errors='ignore'), chunk_size)
        return True
    except (ValueError, UnicodeDecodeError):
        return False


def _decode_chunked_transfer(data):
    ...
    while pos < data_len:
        # Find the \r\n that marks end of chunk size
        crlf_pos = data.find(b'\r\n', pos)
        if crlf_pos == -1:
            # No more chunk markers, append remaining data
            decoded.extend(data[pos:])
            break
        
        # Extract chunk size hex string
        chunk_size_hex = data[pos:crlf_pos].strip()
        ...
        try:
            ...
            # Not a valid hex number, might be actual data
            decoded.extend(data[pos:])
            break
        
        # Check for end marker (0\r\n)
        if chunk_size == 0:
            logger.debug("[CHUNKED_DECODE] Found end marker at position %d", pos)
            break
        ...
        if chunk_end > data_len:
            logger.warning("[CHUNKED_DECODE] Chunk size %d exceeds remaining data, appending rest", chunk_size)
        ...
        # Move to next chunk (skip the \r\n after chunk data)
        pos = chunk_end + 2
    
    return bytes(decoded)


def _stream_bytes_to_producer(reactor, producer, data):
    """Stream bytes data to producer in chunks."""
    pos = 0
    data_len = len(data)
    
    while pos < data_len and not producer.stop_event.is_set():
        chunk_end = min(pos + READ_CHUNK_SIZE, data_len)
...
def s3_download_task(s3_client, bucket, key, extra_args, deferred, parent_logcontext):
    ...
    with LoggingContext(parent_context=parent_logcontext):
        ...
        # CRITICAL FIX: Check if OpenStack is returning chunked-encoded data
        # Read a small sample to detect chunked encoding markers
        try:
            sample = body.read(64)
            if _is_chunked_encoded(sample):
                logger.warning("[S3_FETCH] Detected chunked encoding in response for key=%s, decoding...", key)
                # Read the rest and decode the entire response
                rest = body.read()
                full_data = sample + rest
                decoded_data = _decode_chunked_transfer(full_data)
                logger.info("[S3_FETCH] Decoded %d bytes -> %d bytes for key=%s", len(full_data), len(decoded_data), key)
                # Stream the decoded data
                _stream_bytes_to_producer(reactor, producer, decoded_data)
            else:
                # Not chunked, stream normally (put sample back by streaming it first)
                reactor.callFromThread(producer._write, sample)
                _stream_to_producer(reactor, producer, body, timeout=90.0)
        except Exception as e:
            logger.error("[S3_FETCH] Error processing response for key=%s: %s", key, str(e), exc_info=True)
