...
import os
import sys
from typing import Optional, Union, Callable, List, Tuple, Coroutine
...
class GNServer(App):
    ...
    def run( # type: ignore
        ...
        *,
        ...
        kdc_passive_key_sync_domains: Optional[List[Tuple[int, str, bytes]]] = None,
        kdc_requested_domains: List[str] = [],
        ...
        kdc_active_key_synchronization_callback_domainFilter: Optional[List[str]] = None
    ):
        ...
        self.client.init(gn_server_crt,
                         requested_domains=kdc_requested_domains,
                         active_key_synchronization=kdc_active_key_synchronization,
                         active_key_synchronization_callback=kdc_active_key_synchronization_callback,
                         active_key_synchronization_callback_domainFilter=kdc_active_key_synchronization_callback_domainFilter
                         )


        @self.addEventListener('start', move_to_start=True) # type: ignore
        async def _on_start():
            await self.client._kdc.addServers(servers_keys=kdc_passive_key_sync_domains)
    ...
    def runByVMHost(self,
        ...
        if data['command'] == 'gn:vm-host:start':
            self.run(
                ...
                kdc_requested_domains=kdc_requested_domains,
                kdc_active_key_synchronization=kdc_active_key_synchronization,
                kdc_active_key_synchronization_callback=kdc_active_key_synchronization_callback,
                kdc_active_key_synchronization_callback_domainFilter=kdc_active_key_synchronization_callback_domainFilter
