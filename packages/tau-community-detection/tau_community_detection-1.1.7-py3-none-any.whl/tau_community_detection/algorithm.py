...
from .graph import networkx_to_igraph, load_graph
...
class TauClustering:
    ...
    def __init__(self, graph_source: ig.Graph | nx.Graph | str, population_size, max_generations, config: Optional[TauConfig] = None):
    ...
    def _prepare_graph_source(
        ...
        if isinstance(graph_source, str):
            return load_graph(graph_source, weight_attribute=weight_attribute, default_weight=default_weight)
        raise TypeError(
            "TauClustering expects an igraph.Graph or networkx.Graph instance or a file path string."
        )
    ...
    def _ensure_pool(self, worker_count: int) -> Pool:
        if self._pool is not None and self._pool_processes == worker_count:
            return self._pool
        self._shutdown_pool()
        graph_arg = self.graph_path if hasattr(self, 'graph_path') and self.graph_path else self.graph
