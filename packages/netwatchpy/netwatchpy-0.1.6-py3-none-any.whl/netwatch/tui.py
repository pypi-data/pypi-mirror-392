...
class NetMonitorTUI(App):
    ...
    def __init__(self, interface="all", limit_str=None, log_file=None):
        ...
        # Add properties to cache widgets
        self.total_dl_card: Static | None = None
        self.total_ul_card: Static | None = None
        self.total_usage_card: Static | None = None

        # Check if notification backend is available
        self.notification_backend_available = True
        if platform.system() == "Linux" and dbus_next is None:
            self.notification_backend_available = False
            print(
                "\n[Netwatch WARNING] `dbus-next` library not found. "
                "Linux desktop notifications will be disabled.\n"
                "To enable them, please install the required extras:\n"
                "  pip install \"desktop-notifier[dbus]\"\n",
                file=sys.stderr
            )
        ...
        self._initial_upload, self._initial_download = self._load_persistent_quota()
    ...
    def compose(self) -> ComposeResult:
        ...
        with VerticalScroll(id="main_container"):
            with Horizontal(id="summary_cards"):
                yield Static(f"Total Download\n[b]{get_size(self._initial_download)}[/b]", id="total-dl-card", classes="summary_card")
                yield Static(f"Total Upload\n[b]{get_size(self._initial_upload)}[/b]", id="total-ul-card", classes="summary_card")
                yield Static(f"Total Usage\n[b]{get_size(self._initial_upload + self._initial_download)}[/b]", id="total-usage-card", classes="summary_card")
    ...
    def on_mount(self):
        ...
        # Cache summary card widgets for performance and to avoid race conditions
        try:
            self.total_dl_card = self.query_one("#total-dl-card", Static)
            self.total_ul_card = self.query_one("#total-ul-card", Static)
            self.total_usage_card = self.query_one("#total-usage-card", Static)
        except Exception as e:
            print(f"[Netwatch TUI Error] Could not find summary card widgets on mount: {e}", file=sys.stderr)
            self.exit() # Exit if UI can't be found
        ...
        self.total_upload = self._initial_upload
        self.total_download = self._initial_download
        self.total_usage = self._initial_upload + self._initial_download
        ...
        self.monitor_thread = NetworkMonitorThread(
            ...
            initial_upload=self._initial_upload,
            initial_download=self._initial_download,
        )
    ...
    def _process_data_packet(self, data: dict):
        ...
        if table.row_count > 50:
            try:
                ...
                pass # Table is already empty

    def watch_total_download(self, new):
        if self.total_dl_card: # Use cached widget
            self.total_dl_card.update(
                f"Total Download\n[b]{get_size(new)}[/b]"
            )

    def watch_total_upload(self, new):
        if self.total_ul_card: # Use cached widget
            self.total_ul_card.update(
                f"Total Upload\n[b]{get_size(new)}[/b]"
            )

    async def watch_total_usage(self, new_total_usage: int):
        if self.total_usage_card: # Use cached widget
            self.total_usage_card.update(
            ...
            if new_total_usage >= 0.8 * self.limit_bytes and not self.alert_80_sent:
                ...
                if self.notification_backend_available:
                    try:
                        ...
                        self.notification_backend_available = False
                        self.sub_title = "Notifications disabled (backend error)"

            # 100% alert
            if new_total_usage >= self.limit_bytes and not self.alert_100_sent:
                self.alert_100_sent = True
                if 'bar' in locals(): bar.styles.color = "red"
    ...
    def action_reset_counters(self):
        ...
        if self.limit_bytes:
            try:
                bar = self.query_one(ProgressBar)
                bar.styles.color = None # Reset to default
            except Exception:
                pass
