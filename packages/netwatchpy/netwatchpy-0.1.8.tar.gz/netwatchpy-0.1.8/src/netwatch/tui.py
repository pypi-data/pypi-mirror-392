...
class NetMonitorTUI(App):
    ...
    def __init__(self, interface="all", limit_str=None, log_file=None):
        ...
        # Check for Linux notification backend
    ...
    def on_exit(self):
        ...
        if self._save_lock and self._save_lock.acquire(timeout=2.0):
            try:
    ...
    def _autosave_job(self):
        ...
        if self._save_lock and self._save_lock.acquire(timeout=0.1):
            try:
                self._save_persistent_quota()
            finally:
                self._save_lock.release()
    ...
    async def watch_total_usage(self, new_total_usage: int):
        ...
        if self.limit_bytes:
            try:
                ...
                pass # It will catch up on the next update
    ...
    def action_reset_counters(self):
        ...
        if self._save_lock and self._save_lock.acquire(timeout=1.0):
            try:
                self._save_persistent_quota()
            finally:
                self._save_lock.release()
