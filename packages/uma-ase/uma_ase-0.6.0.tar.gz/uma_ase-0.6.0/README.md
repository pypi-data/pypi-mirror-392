...
- **Visualize tab.** The GUI bundles a JSmol canvas that activates when you switch to *Visualize*. Whatever geometry you upload on *Single Run* mirrors automatically: charge/spin inputs stay in sync, the file name appears in both tabs, and the 3‑D model renders as soon as the viewer becomes visible.
