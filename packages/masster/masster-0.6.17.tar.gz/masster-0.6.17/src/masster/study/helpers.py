...
def migrate_map_id_to_sample_id(self):
    """
    Migrate old map_id column to new sample_id column with UUID7 values.

    This function handles backward compatibility by:
    1. Removing the old map_id column if it exists
    2. Creating a new sample_id column with UUID7 values if it doesn't exist

    This migration is needed for studies that were created before the sample_id
    ...
    if "sample_id" in self.samples_df.columns:
        # If map_id also exists, remove it
        if "map_id" in self.samples_df.columns:
            self.samples_df = self.samples_df.drop("map_id")
            self.logger.info("Removed old map_id column")
        else:
            self.logger.info("sample_id column already exists - no migration needed")
        return

    self.logger.info("Migrating from map_id to sample_id with UUID7 values")

    # Generate new UUID7 values for each sample
    ...
    self.logger.info(
        f"Successfully migrated {sample_count} samples to UUID7-based sample_id format",
    )
