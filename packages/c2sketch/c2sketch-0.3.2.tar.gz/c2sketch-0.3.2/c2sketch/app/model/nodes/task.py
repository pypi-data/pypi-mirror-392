...
from toppyt import (ParallelTask, TaskResult, TaskStatus, after_task,
                    ...
                    write_information)
from toppyt.bulma import BulmaButtons, BulmaTable
...
__all__ = ('edit_task_node',)
def edit_task_node(model_store: ModelStore, plugins, model_id, task_id, path_var):
    ...
    def choose(general):
        ...
        return ParallelTask ([
            ...
            ('main',choose_task([
            ('Attributes','file-alt', edit_task_attributes(model_store, model_id, task_id)),
            ...
            ('Source','code', edit_task_source(model_store,model_id,task_id, path_var)),
            ],'Attributes'))
        ],layout=layout)
...
def edit_task_attributes(model_store: ModelStore, model_id: ModelID, task_id: TaskID):
    def view():
    ...
    def save(updated_fields):
        return write_information(model_store,lambda ps:ps.write_task_attributes(model_id,task_id,updated_fields))
...
def edit_info_reqs(model_store: ModelStore, model_id: ModelID, task_id: TaskID, path_var: TaskVariable[str]):
    ...
    def create_info_req():
        ...
        def save(value):
            ...
            return write_information(model_store, lambda ms: ms.task_create_info_req(model_id,task_id,name,type,read,write,binding))
    ...
    def update_info_req(req_name:str):
        def load():
            async def read(ms: ModelStore):
                req = await ms.task_get_info_req(model_id,task_id,req_name)
                value = {'name': req_name}
        ...
        def save(value):
            ...
            write = 'readwrite' in value and value['readwrite'] in ['write','readwrite']
            binding = None if 'binding' not in value or value['binding'] == '' else value['binding']
            return write_information(model_store, lambda ms: ms.task_update_info_req(model_id,task_id,name,type,read,write,binding))
    ...
    def delete_info_req(req_name: str):
        return write_information(model_store, lambda ms: ms.task_delete_info_req(model_id,task_id,req_name))
    ...
    def select_action():
        ...
        def choose_list_action():
            return with_information(model_store,
                lambda ms:ms.task_info_space_requirements(model_id,task_id),
...
def edit_task_contraints(model_store: ModelStore, model_id, task_id):
    def view():
        editor = BulmaRecordEditor([
            ('for_actor',BulmaTextInput(label = 'For actor', disabled= True)),
            ('at_location',BulmaTextInput(label = 'At location', disabled = True))
        ])
        return view_information(model_store,lambda ms:ms.task_constraints(model_id, task_id), editor=editor)
    ...
    def save(updated_fields):
        return write_information(model_store,lambda ms:ms.write_task_constraints(model_id,task_id,updated_fields))

    return view_with_edit(view(),edit,save)

def edit_task_source(model_store: ModelStore, model_id: ModelID, task_id: TaskID, path_var: TaskVariable[str]):
    ...
    def save(value):
        return after_task(write_information(model_store,lambda ms: ms.write_task_source(model_id,task_id,value)),
    ...
    def task_node_path(model_id, node_id):
        return f'/model/{model_id}' if node_id is None else f'/model/{model_id}/t/{node_id}'
