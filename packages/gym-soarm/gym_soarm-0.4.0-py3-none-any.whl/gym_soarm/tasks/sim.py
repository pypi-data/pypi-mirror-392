...
# Optional imports for model-based rewards
try:
    import torch
    ...
    print("Warning: torch and/or lerobot not available. Using math-based reward mode only.")
...
class SoArmTask(base.Task):
    ...
    def get_observation(self, physics):
        ...
        obs["images"]["wrist.right"] = physics.render(height=480, width=640, camera_id="wrist_camera")
        obs["images"]["diagonal"] = physics.render(height=480, width=640, camera_id="diagonal_camera")
...
class PickPlaceTask(SoArmTask):
    def __init__(self, random=None, reward_mode='math'):
        ...
        if reward_mode == 'model':
            if not TORCH_AVAILABLE:
                raise ImportError("torch and lerobot are required for model-based reward mode")
    ...
    def initialize_episode(self, physics):
        ...
        with physics.reset_context():
            ## NNeed to run after reset_context()
            super().initialize_episode(physics)
    ...
    def _get_reward(self, physics):
        if not TORCH_AVAILABLE:
            raise ImportError("torch is required for model-based reward computation")
        ...
        observation['observation.images.wrist.right'] = torch.tensor(current_gym_obs['images']['wrist.right'].copy()) \
        ...
        observation['observation.images.diagonal'] = torch.tensor(current_gym_obs['images']['diagonal'].copy()) \
                                                            .type(torch.float32) / 255
