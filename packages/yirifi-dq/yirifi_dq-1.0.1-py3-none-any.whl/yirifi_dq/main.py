...
@click.group()
@click.version_option(version="1.0.1", prog_name="yirifi-dq")
@click.pass_context
def cli(ctx):
    """
...
def init():
    """
    Initialize yirifi-dq configuration.

    Creates the config directory and a template .env file for MongoDB connections.

    The config file is created at: ~/.config/yirifi-dq/.env
    ...
    config_dir = Path.home() / ".config" / "yirifi-dq"
    ...
    if not config_dir.exists():
        ...
        console.print(f"[yellow]→[/yellow] Config directory already exists: {config_dir}")

    # Create template .env file
    if not env_file.exists():
        template = """# Yirifi Data Quality CLI Configuration
# MongoDB connection strings for each environment

# Production environment
PRD_MONGODB_URI=mongodb://username:password@host:port/database

# Development environment
DEV_MONGODB_URI=mongodb://localhost:27017/regdb

# UAT environment (optional)
# UAT_MONGODB_URI=mongodb://username:password@host:port/database
"""
        env_file.write_text(template)
        console.print(f"[green]✓[/green] Created config file: {env_file}")
        console.print("\n[bold cyan]Next steps:[/bold cyan]")
        console.print(f"  1. Edit {env_file}")
        console.print("  2. Add your MongoDB connection strings")
        console.print("  3. Run: yirifi-dq list")
    else:
        console.print(f"[yellow]→[/yellow] Config file already exists: {env_file}")
        console.print("\n[dim]To reset, delete the file and run init again.[/dim]")
