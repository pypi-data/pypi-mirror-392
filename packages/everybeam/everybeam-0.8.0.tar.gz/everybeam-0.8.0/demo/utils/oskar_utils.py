...
def read_oskar_beams(root_name):
    ...
    for i, pol in enumerate(["X", "Y"]):
        for j, coord in enumerate(["THETA", "PHI"]):
            filename_amp = f"{root_name}_MHz_S0000_TIME_SEP_CHAN_SEP_AMP_{pol}_{coord}.fits"
            filename_phase = f"{root_name}_MHz_S0000_TIME_SEP_CHAN_SEP_PHASE_{pol}_{coord}.fits"
            d = (
