...
"""
Script for generating the H5PARMATERM_MOCK.h5 file, used for unittesting the
...
for i, tampl in enumerate(time_array_ampl):
    ...
    h5writer.fill_solution_table(
        "amplitude1_coefficients", amplitude_coeffs_tmp, offset_amplitude
    )
    h5writer.fill_solution_table(
        "amplitude2_coefficients", amplitude_coeffs_tmp + 1.0, offset_amplitude
    )
    h5writer.fill_solution_table(
        "slowphase1_coefficients",
        np.zeros_like(amplitude_coeffs_tmp),
        offset_amplitude,
    )
    h5writer.fill_solution_table(
        "slowphase2_coefficients",
        np.ones_like(amplitude_coeffs_tmp),
        offset_amplitude,
    )
