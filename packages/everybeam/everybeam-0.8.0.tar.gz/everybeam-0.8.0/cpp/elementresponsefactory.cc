...
std::shared_ptr<const ElementResponse> ElementResponse::GetInstance(
  ...
  switch (options.element_response_model) {
    ...
    case ElementResponseModel::kOSKARDipoleCos:
      return std::make_shared<OSKARElementResponseDipoleCos>();
    ...
    case ElementResponseModel::kSkaLowFeko:
      return SkaLowElementResponse::GetInstance("SkaLowFeko.h5", options);
    ...
    default:
      ...
      message << "The requested element response model '"
              << options.element_response_model << "' is not implemented.";
