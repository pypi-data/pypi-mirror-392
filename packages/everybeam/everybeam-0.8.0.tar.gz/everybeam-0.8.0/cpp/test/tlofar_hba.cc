...
BOOST_AUTO_TEST_CASE(element_response) {
  ...
  station63.Response(&station63_response, time, frequency_list, direction_s63,
                     std::span(&frequency, 1), station0_dir, tile0_dir);
...
BOOST_AUTO_TEST_CASE(element_response_multi) {
  ...
  station63.ArrayFactor(multi_array_factor.data(), time, frequency_list,
                        direction_s63, frequency_list, station0_dir, tile0_dir);
  ...
  station63.ArrayFactor(&single_array_factor, time, single_frequency,
                        direction_s63, single_frequency, station0_dir,
                        tile0_dir);
...
BOOST_AUTO_TEST_CASE(hba_multi_freq_compare) {
  // Setup.
  const vector3r_t direction = {0.397408, 0.527527, 0.750855};
  const std::vector<double> frequency_list({frequency, frequency + 2000});

  std::vector<aocommon::MC2x2> multi_buffer(frequency_list.size());
  std::vector<aocommon::MC2x2> single_buffer(frequency_list.size());

  point_response->Response(multi_buffer.data(), BeamMode::kFull, 11,
                           frequency_list, direction);

  // Check full response.
  for (size_t f = 0; f < frequency_list.size(); f++) {
    point_response->Response(single_buffer.data(), BeamMode::kFull, 11,
  ...
  // Setup.
  point_response->Response(multi_buffer.data(), BeamMode::kArrayFactor, 11,
                           frequency_list, direction);
  // Check array factor.
  for (size_t f = 0; f < frequency_list.size(); f++) {
    point_response->Response(single_buffer.data(), BeamMode::kArrayFactor, 11,
                             std::span(&frequency_list[f], 1), direction);
  ...
  // Setup.
  point_response->Response(multi_buffer.data(), BeamMode::kElement, 11,
                           frequency_list, direction);
  // Check element response.
  for (size_t f = 0; f < frequency_list.size(); f++) {
    point_response->Response(single_buffer.data(), BeamMode::kElement, 11,
                             std::span(&frequency_list[f], 1), direction);
...
BOOST_AUTO_TEST_CASE(gridded_response_array_factor) {
  ...
  station.Response(&full_beam_p13, time, std::span(&frequency, 1),
                   direction_p13, std::span(&frequency, 1), station0, station0);
  ...
  station.ArrayFactor(&array_factor_p13, time, std::span(&frequency, 1),
                      direction_p13, std::span(&frequency, 1), station0,
                      station0);
  ...
  station.Response(&full_beam_p22, time, std::span(&frequency, 1),
                   direction_p22, std::span(&frequency, 1), station0, station0);
  ...
  station.ArrayFactor(&array_factor_p22, time, std::span(&frequency, 1),
                      direction_p22, std::span(&frequency, 1), station0,
                      station0);
...
BOOST_AUTO_TEST_CASE(integrated_beam) {
  ...
  aocommon::CoordinateSystem coord_system_pb = {width_pb,
                                                height_pb,
                                                coord_system.ra,
                                                coord_system.dec,
                                                (0.25 * M_PI / 180.),
