...
const aocommon::CoordinateSystem kCoordSystem = {
...
BOOST_AUTO_TEST_CASE(under_horizon_grid) {
  ...
  const aocommon::CoordinateSystem under_horizon = {
      width, height, kRa, kDec, 0.4, 0.4, kShiftL, kShiftM};
  std::unique_ptr<GriddedResponse> grid_response =
...
BOOST_AUTO_TEST_CASE(point_response_lobes, *boost::unit_test::tolerance(1e-8)) {
  ...
  for (int i = 0; i < 288; ++i) {
    ...
    aocommon::MC2x2 response_lofar;
    point_lofar.ElementResponse(&response_lofar, lofar_station,
                                std::span(&kFrequency, 1), kDirection,
                                lofar_element);
    aocommon::MC2x2 response_aartfaac;
    point_aartfaac.ElementResponse(&response_aartfaac, i,
                                   std::span(&kFrequency, 1), kDirection,
                                   lofar_element);

    BOOST_CHECK_EQUAL(response_lofar.Get(0), response_aartfaac.Get(0));
    BOOST_CHECK_EQUAL(response_lofar.Get(1), response_aartfaac.Get(1));
    BOOST_CHECK_EQUAL(response_lofar.Get(2), response_aartfaac.Get(2));
    BOOST_CHECK_EQUAL(response_lofar.Get(3), response_aartfaac.Get(3));
  }
}
