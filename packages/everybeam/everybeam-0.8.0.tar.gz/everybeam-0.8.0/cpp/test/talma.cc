...
using everybeam::telescope::Alma;
using everybeam::telescope::Telescope;

BOOST_AUTO_TEST_SUITE(talma)

BOOST_AUTO_TEST_CASE(load_alma) {
  Options options;
  casacore::MeasurementSet ms(ALMA_MOCK_MS);

  std::unique_ptr<Telescope> telescope = Load(ms, options);

  // Assert if we indeed have a ALMA pointer
  BOOST_CHECK(nullptr != dynamic_cast<Alma*>(telescope.get()));
  ...
  BOOST_CHECK(nullptr != dynamic_cast<AiryGrid*>(grid_response.get()));
  ...
  // image ALMABeam output at pixel (0, 0):
  std::vector<std::complex<float>> alma_p00 = {
      {0.93442690372467, 0.}, {0, 0}, {0, 0}, {0.93442690372467, 0.}};
  // ALMABeam output at pixel (2, 3):
  std::vector<std::complex<float>> alma_p23 = {
      {0.977381765842438, 0.}, {0, 0}, {0, 0}, {0.977381765842438, 0.}};
  // ALMABeam output at pixel (10, 12):
  std::vector<std::complex<float>> alma_p1012 = {
      {0.996755182743073, 0.}, {0, 0}, {0, 0}, {0.996755182743073, 0.}};
