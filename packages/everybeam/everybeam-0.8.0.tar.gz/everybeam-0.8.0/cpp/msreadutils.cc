...
constexpr StationCoordinateSystem::Axes oskar_antenna_orientation =
    StationCoordinateSystem::kIdentityAxes;

constexpr StationCoordinateSystem::Axes lofar_antenna_orientation = {
    {
        -M_SQRT1_2,
        -M_SQRT1_2,
        0.0,
...
void TransformToFieldCoordinates(TileConfig& config,
                                 const StationCoordinateSystem::Axes& axes) {
  for (auto& val : config) {
...
vector3r_t TransformToFieldCoordinates(
    const vector3r_t& position, const StationCoordinateSystem::Axes& axes) {
...
std::shared_ptr<BeamFormer> MakeTile(const vector3r_t& position,
  ...
  for (unsigned int id = 0; id < tile_config.size(); id++) {
    ...
    StationCoordinateSystem antenna_coordinate_system;
...
std::shared_ptr<Antenna> ReadAntennaFieldLofar(
  ...
  StationCoordinateSystem coordinate_system =
  ...
  for (size_t i = 0; i < aips_offset.ncolumn(); ++i) {
    ...
    if (name == "LBA") {
      ...
      if (element_response_model == ElementResponseModel::kHamaker) {
        ...
        antenna->SetEnabled(!aips_flag(0, i), !aips_flag(1, i));
...
std::shared_ptr<BeamFormer> LofarStationBeamFormer(
  ...
  beam_former =
      std::make_shared<BeamFormer>(kIdentityCoordinateSystem, phase_reference);
...
// Creates an element for station number `station_id` at the position read from
// the ms The coordinate system is aligned to the local east, north, up
// directions.
std::shared_ptr<Element> MakeLocalEastNorthUpElement(const MeasurementSet& ms,
                                                     size_t station_id) {
  Table antenna_table = ms.antenna();
  casacore::ScalarMeasColumn<casacore::MPosition> position_column(antenna_table,
                                                                  "POSITION");

  casacore::MPosition position = position_column(station_id);

  casacore::MeasFrame frame(position);

  casacore::MDirection::Ref meas_ref(casacore::MDirection::AZEL, frame);
  casacore::MDirection east(casacore::MVDirection(0.0, 1.0, 0.0), meas_ref);
  casacore::MDirection north(casacore::MVDirection(1.0, 0.0, 0.0), meas_ref);
  casacore::MDirection up(casacore::MVDirection(0.0, 0.0, 1.0), meas_ref);

  casacore::MDirection::Convert converter(meas_ref, casacore::MDirection::ITRF);
  casacore::MVDirection east_itrf_v = converter(east).getValue();
  casacore::MVDirection north_itrf_v = converter(north).getValue();
  casacore::MVDirection up_itrf_v = converter(up).getValue();

  // define casacore topocentric east-north-up directions
  // transform directions to itrf

  const MVPosition position_v = position.getValue();
  const vector3r_t origin = {position_v(0), position_v(1), position_v(2)};

  const vector3r_t p = {east_itrf_v(0), east_itrf_v(1), east_itrf_v(2)};
  const vector3r_t q = {north_itrf_v(0), north_itrf_v(1), north_itrf_v(2)};
  const vector3r_t r = {up_itrf_v(0), up_itrf_v(1), up_itrf_v(2)};

  const StationCoordinateSystem coordinate_system{origin, {p, q, r}};

  const size_t id = 0;
  return std::make_shared<Element>(coordinate_system, id);
}
...
std::unique_ptr<Station> ReadSingleStation(const casacore::MeasurementSet& ms,
  ...
  if (telescope_type == TelescopeType::kOSKARTelescope) {
    ...
    // LWA telescope
    station->SetAntenna(MakeLocalEastNorthUpElement(ms, id));
  } else {
