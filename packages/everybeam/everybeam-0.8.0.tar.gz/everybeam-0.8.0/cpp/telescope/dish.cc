...
Dish::Dish(const casacore::MeasurementSet& ms,
    ...
    : Telescope(ms.antenna().nrow(), options),
      dish_coefficients_(std::move(coefficients)) {
  casacore::MSField field_table = ms.field();
  const casacore::String& delay_dir_col_name =
      casacore::MSField::columnName(casacore::MSField::DELAY_DIR);
  casacore::ArrayColumn<double> pointing_dir_col(field_table,
                                                 delay_dir_col_name);
  casacore::ScalarMeasColumn<casacore::MDirection> pointing_dir_measure_col(
      field_table, delay_dir_col_name);
