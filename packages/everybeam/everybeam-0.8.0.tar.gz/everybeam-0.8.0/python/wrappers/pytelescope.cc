...
void init_telescope(py::module& m) {
  py::class_<Telescope, PyTelescope>(m, "Telescope")
      ...
      .def(
          "gridded_response",
          [](Telescope& self,
             const aocommon::CoordinateSystem& coordinate_system, double time,
  ...
  py::class_<PhasedArray, Telescope>(m, "PhasedArray")
      ...
      .def(
          ...
          [](PhasedArray& self, double time, size_t idx, size_t channel,
             bool rotate) -> py::array_t<std::complex<double>> {
            if (self.GetNrChannels() == 0) {
              throw std::runtime_error(
                  "Telescope has no channel information. Please supply an "
                  "actual frequency instead of a channel index.");
            } else if (channel >= self.GetNrChannels()) {
              throw std::runtime_error(
      ...
      .def(
          ...
          [](PhasedArray& self, double time, size_t idx,
            ...
            if (nr_channels == 0) {
              throw std::runtime_error(
                  "Telescope has no channel information. Computing the station "
                  "response for all channels is not possible.");
            }
      ...
      .def(
          ...
          [](PhasedArray& self, double time,
            ...
            if (frequencies.size() == 0) {
              ...
              if (nr_channels == 0) {
                throw std::runtime_error(
                    "Telescope has no channel information. Computing the array "
                    "factor for all channels is not possible.");
              }
              frequencies.resize({nr_channels});
            ...
            for (int d = 0; d < station_indices.ndim(); ++d) {
              result_shape.push_back(station_indices.shape()[d]);
            }
            for (int d = 0; d < frequencies.ndim(); ++d) {
              result_shape.push_back(frequencies.shape()[d]);
            }
            size_t n_directions = 1;
            for (int d = 0; d < directions.ndim() - 1; ++d) {
              result_shape.push_back(directions.shape()[d]);
            ...
            const double* const direction_ptr = directions.data();
            ...
            for (int s = 0; s < station_indices.size(); ++s) {
              for (int f = 0; f < frequencies.size(); ++f) {
                for (size_t d = 0; d < n_directions; ++d) {
                  const vector3r_t direction{direction_ptr[3 * d + 0],
                                             direction_ptr[3 * d + 1],
                                             direction_ptr[3 * d + 2]};
                  const aocommon::MC2x2 response =
                      phased_array_point.UnnormalisedResponse(
                          BeamMode::kArrayFactor, station_indices.data()[s],
                          frequencies.data()[f], direction, station0, tile0);
