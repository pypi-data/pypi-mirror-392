...
def datefmt(xdate, cal=None):
    ...
    def mydate(x, pos):
        ...
        val = int(x + 0.5)
        if val < 0:
            return (xdate[0].to_pydatetime() - CustomBusinessDay(-val,
                                                                 calendar=cal)).strftime(
                '%Y-%m-%d')
        elif val >= len(xdate):
            return (xdate[-1].to_pydatetime() + CustomBusinessDay(val - len(xdate) + 1,
                                                                  calendar=cal)).strftime(
                '%Y-%m-%d')
        else:
...
def plot_sup_res_learn(curdir, hist):
    import os
    if not os.path.isdir(os.path.join(curdir, 'data')): os.mkdir(
        os.path.join(curdir, 'data'))  # image folder
    ...
    def fig_slopeint():
        ...
        plt.gcf().set_size_inches(1000 / plt.gcf().dpi,
                                  1000 / plt.gcf().dpi)  # plt.gcf().dpi=100
        spec = gridspec.GridSpec(ncols=1, nrows=2, figure=plt.gcf(),
                                 height_ratios=[3, 1])
        ...
        dist = np.sqrt(
            np.square(hist.Close.iloc[-3] - hist.Close.iloc[-1]) + np.square(-2))
        ...
        plt.plot([len(hist.Close) - 3, len(hist.Close) - 1],
                 [hist.Close.iloc[-3], hist.Close.iloc[-1]], 'g--')
        ...
        xaxwdt, yaxhgt = ax.get_xlim()[1] - ax.get_xlim()[0], ax.get_ylim()[1] - \
                         ax.get_ylim()[0]
        mvisual = (hist.Close.iloc[-3] - hist.Close.iloc[-1]) * drawdim[1] / yaxhgt / (
                -2 * drawdim[
        ...
        intcpt = ((hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2 - ax.get_ylim()[0]) * \
                 drawdim[1] / yaxhgt - (-(drawdim[0] / 2) / mvisual)
        ann = plt.annotate(
            r'$d=\sqrt{{({}-{})^2+({}-{})^2}}={}$'.format(hist.Close.iloc[-3],
                                                          hist.Close.iloc[-1], 0, 2,
                                                          round(dist, 2)),
            (len(hist.Close) - 2, (hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2),
            ax.transData.inverted().transform(((drawdim[0] * 0.54) + bbox.x0, (
                    -(drawdim[0] * 0.54) / mvisual + intcpt) + bbox.y0)),
            textcoords='data', color='green', ha='center', va='center',
            arrowprops={'arrowstyle': '-[', 'color': 'green'})
        # print(drawdim, ann.xyann, mvisual, intcpt, ax.get_xlim())
        plt.annotate(
            r'$b={}-{}*{}={}-{}*{}={}$'.format(hist.Close.iloc[-1], round(m, 2), 2,
                                               hist.Close.iloc[-3], round(m, 2), 0, b1),
            (len(hist.Close) - 3, hist.Close.iloc[-3]),
            (len(hist.Close) - 3, hist.Close.iloc[-3] - height * 0.1),
            arrowprops={'arrowstyle': '->'})
        plt.plot([len(hist.Close) - 2, len(hist.Close) - 2],
                 [m * 1 + b1, hist.Close.iloc[-2]], 'r--')
        plt.annotate(
            (r'$d=$' + '\n' + r'$\left|{}*{}+{}-{}\right|$' + '\n' + r'$={}$').format(
            ...
            va='center', color='red', arrowprops={'arrowstyle': '-[', 'color': 'red'})
        plt.annotate(r'$m=\frac{{{}}}{{{}}}={}$'.format(
            round(hist.Close.iloc[-3] - hist.Close.iloc[-1], 2), 0 - 2, round(m, 2)),
            (len(hist.Close) - 2,
             (hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2),
            (len(hist.Close) - 2 + 0.2,
             (hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2 - height * 0.1),
            color='black', arrowprops={'arrowstyle': '->'})
        plt.plot([len(hist.Close) - 3, len(hist.Close) - 1],
                 [hist.Close.iloc[-3], hist.Close.iloc[-3]], 'c--')
        plt.annotate(r'$\Delta x={}-{}={}$'.format(0, 2, 0 - 2),
        ...
        plt.plot([len(hist.Close) - 1, len(hist.Close) - 1],
                 [hist.Close.iloc[-3], hist.Close.iloc[-1]], 'c--')
        plt.annotate(
            r'$\Delta y={}-{}={}$'.format(hist.Close.iloc[-3], hist.Close.iloc[-1],
                                          round(
                                              hist.Close.iloc[-3] - hist.Close.iloc[-1],
                                              2)),
            (len(hist.Close) - 1, (hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2),
            (len(hist.Close) - 2 + 0.5,
             (hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2), color='cyan',
        ...
        def redraw(event):
            ...
            drawdim = [bbox.width, bbox.height]
            mvisual = (hist.Close.iloc[-3] - hist.Close.iloc[-1]) * drawdim[
                1] / yaxhgt / (-2 * drawdim[0] / xaxwdt)
            intcpt = ((hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2 - ax.get_ylim()[
                0]) * drawdim[1] / yaxhgt - (-(drawdim[0] / 2) / mvisual)
            # print(drawdim, ann.xyann, mvisual, intcpt, ax.get_xlim())
            ann.xyann = ax.transData.inverted().transform(
                ((drawdim[0] * 0.54) + bbox.x0,
                 (-(drawdim[0] * 0.54) / mvisual + intcpt) + bbox.y0))
    ...
    def fig_linregrs():
        plt.clf()
        plt.rcParams.update({'font.size': 14})
        plt.gcf().set_size_inches(1024 / plt.gcf().dpi,
                                  768 / plt.gcf().dpi)  # plt.gcf().dpi=100
        spec = gridspec.GridSpec(ncols=1, nrows=2, figure=plt.gcf(),
                                 height_ratios=[3, 1])
        plt.subplot(spec[1, 0])
        plt.axis('off')
        ...
        height = hist.Close.iloc[-3:].max() - hist.Close.iloc[-3:].min()
        plt.hlines(ybar, len(hist.Close) - 3, len(hist.Close) - 1, colors='r',
                   linestyles='--')
        ...
        plt.vlines(xbar + len(hist.Close) - 3, hist.Close.iloc[-3:].min(),
                   hist.Close.iloc[-3:].max(), colors='r', linestyles='--')
        plt.annotate(
            r'$\bar{{y}}=\frac{{{}+{}+{}}}{{{}}}={}$'.format(hist.Close.iloc[-3],
                                                             hist.Close.iloc[-2],
                                                             hist.Close.iloc[-1], 3,
                                                             round(ybar, 2)),
            (len(hist.Close) - 2, ybar), (len(hist.Close) - 1, ybar - height * 0.1),
            color='red', va='top', ha='right',
            arrowprops={'arrowstyle': '->', 'color': 'red'})
        m = ((0 - xbar) * (hist.Close.iloc[-3] - ybar) + (1 - xbar) * (
                hist.Close.iloc[-2] - ybar) + (2 - xbar) * (
                     hist.Close.iloc[-1] - ybar)) / (
                    np.square(0 - xbar) + np.square(1 - xbar) + np.square(2 - xbar))
        b = ybar - m * xbar
        SSR = np.square(hist.Close.iloc[-3] - (m * 0 + b)) + np.square(
            hist.Close.iloc[-2] - (m * 1 + b)) + np.square(
            hist.Close.iloc[-1] - (m * 2 + b))
        err1 = np.sqrt(SSR / ((3 - 2) * (
                np.square(0 - xbar) + np.square(1 - xbar) + np.square(2 - xbar))))
        err2 = err1 * np.sqrt((np.square(0) + np.square(1) + np.square(2)) / 3)
        plt.annotate(r'$\hat{{y}}_0={}*{}+{}={}$'.format(round(m, 2), 0, round(b, 2),
                                                         round(m * 0 + b, 2)),
                     (len(hist.Close) - 3, m * 0 + b),
                     (len(hist.Close) - 3 + 0.1, m * 0 + b), va='top',
                     arrowprops={'arrowstyle': '->'})
        plt.annotate(r'$\hat{{y}}_1={}*{}+{}={}$'.format(round(m, 2), 1, round(b, 2),
                                                         round(m * 1 + b, 2)),
                     (len(hist.Close) - 2, m * 1 + b),
                     (len(hist.Close) - 2 + 0.15, m * 1 + b + height * 0.01),
                     arrowprops={'arrowstyle': '->'})
        plt.annotate(r'$\hat{{y}}_2={}*{}+{}={}$'.format(round(m, 2), 2, round(b, 2),
                                                         round(m * 2 + b, 2)),
                     (len(hist.Close) - 1, m * 2 + b),
                     (len(hist.Close) - 1 - 0.1, m * 2 + b + height * 0.1), ha='right',
                     arrowprops={'arrowstyle': '->'})
        plt.plot([len(hist.Close) - 3, len(hist.Close) - 3],
                 [hist.Close.iloc[-3], ybar], color='green')
        plt.plot([len(hist.Close) - 2, len(hist.Close) - 2],
                 [hist.Close.iloc[-2], ybar], color='green')
        plt.plot([len(hist.Close) - 1, len(hist.Close) - 1],
                 [hist.Close.iloc[-1], ybar], color='green')
        plt.annotate(r'$y_0-\bar{{y}}={}$'.format(round(hist.Close.iloc[-3] - ybar, 2)),
                     (len(hist.Close) - 3,
                      (hist.Close.iloc[-3] + ybar) / 2 + height * 0.1),
                     (len(hist.Close) - 3 + 0.1,
                      (hist.Close.iloc[-3] + ybar) / 2 + height * 0.1), color='green',
                     va='center', arrowprops={'arrowstyle': '-[', 'color': 'green'})
        plt.annotate(r'$y_1-\bar{{y}}={}$'.format(round(hist.Close.iloc[-2] - ybar, 2)),
                     (len(hist.Close) - 2,
                      (hist.Close.iloc[-2] + ybar) / 2 + height * 0.1),
                     (len(hist.Close) - 2 + 0.1,
                      (hist.Close.iloc[-2] + ybar) / 2 + height * 0.1), color='green',
                     va='center', arrowprops={'arrowstyle': '-[', 'color': 'green'})
        plt.annotate(r'$y_2-\bar{{y}}={}$'.format(round(hist.Close.iloc[-1] - ybar, 2)),
                     ...
                     arrowprops={'arrowstyle': '-[', 'color': 'green'})
        plt.plot([len(hist.Close) - 3, len(hist.Close) - 3],
                 [hist.Close.iloc[-3], m * 0 + b], color='cyan')
        plt.plot([len(hist.Close) - 2, len(hist.Close) - 2],
                 [hist.Close.iloc[-2], m * 1 + b], color='cyan')
        plt.plot([len(hist.Close) - 1, len(hist.Close) - 1],
                 [hist.Close.iloc[-1], m * 2 + b], color='cyan')
        plt.annotate(
            r'$y_0-\hat{{y}}={}$'.format(round(hist.Close.iloc[-3] - (m * 0 + b), 2)),
            (len(hist.Close) - 3, (hist.Close.iloc[-3] + m * 0 + b) / 2),
            (len(hist.Close) - 3 + 0.1, (hist.Close.iloc[-3] + m * 0 + b) / 2),
            color='cyan', va='center', arrowprops={'arrowstyle': '-[', 'color': 'cyan'})
        plt.annotate(
            r'$y_1-\hat{{y}}={}$'.format(round(hist.Close.iloc[-2] - (m * 1 + b), 2)),
            (len(hist.Close) - 2, (hist.Close.iloc[-2] + m * 1 + b) / 2),
            (len(hist.Close) - 2 + 0.1, (hist.Close.iloc[-2] + m * 1 + b) / 2),
            color='cyan', va='center', arrowprops={'arrowstyle': '-[', 'color': 'cyan'})
        plt.annotate(
            r'$y_2-\hat{{y}}={}$'.format(round(hist.Close.iloc[-1] - (m * 2 + b), 2)),
            (len(hist.Close) - 1,
             (hist.Close.iloc[-1] + m * 2 + b) / 2 - height * 0.05),
            (len(hist.Close) - 1 - 0.1,
             (hist.Close.iloc[-1] + m * 2 + b) / 2 - height * 0.05), color='cyan',
        ...
        plt.annotate((
                     ...
                     ).format(0, round(xbar, 2),
                              round(hist.Close.iloc[-3] - ybar, 2), 1,
                              round(xbar, 2),
                              round(hist.Close.iloc[-2] - ybar, 2), 2,
                              round(xbar, 2),
                              round(hist.Close.iloc[-1] - ybar, 2), 0,
                              round(xbar, 2), 1, round(xbar, 2), 2,
                              round(xbar, 2), round(m, 2),
                              round(hist.Close.iloc[-3] - (m * 0 + b), 2),
                              round(hist.Close.iloc[-2] - (m * 1 + b), 2),
                              round(hist.Close.iloc[-1] - (m * 2 + b), 2),
                              round(SSR, 2),
                              round(SSR, 2), 3, 0, round(xbar, 2), 1,
                              round(xbar, 2), 2, round(xbar, 2),
                              round(err1, 2),
                              round(err1, 2), 0, 1, 2, 3, round(err2, 2)),
                     (len(hist.Close) - 2, m * 1 + b),
                     (len(hist.Close) - 1, hist.Close.iloc[-3:].min()), color='blue',
                     va='bottom', ha='right',
                     arrowprops={'arrowstyle': '->', 'color': 'blue'})
        plt.annotate(
            r'$b={}-{}*{}={}$'.format(round(ybar, 2), round(m, 2), xbar, round(b, 2)),
            (len(hist.Close) - 3, b), (len(hist.Close) - 3 + 0.1, b), color='blue',
            ha='left', arrowprops={'arrowstyle': '->', 'color': 'blue'})
    ...
    def fig_hough():
        plt.clf()
        plt.rcParams.update({'font.size': 14})
        plt.gcf().set_size_inches(1280 / plt.gcf().dpi,
                                  1024 / plt.gcf().dpi)  # plt.gcf().dpi=100
        spec = gridspec.GridSpec(ncols=1, nrows=2, figure=plt.gcf(),
                                 height_ratios=[3, 1])
        plt.subplot(spec[1, 0])
        plt.axis('off')
        ...
        plt.annotate(
            ...
            (0, 0))
        plt.subplot(spec[0, 0])
        plt.plot([len(hist.Close) - 10, len(hist.Close) - 1],
                 [hist.Close.iloc[-10], hist.Close.iloc[-1]], 'ro')
        plt.plot([len(hist.Close) - 10, len(hist.Close) - 1],
                 [hist.Close.iloc[-10], hist.Close.iloc[-1]], 'k-')
        mn, mx = min(hist.Close.iloc[-10], hist.Close.iloc[-1]), max(
            hist.Close.iloc[-10], hist.Close.iloc[-1])
        plt.plot([len(hist.Close) - 10, len(hist.Close) - 1], [mn, mx], 'b--')
        plt.annotate(
            r'Diagonal length=$\sqrt{{{}^2+{}^2}}={}$'.format(9, round(mx - mn, 2),
                                                              round(np.sqrt(np.square(
                                                                  9) + np.square(
                                                                  mx - mn)), 2)),
            (len(hist.Close) - 1, mx), (len(hist.Close) - 1 - 1, mx), ha='right',
            va='top', color='blue', arrowprops={'arrowstyle': '->', 'color': 'blue'})
        # plt.xlim(0, 30)
        # plt.ylim(0, 30)
        ...
        plt.annotate(r'$y={}x+{}$'.format(round(m, 2), round(b, 2)),
        ...
        plt.annotate('', (len(hist.Close) - 10, mn),
                     (len(hist.Close) - 10 + x0, mn + -x0 / m + bperp),
                     arrowprops=dict(arrowstyle="<|-", color='red'))
        plt.gca().add_patch(
            mpatches.Wedge((len(hist.Close) - 10 + x0, mn + -x0 / m + bperp), 1,
                           angle * 180 / np.pi - 180, angle * 180 / np.pi - 90,
                           fill=False))
        plt.gca().add_patch(
            mpatches.Wedge((len(hist.Close) - 10 + x0, mn + -x0 / m + bperp), 0.5,
                           angle * 180 / np.pi - 270, angle * 180 / np.pi - 180,
                           fill=False))
        plt.annotate(r'$90\circ$',
                     (len(hist.Close) - 10 + x0 - 1, mn + -x0 / m + bperp - 2))
        plt.annotate(r'$90\circ$',
                     (len(hist.Close) - 10 + x0 - 1, mn + -x0 / m + bperp + 1))
        plt.gca().add_patch(
            mpatches.Wedge((len(hist.Close) - 10, mn), 3, 0, angle * 180 / np.pi,
                           fill=False))
        plt.annotate(r'$\theta={}^\circ$'.format(round(angle * 180 / np.pi, 2)),
                     (len(hist.Close) - 6.75, mn + 0.1))
        plt.gca().add_patch(mpatches.Wedge((len(hist.Close) - 10, mx), 3, 270,
                                           270 + angle * 180 / np.pi, fill=False))
        plt.annotate(r'$\theta$', (len(hist.Close) - 9.5, mx - 5))
        plt.annotate((
                r'$\rho=\frac{{\left|{}+{}*{}-{}\right|}}{{\sqrt{{1 + {}^2}}}}$' + '\n' + r'$={}\cos {}+{}\sin {}$' + '\n' + r'$={}\cos {}+{}\sin {}$' + '\n' + '$={}$').format(
            round(b, 2), round(m, 2), 0, 0, round(m, 2),
            0, round(angle * 180 / np.pi, 2), round(hist.Close.iloc[-10] - mn, 2),
            round(angle * 180 / np.pi, 2), 9, round(angle * 180 / np.pi, 2),
            hist.Close.iloc[-1] - mn, round(angle * 180 / np.pi, 2),
        ...
        plt.annotate('{}'.format(9), (len(hist.Close) - 5.5, mn),
                     (len(hist.Close) - 5.5, mn + 0.5), ha='center',
                     arrowprops=dict(arrowstyle="->"))
        plt.annotate('{}'.format(round(mx - mn, 2)),
                     (len(hist.Close) - 10, (mn + mx) / 2),
                     (len(hist.Close) - 10 + 0.5, (mn + mx) / 2), ha='left',
                     arrowprops=dict(arrowstyle="->"))
        ...
        plt.savefig(os.path.join(curdir, 'data', 'hough.svg'),
                    format='svg')  # , bbox_inches = extent, pad_inches = 0)
        plt.savefig(os.path.join(curdir, 'data', 'hough.png'),
                    format='png')  # , bbox_inches = extent, pad_inches = 0)
    ...
    def fig_minima():
        ...
        plt.gcf().set_size_inches(1024 / plt.gcf().dpi,
                                  768 / plt.gcf().dpi)  # plt.gcf().dpi=100
        plt.subplot(111)
        plt.plot(minimaIdxs, [h.Close.iloc[x] for x in minimaIdxs], 'yo',
                 label='Minima')
        plt.plot(maximaIdxs, [h.Close.iloc[x] for x in maximaIdxs], 'bo',
                 label='Maxima')
        from findiff import FinDiff
        dx = 1  # grid scale could be amplified with pennies 0.01
        d_dx = FinDiff(0, dx,
                       1)  # acc=3 #for 5-point stencil, currenly uses +/-1 day only
        d2_dx2 = FinDiff(0, dx,
                         2)  # acc=3 #for 5-point stencil, currenly uses +/-1 day only
        clarr = np.asarray(h.Close)
        mom = d_dx(clarr)
        momacc = d2_dx2(clarr)
        for x in range(len(h)):
            ann = plt.gca().annotate(
                '{}\n'.format(round(mom[x], 2), round(momacc[x], 2)),
                (x, h.Close.iloc[x]),
            ...
            ext = ann.get_window_extent(
                renderer=plt.gcf().canvas.get_renderer()).transformed(
                plt.gca().transData.inverted())
            ann = plt.gca().annotate('{}'.format(round(momacc[x], 2)), (x, ext.y0),
                                     ha='center', va='bottom', color='r')
            next = ann.get_window_extent(
                renderer=plt.gcf().canvas.get_renderer()).transformed(
                plt.gca().transData.inverted())
            patch = plt.gca().add_artist(
                mpatches.FancyBboxPatch((min(ext.x0, next.x0) + 0.1, ext.y0),
                                        width=max(ext.width, next.width) - 0.2,
                                        height=ext.height + 3, boxstyle='round',
                                        fc='gray', alpha=0.3))
            plt.gca().annotate(''.format(round(mom[x], 2), round(momacc[x], 2)),
                               (x, h.Close.iloc[x]), (x, ext.y1 + 3),
                               arrowprops={'arrowstyle': '-|>'})
        ...
        plt.savefig(os.path.join(curdir, 'data', 'extrema.svg'), format='svg',
                    bbox_inches='tight')
        plt.savefig(os.path.join(curdir, 'data', 'extrema.png'), format='png',
                    bbox_inches='tight')
    ...
    def fig_reimann():
        ...
        maximaIdxs, pmax, maxtrend, maxwindows = maxs
        plt.clf()
        plt.gcf().set_size_inches(800 / plt.gcf().dpi,
                                  720 / plt.gcf().dpi)  # plt.gcf().dpi=100
        plt.subplot(211)
        ...
        base = trendline[0][0]
        m, b, ser = trendline[1][0], trendline[1][1], hist[-250:][
            base:trendline[0][-1] + 1].Close
        plt.plot(range(base, trendline[0][-1] + 1),
                 hist[-250:][base:trendline[0][-1] + 1].Close, 'b-', label='Price')
        plt.plot((base, trendline[0][-1] + 1),
                 (m * base + b, m * (trendline[0][-1] + 1) + b), 'r-',
                 label='Resistance')
        plt.gca().xaxis.set_major_locator(ticker.MaxNLocator(6))
        # plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m'))
        plt.gca().xaxis.set_major_formatter(
            ticker.FuncFormatter(datefmt(hist[-250:].index)))
        plt.setp(plt.gca().get_xticklabels(), rotation=30, ha='right')
        plt.legend()
        plt.subplot(212)
        ...
        isMin = False
        S = sum(
            [max(0, (m * (x + base) + b) - y if isMin else y - (m * (x + base) + b)) for
             x, y in enumerate(ser)])
        area = S / len(ser)
        for x, y in enumerate(ser):
            plt.bar(x, (m * (x + base) + b) - y if isMin else y - (m * (x + base) + b),
                    color='r' if (y < (m * (x + base) + b) if isMin else y > (
                            m * (x + base) + b)) else 'gray')

        plt.annotate(
            r'S={}, $\frac{{{}}}{{{}}}$={}$\frac{{\$}}{{day}}$'.format(round(S, 2),
                                                                       round(S, 2),
                                                                       len(range(base,
                                                                                 trendline[
                                                                                     0][
        ...
        plt.gca().xaxis.set_major_formatter(
            ticker.FuncFormatter(datefmt(hist[-250:].index)))
        plt.setp(plt.gca().get_xticklabels(), rotation=30, ha='right')
        plt.savefig(os.path.join(curdir, 'data', 'reimann.svg'), format='svg',
                    bbox_inches='tight')
        plt.savefig(os.path.join(curdir, 'data', 'reimann.png'), format='png',
                    bbox_inches='tight')
    ...
    def fig_suppres():
        plot_sup_res_date(hist[-250:].Close, hist[-250:].index, fromwindows=False,
                          sortError=True)
        plt.savefig(os.path.join(curdir, 'data', 'suppreserr.svg'), format='svg',
                    bbox_inches='tight')
        plt.savefig(os.path.join(curdir, 'data', 'suppreserr.png'), format='png',
                    bbox_inches='tight')
        plot_sup_res_date(hist[-250:].Close, hist[-250:].index, fromwindows=False)
        plt.savefig(os.path.join(curdir, 'data', 'suppres.svg'), format='svg',
                    bbox_inches='tight')
        plt.savefig(os.path.join(curdir, 'data', 'suppres.png'), format='png',
                    bbox_inches='tight')
...
def test_sup_res(curdir):
    ...
    result = (([], [np.nan, np.nan], [], [[]]),
              ([3, 9, 15, 21], [0.16666666666666677, 2.499999999999998],
               ...
               [[([3, 9, 15, 21], (0.16666666666666666, 2.5, 0.0, 0.0, 0.0, 0.0))]]))
    assert result == calc_support_resistance(data, extmethod=METHOD_NAIVE)
    result = (([7, 13, 19], [0.1666666666666666, -0.1666666666666652], [([7, 13, 19],
                                                                         (0.16666666666666666,
                                                                          -0.16666666666666652,
                                                                          0.0, 0.0, 0.0,
                                                                          0.0))],
               [[([7, 13, 19],
                  (0.16666666666666666, -0.16666666666666652, 0.0, 0.0, 0.0, 0.0))]]),
              ([3, 9, 15, 21], [0.16666666666666677, 2.499999999999998],
               [([3, 9, 15, 21], (0.16666666666666666, 2.5, 0.0, 0.0, 0.0, 0.0))],
               [[([3, 9, 15, 21], (0.16666666666666666, 2.5, 0.0, 0.0, 0.0, 0.0))]]))
    assert result == calc_support_resistance(data, extmethod=METHOD_NAIVECONSEC)
    result = (([23], [np.nan, np.nan], [], [[]]),
              ([3, 9, 15, 21], [0.16666666666666677, 2.499999999999998],
               [([3, 9, 15, 21], (0.16666666666666666, 2.5, 0.0, 0.0, 0.0, 0.0))],
               [[([3, 9, 15, 21], (0.16666666666666666, 2.5, 0.0, 0.0, 0.0, 0.0))]]))
    assert result == calc_support_resistance(data)
...
def get_extrema(h, extmethod=METHOD_NUMDIFF, accuracy=2):
    ...
    if type(h) is tuple and len(h) == 2 and (
            h[0] is None or check_num_alike(h[0])) and (
            h[1] is None or check_num_alike(h[1])) and (
            not h[0] is None or not h[1] is None):
        hmin, hmax = h[0], h[1]
    ...
    if extmethod == METHOD_NAIVE:
        ...
        def get_minmax(h):
            ...
            minFunc = lambda x: len(x) == 3 and x.iloc[0] > x.iloc[1] and x.iloc[2] > \
                                x.iloc[1]
            maxFunc = lambda x: len(x) == 3 and x.iloc[0] < x.iloc[1] and x.iloc[2] < \
                                x.iloc[1]
            numdiff_extrema = lambda func: np.flatnonzero(
                rollwin.aggregate(func)).tolist()
        ...
        dx = 1  # 1 day interval
        d_dx = FinDiff(0, dx, 1,
                       acc=accuracy)  # acc=3 #for 5-point stencil, currenly uses +/-1 day only
        ...
        raise ValueError(
            'extmethod must be METHOD_NAIVE, METHOD_NAIVECONSEC, METHOD_NUMDIFF')
...
def calc_support_resistance(h, extmethod=METHOD_NUMDIFF, method=METHOD_NSQUREDLOGN,
                            window=125, errpct=0.005, hough_scale=0.01,
                            hough_prob_iter=10,
    ...
    if type(h) is tuple and len(h) == 2 and (
            ...
            not h[0] is None or not h[1] is None):
        hmin, hmax = h[0], h[1]
        if isinstance(hmin, pd.Series):
            hmin = hmin.to_numpy()
        if isinstance(hmax, pd.Series):
            hmax = hmax.to_numpy()
        if not hmin is None and not hmax is None and len(hmin) != len(
                hmax):  # not strict requirement, but contextually ideal
            raise ValueError('h does not have a equal length minima and maxima data')
        h = (hmin, hmax)
        len_h = len(hmax if hmin is None else hmin)
    elif check_num_alike(h):
            ...
            for y in range(x + 1, len(Idxs)):
                ...
                for z in range(y + 1, len(Idxs)):
                    ...
                    trend.append(([Idxs[x], Idxs[y], Idxs[z]],
                                  get_bestfit3(Idxs[x], h[Idxs[x]], Idxs[y], h[Idxs[y]],
                                               Idxs[z], h[Idxs[z]])))
        ...
        for x in range(len(Idxs)):
            ...
            for y in range(0, len(slopes[x])):
                ...
                if res[3] <= fltpct:
                    ...
                    CurIdxs = [CurIdxs[0],
                               CurIdxs[-1]]  # restart search from this point
        return trend
    ...
    def houghpt(Idxs, h, fltpct, min_h, max_h):
        ...
        mx = min(max_size, height)
        scl = (1 / hough_scale) * mx / height
        acc, theta, d = hough_points([(x, int((h[x] - m) * scl)) for x in Idxs], mx,
                                     len_h, np.linspace(-np.pi / 2, np.pi / 2, 360 * 5))
    ...
    def hough(Idxs, h, fltpct, min_h, max_h):
        ...
        for pts, angle, dist in zip(
                *hough_line_peaks(hl, theta, d, threshold=2)):  # > threshold
            y0, y1 = (dist - origin * np.cos(angle)) / np.sin(angle)
            y0, y1 = y0 / scl + m, y1 / scl + m
    ...
    def prob_hough(Idxs, h, fltpct, min_h, max_h):
        ...
        for x in range(hough_prob_iter):
            lines.extend(
                probabilistic_hough_line(image, threshold=2, theta=tested_angles,
                                         line_length=0,
                                         line_gap=int(np.ceil(np.sqrt(
                                             np.square(image.shape[0]) + np.square(
                                                 image.shape[1]))))))
        l = []
    ...
    def merge_lines(Idxs, trend, h, fltpct):
        for x in Idxs:
            ...
            for (s, i) in l[1:]:
                ...
                res = get_bestfit([(p, h[p]) for p in CurIdxs])
                if res[3] <= fltpct:
                    trend[i - 1], trend[i], CurIdxs = ([], None), (CurIdxs, res), list(
                        CurIdxs)
                else:
    ...
    def measure_area(trendline, isMin,
                     h):  # Reimann sum of line to discrete time series data
    ...
    def window_results(trends, isMin, h):
        ...
        for x in trends:
            ...
            for y in range(0, lastwin - fstwin):
                if len(wins[y + 1]) == 0 and len(wins[y]) >= 3: windows[
                    fstwin + y].append(wins[y])
                if len(wins[y]) + len(wins[y + 1]) >= 3:
                    windows[fstwin + y + 1].append(wins[y] + wins[y + 1])
            if lastwin - fstwin == 0 and len(wins[0]) >= 3: windows[fstwin].append(
                wins[0])
    ...
    def overall_line(idxs, vals):
        if len(idxs) <= 1:
            ...
            p, r = np.polynomial.polynomial.Polynomial.fit(idxs, vals, 1,
                                                           full=True)  # more numerically stable
    ...
    def calc_all(idxs, h, isMin):
        ...
        mtrend = [(pts, (res[0], res[1], res[2], res[3], res[4],
                         measure_area((pts, res), isMin, h))) for pts, res in mtrend]
    ...
    return (extremaIdxs[0], pmin, mintrend, minwindows), (extremaIdxs[1], pmax,
                                                          maxtrend, maxwindows)
...
def plot_sup_res_date(hist, idx, numbest=2, fromwindows=True, pctbound=0.1,
                      ...
                      errpct=0.005, hough_scale=0.01, hough_prob_iter=10,
                      sortError=False, accuracy=2):
    import matplotlib.ticker as ticker
    return plot_support_resistance(hist, ticker.FuncFormatter(datefmt(idx)), numbest,
                                   fromwindows,
                                   pctbound, extmethod, method, window, errpct,
                                   hough_scale, hough_prob_iter, sortError, accuracy)


def plot_support_resistance(hist, xformatter=None, numbest=2, fromwindows=True,
                            pctbound=0.1, extmethod=METHOD_NUMDIFF,
                            method=METHOD_NSQUREDLOGN,
                            window=125, errpct=0.005, hough_scale=0.01,
                            hough_prob_iter=10, sortError=False, accuracy=2):
    import matplotlib.pyplot as plt
    import matplotlib.ticker as ticker
    import pandas as pd

    # normalize pandas Series to numpy arrays for positional indexing
    if isinstance(hist, pd.Series):
        hist = hist.to_numpy()
    elif isinstance(hist, tuple) and len(hist) == 2:
        h0, h1 = hist
        if isinstance(h0, pd.Series):
            h0 = h0.to_numpy()
        if isinstance(h1, pd.Series):
            h1 = h1.to_numpy()
        hist = (h0, h1)

    ret = calc_support_resistance(hist, extmethod, method, window, errpct, hough_scale,
    ...
    if len(ret) == 2:
        ...
        if type(hist) is tuple and len(hist) == 2 and check_num_alike(
                hist[0]) and check_num_alike(hist[1]):
            len_h = len(hist[0])
            min_h, max_h = min(min(hist[0]), min(hist[1])), max(max(hist[0]),
                                                                max(hist[1]))
            disp = [(hist[0], minimaIdxs, pmin, 'yo', 'Avg. Support', 'y--'),
                    (hist[1], maximaIdxs, pmax, 'bo', 'Avg. Resistance', 'b--')]
            dispwin = [(hist[0], minwindows, 'Support', 'g--'),
                       (hist[1], maxwindows, 'Resistance', 'r--')]
            disptrend = [(hist[0], mintrend, 'Support', 'g--'),
            ...
            dispwin = [(hist, minwindows, 'Support', 'g--'),
                       (hist, maxwindows, 'Resistance', 'r--')]
            disptrend = [(hist, mintrend, 'Support', 'g--'),
                         (hist, maxtrend, 'Resistance', 'r--')]
            plt.plot(range(len_h), hist, 'k--', label='Close Price')
    else:
        minimaIdxs, pmin, mintrend, minwindows = ([], [], [], []) if hist[
                                                                         0] is None else ret
        maximaIdxs, pmax, maxtrend, maxwindows = ([], [], [], []) if hist[
                                                                         1] is None else ret
        len_h = len(hist[1 if hist[0] is None else 0])
        min_h, max_h = min(hist[1 if hist[0] is None else 0]), max(
            hist[1 if hist[0] is None else 0])
        disp = [(hist[1], maximaIdxs, pmax, 'bo', 'Avg. Resistance', 'b--') if hist[
                                                                                   0] is None else (
            hist[0], minimaIdxs, pmin, 'yo', 'Avg. Support', 'y--')]
        dispwin = [
            (hist[1], maxwindows, 'Resistance', 'r--') if hist[0] is None else (hist[0],
                                                                                minwindows,
                                                                                'Support',
                                                                                'g--')]
        disptrend = [
            (hist[1], maxtrend, 'Resistance', 'r--') if hist[0] is None else (hist[0],
                                                                              mintrend,
                                                                              'Support',
                                                                              'g--')]
    ...
    def add_trend(h, trend, lbl, clr, bFirst):
        for ln in trend[:numbest]:
            ...
            while maxx < len_h:
                ...
                if (h[maxx] > ypred and h[maxx - 1] < ypred or h[maxx] < ypred and h[
                    maxx - 1] > ypred or
