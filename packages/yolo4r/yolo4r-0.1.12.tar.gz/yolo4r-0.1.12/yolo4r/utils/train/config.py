...
def get_args():
    ...
    "--weights", "-w",
    type=str,
    help="Path to pretrained weights (.pt). Optional. If omitted, YOLO defaults will be used.",
    )
    ...
    if not hasattr(args, "weights"):
        args.weights = None
    # Normalize weights path or assign default
    if args.weights is None:
        family = args.model or args.arch or "yolo11n"
        family, _ = normalize_model_name(family)
        default_weight = FAMILY_TO_WEIGHTS.get(family, "yolo11n.pt")
        args.weights = WEIGHTS_DIR / default_weight
    else:
