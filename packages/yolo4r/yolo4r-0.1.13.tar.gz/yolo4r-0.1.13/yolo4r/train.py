...
def train_yolo(args, mode="train", checkpoint=None, resume_flag=False):
    ...
    if mode == "update":
        paths = get_training_paths(args.dataset_folder, test=args.test)
    ...
    if mode == "scratch":
        ...
        if checkpoint:
            ...
            if custom_arch_supplied:
                ...
                model_source = str(ensure_weights(
                    args.weights,
                    model_type=(args.model or args.arch or "yolo11n").split("/")[-1]
                ))
...
def main():
    ...
    try:
        checkpoint, resume_flag = get_checkpoint_and_resume(
            ...
            runs_dir=get_training_paths(args.dataset_folder, test=args.test)["runs_root"],
