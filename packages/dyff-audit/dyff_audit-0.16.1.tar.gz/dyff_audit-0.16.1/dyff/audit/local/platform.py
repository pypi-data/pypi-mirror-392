...
class _Curves:
    ...
    def get(self, *, analysis: str) -> list[Curve]:
        """Get all the curves output by the given analysis."""
        try:
            file = self._platform.entity_path(analysis) / ".dyff" / "curves.json"
            with open(file, "r") as fin:
                curves_obj = json.load(fin)
...
class DyffLocalPlatform:
    ...
    @property
    def curves(self) -> _Curves:
        """Operations on :class:`~dyff.schema.platform.Curve` entities."""
        return self._curves
