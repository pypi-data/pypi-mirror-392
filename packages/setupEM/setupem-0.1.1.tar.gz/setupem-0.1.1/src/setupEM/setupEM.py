...
import sys, json, os, tempfile, pathlib, ast
from PySide6.QtWidgets import (
    ...
    QCheckBox, QAbstractItemView,QStyleFactory,QTableWidgetItem, QPlainTextEdit    
    )
from PySide6.QtGui import QAction, QColor, QTextCharFormat, QFont, QSyntaxHighlighter
from PySide6.QtCore import Qt, QRegularExpression, QProcess
...
'''
def add_struct_to_simulation_ports (all_ports_list):
    # import structure from JSON to all_simulation_ports 

    for port in all_ports_list:
        portnumber = port.get(portnumber)
        source_layernum = port.get(source_layernum)
        target_layername = port.get(target_layername)
        from_layername = port.get(from_layername)
        to_layername = port.get(to_layername)
        direction = port.get(direction)
        port_Z0 = port.get(port_Z0)
        voltage = port.get(voltage)

        sim_port = simulation_setup.simulation_port(portnumber, 
                                                    voltage, 
                                                    port_Z0, 
                                                    source_layernum, 
                                                    target_layername=target_layername, 
                                                    from_layername=from_layername, 
                                                    to_layername=to_layername, 
                                                    direction=direction)
        
        simulation_ports.add_port(sim_port)
'''
...
class FileInputTab(QWidget):
    ...
    def __init__(self, MainWindow):
        ...
        self.gds_file_edit = QLineEdit("Please choose a file ===>")
        ...
        self.XML_file_edit = QLineEdit("Please choose a file ===>")
    ...
    def browse_gds_file(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(self, "Select GDSII File",previous_directory,"*.gds;;*.*")
        if file_path:
            self.gds_file_edit.setText(file_path)
            saved_values ["GdsFile"] = file_path.replace('\\','/')
    ...
    def browse_XML_file(self):
        ...
        if filename:
            ...
            # file is read when leaving this tab
    ...
    def read_XML(self, filename):
        if pathlib.Path(filename).exists():
            materials_list, dielectrics_list, metals_list = stackup_reader.read_substrate (filename)
            self.MainWindow.ports_tab.update_layers(metals_list)
    ...
    def save_values(self):
        ...
        self.read_XML(saved_values ["SubstrateFile"])
...
class CreateModelTab(QWidget):
    def __init__(self, MainWindow):
        ...
        self.file_group = QGroupBox("Output files for Palace model")
        ...
        self.create_model_btn = QPushButton("⚙️ Create mesh and Palace model file")
    ...
    def load_values(self):
        ...
        if model_basename == "":
            if gdsfile != "":
                model_basename = 'run_' + os.path.basename(gdsfile).replace('.gds', '')
...
class ModelEditorTab(QWidget):
    ...
    def create_model_text(self, forExport=False):
        ...
        if forExport:
            add_text("\n# run after crerating mesh and Palace config.json ")
...
class MainWindow(QMainWindow):
    ...
    def create_menu_bar(self):
        ...
        # disable export by default, onyl enables when on Code tab
        self.export_model_action.setEnabled(False)

        # browse_action.triggered.connect(self.browse_settings_file)
    ...
    def import_from_python(self):
        ...
        if file_path:
            ...
            QMessageBox.information(self, f"Could not load file {file_path}")
