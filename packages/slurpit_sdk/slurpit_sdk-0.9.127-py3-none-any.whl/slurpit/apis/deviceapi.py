...
class DeviceAPI(BaseAPI):
    ...
    async def add_tags(self, device_id, tag_ids: list[int]):
        """
        ...
        Returns:
            dict: The response from the API containing the updated device information.
        """
    ...
    async def remove_tags(self, device_id: int, tag_ids: list[int]):
        """
        ...
        Returns:
            Device: response from the API containing the confirmation message.
        """
        url = f"{self.base_url}/devices/{device_id}/tags"
        payload = {"tag_ids": tag_ids}
