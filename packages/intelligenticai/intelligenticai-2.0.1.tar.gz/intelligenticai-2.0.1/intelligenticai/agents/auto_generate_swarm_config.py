...
def parse_yaml_from_agent_group_markdown(markdown_text: str) -> dict:
    """
    Extracts and prepares YAML content from a Markdown-style 'Auto-AgentGroup-Builder' block and parses it.

    Args:
        markdown_text (str): The Markdown text containing the YAML inside 'Auto-AgentGroup-Builder' block.

    Returns:
        dict: A parsed Python dictionary of the YAML content.
    """
    # Match the 'Auto-AgentGroup-Builder' block with YAML inside triple backticks
    pattern = r"```yaml\s*\n(.*?)```"
    match = re.search(pattern, markdown_text, re.DOTALL)

    if not match:
        raise ValueError(
            "No YAML content found in the 'Auto-AgentGroup-Builder' block."
        )
...
AUTO_GEN_PROMPT = """
You are a specialized agent responsible for creating YAML configuration files for multi-agent agent_groups. Your role is to generate well-structured YAML that defines both individual agents and agent_group architectures based on user requirements.
...
4. When a agent_group is needed, include a `agent_group_architecture` section with:
   Mandatory fields:
   - name (string)
   - agent_group_type (string: "ConcurrentWorkflow" or "SequentialWorkflow") [AgentRearrange, MixtureOfAgents, SpreadSheetAgentGroup, SequentialWorkflow, ConcurrentWorkflow]	
...
agent_group_architecture:
  ...
  description: "AgentGroup purpose and goals"
  agent_group_type: "ConcurrentWorkflow"
  max_loops: 5
  task: "Main agent_group task description"
```
...
6. Tasks should be specific and aligned with the agent/agent_group purpose

When generating a YAML configuration:
1. Ask for specific requirements about the agents and agent_group needed
2. Determine if a agent_group architecture is necessary based on the task complexity
...
# Multi-Agent AgentGroup Example
agents:
...
agent_group_architecture:
  name: "Research-Analysis-AgentGroup"
  description: "A agent_group for comprehensive research analysis and insight generation"
...
def generate_agent_group_config(
    task: str,
    file_name: str = "agent_group_config_output.yaml",
    model_name: str = "gpt-4.1",
    *args,
    **kwargs,
):
    """
    Generates a agent_group configuration based on the provided task and model name.

    This function attempts to generate a agent_group configuration by running an agent with the specified task and model name.
    It then parses the output into YAML format and creates agents based on the parsed YAML content.

    Args:
        task (str): The task to be performed by the agent_group.
        file_name (str, optional): The file name for the output YAML configuration. Defaults to "agent_group_config_output.yaml".
    ...
    Returns:
        Any: The output of the agent_group configuration generation process. This can be a AgentGroupRouter instance or an error message.
    """
    formatter.print_panel(
        "Auto Generating AgentGroup...", "Auto AgentGroup Builder"
    )
    ...
    def attempt_generate_agent_group_config():
        try:
            model = LiteLLM(model_name=model_name)

            # Initialize the agent
            agent = Agent(
                agent_name="Auto-AgentGroup-Builder",
                system_prompt=AUTO_GEN_PROMPT,
                llm=model,
                max_loops=1,
                dynamic_temperature_enabled=True,
                saved_state_path="agent_group_builder.json",
                user_name="agent_groups_corp",
                output_type="str",
            )

            # Generate output from the agent
            raw_output = agent.run(task, *args, **kwargs)
            yaml_content = parse_yaml_from_agent_group_markdown(raw_output)
            print(yaml_content)
            ...
            formatter.print_panel(
                "AgentGroup configuration generated successfully.",
                "Success",
            )
            ...
            formatter.print_panel(
                f"Error generating agent_group configuration: {str(e)}",
                "Error",
            )
            raise

    return attempt_generate_agent_group_config()
