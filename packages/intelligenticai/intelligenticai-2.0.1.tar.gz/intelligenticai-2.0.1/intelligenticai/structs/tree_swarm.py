...
logger = initialize_logger(log_folder="tree_agent_group")
...
class ForestAgentGroup:
    def __init__(
        self,
        name: str = "default-forest-agent_group",
        description: str = "Standard forest agent_group",
        trees: List[Tree] = [],
        ...
        **kwargs,
        ...
        """
        Initialize a ForestAgentGroup with multiple trees of agents.

        Args:
            name (str): Name of the forest agent_group
            description (str): Description of the forest agent_group
            trees (List[Tree]): A list of trees in the structure
            shared_memory (Any): Shared memory object for inter-tree communication
            rules (str): Rules governing the forest agent_group behavior
            verbose (bool): Whether to enable verbose logging
            *args: Additional positional arguments
            **kwargs: Additional keyword arguments
        """
        ...
        self.save_file_path = f"forest_agent_group_{uuid.uuid4().hex}.json"
    ...
    def run(self, task: str, img: str = None, *args, **kwargs) -> Any:
        ...
        try:
            ...
            if self.verbose:
                logger.error(
                    f"Error detected in the ForestAgentGroup, check your inputs and try again ;) {error}"
                )
...
# # Create the ForestAgentGroup
# multi_agent_structure = ForestAgentGroup(trees=[tree1, tree2])
