...
def get_agent_groups_info(agent_groups: List[Callable]) -> str:
    """
    Fetch and format information about all available agent_groups in the system.

    This function provides a comprehensive overview of all agent_groups currently
    available in the system, including their names, descriptions, agent counts,
    and agent_group types. It's useful for debugging, monitoring, and system introspection.

    Args:
        agent_groups (List[Callable]): List of agent_group instances to get information about.
                               Each agent_group should have name, description, agents, and agent_group_type attributes

    Returns:
        str: A formatted string containing detailed information about all agent_groups.
             Returns "No agent_groups currently available in the system." if the list is empty.

    Note:
        - Each agent_group is expected to have the following attributes:
          - name: The name of the agent_group
          - description: A description of the agent_group's purpose
          - agents: A list of agents in the agent_group
          - agent_group_type: The type/category of the agent_group
        - The output is formatted for human readability with clear section headers

    Example:
        >>> agent_groups = [agent_group1, agent_group2, agent_group3]
        >>> info = get_agent_groups_info(agent_groups)
        >>> print(info)
        Available AgentGroups:

        [AgentGroup 1]
        Name: Data Processing AgentGroup
        Description: Handles data analysis tasks
        Length of Agents: 5
        AgentGroup Type: Analysis
        ...
    """
    if not agent_groups:
        return "No agent_groups currently available in the system."
    ...
    for idx, agent_group in enumerate(agent_groups, 1):
        agent_group_info.extend(
            [
                f"[AgentGroup {idx}]",
                f"Name: {agent_group.name}",
                f"Description: {agent_group.description}",
                f"Length of Agents: {len(agent_group.agents)}",
                f"AgentGroup Type: {agent_group.agent_group_type}",
                "",  # Empty string for line spacing between agent_groups
            ]
        )

    return "\n".join(agent_group_info).strip()
