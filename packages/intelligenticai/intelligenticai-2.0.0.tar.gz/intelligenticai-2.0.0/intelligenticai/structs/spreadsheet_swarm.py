...
logger = initialize_logger(log_folder="spreadsheet_agent_group")
...
class SpreadSheetAgentGroup:
    """
    A agent_group that processes tasks concurrently using multiple agents.

    Args:
        name (str, optional): The name of the agent_group. Defaults to "Spreadsheet-AgentGroup".
        description (str, optional): The description of the agent_group. Defaults to "A agent_group that processes tasks concurrently using multiple agents.".
        agents (Union[Agent, List[Agent], None], optional): The agents participating in the agent_group. If None, agents will be loaded from load_path. Defaults to None.
        autosave (bool, optional): Whether to enable autosave of agent_group metadata. Defaults to True.
        save_file_path (str, optional): The file path to save the agent_group metadata as a CSV file. Defaults to "spreedsheet_agent_group.csv".
        max_loops (int, optional): The number of times to repeat the agent_group tasks. Defaults to 1.
    ...
    def __init__(
        self,
        name: str = "Spreadsheet-AgentGroup",
        description: str = "A agent_group that processes tasks concurrently using multiple agents and saves the metadata to a CSV file.",
        ...
        **kwargs,
        ...
        if self.workspace_dir:
            ...
            self.save_file_path = os.path.join(
                self.workspace_dir,
                f"spreadsheet_agent_group_run_id_{uuid_hex}.csv",
            )
        else:
            self.save_file_path = (
                f"spreadsheet_agent_group_run_id_{uuid_hex}.csv"
    ...
    def reliability_check(self):
        """
        Check the reliability of the agent_group.
        ...
        if self.verbose:
            logger.info(
                f"SpreadSheetAgentGroup Name: {self.name} reliability checks in progress..."
            )
        ...
        if self.verbose:
            logger.info(
                f"SpreadSheetAgentGroup Name: {self.name} reliability check passed."
            )
            logger.info(
                f"SpreadSheetAgentGroup Name: {self.name} is ready to run."
            )
    ...
    def run_from_config(self):
        ...
        return {
            "run_id": f"spreadsheet_agent_group_run_{uuid_hex}",
            "name": self.name,
    ...
    def _run(self, task: str = None, *args, **kwargs):
        """
        Run the agent_group either with a specific task or using configured tasks.
        ...
        Returns:
            dict: Summary of the agent_group execution.
        """
        # Load agents from CSV if no agents are provided but load_path is
        if not self.agents and self.load_path:
            self.load_from_csv()
    ...
    def run(self, task: str = None, *args, **kwargs):
        """
        Run the agent_group with the specified task.
        ...
        try:
            ...
            logger.error(f"Error running agent_group: {e}")
            raise e

    def _run_tasks(self, task: str, *args, **kwargs):
        """
        Run the agent_group tasks concurrently.
    ...
    def export_to_json(self):
        """
        Export the agent_group outputs to JSON.

        Returns:
            str: The JSON representation of the agent_group outputs.
        """
        import json
    ...
    def data_to_json_file(self):
        """
        Save the agent_group metadata to a JSON file.
        """
        out = self.export_to_json()

        create_file_in_folder(
            folder_path=f"{self.workspace_dir}/Spreedsheet-AgentGroup-{self.name}/{self.name}",
            file_name=f"spreedsheet-agent_group-{uuid_hex}-metadata.json",
            content=out,
        )

    def _save_metadata(self):
        """
        Save the agent_group metadata to CSV and JSON.
        """
        if self.autosave:
            self._save_to_csv()
    ...
    def _save_to_csv(self):
        """
        Save the agent_group metadata to a CSV file.
        """
        logger.info(
            f"Saving agent_group metadata to: {self.save_file_path}"
        )
        run_id = uuid.uuid4()
