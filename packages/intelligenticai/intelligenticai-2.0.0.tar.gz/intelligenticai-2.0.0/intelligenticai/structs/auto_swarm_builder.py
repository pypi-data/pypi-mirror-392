...
from intelligenticai.structs.ma_utils import set_random_models_for_agents
from intelligenticai.structs.agent_group_router import AgentGroupRouter, AgentGroupType
from intelligenticai.utils.litellm_wrapper import LiteLLM

load_dotenv()

execution_types = [
    "return-agents",
    "execute-agent_group-router",
    "return-agent_group-router-config",
...
- **SpreadSheetAgentGroup**: Structured data processing with coordinated workflows
...
- **AutoAgentGroupBuilder**: Self-organizing and self-optimizing agent teams
- **HiearchicalAgentGroup**: Layered decision-making with management and execution tiers
...
- **HeavyAgentGroup**: High-capacity processing with multiple specialized agents
...
class AgentSpec(BaseModel):
    ...
    agent_name: Optional[str] = Field(
        None,
        description="The unique name assigned to the agent, which identifies its role and functionality within the agent_group.",
    )
    ...
    role: Optional[str] = Field(
        "worker",
        description="The designated role of the agent within the agent_group, which influences its behavior and interaction with other agents.",
    )
...
class Agents(BaseModel):
    """Configuration for a collection of agents that work together as a agent_group to accomplish tasks."""

    agents: List[AgentSpec] = Field(
        description="A list containing the specifications of each agent that will participate in the agent_group, detailing their roles and functionalities."
    )


class AgentsConfig(BaseModel):
    """Configuration for a list of agents in a agent_group"""
...
class AutoAgentGroupBuilder:
    """A class that automatically builds and manages agent_groups of AI agents.

    This class handles the creation, coordination and execution of multiple AI agents working
    together as a agent_group to accomplish complex tasks. It uses a sophisticated boss agent system
    to delegate work and create new specialized agents as needed.

    Args:
        name (str): The name of the agent_group. Defaults to "auto-agent_group-builder".
        description (str): A description of the agent_group's purpose. Defaults to "Auto AgentGroup Builder".
    ...
    def __init__(
        self,
        name: str = "auto-agent_group-builder",
        description: str = "Auto AgentGroup Builder",
        verbose: bool = True,
        max_loops: int = 1,
    ...
    def reliability_check(self):
        """Perform reliability checks on the AutoAgentGroupBuilder configuration.
        ...
        if self.max_loops == 0:
            raise ValueError(
                f"AutoAgentGroupBuilder: {self.name} max_loops cannot be 0"
            )

        logger.info(
            f"Initializing AutoAgentGroupBuilder: {self.name} Description: {self.description}"
        )

    def _execute_task(self, task: str):
        """Execute a task by creating agents and initializing the agent_group router.

        Args:
            task (str): The task to execute

        Returns:
            Any: The result of the agent_group router execution
        """
        ...
        return self.initialize_agent_group_router(agents=agents, task=task)
    ...
    def create_router_config(self, task: str):
        try:
            logger.info(
                f"Creating agent_group router config for task: {task}"
            )

            model = self.build_llm_agent(config=AgentGroupRouterConfig)
            ...
            logger.error(
                f"Error creating agent_group router config: {str(e)} Traceback: {traceback.format_exc()}",
                exc_info=True,
            )
            raise e
    ...
    def initialize_agent_group_router(self, agents: List[Agent], task: str):
        """Initialize and run the agent_group router.
        ...
        """
        try:
            logger.info("Initializing agent_group router")
            model = self.build_llm_agent(config=AgentGroupRouterConfig)

            agent_group_spec = model.run(
                f"Create the agent_group spec for the following task: {task}"
            )

            print(agent_group_spec)

            print(type(agent_group_spec))

            agent_group_router = AgentGroupRouter(
                name=agent_group_spec["name"],
                description=agent_group_spec["description"],
                max_loops=1,
                agent_group_type=agent_group_spec["agent_group_type"],
                rearrange_flow=agent_group_spec["rearrange_flow"],
                rules=agent_group_spec["rules"],
                multi_agent_collab_prompt=agent_group_spec[
            ...
            logger.info("Starting agent_group router execution")
            return agent_group_router.run(task)
        except Exception as e:
            logger.error(
                f"Error in agent_group router initialization/execution: {str(e)}",
                exc_info=True,
            )
            raise

    def batch_run(self, tasks: List[str]):
        """Run the agent_group on a list of tasks.
    ...
    def run(self, task: str, *args, **kwargs):
        """Run the agent_group on a given task.
        ...
        Returns:
            Any: The result of the agent_group execution
        ...
        try:
            ...
            elif self.execution_type == "return-agent_group-router-config":
            ...
            logger.error(
                f"AutoAgentGroupBuilder: Error in agent_group execution: {str(e)} Traceback: {traceback.format_exc()}",
                exc_info=True,
            )
            raise e
