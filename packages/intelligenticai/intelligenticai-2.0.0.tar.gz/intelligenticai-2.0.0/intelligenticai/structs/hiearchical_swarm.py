"""
Hierarchical AgentGroup Implementation

This module provides a hierarchical agent_group architecture where a director agent coordinates
...
- Auto build agents from input prompt - and then add them to the agent_group
...
from intelligenticai.prompts.hiearchical_system_prompt import (
    HIEARCHICAL_AGENT_GROUP_SYSTEM_PROMPT,
...
class HierarchicalAgentGroupDashboard:
    """
    Futuristic Arasaka Corporation-style dashboard for hierarchical agent_group monitoring.

    This dashboard provides a professional, enterprise-grade interface with red and black
    color scheme, real-time monitoring of agent_group operations, and cyberpunk aesthetics.

    Attributes:
        console (Console): Rich console instance for rendering
        live_display (Live): Live display for real-time updates
        agent_group_name (str): Name of the agent_group being monitored
    ...
    def __init__(self, agent_group_name: str = "AgentGroups Corporation"):
        """
        Initialize the Arasaka dashboard.

        Args:
            agent_group_name (str): Name of the agent_group to display in the dashboard
        """
        ...
        # AgentGroup information
        self.agent_group_description = ""
    ...
    def _create_header(self) -> Panel:
        """Create the dashboard header with AgentGroups Corporation branding."""
        header_text = Text()
        header_text.append(
            "╔══════════════════════════════════════════════════════════════════════════════╗\n",
            style="bold red",
        ...
        header_text.append("                    ", style="bold red")
        header_text.append(
            "AGENT_GROUPS CORPORATION", style="bold white on red"
        )
        header_text.append("                    ", style="bold red")
        header_text.append("║\n", style="bold red")
        header_text.append("║", style="bold red")
        header_text.append("                    ", style="bold red")
        header_text.append(
            "HIERARCHICAL AGENT_GROUP OPERATIONS CENTER", style="bold red"
        )
        header_text.append("                    ", style="bold red")
        header_text.append("║\n", style="bold red")
        header_text.append(
    ...
    def _create_status_panel(self) -> Panel:
        ...
        status_text.append(
            "By the AgentGroups Corporation", style="bold cyan"
        )
        status_text.append("\n", style="white")
        status_text.append(
        ...
        # AgentGroup information
        status_text.append("AGENT_GROUP NAME: ", style="bold white")
        status_text.append(f"{self.agent_group_name}", style="bold cyan")

        status_text.append("\n", style="white")
        status_text.append("DESCRIPTION: ", style="bold white")
        status_text.append(f"{self.agent_group_description}", style="white")
    ...
    def update_agent_group_info(
        ...
        """Update the dashboard with agent_group-specific information."""
        self.agent_group_name = name
        self.agent_group_description = description
        self.max_loops = max_loops
...
class HierarchicalOrder(BaseModel):
    """
    Represents a single task assignment within the hierarchical agent_group.
    ...
    Attributes:
        agent_name (str): The name of the agent assigned to execute the task.
                         Must match an existing agent in the agent_group.
    ...
    agent_name: str = Field(
        ...,
        description="Specifies the name of the agent to which the task is assigned. This is a crucial element in the hierarchical structure of the agent_group, as it determines the specific agent responsible for the task execution.",
    )
    task: str = Field(
        ...,
        description="Defines the specific task to be executed by the assigned agent. This task is a key component of the agent_group's plan and is essential for achieving the agent_group's goals.",
    )
...
class AgentGroupSpec(BaseModel):
    """
    Defines the complete specification for a hierarchical agent_group execution.

    This class contains the overall plan and all individual orders that the director
    creates to coordinate the agent_group's activities. It serves as the structured output
    format for the director agent.

    Attributes:
        plan (str): A comprehensive plan outlining the sequence of actions and strategy
                   for the entire agent_group to accomplish the given task.
        orders (List[HierarchicalOrder]): A list of specific task assignments to
                                         individual agents within the agent_group.
    """

    plan: str = Field(
        ...,
        description="A plan generated by the director agent for the agent_group to accomplish the given task, where the director autonomously reasons through the problem, devises its own strategy, and determines the sequence of actions. "
        ...
        "It serves as a blueprint for the agent_group, enabling agents to follow the director's self-derived guidance and adapt as needed throughout the process.",
    )

    orders: List[HierarchicalOrder] = Field(
        ...,
        description="A collection of task assignments to specific agents within the agent_group. These orders are the specific instructions that guide the agents in their task execution and are a key element in the agent_group's plan.",
    )


class HierarchicalAgentGroup:
    """
    A hierarchical agent_group orchestrator that coordinates multiple agents through a director.
    ...
    The agent_group maintains conversation history throughout the process, allowing for
    context-aware decision making and iterative refinement of results.

    Attributes:
        name (str): The name identifier for this agent_group instance.
        description (str): A description of the agent_group's purpose and capabilities.
        director (Optional[Union[Agent, Callable, Any]]): The director agent that
                                                         coordinates the agent_group.
        agents (List[Union[Agent, Callable, Any]]): List of worker agents available
                                                   for task execution.
        max_loops (int): Maximum number of feedback loops the agent_group can perform.
        output_type (OutputType): Format for the final output of the agent_group.
    ...
    def __init__(
        self,
        name: str = "HierarchicalAgentAgentGroup",
        description: str = "Distributed task agent_group",
        ...
        director_system_prompt: str = HIEARCHICAL_AGENT_GROUP_SYSTEM_PROMPT,
        ...
        **kwargs,
        ...
        """
        Initialize a new HierarchicalAgentGroup instance.

        Args:
            name (str): The name identifier for this agent_group instance.
            description (str): A description of the agent_group's purpose.
    ...
    def initialize_agent_group(self):
        if self.interactive:
        ...
        self.dashboard = None
        if self.interactive:
            self.dashboard = HierarchicalAgentGroupDashboard(self.name)
            # Enable detailed view for better output visibility
            self.dashboard.detailed_view = True
            # Pass additional agent_group information to dashboard
            self.dashboard.update_agent_group_info(
                name=self.name,
    ...
    def init_agent_group(self):
        """
        Initialize the agent_group with proper configuration and validation.
        ...
        Raises:
            ValueError: If the agent_group configuration is invalid.
        """
    ...
    def setup_director(self):
        ...
        Creates a new director agent with the AgentGroupSpec schema for structured
        output, enabling it to create plans and distribute orders effectively.

        Returns:
            Agent: A configured director agent ready to coordinate the agent_group.

        Raises:
            Exception: If director setup fails due to configuration issues.
        """
        try:
            schema = BaseTool().base_model_to_dict(AgentGroupSpec)
            ...
            logger.error(
                f"{error_msg}\n[TRACE] Traceback: {traceback.format_exc()}\n[BUG] If this issue persists, please report it at: https://github.com/kyegomez/agent_groups/issues"
            )
    ...
    def reliability_checks(self):
        """
        Perform validation checks to ensure the agent_group is properly configured.
        ...
        try:
            if not self.agents or len(self.agents) == 0:
                raise ValueError(
                    "No agents found in the agent_group. At least one agent must be provided to create a hierarchical agent_group."
                )
    ...
    def run_director(
        ...
        Returns:
            AgentGroupSpec: The director's output containing the plan and orders.
    ...
    def step(
        ...
        **kwargs,
        ...
        """
        Execute a single step of the hierarchical agent_group workflow.

        This method performs one complete iteration of the agent_group's workflow:
    ...
    def run(
        ...
        **kwargs,
        ...
        """
        Execute the hierarchical agent_group for the specified number of feedback loops.

        This method orchestrates the complete agent_group execution, performing multiple
        ...
        Args:
            task (str, optional): The initial task to be processed by the agent_group.
        ...
        Raises:
            Exception: If agent_group execution fails.
        """
        try:
            ...
            while current_loop < self.max_loops:
                ...
                # Execute one step of the agent_group
                try:
            ...
            error_msg = f"[ERROR] AgentGroup run failed: {str(e)}"
            logger.error(
                f"{error_msg}\n[TRACE] Traceback: {traceback.format_exc()}\n[BUG] If this issue persists, please report it at: https://github.com/kyegomez/agent_groups/issues"
            )
    ...
    def _get_interactive_task(self) -> str:
        ...
        if self.dashboard:
            self.dashboard.console.print(
                "\n[bold red]AGENT_GROUPS CORPORATION[/bold red] - [bold white]TASK INPUT REQUIRED[/bold white]"
            )
            self.dashboard.console.print(
                "[bold cyan]Enter your task for the hierarchical agent_group:[/bold cyan]"
            )
    ...
    def feedback_director(self, outputs: list):
        ...
        try:
            ...
            feedback_director = Agent(
                ...
                model_name=self.director_model_name,
                max_loops=1,
                system_prompt=HIEARCHICAL_AGENT_GROUP_SYSTEM_PROMPT,
            )
    ...
    def call_single_agent(
        ...
        **kwargs,
        ...
        Raises:
            ValueError: If the specified agent is not found in the agent_group.
        ...
        try:
            ...
            if agent is None:
                ...
                raise ValueError(
                    f"Agent '{agent_name}' not found in agent_group. Available agents: {available_agents}"
                )
    ...
    def execute_orders(
        ...
        try:
            ...
            error_msg = (
                "\n"
                + "=" * 60
                + "\n[AGENT_GROUPS ERROR] Order Execution Failure\n"
                + "-" * 60
                + f"\nError   : {str(e)}"
                f"\nTrace   :\n{traceback.format_exc()}"
                + "-" * 60
                + "\nIf this issue persists, please report it:"
                "\n  https://github.com/kyegomez/agent_groups/issues"
                "\n" + "=" * 60 + "\n"
            )
    ...
    def batched_run(
        ...
        **kwargs,
        ...
        """
        Execute the hierarchical agent_group for multiple tasks in sequence.

        This method processes a list of tasks sequentially, running the complete
        agent_group workflow for each task. Each task is processed independently with
        its own conversation context and results.

        Args:
            tasks (List[str]): List of tasks to be processed by the agent_group.
        ...
        try:
            ...
            error_msg = f"[ERROR] Batched hierarchical agent_group run failed: {str(e)}"
            logger.error(
                f"{error_msg}\n[TRACE] Traceback: {traceback.format_exc()}\n[BUG] If this issue persists, please report it at: https://github.com/kyegomez/agent_groups/issues"
            )
