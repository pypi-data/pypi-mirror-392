...
import json
import uuid
from intelligenticai.utils.file_processing import create_file_in_folder
from abc import ABC
...
from intelligenticai.structs.omni_agent_types import AgentType
from pydantic import BaseModel
from intelligenticai.utils.loguru_logger import initialize_logger

logger = initialize_logger(log_folder="base_agent_group")


class BaseAgentGroup(ABC):
    """
    Base AgentGroup Class for all multi-agent systems
    ...
    Methods:
        communicate: Communicate with the agent_group through the orchestrator, protocols, and the universal communication layer
        run: Run the agent_group
        step: Step the agent_group
        add_agent: Add a agent to the agent_group
        remove_agent: Remove a agent from the agent_group
        broadcast: Broadcast a message to all agents
        reset: Reset the agent_group
        ...
        get_agent_group_size: Get the size of the agent_group
        get_agent_group_status: Get the status of the agent_group
        save_agent_group_state: Save the agent_group state
        loop: Loop through the agent_group
        run_async: Run the agent_group asynchronously
        run_batch_async: Run the agent_group asynchronously
        run_batch: Run the agent_group asynchronously
        batched_run: Run the agent_group asynchronously
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialize the agent_group with agents"""
        self.name = name
    ...
    def communicate(self):
        """Communicate with the agent_group through the orchestrator, protocols, and the universal communication layer"""
        ...
    ...
    def agent_exists(self, name: str):
        """
        Check if an agent exists in the agent_group.
    ...
    def get_agent_group_size(self) -> int:
        """Get the size of the agent_group"""

    # #@abstractmethod
    def get_agent_group_status(self) -> Dict:
        """Get the status of the agent_group"""

    # #@abstractmethod
    def save_agent_group_state(self):
        """Save the agent_group state"""
    ...
    async def aloop(
        ...
        **kwargs,
        ...
        """Asynchronous loop through the agent_group
    ...
    def run_async(self, task: Optional[str] = None, *args, **kwargs):
        """Run the agent_group asynchronously
    ...
    def add_agent_group_entry(self, agent_group):
        """
        Add the information of a joined AgentGroup to the registry.

        Args:
            agent_group (AgentGroupManagerBase): Instance of AgentGroupManagerBase representing the joined AgentGroup.
    ...
    def retrieve_agent_group_information(self, agent_group_id: str):
        """
        Retrieve the information of a specific AgentGroup from the registry.

        Args:
            agent_group_id (str): Unique identifier of the AgentGroup.

        Returns:
            AgentGroupManagerBase: Instance of AgentGroupManagerBase representing the retrieved AgentGroup, or None if not found.
        """
    ...
    def join_agent_group(
        ...
        """
        Add a relationship between a AgentGroup and an Agent or other AgentGroup to the registry.

        Args:
            from (Agent | AgentGroupManagerBase): Instance of Agent or AgentGroupManagerBase representing the source of the relationship.
        """
    ...
    def agent_error_handling_check(self):
        try:
            if self.agents is None:
                message = "You have not passed in any agents, you need to input agents to run a agent_group"
                logger.info(message)
    ...
    def agent_group_initialization(self, *args, **kwargs):
        """
        Initializes the hierarchical agent_group.
        ...
        """
        logger.info(
            f"Initializing the hierarchical agent_group: {self.name}"
        )
        logger.info(f"Purpose of this agent_group: {self.description}")
        ...
        if len(self.agents) == 0:
            logger.info(
                "No agents found. Please add agents to the agent_group."
            )
            return None
        ...
        if self.director is None:
            logger.info(
                "No director found. Please add a director to the agent_group."
            )
            return None

        logger.info(
            f"Initialization complete for the hierarchical agent_group: {self.name}"
        )

    def export_output_schema(self):
        """
        Export the output schema of the agent_group.

        Returns:
            dict: The output schema of the agent_group.
    ...
    def list_agents(self):
        """
        List all agents in the agent_group.
