...
from intelligenticai.schemas.agent_step_schemas import ManySteps
...
class MetadataSchema(BaseModel):
    ...
    name: Optional[str] = Field(
        "RoundRobinAgentGroup", description="Name of the agent_group"
    )
...
class RoundRobinAgentGroup(BaseAgentGroup):
    """
    A agent_group implementation that executes tasks in a round-robin fashion.

    Args:
        agents (List[Agent], optional): List of agents in the agent_group. Defaults to None.
    ...
    Attributes:
        agents (List[Agent]): List of agents in the agent_group.
    ...
    def __init__(
        self,
        name: str = "RoundRobinAgentGroup",
        description: str = "A agent_group implementation that executes tasks in a round-robin fashion.",
    ...
    def run(self, task: str, *args, **kwargs):
        ...
        """
        if not self.agents:
            logger.error("No agents configured for the agent_group")
            raise ValueError("No agents configured for the agent_group")
