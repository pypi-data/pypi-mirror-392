...
Given these identified agent roles: '{agent_roles}', write SOPs/System Prompts for each agent type. Ensure that each SOP/Prompt is tailored to the specific functionalities of the agent, considering the operational context and objectives of the agent_group team.
...
# Prompt for AgentGroup Assembly Agent
AGENT_GROUP_ASSEMBLY_AGENT_PROMPT = """
...
1. **Import Statements**:
   - Begin with necessary Python imports. Import the 'Agent' class from the 'agent_groups.structs' module.
   - Import the language or vision model from 'agent_groups.models', depending on the nature of the agent_group (text-based or image-based tasks).
   - Import the SOPs for each agent type from intelligenticai.prompts.(insert agent_group team name here). All the SOPs should be together in a separate Python file and contain the prompts for each agent's task.
   - Use os.getenv for the OpenAI API key.

2. **Initialize the AI Model**:
   - If the agent_group involves text processing, initialize 'OpenAIChat' with the appropriate API key.
...
4. **Define the AgentGroup's Workflow**:
...
Here is an example of a a working agent_group script that you can use as a rough template for the logic:
import os
from dotenv import load_dotenv
from agent_group_models import OpenAIChat
from intelligenticai.structs import Agent
import intelligenticai.prompts.agent_group_daddy as sdsp
...
agent_group_assembly_agent = Agent(llm=llm, sop=sdsp.AGENT_GROUP_ASSEMBLY_AGENT_PROMPT, max_loops=1)
...
agent_group_assembly_output = agent_group_assembly_agent.run(agent_configuration_output)
testing_optimization_output = testing_optimization_agent.run(agent_group_assembly_output)
"""


# Prompt for Testing and Optimization Agent
TESTING_OPTIMIZATION_AGENT_PROMPT = """
Review this Python script for agent_group demonstration: '{agent_group_script}'. Create a testing and optimization plan that includes methods for validating each agent's functionality and the overall performance of the agent_group. Suggest improvements for efficiency and effectiveness.
