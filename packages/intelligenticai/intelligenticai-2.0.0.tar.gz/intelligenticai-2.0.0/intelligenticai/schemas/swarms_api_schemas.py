...
class AgentGroupSpec(BaseModel):
    name: Optional[str] = Field(
        None,
        description="The name of the agent_group, which serves as an identifier for the group of agents and their collective task.",
        max_length=100,
    )
    description: Optional[str] = Field(
        None,
        description="A comprehensive description of the agent_group's objectives, capabilities, and intended outcomes.",
    )
    agents: Optional[List[AgentSpec]] = Field(
        None,
        description="A list of agents or specifications that define the agents participating in the agent_group.",
    )
    max_loops: Optional[int] = Field(
        default=1,
        description="The maximum number of execution loops allowed for the agent_group, enabling repeated processing if needed.",
    )
    agent_group_type: Optional[AgentGroupType] = Field(
        None,
        description="The classification of the agent_group, indicating its operational style and methodology.",
    )
    ...
    task: Optional[str] = Field(
        None,
        description="The specific task or objective that the agent_group is designed to accomplish.",
    )
    img: Optional[str] = Field(
        None,
        description="An optional image URL that may be associated with the agent_group's task or representation.",
    )
    return_history: Optional[bool] = Field(
        True,
        description="A flag indicating whether the agent_group should return its execution history along with the final output.",
    )
    rules: Optional[str] = Field(
        None,
        description="Guidelines or constraints that govern the behavior and interactions of the agents within the agent_group.",
    )
    tasks: Optional[List[str]] = Field(
        None,
        description="A list of tasks that the agent_group should complete.",
    )
    messages: Optional[
        Union[List[Dict[Any, Any]], Dict[Any, Any]]
    ] = Field(
        None,
        description="A list of messages that the agent_group should complete.",
    )
    stream: Optional[bool] = Field(
        False,
        description="A flag indicating whether the agent_group should stream its output.",
    )
