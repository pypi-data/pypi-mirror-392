...
from intelligenticai.agents.auto_generate_agent_group_config import (
    generate_agent_group_config,
)
...
from intelligenticai.utils.formatter import formatter
...
class AgentGroupCLIError(Exception):
    """Custom exception for AgentGroup CLI errors"""
...
def show_ascii_art():
    ...
    panel = Panel(
        ...
        title="[bold]Intelligentic CLI[/bold]",
    )

    console.print(panel)

    formatter.print_panel(
        "Access the full Intelligentic CLI documentation and API guide at https://docs.intelligentic.world/en/latest/intelligentic/cli/cli_reference/. For help with a specific command, use intelligentic <command> --help to unlock the full power of Intelligentic CLI.",
...
def check_intelligentic_version(
    verbose: bool = False,
) -> tuple[bool, str, str, str]:
    """Check if intelligentic is at the latest version."""
    try:
        ...
        if verbose:
            console.print(
                "[dim]🔍 Attempting to detect Intelligentic version...[/dim]"
            )
        ...
        try:
            ...
            current_version = importlib.metadata.version("intelligentic")
            if verbose:
                console.print(
        ...
        if current_version == "Unknown":
            try:
                import intelligenticai

                current_version = getattr(
                    intelligentic, "__version__", "Unknown"
                )
                if verbose:
                    console.print(
        ...
        if current_version == "Unknown":
            try:
                ...
                intelligentic_path = intelligentic.__file__
                if intelligentic_path:
                    init_file = os.path.join(
                        os.path.dirname(intelligentic_path), "__init__.py"
                    )
        ...
        if current_version == "Unknown":
            try:
                ...
                if intelligentic_path:
                    ...
                    current_dir = os.path.dirname(intelligentic_path)
        ...
        try:
            ...
            with httpx.Client(timeout=5.0) as client:
                response = client.get(
                    "https://pypi.org/pypi/intelligentic/json"
                )
...
def run_setup_check(verbose: bool = False):
    ...
    console.print(
        "\n[bold blue]🔍 Running Intelligentic Environment Setup Check[/bold blue]\n"
    )
    ...
    checks = [
        ...
        ("Intelligentic Version", check_intelligentic_version(verbose)),
    ...
    if all_passed:
        summary_panel = Panel(
            Align.center(
                "[bold green]🎉 All checks passed! Your environment is ready for Intelligentic.[/bold green]"
            ),
    ...
    if not all_passed:
            ...
            *extra,
            ...
            if not passed:
                if "WORKSPACE_DIR" in check_name:
                    ...
                    "Intelligentic Version" in check_name and len(extra) > 0
                ):
                    latest_version = extra[0]
                    if latest_version != "Unknown":
                        recommendations.append(
                            f"Update Intelligentic: pip install --upgrade intelligentic (latest: {latest_version})"
                        )
        ...
        console.print(
            "\n[dim]Run 'intelligentic setup-check' again after making changes to verify.[/dim]"
        )
...
def create_command_table() -> Table:
    ...
    commands = [
        ...
        ("auto-upgrade", "Update Intelligentic to the latest version"),
        ("book-call", "Schedule a strategy session with our team"),
        ("autoagent_group", "Generate and execute an autonomous agent_group"),
        (
            "setup-check",
...
def create_detailed_command_table() -> Table:
    ...
    table = Table(
        ...
        header_style=f"bold {COLORS['primary']}",
        border_style=COLORS["secondary"],
        title="🚀 Intelligentic CLI - Complete Command Reference",
    ...
    commands = [
        {
            ...
            "usage": "intelligentic onboarding [--verbose]",
            "args": "--verbose",
        },
        {
            "cmd": "help",
        ...
        },
        {
            ...
            "usage": "intelligentic get-api-key",
            "args": "None",
        },
        {
            "cmd": "check-login",
            ...
            "usage": "intelligentic check-login",
            "args": "None",
        },
        {
            "cmd": "run-agents",
            ...
            "usage": "intelligentic run-agents --yaml-file agents.yaml",
            "args": "--yaml-file",
        },
        {
            "cmd": "load-markdown",
            ...
            "usage": "intelligentic load-markdown --markdown-path ./agents/",
            "args": "--markdown-path",
        },
        {
            "cmd": "agent",
            ...
            "usage": "intelligentic agent --name 'Agent' --task 'Analyze data'",
            "args": "--name, --task",
        },
        {
            "cmd": "auto-upgrade",
            "category": "Maintenance",
            "desc": "Update Intelligentic to latest version",
            "usage": "intelligentic auto-upgrade",
            "args": "None",
        },
        {
            "cmd": "book-call",
            ...
            "usage": "intelligentic book-call",
            "args": "None",
        },
        {
            "cmd": "autoagent_group",
            "category": "AI Gen",
            "desc": "Generate autonomous agent_group config",
            "usage": "intelligentic autoagent_group --task 'analyze data' --model gpt-4",
            "args": "--task, --model",
        },
        {
            "cmd": "setup-check",
            ...
            "usage": "intelligentic setup-check [--verbose]",
            "args": "--verbose",
        },
    ]
...
def show_help():
    ...
    console.print(
        "\n[bold]Intelligentic CLI - Command Reference[/bold]\n",
    ...
    usage_panel = Panel(
        ...
        "• [yellow]intelligentic onboarding[/yellow] - Environment setup check\n"
        "• [yellow]intelligentic setup-check[/yellow] - Check your environment\n"
        "• [yellow]intelligentic agent --name 'MyAgent' --task 'Hello World'[/yellow] - Create agent\n"
        "• [yellow]intelligentic autoagent_group --task 'analyze data' --model gpt-4[/yellow] - Auto-generate agent_group",
    ...
    docs_panel = Panel(
        "📚 [bold]Documentation:[/bold] https://docs.intelligentic.world\n"
        "🐛 [bold]Support:[/bold] https://github.com/kyegomez/intelligentic/issues\n"
    ...
    console.print(
        "\n[dim]💡 Tip: Use [bold]intelligentic setup-check --verbose[/bold] for detailed environment diagnostics[/dim]"
    )
...
def get_api_key():
    ...
    with create_spinner("Opening API key portal...") as progress:
        ...
        webbrowser.open("https://intelligentic.world/platform/api-keys")
        time.sleep(1)
        progress.remove_task(task)
    console.print(
...
def run_autoagent_group(task: str, model: str):
    """Run autoagent_group with enhanced error handling"""
    try:
        console.print(
            "[yellow]Initializing autoagent_group configuration...[/yellow]"
        )

        # Validate inputs
        if not task or task.strip() == "":
            raise AgentGroupCLIError("Task cannot be empty")

        if not model or model.strip() == "":
            raise AgentGroupCLIError("Model name cannot be empty")

        # Attempt to generate agent_group configuration
        console.print(
            f"[yellow]Generating agent_group for task: {task}[/yellow]"
        )
        result = generate_agent_group_config(task=task, model=model)

        if result:
            console.print(
                "[green]✓ AgentGroup configuration generated successfully![/green]"
            )
        else:
            raise AgentGroupCLIError(
                "Failed to generate agent_group configuration"
            )
        ...
        if "No YAML content found" in str(e):
            ...
            show_error(
                f"Error during autoagent_group execution: {str(e)}",
                "For debugging, try:\n"
...
def main():
    try:
        ...
        parser = argparse.ArgumentParser(
            description="Intelligentic Cloud CLI"
        )
        parser.add_argument(
            "command",
            choices=[
        ...
        # AgentGroup agent specific arguments
        parser.add_argument(
            "--name",
            type=str,
        ...
        try:
            if args.command == "onboarding":
                ...
                console.print(
                    "[yellow]Note: 'intelligentic onboarding' now runs the same checks as 'intelligentic setup-check'[/yellow]"
                )
                ...
                try:
                    ...
                    if "context_length_exceeded" in error_msg:
                        ...
                        show_error(
                            ...
                            "3. Run 'intelligentic get-api-key' to get a new key",
                        )
                    else:
                        show_error(
                            "Execution Error",
                ...
                result = create_agent_group_agent(
                    name=args.name,
                ...
                webbrowser.open(
                    "https://cal.com/intelligentic/intelligentic-strategy-session"
                )
            elif args.command == "autoagent_group":
                if not args.task:
                    show_error(
                        "Missing required argument: --task",
                        "Example usage: python cli.py autoagent_group --task 'analyze this data' --model gpt-4",
                    )
                    exit(1)
                run_autoagent_group(args.task, args.model)
