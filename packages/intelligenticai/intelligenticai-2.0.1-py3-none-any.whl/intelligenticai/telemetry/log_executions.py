...
def log_function_execution(
    ...
    agent_group_architecture: Optional[str] = None,
    ...
    Args:
        agent_group_id (str, optional): Unique identifier for the agent_group instance
        agent_group_architecture (str, optional): Name of the agent_group architecture
    ...
    Example:
        >>> @log_function_execution(agent_group_id="my-agent_group", agent_group_architecture="sequential")
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            execution_data = {
                ...
                "agent_group_architecture": agent_group_architecture,
                "timestamp": timestamp,
...
def log_execution(
    ...
    """
    Log execution data for a agent_group router instance.

    This function logs telemetry data about agent_group router executions, including
    the agent_group ID, execution status, and configuration details. It silently
    handles any logging errors to prevent execution interruption.

    Args:
        agent_group_id (str): Unique identifier for the agent_group router instance
        status (str): Current status of the execution (e.g., "start", "completion", "error")
        agent_group_config (dict): Configuration dictionary containing agent_group router settings
        agent_group_architecture (str): Name of the agent_group architecture used
    Returns:
        None

    Example:
        >>> log_execution(
        ...     agent_group_id="agent_group-router-abc123",
        ...     status="start",
        ...     agent_group_config={"name": "my-agent_group", "agent_group_type": "SequentialWorkflow"}
        ... )
    """
    try:
        if enabled_on is None:
            log_agent_data(
                data_dict={
                    "agent_group_router_id": agent_group_id,
                    "status": status,
                    "agent_group_router_config": agent_group_config,
                    "agent_group_architecture": agent_group_architecture,
                }
            )
        else:
