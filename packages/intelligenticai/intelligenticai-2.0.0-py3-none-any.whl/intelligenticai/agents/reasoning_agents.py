...
from intelligenticai.agents.i_agent import (
...
class ReasoningAgentRouter:
    """
    A router for advanced reasoning agent agent_groups.

    The ReasoningAgentRouter enables dynamic selection, instantiation, and caching of various
    reasoning agent types ("agent_groups") for flexible, robust, and scalable problem-solving.
    ...
    Args:
        ...
        agent_group_type (agent_types): Type of reasoning agent_group to use.
    ...
    Example:
        >>> router = ReasoningAgentRouter(agent_group_type="reasoning-duo")
    ...
    def __init__(
        ...
        max_loops: int = 1,
        agent_group_type: agent_types = "reasoning-duo",
        num_samples: int = 1,
    ...
    def select_agent_group(self):
        """
        Select and initialize the appropriate reasoning agent_group based on the specified agent_group type.

        Returns:
            The selected reasoning agent_group instance.

        Raises:
            ValueError: If the specified agent_group type is invalid.
        """
        try:
            if self.agent_group_type in self.agent_factories:
                return self.agent_factories[self.agent_group_type]()
            else:
                raise ReasoningAgentInitializationError(
                    f"ReasoningAgentRouter Error: Invalid agent_group type: {self.agent_group_type}"
                )
        except Exception as e:
            raise ReasoningAgentInitializationError(
                f"ReasoningAgentRouter Error: {e} Traceback: {traceback.format_exc()} If the error persists, please check the agent's configuration and try again. If you would like support book a call with our team at https://cal.com/agent_groups"
            )

    def run(self, task: str, *args, **kwargs):
        """
        Execute the reasoning process of the selected agent_group on a given task.
        ...
        """
        try:
            agent_group = self.select_agent_group()
            return agent_group.run(task=task, *args, **kwargs)
        except Exception as e:
