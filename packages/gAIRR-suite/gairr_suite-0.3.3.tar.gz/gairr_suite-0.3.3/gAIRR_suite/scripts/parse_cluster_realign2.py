...
def cluster_separate(fn_cluster_contig, fn_sam):
    ...
    for segments in f_sam:
        if segments.is_unmapped == True:
            continue
        contig_name = segments.reference_name
        dict_contig[contig_name][3].append(segments)
...
def mark_edit_region(contig_name, contig_info, ignore_S=False):
    ...
    for fields in list_read_field:
        read_name = fields.query_name
        read_SEQ  = fields.query_sequence
        cigar     = fields.cigartuples
        sam_flag  = fields.flag

        # pass the read if the alignment is a supplementary alignment or a big chunk of read being soft-clipped
        if fields.is_supplementary:
            continue
        for ele in cigar:
            if ele[1] == 4 and ele[0] >= len(read_SEQ)/15:
                continue

        edit_dist = fields.get_tag('NM')  # NM:i:2 tag
        MD_tag    = fields.get_tag('MD')  # MD:Z:38G2A20
        start_pos = fields.reference_start
