...
def test_mock_function_fallback():
    """Test that mock functions are used when function not in registry."""
    src = """
fn step1(x: Int) -> Int;
...
"""
    ...
    # Default mock returns {"mock": "step1", "input": 42}
    assert isinstance(step.output_snapshot, dict)
    assert step.output_snapshot["mock"] == "step1"
    assert step.output_snapshot["input"] == 42
    assert bundle.output_value == step.output_snapshot
...
def test_mixed_mock_and_real_functions():
    """Test proof generation with mix of mock and real functions."""
    src = """
fn real_fn(x: Int) -> Int;
fn mock_fn(x: Int) -> Int;

pipeline main(Int) -> Int { real_fn -> mock_fn }
"""
    bundle = run_program_with_proof(
        source=src,
        input_value=10,
    ...
    # First step uses real function
    assert bundle.steps[0].step_name == "real_fn"
    assert bundle.steps[0].output_snapshot == 20

    # Second step uses mock function
    assert bundle.steps[1].step_name == "mock_fn"
    assert bundle.steps[1].input_snapshot == 20
    assert isinstance(bundle.steps[1].output_snapshot, dict)
    assert bundle.steps[1].output_snapshot["mock"] == "mock_fn"
    assert bundle.steps[1].output_snapshot["input"] == 20
