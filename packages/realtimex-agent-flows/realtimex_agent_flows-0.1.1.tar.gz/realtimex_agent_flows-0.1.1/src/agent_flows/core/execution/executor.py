...
class FlowExecutor:
    ...
    def _build_final_result(
        ...
        # Extract UI components and flow_as_output from finish metadata if present
        ui_components_data = None
        flow_as_output = False

        if execution_result.finish_metadata:
            # Get UI component data from finish step
            if "ui_component_data" in execution_result.finish_metadata:
                ui_component_data = execution_result.finish_metadata["ui_component_data"]
                # Wrap in list format expected by FlowResult
                ui_components_data = [ui_component_data]

            # Get flow_as_output flag from finish step
    ...
    def _build_error_result(
        ...
        # Include raw UI component in error result (no interpolation for early failures)
        ui_components = None
        if isinstance(flow_source, FlowConfig) and flow_source.ui_components:
            ui_components = flow_source.ui_components
