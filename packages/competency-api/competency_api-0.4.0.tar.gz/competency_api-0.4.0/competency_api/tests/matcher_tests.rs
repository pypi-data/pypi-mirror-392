...
mod tests {
    ...
    fn test_exact_skill_matches() -> Result<()> {
        ...
        // Debug output
        eprintln!("Python probability: {}", skill_map["Python"].probability);
        eprintln!("Java probability: {}", skill_map["Java"].probability);
        eprintln!("SQL probability: {}", skill_map["SQL"].probability);
        eprintln!("Overall score: {}", result.overall_score);

        // Python: Candidate has 4/5 vs required 5/5, still high probability
        assert!(skill_map["Python"].probability > 0.7, "Python probability: {}", skill_map["Python"].probability);
        // Java: Candidate matches exactly 3/5
        assert!(skill_map["Java"].probability > 0.90, "Java probability: {}", skill_map["Java"].probability);
        // SQL: Candidate matches exactly at max level 5/5
        assert!(skill_map["SQL"].probability > 0.90, "SQL probability: {}", skill_map["SQL"].probability);
    ...
    fn test_skill_gaps() -> Result<()> {
        ...
        // Python: Candidate has 2/5 vs required 3/5 - still decent match due to exact skill name
        assert!(skill_map["Python"].probability < 0.9, "Python probability: {}", skill_map["Python"].probability);
        // Data Science: Partial match through Data Analysis
        assert!(skill_map["Data Science"].probability < 0.5, "Data Science probability: {}", skill_map["Data Science"].probability);
        // Statistics: No direct match, but some transfer from Data Analysis
        assert!(skill_map["Statistics"].probability < 0.6, "Statistics probability: {}", skill_map["Statistics"].probability);
    ...
    fn test_comprehensive_semantic_similarity() -> Result<()> {
        ...
        let test_cases = vec![
            // Very high similarity - same domain, strong semantic match
            (vec![create_skill("PostgreSQL", 5, 5)], vec![create_skill("SQL", 4, 5)], 0.85, "PostgreSQL is SQL"),
            (vec![create_skill("JavaScript", 5, 5)], vec![create_skill("TypeScript", 3, 5)], 0.70, "JS/TS related"),

            // Good similarity - same domain helps
            (vec![create_skill("Python", 5, 5)], vec![create_skill("Programming", 4, 5)], 0.60, "Python is programming"),
            (vec![create_skill("Machine Learning", 5, 5)], vec![create_skill("Data Science", 4, 5)], 0.50, "ML within DS domain"),
            (vec![create_skill("Statistics", 4, 5)], vec![create_skill("Data Analysis", 3, 5)], 0.45, "Stats relates to analysis"),

            // Moderate similarity
            (vec![create_skill("MySQL", 4, 5)], vec![create_skill("Database", 3, 5)], 0.40, "MySQL is database"),
        ];
    ...
    fn test_tech_stack_similarity() -> Result<()> {
        ...
        // With proper domain classification (same domain), score improves significantly
        // But embeddings still struggle with specific tech → abstract concept mapping
        assert!(result.overall_score > 0.35, "Tech stack score: {}", result.overall_score);
        Ok(())
    }
    ...
    fn test_perfect_match_different_scales() -> Result<()> {
        ...
        // Perfect relative match (both 100%) - adjusted based on actual distribution behavior
        assert!(result.overall_score > 0.95, "Perfect match score: {}", result.overall_score);
        Ok(())
    }
