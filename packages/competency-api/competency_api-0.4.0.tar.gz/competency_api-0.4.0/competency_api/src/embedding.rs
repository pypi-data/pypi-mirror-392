...
impl SkillEmbedder {
    ...
    /// Embed unique skills and return a lookup map for efficient batch processing.
    ///
    /// This method optimizes batch operations by:
    /// 1. Collecting unique skill names
    /// 2. Batch embedding all unique names in a single model call
    /// 3. Classifying domains for each unique skill
    /// 4. Returning a lookup map for O(1) access
    ///
    /// The returned embeddings and domains can be reused across multiple skill instances,
    /// while Beta distributions should be computed per-instance based on proficiency levels.
    ///
    /// # Arguments
    /// * `skills` - Slice of unique skill references to embed
    ///
    /// # Returns
    /// HashMap mapping skill names to (embedding vector, domain classification) tuples
    pub fn embed_unique_skills(
        &mut self,
        skills: &[&Skill],
    ) -> Result<HashMap<String, (Vec<f32>, SkillDomain)>> {
        if skills.is_empty() {
        ...
        let mut texts = Vec::with_capacity(skills.len());
        let mut result = HashMap::with_capacity(skills.len());
        ...
        // Build lookup map: skill name -> (embedding, domain)
        for (skill, embedding) in skills.iter().zip(embeddings) {
            let domain = SkillDomain::classify_skill(&skill.name);
            result.insert(skill.name.clone(), (embedding, domain));
        }
...
impl SkillEmbedderTrait for SkillEmbedder {
    ...
    fn embed_unique_skills(
        &mut self,
        skills: &[&Skill],
    ) -> Result<HashMap<String, (Vec<f32>, SkillDomain)>> {
        self.embed_unique_skills(skills)
    }
}