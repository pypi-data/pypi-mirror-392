...
**Note on parallelization:** We initially tried using Rayon to parallelize the loop, but the trait objects (`Box<dyn SkillEmbedder>`, `Box<dyn SimilarityCalculator>`, etc.) aren't `Sync`, which means they can't be safely shared across threads. Fixing this would require significant refactoring to make all components thread-safe, which is out of scope for now. The sequential loop is fast enough for most use cases.
