...
### Multi-Factor Individual Scoring

For each required skill, the system calculates a score using multiple factors:

1. **Semantic Similarity**: Cosine similarity between embeddings determines skill relatedness
2. **Probabilistic Assessment**: Beta distributions model skill uncertainty and calculate exceedance probability
3. **Direct Ratio**: Weighted average of candidate skill levels vs requirements
4. **Adaptive Combination**: Weights adjust based on confidence and whether requirements are met
5. **Penalty Application**: Low similarity or high scores receive appropriate penalties

```
individual_score = ratio_weight × direct_ratio + prob_weight × exceedance_prob
```
...
1. Rewarding exact matches appropriately
2. Recognizing related expertise through semantic similarity
3. Quantifying uncertainty with probability distributions
4. Combining multiple scoring approaches adaptively
5. Providing explainable, interpretable results
