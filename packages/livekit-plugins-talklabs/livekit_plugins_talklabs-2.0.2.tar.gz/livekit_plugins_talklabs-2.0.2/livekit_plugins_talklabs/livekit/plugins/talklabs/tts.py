...
class TalkLabsTTS(lk_tts.TTS):
    ...
    def __init__(
        ...
        """
        Initialize TalkLabs TTS plugin.

        Args:
            api_key: TalkLabs API key. If not provided, uses TALKLABS_TTS_API_KEY env var.
            base_url: TalkLabs API base URL
            voice: Voice ID (e.g., "pt-br/adam_rocha")
        ...
        self.api_key = api_key or os.environ.get("TALKLABS_TTS_API_KEY")
        if not self.api_key:
            raise ValueError(
                "TalkLabs API key is required. "
                "Provide via api_key parameter or "
                "TALKLABS_TTS_API_KEY environment variable."
            )
