...
class Geocif:
    ...
    def _execute_ml_pipeline(self, dict_selected_features: Dict, dict_best_cei: Dict):
        ...
        for stage in pbar:
            ...
            try:
                ...
                breakpoint()
                self.logger.exception(f"Error in ML loop for stage {stage}: {e}")
    ...
    def loop_ml(
        ...
        for idx, region_id in enumerate(pbar):
            try:
                ...
                self.logger.exception(f"Error processing region {region_id}: {e}")
    ...
    def _clean_training_features(self, X_train: pd.DataFrame) -> pd.DataFrame:
        ...
        print(f"Columns before cleaning: {X_train.columns.tolist()}")
        print(f"Region column has NaNs: {X_train['Region'].isna().any()}")
        ...
        print(f"Columns after cleaning: {X_train.columns.tolist()}")
    ...
    def _run_xai_if_enabled(
        ...
        try:
            ...
            self.logger.exception(f"Error in XAI: {e}")
    ...
    def _store_results(self, experiment_id: str, df: pd.DataFrame):
        ...
        try:
            ...
            self.logger.exception(f"Error storing results for {experiment_id}: {e}")
