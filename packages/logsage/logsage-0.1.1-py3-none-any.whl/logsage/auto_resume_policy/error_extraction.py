...
def extract_numbers(lines: list | None = None) -> list:
    ...
    for line in lines:
        match = pattern.match(line)
        if match:
            numbers.add(int(match.group(1)) // DOMAIN_SIZE)
...
def get_texts_for_first_unique_cluster(texts: list | None = None, cluster_ids: list | None = None) -> list:
    ...
    for text, cluster_id in zip(texts, cluster_ids, strict=False):
...
def create_drain_clusters(
    ...
    for text, pattern in zip(tm_text, tm_text_pattern, strict=False):
    ...
    dict_text_pattern = dict(zip(tm_text_short_original, tm_text_pattern_original, strict=False))
    dict_text_pattern_long = dict(zip(tm_text_original, tm_text_pattern_original, strict=False))
...
def create_clusters(
    ...
    pattern_map = dict(zip(tm_text_original, tm_text_pattern_original, strict=False))
...
def create_clusters_without_drain(
    ...
    dict_text_pattern_long = dict(zip(tm_text, tm_text_pattern, strict=True))

    seen = set()
    new_text_list, new_pattern_list = [], []
    for text, pattern in zip(tm_text, tm_text_pattern, strict=True):
        if pattern not in seen:
            new_text_list.append(text)
            new_pattern_list.append(pattern)
            seen.add(pattern)
...
def return_application_errors(llm: ChatNVIDIA, tm_text: list, cache_dict: LRUCache) -> ApplicationData:
    ...
    if not iteration_flag and len(tm_text_iteration_pattern_verify) > 0:
        ...
        for text, pattern in zip(tm_text_iteration, tm_text_iteration_pattern, strict=True):
            if pattern not in seen:
                new_text_list.append(text)
                new_pattern_list.append(pattern)
                seen.add(pattern)
        ...
        dict_text_pattern = dict(zip(tm_text, tm_text_pattern, strict=True))
        dict_text_pattern_long = dict(zip(tm_text, tm_text_pattern, strict=True))

    # checkpointing_lines = [line for line in tm_text_original if "checkpoint" in line]

    # checkpointing_lines, checkpointing_patterns, _, _ = create_clusters_without_drain(
    #    checkpointing_lines, checkpointing_lines
    # )
    ...
    if len(tm_text_llm) > 0:
        ...
        """
        if len(batch_list_temp) == BATCH_LLM_CALLS:
            batch_list_reverse = batch_list.copy()
            batch_list_reverse.reverse()
            batch_list = prioritize_lines_by_keywords(batch_list_reverse,HIGH_PRIO_WORDS)
            batch_list = batch_list[BATCH_LLM_CALLS:]
        else:
        ...
        for i, answer in enumerate(batch_answer):
            if answer.lower() == "yes":
                ...
                batch_benign_pattern = [line for line in batch_list_pattern[i].split("\n") if line.strip()]
                for benign_pattern in batch_benign_pattern:
        ...
        """
        if len(errors_list_temp) == BATCH_LLM_CALLS:
            errors_list_reverse = errors_list.copy()
            errors_list_reverse.reverse()
            errors_list = prioritize_lines_by_keywords(errors_list_reverse, HIGH_PRIO_WORDS)
            errors_list = errors_list[BATCH_LLM_CALLS:]
        else:
        ...
        error_list_batch = [errors_list[i] for i in range(len(errors_list)) if batch_answer[i].lower() == "yes"]
        benign_list_pattern_cache = [
            dict_text_pattern[errors_list[i]] for i in range(len(errors_list)) if batch_answer[i].lower() == "no"
        ]

        errors_list_pattern_cache = [
            dict_text_pattern[errors_list[i]] for i in range(len(errors_list)) if batch_answer[i].lower() == "yes"
        ]
    ...
    if len(errors_list_unique) == 0:
        logger.info("No errors found in any batch")
        return ApplicationData(
            application_errors_list_full=[],
            application_errors_list_unique=[],
            tm_text_unique=tm_text,
    ...
    for error in unified_errors_list:
        if "slurmstepd" in error[0]:
            if "CANCELLED AT" in error[0] and "DUE TO TIME LIMIT" in error[0]:
                break
            if "CANCELLED AT" in error[0] and "DUE TO NODE FAILURE" not in error[0]:
                continue
    ...
    list_pattern_before_cp = [pattern for i, pattern in enumerate(tm_text_pattern_original) if i < last_cp_idx]

    for error in unified_errors_iteration:
        if last_cp_idx == -1 or (error[2] > last_cp_idx and error[1] not in list_pattern_before_cp):
    ...
    if error_flag:
        ...
        filtered_text = "\n".join(
            ...
            if not ("slurmstepd" in line and "CANCELLED AT" in line and "DUE TO NODE FAILURE" not in line)
        )[-CONTEXT_SIZE:]
