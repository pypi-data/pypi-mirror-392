...
class sysHelper:
    ...
    @staticmethod
    def input(prompt, not_empty=False, number_only=None, default_value=None, choice=None):
        """
        输入控制
            ...
            if number_only is not None:
                ...
                if isinstance(number_only,list) and len(number_only) == 2:
                    ...
                    if (min_num is not None and input_num < min_num) or (max_num is not None and input_num > max_num):
                        print("输入无效,范围[%s ~ %s]，请重新输入!" % (min_num if min_num is not None else 'Min',max_num if max_num is not None else 'Max'))
                        continue
