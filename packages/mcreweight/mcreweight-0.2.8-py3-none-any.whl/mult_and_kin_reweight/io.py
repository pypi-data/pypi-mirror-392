...
def save_data(input_path, tree, output_path, output_tree, branch, weights):
    ...
    with uproot.open(input_path) as f:
        data = f[tree].arrays(library="ak")
    data[branch] = ak.Array(weights)
    branch_dict = {k: data[k] for k in data.fields}
    branch_types = {k: data[k].type for k in data.fields}
    output_dir = Path(output_path).parent
    output_dir.mkdir(parents=True, exist_ok=True)

    with uproot.recreate(output_path) as f:
        #f.mkrntuple(output_tree, data) # WRITE RNTUPLE
        tree_writer = f.mktree(output_tree, branch_types)
        tree_writer.extend(branch_dict)
