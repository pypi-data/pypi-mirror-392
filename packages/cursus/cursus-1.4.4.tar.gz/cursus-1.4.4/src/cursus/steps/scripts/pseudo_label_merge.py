...
def main(
    ...
    use_auto_split_ratios = environ_vars.get("USE_AUTO_SPLIT_RATIOS", "true").lower() == "true"
    
    # Parse split ratios (None if not provided or if auto-inference enabled)
    train_ratio_str = environ_vars.get("TRAIN_RATIO")
    train_ratio = float(train_ratio_str) if train_ratio_str and not use_auto_split_ratios else None
    
    test_val_ratio_str = environ_vars.get("TEST_VAL_RATIO")
    test_val_ratio = float(test_val_ratio_str) if test_val_ratio_str and not use_auto_split_ratios else None
