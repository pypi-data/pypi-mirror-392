...
def _health_check_monitor() -> None:
    ...
    while not _stop_monitoring.is_set():
        # Wait for 400ms, but check stop flag more frequently
        if _stop_monitoring.wait(timeout=0.4):
            break
...
def run_server(target_url: str, port: int) -> None:
    ...
    A background monitor thread will check the server health every 400ms
