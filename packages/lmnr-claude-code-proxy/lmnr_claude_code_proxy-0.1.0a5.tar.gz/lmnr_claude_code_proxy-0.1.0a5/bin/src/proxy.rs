...
struct SpanCapturingStream<S> {
    inner: S,
    accumulated: Arc<AsyncMutex<Vec<Bytes>>>,
    request_body: String,
        ...
        match Pin::new(&mut self.inner).poll_next(cx) {
            Poll::Ready(Some(Ok(bytes))) => {
                ...
                tokio::spawn(async move {
                    accumulated.lock().await.push(bytes_clone);
                });
                Poll::Ready(Some(Ok(bytes)))
            }
            Poll::Ready(None) => {
                // Stream ended - create span
                // Capture end time right after stream ends
                ...
                background_tasks.lock().unwrap().spawn(async move {
                    let chunks = accumulated.lock().await;
                    ...
                    if let Some(trace) = trace {
                        ...
                        eprintln!("No current trace found, not creating span");
                    }
                });
...
async fn forward_request(
    ...
    let capturing_stream = SpanCapturingStream {
        inner: body_stream,
        accumulated: Arc::new(AsyncMutex::new(Vec::new())),
