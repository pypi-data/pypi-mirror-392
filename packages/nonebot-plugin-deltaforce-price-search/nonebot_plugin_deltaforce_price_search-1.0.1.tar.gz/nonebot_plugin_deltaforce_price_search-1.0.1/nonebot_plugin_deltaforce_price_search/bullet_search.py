...
__plugin_meta__ = PluginMetadata(
    name="物品搜索",
    description="搜索DeltaForce游戏中的各种物品价格信息",
    usage="发送'搜索 [分类/关键词]'来搜索相关物品",
    type="application",
    homepage="https://github.com/orzice/DeltaForcePrice",
)

# 创建命令处理器
item_search = on_command("搜索", aliases={"search", "物品搜索", "查价"}, priority=10, block=True)
category_search = on_command("分类", aliases={"category", "物品分类"}, priority=10, block=True)
...
async def handle_item_search(event: MessageEvent, args: Message = CommandArg()):  # 添加event参数
    ...
    if not keyword:
        ...
        await item_search.send("请提供搜索关键词，例如：搜索 M4 或 搜索 头盔 三级")
        return
