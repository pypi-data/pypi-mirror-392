...
class RayvisionTransferRaysync():
    ...
    def start_transfer(self, server_ip, server_port, local_path, server_path, storage_id, input_id=None, task_type=None,
        ...
        if task_id not in self._task_failed_dict:  # first create task
            ...
            params = {
                ...
                "web-server-ip": server_ip,
                "web-server-port": 32002,
    ...
    def look_task_status(self, transfer_task_id, task_id, max_timeout, task_type):
        ...
        while True:
            ...
            self.logger.info('-----transfer task states----- {}'.format(status))
            self.logger.info('-----total file count----- {}'.format(response["task-list"][0]['file-max']))
            self.logger.info('-----success file count----- {}'.format(response["task-list"][0]['file-pos']))
            self.logger.info('-----transfer speed: {}Byte/s-----'.format(response["task-list"][0]['speed']))

            if status in ["idle"]:
                continue
            elif status == "start":
                file_params = {"task-id": transfer_task_id, "list-type": "start", "page-size": 10, "page-number": 1}
                file_response = self.post(self._url.get_file_list, file_params)
                if file_response and file_response.get("file-list"):
                    for file_dic in file_response['file-list']:
                        if int(file_dic['size']) == 0:
                            continue
                        self.logger.info('Transmitting: %s Transfer completed: %s' % (file_dic['local-path'], str(round(file_dic['pos']*100/file_dic['size'], 2)))+'%')
            elif status == "ready":
