...
def migrate_user_data(adapter_id: str, real_qq: str):
    ...
    try:
        ...
        # 如果旧section存在且新section不存在
        if config.has_section(old_section) and not config.has_section(new_section):
        ...
        # 如果新旧section都存在，保留新section，删除旧section
        elif config.has_section(old_section) and config.has_section(new_section):
            config.remove_section(old_section)
            with open(sign_config_path, 'w', encoding='utf-8') as f:
                config.write(f)
            print(f"删除重复的旧section: {old_section}")
