...
def init_paths():
    ...
    # 检查目录权限（使用print而不是logger，因为logger可能还未完全初始化）
    try:
        test_file = data_dir / "test_write.tmp"
        with open(test_file, 'w') as f:
            f.write('test')
        test_file.unlink()
        print(f"数据目录可写: {data_dir}")  # 使用print而不是logger
    except Exception as e:
        print(f"数据目录不可写: {data_dir}, 错误: {e}")  # 使用print而不是logger
...
def save_user_mapping():
    ...
    try:
        ...
        # 保存到数据目录的data.ini文件（使用第一次获取的路径）
        with open(data_config_path, 'w', encoding='utf-8') as f:
            data_config.write(f)

        logger.info(f"成功保存用户映射，共{len(user_id_mapping)}个用户")
        return True
    except Exception as e:
    ...
    try:
        ...
        if data_config_path.exists():
            ...
            if data_config.has_section('UserMapping'):
                ...
                logger.info(f"成功加载用户映射，共{len(user_id_mapping)}个用户")
                return True
            else:
                logger.warning("data.ini文件中没有找到UserMapping节")
        else:
...
def init_plugin():
    ...
    success = load_user_mapping()
    
    if not success:
        logger.warning("加载用户映射失败或没有映射数据")
        user_id_mapping.update({
            '7084F51C2C820B6E97CD40B820A0A166': '2529464880',
        })
        if save_user_mapping():
            logger.info("使用默认用户映射并保存成功")
        else:
            logger.error("保存默认用户映射失败")
    
    logger.info(f"插件初始化完成，当前用户映射数量: {len(user_id_mapping)}")
