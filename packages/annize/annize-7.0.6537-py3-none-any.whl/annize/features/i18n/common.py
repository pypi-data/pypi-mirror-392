...
class _ProjectDefinedTranslationProvider(annize.i18n.TranslationProvider):
    ...
    def translate(self, string_name, *, culture):
        return self.__translations_for_string_name(string_name).get(culture.full_name)
...
class Culture(annize.i18n.Culture):
    ...
    def __init__(self, *, iso_639_1_language_code: str, region_code: str|None,
        ...
        iso_639_1_language_code = iso_639_1_language_code.lower()
        region_code = region_code.upper() if region_code else region_code

        english_lang_name = annize.i18n.culture_by_spec(
            f"{iso_639_1_language_code}{f"_{region_code}" if region_code else ""}").english_lang_name
