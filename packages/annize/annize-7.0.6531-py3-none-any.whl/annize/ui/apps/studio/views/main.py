...
"""
# TODO: split annize into annize and annize-pino (with nearly all features in the latter) ?!
- have a new git repo annize_pidev.git (krrez should clone it)
- give it an annize project config, let it build a package annize_pidev (depending on annize)
- have most features there (not: task, i18n, base(parts?!), ...)
- users (like krrez annize bits): install annize_pidev instead
"""
