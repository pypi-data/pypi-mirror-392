...
class ExpressionDocumentationReport(BaseReport):
    ...
    def _generate_expression_catalog(
        ...
        lines.append("```")
        lines.append(f"📈 {total_expressions} Expressions Documented")
        lines.append(f"⭐ {avg_complexity:.1f} Average Complexity")
        lines.append(f"🔗 {total_dependencies} Total Dependencies")
        lines.append("```")
        lines.append("")
    ...
    def _format_expression_section(self, expr: Dict[str, Any]) -> List[str]:
        ...
        if complexity >= 3:
            lines.append("> **📊 Analysis Note:** This is a complex expression with multiple dependencies. ")
    ...
    def _get_type_emoji(self, op_type: str) -> str:
        ...
        return emoji_map.get(op_type.lower(), '📌')
