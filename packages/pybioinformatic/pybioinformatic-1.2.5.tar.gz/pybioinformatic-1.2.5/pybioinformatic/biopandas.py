...
from pandas import cut, set_option, read_csv, Series, DataFrame, ExcelWriter
...
def read_file_as_df_from_stdin(
    sep: str = '\s+',
    line_terminator: str = '\n',
    comment: str = '#',
    header: int = 0,
    ...
    df = read_csv(
        StringIO(''.join(open_file('-').readlines())),
        sep=sep,
...
def interval_stat(
    ...
    ret = cut(x=ser, bins=bins, precision=precision, include_lowest=True).value_counts(sort=False).to_frame(name)
