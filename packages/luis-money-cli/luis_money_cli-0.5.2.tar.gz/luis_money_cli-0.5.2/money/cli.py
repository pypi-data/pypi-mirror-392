...
from .savings import quantity as savings_quantity
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    # money savings quantity <MonthName>
    sav_qty = savings_sub.add_parser(
        "quantity",
        help="Show 20 percent of the total income for the specified month in the current year",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.command == "savings" and args.savings_cmd == "quantity":
        return savings_quantity.run(args.month)
