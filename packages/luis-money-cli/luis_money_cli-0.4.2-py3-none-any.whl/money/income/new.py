...
def create(month: str, value: str) -> int:
    """Handle `money income new <MonthName> <ValueInEuros>`.

    Saves/updates the income for the specified month in the current year.
    """
    try:
        m = _parse_month(month)
        amount_cents = _parse_euros_to_cents(value)
        year = datetime.now().year
        rec = storage.set_month_income(year, m, amount_cents)

        # Pretty print
        month_name = calendar.month_name[m]
        euros = Decimal(rec["amount_cents"]) / Decimal(100)
        print(
            f"Income set: {year}-{month_name} -> €{euros:.2f} (id={rec['id']}, updated_at={rec['updated_at']})"
        )
        return 0
