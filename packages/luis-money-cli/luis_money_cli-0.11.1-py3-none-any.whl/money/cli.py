...
from money.commands.accounts import create as accounts_new, list as accounts_list
from money.commands.income import create as income_new, list as income_list
from money.commands.income import delete as income_delete
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    # money income list
    income_sub.add_parser("list", help="List all saved incomes")

    # money income delete <MonthName> <Year>
    inc_del = income_sub.add_parser(
        "delete",
        help="Delete the income for the specified month and year",
    )
    inc_del.add_argument(
        "month",
        help="Full month name only (case-insensitive), e.g., 'January', 'november'",
    )
    inc_del.add_argument(
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.command == "accounts" and args.account_cmd == "create":
        return accounts_new.create(args.name)

    if args.command == "accounts" and args.account_cmd == "list":
        return accounts_list.run()

    if args.command == "income" and args.income_cmd == "create":
        return income_new.create(args.month, args.year, args.value)

    if args.command == "income" and args.income_cmd == "list":
        return income_list.run()

    if args.command == "income" and args.income_cmd == "delete":
        return income_delete.run(args.month, args.year)
