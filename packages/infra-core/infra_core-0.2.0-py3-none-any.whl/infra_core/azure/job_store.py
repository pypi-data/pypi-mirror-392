...
from typing import Any, Generic, Mapping, Optional, Tuple, TypeVar, cast
...
class AzureTableRecordProtocol:
    ...
    @classmethod
    def from_entity(
        cls, entity: Mapping[str, Any]
    ) -> "AzureTableRecordProtocol":  # pragma: no cover - Protocol stub
        raise NotImplementedError
...
class TableConfig:
    ...
    def from_env(
        ...
        *,
        ...
        connection = (
            source.get(connection_var) or source.get(fallback_connection_var) or ""
        ).strip()
        if not connection:
        ...
        if not table_name:
            raise AzureTableError(
                f"{table_var} environment variable must be set for Azure job tracking"
            )
...
def _table_client(config: TableConfig) -> Any:
    try:
        ...
        raise AzureTableError(
            "azure-data-tables package is required for Azure job tracking. Install it with `pip install azure-data-tables`."
        ) from exc
...
class AzureTableJobStore(Generic[RecordT]):
    ...
    def fetch(self, batch_id: str, job_id: str) -> Optional[RecordT]:
        try:
    ...
    ) -> Tuple[list[RecordT], Optional[str]]:
        filter_expr = f"PartitionKey eq '{batch_id}'"
        collected: list[RecordT] = []
        next_token = continuation_token

        while True:
            pager = self._client.list_entities(
                filter=filter_expr, results_per_page=limit
