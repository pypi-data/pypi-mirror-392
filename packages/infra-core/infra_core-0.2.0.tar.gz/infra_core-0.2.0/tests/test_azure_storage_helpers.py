...
def test_atomic_replace_uses_move_on_exdev(
    ...
    ) -> None:  # pragma: no cover - raises special case
...
def test_settings_resolve_and_cache_key(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "infra_core.azure.storage.AzureStorageSettings.from_env", fake_from_env
    )
...
def test_async_service_client_account_url(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "infra_core.azure.storage.AsyncDefaultAzureCredential", lambda: FakeCred()
    )
    monkeypatch.setattr(
...
def test_current_prefix_prefers_config() -> None:
    ...
    client = AzureStorageClient(
        config=config, settings=settings, enabled=True, swallow_errors=True
    )
...
def test_log_unconfigured_debug(monkeypatch: pytest.MonkeyPatch) -> None:
    client = AzureStorageClient(
        config=None, settings=AzureStorageSettings(container=None), enabled=True
    )
    ...
    class DummyConfig:
        ...
        def create_async_service_client(self):
            return SimpleNamespace(
                get_container_client=lambda container: DummyContainer()
            ), None
...
def test_download_to_path_handles_not_found(tmp_path: Path) -> None:
    ...
    class BrokenConfig:
        ...
        def create_async_service_client(self):
            return SimpleNamespace(
                get_container_client=lambda container: BrokenContainer()
            ), None
