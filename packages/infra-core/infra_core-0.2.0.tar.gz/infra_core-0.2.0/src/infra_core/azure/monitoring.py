...
TELEMETRY_EVENTS: Final[frozenset[str]] = frozenset(
    event.value for event in TelemetryEvent
)
...
class RunLogWriter:
    ...
    def __init__(
        ...
        storage_root: Path,
        *,
        azure_client: Optional[azure_storage.AzureStorageClient] = None,
    ...
    def _mirror(self) -> None:
        ...
        if (
            hasattr(client, "is_configured") and not client.is_configured()
...
class HeartbeatMonitor:
    ...
    def __init__(
        ...
        *,
        ...
        slug_timeout_seconds: Optional[float] = 300.0,
        ...
        self._slug_timeout = (
            float(slug_timeout_seconds) if slug_timeout_seconds else None
        )
        ...
        self._slug_started_at: Optional[float] = None
        self._last_persist: float = 0.0
        self._timed_out_slug: Optional[str] = None
    ...
    def set_slug(self, slug: Optional[str]) -> None:
        with self._lock:
            self._current_slug = slug
            self._slug_started_at = time.monotonic() if slug else None
            self._timed_out_slug = None
            force_timeout = (
                self._force_first_slug_timeout
                and not self._force_timeout_fired
    ...
    def _trigger_timeout(self, slug: str, elapsed: float) -> None:
        ...
        if self._log_writer:
            self._log_writer.write(
                "heartbeat_timeout", slug=slug, elapsed_seconds=round(elapsed, 2)
            )
        if self._on_timeout:
            try:
                ...
                logger.warning(
                    "Heartbeat timeout callback failed for %s", slug, exc_info=True
                )
...
class TelemetryClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._logger = logging.getLogger(
            logger_name or f"{service_name}.azure.telemetry"
        )
        self._enabled = bool(
    ...
    def track_event(self, name: "TelemetryEvent | str", **properties: Any) -> None:
        ...
        if not self._enabled:
            return
        self._logger.info(
            "telemetry_event %s", json.dumps(payload, ensure_ascii=False, default=str)
        )
    ...
    def track_error(self, name: "TelemetryEvent | str", **properties: Any) -> None:
        ...
        if not self._enabled:
            return
        self._logger.error(
            "telemetry_error %s", json.dumps(payload, ensure_ascii=False, default=str)
        )

    def _build_payload(
        self, event: TelemetryEvent, properties: Dict[str, Any]
    ) -> Dict[str, Any]:
        metadata: Dict[str, Any] = dict(properties)
        service = _coerce_required_str(
            metadata.pop("service", self._service_name), field="service"
        )
        batch_id = _coerce_optional_str(
            metadata.pop("batch_id", None), field="batch_id"
        )
