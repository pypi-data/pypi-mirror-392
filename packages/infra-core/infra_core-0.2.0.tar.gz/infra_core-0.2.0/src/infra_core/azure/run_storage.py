...
def resolve_base_path(
    env_var: str, *, fallback_envs: Iterable[str] | None = None
) -> Path:
...
def build_run_storage_path(
    env_var: str, *segments: str, fallback_envs: Iterable[str] | None = None
) -> Path:
...
def collect_blob_files(
    client: AzureStorageClient, storage_path: Path
    ...
    items: List[dict[str, int | str]] = [
        {"path": relative_path, "size": size} for relative_path, size in entries
    ]
    items.sort(key=lambda item: item["path"])
    return items
