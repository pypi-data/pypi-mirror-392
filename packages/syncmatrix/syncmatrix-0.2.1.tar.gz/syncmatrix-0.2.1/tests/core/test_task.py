...
import syncmatrix
from syncmatrix.core import Flow, Parameter, Task
...
class TestCreateTask:
    ...
    def test_create_task_with_trigger(self):
        t1 = Task()
        assert t1.trigger is syncmatrix.triggers.all_successful

        t2 = Task(trigger=syncmatrix.triggers.all_failed)
        assert t2.trigger == syncmatrix.triggers.all_failed
...
def test_groups():
    ...
    with syncmatrix.context(_group="test"):
        t3 = Task()
...
def test_tags():
    ...
    with syncmatrix.context(_tags=["test"]):
        t4 = Task()
        assert t4.tags == set(["test"])

    with syncmatrix.context(_tags=["test1", "test2"]):
        t5 = Task(tags=["test3"])
