...
import syncmatrix
from syncmatrix.core.flow import Flow
from syncmatrix.core.task import Parameter, Task
from syncmatrix.signals import SyncmatrixError
from syncmatrix.tasks.core.function_task import FunctionTask
...
class TestCreateFlow:
    ...
    def test_create_flow_with_schedule(self):
        f1 = Flow()
        assert isinstance(f1.schedule, syncmatrix.schedules.NoSchedule)

        cron = syncmatrix.schedules.CronSchedule("* * * * *")
...
def test_that_flow_adds_and_removes_itself_from_syncmatrix_context():
    assert "_flow" not in syncmatrix.context
    with Flow() as f1:
        assert syncmatrix.context._flow is f1
        with Flow() as f2:
            assert syncmatrix.context._flow is f2
        assert syncmatrix.context._flow is f1
    assert "_flow" not in syncmatrix.context
