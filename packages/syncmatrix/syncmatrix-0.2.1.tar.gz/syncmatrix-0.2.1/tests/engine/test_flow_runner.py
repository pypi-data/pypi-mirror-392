...
from syncmatrix.engine import FlowRunner
from syncmatrix.engine.state import (
    State,
    Pending,
    Retrying,
    Scheduled,
...
from syncmatrix.utilities.tests import run_flow_runner_test
...
def test_flow_runner_runs_flow_with_2_dependent_tasks_and_first_task_fails_and_second_has_trigger():
    flow = syncmatrix.Flow()
    task1 = ErrorTask()
    task2 = SuccessTask(trigger=syncmatrix.triggers.all_failed)
...
def test_flow_runner_returns_task_states_even_if_it_doesnt_run():
    # https://gitlab.com/syncmatrix/syncmatrix/issues/15
    flow = syncmatrix.Flow()
    task1 = SuccessTask()
    task2 = ErrorTask()
