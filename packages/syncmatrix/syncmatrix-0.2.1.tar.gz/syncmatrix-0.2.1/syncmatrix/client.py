...
import syncmatrix
from syncmatrix.utilities.collections import to_dotdict
...
class Client:
    """
    Client for the Syncmatrix API.
    """

    def __init__(self, api_server=None, graphql_server=None, token=None):
        if api_server is None:
            api_server = syncmatrix.config.get("syncmatrix", "api_server")
            if not api_server:
                api_server = syncmatrix.context.Context.get("api_server", None)
                if not api_server:
                    raise ValueError("Could not determine API server.")
        self._api_server = api_server

        if graphql_server is None:
            graphql_server = syncmatrix.config.get("syncmatrix", "graphql_server")
            if not graphql_server:
                graphql_server = syncmatrix.context.Context.get("graphql_server", None)
                if not graphql_server:
                    graphql_server = api_server
        self._graphql_server = graphql_server

        if token is None:
            token = syncmatrix.context.Context.get("token", None)
        self._token = token
    ...
    def _get(self, path, *, _json=True, _server=None, **params):
        """
        Convenience function for calling the Syncmatrix API with token auth.

        Args:
            path: the path of the API url. For example, to GET
                http://syncmatrix-server/v1/auth/login, path would be 'auth/login'.
    ...
    def graphql(self, query, **variables):
        """
        Convenience function for running queries against the Syncmatrix GraphQL
        API
        """
        result = self._post(
            path="",
...
class Flows(ClientModule):
    ...
    def load(self, flow_id, safe=True):
        ...
        if safe:
            ...
            return syncmatrix.Flow.safe_deserialize(data["flow"]["safe_serialized"])
        else:
            data = self._graphql(
                """
                query($flowId: String!) {
            ...
            return syncmatrix.Flow.deserialize(data["flow"])
...
class FlowRuns(ClientModule):
    ...
    def get_state(self, flow_run_id):
        ...
        return syncmatrix.engine.state.FlowState(
    ...
    #         flow_run_id = syncmatrix.context.Context.get('flow_run_id')
    #     data = self._post(
...
class TaskRuns(ClientModule):
    ...
    def get_state(self, task_run_id):
        ...
        return syncmatrix.engine.state.TaskState(
            state=state.get("state", None), result=state.get("result", None)
        )
