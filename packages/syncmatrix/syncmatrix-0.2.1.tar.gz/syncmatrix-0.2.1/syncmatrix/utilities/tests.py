...
def set_config(keys, value):
    try:
        old_config = copy.copy(syncmatrix.config.__dict__)

        config = syncmatrix.config
        ...
        syncmatrix.config.__dict__.clear()
        syncmatrix.config.__dict__.update(old_config)


@contextmanager
def raise_on_fail():
    with syncmatrix.context(_raise_on_fail=True):
        yield
...
def run_flow_runner_test(
    ...
    Args:
        flow (syncmatrix.Flow): the Flow to test
    ...
    flow_runner = syncmatrix.engine.flow_runner.FlowRunner(flow=flow, executor=executor)
