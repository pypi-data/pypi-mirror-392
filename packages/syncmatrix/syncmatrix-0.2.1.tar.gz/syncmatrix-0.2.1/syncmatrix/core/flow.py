...
from syncmatrix.utilities.functions import cache
from syncmatrix.utilities.json import Serializable
from syncmatrix.utilities.tasks import as_task
...
class Flow(Serializable):
    def __init__(
        ...
        name: str = None,
        version: str = None,
        schedule: syncmatrix.schedules.Schedule = None,
        description: str = None,
        environment: syncmatrix.environments.Environment = None,
        ...
        self.schedule = schedule or syncmatrix.schedules.NoSchedule()
        ...
        self._syncmatrix_version = syncmatrix.__version__
        self._cache = {}
    ...
    def __enter__(self) -> "Flow":
        self.__previous_flow = syncmatrix.context.get("_flow")
        syncmatrix.context.update(_flow=self)
        return self

    def __exit__(self, _type, _value, _tb) -> None:  # type: ignore
        del syncmatrix.context._flow
        if self.__previous_flow is not None:
            syncmatrix.context.update(_flow=self.__previous_flow)
    ...
    def set_dependencies(
        ...
        Args:
            task (Object): a Task that will become part of the Flow. If the task is not a
                Task subclass, Syncmatrix will attempt to convert it to one.

            upstream_tasks ([object]): Tasks that will run before the task runs. If any task
                is not a Task subclass, Syncmatrix will attempt to convert it to one.
            ...
            keyword_tasks ({key: object}): The results of these tasks
                ...
                arguments. If any task is not a Task subclass, Syncmatrix will attempt to
                convert it to one.
        """
    ...
    def run(self, parameters=None, executor=None, **kwargs):
        ...
        Run the flow.
        """
        runner = syncmatrix.engine.flow_runner.FlowRunner(flow=self, executor=executor)
...
def get_hash(obj: object) -> bytes:
    ...
    if isinstance(obj, bytes):
        ...
        obj = syncmatrix.core.task.get_task_info(obj)
    elif isinstance(obj, Edge):
        obj = dict(
            upstream=syncmatrix.core.task.get_task_info(obj.upstream_task),
            downstream=syncmatrix.core.task.get_task_info(obj.downstream_task),
            key=obj.key,
        )
