...
from syncmatrix.environments import Environment
from syncmatrix.utilities.json import Serializable, to_qualified_name

if TYPE_CHECKING:
    from syncmatrix.core.flow import Flow  # pylint: disable=W0611
    ...
    def __new__(cls, name, parents, methods):
        ...
        if run_sig.varargs:
            raise ValueError(
                ...
                "supported, because all Syncmatrix arguments are stored as "
...
class Task(Serializable, metaclass=SignatureValidator):
    def __init__(
        ...
        self.group = str(group or syncmatrix.context.get("_group", ""))

        # avoid silently iterating over a string
        if isinstance(tags, str):
            raise TypeError("Tags should be a set of tags, not a string.")
        self.tags = set(tags or syncmatrix.context.get("_tags", []))
        ...
        self.trigger = trigger or syncmatrix.triggers.all_successful
    ...
    def __call__(
        ...
        flow = syncmatrix.context.get("_flow", None)
        if not flow:
    ...
    def set_dependencies(
        ...
        flow = flow or syncmatrix.context.get("_flow", None)
        if not flow:
            raise ValueError(
...
class Parameter(Task):
    ...
    def run(self) -> Any:
        params = syncmatrix.context.get("_parameters", {})
        if self.required and self.name not in params:
            raise syncmatrix.signals.FAIL(
