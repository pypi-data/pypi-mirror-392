...
import syncmatrix
from syncmatrix import signals
from syncmatrix.core import Task
...
class TaskRunner:
    def __init__(
        ...
        self.task = task
        if executor is None:
            executor = syncmatrix.engine.executors.LocalExecutor()
    ...
    def handle_fail(self, state, fail_mode=None, raise_on_fail=False):
        ...
        if isinstance(fail_mode, signals.TRIGGERFAIL):
            ...
            if raise_on_fail:
                ...
                return state
        run_number = syncmatrix.context.get("_task_run_number", 1)
    ...
    def handle_retry(self, state, retry_time=None):
        ...
        # run_number = syncmatrix.context.get('_task_run_number', 0)
