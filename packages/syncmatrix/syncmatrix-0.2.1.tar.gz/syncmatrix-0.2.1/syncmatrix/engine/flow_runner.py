...
from syncmatrix.engine.state import State, Failed, Pending, Running, Success
...
class FlowRunner:
    def __init__(
        self,
        flow: Flow,
        executor: "syncmatrix.engine.executors.Executor" = None,
        logger_name: str = None,
    ) -> None:
        self.flow = flow
        self.executor = executor or syncmatrix.engine.executors.LocalExecutor()
    ...
    def run(
        ...
        with syncmatrix.context(context):

            with self.executor.start():

                raise_on_fail = syncmatrix.context.get("_raise_on_fail", False)
                ...
                except syncmatrix.signals.DONTRUN as e:
    ...
    def _run(
        ...
        missing = set(required_params).difference(
            syncmatrix.context.get("_parameters", [])
        )
        if missing:
        ...
        if state.is_finished():
            raise syncmatrix.signals.DONTRUN("Flow run has already finished.")

        # this must be pending or running
        elif not (state.is_pending() or state.is_running()):
            raise syncmatrix.signals.DONTRUN("Flow is not ready to run.")
