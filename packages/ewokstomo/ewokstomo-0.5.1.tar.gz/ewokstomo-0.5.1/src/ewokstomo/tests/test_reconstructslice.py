...
def get_data_file(file_name):
    return DATA_ROOT / "RAW_DATA" / RAW_COLLECTION / file_name / f"{file_name}.h5"
...
def test_reconstructslice_workflow_outputs(workflow, tmp_dataset_path):
    ...
    assert rec_path.exists(), "Reconstructed slices directory does not exist"
    assert rec_path.is_file(), "Reconstructed slices path is not a file"
    slices_gallery = rec_path.parents[1] / "gallery"
    ...
    slice_images = sorted(slices_gallery.glob("*.jpg"))
    ...
    projection_images = sorted(projection_gallery.glob("*.jpg"))
    assert (
