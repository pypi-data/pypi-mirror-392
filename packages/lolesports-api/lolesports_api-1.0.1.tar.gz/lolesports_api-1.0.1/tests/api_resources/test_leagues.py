...
from lolesports_api.types import LeagueListResponse
...
class TestLeagues:
    ...
    def test_method_list(self, client: LolesportsAPI) -> None:
        league = client.leagues.list(
            query={"id": 0},
        )
        assert_matches_type(LeagueListResponse, league, path=["response"])

    @pytest.mark.skip(reason="Prism tests are disabled")
    @parametrize
    def test_raw_response_list(self, client: LolesportsAPI) -> None:
        response = client.leagues.with_raw_response.list(
            query={"id": 0},
        )
    ...
    def test_streaming_response_list(self, client: LolesportsAPI) -> None:
        with client.leagues.with_streaming_response.list(
            query={"id": 0},
        ) as response:
...
class TestAsyncLeagues:
    ...
    async def test_method_list(self, async_client: AsyncLolesportsAPI) -> None:
        league = await async_client.leagues.list(
            query={"id": 0},
        )
        assert_matches_type(LeagueListResponse, league, path=["response"])
    ...
    async def test_raw_response_list(self, async_client: AsyncLolesportsAPI) -> None:
        response = await async_client.leagues.with_raw_response.list(
            query={"id": 0},
        )
    ...
    async def test_streaming_response_list(self, async_client: AsyncLolesportsAPI) -> None:
        async with async_client.leagues.with_streaming_response.list(
            query={"id": 0},
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
