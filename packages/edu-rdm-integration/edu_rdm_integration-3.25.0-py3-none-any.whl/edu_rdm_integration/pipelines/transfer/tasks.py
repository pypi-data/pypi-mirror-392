...
# Дельта в минутах с последнего удачного запуска
RDM_TRANSFER_INTERVAL_DELTA = int(getattr(settings, 'RDM_TRANSFER_INTERVAL_DELTA', None) or 5)
...
def split_interval(
    ...
    if delta is None:
        delta = RDM_TRANSFER_INTERVAL_DELTA
...
class BaseTransferLatestEntitiesDataPeriodicTask(BaseTransferLatestEntitiesDataMixin, UniquePeriodicAsyncTask):
    ...
    def _run_collect_model_data(self, model: str, task_id: str) -> None:
        ...
        for begin, end in split_interval(manager_last_collected, timezone.now()):
    ...
    def _run_export_entity_data(self, entity: str, task_id: str) -> None:
        ...
        for begin, end in split_interval(manager_last_exported, timezone.now()):
...
class TransferLatestEntitiesDataPeriodicTask(
    SelfReschedulingAsyncTask, BaseTransferLatestEntitiesDataPeriodicTask
):
...
class TransferLatestEntitiesDataFastPeriodicTask(
    SelfReschedulingAsyncTask, BaseTransferLatestEntitiesDataPeriodicTask
):
...
class TransferLatestEntitiesDataLongPeriodicTask(
    SelfReschedulingAsyncTask, BaseTransferLatestEntitiesDataPeriodicTask
):
