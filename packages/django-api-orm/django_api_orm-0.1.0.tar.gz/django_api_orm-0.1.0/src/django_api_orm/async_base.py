...
class AsyncQuerySet(Generic[T]):
    ...
    All methods that fetch data are async.
    ...
    def __init__(self, model_class: type[T], manager: "AsyncManager[T]") -> None:
    ...
    def _clone(self) -> "AsyncQuerySet[T]":
        ...
        Returns:
            A new AsyncQuerySet with the same parameters
        """
        qs = AsyncQuerySet(self.model_class, self.manager)
    ...
    def __getitem__(self, key: int | slice) -> "AsyncQuerySet[T]":
        """Support slicing (indexing not supported for async).

        Args:
            key: Slice object (int indexing not supported in async context)
        ...
        Note:
            Integer indexing like `queryset[0]` is not supported for async querysets.
            Use `await queryset.first()` instead.
        """
        if isinstance(key, int):
            raise TypeError(
                "Integer indexing is not supported for async querysets. "
                "Use 'await queryset.first()' or 'await queryset.last()' instead."
            )
            ...
            raise TypeError("AsyncQuerySet indices must be slices")
    ...
    async def _fetch(self) -> None:
        ...
        response = await self.manager.client.get(self.manager.get_endpoint(), params=params)
    ...
    # Filtering methods (sync - return new QuerySet)
    ...
    def __repr__(self) -> str:
        ...
        if self._result_cache is not None:
            return f"<AsyncQuerySet {list(self._result_cache)}>"
        return f"<AsyncQuerySet for {self.model_class.__name__}>"


class AsyncManager(Generic[T]):
    """Django-like AsyncManager for model querying.
    ...
    Example:
        ...
        >>> policies = await Policy.objects.filter(status='active').alen()
    """

    def __init__(self, model_class: type[T], client: AsyncServiceClient) -> None:
    ...
    def all(self) -> AsyncQuerySet[T]:
        ...
        Example:
            ...
            >>> async for policy in all_policies:
            ...     print(policy)
        """
        return AsyncQuerySet(self.model_class, self)
    ...
    async def get(self, **kwargs: Any) -> T:
        ...
        Example:
            >>> policy = await Policy.objects.get(id=123)
        """
        return await self.all().get(**kwargs)
    ...
    async def values(self, *fields: str) -> list[dict[str, Any]]:
        ...
        Example:
            >>> policies = await Policy.objects.values('id', 'policy_number')
        """
        return await self.all().values(*fields)
    ...
    async def values_list(self, *fields: str, flat: bool = False) -> list[Any]:
        ...
        Example:
            >>> ids = await Policy.objects.values_list('id', flat=True)
        """
        return await self.all().values_list(*fields, flat=flat)
    ...
    async def create(self, **kwargs: Any) -> T:
        ...
        Example:
            >>> policy = await Policy.objects.create(
        ...
        # Make API request
        response = await self.client.post(self.get_endpoint(), data=data)
    ...
    async def get_or_create(
        ...
        Example:
            >>> policy, created = await Policy.objects.get_or_create(
            ...     policy_number='POL-001',
    ...
    async def update_or_create(
        ...
        Example:
            >>> policy, created = await Policy.objects.update_or_create(
            ...     policy_number='POL-001',
    ...
    async def bulk_create(self, objs: list[dict[str, Any]]) -> list[T]:
        ...
        Example:
            >>> policies = await Policy.objects.bulk_create([
        ...
        for obj_data in objs:
            obj = await self.create(**obj_data)
...
class AsyncAPIModel:
    ...
    Provides Django ORM-like interface for working with API resources asynchronously.
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        if "_pydantic_instance" in kwargs:
            ...
            self._client: AsyncServiceClient | None = kwargs.get("_client")
        else:
    ...
    @classmethod
    def from_api(
        cls, data: dict[str, Any], client: AsyncServiceClient | None = None
    ) -> "AsyncAPIModel":
    ...
    async def save(self, update_fields: list[str] | None = None) -> None:
        ...
        if hasattr(self, "id") and self.id:
            ...
            response = await self._client.patch(endpoint, data=data)
        else:
            # Create new
            response = await self._client.post(self.get_endpoint(), data=data)
...
def register_async_models(client: AsyncServiceClient, *model_classes: type[AsyncAPIModel]) -> None:
    """Register model classes with an async client.

    This assigns an AsyncManager instance to each model's 'objects' attribute.

    Args:
        client: AsyncServiceClient instance
    ...
    for model_class in model_classes:
        model_class.objects = AsyncManager(model_class, client)
