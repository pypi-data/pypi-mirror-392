...
class Logger:
    ...
    def log_writer(shdata: SharedData):
        ...
        while True:
            ...
            Logger._log_writer_file(rec, loghash)
    ...
    def _log_writer_extend(shdata, rec, asctime, loghash):
        ...
        try:
            logcoll = shdata.table('Hashs','RT','WORKERPOOL','LOGS/'+asctime.strftime('%Y%m%d'), 
                names = ['hash','mtime'],
                formats = ['|S16','<M8[ns]'],
    ...
    def _log_writer_upsert(shdata, rec, asctime, loghash):
        ...
        try:
            ...
            # insert into specific state table if found
            is_state_msg = False
            ...
            # insert into specific level table if not a state message
            if not is_state_msg:
                hbcoll = shdata.table(
                    'Text','D1','WORKERPOOL',str.upper(rec['level']),
                    names = ['date','hash','mtime'],
                    formats = ['<M8[ns]','|S64','<M8[ns]'],
                    size=1e6,
                    is_schemaless=True
