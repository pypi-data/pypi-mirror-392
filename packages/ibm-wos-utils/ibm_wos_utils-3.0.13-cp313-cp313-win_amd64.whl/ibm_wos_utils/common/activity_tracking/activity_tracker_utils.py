...
class ActivityTrackerUtils:
    ...
    @classmethod
    async def log_event_async(cls, at_event_info: ActivityTrackingInfo, outcome: str, user: User, service_instance_id, status_code: int, request_path: str, request_method: str, request_user_agent: str, client_session: ClientSession | None = None) -> None:
        """
        Logs event for Activity Tracker in the file mount.
        :at_event_info: The AT event information object.
        :outcome: The outcome of the action.
        ...
        :request_method: Value of request.method.
        :request_user_agent: Value of the context-header user-agent.
        :client_session": optional client session to be used for aiohttp
        ...
        try:
            ...
            if Env.is_cpd():
                ...
                await audit_logger_client.log_event_async(payload)
            else:
                # Getting the file path
                file_path = cls.get_file_path()
