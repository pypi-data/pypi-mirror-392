...
class RedisAsyncClient:
    ...
    async def set(self, key: str, value: str, ex: int = None, px: int = None, nx: bool = False, xx: bool = False, keepttl: bool = False, retry_count: int = 0) -> bool:
        ...
        try:
            if self.enable_locking_keys:
                ...
                if await self.get(key=lock_key) is not None:
                    ...
                    self.logger.log_info(
                        f"Lock is present on the key '{key}', hence it cannot be modified.")
                    return key_stored
            ...
            self.logger.log_debug(f"Setting key '{key}' to Redis.")
            ...
            if key_stored is None and nx:
                key_stored = False
                self.logger.log_info(
                    f"Could not store key '{key}' as it was already there as 'nx' was set to true.", start_time=start_time)
            else:
                self.logger.log_info(
                    f"Successfully set key '{key}' to Redis.", start_time=start_time)
        except redis.ConnectionError as ce:
            if retry_count >= self.redis_connection_max_retry_count:
                self.logger.log_error(
                    f"Failed to get a connection from the pool to Redis for operation 'SET' on key '{key}'. Error: '{str(ce)}'.")
                raise
            else:
                ...
                self.logger.log_warning(
                    f"Connection to redis failed due to error for operation 'SET' on key '{key}': '{str(ce)}' will retry {retry_count + 1} time after sleeping for {time_to_sleep} seconds.")
                await asyncio.sleep(time_to_sleep)
                key_stored = await self.set(key, value, ex, px, nx, xx, keepttl, retry_count+1)
        except Exception as ex:
            self.logger.log_error(
                f"Failed to set key '{key}' in Redis. Error: '{str(ex)}'.")
            raise
    ...
    async def get(self, key: str, retry_count: int = 0) -> bytes:
        ...
        try:
            ...
            self.logger.log_debug(f"Getting key '{key}' from Redis.")
            start_time = DateUtil.current_milli_time()
            val = await rconn.get(key)
            self.logger.log_info(
                f"Successfully retrieved key '{key}' from Redis.", start_time=start_time)
        except redis.ConnectionError as ce:
            if retry_count >= self.redis_connection_max_retry_count:
                self.logger.log_error(
                    f"Failed to get a connection from the pool to Redis for operation 'GET' on key '{key}'. Error: '{str(ce)}'.")
                raise
            else:
                # Doing binary exponential backoff for retrying with a random buffer time so that not all threads retry at the same time
                time_to_sleep = 2 ** (retry_count + 1) + random.randint(0, 10)
                self.logger.log_warning(
                    f"Connection to redis failed due to error for operation 'GET' on key '{key}': '{str(ce)}' will retry {retry_count + 1} time after sleeping for {time_to_sleep} seconds.")
                await asyncio.sleep(time_to_sleep)
                val = await self.get(key, retry_count+1)
        except Exception as ex:
            self.logger.log_error(
                f"Failed to get key '{key}' from Redis. Error: '{str(ex)}'.")
            raise
    ...
    async def hget(self, name: str, key: str, retry_count: int = 0) -> str:
        ...
        try:
            ...
            # Getting the value from Redis
            self.logger.log_info(
                f"Getting key '{key}' from sorted set '{name}' in Redis.")
            start_time = DateUtil.current_milli_time()
            val = await rconn.hget(name, key)
            self.logger.log_info(
                f"Successfully retrieved key '{key}' from sorted set '{name}' in Redis.", start_time=start_time)
        except redis.ConnectionError as ce:
            if retry_count >= self.redis_connection_max_retry_count:
                self.logger.log_error(
                    f"Failed to get a connection from the pool to Redis for operation 'HGET' on key '{key}' from sorted set '{name}'. Error: '{str(ce)}'.")
                raise
            else:
                # Doing binary exponential backoff for retrying with a random buffer time so that not all threads retry at the same time
                time_to_sleep = 2 ** (retry_count + 1) + random.randint(0, 10)
                self.logger.log_warning(
                    f"Connection to redis failed due to error for operation 'HGET' on key '{key}' from sorted set '{name}': '{str(ce)}' will retry {retry_count + 1} time after sleeping for {time_to_sleep} seconds.")
                await asyncio.sleep(time_to_sleep)
                val = await rconn.hget(name, key)
        except Exception as ex:
            self.logger.log_error(
                f"Failed to get key '{key}' from sorted set '{name}' in Redis. Error: '{str(ex)}'.")
            raise
    ...
    async def zrange(
        ...
        try:
            ...
            self.logger.log_debug(
                f"Getting range from sorted set '{name}' from Redis.")
            start_time = DateUtil.current_milli_time()
            ...
            self.logger.log_info(
                f"Successfully retrieved range from sorted set '{name}' from Redis.", start_time=start_time)
        except redis.ConnectionError as ce:
            if retry_count >= self.redis_connection_max_retry_count:
                self.logger.log_error(
                    f"Failed to get a connection from the pool to Redis for operation 'ZRANGE' on sorted set '{name}'. Error: '{str(ce)}'.")
                raise
            else:
                # Doing binary exponential backoff for retrying with a random buffer time so that not all threads retry at the same time
                time_to_sleep = 2 ** (retry_count + 1) + random.randint(0, 10)
                self.logger.log_warning(
                    f"Connection to redis failed due to error for operation 'ZRANGE' on sorted set '{name}': '{str(ce)}' will retry {retry_count + 1} time after sleeping for {time_to_sleep} seconds.")
                await asyncio.sleep(time_to_sleep)
            ...
            self.logger.log_error(
                f"Failed to get range from sorted set '{name}' from Redis. Error: '{str(ex)}'.")
            raise
    ...
    async def zscore(self, name: str, value: any, retry_count: int = 0) -> any:
        ...
        try:
            ...
            # Getting the value from Redis
            self.logger.log_debug(
                f"Getting score for value '{value}' in sorted set '{name}' from Redis.")
            start_time = DateUtil.current_milli_time()
            score = await rconn.zscore(name, value)
            self.logger.log_info(
                f"Successfully retrieved score for value '{value}' from sorted set '{name}' from Redis.", start_time=start_time)
        except redis.ConnectionError as ce:
            if retry_count >= self.redis_connection_max_retry_count:
                self.logger.log_error(
                    f"Failed to get a connection from the pool to Redis for operation 'ZSCORE' on sorted set '{name}'. Error: '{str(ce)}'.")
                raise
            else:
                # Doing binary exponential backoff for retrying with a random buffer time so that not all threads retry at the same time
                time_to_sleep = 2 ** (retry_count + 1) + random.randint(0, 10)
                self.logger.log_warning(
                    f"Connection to redis failed due to error for operation 'ZSCORE' for value '{value}' in sorted set '{name}': '{str(ce)}' will retry {retry_count + 1} time after sleeping for {time_to_sleep} seconds.")
                await asyncio.sleep(time_to_sleep)
                score = await self.zscore(name, value, retry_count+1)
        except Exception as ex:
            self.logger.log_error(
                f"Failed to get score for value {value} in sorted set '{name}' from Redis. Error: '{str(ex)}'.")
            raise
        ...
        try:
            if self.enable_locking_keys:
                ...
                if await self.get(key=lock_key) is not None:
                    ...
                    self.logger.log_info(
                        f"Lock is present on the key '{key}', hence it cannot be deleted.")
                    return key_deleted
            ...
            self.logger.log_debug(f"Deleting key '{key}' from Redis.")
            start_time = DateUtil.current_milli_time()
            await rconn.delete(key)
            key_deleted = True
            self.logger.log_info(
                f"Successfully deleted key '{key}' from Redis.", start_time=start_time)
        except redis.ConnectionError as ce:
            if retry_count >= self.redis_connection_max_retry_count:
                self.logger.log_error(
                    f"Failed to get a connection from the pool to Redis for operation 'DELETE' on key '{key}'. Error: '{str(ce)}'.")
                raise
            else:
                # Doing binary exponential backoff for retrying with a random buffer time so that not all threads retry at the same time
                time_to_sleep = 2 ** (retry_count + 1) + random.randint(0, 10)
                self.logger.log_warning(
                    f"Connection to redis failed due to error for operation 'DELETE' on key '{key}': '{str(ce)}' will retry {retry_count + 1} time after sleeping for {time_to_sleep} seconds.")
                await asyncio.sleep(time_to_sleep)
                key_deleted = await self.delete(key, retry_count+1)
        except Exception as ex:
            self.logger.log_error(
                f"Failed to delete key '{key}' from Redis. Error: '{str(ex)}'.")
            raise
    ...
    async def zrem(self, name: str, *values, retry_count: int = 0) -> int:
        ...
        try:
            ...
            # Deleting the value from Redis
            self.logger.log_debug(
                f"Deleting values '{values}' from sorted set '{name}' in Redis.")
            start_time = DateUtil.current_milli_time()
            num_deleted = await rconn.zrem(name, *values)
            self.logger.log_info(
                f"Successfully deleted values '{values}' with num_deleted '{num_deleted}' from sorted set '{name}' in Redis.", start_time=start_time)
        except redis.ConnectionError as ce:
            if retry_count >= self.redis_connection_max_retry_count:
                self.logger.log_error(
                    f"Failed to get a connection from the pool to Redis for operation 'ZREM' on sorted set '{name}'. Error: '{str(ce)}'.")
                raise
            else:
                # Doing binary exponential backoff for retrying with a random buffer time so that not all threads retry at the same time
                time_to_sleep = 2 ** (retry_count + 1) + random.randint(0, 10)
                self.logger.log_warning(
                    f"Connection to redis failed due to error for operation 'ZREM' on sorted set '{name}': '{str(ce)}' will retry {retry_count + 1} time after sleeping for {time_to_sleep} seconds.")
                await asyncio.sleep(time_to_sleep)
                num_deleted = await self.zrem(name, *values, retry_count+1)
        except Exception as ex:
            self.logger.log_error(
                f"Failed to delete values '{values}' from sorted set '{name}' in Redis. Error: '{str(ex)}'.")
            raise
    ...
    async def ttl(self, key: str, retry_count: int = 0) -> int:
        ...
        try:
            ...
            self.logger.log_debug(f"Getting TTL for key '{key}' from Redis.")
            start_time = DateUtil.current_milli_time()
            ttl = await rconn.ttl(key)
            self.logger.log_info(
                f"Successfully retrieved the TTL for key '{key}' from Redis.", start_time=start_time)
        except redis.ConnectionError as ce:
            if retry_count >= self.redis_connection_max_retry_count:
                self.logger.log_error(
                    f"Failed to get a connection from the pool to Redis for operation 'TTL' on key '{key}'. Error: '{str(ce)}'.")
                raise
            else:
                # Doing binary exponential backoff for retrying with a random buffer time so that not all threads retry at the same time
                time_to_sleep = 2 ** (retry_count + 1) + random.randint(0, 10)
                self.logger.log_warning(
                    f"Connection to redis failed due to error for operation 'TTL' on key '{key}': '{str(ce)}' will retry {retry_count + 1} time after sleeping for {time_to_sleep} seconds.")
                await asyncio.sleep(time_to_sleep)
                ttl = await self.ttl(key, retry_count+1)
        except Exception as ex:
            self.logger.log_error(
                f"Failed to get TTL for key '{key}' from Redis. Error: '{str(ex)}'.")
            raise
    ...
    async def keys(self, pattern: str) -> list:
        ...
        try:
            ...
            self.logger.log_info(
                f"Successfully retrieved the keys with pattern '{pattern}' from Redis.", start_time=start_time)
        except Exception as ex:
            self.logger.log_error(
                f"Failed to get keys with pattern '{pattern}' from Redis. Error: '{str(ex)}'.")
            raise
    ...
    async def register_script(self, script: str) -> callable:
        ...
        try:
            ...
            self.logger.log_info(
                "Successfully registered script in Redis.", start_time=start_time)
        except Exception as ex:
            self.logger.log_error(
                f"Failed to register script in Redis. Error: '{str(ex)}'.")
            raise
    ...
    async def lock_key(self, key: str, expire_time: int = None) -> bool:
        ...
        try:
            if not self.enable_locking_keys:
                self.logger.log_warning(
                    f"Not locking key '{key}' as locking is disabled via flag 'enable_locking_keys' being set to false.")
                return lock_ack
            ...
            if not lock_ack:
                # Lock already exists
                self.logger.log_info(
                    f"Lock already exists for key '{key}, hence it cannot be acquired.'")
        except Exception as ex:
            self.logger.log_error(
                f"Failed to get lock on key '{key}' from Redis. Error: '{str(ex)}'.")
            raise
    ...
    async def unlock_key(self, key: str) -> bool:
        ...
        try:
            if not self.enable_locking_keys:
                self.logger.log_warning(
                    f"Not unlocking key '{key}' as locking is disabled via flag 'enable_locking_keys' being set to false.")
                return
            ...
            self.logger.log_error(
                f"Failed to release lock on key '{key}' from Redis. Error: '{str(ex)}'.")
            raise
    ...
    async def block_lock(self, lock_name: str, lock_ttl: int = 600, wait_interval: int = 3) -> bool:
        ...
        try:
            ...
            if lock_ack:
                # Lock acquired
                self.logger.log_info(
                    f"Lock with name '{lock_name}' successfully acquired for at max '{lock_ttl}' seconds.")
            else:
                # Lock could not be acquired as it was already present
                lock_ttl = await self.ttl(key=lock_name)
                self.logger.log_info(
                    f"Lock with name '{lock_name}' already exists, hence cannot be acquired, waiting for the lock to be released till max '{lock_ttl}' seconds (TTL of the lock).")
                total_wait = 0
                ...
                if not lock_ack:
                    ...
                    self.logger.log_error(
                        f"Lock '{lock_name}' could not be acquired even after waiting for '{total_wait} seconds.'")
        except Exception as ex:
            self.logger.log_error(
                f"Failed to block lock with name '{lock_name}' in Redis. Error: '{str(ex)}'.")
            raise
    ...
    async def release_lock(self, lock_name: str) -> bool:
        ...
        try:
            ...
            self.logger.log_error(
                f"Failed to release lock '{lock_name}'. Error: '{str(ex)}'.")
            raise
    ...
    async def aclose(self) -> None:
        ...
        try:
            ...
            self.logger.log_error(
                f"Failed to close the connection pool with error: {str(ex)}")
            raise
    ...
    async def _get_connection(self, retry_count: int = 0, key: str = None, operation: str = None) -> redis.Redis:
        ...
        self.logger.log_debug(
            f"Getting a connection from the pool to Redis for operation '{operation}' on key '{key}'.")
        start_time = DateUtil.current_milli_time()
        try:
            rconn = redis.StrictRedis(connection_pool=self.conn_pool, socket_keepalive=True,
                                      health_check_interval=self.health_check_interval, retry_on_timeout=True)
            self.logger.log_debug(
                f"Successfully got a connection from the pool to Redis for operation '{operation}' on key '{key}'.", start_time=start_time)
        except redis.ConnectionError as ce:
            if retry_count >= self.redis_connection_max_retry_count:
                self.logger.log_error(
                    f"Failed to get a connection from the pool to Redis for operation '{operation}' on key '{key}'. Error: '{str(ce)}'.")
                raise
            else:
                # Doing binary exponential backoff for retrying with a random buffer time so that not all threads retry at the same time
                time_to_sleep = 2 ** (retry_count + 1) + random.randint(0, 10)
                self.logger.log_warning(
                    f"Connection to redis failed due to error for operation '{operation}' on key '{key}': '{str(ce)}' will retry {retry_count + 1} time after sleeping for {time_to_sleep} seconds.")
                await asyncio.sleep(time_to_sleep)
                rconn = self._get_connection(
                    retry_count=retry_count + 1, key=key, operation=operation)
        except Exception as ex:
            self.logger.log_error(
                f"Failed to get a connection from the pool to Redis for operation '{operation}' on key '{key}'. Error: '{str(ex)}'.")
            raise
