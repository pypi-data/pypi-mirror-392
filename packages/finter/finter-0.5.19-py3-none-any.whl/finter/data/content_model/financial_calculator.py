...
class FinancialCalculator:
    ...
    def __init__(self, df: pd.DataFrame | pl.DataFrame, trading_days=None):
        """
    ...
    def __getattr__(self, name: str):
        ...
        This allows using polars methods directly on FC instances:
            fc.join({...}).filter(pl.col("id") == 12170).sort("pit")
        """
        return getattr(self._df_long, name)
    ...
    def apply_rolling(
        ...
        var_cols = [col for col in self._df_long.columns if col not in ["id", "pit", "fiscal"]]

        # If no variables specified and we have a 'value' column, use default behavior
        if variables is None:
            if "value" in var_cols:
                result_long = self._apply_rolling_impl(self._df_long, quarters, operation)
                return FinancialCalculator(result_long, trading_days=self._trading_days)
            else:
        ...
        for var in var_cols:
            if var in variables:
                ...
                df_var = self._df_long.select(["id", "pit", "fiscal", var]).rename({var: "value"})
    ...
    def join(
        ...
        # Use trading_days from first FC that has it
        trading_days = None
        for fc in dfs.values():
            if fc._trading_days is not None:
                trading_days = fc._trading_days
                break

        return FinancialCalculator(result_long, trading_days=trading_days)
    ...
    def _join_long_dfs_impl(
        ...
        for name, df_long in dfs.items():
            df_clean = (
                df_long.filter(pl.col("fiscal").is_not_null())
                .unique(subset=["id", "pit", "fiscal"], keep="last")
            )
            ...
            var_result = (
                ...
                .agg([
                    pl.col("value").sort_by("pit_right", descending=True).first().alias(name)
                ])
            )
    ...
    def _apply_expression_impl(df_long: pl.DataFrame, expression: str) -> pl.DataFrame:
        ...
        # Sort by length descending to avoid partial replacements (e.g., "current_assets" before "current")
        pl_expr = expression
        for var_col in sorted(var_cols, key=len, reverse=True):
            pl_expr = pl_expr.replace(var_col, f'pl.col("{var_col}")')
    ...
    def _to_wide_impl(df_long: pl.DataFrame) -> pd.DataFrame:
        ...
        var_cols = [col for col in df_long.columns if col not in ["id", "pit", "fiscal"]]
        ...
        for var in var_cols:
            ...
            df_var = df_long.select(["id", "pit", "fiscal", var] if "fiscal" in df_long.columns else ["id", "pit", var])
