...
def _discover_windows_shortcuts(limit: int | None = None) -> List[AppInfo]:
    ...
    for base in WINDOWS_SHORTCUT_DIRS:
        ...
        for shortcut in base.rglob("*.lnk"):
            ...
            # 解析快捷方式获取实际路径（避免通过 .lnk 启动导致冲突）
            target_path = resolve_windows_shortcut(shortcut)
            process_name = _process_name_from_shortcut(shortcut)

            # 优先使用实际路径，如果解析失败则使用快捷方式路径
            actual_path = target_path if target_path else str(shortcut)

            apps.append(
                AppInfo(
                    name=name,
                    path=actual_path,  # ← 使用实际路径而不是 .lnk
            ...
            for shortcut in base_dir.rglob("*.lnk"):
                try:
                    ...
                    if query_lower in name.lower():
                        ...
                        # 优先使用实际路径，如果解析失败则使用快捷方式路径
                        actual_path = target_path if target_path else str(shortcut)
                        logger.info(f"  快捷方式目标: {actual_path}")
