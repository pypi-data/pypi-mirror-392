...
class AppLauncherService:
    ...
    async def open_app(self, query: str) -> Dict[str, Any]:
        ...
        logger.info(f"=" * 80)
        logger.info(f"收到打开应用请求: '{query}'")
        logger.info(f"=" * 80)
        
        # 步骤1: 尝试从注册表查找
        logger.info(f"步骤1: 从注册表查找应用")
        app = self.registry.find(query)

        # 步骤2: 如果注册表找不到，使用动态搜索
        if not app:
            logger.info(f"注册表未找到应用 '{query}'，启动动态搜索")
            logger.info(f"步骤2: 动态搜索应用")
            app = search_app_dynamically(query)

            if not app:
                logger.error(f"动态搜索也未找到应用: {query}")
                raise ValueError(
                    f"未找到匹配的应用: {query}\n"
                    f"已搜索范围:\n"
                    f"  - 配置文件中的应用\n"
            ...
            logger.info(f"✓ 动态搜索找到应用: {app.name}")
            logger.info(f"  路径: {app.path}")
            logger.info(f"  关键词: {app.keywords}")
            logger.info(f"  进程名: {app.process_name}")
        else:
            logger.info(f"✓ 从注册表找到应用: {app.name}")
            logger.info(f"  路径: {app.path}")
            logger.info(f"  关键词: {app.keywords}")
            logger.info(f"  进程名: {app.process_name}")

        # 步骤3: 使用激活器打开应用
        logger.info(f"步骤3: 调用激活器打开应用")
        loop = asyncio.get_running_loop()
        result = await loop.run_in_executor(None, activate_app, app)
        
        logger.info(f"激活器返回结果: 成功={result.get('success')}, 消息={result.get('message')}")
