...
class WindowsActivator:
    ...
    def activate(self, app: AppInfo) -> Dict[str, Any]:
        """主激活逻辑 - 三层回退策略"""
        self.steps.clear()
        LOGGER.info(f"========== 开始激活应用: {app.name} ==========")
        LOGGER.info(f"应用路径: {app.path}")
        LOGGER.info(f"应用进程名配置: {app.process_name}")
        LOGGER.info(f"AutoHotkey 可用: {self.ahk_path is not None}")
    ...
    def _activate_existing_window(
        ...
        """激活已存在的窗口"""
        LOGGER.info(f"========== 激活已存在的窗口 ==========")
        LOGGER.info(f"应用: {app.name}, hwnd: {hwnd}, 进程名: {process_name}")
        ...
        try:
            ...
            try:
                ...
                LOGGER.info(f"窗口位置: left={rect[0]}, top={rect[1]}, right={rect[2]}, bottom={rect[3]}")
            except Exception as e:
            ...
            try:
                ...
                LOGGER.warning(f"获取窗口类名失败: {e}")
            
            LOGGER.info(f"窗口状态: 标题='{title}', 可见={is_visible}, 最小化={is_iconic}")
        ...
        if not is_visible and not is_iconic:
            tray_hint = True
            LOGGER.warning(f"窗口不可见且未最小化，可能在系统托盘中")
        ...
        LOGGER.info("========== 策略 1: 尝试 Win32 API 温和激活 ==========")
        try:
            if self._gentle_activate(hwnd):
                ...
                try:
                    ...
                    LOGGER.info(f"激活后状态: 窗口可见={final_visible}, 前台窗口={final_fg}, 目标窗口={hwnd}, 是否前台={'是' if final_fg == hwnd else '否'}")
                except Exception as e:
                    LOGGER.warning(f"验证激活结果失败: {e}")
                
                return self._result(True, f"成功激活 {app.name}")
            else:
                LOGGER.warning("✗ Win32 API 激活失败")
        except Exception as e:
        ...
        if tray_hint and self.ahk_path:
            LOGGER.info("========== 策略 2: 尝试 AutoHotkey 激活 ==========")
            ...
            try:
                ...
                if result['success']:
                    ...
                    LOGGER.warning(f"✗ AutoHotkey 激活失败: {result.get('message')}")
        ...
        if allow_shell_retry:
            LOGGER.info("========== 策略 3: 尝试 Shell Start ==========")
...
'''
    ...
    def _gentle_activate(self, hwnd: int) -> bool:
        """温和激活窗口（Win32 API）"""
        try:
            LOGGER.info(f"[温和激活] 开始激活窗口 hwnd={hwnd}")
            ...
            try:
                ...
                is_iconic = win32gui.IsIconic(hwnd)
                title = win32gui.GetWindowText(hwnd)
                LOGGER.info(f"[温和激活] 当前窗口状态: 标题='{title}', 可见={is_visible}, 最小化={is_iconic}")
            except Exception as e:
                LOGGER.error(f"[温和激活] 窗口句柄无效或已失效: {e}")
                self.steps.append(f"窗口句柄无效或已失效: {e}")
                return False
            
            # 1. 恢复最小化窗口
            if is_iconic:
                LOGGER.info("[温和激活] 窗口已最小化，正在恢复...")
                self.steps.append("窗口已最小化，正在恢复")
                win32gui.ShowWindow(hwnd, win32con.SW_RESTORE)
                time.sleep(0.5)
                LOGGER.info("[温和激活] 窗口恢复命令已发送")
            elif not is_visible:
                # 窗口不可见且未最小化 = 托盘隐藏
                LOGGER.warning("[温和激活] 窗口在托盘中（不可见且未最小化）")
                self.steps.append("窗口在托盘中（不可见且未最小化）")
                
                # 尝试发送 WM_SYSCOMMAND 消息恢复窗口
                LOGGER.info("[温和激活] 尝试通过 WM_SYSCOMMAND 恢复托盘窗口")
                ...
                try:
                    ...
                    try:
                        if win32gui.IsWindowVisible(hwnd):
                            LOGGER.info("[温和激活] ✓ WM_SYSCOMMAND 恢复成功，窗口已可见")
                            self.steps.append("WM_SYSCOMMAND 恢复成功")
                            # 继续执行置前逻辑
                        else:
                            LOGGER.warning("[温和激活] ✗ WM_SYSCOMMAND 未生效，窗口仍不可见")
                            self.steps.append("WM_SYSCOMMAND 未生效，需要特殊处理")
                            return False
                    except Exception as e:
                        LOGGER.warning(f"[温和激活] 再次检查窗口状态时出错: {e}")
                        # 假设成功，继续执行
                        self.steps.append("WM_SYSCOMMAND 已发送（无法验证）")
                except Exception as e:
                    LOGGER.error(f"[温和激活] 发送 WM_SYSCOMMAND 失败: {e}")
                    self.steps.append(f"发送 WM_SYSCOMMAND 失败: {e}")
                    return False
            
            # 2. 置前窗口
            LOGGER.info("[温和激活] 开始置前窗口操作")
    ...
    def _bring_to_front(self, hwnd: int) -> bool:
        """将窗口置前（多种方法组合）"""
        try:
            LOGGER.info(f"[置前窗口] 开始将窗口置前 hwnd={hwnd}")
            
            # 方法1: SetWindowPos 置顶技巧
            try:
                LOGGER.info("[置前窗口] 方法1: SetWindowPos 置顶技巧")
                ...
                LOGGER.info("[置前窗口] ✓ SetWindowPos 置顶成功")
                self.steps.append("使用 SetWindowPos 置顶成功")
            except Exception as e:
                LOGGER.warning(f"[置前窗口] ✗ SetWindowPos 失败: {e}")
            ...
            try:
                LOGGER.info("[置前窗口] 方法2: AttachThreadInput 技巧")
                fg_hwnd = win32gui.GetForegroundWindow()
                LOGGER.info(f"[置前窗口] 当前前台窗口: {fg_hwnd}, 目标窗口: {hwnd}")
                
                if fg_hwnd and fg_hwnd != hwnd:
                    fg_thread = win32process.GetWindowThreadProcessId(fg_hwnd)[0]
                    target_thread = win32process.GetWindowThreadProcessId(hwnd)[0]
                    LOGGER.info(f"[置前窗口] 前台线程: {fg_thread}, 目标线程: {target_thread}")
                    
                    if fg_thread != target_thread:
                        LOGGER.info("[置前窗口] 线程不同，使用 AttachThreadInput")
                        import ctypes
                        ...
                        LOGGER.info("[置前窗口] ✓ AttachThreadInput 获取焦点成功")
                        self.steps.append("使用 AttachThreadInput 获取焦点成功")
                    else:
                        LOGGER.info("[置前窗口] 线程相同，直接设置焦点")
                        win32gui.BringWindowToTop(hwnd)
                        win32gui.SetForegroundWindow(hwnd)
                        LOGGER.info("[置前窗口] ✓ 直接设置焦点成功")
                        self.steps.append("直接设置焦点成功")
                else:
                    LOGGER.info("[置前窗口] 目标窗口已是前台窗口或无前台窗口")
            except Exception as e:
                LOGGER.warning(f"[置前窗口] ✗ AttachThreadInput 失败: {e}")
                self.steps.append(f"AttachThreadInput 失败: {e}")
                try:
                    win32gui.BringWindowToTop(hwnd)
                    win32gui.SetForegroundWindow(hwnd)
                    LOGGER.info("[置前窗口] 回退方法成功")
                except Exception as e2:
                    LOGGER.warning(f"[置前窗口] 回退方法也失败: {e2}")
            
            time.sleep(0.15)
            
            # 验证（添加完善的异常处理）
            LOGGER.info("[置前窗口] 开始验证窗口状态")
            try:
                # 验证窗口是否可见
                try:
                    is_visible = win32gui.IsWindowVisible(hwnd)
                    title = win32gui.GetWindowText(hwnd)
                    LOGGER.info(f"[置前窗口] 验证: 窗口可见={is_visible}, 标题='{title}'")
                except Exception as e:
                    LOGGER.warning(f"[置前窗口] 验证窗口可见性时出错: {e}")
                ...
                if is_visible:
                    ...
                    try:
                        ...
                        LOGGER.info(f"[置前窗口] 当前前台窗口: {fg_hwnd}, 目标窗口: {hwnd}")
                        if fg_hwnd == hwnd:
                            LOGGER.info("[置前窗口] ✓ 窗口激活成功（已获得焦点）")
                            self.steps.append("窗口激活成功（已获得焦点）")
                        else:
                            try:
                                fg_title = win32gui.GetWindowText(fg_hwnd)
                                LOGGER.warning(f"[置前窗口] ⚠ 窗口已显示但未获得焦点，前台窗口是: {fg_title}")
                            except:
                                LOGGER.warning(f"[置前窗口] ⚠ 窗口已显示但未获得焦点")
                            self.steps.append("窗口已显示（但未获得焦点）")
                    except Exception as e:
                        LOGGER.warning(f"[置前窗口] 检查焦点时出错: {e}")
                        self.steps.append("窗口已显示（无法验证焦点）")
                    return True
                else:
                    LOGGER.warning("[置前窗口] ✗ 窗口仍不可见")
                    self.steps.append("窗口仍不可见")
                    return False
            except Exception as e:
                LOGGER.error(f"[置前窗口] 验证窗口状态时发生异常: {e}", exc_info=True)
    ...
    def _find_window_by_process(process_name: str):
        ...
        LOGGER.info(f"[窗口查找] 开始查找进程: {process_name}")
        ...
        LOGGER.info(f"[窗口查找] 搜索目标: {target}, 别名: {aliases}")
        ...
        def callback(hwnd, _):
            ...
            if any(ignore in title_lower for ignore in IGNORE_TITLES):
                LOGGER.debug(f"[窗口查找] 跳过服务窗口: hwnd={hwnd}, 标题='{title}'")
                return True
            ...
            if exe_basename in candidates or any(candidate in exe_name for candidate in candidates):
                LOGGER.info(f"[窗口查找] 找到匹配窗口: hwnd={hwnd}, 标题='{title}', 进程={exe_basename}, PID={pid}")
                try:
                    ...
                    # 记录窗口样式
                    is_visible = win32gui.IsWindowVisible(hwnd)
                    LOGGER.info(f"[窗口查找] 窗口属性: 可见={is_visible}, Style={hex(style)}, ExStyle={hex(ex_style)}")
                    
                    # 过滤工具窗口
                    if ex_style & win32con.WS_EX_TOOLWINDOW:
                        LOGGER.debug(f"[窗口查找] 跳过工具窗口: hwnd={hwnd}")
                        return True
                    
                    # 必须有标题栏
                    if not (style & win32con.WS_CAPTION):
                        LOGGER.debug(f"[窗口查找] 跳过无标题栏窗口: hwnd={hwnd}")
                        return True
                    
                    # 区分可见和隐藏
                    if is_visible:
                        LOGGER.info(f"[窗口查找] ✓ 添加到可见窗口列表: hwnd={hwnd}, 标题='{title}'")
                        visible_hwnds.append((hwnd, title, style))
                    else:
                        LOGGER.info(f"[窗口查找] ✓ 添加到隐藏窗口列表: hwnd={hwnd}, 标题='{title}'")
                        hidden_hwnds.append((hwnd, title, style))
                except Exception as e:
                    LOGGER.warning(f"[窗口查找] 获取窗口属性失败: hwnd={hwnd}, 错误={e}")
                    pass
            return True
        ...
        try:
            ...
            LOGGER.error(f"[窗口查找] EnumWindows 失败: {e}")
            return None
        
        LOGGER.info(f"[窗口查找] 找到 {len(visible_hwnds)} 个可见窗口, {len(hidden_hwnds)} 个隐藏窗口")
        ...
        def score_window(item):
            ...
            LOGGER.debug(f"[窗口评分] hwnd={hwnd}, 标题='{title}', 分数={score}")
            return score
        ...
        if all_windows:
            ...
            best_score = score_window(best_window)
            LOGGER.info(f"[窗口查找] 选择最佳窗口: hwnd={best_window[0]}, 标题='{best_window[1]}', 分数={best_score}")
            
            # 打印所有候选窗口
            LOGGER.info(f"[窗口查找] 所有候选窗口:")
            for i, (hwnd, title, style) in enumerate(all_windows[:5]):  # 只显示前5个
                is_visible = "可见" if (hwnd, title, style) in visible_hwnds else "隐藏"
                LOGGER.info(f"  #{i+1}: hwnd={hwnd}, 标题='{title}', 状态={is_visible}, 分数={score_window((hwnd, title, style))}")
            
            return best_window[0]
        
        LOGGER.warning(f"[窗口查找] 未找到任何匹配的窗口")
        return None
    ...
    def _result(self, success: bool, message: str) -> Dict[str, Any]:
        """返回结果"""
        LOGGER.info(f"=" * 80)
        LOGGER.info(f"激活结果: {'成功' if success else '失败'}")
        LOGGER.info(f"消息: {message}")
        LOGGER.info(f"=" * 80)
        LOGGER.info(f"详细操作步骤:")
