    ...
    def _gentle_activate(self, hwnd: int) -> bool:
        ...
        try:
            ...
            if is_iconic:
                ...
                # 窗口不可见且未最小化 = 托盘隐藏
                # 对于托盘应用，WM_SYSCOMMAND 可能与某些应用（如 QQ NT/Electron）冲突
                # 返回 False 让上层使用 Shell start 重新启动
                LOGGER.warning("[温和激活] 窗口在托盘中，跳过 WM_SYSCOMMAND，返回失败以使用 Shell start")
                self.steps.append("窗口在托盘中，将使用 Shell start 重新启动")
                return False
