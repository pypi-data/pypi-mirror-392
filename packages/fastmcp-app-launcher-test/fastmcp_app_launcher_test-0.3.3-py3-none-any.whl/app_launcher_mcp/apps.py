...
def _load_package_example_apps() -> List[AppInfo]:
    try:
        package_files = resources.files(__package__ or "app_launcher_mcp")
    except Exception:
        return []

    try:
        config_ref = package_files.joinpath("mcp-apps.example.json")
    ...
    try:
        data = json.loads(config_ref.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise ValueError("内置示例配置解析失败") from exc
...
def load_from_config(paths: Sequence[Path] | None = None) -> List[AppInfo]:
    candidate_paths = _config_paths_override() + tuple(paths or _default_config_paths())
    for path in candidate_paths:
        ...
        apps = [_app_from_mapping(item) for item in payload]
        return [app for app in apps if app]
    return _load_package_example_apps()
