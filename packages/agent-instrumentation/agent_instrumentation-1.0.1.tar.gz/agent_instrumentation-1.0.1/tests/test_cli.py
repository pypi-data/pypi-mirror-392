...
from agent_instrumentation.cli import main
...
class TestRunWithSitecustomize:
    ...
    def test_successful_execution(self, mock_run, tmp_path):
        ...
        with patch.object(Path, 'parent', tmp_path):
            ...
            with patch('agent_instrumentation.cli.main.check_sitecustomize_conflicts'):
                with pytest.raises(SystemExit) as exc_info:
    ...
    def test_bootstrap_directory_not_found(self, tmp_path):
        ...
        package_dir = tmp_path / "agent_instrumentation"
    ...
    def test_warns_when_sitecustomize_exists(self, tmp_path):
        ...
        try:
            ...
            with patch('sys.stderr', StringIO()) as mock_stderr:
                ...
                assert "This may conflict with AMP instrumentation" in stderr_output
        finally:
...
class TestCLI:
    ...
    @patch('agent_instrumentation.cli.main.run_with_sitecustomize')
    def test_cli_passes_args(self, mock_run):
        """Test that CLI passes arguments to run_with_sitecustomize."""
        with patch.object(sys, 'argv', ['agent-trace', 'python', 'script.py', '--arg']):
            main.cli()
