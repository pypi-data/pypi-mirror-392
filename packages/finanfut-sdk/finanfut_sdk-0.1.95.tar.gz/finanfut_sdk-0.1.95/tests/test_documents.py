...
def test_upload_reads_file_and_encodes_payload(tmp_path: Path) -> None:
    ...
    assert args[0] == "POST"
    assert args[1].endswith("/api/v1/agents/document-qa/files")
    encoded_payload = kwargs["json"]["content_base64"]
    assert base64.b64decode(encoded_payload).decode("utf-8") == "hello world"
