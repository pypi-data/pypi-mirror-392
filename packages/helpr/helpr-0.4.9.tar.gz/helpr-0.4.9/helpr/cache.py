...
class RedisHelper:
    ...
    def save_string(self, key: str, value: str, expire: Optional[int] = None) -> bool:
        return bool(self.redis_client.set(key, value, ex=expire))

    def save_string_if_not_exists(
        self, key: str, value: str, expire: Optional[int] = None
    ) -> bool:
        return bool(self.redis_client.set(key, value, ex=expire, nx=True))
    ...
    def save_sorted_set(
        ...
        for value, score in values.items():
            self.redis_client.zadd(key, {value: score}, incr=True)
        if expire is not None:
            self.redis_client.expire(key, expire)
        return True
    ...
    def del_hmset_fields(self, key: str, fields: List[str]) -> bool:
        return bool(self.redis_client.hdel(key, *fields))
    ...
    def get_lock(self, key: str, timeout: Optional[float] = None) -> redis.lock.Lock:
        return self.redis_client.lock(key, timeout=timeout)
    ...
    def zunionstore(
        ...
        """
        return self.redis_client.zunionstore(destination, keys, aggregate=aggregate)
    ...
    def save_json(
        ...
        if overwrite:
            return bool(self.redis_client.set(key, data, ex=expire))
        return bool(self.redis_client.set(key, data, ex=expire, nx=True))
            ...
            if not self.enable_multidb:
                ...
                processed_keys = [
                    (
                        ...
                        else str(key).replace(db_prefix, "", 1)
                    )
                    for key in keys
                ]
            else:
