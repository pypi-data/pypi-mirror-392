...
from import_completer.code_scanner import extract_symbols_from_file
from import_completer.code_scanner import discover_python_modules
import itertools
...
# DEFAULT_DATABASE_PATH = Path("./.import_completer_symbols.db")
...
class DatabaseHandler:
    ...
    async def perform_rebuild_for_origin(self, origin_index: int, symbols: Iterable[ScannedSymbol]) -> None:
        assert self._connection is not None, "Database connection not initialized!"
        ...
        async with self._connection.cursor() as cursor:
            ...
            logger.info(f"Added {cursor.rowcount} symbols for origin_index: {origin_index}")
...
def generate_symbols_from_paths(*paths: str | Path) -> Iterable[ScannedSymbol]:
    for path in (Path(p).absolute() for p in paths):
        if not path.is_dir():
            continue
        for module_path in discover_python_modules(path):
            yield from extract_symbols_from_file(module_path)
...
async def main():
    ...
    async with DatabaseHandler() as handler:
        await handler.create_schema()
        await handler.create_indices()
        # await handler.perform_full_symbols_rebuild(generate_symbols_from_paths(*args.path))
        await handler.perform_partial_symbols_rebuild(generate_symbols_from_paths(*args.path))

        if args.query:
            print(f"Query: {args.query}")
            async for result in handler.lookup_symbol_name(args.query):
                print(f"  {result.kind}: {result.name} ({result.file_path}:{result.line})")
