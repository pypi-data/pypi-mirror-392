...
def discover_python_modules(directory: str | Path) -> Generator[Path, None, None]:
    """
    ...
    for child_entity_path in directory.iterdir():
        if child_entity_path.is_file() and RE_PYTHON_VALID_MODULE_FILE_NAME.match(child_entity_path.name) is not None:
            yield child_entity_path
        elif child_entity_path.is_dir() and RE_PYTHON_VALID_MODULE_DIRECTORY_NAME.match(child_entity_path.name) is not None:
            # Recursively process subdirectories if they are Python packages
            if (child_entity_path / "__init__.py").exists() or (child_entity_path / "__init__.pyi").exists():
...
def extract_symbols_from_file(file_path: Path, origin_path: Path) -> Iterator[ScannedSymbol]:
    """
    ...
    yield from extract_symbols(source_code, annotated_file_path=file_path, origin_path=origin_path)


def extract_symbols(source_code: bytes, annotated_file_path: Path, origin_path: Path) -> Iterator[ScannedSymbol]:
    """
    ...
    def _make_scanned_symbol(kind: Literal["function", "class", "variable"], node: Node) -> ScannedSymbol:
        assert node.text is not None
        return ScannedSymbol(kind=kind, name=node.text.decode("utf-8"), parent_module=parent_module, metadata={})
...
if __name__ == "__main__":
    ...
    for path in (Path(p) for p in sys.path):
        ...
        for module_path in discover_python_modules(path):
            ...
            for symbol in extract_symbols_from_file(module_path, annotated_source_sys_path=path):
