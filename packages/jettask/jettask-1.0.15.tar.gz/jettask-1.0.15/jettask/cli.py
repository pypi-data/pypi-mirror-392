...
def webui_consumer(task_center, check_interval, concurrency, prefetch, api_key, debug):
    import asyncio
...
@click.option('--api-key', envvar='JETTASK_API_KEY', help='API密钥用于请求鉴权（与API服务器保持一致）')
@click.option('--debug', is_flag=True, help='启用调试模式')
def scheduler(task_center, interval, batch_size, check_interval, api_key, debug):
    import asyncio
