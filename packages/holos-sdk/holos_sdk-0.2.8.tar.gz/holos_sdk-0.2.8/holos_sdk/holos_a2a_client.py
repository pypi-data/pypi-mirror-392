...
class HolosTracingA2AClient(Client):
    ...
    async def _read_response_from_event_stream(self, event_stream: AsyncIterator[ClientEvent | Message]) -> AsyncIterator[ClientEvent | Message]:
    ...
    async def send_message(self, request: Message, *, context: ClientCallContext | None = None, from_objects = None) -> AsyncIterator[ClientEvent | Message]:
        ...
        assignment = Assignment(object_id=request.message_id, assignee_id=self._card.url, assignee_name=self._card.name)
        ...
        async for response in self._read_response_from_event_stream(event_stream):
            yield response
    ...
    async def send_plan_streaming( self, plan: Plan, context: Optional[ClientCallContext] = None, from_objects = None) -> AsyncIterator[ClientEvent | Message]:
        ...
        try:
            ...
            return self.send_message(message, context=context, from_objects=message_from_objects)
