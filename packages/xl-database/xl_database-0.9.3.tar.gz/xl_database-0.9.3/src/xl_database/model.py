            ...
            for key, map in self.__key_derive_map__.items():
                if k == key:
                    if isinstance(map, list):
                        ...
                        func = map['func']
                        tmp[name] = func(v)
        for relationship_name, map in self.__relationship_map__.items():
            relationship = getattr(self, relationship_name)
            if isinstance(map, list):
                for map_ in map:
                    name = map_['name']
                    func = map_['func']
                    default = map_['default'] if 'default' in map_ else ''
                    tmp[name] = func(relationship) if relationship else default
            else:
                name = map['name']
                func = map['func']
                default = map['default'] if 'default' in map else ''
                tmp[name] = func(relationship) if relationship else default
        return tmp
