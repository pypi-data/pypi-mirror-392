...
def _handle_auth_service_error(exc: AuthServiceError, store: AuthStore) -> None:
    ...
    if 'HTTP 401' in message:
        ...
        raise click.ClickException(
            ...
            f'{colorama.Style.BRIGHT}konduktor auth login '
            f'{colorama.Style.RESET_ALL}again.'
        ) from exc
...
def login(force: bool):
    ...
    click.echo(f'  Open: {colorama.Style.BRIGHT}{url_hint}{colorama.Style.RESET_ALL}')
    if not device.verification_uri_complete:
        click.echo(
            f'  Code: '
            f'{colorama.Style.BRIGHT}{device.user_code}{colorama.Style.RESET_ALL}'
        )
    ...
    while time.time() < deadline:
        ...
        if status.status == 'approved':
            ...
            click.echo(f'Session expires at {expires_at.astimezone().isoformat()}')
            return
