...
dat = pd.DataFrame()
time_close1 = datetime.strptime("11:33:00", '%H:%M:%S')
time_open1 = datetime.strptime("9:30:00", '%H:%M:%S')
time_close2 = datetime.strptime("15:03:00", '%H:%M:%S')
...
def login(token_str):
    global token, str_bg, bLogin
    token = token_str
    # 将内容转换为字符串列表
    if debug == 1:
        url = f"http://127.0.0.1:5000/api/interface/auth"
    else:
        url = f"https://cmes-data.com:8080/api/api/interface/auth"
    payload = {
        "token": token ,
        "type": "a_gu"
    }
...
def get_real_hq(all_stock):
    ...
    if time_str < time_open1 or time_str > time_close2 or (time_str > time_close1 and time_str < time_open2):
        return pd.DataFrame()
