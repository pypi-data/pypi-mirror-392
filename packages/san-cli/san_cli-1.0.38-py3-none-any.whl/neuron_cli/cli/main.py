...
def cmd_setup(args):
    """Auto-setup SAN CLI API Server as system service"""
    import os
    import sys
    import subprocess
    import platform
    ...
    if args.uninstall:
        logger.info("🗑️  Uninstalling SAN CLI API Server service...")
        
        if platform.system() == "Darwin":
            # macOS
            plist_path = os.path.expanduser("~/Library/LaunchAgents/com.nexuscore.san-cli-api.plist")
            if os.path.exists(plist_path):
                subprocess.run(["launchctl", "unload", plist_path], stderr=subprocess.DEVNULL)
                os.remove(plist_path)
                logger.info("✅ Service uninstalled successfully!")
            else:
                logger.warning("⚠️  Service not found")
        
        elif platform.system() == "Linux":
            # Linux
            service_file = "/etc/systemd/system/san-cli-api.service"
            if os.path.exists(service_file):
                subprocess.run(["sudo", "systemctl", "stop", "san-cli-api"], stderr=subprocess.DEVNULL)
                subprocess.run(["sudo", "systemctl", "disable", "san-cli-api"], stderr=subprocess.DEVNULL)
    ...
    # Install service
    logger.info("🚀 Setting up SAN CLI API Server as system service...")
    
    # Get path to san binary
    san_path = shutil.which("san")
    if not san_path:
        logger.error("❌ Error: 'san' command not found in PATH")
        return 1
...
<dict>
    <key>Label</key>
    <string>com.nexuscore.san-cli-api</string>
    
    <key>ProgramArguments</key>
    <array>
        <string>{san_path}</string>
    ...
    <string>/tmp/san-cli-api.log</string>
    ...
    <dict>
        <key>PATH</key>
        <string>/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:{os.path.dirname(san_path)}</string>
    </dict>
</dict>
</plist>
"""
        ...
        if result.returncode == 0:
            ...
            logger.info(f"   Config: {plist_path}")
            logger.info("   Logs: /tmp/san-cli-api.log")
            logger.info("")
            logger.info("Commands:")
            logger.info("   Start:   launchctl start com.nexuscore.san-cli-api")
            logger.info("   Stop:    launchctl stop com.nexuscore.san-cli-api")
            logger.info("   Status:  launchctl list | grep san-cli-api")
            logger.info("   Logs:    tail -f /tmp/san-cli-api.log")
            logger.info("")
            logger.info("🎉 API Server will now start automatically on boot!")
        else:
...
Description=SAN CLI API Server
After=network.target

[Service]
Type=simple
User={os.getenv('USER')}
ExecStart={san_path} api-server --foreground
Restart=always
RestartSec=10
StandardOutput=append:/tmp/san-cli-api.log
StandardError=append:/tmp/san-cli-api.log
...
"""
        ...
        try:
            with open("/tmp/san-cli-api.service", 'w') as f:
                f.write(service_content)
            
            subprocess.run(["sudo", "mv", "/tmp/san-cli-api.service", service_file], check=True)
            subprocess.run(["sudo", "systemctl", "daemon-reload"], check=True)
            subprocess.run(["sudo", "systemctl", "enable", "san-cli-api"], check=True)
            subprocess.run(["sudo", "systemctl", "start", "san-cli-api"], check=True)
            
            logger.info("✅ Service installed and started successfully!")
            logger.info(f"   Config: {service_file}")
            logger.info("   Logs: /tmp/san-cli-api.log")
            logger.info("")
            logger.info("Commands:")
            logger.info("   Start:   sudo systemctl start san-cli-api")
            logger.info("   Stop:    sudo systemctl stop san-cli-api")
            logger.info("   Status:  sudo systemctl status san-cli-api")
            logger.info("   Logs:    sudo journalctl -u san-cli-api -f")
            logger.info("")
            logger.info("🎉 API Server will now start automatically on boot!")
...
def cmd_api_server(args):
    ...
    # Check if --foreground flag is set (daemon is default)
    daemon = not getattr(args, 'foreground', False)
    
    if daemon:
        # Check if already running by reading announcement file
        log_path = os.path.join(tempfile.gettempdir(), 'san-cli-api.log')
        announcement_path = os.path.join(tempfile.gettempdir(), 'san-cli-api.json')
            ...
            try:
                ...
                try:
                    ...
                    if pid and psutil.pid_exists(pid):
                        logger.info(f"✅ SAN CLI API Server already running (PID: {pid})")
                        logger.info(f"   Port: {port}")
                        logger.info(f"   Logs: {log_path}")
                        logger.info(f"   To stop: kill {pid}")
                        return 0
                except ImportError:
                    # If psutil not available, trust the announcement file
                    if pid:
                        logger.info(f"✅ SAN CLI API Server already running (PID: {pid})")
                        logger.info(f"   Port: {port}")
                        logger.info(f"   Logs: {log_path}")
                ...
                # Announcement file exists but is invalid, continue to start
                pass
        # Start as daemon (cross-platform)
        logger.info("🚀 Starting SAN CLI API Server in background...")
        log_path = os.path.join(tempfile.gettempdir(), 'san-cli-api.log')

        # POSIX: use fork/setsid when available
        if hasattr(os, 'fork'):
            pid = os.fork()
            if pid > 0:
                logger.info(f"✅ API Server started (PID: {pid})")
                logger.info(f"   Logs: {log_path}")
                logger.info(f"   To stop: kill {pid}")
                return 0
            ...
            # Non-POSIX fallback: spawn detached subprocess
            creationflags = 0
            ...
            with open(log_path, 'a') as f:
                proc = subprocess.Popen(
                    [sys.executable, '-c',
                     'from neuron_cli.api_server import start_server; start_server()'],
                    stdout=f,
                    stderr=f,
            ...
            logger.info(f"   Logs: {log_path}")
            logger.info(f"   To stop: kill {proc.pid}")
            return 0
    else:
        # Run in foreground
...
For more information: https://nexuscore.cloud/neuron/docs
    ...
    # Setup command (NEW - Auto-install everything)
    setup_parser = subparsers.add_parser(
        'setup',
        help='Auto-setup SAN CLI API Server as system service'
    )
    setup_parser.add_argument('--uninstall', action='store_true', help='Uninstall the service')
    ...
    api_server_parser.add_argument('--foreground', action='store_true', help='Run in foreground (default: background)')
    api_server_parser.set_defaults(func=cmd_api_server, daemon=True)
