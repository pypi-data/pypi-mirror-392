            ...
            if 'registry.nexuscore.cloud' in image:
                ...
                if device_api_key:
                    login_result = subprocess.run(
                        ['docker', 'login', 'registry.nexuscore.cloud', '-u', 'device', '--password-stdin'],
                        input=device_api_key,
                        capture_output=True,
                        timeout=30,
                        text=True
                    )
                    if login_result.returncode == 0:
                        ...
                        logger.warning(f"   ⚠️  Registry login failed: {login_result.stderr.strip()}")
                        logger.warning(f"   Attempting pull anyway (may use cached image)...")
                else:
                    logger.warning(f"   ⚠️  No device API key found, attempting pull anyway...")
