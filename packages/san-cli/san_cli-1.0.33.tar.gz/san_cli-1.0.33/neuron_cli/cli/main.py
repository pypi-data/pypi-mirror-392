        ...
        try:
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                try:
                    ...
                    # Look for neuron_cli/api_server.py specifically
                    cmdline_str = ' '.join(str(arg) for arg in cmdline)
                    if 'neuron_cli' in cmdline_str and 'api_server' in cmdline_str and 'start_server' in cmdline_str:
                        logger.info(f"✅ SAN CLI API Server already running (PID: {proc.info['pid']})")
                        logger.info(f"   To stop: kill {proc.info['pid']}")
                        return 0
