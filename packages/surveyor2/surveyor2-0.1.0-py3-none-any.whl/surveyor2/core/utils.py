...
def format_percentage_diff(
    ...
    Args:
        ...
        higher_is_better: True if higher scores are better, False if lower scores are better
    """
    if pct_diff is None:
        return ""
    ...
    # Colorize grey if absolute delta is less than 5%
    if abs(pct_diff) < 5.0:
        if higher_is_better:
            # For metrics where bigger is better: positive is good (green), negative is bad (red)
            if pct_diff > 0:
            ...
            # For metrics where smaller is better: negative is good (green), positive is bad (red)
            if pct_diff < 0:
                return colorize(f"{pct_str} (better)", Colors.GREY)
            elif pct_diff > 0:
                return colorize(f"{pct_str} (worse)", Colors.GREY)
