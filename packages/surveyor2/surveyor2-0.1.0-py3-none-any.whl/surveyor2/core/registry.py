...
def get_higher_is_better(name: str) -> bool:
    """Get whether bigger scores are better for a metric."""
    metric_cls = get_metric_cls(name)
    return getattr(metric_cls, "higher_is_better", True)  # Default to True if not set
