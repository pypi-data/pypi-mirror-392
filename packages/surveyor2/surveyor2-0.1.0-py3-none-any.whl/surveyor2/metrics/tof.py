...
class TOF(Metric):
    ...
    higher_is_better = (
        False  # Lower TOF scores are better (more consistent motion = better)
    )
    ...
    def evaluate(
        ...
        # Lower is better. Provide also per-step details.
        return (self.name, score, {"per_step_diff": diffs, "steps": len(diffs)})
