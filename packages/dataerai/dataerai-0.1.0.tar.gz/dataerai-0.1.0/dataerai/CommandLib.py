# @package portal.CommandLib
# Provides a high-level client interface to the Portal server
#
# The Portal CommandLib module contains a single API class that provides
# a high-level client interface for interacting with a Portal server. This
# module relies on the Portal MessageLib and Connection modules  for lower-
...
class API:
    """
    A high-level messaging interface to the Portal core server

    The Portal CommandLib.API class provides a high-level interface
    for sending requests to a Portal server. Requests are sent via python
    ...
    Portal client package. Basic functionality of th API class mirrors the
    capabilities exposed in the Portal CLI.
    ...
    On success, a secure connection is established to the configured Portal
    ...
    def loginByPassword(self, uid, password):
        ...
        using the supplied Portal user ID and password.
    ...
    def generateCredentials(self):
        ...
        Requests the Portal server to generate and send local credentials for
    ...
    def repoCreate(
        ...
        ----------
        repo_id : str
            The id of the data repository i.e. "portal-home" internally this
            will be represented as "repo/portal-home"
        title : str
        ...
        path : str
            ...
            portal repo server. i.e. if I have a POSIX path
            /home/tony_stark/inventions/portal-home and the endpoint path
            pointed to /home/tony_stark/inventions then the POSIX path could be
            set to /portal-home, NOTE the last folder in the path must have
            the same name as the repo_id.
        exp_path : str
        admins : list[str]
            A list of Portal users that will have repository admin rights on
            the repository. i.e. ["u/tony_stark", "u/pepper"]

        Returns
        -------
        msg : RepoDataReply Google protobuf message response from Portal
    ...
    def collectionCreate(
        ...
        and will be presented in Portal catalog browser.
        ...
        topic : str
            ...
            readable and will be presented in Portal catalog browser.
        parent_id : str, Optional. Default = "root"
    ...
    def setupCredentials(self):
        ...
        This command installs Portal credentials for the current user in the
        configured client configuration directory. Subsequent use of the
        Portal API will read these credentials instead of requiring manual
        authentication.
    ...
    def _setSaneDefaultOptions(self):
        ...
        if "server_host" not in opts:
            self.cfg.set("server_host", "dataerai.com")
            opts["server_host"] = "dataerai.com"
            save = True
        ...
        if "server_pub_key_file" not in opts:
            ...
            if "server_cfg_dir" in opts:
                serv_key_file = os.path.expanduser(
                    os.path.join(opts["server_cfg_dir"], "portal-core-key.pub")
                )
            ...
            if not serv_key_file or not os.path.exists(serv_key_file):
                ...
                if "client_cfg_dir" in opts:
                    serv_key_file = os.path.expanduser(
                        os.path.join(opts["client_cfg_dir"], "portal-core-key.pub")
                    )
                    self.cfg.set("server_pub_key_file", serv_key_file)
                    opts["server_pub_key_file"] = serv_key_file
                    save = True
                ...
                if not os.path.exists(serv_key_file):
                    ...
                    url = "https://" + opts["server_host"] + "/portal-core-key.pub"
        ...
        if "client_pub_key_file" not in opts or "client_priv_key_file" not in opts:
            ...
            if "client_pub_key_file" not in opts:
                key_file = os.path.expanduser(
                    os.path.join(opts["client_cfg_dir"], "portal-user-key.pub")
                )
            ...
            if "client_priv_key_file" not in opts:
                key_file = os.path.expanduser(
                    os.path.join(opts["client_cfg_dir"], "portal-user-key.priv")
                )
