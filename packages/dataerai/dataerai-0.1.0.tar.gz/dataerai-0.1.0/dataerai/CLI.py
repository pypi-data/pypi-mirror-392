# @namespace portal.CLI
# @brief Provides a high-level client interface to the Portal server
#
# The Portal CLI module provides a high-level, text-based client
    ...
    while True:
        ...
        try:
            if _first:
                ...
                if session is None:
                    session = PromptSession(
                        history=FileHistory(os.path.expanduser("~/.portal-hist"))
                    )
                if _cur_ctx != _uid:
                ...
                _cli(prog_name="portal", args=_args, standalone_mode=False)
            ...
            if _interactive and _first:
                api_version = f"{Version_pb2.PORTAL_COMMON_PROTOCOL_API_MAJOR}."
                api_version += f"{Version_pb2.PORTAL_COMMON_PROTOCOL_API_MINOR}."
                api_version += f"{Version_pb2.PORTAL_COMMON_PROTOCOL_API_PATCH}"
                release_version = f"{Version_pb2.PORTAL_RELEASE_YEAR}."
                release_version += f"{Version_pb2.PORTAL_RELEASE_MONTH}."
                release_version += f"{Version_pb2.PORTAL_RELEASE_DAY}."
                release_version += f"{Version_pb2.PORTAL_RELEASE_HOUR}."
                release_version += f"{Version_pb2.PORTAL_RELEASE_MINUTE}"
                _print_msg(1, f"Welcome to Portal CLI, version {VERSION.__version__}")
                _print_msg(
...
# function establishes a secure connection to the configured Portal core
# server.
#
# @param opts - Configuration options (optional)
# @return Authentication Portal user ID, or None if could not authenticate
...
#
# This method attempts manual authentication using the supplied Portal user
# ID and password.
#
# @param uid - Portal user ID
...
#
# This functions executes a text-based Portal command in the same format as
# used by the Portal CLI. Instead of printing output, this function returns
# the received Portal server reply directly to the caller as a Python
# protobuf message instance. Refer to the *.proto files for details on
# the message interface.
#
# @param command - String containing CLI-style Portal command
# @exception Exception: if called prior to init(), or if command parsing fails.
# @return Portal reply message
...
def _cli(ctx, *args, **kwargs):
    """'portal' is the command-line interface (CLI) for the Portal federated data management
    service and may be used to access many of the features available via the Portal web
    portal. This CLI may be used interactively (human-friendly output) or for scripting (JSON
    output) by specifying the -s option.

    When the portal CLI is run without any command arguments, a interactive shell session is
    started. While in the shell, commands should be entered without specifying the 'portal'
    prefix.
    """
...
@click.option(
    ...
    is_flag=True,
    required=False,
    help="Raw data file is external to Portal (unmanaged)",
)
@click.option(
    "-m",
    "--metadata",
...
def _dataPut(data_id, path, wait, extension, encrypt, context):
    """
    Put (upload) raw data located at PATH to Portal record ID.  The ID
...
def _data_batch_create(collection, file, context):
    ...
    must comply with the Portal record input schema (see online documentation).
    """
...
def _taskList(time_from, to, since, status, offset, count):
    ...
    most recent first. Note that the Portal server periodically purges
...
def _setup(ctx):
    """
    Setup local credentials. This command installs Portal credentials for the
    current user in the configured client configuration directory. Subsequent
    use of the Portal CLI will read these credentials instead of requiring
    ...
    if pub_file is None:
        pub_file = os.path.join(cfg_dir, "portal-user-key.pub")
    ...
    if priv_file is None:
        priv_file = os.path.join(cfg_dir, "portal-user-key.priv")
...
def _help_cli(ctx, command):
    """
    Show Portal CLI help. Include a command name as the argument to see
    command-specific help.
    """

    if not command:
        click.echo("Portal _cli, version {}\n".format(VERSION.__version__))
...
"Portal server. Optional argument determines number of data replies received "
...
def _initialize(opts):
    ...
    try:
        ...
        if _interactive and _capi._mapi.new_client_avail:
            click.echo(
                ">>>> A newer Portal client ({}) is available <<<<".format(
