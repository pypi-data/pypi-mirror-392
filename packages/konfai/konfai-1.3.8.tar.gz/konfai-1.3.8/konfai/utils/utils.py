...
def list_supported_files(path: str):
    path = Path(path)
    if path.is_file():
...
def setup_apps(parser: argparse.ArgumentParser, user_dir: Path, tmp_dir_default: Path) -> tuple[DistributedObject, str]:
    ...
    infer_p.add_argument(
        "-i", "--input", type=lambda p: get_path(Path(p)), required=True, help="Input path: either a single volume file OR a dataset directory"
    )
    infer_p.add_argument("-o", "--output", type=lambda p: get_path(Path(p)), default="./Predictions", help="Optional output volume path")
    ...
    infer_p.add_argument("--tmp_dir", type=lambda p: Path(p).absolute(), default=tmp_dir_default, help="Use a temporary directory.")
    ...
    eval_p.add_argument("-o", "--output", type=lambda p: get_path(Path(p)), default="./Evaluations", help="Optional output volume path")
    eval_p.add_argument("--tmp_dir", type=lambda p: Path(p).absolute(), default=tmp_dir_default, help="Use a temporary directory.")
    ...
    unc_p.add_argument("-o", "--output", type=lambda p: get_path(Path(p)), default="./Evaluations", help="Optional output volume path")
    unc_p.add_argument("--tmp_dir", type=lambda p: Path(p).absolute(), default=tmp_dir_default, help="Use a temporary directory.")
    ...
    if args.command != "fine-tune":
        ...
        for idx, file in enumerate(list_supported_files(args.input)):
            print("jdckez")
            dataset.write("Volume", "P{:03d}".format(idx), sitk.ReadImage(str(file)))
    ...
    if args.command == "infer":
        ...
        for idx, file in enumerate(list_supported_files(args.gt)):
            dataset.write("Reference", "P{:03d}".format(idx), sitk.ReadImage(str(file)))
        if not args.mask:
            names = dataset.get_names("Volume")
            for name in names:
                data, attr = dataset.read_data("Volume", name)
                dataset.write("Mask", name, np.zeros_like(data), attr)
        else:
            for idx, file in enumerate(list_supported_files(args.mask)):
                dataset.write("Mask", "P{:03d}".format(idx), sitk.ReadImage(str(file)))
