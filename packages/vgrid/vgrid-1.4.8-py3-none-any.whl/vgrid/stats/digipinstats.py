...
def digipininspect(resolution):
    ...
    digipin_gdf["ipq"] = (
        4
        * np.pi
        * digipin_gdf["cell_area"]
        / (digipin_gdf["cell_perimeter"] ** 2)
    )
...
def digipin_compactness_ipq(digipin_gdf: gpd.GeoDataFrame, crs: str | None = 'proj=moll'):
    """
...
def digipin_compactness_cvh(digipin_gdf: gpd.GeoDataFrame, crs: str | None = 'proj=moll'):
    """
    ...
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("bottom", size="5%", pad=0.1)   
    digipin_gdf = digipin_gdf[~digipin_gdf["crossed"]]  # remove cells that cross the dateline 
