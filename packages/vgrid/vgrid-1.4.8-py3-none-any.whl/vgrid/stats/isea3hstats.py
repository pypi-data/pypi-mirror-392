...
def isea3hinspect(res):
    ...
    isea3h_gdf = isea3hgrid(res, output_format="gpd")
...
def isea3h_norm_area_hist(isea3h_gdf: gpd.GeoDataFrame):
    ...
    isea3h_gdf = isea3h_gdf[~isea3h_gdf["crossed"]]
...
def isea3h_compactness_cvh(isea3h_gdf: gpd.GeoDataFrame, crs: str | None = 'proj=moll'):
    ...
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("bottom", size="5%", pad=0.1)   
    isea3h_gdf = isea3h_gdf[~isea3h_gdf["crossed"]]  # remove cells that cross the dateline 
...
def isea3hinspect_cli():
    ...
    print(isea3hinspect(resolution))
