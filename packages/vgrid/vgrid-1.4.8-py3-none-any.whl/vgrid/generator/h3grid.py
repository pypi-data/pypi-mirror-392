...
def h3_grid(resolution, split_antimeridian=False):
        ...
        with tqdm(total=total_cells, desc="Generating H3 DGGS", unit=" cells") as pbar:
            for cell in base_cells:
                ...
                for child_cell in child_cells:
                    cell_polygon = h32geo(child_cell, split_antimeridian=split_antimeridian)
...
def h3_grid_within_bbox(resolution, bbox, split_antimeridian=False):
    resolution = validate_h3_resolution(resolution)
    ...
    if total_cells > MAX_CELLS:
        ...
        for bbox_buffer_cell in tqdm(bbox_buffer_cells, desc="Generating H3 DGGS"):
            cell_polygon = h32geo(
                bbox_buffer_cell, split_antimeridian=split_antimeridian
            )
...
def h3_grid_ids(resolution, split_antimeridian=False):
    """
...
def h3_grid_within_bbox_ids(resolution, bbox, split_antimeridian=False):
    """
...
def h3_grid_resample(
    resolution, geojson_features, output_format="geojson", split_antimeridian=False
):
    resolution = validate_h3_resolution(resolution)
    geometries = [
    ...
    for h3_cell in tqdm(h3_cells, desc="Generating H3 DGGS", unit=" cells"):
        cell_polygon = h32geo(h3_cell, split_antimeridian=split_antimeridian)
...
def h3grid(resolution, bbox=None, output_format="gpd", split_antimeridian=False):
    """
    ...
    """
    if bbox is None:
        h3_gdf = h3_grid(resolution, split_antimeridian=split_antimeridian)
    else:
        h3_gdf = h3_grid_within_bbox(
            resolution, bbox, split_antimeridian=split_antimeridian
        )
    output_name = f"h3_grid_{resolution}"
...
def h3grid_cli():
    ...
    parser.add_argument(
        "--fix-antimeridian",
        action="store_true",
        help="Apply antimeridian fixing to generated polygons",
    )
