...
# 'ivea4r','ivea9r','ivea3h','ivea7h','ivea7h_z7','rtea4r','rtea9r','rtea3h','rtea7h','rtea7h_z7','healpix','rhealpix'
    num_edges = 4
    if dggs_type in ["isea3h", 'isea7h','isea7h_z7',"ivea3h", "ivea7h","ivea7h_z7", "rtea3h", "rtea7h","rtea7h_z7"]:
    ...
    if dggs_type == "gnosis":
        ...
        num_cells = 10 * (4**res)
    elif dggs_type in ["isea7h", "isea7h_z7", "ivea7h","ivea7h_z7", "rtea7h","rtea7h_z7"]:
        num_cells = 10 * (7**res)+2
...
def dggalinspect(dggs_type: str, res: int, crs: str | None = None, split_antimeridian: bool = False) -> gpd.GeoDataFrame:
    """
    ...
    Args:
        ...
        crs: CRS/projection for grid generation. If None, defaults to WGS84.
    ...
    """
    dggal_gdf = dggalgen(dggs_type, res, output_format="gpd", crs=crs, split_antimeridian=split_antimeridian)
...
def dggal_norm_area(dggs_type: str, dggal_gdf: gpd.GeoDataFrame, crs: str | None = "proj=moll"):  # type: ignore
    fig, ax = plt.subplots(figsize=(10, 5))
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("bottom", size="5%", pad=0.1)
    vmin, vcenter, vmax  = dggal_gdf["norm_area"].min(), 1.0, dggal_gdf["norm_area"].max()
    norm = TwoSlopeNorm(vmin=vmin, vcenter=vcenter, vmax=vmax)
...
def dggal_compactness_cvh(dggs_type: str, dggal_gdf: gpd.GeoDataFrame, crs: str | None = 'proj=moll'):
    """
...
def dggalinspect_cli():
    ...
    CLI options:
      -t, --dggs_type
      -r, --resolution
      -crs: CRS/projection for grid generation
      --fix-antimeridian: Apply antimeridian fixing to the resulting polygons
    """
    parser = argparse.ArgumentParser(add_help=False)
    parser.add_argument(
    ...
    crs = args.crs
    print(dggalinspect(dggs_type, resolution, crs=crs, split_antimeridian=args.fix_antimeridian))
