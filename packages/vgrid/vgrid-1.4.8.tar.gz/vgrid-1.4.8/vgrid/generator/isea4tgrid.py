...
def isea4t_grid(resolution, split_antimeridian=False):
...
def isea4t_grid_within_bbox(resolution, bbox, split_antimeridian=False):
    resolution = validate_isea4t_resolution(resolution)
...
def isea4t_grid_resample(resolution, geojson_features, split_antimeridian=False):
    resolution = validate_isea4t_resolution(resolution)
    accuracy = ISEA4T_RES_ACCURACY_DICT.get(resolution)
    geometries = [
...
def isea4tgrid(resolution, bbox=None, output_format="gpd", split_antimeridian=False):
    ...
    if bbox is None:
        ...
        gdf = isea4t_grid(resolution, split_antimeridian=split_antimeridian)
    else:
        gdf = isea4t_grid_within_bbox(resolution, bbox, split_antimeridian=split_antimeridian)
