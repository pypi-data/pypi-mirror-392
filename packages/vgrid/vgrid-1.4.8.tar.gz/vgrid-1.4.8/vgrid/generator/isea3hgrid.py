...
def isea3h_grid(resolution, split_antimeridian=False):
    """
    ...
    for child in tqdm(children, desc="Generating ISEA3H DGGS", unit=" cells"):
        try:
            ...
            cell_polygon = isea3h2geo(isea3h_id, split_antimeridian=split_antimeridian)
...
def isea3h_grid_within_bbox(resolution, bbox, split_antimeridian=False):
    resolution = validate_isea3h_resolution(resolution)
...
def isea3hgrid(resolution, bbox=None, output_format="gpd", split_antimeridian=False):
    """
    ...
    if bbox is None:
        ...
        gdf = isea3h_grid(resolution, split_antimeridian=split_antimeridian)
    else:
        gdf = isea3h_grid_within_bbox(resolution, bbox, split_antimeridian=split_antimeridian)
...
def isea3hgrid_cli():
    ...
    if platform.system() == "Windows":
        try:
            result = isea3hgrid(resolution, bbox, args.output_format, split_antimeridian=args.fix_antimeridian)
