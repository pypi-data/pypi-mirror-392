...
def isea3h2geo(isea3h_ids, split_antimeridian=False):
    """
...
def isea3h2geo_cli():
    ...
    if platform.system() == "Windows":
        polys = isea3h2geo(args.isea3h, split_antimeridian=args.fix_antimeridian)
        return polys
    else:
        print("ISEA3H is only supported on Windows systems")


def isea3h2geojson(isea3h_ids, split_antimeridian=False):
    """
...
def isea3h2geojson_cli():
    ...
    args = parser.parse_args()
    if platform.system() == "Windows":
        geojson_data = json.dumps(isea3h2geojson(args.isea3h, split_antimeridian=args.fix_antimeridian))
        print(geojson_data)
    else:
        print("ISEA3H is only supported on Windows systems")
