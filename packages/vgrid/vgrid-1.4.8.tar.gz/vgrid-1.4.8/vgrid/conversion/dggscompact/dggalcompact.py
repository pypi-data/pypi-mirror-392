...
def dggalcompact(
    ...
    for dggal_id_compact in dggal_ids_compact:
        try:
            ...
            cell_polygon = dggal2geo(dggs_type, dggal_id_compact, split_antimeridian=split_antimeridian)
...
def dggalexpand(
    ...
    for zone_id_expand in zone_ids_expand:
        try:
            ...
            cell_resolution = dggrs.getZoneLevel(zone)
            cell_polygon = dggal2geo(dggs_type, zone_id_expand, split_antimeridian=split_antimeridian)
            num_edges = dggrs.countZoneEdges(zone)
            row = geodesic_dggs_to_geoseries(
                f"dggal_{dggs_type}",
                zone_id_expand,
