...
class ChatCLI:
    ...
    async def open_settings(self, focus: Optional[str] = None) -> None:
        ...
        rec_keys = {"gpt-5.1", "gpt-5", "gpt-5-codex"}
        ...
        items: List[Dict[str, Any]] = [
            ...
            {"label": "Preambles & First-turn", "type": "group", "items": [
                {"id": "preambles_enabled", "label": "Enable tool call preambles (explanations before tool use)", "type": "bool"},
    ...
    async def select_model_menu(self) -> None:
        ...
        for m, lbl in recommended:
            # Annotate recommended models with plain text instead of a star icon.
            choices.append((m, f"{lbl} (recommended)"))
    ...
    async def _stream_once(self, user_input: str) -> str:
        ...
        try:
                ...
                if self._restart_after_summary:
                    ...
                    self._did_inject_working_memory = False
                    # Allow custom first-turn text to inject again
                    try:
                        self._did_inject_custom_first_turn = False
                    except Exception:
                        pass
    ...
    async def _wizard_model_step(self) -> None:
        """First-time wizard: pick the default model via a menu (no Y/N)."""
        choice = await self._menu_choice(
            "Default model",
            "Pick the default model henosis-cli should use for new chats:",
            [
                ("gpt-5.1", "Use recommended: gpt-5.1"),
                ("picker", "Choose another model from the model list"),
                ("server", "Use server default (no override)"),
            ],
        )
        if choice is None:
            # If user cancels, leave model as-is (likely server default)
            return
        if choice == "gpt-5.1":
            self.model = "gpt-5.1"
        elif choice == "picker":
    ...
    async def _maybe_run_first_time_wizard(self, force: bool = False) -> None:
        ...
        preamble_choice = await self._menu_choice(
            ...
            [
                ("on", "Enable preambles (explain tool usage before calling tools)"),
        ...
        # Final recommendation banner as a menu (no Y/N)
        if self.model != "gpt-5.1":
            final_model_choice = await self._menu_choice(
                "Recommended model",
                "For best results we recommend using gpt-5.1 as your default model.",
                [
                    (
                        "keep",
                        f"Keep current default ({self.model or 'server default'})",
                    ),
                    ("switch", "Switch to gpt-5.1 now"),
                ],
            )
            if final_model_choice == "switch":
                self.model = "gpt-5.1"
                await self._save_server_settings()
