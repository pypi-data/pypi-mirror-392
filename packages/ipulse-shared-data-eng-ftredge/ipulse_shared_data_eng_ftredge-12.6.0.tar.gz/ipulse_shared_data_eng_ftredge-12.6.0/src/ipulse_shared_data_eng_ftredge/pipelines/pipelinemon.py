...
class Pipelinemon:
    ...
    def generate_execution_summary(self, countable_subj_name: str, total_countables: int, subj_resource:Resource=AbstractResource.PIPELINE_SUBJECT_SEQUENCE, final_level:Optional[LogLevel]=None) -> str:
        ...
        if (self._subject_counts and 
            ...
            failures = self._subject_counts.get('countables_errors', 0)
            ...
            failures = sum(event_counts[str(status)] for status in ProgressStatus.failure_statuses())
        ...
        - FINISHED_WITH_ISSUES {str(subj_resource)}(s): [{failures}/{total_countables}] {countable_subj_name}(s)
