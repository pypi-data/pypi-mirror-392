...
def MakeScalarWrapper( proxy_object, example_value ):
	try: example_value.dtype
	except: pass
	else:
		if   example_value.dtype.name.rstrip( '0123456789' ) == 'int':   example_value =   int( example_value )
		elif example_value.dtype.name.rstrip( '0123456789' ) == 'float': example_value = float( example_value )
...
class ManagedProperty( object ):
	def __init__( self, default, transfer=None, doc='', canonical_name=None, read_only=False, notes='', names=(), default_string='', accelerate=True ):
		"""
