...
def _readfile( filename, mode='rt' ):
	with open( filename, mode ) as fh: return fh.read()

def _doc( doc, name=None ):
	source_filename = _as_filename( doc )
	if source_filename: doc = _readfile( source_filename, 'rt' ).replace( '\r\n', '\n' ).replace( '\r', '\n' )
	doc = '\n' + doc.lstrip( '\n' ).rstrip() + '\n'
	doc = doc.replace( '\t', ' ' * 4 )
...
def _auto_compose_rst():
	...
	for i, fullFilePath in enumerate( examples ):
		...
		tocEntry = '   %s\n' % rstStem
		#oneLineSummary = [ line[ 2: ].strip() for line in _readfile( fullFilePath ,'rt' ).replace( '\r\n', '\n' ).replace( '\r', '\n' ).split( '\n' ) if line.startswith( '#:' ) ]
