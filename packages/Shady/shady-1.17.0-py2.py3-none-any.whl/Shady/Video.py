...
class VideoSource( Rendering.Scheduled ):
	...
	def __init__( self, source=None, stimulus=None, dims=None ):
		self.__handle = None
		self.__live = None
		...
		self.__frameWidth  = None if dims is None else dims[ 0 ]
		self.__frameHeight = None if dims is None else dims[ 1 ]
	...
	def source():
		...
		def fset( self, value ):
			...
			if value is None:
				...
				if self.__frameWidth:  self.__handle.set( getattr( cv2, 'CAP_PROP_FRAME_WIDTH',  3 ), self.__frameWidth  )
				if self.__frameHeight: self.__handle.set( getattr( cv2, 'CAP_PROP_FRAME_HEIGHT', 4 ), self.__frameHeight )
	...
	def Snap( self, pil=False ):
		handle = self.__handle
		if not handle or not handle.isOpened(): raise RuntimeError( 'cannot take snapshot - video source not ready' )
		success, image = handle.read()
		if not success: raise RuntimeError( 'snapshot failed' )
		image = image[ :, :, ::-1 ]
		if pil: import PIL.Image; image = PIL.Image.fromarray( image )
		return image
