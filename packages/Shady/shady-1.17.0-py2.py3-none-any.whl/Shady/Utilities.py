...
def _ReadFileInto( filename, d ):
	with open( arg, 'rt' ) as fh:
		exec( fh.read(), dict( nan=float( 'nan' ), inf=float( 'inf' ) ), d )
...
def WorldConstructorCommandLine( argv=None, doc=None, **defaults ):
	...
	opt( 'clearColor',           default=None,  type=( None, int, float, tuple, list ), min=0.0, max=1.0, length=3 )
	opt( 'overlayMode',          default=0,     type=( int ) )
...
def AutoFinish( world, shell=False, prefer_ipython=True, plot='auto' ):
	...
	keyListener = None
	if world.constructorOptions.get( 'overlayMode', False ):
		print( 'Since the World was created with overlayMode=True, the Shady window will not listen to keystrokes.\nNow attempting to load a third-party global keystroke listener...' )
		from . import KeystrokesWithoutWindow # needs third-party package pynput; gives the World a synchronous ListenForKeystrokes() method that you can call from the shell
		if shell or Interactive(): print( 'Activate the listener by calling the World instance\'s .ListenForKeystrokes() method.' )
		else: keyListener = KeystrokesWithoutWindow.KeyListener( world )
	try:
		if keyListener: keyListener.Start( threaded=True )
		if shell:
		...
		if keyListener: keyListener.Join()
