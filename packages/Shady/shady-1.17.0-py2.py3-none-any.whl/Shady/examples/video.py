...
if __name__ == '__main__':
	...
	sourceWidth  = cmdline.Option( 'sourceWidth',  None,  type=( None, int ), min=0, container=None, doc="Overrides the default video width (useful for camera sources, if hardware and driver support it)." )
	sourceHeight = cmdline.Option( 'sourceHeight', None,  type=( None, int ), min=0, container=None, doc="Overrides the default video height (useful for camera sources, if the hardware and driver support it)." )
	...
	""#>
	if sourceWidth or sourceHeight:
		"""
		With your command-line options, you have chosen to explicitly
		override the source dimensions. To set that up, we'll have to
		create a `VideoSource` instance explicitly rather than just
		supplying your int or str `source` value as the `.video`
		property of the `Stimulus`, which would create a `VideoSource`
		implicitly. The explicit route is easy enough, though:
		"""#:
		source = Shady.Video.VideoSource( source, dims=[ sourceWidth, sourceHeight ] )
