...
def get_device_server(dev):
    return get_device_info(dev).server

def get_device_admin(dev):
    return get_device('dserver/'+get_device_server(dev))
...
def get_attribute_info(model, attribute=None):
    ...
    if isString(model):
        if attribute is None:
            model, attribute = model.strip().rsplit("/", 1)
...
def set_attribute_config(device, attribute, config, events=True, verbose=False):
    ...
    attribute = attribute.strip().rsplit('/')[-1]
...
def set_attribute_events(
    ...
    dev, attr = target.strip().rsplit("/", 1)
...
def set_attribute_label(target, label="", unit=""):
    if target.lower().strip().rsplit("/")[-1] in ("state", "status"):
        return
...
def parse_black_box_row(row):
    token = r"[0-9a-zA-Z]+"
    rip = r"(" + token + r"\." + token + r"\." + token + r"(P:" + r"\." + token + r")?)"
    pid = r"(?:PID[\ =])([0-9]+)"
...
def diff_properties(dev1, dev2, pattern="*", show = False):
    """
    Compares properties matching pattern between two devices
    """
    props1 = fn.tango.get_matching_device_properties(dev1, pattern)
    props2 = fn.tango.get_matching_device_properties(dev2, pattern)
    keys = sorted(set(k for l in (props1, props2) for k in l))
    diffs = fn.defaultdict(dict)
    for k in keys:
        if props1.get(k) != props2.get(k):
            print('%s.%s != %s.%s' % (dev1, k, dev2, k))
            if show:
                print('%s.%s: %s' % (dev1, k, props1.get(k)))
                print('%s.%s: %s' % (dev2, k, props2.get(k)))
            diffs[k][dev1] = props1.get(k)
            diffs[k][dev2] = props2.get(k)
    return diffs
...
def check_attribute(model, attr='', readable=True, timeout=0, brief=False, 
    ...
    try:
        if isinstance(model, DeviceProxy):
            ...
            dev, attr = (model,attr) if attr else model.strip().rsplit("/", 1)
