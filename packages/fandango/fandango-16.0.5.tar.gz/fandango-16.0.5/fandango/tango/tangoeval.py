...
class TangoEval(object):
    ...
    rewhat = r"(?:(?:[\.])(?P<what>"+what+"))?"
    reattr = r"(?:(?:[/])(?P<attribute>(?:" + wdot + "))" + rewhat + ")?"
    retango = r'(?:' + reheader + redev + reattr + ')' # +'(?!/)'
    retango = r'(?:' + redev + reattr + ')' # +'(?!/)'
    ...
    def find_all(self, regexp, text, as_dict=False):
        """
        This method will return all appearances of regexp in text as groupdicts.
        It doesn't use re.findall as we have to skip the last matching character of each appearance to skip quotes.
        """
        matches = []
        ...
        while pos < len(text):
            ...
            # Move past the second-to-last character of the match to avoid re-matching
            pos = match.start() + max(1, len(match.group(0)) - 1)
        self.trace() and self.trace('find_all:'+str(matches))
        return matches
    ...
    def parse_variables(self, formula, _locals=None, parsed=False, asDict=False):
        ...
        variables = [s for s in self.find_all(self.regexp, formula, True)]
        self.trace("parse_variables(...): %s" % (variables))
        if not asDict:
            variables = [
                (m.get("device", ""), m.get("attribute", ""), m.get("what", ""))
                for m in variables
            ]
        return variables

    def read_attribute(self, device, attribute="", what="",
                       _raise=True, timeout=None):
        """
        ...
        try:
            if aname not in self.attributes:
                ...
                if dp is None:
                    raise Exception(
                        'TangoEval_AttributeDoesntExist_%s'%attribute)
                try:
    ...
    def eval(self, formula=None, previous=None, _locals=None, _raise=FAILED_VALUE):
        ...
        variables = self.parse_variables(
            self.formula, parsed=True, asDict=True
        ...
        targets = [
            (
                device + (attribute and "/%s" % attribute)
                + (".%s" % what if what else ''),
                device,
                attribute,
                what,
            )
            for device, attribute, what in
            [ (('{}/'.format(m['host']) if 'host' in m else '')+m['device'],
               m.get('attribute') or '',
               m.get('what') or '')
               for m in variables]
        ]
