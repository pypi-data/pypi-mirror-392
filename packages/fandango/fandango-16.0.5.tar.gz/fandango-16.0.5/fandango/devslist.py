...
class DeviceList(dict):
    ...
    * __contains__( self, item)
    ...
    def __init__(self, _dict={}):
        ...
        self.check_dp_thread = threading.Thread(
            None, self.check_proxies, "check_proxies"
        )
    ...
    def append(self, value):
        if isinstance(value, PyTango.DeviceImpl):
            key = value.get_name()
        elif isinstance(value, PyTango.DeviceProxy):
            key = value.name()
    ...
    def isAlive(self):
        return self.check_dp_thread.isAlive()
            ...
            for dname, idev in self.items():
                ...
                if self[dname].State == PyTango.DevState.UNKNOWN:
                    ...
                    try:
                        ...
                        idev.dp_errors = 0
                        idev.last_time = fn.now()
                        print(
                            "In DevsList.check_proxies: proxy to "
                            + self.ParentName
                            + " device initialized."
                        )
                    except Exception as e:
                        print(
                            "EXCEPTION in DevsList.check_proxies:, %s Proxy Initialization failed!"
                            % dname,
                            getLastException(),
                        )
                    ...
                    try:
                        ...
                        print(
                            "EXCEPTION in DevsList.check_proxies, Ping to device ",
                            self.ParentName,
                            " failed!: ",
                            getLastException(),
                        )
                ...
                if all([v.State == PyTango.DevState.UNKNOWN for v in list(self.values())]):
...
class innerDevice(objects.Object):
    def __init__(self, device):
        ...
        self.subscribers = (
            []
        )  # This will be a list of objects, each being a device, a threading.Event or a method to be called
        self.last_comm = ""
        self.last_time = 0.0  # Last succesful communication
        self.last_retry = 0.0  # Last communication retry
