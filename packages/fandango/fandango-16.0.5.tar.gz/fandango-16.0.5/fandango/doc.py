...
def generate_rest_files(module, path="source"):
    ...
    if fandango.isString(module):
        module = fandango.loadModule(module)
    submodules = [
        (o, v)
        for o, v in list(vars(module).items())
    ...
    for o, v in submodules:
        ...
        if not os.path.isfile(filename):
            ...
            open(filename, "w").write(
                DEFAULT_MODULE % (v.__name__, "=" * len(v.__name__), v.__name__)
            )
...
def get_vars_docs(module_name, module_vars, title="Variables", subtitle=True):
    defs = [
        (get_rest_title(f, *TM3) if subtitle else "")
        + DEFAULT_VARIABLE % (module_name, f)
...
def get_function_docs(module_name, module_vars, title="Functions", subtitle=True):
    functions = [
        (f, v)
        for f, v in list(module_vars.items())
        if inspect.isfunction(v) and v.__module__ == module_name
    ]
    defs = [
        (get_rest_title(f[0], *TM3) if subtitle else "")
        + DEFAULT_FUNCTION % (module_name, f[0])
        for f in functions
    ]
    if defs:
        return "\n" + get_rest_title(title, *TM2) + "\n".join(defs)
...
def get_autodoc(
    module_name, module_scope, module_vars=[], module_doc="", module_postdoc="\n----\n"
):
    """
...
def get_fn_autodoc(
    ...
    try:
        ...
        if not any(c in module_doc for c in ("contents::", "toctree::")):
            module_doc = ".. contents::\n\n" + module_doc
        module_doc = get_autodoc(
            module_name, module_scope, module_vars, module_doc, module_postdoc=post
        )
    except:
        print("failed to update %s.__doc__" % module_name)
    return module_doc
