...
class EventListener(Logger, Object):  # Logger,
    """
    Implementation of a Listener to events coming from Tango, it connects
    to an EventSource (e.g. a Tango Attribute) and executes its callbacks.
    A single EventSource may redirect to multiple listeners,
    and a listener can receive events from multiple sources.
...
class BufferedEventListener(EventListener, ThreadedObject):
    ...
    def storeEvent(self, src, type_, value):
        ...
        if not self.get_started() or self.queue.qsize() >= self.size:
            self.start() #triggers thread process
    ...
    def process(self):
        ...
        while True:
            try:
                ...
                break
        if self.lasts:
            self.buffer = self.buffer[-(self.size-len(self.lasts)):]
            self.buffer.extend(self.lasts)
...
class EventSource(Logger, SingletonMap):
    ...
    def __init__(self, name, keeptime=1000.0, fake=False, parent=None, timeout_ms=3000, \
            **kw):
        """
        **kw used to keep compatibility with old api
        ...
        elif self.use_events and not isIterable(self.use_events):
            self.use_events = [self.use_events]
        ...
        try:
            if timeout_ms and check_device(self.proxy):
                self.proxy.set_timeout_millis(timeout_ms)
        except:
            self.warning('Unable to %s.set_timeout_millis(%s)'
                         % (self.normal_name, timeout_ms))
    ...
    def subscribeEvents(self, types=None, asynchronous=True):
        try:
            ...
            if not isIterable(self.use_events):
                self.use_events = [self.use_events] if self.use_events else []
...
class EventCounter:
    def __init__(self, models=[], timeout=10., event_type=EventType.CHANGE_EVENT):
        self.event_t0s = dict()
        self.event_t1s = dict()
        self.event_ids = dict()
        self.proxies = dict()
    ...
    def setModels(self, models):
        for m in models:
            ...
            print(m)
            dev, attr = m.rsplit('/',1)
            if m not in self.proxies:
                self.proxies[m] = get_device(m)
            if m not in self.event_ids:
                try:
                    self.event_ids[m] = self.proxies[m].subscribe_event(
                        attr,self.event_type,self,[])
                except Exception as e:
                    print(m,'failed to subscribe')
                    traceback.print_exc()
                    self.event_counts[m] = 0

    def push_event(self, event):
        m = event.attr_name
        m = '/'.join(m.split('/')[-4:])
        if m not in self.event_t0s:
            self.event_t0s[m] = now()
        self.event_t1s[m] = now()
        self.event_counts[m] += 1
        if now() > (self.event_t0s[m]+self.timeout):
            self.unsuscribe_event(m)

    def unsuscribe_event(self, model):
        if self.event_counts[model] > 1:
            freq = self.event_counts[model] / (
                self.event_t1s[model] - self.event_t0s[model])
        else:
            freq = self.event_counts[model]
        print(model,'unsubscribed',self.event_counts[model],
              'events', freq,'Hz')
        self.proxies[model].unsubscribe_event(self.event_ids[model])
        self.event_ids.pop(model, None)

    def unsubscribe_all(self):
        """
        Unsubscribe all events and print statistics
        """
        for m in list(self.event_ids.keys())[:]:
            self.unsuscribe_event(m)
