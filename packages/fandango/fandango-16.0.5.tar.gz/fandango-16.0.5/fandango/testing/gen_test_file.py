...
try:
    ...
    if not os.path.exists(fname):
        ...
        docs.update(
            (k, getattr(v, "__doc__", "")) for k, v in list(mod.__dict__.items())
        )
    ...
    decs = []
    for l in lines:
        if fandango.clmatch(r"^(def|class)[\ ].*", l):
    ...
    if action == "generate":
        ...
        print("%d test cases found: %s" % (len(tests), list(tests.keys())))
        for l in decs:
            ...
            outs.append(
                "    #assert %s.%s" % (fname.replace("/", ".").replace(".py", ""), l)
            )
            outs.append("")
        ...
        print("\n coverage = %d / %d = %f" % (len(decs), done, float(len(decs)) / done))
except:
    traceback.print_exc()
    print(__doc__)
