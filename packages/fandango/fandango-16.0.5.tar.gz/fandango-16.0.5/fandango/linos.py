...
def shell_command(*commands, **keywords):
    ...
    @param strict: do not return result if rcode != 0
    @return the last stdout result"""
    if not commands:
        return
    elif isinstance(commands, str):
        commands = [commands]
    split = keywords.get("split", False)
    strict = keywords.get("strict", False)
    ...
    if not result or (strict and rcode != 0):
        return
    elif split:
...
def sendmail(subject, text, receivers, sender="", attachments=None, 
    ...
    chars = sorted(set(re.findall(r"[^a-zA-Z0-9/\ \n\.\=\(\),\[\]_\-]", text)))
    for c in chars:
...
def get_cpu(pid):
    ...
    return float(shell_command("ps h -p %d -o pcpu" % pid))
    ...
    for f in sorted(fs):
        if is_dir("%s/%s" % (origin, f)):
            ...
            if not overwrite and file_exists("%s/%s" % (destination, f)):
                ...
                continue
            size, t0 = get_file_size("%s/%s" % (origin, f)), time.time()
...
def diffdir(origin, destination, caseless=False, checksize=True):
    ...
    for f in sorted(fs):
        ...
        if not (f in nfs) and (not caseless or not f.lower() in lnfs):
            ...
            if get_file_size(df) != get_file_size("%s/%s" % (destination, f)):
                print("---> %s and %s differs!" % (df, "%s/%s" % (destination, f)))
                missing.append(df)
    for n in sorted(nfs):
...
def findfolders(target="", parent="", filter_=True, printout=False):
    ...
    def get_folders(path):
        folders = [
            "%s/%s" % (path, f)
            for f in filter_folders(listdir(path, folders=True))
...
def ping(ips, threaded=False, timeout=1):
    """
    if threaded = False, it will try to return ping in ms

    ips =  ['ivc%02d01'%(i+1) for i in range(16)]
    #ips = ["10.0.1.1", "10.0.1.3", "10.0.1.11", "10.0.1.51"]

    
    Based on code By Noah Gift's, PyCon 2008
    """
    import subprocess
    rtype = type(ips)
    ...
    def _ping(ip):
        r = shell_command("ping -c 1 -w %d %s" 
                % (int(max((1, timeout))), ip),
            strict=True, split=True)
        if not r:
            return False
        else:
            m = fun.clsearch(fun.re_float, r[1].split('time=')[-1])
            if m:
    ...
    if not threaded:
        results = dict((ip, _ping(ip)) for ip in ips)
        ...
        ## WARNING ... THIS IMPLEMENTATION OF THE THREAD IS GIVING 
        # A LOT OF PROBLEMS DUE TO THREAD NOT CLOSING! (waiting at q.get())
        def pinger(i, q, r):
            """Pings subnet"""
