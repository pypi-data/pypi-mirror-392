...
def find_module(module):
    ...
    if py3:
        try:
            ...
            path = spec.origin
            if path == 'built-in':
                path = sorted(p for p in sys.path if p.startswith('/usr/lib')
                                and 'dist-packages' in p)[0]
            if path.split('.')[-1] in ('py','pyc','wheel','so','whl'):
        ...
        if "." not in module:
            try:
                ...
                try:
                    dp = sorted(p for p in sys.path if p.startswith('/usr/lib')
                                and 'dist-packages' in p)[0]
                    r = imp.find_module(module,[dp])[1]
                except:
                    traceback.print_exc()
                    raise e
            return r
...
def load_module(module, modulename=None, exec_module=True, pythonpath=''):
    """
    ...
    is_path = '/' in module or '.py' in module
    if not modulename:
        modulename = module.split("/")[-1].split(".py")[0]
        modulename = replaceCl(r"[-\.]", "_", modulename)
    ...
    try:
        import imp
        fl.tracer(level=4) and fl.trace('load_module(): using py2 imp',4)
        if is_path:
            fl.tracer(level=4) and fl.trace(
                'load_module(): load_source(%s,%s)' % (modulename,module),4)
            return imp.load_source(modulename, module)
        elif "." not in module:
            return imp.load_module(module, *imp.find_module(module))
        else:
            parent, child = module.rsplit(".", 1)
            mparent = load_module(parent)
            args = imp.find_module(child, mparent.__path__)
            mchild = imp.load_module(module, *args)
            return mchild

    except Exception as e:
        fl.tracer(level=4) and fl.trace(fl.except2str())
        if not py3:
            raise e
        else:
            """
            Still strugging to achieve loadModule(path_to_lib) using importlib
            It can load only modules already available in sys.path

            import importlib
            p = '/home/srubio/LIBS/python/fandango/objects.py'
            spec = importlib.util.spec_from_file_location('fandango.objects',p)
            m = importlib.util.module_from_spec(spec)
            fo = importlib.import_module('fandango.objects',m)
            # fails, cannot find fandango module

            spec.loader.exec_module?
            mm =  spec.loader.exec_module(m)

            package = m.__package__
            path = m.__file__
            package_path = '/home/srubio/LIBS/python/' + package

            In [21]: spec = importlib.util.spec_from_file_location('fandango','/home/srubio/LIBS/python/fandango/')
            In [22]: spec
            In [23]: spec = importlib.util.spec_from_file_location('fandango','/home/srubio/LIBS/python/fandango/__init__.py')
            In [24]: spec
            Out[24]: ModuleSpec(name='fandango', loader=<_frozen_importlib_external.SourceFileLoader object at 0x7fe74af81a10>, origin='/home/srubio/LIBS/python/fandango/__init__.py', submodule_search_locations=['/home/srubio/LIBS/python/fandango'])
            """
            fl.tracer(level=4) and fl.trace('load_module(): using py3 importlib',4)
            import importlib
            if is_path:
            ...
            if exec_module:
                try:
                    ...
                    #Module loaded, but not executable, usual with submodules
                    #Silent fail ...
                    pass
            return m
...
class Struct(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'default' in kwargs:
            ...
            self.__use_default = self.__default = None
    ...
    def keys(self):
        return list(k for k in self.__dict__.keys()
                    if not k.startswith('__')
                    and not k.startswith('_Struct__'))

    def values(self):
        return list(self.__dict__[k] for k in self.keys())

    def items(self):
        return list((k,self.__dict__[k]) for k in self.keys())
    ...
    def get(self, k, default=None):
        try:  # Some keys may raise exception
            default = default if default is not None else self.__default
    ...
    def setdefault(self, v):
        self.__default = v
        self.__use_default = v is not None
    ...
    def __getitem__(self, k):
        if self.__use_default and not self.has_key(k):
            return self.__default
...
class ValuedMethod(Decorator):
    """
        This method can be used to disable a logger inline
        self.log = ValuedMethod(self.log, self.logLevel < 20)

        then, calling log this way is possible:
        self.log and self.log('message')
    """
...
class Cached(Decorator):
    ...
    It requires arguments to be Hashable in order to compare matches
    """
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.func is None:
            # Deferred decorator
            self.log and self._log("Cached(): decorating %s" % str(args[0]))
    ...
    def _log(self, msg, force=False):
        if isCallable(self.log):
            ...
            print('Cached(%s): %s' % (self.func, msg))
    ...
    def prune(self, expire=None, depth=None):
        try:
            ...
            cache = sorted((v,k) for k,v in self._expires.items()
                    if v > expire and k in self.cache)
            if len(cache) != len(self.cache) or len(cache) > self.depth:
                self.log and self._log("pruning: %s => %s" % (len(self.cache), len(cache)))
                pass

            self.cache = {k:self.cache[k] for v,k in cache[-self.depth :]}
            self._expires = {k:v for k,v  in self._expires.items() if k in self.cache}
            return list(self.cache.keys()) #sorted may fail!
        finally:
            # self.lock.release()
            pass
    ...
    def execute(self, *args, **kwargs):
        ...
        try:
            if self._obj_parent is not None and args[0] is self._obj_parent:
                kargs = list(args)
                kargs[0] = id(args[0])
            else:
                kargs = args

            #key = time.time(), tuple(kargs), tuple(kwargs.items())
            key = tuple(kargs), tuple(kwargs.items())
            if not isHashable(key):
                raise Exception()
        except:
            self.log and self._log(traceback.format_exc())
            self.log and self._log("Cached: unhashable arguments!", force=True)
            expire = 0
        ...
        if not self.depth or not expire:
            ...
            t2 = time.time()
            # self._log("Cached.check: check %s in [%d] (%f secs) %s" %
            #     (key, len(cache), t2-t0, self._obj_parent), force = ((t2-t0)>1e-3))
            v = self.cache.get(key,None)

            if v:
                t3 = time.time()
                # self._log("(%s) was in cache ... (%f secs)" % (
                #     key, t3-t0), force = ((t3-t0)>1e-3))
            else:
                t3 = time.time()
                # self._log("Cached.check: %s NOT IN [%d] (%f secs) %s" %
                #     (key, len(cache), t3-t0, self._obj_parent), force = ((t3-t0)>1e-3))
                try:
                    # self._log("%s(%s,%s) = ..." % (self.func, args, kwargs))
                    v = self.func(*args, **kwargs)
                except Exception as e:
                    self.log and self._log(traceback.format_exc(),True)
                    v = e
