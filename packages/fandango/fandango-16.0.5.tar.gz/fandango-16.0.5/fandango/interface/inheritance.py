...
def updateChildClassDicts(Child, Parent, Exclude=[]):
    #print('%s(%s)'%(Child.__name__,Parent.__name__))
    for attribute in dir(Child):
        ...
        if (
            ...
            if isinstance(pattr, dict):
                #print('%s.%s.update(%s)'%(Child.__name__,attribute,Parent.__name__))
                cattr.update(
                    (k, v)
...
def FullTangoInheritance(
    ...
    """
    print('FullTangoInheritance(%s,%s,%s): hard inheritance deprecated in T9, calling updateChildClassDicts instead'
          % (name, childClass, parentClass))
    updateChildClassDicts(childClass,parentClass,Exclude)
