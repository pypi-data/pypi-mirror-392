...
def get_cmd_descriptions(device):
    return dict(
        (c.cmd_name, c) for c in fandango.get_device(device).command_list_query()
    )
...
def copy_cmd_list(device):
    ...
    for c, obj in list(cmd_objs.items()):
        cmd_list[c] = [
            [obj.in_type, obj.in_type_desc],
            [obj.out_type, obj.out_type_desc],
        ]
    return cmd_list
...
def copy_attr_list(device):
    ...
    for a, obj in list(attr_objs.items()):
        attr_list[a] = [
            [PyTango.CmdArgType.values[obj.data_type], obj.data_format, obj.writable]
        ]
        if obj.data_format == PyTango.SPECTRUM:
            attr_list[a][0].append(obj.max_dim_x)
        if obj.data_format == PyTango.IMAGE:
            attr_list[a][0].append(obj.max_dim_y)
    return attr_list
...
class Doppelganger(fandango.SingletonMap):
    ...
    def __call__(self, argin=None):
        if self.mode in (self.BYPASS, self.RECORD):
            r = self.proxy.command_inout(
                *([self.command, argin] if argin is not None else [self.command])
            )
...
class CopyCatDS(DynamicDS):
    ...
    def init_device(self):
        ...
        )  # Needed to do it twice to generate StaticAttributes properly
        fandango.tango.put_device_property(
            self.get_name(),
            dict((k, getattr(self, k)) for k in CopyCatDSClass.device_property_list),
            db=self.db,
        )

        self.setLogLevel(getattr(self,'LogLevel','INFO'))
            ...
            for a, t in sorted(copy_attr_list(self.TargetDevice).items()):
                if any(fandango.matchCl(e, a) for e in self.CopyAttributes):
                    ...
                    if not isTypeSupported(ttype):
                        self.warning(
                            "%s attribute of type %s is not supported by DynamicDS"
                            % (a, ttype.name)
                        )
                    else:
                        dims = len(t[0][3:])
                        if dims == 2:
                            self.warning(
                                "%s attribute of type IMAGE is not supported by DynamicDS"
                                % (a)
                            )
                        else:
                            ttype = (
                                ttype.name
                                if not dims
                                else ttype.name.replace("Dev", "DevVar") + "Array"
                            )
                            if self.ReadOnly or writable == PyTango.AttrWriteType.READ:
                                ...
                                formula = (
                                    "%s=%s(QATTR('%s') if not WRITE else WATTR('%s',VALUE))"
                                    % (
                                        a,
                                        ttype,
                                        self.TargetDevice + "/" + a,
...
class CopyCatServer(DynamicServer):
    ...
    def load_class(self, c):
        if c.endswith("_Copy"):
            ...
            DynamicServer.load_class(self, c)
    ...
    def main(self, class_override=False):
        ...
        targets = dict(
            (d, fandango.tango.get_device_property(d, "TargetDevice")) for d in doppels
        )
        classes = {}
        ...
        if class_override:
            for t in set(targets.values()):
                if t:
                    classes[t] = choose_db(t, self.db).get_class_for_device(
                        t if ":" not in t else t.split("/", 1)[-1]
                    )
            print(
                "Devices: \n%s"
                % "\n".join(
                    sorted(
                        "%s = %s(%s)" % (d, t, classes.get(t, None))
                        for d, t in list(targets.items())
                    )
                )
            )
        ...
        if class_override and classes:
            ...
            for d in targets:
                fandango.tango.add_new_device(
                    self.name, classes[targets[d]] + "_Copy", d
                )
...
def __test_CopyCatDS(target="sys/tg_test/1"):
    ...
    PyTango.Database().put_device_property(
        "test/copycatds/01", {"TargetDevice": [target]}
    )
