...
class DatabaseConf():
    def __init__(self,dbname='maxbus'):
        ...
        # 只有 mssql 才需要指定pylib, 可以指定为 pymssql 或 pyodbc
        self._dbpylib = sc.get_keyvalue('%s_pylib' % dbname,consul=sc.consul)
        self._dbdriver = f"mssql+{self._dbpylib or 'pymssql'}" if self._dbtype == 'mssql' else 'mysql+mysqldb'
    ...
    def sqlalchemy_database_uri(self):
        ...
        return f'{self._dbdriver}://{self._user}:{self._password}@{self._host}:{self._port}/{self._name}{"?driver=SQL+Server" if self._dbpylib=="pyodbc" and self._dbtype == "mssql" else ""}'
