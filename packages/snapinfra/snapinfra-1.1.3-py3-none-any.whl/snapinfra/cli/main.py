...
async def comprehensive_generate_command(
    ...
    try:
        ...
        # Create diagrams directory first (always, even if generation fails)
        from pathlib import Path as _Path
        output_path = _Path(output_dir)
        diagrams_dir = output_path / "diagrams"
        diagrams_dir.mkdir(parents=True, exist_ok=True)
        
        # Generate Architecture as Code diagrams first
        diagram_data = None
        try:
            if not quiet:
                console.print("\nGenerating Architecture as Code diagrams...")
            diagram_data = await chat._generate_architecture_diagram(project_plan, user_prompt)
            if diagram_data:
                # Save diagrams to output directory
                
                # Save diagram files if available
                if 'python_code' in diagram_data:
                    python_file = diagrams_dir / "architecture.py"
                    python_file.write_text(diagram_data['python_code'], encoding='utf-8')
                    if not quiet:
                ...
                if 'mermaid_code' in diagram_data:
                    ...
                    mermaid_file.write_text(diagram_data['mermaid_code'], encoding='utf-8')
                    if not quiet:
                ...
                if 'd2_code' in diagram_data:
                    ...
                    d2_file.write_text(diagram_data['d2_code'], encoding='utf-8')
                    if not quiet:
            ...
            # Continue with project generation even if diagrams fail
...
try:
    from .diagram import add_diagram_commands
