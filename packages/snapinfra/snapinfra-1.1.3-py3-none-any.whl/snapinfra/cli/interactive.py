...
"""
            ...
            # Count diagrams that will be saved (but don't save here - let caller decide location)
            diagrams_saved = 0
            if python_code:
                diagrams_saved += 1
            if mermaid_code:
            ...
            if diagrams_saved > 0:
                console.print(f"[bold green]Architecture as Code generated successfully![/bold green]")
                console.print(f"Generated {diagrams_saved} diagram format(s)")

            # Return diagram data for compatibility (caller will save to appropriate location)
            return {
...
"""
    ...
    async def _open_diagram_viewer(self, diagram_data: Dict):
        ...
        try:
            from ..diagram.viewer_server import start_viewer_async, DiagramData as ViewerDiagramData
            import uuid
            import webbrowser
            import asyncio
            ...
            if viewer_diagrams:
                ...
                console.print(f"[cyan]Starting interactive viewer on http://localhost:{port}...[/cyan]")
                ...
                console.print("Press Ctrl+C to stop the viewer\n")
                
                # Start the async viewer server
                await start_viewer_async(viewer_diagrams, port=port)
            else:
            ...
            try:
                ...
                if await multi_agent.initialize():
                    ...
                    if project_result and project_result.get('generated_files'):
                        # Generate Architecture as Code diagrams
                        import os
                        diagrams_dir = os.path.join(project_dir, "diagrams")
                        os.makedirs(diagrams_dir, exist_ok=True)
                        
                        try:
                            # Use the original project plan from the result (which contains the file list)
                            project_plan = {
                                'name': project_name,
                                'tech_stack': project_result.get('tech_stack', []),
                                'project_type': project_result.get('project_type', 'application'),
                                'files': project_result.get('files', []),  # Original file list from plan
                                'database': project_result.get('database', 'none'),
                                'folder_structure': project_result.get('folder_structure', [])
                            }
                            
                            console.print("\nGenerating Architecture as Code diagrams...", style="bold cyan")
                            diagram_data = await self._generate_architecture_diagram(project_plan, description)
                            
                            if diagram_data:
                                # Save diagrams to project directory
                                if 'python_code' in diagram_data and diagram_data['python_code']:
                                    python_file = os.path.join(diagrams_dir, "architecture.py")
                                    with open(python_file, 'w', encoding='utf-8') as f:
                                        f.write(diagram_data['python_code'])
                                    console.print(f"  Saved Python diagram: diagrams/architecture.py", style="green")
                                
                                if 'mermaid_code' in diagram_data and diagram_data['mermaid_code']:
                                    mermaid_file = os.path.join(diagrams_dir, "architecture.mmd")
                                    with open(mermaid_file, 'w', encoding='utf-8') as f:
                                        f.write(diagram_data['mermaid_code'])
                                    console.print(f"  Saved Mermaid diagram: diagrams/architecture.mmd", style="green")
                                
                                if 'd2_code' in diagram_data and diagram_data['d2_code']:
                                    d2_file = os.path.join(diagrams_dir, "architecture.d2")
                                    with open(d2_file, 'w', encoding='utf-8') as f:
                                        f.write(diagram_data['d2_code'])
                                    console.print(f"  Saved D2 diagram: diagrams/architecture.d2", style="green")
                                
                                # Ask if user wants to open the diagram viewer
                                from rich.prompt import Confirm
                                if Confirm.ask("\nOpen diagrams in interactive viewer?", default=False):
                                    try:
                                        await self._open_diagram_viewer(diagram_data)
                                    except Exception as e:
                                        console.print(f"Failed to open viewer: {e}", style="yellow")
                            else:
                                console.print("  No diagram data generated (Groq may not be configured)", style="yellow")
                        except Exception as e:
                            console.print(f"Architecture diagram generation failed: {e}", style="yellow")
                            import traceback
                            traceback.print_exc()
                            # Continue with file generation even if diagrams fail
                        
                        await self._write_multi_agent_files(project_result, project_dir)
                        multi_agent.cleanup()
                        return
                    else:
                        console.print("Multi-agent generation failed", style="red")
                        multi_agent.cleanup()
                        return
                
            except Exception as e:
                console.print(f"Multi-agent system error: {e}", style="red")
                return
