...
class Builder(object):
    ...
    def __init__(self, data, local):
        ...
        self.tmp = None
        if local:
            now = data.get("outputDir", "cases/" + datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))
...
""")
    ...
    def openV2Sim(self):
        ...
        subprocess.Popen([sys.executable, "gui_main.py", "-d", os.path.dirname(self.files["config"])])
