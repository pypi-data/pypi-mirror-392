...
async def pipeline_progress_by_channel(request: Request) -> JSONResponse:
    ...
    # NOTE: Dectris specific: raw_frame channel progress can be accessed only
    # via AcquisitionSystem.
    # TODO(mdu): should be factored to detector-specific logic along with
    # associated code in acqsys.prepare(), pipeline.prepare().
    if channel == "raw_frame":
        return JSONResponse((await lima2.nb_frames_xferred()).sum)

    pipeline = await lima2.get_pipeline(uuid=uuid)

    pcounter = await pipeline.channel_progress(channel=channel)

    return JSONResponse(pcounter.sum)
