...
def rc_jsonrpc(method: str, args: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(f"Respuesta JSON inválida: {resp.text[:500]}") from e
    
    # Validar que data sea un dict
    if not isinstance(data, dict):
        raise RuntimeError(f"Respuesta RPC inválida (esperado dict, obtenido {type(data).__name__}): {str(data)[:500]}")
    
    # Manejo de errores JSON-RPC
    if data.get("error"):
        err = data["error"]
        # Asegurar que err es un dict
    ...
    # Retornar result si existe, si no retornar data completo
...
def get_recent_changesets(
    ...
    try:
        ...
        try:
            ...
            return {
                "error": f"Error al obtener información del repositorio: {str(e)}",
                "changesets": []
            }
        
        # Validar que repo_result sea un dict
        if not isinstance(repo_result, dict):
            return {
                "error": f"Respuesta inesperada del servidor (esperado dict, obtenido {type(repo_result).__name__}): {str(repo_result)[:200]}",
                "changesets": []
            }
        ...
        try:
            ...
            return {
                "error": f"Error al obtener changesets: {str(e)}",
                "changesets": [],
                "metadata": {
