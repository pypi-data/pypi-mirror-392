...
class Formatter:
    def __format_default(self, data, width, h):
        ...
        for row in data:
            i = 0
            for word in row:
                text += f"| {word}" + (" " * (width[i] - len(word) + 1))
                i += 1
            text += "|\n"
            if header:
                header = False
...
def format(data, hasHeader, border: Border = Border.DEFAULT):
    ...
    match border:
        case Border.DEFAULT:
            return f._Formatter__format_default(new_data, max_width, hasHeader)
        case Border.TABULAR:
            return f._Formatter__format_tabular(new_data, max_width, hasHeader)
        case Border.TABULAR_ROUNDED:
            return f._Formatter__format_tabular_rounded(new_data, max_width, hasHeader)
        case Border.TABULAR_DOUBLE:
            return f._Formatter__format_tabular_double(new_data, max_width, hasHeader)
        case Border.NO_BORDER:
            if hasHeader:
                raise TypeError("In No Border Mode, Data Cannot Have Header")
            return f._Formatter__format_no_border(new_data, max_width)
