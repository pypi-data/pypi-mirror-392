...
class Gamma:
    ...
    # ------------------------------------------------------------
    # Métodos vectorizados — pickeables y sin lambdas anidadas
    # ------------------------------------------------------------
    def pdf_func(self, x, a, b):
        return gamma_dist.pdf(x, a, scale=1 / b)
        # x = np.asarray(x, dtype=float)
        # return np.array([float(self.raw_pdf(xv, a, b)) for xv in x])

    def sf_func(self, x, a, b):
        return gamma_dist.sf(x, a, scale=1 / b)
        # x = np.asarray(x, dtype=sfloat)
    ...
    def __getstate__(self):
        ...
        # Eliminar funciones lambdify (no pickeables)
