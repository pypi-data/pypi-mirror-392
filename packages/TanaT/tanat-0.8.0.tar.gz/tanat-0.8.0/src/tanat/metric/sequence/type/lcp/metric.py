...
from .utils import _process_lcp_pair
from .settings import LCPSequenceMetricSettings
...
class LCPSequenceMetric(SequenceMetric, register_name="lcp"):
    ...
    This similarity measure ($\\mathcal{A}$) computes the length of the longest common prefix
    ...
    * normalized harmonic distance: $$d(x,y) = \\frac{\\mathcal{A}}{\\sqrt{|x|\\times |y|}}$$
    * non-normalized additive distance: $$d(x,y) = |x|+|y| - 2 \\times \\mathcal{A}$$
    ...
    def __call__(self, seq_a, seq_b, **kwargs):
        ...
        with self.with_tmp_settings(**kwargs):  # temporarily override settings
            _, lcp, len_a, len_b = _process_lcp_pair(
                (seq_a.to_numpy(), seq_b.to_numpy(), None)
            )
            return self._compute_metric(lcp, len_a, len_b)
    ...
    def _collect_serial(self, sequence_pool, pbar):
        for pairs_chunk in chunks_from_generator(
            ...
            for prepared_pair in prepared_pairs:
                pair_ids, lcp, len_a, len_b = _process_lcp_pair(prepared_pair)
                pbar.update(1)
                yield pair_ids, self._compute_metric(lcp, len_a, len_b)
        ...
        with multiprocessing.Pool(self.settings.max_workers) as pool:
            try:
                for pairs_chunk in chunks_from_generator(
                    ...
                    prepared_pairs = self._prepare_pairs_with_arrays(pairs_chunk)
                    for pair_ids, lcp, len_a, len_b in pool.imap(
                        _process_lcp_pair, prepared_pairs
                    ):
                        pbar.update(1)
                        yield pair_ids, self._compute_metric(lcp, len_a, len_b)
            finally:
    ...
    def _compute_metric(self, lcp_length, len_a, len_b):
        if self.settings.as_dist:
            if self.settings.norm:
                return 1.0 - float(lcp_length) / np.sqrt(len_a * len_b)
            return float(len_a + len_b - 2 * lcp_length)
        return float(lcp_length)
