...
def dht(my_board, pin, callback):
    ...
    my_board.set_pin_mode_digital_input(pin_number=2, callback=the_callback)
    my_board.set_pin_mode_dht(pin, dht_type=22, callback=the_callback())
    # wait forever
    while True:
        try:
            time.sleep(.01)
...
try:
    # board.set_pin_mode_dht(8, the_callback)
    board.set_pin_mode_dht(4, the_callback)
    # board.set_pin_mode_dht(10, the_callback)
    # board.set_pin_mode_dht(11, the_callback)
    # board.shutdown()
    while True:
        time.sleep(.1)
