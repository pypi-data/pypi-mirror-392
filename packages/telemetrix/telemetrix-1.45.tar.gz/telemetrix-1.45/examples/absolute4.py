...
def step_absolute(the_board):
    ...
    motor2 = the_board.set_pin_mode_stepper(interface=1, pin1=PULSE_PIN2,
                                           pin2=DIRECTION_PIN2)
    ...
    the_board.stepper_set_current_position(motor, 0)
    the_board.stepper_set_max_speed(motor, 400)
    the_board.stepper_set_acceleration(motor, 800)

    the_board.stepper_set_current_position(motor2, 0)
    the_board.stepper_set_max_speed(motor2, 400)
    the_board.stepper_set_acceleration(motor2, 800)

    # set the absolute position in steps
    the_board.stepper_move_to(motor, -32769)
    the_board.stepper_move_to(motor2, -32769)
    ...
    time.sleep(.2)
    the_board.stepper_run(motor2, completion_callback=the_callback)


    # wait for first operation to complete for both motors
    while exit_flag < 1:
        time.sleep(.2)
    ...
    the_board.stepper_move_to(motor2, 2000)
    ...
    # keep application running until 2nd motor operation is complete
    while exit_flag < 3:
        try:
            time.sleep(.2)
