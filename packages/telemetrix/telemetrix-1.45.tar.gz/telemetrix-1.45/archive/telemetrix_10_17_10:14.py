...
class Telemetrix(threading.Thread):
    ...
    def _i2c_read_report(self, data):
        ...
        for i in range(count):
            ...
            data.append(x)
        cb_list = [PrivateConstants.I2C_READ_REPORT, data[1], data[2], data[0], data[3]]
        for i in range(number_of_data_bytes):
            cb_list.append(data[i + 4])
    ...
    def _i_am_here(self, data):
        ...
        """
        print(f'iah {data}')
        self.reported_arduino_id = data[2]
    ...
    def _report_debug_data(self, data):
        ...
        print(f'DEBUG ID: {data[1]} Value: {value}')
    ...
    def _reporter(self):
        ...
        while self._is_running() and not self.shutdown_flag:
            if len(self.the_deque):
                print(len(self.the_deque))
                # get next byte from the deque and process it

                num_bytes_to_retrieve = self.the_deque.popleft()

                # print('data ', data)
                print(f'numbytes {num_bytes_to_retrieve}')
                # get the full report data
                # this list will be populated with the received data for the report
                response_data = []

                # get the full report packet
                # this will include the command as well as any data
                for i in range(num_bytes_to_retrieve):
                    while len(self.the_deque) == 0:
                        pass
                ...
                dispatch_entry = self.report_dispatch.get(response_data[0])

                # this calls the method retrieved from the dispatch table
                try:

                    method = dispatch_entry
                    print(f'data passed {response_data[1:len(response_data)]}')
                    method(response_data[1:len(response_data)])
