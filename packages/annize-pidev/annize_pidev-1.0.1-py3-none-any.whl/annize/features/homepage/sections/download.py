...
class Section(annize.features.homepage.common.HomepageSection):
    ...
    def generate_content(self, info):  # TODO make package generation in "en" culture context ?!
        ...
        if package_list:
            do_return = True
            head_str = annize.i18n.tr("an_HP_DL_PackagesAvailable")
            if dependencies:
                requirements_label = annize.i18n.tr("an_HP_DL_TheReqs")
                dependenciesref = f":ref:`{requirements_label}<hp_downl_deps>`"
                head_str += " " + annize.i18n.tr("an_HP_DL_CheckReqs").format(dependenciesref=dependenciesref)
        ...
        if dependencies:
            ...
            content.append_rst(annize.features.documentation.sphinx.rst.heading(
                annize.i18n.tr("an_HP_DL_Deps"), level=0, anchor="hp_downl_deps"))
            content.append_rst(annize.i18n.tr("an_HP_DL_Uses3rdParty").format(project_name=project_name))
    ...
    def __generate_package_list(self, info: annize.features.homepage.common.HomepageSection._GenerateInfo):
        content = ""
        file_label = annize.i18n.tr("an_HP_DL_File")
        release_time_label = annize.i18n.tr("an_HP_DL_Releasedate")
        sha256sum_label = annize.i18n.tr("an_HP_DL_Sha256sum")
        size_label = annize.i18n.tr("an_HP_DL_Size")
...
def friendly_file_size(value: int) -> str:
    unit_str = annize.i18n.tr("an_HP_DL_FriendlySize_B")
    for unit_str_ in (
