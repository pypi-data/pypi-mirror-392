...
class Required(Kind):
    ...
    def __init__(self):
        super().__init__(label=annize.i18n.TrStr.tr("an_Dep_Required"), importance=0)


class Recommended(Kind):

    def __init__(self):
        super().__init__(label=annize.i18n.TrStr.tr("an_Dep_Recommended"), importance=-200_000)


class Included(Kind):

    def __init__(self):
        super().__init__(label=annize.i18n.TrStr.tr("an_Dep_Included"), importance=-400_000)


class GnuLinux(Dependency):

    def __init__(self, *, kind=None, comment):
        super().__init__(kind=kind or Recommended(), label=annize.i18n.TrStr.tr("an_Dep_GnuLinux"), icon="linux",
                         comment=comment)
...
class _ArtworkComment(annize.i18n.TrStr):
    ...
    def get_variant(self, culture):
        ...
        if self.__license:
            ...
            full_comment.append(annize.i18n.tr("an_Dep_Artwork_License").format(license=license_))
        if self.__origin:
            full_comment.append(f"`{annize.i18n.tr("an_Dep_Artwork_Origin")} <{self.__origin}>`__")
