...
import annize.features.branding
...
class HtmlOutputGenerator(annize.features.documentation.sphinx.output.common.OutputGenerator):
    ...
    def prepare_generate(self, info):
        ...
        if theme == "pimpinella_anisum":
            ...
            base_color = annize.features.branding.brand_color()
    ...
    def multi_language_frame(self, document):
        ...
        try:
            title = annize.i18n.translate(document.title, culture=annize.i18n.unspecified_culture)
        except annize.i18n.TranslationUnavailableError:
            title = ""
        ...
        result.file.path()("index.html").write_file(
            ...
            f"    }}"
            f"    if (c && c.includes('_'))"
            f"        return tryCulture(c.split('_')[0]);"
            f"}};"
            f"{f"tryCulture('{result.culture_names[0]}') ||" if len(result.culture_names) == 1 else ""}"
            f"tryCulture(myLanguage) || tryCulture('en_US') || tryCulture('');"
            f"</script>"
            f"</head>"
            f"<body>"
            f"<h1>🗣 🌐 ❓</h1>"
