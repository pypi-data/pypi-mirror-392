        ...
        try:
            with flow_builder_tracing_context(
                ...
                if current_flow:
                    ...
                    files_state["flow.json"] = json.dumps(simplified_flow, indent=2)
                ...
                async for stream_mode, chunk in agent.astream(
                    ...
                    if stream_mode == "messages":
                        ...
                        for node_name, node_output in chunk.items():
                            # Extract structured state updates from node output
                            state_updates = {
                                k: v for k, v in node_output.items() if k != "messages"
                            }

                            # Check for flow updates and stream them to frontend
                            if (
                                "files" in state_updates
                                and "flow.json" in state_updates["files"]
                            ):
                                try:
                                    # Parse agent's simplified flow and send to frontend
                                    # Frontend will merge with its own UI state
                                    agent_updated_flow = json.loads(
                                        state_updates["files"]["flow.json"]
                                    )
                        ...
                        yield create_stream_response(
                            event_type="custom", data=chunk, node=None
                        )
