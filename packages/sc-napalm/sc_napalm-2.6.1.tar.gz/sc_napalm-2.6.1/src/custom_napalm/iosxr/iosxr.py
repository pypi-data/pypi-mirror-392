...
class SCIOSXR_SSH(SCBaseDriver, NetworkDriver):
    """
    ...
    INVENTORY_TO_TYPE = {
        ...
        r"Chassis": None,
        r"Power Supply": "psu",
        r".*": None, # Cannot deal with weird wan things I just really want optics
    }
    ...
    def get_config(
        ...
        if sanitized:
            raise NotImplementedError("Sanitized config not supported")

        if format != "text":
            raise NotImplementedError("Only text config format supported")
    ...
    def get_optics(self):
        """
        ssh based "get optics". Just like other platforms,
        For this platform you need to get levels on each
        individual interface
        """
        interfaces = self._send_command("show interfaces brief")
        phys = textfsm_extractor(self, "sh_int_brief_phy", interfaces)

        output = {}
            
        for phy in phys:
            i_name = f"{phy['prefix']}{phy['slot']}"
            output[i_name] = {"physical_channels": {"channels": {}}}
            i_channels = []

            ctrlr = self._send_command(f"show controllers optics {phy['slot']}")
            levels = textfsm_extractor(self, "controllers_optics", ctrlr)

            for channel in levels:
                i_channels.append({
                    "index":channel["lane"] if channel["lane"] else 0,
                    "input_power": channel["rx"],
                    "output_power": channel["tx"],
                    "laser_bias_current": channel["bias"],
                }
                )
            output[i_name]["physical_channels"]["channels"] = i_channels
