...
class QtColoredSVGIcon(QIcon):
    ...
    def colored(
        ...
        -------
        QtColoredSVGIcon
            A pre-colored QtColoredSVGIcon (which may still be recolored)
        """
        if not color and theme:
            from qtextra.config import THEMES

            color = getattr(THEMES.get_theme(theme), theme_key)

        return QtColoredSVGIcon(self._svg, color, opacity)
    ...
    def from_resources(
        ...
        ----------
        icon_name : str
            The name of the icon svg to load (just the stem).  Must be in the icons folder.
