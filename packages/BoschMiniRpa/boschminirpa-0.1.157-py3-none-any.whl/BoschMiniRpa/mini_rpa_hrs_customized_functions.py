        ...
        if not card_obj:
            ...
            if card_type == 'Service':
                if int(service_year) % 5 == 0:
                    font = ImageFont.truetype(font_path, 45, index=1)
                    draw.text((845, 587), user_name, font=font, fill=(0, 0, 0), stroke_width=0.5, stroke_fill=(0, 0, 0))
                else:
...
def hrs_generate_promotion_email_content(group_level, user_name, seq_id, template_folder_path,
    ...
    try:
        ...
        card_path = template_folder_path + os.sep + 'Card Template' + os.sep + f'{group_level}.jpg'
        ...
        if not card_obj:
            ...
            draw.text((850, 1250), '吕志超', font=font, stroke_width=1, fill=(0, 0, 0), stroke_fill=(0, 0, 0))
            font = ImageFont.truetype(font_path, 35, index=0)
            draw.text((925, 1332), 'PC5A', font=font, fill=(0, 0, 0), stroke_fill=(0, 0, 0))
...
def hrs_send_promotion_email(group_column, email_to_column, user_name_column, email_cc_column, email_subject, email_header, email_account, email_password,
    ...
    for column in promotion_data.columns:
        promotion_data[column] = promotion_data[column].str.strip().str.split('.', expand=True)[0]
...
def hrs_prepare_template_email_data(email_to_column, email_cc_column, email_column_name, email_field_name, template_file_path, customized_email_body, smb_user_name, user_password,
                                    server_name, share_name, port):
    ...
    if template_data.empty:
        ...
        for row_index in template_data.index:
            ...
            email_body += email_filed_text + '\n'
            email_body += customized_email_body + '\n'
