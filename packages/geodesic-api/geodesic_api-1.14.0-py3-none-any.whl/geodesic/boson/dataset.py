...
class Dataset(Object):
    ...
    def _upload_file(self, filepath: str) -> None:
        ...
        raise_on_error(
            upload_client.post(
                files={"file": open(filepath, "rb")}, data={"filename": pathlib.Path(filepath).name}
            )
        )

    def watch_status(
        self, poll_interval: float = 2.0, show_progress_bar: bool = True, progress_bar_class=None
    ) -> dict:
