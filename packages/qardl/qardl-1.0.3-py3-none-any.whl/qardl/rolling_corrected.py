...
class RollingQARDL:
    ...
    def __init__(
        ...
        if isinstance(y, pd.Series):
            y_array = y.values
            self.dates = y.index if hasattr(y, 'index') else None
        else:
        ...
        if isinstance(X, pd.DataFrame):
            X_array = X.values
            self.var_names = ['Y'] + X.columns.tolist()
            if self.dates is None and hasattr(X, 'index'):
                self.dates = X.index
        else:
            ...
            k = X_array.shape[1]
            self.var_names = ['Y'] + [f"X{i+1}" for i in range(k)]
