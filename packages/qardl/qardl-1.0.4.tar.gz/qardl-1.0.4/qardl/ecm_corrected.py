    ...
    def _convert_to_ecm(self):
        ...
        for tau in self.model.tau:
            ...
            if q > 0:
                ...
                if delta.shape == (k, q):
                    # Direct assignment when shapes match
                    theta_star = delta.copy()
                elif delta.ndim == 1 and len(delta) == q:
                    # If delta is 1D (single regressor), tile it
                    for i in range(k):
                        theta_star[i, :] = delta
                elif delta.shape[1] == q:
                    # Normal case - just copy row by row
                    for i in range(k):
                        theta_star[i, :] = delta[i, :q]
                else:
                    # Fallback - zero padding/truncation
                    min_cols = min(delta.shape[1], q)
                    theta_star[:, :min_cols] = delta[:, :min_cols]
            else:
