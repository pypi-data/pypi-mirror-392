...
class ComfyAPI(GPUMonitor):
    ...
    def prepare_workflow(self, workflow_json: dict[str, Any], settings_scaled_by_gpu_count: list[dict[str, Any]], settings_fixed_per_gpu: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    async def send_workflow_to_comfyui_and_wait_for_images(self, workflow_json: dict[str, Any], comfyui_url_api: str, path_to_save_images: str) -> list[str]:
    ...
    async def execute_workflow_TtI(self, workflow_json: dict[str, Any], models: list[dict[str, Any]], loras: list[dict[str, Any]] , settings_scaled_by_gpu_count: list[dict[str, Any]], settings_fixed_per_gpu: list[dict[str, Any]]) -> list[str]:
        ...
        final_workflow_json = self.prepare_workflow(workflow_json, settings_scaled_by_gpu_count, settings_fixed_per_gpu)

        generate_images_corontine = await asyncio.gather(
            *[self.send_workflow_to_comfyui_and_wait_for_images(final_workflow_json, f"http://localhost:{session['port']}", os.path.join(session["path"], "output")) for session in self.sessions_comfyui]
        )
