...
3. Follow the documented interaction sequence: if a New Message button is listed, you MUST `wait_for_element` it and click it before anything else. Then validate the “To:” field image → click → type the phone number; validate the message field image → click → type the message; send via Enter (or follow the documented send step) using the coordinates returned by detection for any listed control.
