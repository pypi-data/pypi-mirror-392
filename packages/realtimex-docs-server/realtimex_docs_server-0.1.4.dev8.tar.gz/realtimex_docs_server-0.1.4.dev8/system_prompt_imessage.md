...
3. Follow the documented interaction sequence WITHOUT skipping: if a New Message button is listed, you MUST `wait_for_element` it and click it BEFORE touching the “To:” field. Then validate the “To:” field image → click → type the phone number → press Enter; validate the message field image → click → type the message; send via Enter (or the documented send step) using ONLY the coordinates returned by detection.
