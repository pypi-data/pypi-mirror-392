...
def process_combined_stacks(data_files, output_path):
    ...
    for file_path in data_files:
        ...
        # 处理Python堆栈
        python_stacks = data.get('python_stack', [])
        for thread in python_stacks:
            frames = thread.get('frames', [])
            # 反转堆栈顺序（从底部到顶部）
            stack_frames = ["PYTHON_STACK"]  # 标记为Python堆栈
            for frame in reversed(frames):
                func_name = frame.get('name', 'unknown')
                filename = frame.get('filename', 'unknown')
                line = frame.get('line', 0)
                func_file_name = f"{func_name}@{filename}:{line}"
                stack_frames.append(func_file_name)
            
            # 插入到trie树
            if len(stack_frames) > 1:  # 确保有实际的堆栈帧
                stack_trie.insert(stack_frames, rank)

        # 处理原生堆栈
        native_stacks = data.get('parsed_native_stack', [])
        for thread in native_stacks:
            frames = thread.get('frames', [])
            stack_frames = ["NATIVE_STACK"]  # 标记为Native堆栈
            # 反转帧顺序，使栈底在前，栈顶在后
            for frame in reversed(frames):
                ...
                # 处理帧信息
                processed_frame = process_frame_info(clean_frame)
                stack_frames.append(processed_frame)
    ...
    os.system(
        ...
        f"> {output_path}/combined_stack.svg"
    )


def process_frame_info(frame):
    """处理单个帧信息，提取函数名和文件信息"""
    # 移除帧号 (#0, #1, etc.)
    frame = re.sub(r'#\d+\s+', '', frame)
    
    # 移除地址信息（如 0x00007f02db9268c0 in）
    frame = re.sub(r'0x[0-9a-fA-F]+ in ', '', frame)
    
    # 移除"in "前缀（如果有的话）
    if frame.startswith('in '):
        frame = frame[3:]
    
    # 处理 "function_name (...) at file:line" 格式
    at_match = re.search(r'^(.*?)\s+at\s+(.+)$', frame)
    from_match = re.search(r'^(.*?)\s+from\s+(.+)$', frame)
    
    if at_match:
        func_name = at_match.group(1).strip()
        file_info = at_match.group(2).strip()
        # 移除可能的括号内容
        func_name = re.sub(r'\s*\(.*?\)$', '', func_name)
        return f"{func_name}@{file_info}"
    elif from_match:
        func_name = from_match.group(1).strip()
        file_info = from_match.group(2).strip()
        # 移除可能的括号内容
        func_name = re.sub(r'\s*\(.*?\)$', '', func_name)
        return f"{func_name}@{file_info}"
    else:
        # 处理已经格式化的 "function@file" 格式
...
def main():
    ...
    # 处理组合堆栈（Python和Native）
    process_combined_stacks(data_files, str(path))
