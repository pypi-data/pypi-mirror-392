...
def _xgboost_n_iterations(tree_limit, num_stacked_models=1):
    """Convert tree_limit to iteration count for XGBoost iteration_range.
    
    XGBoost 2.0+ replaced ntree_limit with iteration_range parameter.
    This helper converts the old tree_limit to the new format.
    
    Parameters
    ----------
    tree_limit : int or None
        Number of trees to use (old format)
    num_stacked_models : int
        Number of models in the stack (for multi-output models)
        
    Returns
    -------
    n_iterations : int
        Number of iterations for iteration_range parameter.
        0 or -1 means use all iterations.
    """
    if tree_limit is None or tree_limit == -1 or tree_limit == 0:
        return 0  # 0 means use all iterations in iteration_range
    return tree_limit // max(num_stacked_models, 1)
...
class Tree(Explainer):
    ...
    def __init__(self, model, data = None, model_output="raw", feature_perturbation="tree_path_dependent", algorithm="auto", n_jobs=-1, memory_tolerance=-1, feature_names=None, approximate=False, shortcut=False, **deprecated_options):
        ...
        if self.model.model_output == "log_loss":
            ...
            # For tree_path_dependent without background data, let the C extension compute expected_value
            # during the TreeSHAP algorithm. The extension will return it in phi[0, -1, :].
            # Pre-computing it here as base_offset + sum(values[:,0]) is incorrect because it doesn't
            # account for the tree structure and sample weights properly.
            self.expected_value = None
            # Legacy calculation (incorrect):
            # self.expected_value = self.model.values[:,0].sum(0) + self.model.base_offset
            if self.model.model_output != "raw":
    ...
    def shap_values(self, X, y=None, tree_limit=None, approximate=False, check_additivity=True, from_call=False):
        ...
        if self.feature_perturbation == "tree_path_dependent" and self.model.model_type != "internal" and self.data is None:
            ...
            if self.model.model_type == "xgboost" and self.shortcut:
                import xgboost
                # Calculate iteration count for XGBoost 2.0+ iteration_range parameter
                num_stacked = getattr(self.model, 'num_stacked_models', 1)
                n_iterations = _xgboost_n_iterations(tree_limit, num_stacked)
                
                # Ensure X is a DMatrix (required for iteration_range)
                ...
                try:
                    # Use iteration_range instead of deprecated ntree_limit
    ...
    def __init__(self, model, data=None, data_missing=None, model_output=None):
        ...
        if type(model) is dict and "trees" in model:
            ...
            try:
                ...
                # XGBoost 3.0+ UBJSON format not supported by binary loader
                # Fall back to JSON parsing (less precise but works)
                warnings.warn(
                    f"Binary model loading failed ({type(e).__name__}), falling back to JSON parsing. "
                    "This may have slight numerical precision differences. "
                    "Consider updating FastTreeSHAP for full XGBoost 3.0 UBJSON support.",
                    UserWarning
                )
                import json
                json_trees = get_xgboost_json(self.original_model)
                self.trees = [SingleTree(json.loads(t), data=data, data_missing=data_missing) for t in json_trees]
                
                # Load base_score and objective from config
                try:
                    booster_config = json.loads(self.original_model.save_config())
                    learner_param = booster_config.get('learner', {}).get('learner_model_param', {})
                    objective = booster_config.get('learner', {}).get('objective', {})
                    
                    # Get base_score and objective name
                    base_score_raw = float(learner_param.get('base_score', 0.5))
                    objective_name = objective.get('name', 'binary:logistic')
                    
                    # Apply transformation based on objective (same logic as XGBTreeModelLoader)
                    if objective_name in ["binary:logistic", "reg:logistic"]:
                        self.base_offset = scipy.special.logit(base_score_raw)
                    elif objective_name in ["reg:gamma", "reg:tweedie", "count:poisson", "survival:cox", "survival:aft"]:
                        self.base_offset = np.log(base_score_raw)
                    else:
                        self.base_offset = base_score_raw
                    
                    self.objective = objective_name_map.get(objective_name, None)
                    self.tree_output = tree_output_name_map.get(objective_name, None)
                    self.num_stacked_models = int(learner_param.get('num_class', 0))
                except Exception as config_error:
                    # Ultimate fallback if config parsing fails
                    warnings.warn(f"Could not read model config ({config_error}), using defaults", UserWarning)
                    self.base_offset = scipy.special.logit(0.5)  # logit(0.5) = 0.0 for binary
                    self.objective = None
                    self.tree_output = None
                    self.num_stacked_models = 0
            ...
            try:
                ...
                # XGBoost 3.0+ UBJSON format fallback
                warnings.warn(
                    f"Binary model loading failed, falling back to JSON parsing. "
                    "This may have slight numerical precision differences.",
                    UserWarning
                )
                import json
                ...
                try:
                    ...
                    warnings.warn(f"Could not read model config ({config_error}), using defaults", UserWarning)
                    self.base_offset = scipy.special.logit(0.5)  # logit(0.5) = 0.0
                    self.objective = None
                    self.tree_output = None
                    self.num_stacked_models = 0
            if self.model_output == "predict_proba":
            ...
            try:
                ...
                warnings.warn("Binary model loading failed, falling back to JSON parsing.", UserWarning)
                import json
                json_trees = get_xgboost_json(self.original_model)
                self.trees = [SingleTree(json.loads(t), data=data, data_missing=data_missing) for t in json_trees]
                ...
                try:
                    ...
                    # For regression, base_score should not need transformation (usually)
                    base_score_raw = float(learner_param.get('base_score', 0.0))
                    objective_name = objective.get('name', 'reg:squarederror')
            ...
            try:
                ...
                # Load base_score from config
                try:
                    booster_config = json.loads(self.original_model.save_config())
                    learner_param = booster_config.get('learner', {}).get('learner_model_param', {})
                    self.base_offset = float(learner_param.get('base_score', 0.0))
                except:
                    self.base_offset = 0.0
            # Note: for ranker, leaving tree_output and objective as None as they
            # are not implemented in native code yet
            if xgb_loader and xgb_loader.num_class > 0:
                self.num_stacked_models = xgb_loader.num_class
        ...
        # deprecated dictionary support (with sklearn singlular style "feature" and "value" names)
            ...
            def extract_data(node, tree):
                ...
                if "children" in node:
                    ...
                    # Handle both integer indices and XGBoost 3.x string feature names (e.g., 'f0')
                    split_feature = node["split"]
                    if isinstance(split_feature, str) and split_feature.startswith('f'):
                        tree.features[i] = int(split_feature[1:])
                    else:
                        tree.features[i] = int(split_feature) if isinstance(split_feature, str) else split_feature
...
class XGBTreeModelLoader(object):
    ...
    def __init__(self, xgb_model):
        import xgboost
        # XGBoost 2.0+ uses UBJSON by default, but we need the legacy binary format
        # Try to request deprecated binary format for compatibility
        try:
            if version.parse(xgboost.__version__) >= version.parse("2.0.0"):
                # For XGBoost 2.0+, try to get binary format
                # Note: This may not work with all model types (e.g., categorical features)
                try:
                    self.buf = xgb_model.save_raw(raw_format="deprecated")
                except (TypeError, ValueError):
                    # If deprecated format not available, use ubj and try to parse
                    self.buf = xgb_model.save_raw(raw_format="ubj")
            else:
                # XGBoost 1.x uses binary format by default
                self.buf = xgb_model.save_raw()
            
            # new in XGBoost 1.1, 'binf' is appended to the buffer
            self.buf = self.buf.lstrip(b'binf')
        except Exception:
            # Fallback: try default save_raw
    ...
    def get_trees(self, data=None, data_missing=None):
        ...
        # Use float64 for values so compute_expectations can modify in-place
        self.values = np.zeros((shape[0], shape[1], 1), dtype=np.float64)
        trees = []
        for i in range(self.num_trees):
            for j in range(self.num_nodes[i]):
                ...
                if self.node_cleft[i][j] >= 0:
                    ...
                    # For leaf nodes, use base_weight which contains the actual leaf value
                    self.values[i,j] = self.base_weight[i][j]
