...
CoverTree<Point>::kNearestNodes(const Point &p, const unsigned int k) const
{
  ...
  if (_numNodes < 2)
    return std::vector<CoverTreeNode *>(1, _root);
                                  ...
                                  bool &multi)
{
  ...
  if (level > _minLevel)
    remove_rec(p, coverSets, level - 1, multi);
  if (minNode->hasPoint(p))
  {
    ...
    if (parent != NULL)
      parent->removeChild(level, minNode);
...
void CoverTree<Point>::insert(const Point &newPoint)
{
  ...
  else
  {
    ...
    insert_rec(newPoint,
               std::vector<distNodePair>(1, std::make_pair(_root->distance(newPoint), _root)),
               _maxLevel);
  }
}
