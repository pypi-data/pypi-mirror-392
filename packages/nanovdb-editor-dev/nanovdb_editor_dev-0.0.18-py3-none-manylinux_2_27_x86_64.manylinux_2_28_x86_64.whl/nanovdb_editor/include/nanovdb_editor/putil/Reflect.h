...
PNANOVDB_INLINE const char* pnanovdb_reflect_type_to_string(pnanovdb_uint32_t type)
{
    switch (type)
    {
    case PNANOVDB_REFLECT_TYPE_UNKNOWN:
        return "unknown";
    case PNANOVDB_REFLECT_TYPE_VOID:
        return "void";
    case PNANOVDB_REFLECT_TYPE_FUNCTION:
        return "function";
    case PNANOVDB_REFLECT_TYPE_STRUCT:
        return "struct";
...
{
    if (pnanovdb_reflect_string_compare(name, "unknown") == 0)
    {
        return PNANOVDB_REFLECT_TYPE_UNKNOWN;
    }
    else if (pnanovdb_reflect_string_compare(name, "void") == 0)
    {
        return PNANOVDB_REFLECT_TYPE_VOID;
    }
    else if (pnanovdb_reflect_string_compare(name, "function") == 0)
    {
        return PNANOVDB_REFLECT_TYPE_FUNCTION;
    }
    else if (pnanovdb_reflect_string_compare(name, "struct") == 0)
    {
        return PNANOVDB_REFLECT_TYPE_STRUCT;
    }
    else if (pnanovdb_reflect_string_compare(name, "int32") == 0)
    {
        return PNANOVDB_REFLECT_TYPE_INT32;
    }
    else if (pnanovdb_reflect_string_compare(name, "uint32") == 0)
    {
        return PNANOVDB_REFLECT_TYPE_UINT32;
    }
    else if (pnanovdb_reflect_string_compare(name, "float") == 0)
    {
        return PNANOVDB_REFLECT_TYPE_FLOAT;
    }
    else if (pnanovdb_reflect_string_compare(name, "bool32") == 0)
    {
        return PNANOVDB_REFLECT_TYPE_BOOL32;
    }
    else if (pnanovdb_reflect_string_compare(name, "uint8") == 0)
    {
        return PNANOVDB_REFLECT_TYPE_UINT8;
    }
    else if (pnanovdb_reflect_string_compare(name, "uint16") == 0)
    {
        return PNANOVDB_REFLECT_TYPE_UINT16;
    }
    else if (pnanovdb_reflect_string_compare(name, "uint64") == 0)
    {
        return PNANOVDB_REFLECT_TYPE_UINT64;
    }
    else if (pnanovdb_reflect_string_compare(name, "char") == 0)
    {
        return PNANOVDB_REFLECT_TYPE_CHAR;
    }
...
PNANOVDB_INLINE pnanovdb_bool_t pnanovdb_reflect_layout_compare(const pnanovdb_reflect_data_type_t* dst_type,
                                                                const pnanovdb_reflect_data_type_t* src_type)
{
    ...
    if (dst_type->data_type != src_type->data_type || dst_type->element_size != src_type->element_size ||
    ...
    for (pnanovdb_uint64_t child_idx = 0u; child_idx < dst_type->child_reflect_data_count; child_idx++)
    {
        ...
        if (child_dst->reflect_mode != child_src->reflect_mode || child_dst->data_offset != child_src->data_offset ||
...
PNANOVDB_INLINE void pnanovdb_reflect_copy_by_name(void* dst_data,
                                                   ...
                                                   const pnanovdb_reflect_data_type_t* src_type)
{
    ...
    pnanovdb_uint64_t safe_copy_size =
        src_type->element_size < dst_type->element_size ? src_type->element_size : dst_type->element_size;
    ...
    if (dst_type != src_type)
    {
        ...
        for (pnanovdb_uint64_t dst_idx = 0u; dst_idx < dst_type->child_reflect_data_count; dst_idx++)
        {
            ...
            for (pnanovdb_uint64_t srcCount = 0u;
                 match_src_idx == ~0llu && srcCount < src_type->child_reflect_data_count; srcCount++)
            {
            ...
            if (match_src_idx < src_type->child_reflect_data_count)
            {
                ...
                if (child_src->data_type->data_type == PNANOVDB_REFLECT_TYPE_STRUCT &&
                    (child_src->reflect_mode == PNANOVDB_REFLECT_MODE_VALUE ||
                     child_src->reflect_mode == PNANOVDB_REFLECT_MODE_VALUE_VERSIONED))
                {
                    pnanovdb_reflect_copy_by_name(dst_data8 + child_dst->data_offset, child_dst->data_type,
                ...
                else
                {
                    ...
                    {
                        pnanovdb_reflect_memcpy(dst_data8 + child_dst->data_offset, src_data8 + child_src->data_offset,
                                                (child_dst->reflect_mode & PNANOVDB_REFLECT_MODE_POINTER_ARRAY) ?
                                                    sizeof(void*) :
                                                    child_dst->data_type->element_size);
                    }
                    if (child_dst->reflect_mode & PNANOVDB_REFLECT_MODE_ARRAY)
                    {
                        if (child_dst->array_size_offset != child_src->array_size_offset)
                        {
                            pnanovdb_reflect_memcpy(dst_data8 + child_dst->array_size_offset,
                                                    src_data8 + child_src->array_size_offset, sizeof(pnanovdb_uint64_t));
                        }
                    }
                    if (child_dst->reflect_mode & PNANOVDB_REFLECT_MODE_VALUE_VERSIONED)
                    {
                        ...
                        {
                            pnanovdb_reflect_memcpy(dst_data8 + child_dst->version_offset,
                                                    src_data8 + child_src->version_offset, sizeof(pnanovdb_uint64_t));
                        }
                    }
                }
            }
            ...
            {
                pnanovdb_reflect_clear(dst_data8 + child_dst->data_offset,
                                       (child_dst->reflect_mode & PNANOVDB_REFLECT_MODE_POINTER_ARRAY) ?
                                           sizeof(void*) :
                                           child_dst->data_type->element_size);
                if (child_dst->reflect_mode & PNANOVDB_REFLECT_MODE_ARRAY)
                {
                    pnanovdb_reflect_clear(dst_data8 + child_dst->array_size_offset, sizeof(pnanovdb_uint64_t));
                }
                if (child_dst->reflect_mode & PNANOVDB_REFLECT_MODE_VALUE_VERSIONED)
                {
                    pnanovdb_reflect_clear(dst_data8 + child_dst->version_offset, sizeof(pnanovdb_uint64_t));
                }
            }
        }
    }
...
#define PNANOVDB_REFLECT_VALIDATE(type)                                                                                 \
    PNANOVDB_INLINE type* PNANOVDB_REFLECT_XCONCAT(type, _pnanovdb_validate_ptr)(const type* v)                         \
    {                                                                                                                   \
        return (type*)v;                                                                                                \
    }                                                                                                                   \
    PNANOVDB_INLINE type** PNANOVDB_REFLECT_XCONCAT(type, _pnanovdb_validate_ptr_ptr)(const type* const* v)             \
    {                                                                                                                   \
        return (type**)v;                                                                                               \
    }                                                                                                                   \
    PNANOVDB_INLINE type*** PNANOVDB_REFLECT_XCONCAT(type, _pnanovdb_validate_ptr_ptr_ptr)(const type* const* const* v) \
    {                                                                                                                   \
        return (type***)v;                                                                                              \
    }

#if defined(__cplusplus)
#    define PNANOVDB_REFLECT_VALIDATE_VALUE(type, name)                                                                \
        PNANOVDB_REFLECT_XCONCAT(type, _pnanovdb_validate_ptr)(&((PNANOVDB_REFLECT_TYPE*)0)->name)
#    define PNANOVDB_REFLECT_VALIDATE_POINTER(type, name)                                                              \
        PNANOVDB_REFLECT_XCONCAT(type, _pnanovdb_validate_ptr_ptr)(&((PNANOVDB_REFLECT_TYPE*)0)->name)
#    define PNANOVDB_REFLECT_VALIDATE_ARRAY(type, name)                                                                \
        PNANOVDB_REFLECT_XCONCAT(type, _pnanovdb_validate_ptr_ptr)(&((PNANOVDB_REFLECT_TYPE*)0)->name)
#    define PNANOVDB_REFLECT_VALIDATE_POINTER_ARRAY(type, name)                                                        \
        PNANOVDB_REFLECT_XCONCAT(type, _pnanovdb_validate_ptr_ptr_ptr)(&((PNANOVDB_REFLECT_TYPE*)0)->name)
#    define PNANOVDB_REFLECT_VALIDATE_VALUE_VERSIONED(type, name)                                                      \
        PNANOVDB_REFLECT_XCONCAT(type, _pnanovdb_validate_ptr)(&((PNANOVDB_REFLECT_TYPE*)0)->name)
#    define PNANOVDB_REFLECT_VALIDATE_POINTER_VERSIONED(type, name)                                                    \
        PNANOVDB_REFLECT_XCONCAT(type, _pnanovdb_validate_ptr_ptr)(&((PNANOVDB_REFLECT_TYPE*)0)->name)
#    define PNANOVDB_REFLECT_VALIDATE_ARRAY_VERSIONED(type, name)                                                      \
        PNANOVDB_REFLECT_XCONCAT(type, _pnanovdb_validate_ptr_ptr)(&((PNANOVDB_REFLECT_TYPE*)0)->name)
#    define PNANOVDB_REFLECT_VALIDATE_POINTER_ARRAY_VERSIONED(type, name)                                              \
        PNANOVDB_REFLECT_XCONCAT(type, _pnanovdb_validate_ptr_ptr_ptr)(&((PNANOVDB_REFLECT_TYPE*)0)->name)
#else
...
#define PNANOVDB_REFLECT_BUILTIN_IMPL(enumName, typeName)                                                              \
    static const pnanovdb_reflect_data_type_t PNANOVDB_REFLECT_XCONCAT(                                                \
        typeName, _pnanovdb_reflect_data_type) = { enumName, sizeof(typeName), 0, 0, 0, 0 };                           \
    PNANOVDB_REFLECT_VALIDATE(typeName)

#define PNANOVDB_REFLECT_STRUCT_OPAQUE_IMPL(name)                                                                      \
    static const pnanovdb_reflect_data_type_t PNANOVDB_REFLECT_XCONCAT(                                                \
        name, _pnanovdb_reflect_data_type) = { PNANOVDB_REFLECT_TYPE_STRUCT, 0llu, #name, 0, 0, 0 };                   \
...
#define PNANOVDB_REFLECT_END(defaultValue)                                                                             \
    ...
    ;                                                                                                                  \
    static const pnanovdb_reflect_data_type_t PNANOVDB_REFLECT_XCONCAT(                                                \
        PNANOVDB_REFLECT_TYPE, _pnanovdb_reflect_data_type) = {                                                        \
...
#    define PNANOVDB_REFLECT_SIZE_OFFSET(name_size)                                                                    \
        (pnanovdb_uint64_t) PNANOVDB_REFLECT_VALIDATE_VALUE(pnanovdb_uint64_t, name_size)
#    define PNANOVDB_REFLECT_VERSION_OFFSET(version)                                                                   \
        (pnanovdb_uint64_t) PNANOVDB_REFLECT_VALIDATE_VALUE(pnanovdb_uint64_t, version)
#else
...
#define PNANOVDB_REFLECT_GENERIC(reflectMode, type, name, ARRAY, VERSION, reflectHints, metadata)                      \
    { reflectHints,                                                                                                    \
      ...
      VERSION,                                                                                                         \
      metadata },
#define PNANOVDB_REFLECT_VALUE(type, name, reflectHints, metadata)                                                     \
    PNANOVDB_REFLECT_GENERIC(VALUE, type, name, 0, 0, reflectHints, metadata)
#define PNANOVDB_REFLECT_POINTER(type, name, reflectHints, metadata)                                                   \
    PNANOVDB_REFLECT_GENERIC(POINTER, type, name, 0, 0, reflectHints, metadata)
#define PNANOVDB_REFLECT_ARRAY(type, name, name_size, reflectHints, metadata)                                          \
    PNANOVDB_REFLECT_GENERIC(ARRAY, type, name, PNANOVDB_REFLECT_SIZE_OFFSET(name_size), 0, reflectHints, metadata)
#define PNANOVDB_REFLECT_POINTER_ARRAY(type, name, name_size, reflectHints, metadata)                                  \
    PNANOVDB_REFLECT_GENERIC(                                                                                          \
        POINTER_ARRAY, type, name, PNANOVDB_REFLECT_SIZE_OFFSET(name_size), 0, reflectHints, metadata)
#define PNANOVDB_REFLECT_VALUE_VERSIONED(type, name, version, reflectHints, metadata)                                  \
    PNANOVDB_REFLECT_GENERIC(                                                                                          \
        VALUE_VERSIONED, type, name, 0, PNANOVDB_REFLECT_VERSION_OFFSET(version), reflectHints, metadata)
#define PNANOVDB_REFLECT_POINTER_VERSIONED(type, name, version, reflectHints, metadata)                                \
    PNANOVDB_REFLECT_GENERIC(                                                                                          \
        POINTER_VERSIONED, type, name, 0, PNANOVDB_REFLECT_VERSION_OFFSET(version), reflectHints, metadata)
#define PNANOVDB_REFLECT_ARRAY_VERSIONED(type, name, name_size, version, reflectHints, metadata)                       \
    PNANOVDB_REFLECT_GENERIC(ARRAY_VERSIONED, type, name, PNANOVDB_REFLECT_SIZE_OFFSET(name_size),                     \
                             PNANOVDB_REFLECT_VERSION_OFFSET(version), reflectHints, metadata)
#define PNANOVDB_REFLECT_POINTER_ARRAY_VERSIONED(type, name, name_size, version, reflectHints, metadata)               \
    PNANOVDB_REFLECT_GENERIC(POINTER_ARRAY_VERSIONED, type, name, PNANOVDB_REFLECT_SIZE_OFFSET(name_size),             \
                             PNANOVDB_REFLECT_VERSION_OFFSET(version), reflectHints, metadata)

/// Function Pointer
static const pnanovdb_reflect_data_type_t function_pnanovdb_reflect_data_type = {
    PNANOVDB_REFLECT_TYPE_FUNCTION, 0llu, 0, 0, 0, 0
};
...
/// Void
static const pnanovdb_reflect_data_type_t void_pnanovdb_reflect_data_type = {
    PNANOVDB_REFLECT_TYPE_VOID, 0llu, 0, 0, 0, 0
};
...
#define PNANOVDB_REFLECT_INTERFACE_IMPL()                                                                              \
    PNANOVDB_INLINE void PNANOVDB_REFLECT_XCONCAT(PNANOVDB_REFLECT_TYPE, _duplicate)(                                  \
        PNANOVDB_REFLECT_TYPE * dst, const PNANOVDB_REFLECT_TYPE* src)                                                 \
    {                                                                                                                  \
        const pnanovdb_reflect_data_type_t* dst_type =                                                                 \
            &PNANOVDB_REFLECT_XCONCAT(PNANOVDB_REFLECT_TYPE, _pnanovdb_reflect_data_type);                             \
        pnanovdb_reflect_copy_by_name(dst, dst_type, src, src->interface_pnanovdb_reflect_data_type);                  \
...
#define PNANOVDB_CAST_PAIR(X, Y)                                                                                       \
    PNANOVDB_INLINE X* cast(Y* ptr)                                                                                    \
    {                                                                                                                  \
        return (X*)ptr;                                                                                                \
    }                                                                                                                  \
    PNANOVDB_INLINE Y* cast(X* ptr)                                                                                    \
    {                                                                                                                  \
        return (Y*)ptr;                                                                                                \
    }                                                                                                                  \
    PNANOVDB_INLINE const X* cast(const Y* ptr)                                                                        \
    {                                                                                                                  \
        return (X*)ptr;                                                                                                \
    }                                                                                                                  \
    PNANOVDB_INLINE const Y* cast(const X* ptr)                                                                        \
    {                                                                                                                  \
        return (Y*)ptr;                                                                                                \
    }

#define PNANOVDB_CAST_PAIR_NAMED(name, X, Y)                                                                           \
    PNANOVDB_INLINE X* name##_cast(Y* ptr)                                                                             \
    {                                                                                                                  \
        return (X*)ptr;                                                                                                \
    }                                                                                                                  \
    PNANOVDB_INLINE Y* name##_cast(X* ptr)                                                                             \
    {                                                                                                                  \
        return (Y*)ptr;                                                                                                \
    }                                                                                                                  \
    PNANOVDB_INLINE const X* name##_cast(const Y* ptr)                                                                 \
    {                                                                                                                  \
        return (X*)ptr;                                                                                                \
    }                                                                                                                  \
    PNANOVDB_INLINE const Y* name##_cast(const X* ptr)                                                                 \
    {                                                                                                                  \
        return (Y*)ptr;                                                                                                \
    }
