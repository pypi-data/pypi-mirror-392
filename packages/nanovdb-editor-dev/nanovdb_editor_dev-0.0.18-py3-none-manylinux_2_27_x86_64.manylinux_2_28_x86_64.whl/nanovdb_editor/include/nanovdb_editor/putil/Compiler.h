...
typedef struct pnanovdb_compiler_t
{
    ...
    void(PNANOVDB_ABI* set_diagnostic_callback)(pnanovdb_compiler_instance_t* instance,
                                                pnanovdb_compiler_diagnostic_callback callback);

    pnanovdb_bool_t(PNANOVDB_ABI* compile_shader_from_file)(pnanovdb_compiler_instance_t* instance,
                                                            const char* filename,
                                                            pnanovdb_compiler_settings_t* settings,
                                                            pnanovdb_bool_t* shader_updated);

    pnanovdb_bool_t(PNANOVDB_ABI* execute_cpu)(pnanovdb_compiler_instance_t* instance,
                                               const char* filename,
...
static inline void pnanovdb_compiler_load(pnanovdb_compiler_t* compiler)
{
    void* compiler_module =
        pnanovdb_load_library("pnanovdbcompiler.dll", "libpnanovdbcompiler.so", "libpnanovdbcompiler.dylib");
    if (!compiler_module)
    {
#if defined(_WIN32)
    ...
    PFN_pnanovdb_get_compiler get_compiler =
        (PFN_pnanovdb_get_compiler)pnanovdb_get_proc_address(compiler_module, "pnanovdb_get_compiler");
    if (!get_compiler)
    {
...
static inline void pnanovdb_compiler_free(pnanovdb_compiler_t* compiler)
{
    pnanovdb_free_library(compiler->module);
}

static inline void pnanovdb_compiler_settings_init(pnanovdb_compiler_settings_t* settings)
{
