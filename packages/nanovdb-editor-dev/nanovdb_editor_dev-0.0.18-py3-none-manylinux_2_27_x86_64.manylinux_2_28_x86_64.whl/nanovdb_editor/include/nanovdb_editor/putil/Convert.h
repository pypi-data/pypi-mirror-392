...
/*!
    \file   nanovdb_editor/nanovdb/putil/Convert.h

    \author Andrew Reidmeyer

    \brief Converts Node2 Grids to types like OnIndex
*/
...
PNANOVDB_FORCE_INLINE void pnanovdb_node2_convert_to_grid_type(pnanovdb_buf_t dst_buf,
                                                               ...
                                                               pnanovdb_buf_t buf)
{
    ...
    for (pnanovdb_uint32_t node_type = 0u; node_type < 5u; node_type++)
    {
            ...
            for (pnanovdb_uint32_t root_n = 0u; root_n < 32768u; root_n++)
            {
                ...
                if (key != pnanovdb_node2_end_key)
                {
                    ...
                    pnanovdb_uint32_t state =
                        pnanovdb_node2_get_value_mask_bit(buf, root, PNANOVDB_NODE2_TYPE_ROOT, root_n) ? 1 : 0;
        ...
        for (pnanovdb_uint32_t root_n = 0u; root_n < 32768u; root_n++)
        {
            ...
            if (node_type == 4u)
            {
                ...
                pnanovdb_uint32_t root_tile_local_idx = pnanovdb_node2_mask_n_to_idx(
                    buf, root.idx64 + pnanovdb_node2_off_value_mask_prefix_sum[PNANOVDB_NODE2_TYPE_ROOT],
                ...
                if (root_tile_is_active && dst_grid_type == PNANOVDB_GRID_TYPE_ONINDEX)
                {
                    pnanovdb_address_t val_addr =
                        pnanovdb_root_tile_get_value_address(dst_grid_type, dst_buf, dst_root_tile);
            ...
            if (node_type == 1u)
            {
                ...
                pnanovdb_node2_memcpy(dst_buf,
                                      pnanovdb_address_offset(dst_upper.address, PNANOVDB_UPPER_OFF_CHILD_MASK), buf,
                                      pnanovdb_address_offset_product(
                                          pnanovdb_address_null(),
                                          upper.idx64 + pnanovdb_node2_off_child_mask[PNANOVDB_NODE2_TYPE_UPPER], 8u),
                                      32768u / 8u);
                pnanovdb_node2_memcpy(dst_buf,
                                      pnanovdb_address_offset(dst_upper.address, PNANOVDB_UPPER_OFF_VALUE_MASK), buf,
                                      pnanovdb_address_offset_product(
                                          pnanovdb_address_null(),
                                          upper.idx64 + pnanovdb_node2_off_value_mask[PNANOVDB_NODE2_TYPE_UPPER], 8u),
                                      32768u / 8u);

                pnanovdb_root_tile_set_child(
                    dst_buf, dst_root_tile, pnanovdb_address_diff(dst_upper.address, dst_root.address));
            ...
            for (pnanovdb_uint32_t upper_n = 0u; upper_n < 32768u; upper_n++)
            {
                if (node_type == 4u)
                {
                    ...
                    pnanovdb_uint32_t upper_local_idx = pnanovdb_node2_mask_n_to_idx(
                        buf, upper.idx64 + pnanovdb_node2_off_value_mask_prefix_sum[PNANOVDB_NODE2_TYPE_UPPER],
                    ...
                    {
                        pnanovdb_address_t val_addr =
                            pnanovdb_upper_get_table_address(dst_grid_type, dst_buf, dst_upper, upper_n);
                ...
                if (node_type == 2u)
                {
                    ...
                    pnanovdb_node2_memcpy(
                        dst_buf, pnanovdb_address_offset(dst_lower.address, PNANOVDB_LOWER_OFF_CHILD_MASK), buf,
                        pnanovdb_address_offset_product(
                            pnanovdb_address_null(),
                            lower.idx64 + pnanovdb_node2_off_child_mask[PNANOVDB_NODE2_TYPE_LOWER], 8u),
                        4096u / 8u);
                    pnanovdb_node2_memcpy(
                        dst_buf, pnanovdb_address_offset(dst_lower.address, PNANOVDB_LOWER_OFF_VALUE_MASK), buf,
                        pnanovdb_address_offset_product(
                            pnanovdb_address_null(),
                            lower.idx64 + pnanovdb_node2_off_value_mask[PNANOVDB_NODE2_TYPE_LOWER], 8u),
                        4096u / 8u);
                ...
                for (pnanovdb_uint32_t lower_n = 0u; lower_n < 4096u; lower_n++)
                {
                    if (node_type == 4u)
                    {
                        ...
                        pnanovdb_uint32_t lower_local_idx = pnanovdb_node2_mask_n_to_idx(
                            buf, lower.idx64 + pnanovdb_node2_off_value_mask_prefix_sum[PNANOVDB_NODE2_TYPE_LOWER],
                        ...
                        {
                            pnanovdb_address_t val_addr =
                                pnanovdb_lower_get_table_address(dst_grid_type, dst_buf, dst_lower, lower_n);
                            pnanovdb_write_uint64(dst_buf, val_addr, lower_value_idx + lower_local_idx);
                        }
                    }
                    pnanovdb_node2_handle_t leaf =
                        pnanovdb_node2_get_child(buf, lower, PNANOVDB_NODE2_TYPE_LOWER, lower_n);
                    if (leaf.idx64 == 0u)
                    {
                        continue;
                    }
                    ...
                    if (node_type == 3u)
                    {
                        ...
                        pnanovdb_node2_memcpy(
                            dst_buf, pnanovdb_address_offset(dst_leaf.address, PNANOVDB_LEAF_OFF_VALUE_MASK), buf,
                            pnanovdb_address_offset_product(
                                pnanovdb_address_null(),
                                leaf.idx64 + pnanovdb_node2_off_value_mask[PNANOVDB_NODE2_TYPE_LEAF], 8u),
                            512u / 8u);
                    ...
                    if (node_type == 4u)
                    {
                        pnanovdb_uint64_t leaf_value_idx =
                            pnanovdb_node2_read(buf, leaf.idx64 + pnanovdb_node2_off_value_idx);
                        pnanovdb_leaf_handle_t dst_leaf =
                            pnanovdb_lower_get_child(dst_grid_type, dst_buf, dst_lower, lower_n);
                        if (dst_grid_type == PNANOVDB_GRID_TYPE_ONINDEX)
                        {
                            pnanovdb_address_t offset_addr =
                                pnanovdb_leaf_get_table_address(dst_grid_type, dst_buf, dst_leaf, 0u);
                            ...
                            {
                                pnanovdb_uint64_t mask = pnanovdb_read_uint64(
                                    dst_buf,
    ...
    if (dst_grid_type == PNANOVDB_GRID_TYPE_ONINDEX)
    {
        ...
        pnanovdb_gridblindmetadata_set_data_offset(
            dst_buf, dst_metadata, pnanovdb_address_diff(dst_data, dst_metadata.address));
    ...
    // validation
    {
        ...
        for (pnanovdb_uint32_t dst_tile_idx = 0u; dst_tile_idx < dst_tile_count; dst_tile_idx++)
        {
            ...
            printf("tile[%d] child(%llu) key(%llx) state(%u)\n", dst_tile_idx, (unsigned long long int)child,
                   (unsigned long long int)key, state);
        }
            ...
            for (pnanovdb_uint32_t upper_n = 0u; upper_n < 32768u; upper_n++)
            {
                ...
                for (pnanovdb_uint32_t lower_n = 0u; lower_n < 4096u; lower_n++)
                {
                    ...
                    for (pnanovdb_uint32_t leaf_n = 0u; leaf_n < 512u; leaf_n++)
                    {
                        ...
                        pnanovdb_coord_t ijk = { root_ijk.x + upper_ijk.x + lower_ijk.x + leaf_ijk.x,
                        ...
                        pnanovdb_address_t val_addr = pnanovdb_readaccessor_get_value_address_and_level(
                            dst_grid_type, dst_buf, PNANOVDB_REF(acc), PNANOVDB_REF(ijk), PNANOVDB_REF(level));
                        ...
                        pnanovdb_uint32_t leaf_local_idx = pnanovdb_node2_mask_n_to_idx(
                            buf, leaf.idx64 + pnanovdb_node2_off_value_mask_prefix_sum[PNANOVDB_NODE2_TYPE_LEAF],
                        ...
                        if (leaf_is_active)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    printf(
                                        "tree(%d,%d,%d,%d) acc(%llu,%llu,%llu,%llu) val_idx(%llu) vs node2_val_idx(%llu)\n",
                                        root_n, upper_n, lower_n, leaf_n,
                                        (unsigned long long int)acc.root.address.byte_offset,
                                        (unsigned long long int)acc.upper.address.byte_offset,
                                        (unsigned long long int)acc.lower.address.byte_offset,
                                        (unsigned long long int)acc.leaf.address.byte_offset,
