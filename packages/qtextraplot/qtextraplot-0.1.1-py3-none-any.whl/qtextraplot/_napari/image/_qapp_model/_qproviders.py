...
def _provide_viewer(public_proxy: bool = True) -> Viewer:
    """Provide `PublicOnlyProxy` (allows internal napari access) of current viewer."""
    if _qviewer := _provide_qt_viewer():
        if public_proxy:
            return PublicOnlyProxy(_qviewer.viewer)
        return _qviewer.viewer
    return None


def _provide_viewer_or_raise(msg: str = "", public_proxy: bool = False) -> Viewer:
...
def _provide_active_layer() -> layers.Layer:
    return v.layers.selection.active if (v := _provide_viewer()) else None


def _provide_active_layer_list() -> components.LayerList:
