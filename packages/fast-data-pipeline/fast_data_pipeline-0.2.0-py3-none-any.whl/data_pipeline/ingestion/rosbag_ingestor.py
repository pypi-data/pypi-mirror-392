...
class RosbagIngester(BaseIngester):
    ...
    def process_file(self, folder_path: str) -> Optional[str]:
        """
        Process single rosbag folder and convert to HDF5.
        ...
        try:
            ...
            logger.warning(f"Processing interrupted by user for {folder_name}")
            raise
    ...
    def _stream_and_write_hybrid(self, folder_path: str, h5file: tables.File):
        """Process rosbag with unified smart writing for all message types."""
        ...
        with AnyReader([Path(folder_path)], default_typestore=typestore) as reader:
            ...
            if not connections:
                logger.error(f"No matching topics found in {folder_path}")
                raise ValueError(f"No valid topics to process in {folder_path}")
            ...
            with tqdm(desc=f"Processing {os.path.basename(folder_path)}", unit="msg") as pbar:
            ...
            if processed_count == 0:
                raise ValueError(f"No messages processed successfully from {folder_path}")
...
if __name__ == "__main__":
    ...
    TEST_INPUT_DIR = "/mnt/kit_workspace/new_rosbags/2025_10_29"
    TEST_OUTPUT_DIR = "/mnt/kit_workspace/new_rosbags/2025_10_29/rosbags"
