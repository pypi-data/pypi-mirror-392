...
class RubixMessageHandler:
    ...
    def _load_or_deploy_nft(self) -> str:
        # Case 1: token.txt exists and has a value → use it
        if self.token_path.exists():
            ...
            if token:
                ...
                print(f"⚠️ token.txt at {self.token_path} is empty, deploying new NFT…")
        else:
            print(f"⚠️ token.txt not found at {self.token_path}, deploying new NFT…")

        # Case 2: missing or empty → deploy a new NFT
