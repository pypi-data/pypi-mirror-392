...
import argparse, platformdirs, configparser, sys, collections, hashlib
...
reaper.Main_OnCommand(reaper.NamedCommandLookup("_SWSMARKERLIST9"),0) -- erase markers
...
def parse_otio_args():
    ...
    if args.clip_arg != None:
        if args.otio_and_al != []:
            ...
            clip_names = args.clip_arg
            logger.debug('mode, c_option_arg, otio_file, render_file, exit:')
            return None, None, clip_names, args.e, args.t #############################
    def _is_otio(f):
        components = f.split('.')
        if len(components) == 1:
            return False
...
local tr = reaper.GetTrack(0,reaper.CountTracks() - 2)
...
def find_mode_from_args(movie_file, clip_names, frames_per_second, clips):
    ...
    if movie_duration > otio_duration * 1.0012:
        print(f'Error: movie is longer than otio timeline {(movie_duration, otio_duration)} frames, bye.')
        sys.exit(0)
    if movie_duration > otio_duration:
        print(f'Warning: movie is longer than otio timeline {(movie_duration, otio_duration)}, has the clip VBR?')
...
def clip_subset(clips, movie_TC_sec, movie_length, clip_names_abbr, fps):
    ...
    if len(clip_names_abbr) == 2:
        # find clip subset from named clips as arguments
        ...
        # find clip subset from movie_file length and position.
        # but 1st, consistency check:
        if len(clip_names_abbr) != 0:
            print('Error in processing argument...')
            raise ValueError(f'Error in processing argument, here clip_names_abbr {clip_names_abbr} should be []')
    fr_A = fr(movie_TC_sec)
    fr_B = fr(movie_TC_sec + movie_length*0.99)
...
def main():
    ...
    logger.debug('otio_file, movie_file, clip_names, exit_flag, show_tc:')
    logger.debug(f'{otio_file}, {movie_file}, {clip_names}, {exit_flag}, {show_tc}.')
    if len(clip_names) == 1:
    ...
    if movie_file != None:
        ...
        if m == None:
            print(f'Warning, no timecode found for movie {movie_file}')
        ...
        mov_TCinSeconds = HH*3600  + MM*60 + SS + FF/frames_per_second
        mov_duration = float(ffmpeg.probe(movie_file)['format']['duration'])
        logger.debug(f'mov_TCinSeconds: {mov_TCinSeconds}, mov_duration: {mov_duration} for movie_file {movie_file}')
    match mode:
        ...
        case mamdav.Modes.INTERCLIP_SOME:
            ...
            clips = clip_subset(clips, mov_TCinSeconds, mov_duration, clip_names, frames_per_second)
            logger.debug(f'INTERCLIP_SOME, clips: {clips}')
    ...
    if movie_file == None:
        ...
        movie_scrpt = InsertMedia(mov_TCinSeconds, pathlib.Path(movie_file).resolve())
