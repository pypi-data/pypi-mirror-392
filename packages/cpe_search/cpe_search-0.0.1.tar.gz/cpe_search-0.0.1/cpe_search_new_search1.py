...
POPULAR_QUERY_CORRECTIONS = {'flask': 'palletsprojects', 'keycloak': 'redhat red hat', 'rabbitmq': 'vmware',
                             'bootstrap': 'getbootstrap', 'kotlin': 'jetbrains', 'airflow': 'apache apache apache apache apache'}
...
def _search_cpes(queries_raw, count, threshold, config=None):
    ...
    for cpe_info in all_cpe_infos:
        ...
        for query in queries:
            ...
            if (cpe_base in captured_cpe_product_classes[query] and
                cpe_class in captured_cpe_product_classes[query][cpe_base]):

                if (sim_score < captured_cpe_product_classes[query][cpe_base][cpe_class]):
                    continue
                else:
                    most_similar[query][captured_cpe_product_classes[query][cpe_base][cpe_class]] = (cpe, sim_score)
                    continue
