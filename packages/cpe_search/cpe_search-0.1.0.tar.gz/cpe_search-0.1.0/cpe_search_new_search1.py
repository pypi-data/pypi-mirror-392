...
POPULAR_QUERY_CORRECTIONS = {
    ...
    "kotlin": "jetbrains",
    "airflow": "apache apache apache apache apache",
}
    ...
    def load_config_dict(_dict, db_type):
        ...
        for key, val in _dict.items():
            if isinstance(val, dict):
                ...
                db_type == "sqlite" and key.lower() == "database_name"
            ):
...
def _get_alternative_queries(init_queries):
    ...
    for query in init_queries:
        ...
        if "citrix" in query and (
            query.startswith("adc ") or query.endswith(" adc") or " adc " in query
        ):
            alt_query = query.replace("adc", "application delivery controller")
            alt_queries_mapping[query].append(alt_query)

        # check for dell 'omsa' abbreviation
        if "dell" in query and (
            query.startswith("omsa ") or query.endswith(" omsa") or " omsa " in query
        ):
        ...
        for product, helper_query in POPULAR_QUERY_CORRECTIONS.items():
            if product in query and not any(word in query for word in helper_query.split(" ")):
                alt_queries_mapping[query].append(query + " " + helper_query)
...
def _search_cpes(queries_raw, count, threshold, config=None):
    ...
    processed_cpes = set()
    captured_cpe_product_classes = {}  # cpe-class --> number of non-wildcard fields
    for query in queries:
        captured_cpe_product_classes[query] = {}
    ...
    for cpe_info in all_cpe_infos:
        ...
        for query in queries:
            ...
            # if cpe.startswith('cpe:2.3:a:apache:airflow:'):
            #     print('-'*80)
            #     print(cpe, sim_score)

            # if cpe.startswith('cpe:2.3:a:wordpress:wordpress:5.7.2:'):
            #     print(cpe)
            ...
            cpe_class = 10 - sum(cpe_field in ("*", "-", "") for cpe_field in cpe.split(":"))

            if (
                cpe_base in captured_cpe_product_classes[query]
                and cpe_class in captured_cpe_product_classes[query][cpe_base]
            ):

                if sim_score < captured_cpe_product_classes[query][cpe_base][cpe_class]:
                    continue
                else:
                    most_similar[query][
                        captured_cpe_product_classes[query][cpe_base][cpe_class]
                    ] = (cpe, sim_score)
                    continue

            insert_idx = None
            for i, (cur_cpe, cur_sim_score) in enumerate(most_similar[query]):
            ...
            prev_cpe = None
            if insert_idx == 0:
                most_similar[query] = [(cpe, sim_score)] + most_similar[query][1:]
            elif insert_idx:
                prev_cpe = most_similar[query][-1][0]
                most_similar[query] = (
                ...
                # if prev_cpe != 'N/A':
                #     prev_cpe_base = ':'.join(prev_cpe.split(':')[:5]) + ':'
                #     prev_cpe_class = 10 - sum(cpe_field in ('*', '-', '') for cpe_field in prev_cpe.split(':'))
                #     captured_cpe_product_classes[query][prev_cpe_base].pop(prev_cpe_class, None)
            elif len(most_similar[query]) < 1000:
                most_similar[query].append((cpe, sim_score))
                insert_idx = len(most_similar[query]) - 1
            else:
                continue

            # add cpe class entry
            if cpe_base not in captured_cpe_product_classes[query]:
                captured_cpe_product_classes[query][cpe_base] = {}
            captured_cpe_product_classes[query][cpe_base][cpe_class] = insert_idx

    # only return the number of requested CPEs
    if count != -1:
        for query in queries:
            print(len(most_similar[query]))
            # print(most_similar[query])
            print(most_similar[query][:5])
...
def search_cpes(query, count=3, threshold=-1, config=None):
    ...
    if not MATCH_CPE_23_RE.match(query):
        cpes = _search_cpes(
            [query], count=count, threshold=CPE_SEARCH_THRESHOLD_ALT, config=config
        )
        cpes = cpes.get(query, [])
...
if __name__ == "__main__":
    ...
    if (
        ...
        and not os.path.isfile(config["DATABASE_NAME"])
    ):
        if not SILENT:
