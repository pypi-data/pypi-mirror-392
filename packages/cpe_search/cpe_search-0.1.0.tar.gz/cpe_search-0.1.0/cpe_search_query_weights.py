...
def _search_cpes(queries_raw, count, threshold, keep_data_in_memory=False):
    ...
    # Challenge: proftpd 1.3.3c / proftpd 1.3.3 c vs. datatables 1.9.4
    query_infos = {}
    most_similar = {}
    all_query_words = set()
    for query in queries:
        ...
        for i, word in enumerate(words_query):
            if word not in word_weights_query:  # always use greatest weight
                word_weights_query[word] = math.exp(-0.23 * i)
