    ...
    for cpe_info in all_cpe_infos:
        ...
        for query in queries:
            ...
            if sim_score > most_similar[query][0][1]:
                ...
                if cpe_base not in captured_cpe_product_classes:
                    captured_cpe_product_classes[cpe_base] = set()
                cpe_class = 10 - sum(
                    cpe_field in ("*", "-", "") for cpe_field in cpe.split(":")
                )
                captured_cpe_product_classes[cpe_base].add(cpe_class)
            # elif not most_similar[query][0][0].startswith(cpe_base):
                ...
                for i, (cur_cpe, cur_sim_score) in enumerate(most_similar[query][1:]):
                    if sim_score > cur_sim_score:
                        ...
                        if not (
                            cpe_base in captured_cpe_product_classes
                            and cpe_class in captured_cpe_product_classes[cpe_base]
                        ):
                            #     break
                ...
                if insert_idx:
                    ...
                    if len(most_similar[query]) < count:
                        ...
                        if prev_cpe.startswith("cpe:"):  # filter out N/A and similar
                            prev_cpe_base = ":".join(prev_cpe.split(":")[:5]) + ":"
                            prev_cpe_class = 10 - sum(
                                cpe_field in ("*", "-", "") for cpe_field in prev_cpe.split(":")
                            )
                            captured_cpe_product_classes[prev_cpe_base].discard(prev_cpe_class)
                        # print('-'*80)
                        # print(cpe)
                        # print(prev_cpe)
                        # import pprint
