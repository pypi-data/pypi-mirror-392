...
def test_mm_output_structure_and_types():
    items_old = {"i": IRF({"a": 1.0, "b": 0.0}, "2PL")}
    items_new = {"j": IRF({"a": 2.0, "b": 1.0}, "2PL")}
    common_old = ["i"]
    common_new = ["j"]

    out = mean_mean(items_new, items_old, common_new, common_old)
...
def test_mm_raises_on_missing_keys():
    # Missing 'a' should raise KeyError at IRF construction
    with pytest.raises(KeyError, match="2PL model requires 'a' and 'b' parameters"):
        IRF({"b": 0.0}, "2PL")  # Missing 'a'
    
    # Missing 'b' should also raise KeyError at IRF construction
    with pytest.raises(KeyError, match="2PL model requires 'a' and 'b' parameters"):
