...
class TrainAction(Action):
    ...
    **Important notes when using train with is_tune=True:**
    ...
    def _override_best_trial(self, best_result):
        ...
        if not trial_id:
            self.run.log_message('Skipping override-best-trial request: trial_id missing.')
            return

        payload = {'trial_id': trial_id, **best_config}

        url = f'trains/{self.job_id}/override-best-trial/'
        self.run.log_message(f'Calling override-best-trial: {url} payload={payload}')
