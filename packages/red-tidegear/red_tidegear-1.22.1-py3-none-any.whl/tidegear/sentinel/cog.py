...
class SentinelCog(Cog):
            ...
            for user in await PartialUser.objects().where(PartialUser.user_id == user_id):
                await user.update_self(
                    values={
                        ...
                        PartialUser.last_updated: PartialUser.last_updated.default,
                    }
                )

        match requester:
            case "discord_deleted_user":  # Discord deleted the user and requested data deletion for the deleted user's account.
            ...
            case "user":  # A user requested their data be deleted, and the bot has 'strict' data deletion turned off.
                await self.sentinel_config.user_from_id(user_id).clear()
            case _:
    ...
    async def get_sentinel_type_config_with_fallback(self, guild: discord.Guild | int, moderation_type: ModerationType, attr: str) -> Any:
