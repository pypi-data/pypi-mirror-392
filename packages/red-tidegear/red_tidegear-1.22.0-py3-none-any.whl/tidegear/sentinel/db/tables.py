...
from discord import Guild, Member, Object, Role, User, abc
...
class AbstractPartial:
    ...
    @abstractmethod
    def in_guild(self, guild: Guild) -> bool:
...
class PartialRole(Table, AbstractPartial):
    ...
    Attributes: Columns:
        ...
        last_updated: The last time the role was upserted.
    """
    ...
    @override
    def discord_object(self) -> Object:
        return Object(id=self.role_id, type=Role)
    ...
    @overload
    async def fetch(self, bot: Red, *, fetch: Literal[True] = ..., upsert: bool = ...) -> Role: ...

    @overload
    async def fetch(self, bot: Red, *, fetch: Literal[False] = False, upsert: bool = ...) -> Role | None: ...

    async def fetch(self, bot: Red, *, fetch: bool = False, upsert: bool = True) -> Role | None:
    ...
    @classmethod
    async def upsert(cls, role: Role) -> Self:
        ...
        if fetched_role := await query:
            await fetched_role.update_self(values={cls.last_known_name: role.name, cls.last_updated: TimestamptzNow().python()})
