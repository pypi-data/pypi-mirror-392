...
try:
    ...
    def validate_xml(*_args, **_kwargs):
        return True, None


def generate_schema(*_args, **_kwargs):
    """Fallback schema generator used when legacy module is absent."""
    return {}

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).resolve().parents[5]
NEW_PROMPT_ROOT = PROJECT_ROOT / "apps" / "web_app" / "src" / "web_app" / "agents" / "prompts" / "prompts"
...
class PromptLoader:
    ...
    def _resolve_path(self, raw_path: str) -> Path:
        ...
        relative_parts = Path(raw_path)
        if relative_parts.parts[:2] == ("afasask", "prompts"):
            fallback = NEW_PROMPT_ROOT / Path(*relative_parts.parts[2:])
            if fallback.exists():
                return fallback
    ...
    def _is_inclusion_allowed(self, prompt_path: str) -> bool:
        ...
        """
        try:
            from afasask.agent.agent_config import is_inclusion_enabled, ENABLE_PROMPT_INCLUSIONS
    ...
    def _resolve_include_path(self, prompt_path: str, base_path: str = None) -> str:
        ...
        # Try relative to afasask/prompts/ (default behavior)
        if not prompt_path.startswith("afasask/prompts"):
            afasask_path = os.path.join("afasask/prompts", prompt_path)
            if os.path.exists(afasask_path):
                return afasask_path
        
        # If the path already starts with afasask/prompts, use it as-is
        if prompt_path.startswith("afasask/prompts"):
            return prompt_path
    ...
    def load(self, path: str, schema_gen_source: dict = None, skip_xml_validation: bool = False) -> Union['Prompts', str]:
        ...
        if not path.endswith(".json") and not path.endswith(".md"):
            # Ensure path starts with 'afasask/prompts/'
            if not path.startswith("afasask/prompts"):
                path = os.path.join("afasask/prompts", path)

            fallback: Path | None = None
            if not os.path.exists(path):
                path_obj = Path(path)
                if path_obj.parts[:2] == ("afasask", "prompts"):
                    fallback = NEW_PROMPT_ROOT / Path(*path_obj.parts[2:])
                else:
                    fallback = NEW_PROMPT_ROOT / path_obj
        ...
        # If path is a file
            ...
            if path.endswith(".json"):
                ...
                if not file_path.exists():
                    if file_path.parts[:2] == ("afasask", "prompts"):
                        fallback = NEW_PROMPT_ROOT / Path(*file_path.parts[2:])
                    else:
                        fallback = NEW_PROMPT_ROOT / file_path
                    if fallback and fallback.exists():
                        file_path = fallback
                    else:
                        raise FileNotFoundError(f"Prompt file not found: {path}")

                content = self.load_prompt(str(file_path))
                # Resolve any prompt includes in the content
                content = self._resolve_prompt_includes(content, base_path=os.path.dirname(str(file_path)))
                return content
...
try:
    base_info_prompt = PromptLoader().load_prompt(os.path.join("afasask/prompts", "base_info.md"))
except FileNotFoundError:
    logger.warning("Base info prompt not found; continuing with empty fallback.")
    base_info_prompt = ""
...
class Prompts:
    ...
    def format(self, skip_xml_validation: bool = False, **kwargs) -> 'Prompts':
        ...
        logger.info(f"Formatting prompts with kwargs: {kwargs.keys()}")
