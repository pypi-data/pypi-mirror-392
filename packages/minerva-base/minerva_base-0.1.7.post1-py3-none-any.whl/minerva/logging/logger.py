...
from minerva.settings import DEV
...
class SizedLogBuffer:
    def __init__(
        ...
        The buffer can be overwritten by an env variable MINERVA_LOG_RETRIEVER_BUFFER_SIZE
    ...
    def max(self) -> int:
        ...
        if self._max == 0:
            env_buffer_size = os.getenv("MINERVA_LOG_RETRIEVER_BUFFER_SIZE", "0")
...
def configure(
    *,
    ...
    if disable and log_level is None and log_file is None:
        logger.disable("minerva")
    if os.getenv("MINERVA_LOG_LEVEL", "").upper() in VALID_LOG_LEVELS and log_level is None:
        log_level = os.getenv("MINERVA_LOG_LEVEL")
    if log_level is None:
        log_level = "ERROR"

    if log_file is None:
        env_log_file = os.getenv("MINERVA_LOG_FILE", "")
        log_file = Path(env_log_file) if env_log_file else None

    if log_env is None:
        log_env = os.getenv("MINERVA_LOG_ENV", "")
    ...
    if log_env.lower() == "container" or log_env.lower() == "container_json":
        ...
        if os.getenv("MINERVA_LOG_FORMAT") and log_format is None:
            log_format = os.getenv("MINERVA_LOG_FORMAT")
        ...
        if not log_file:
            ...
            log_file = cache_dir / "minerva.log"
