...
from minerva.services.auth.utils import get_current_active_user
from minerva.services.database.models import User
...
from minerva.services.database.models.vertex_builds.model import VertexBuildTable
from minerva.services.deps import get_session, session_scope
from minerva.services.store.utils import get_lf_version_from_pypi

if TYPE_CHECKING:
    from minerva.services.chat.service import ChatService
    from minerva.services.store.schema import StoreComponentCreate
...
async def check_minerva_version(component: StoreComponentCreate) -> None:
    ...
    minerva_version = await get_lf_version_from_pypi()
    if minerva_version is None:
        raise HTTPException(status_code=500, detail="Unable to verify the latest version of Minerva")
    if minerva_version != component.last_tested_version:
        logger.warning(
            f"Your version of Minerva ({component.last_tested_version}) is outdated. "
            f"Please update to the latest version ({minerva_version}) and try again."
        )
