...
from minerva.field_typing.constants import CUSTOM_COMPONENT_SUPPORTED_TYPES, DEFAULT_IMPORT_STRING
...
def create_class(code, class_name):
    ...
    # Replace from minerva import CustomComponent with from minerva.custom import CustomComponent
    code = code.replace("from minerva import CustomComponent", "from minerva.custom import CustomComponent")
    code = code.replace(
...
def get_default_imports(code_string):
    ...
    minerva_imports = list(CUSTOM_COMPONENT_SUPPORTED_TYPES.keys())
    necessary_imports = find_names_in_code(code_string, minerva_imports)
    minerva_module = importlib.import_module("minerva.field_typing")
    default_imports.update({name: getattr(minerva_module, name) for name in necessary_imports})
