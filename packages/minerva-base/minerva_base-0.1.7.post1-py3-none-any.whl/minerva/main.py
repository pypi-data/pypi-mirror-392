...
from minerva.api import health_check_router, log_router, router, router_v2
...
from minerva.logging.logger import configure
from minerva.middleware import ContentSizeLimitMiddleware
from minerva.services.deps import get_settings_service, get_telemetry_service
from minerva.services.utils import initialize_services, teardown_services
...
def get_lifespan(*, fix_migration=False, version=None):
    ...
    async def lifespan(_app: FastAPI):
        ...
        # Startup message
        if version:
            rprint(f"[bold green]Starting Minerva v{version}...[/bold green]")
        else:
            rprint("[bold green]Starting Minerva...[/bold green]")
        ...
        try:
            ...
            if "minerva migration --fix" not in str(exc):
            ...
            # Final message
            rprint("[bold red]Minerva shutdown complete[/bold red]")
...
def create_app():
    ...
    app = FastAPI(lifespan=lifespan, title="Minerva", version=__version__)
    app.add_middleware(
    ...
    if prome_port_str := os.environ.get("MINERVA_PROMETHEUS_PORT"):
    ...
    if settings.mcp_server_enabled:
        from minerva.api.v1 import mcp_router
...
def get_static_files_dir():
    """Get the static files directory relative to Minerva's main.py file."""
...
if __name__ == "__main__":
    ...
    from minerva.__main__ import get_number_of_workers

    configure()
    uvicorn.run(
        "minerva.main:create_app",
        host="127.0.0.1",
        port=7860,
