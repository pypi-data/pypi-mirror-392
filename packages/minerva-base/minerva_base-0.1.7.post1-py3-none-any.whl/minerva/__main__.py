...
from minerva.initial_setup.setup import get_or_create_default_folder
from minerva.logging.logger import configure, logger
from minerva.main import setup_app
from minerva.services.database.utils import session_getter
from minerva.services.deps import get_db_service, get_settings_service, session_scope
from minerva.services.settings.constants import DEFAULT_SUPERUSER
from minerva.services.utils import initialize_services
from minerva.utils.version import fetch_latest_version, get_version_info
...
def run_on_mac_or_linux(host, port, log_level, options, app):
    webapp_process = Process(target=run_minerva, args=(host, port, log_level, options, app))
...
def run_on_windows(host, port, log_level, options, app) -> None:
    """Run the Minerva server on Windows."""
    print_banner(host, port)
    run_minerva(host, port, log_level, options, app)
...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    Example:
        >>> build_version_notice("1.0.0", "minerva")
        'A new version of minerva is available: 1.1.0'
    """
    with suppress(httpx.ConnectError):
        latest_version = fetch_latest_version(package_name, include_prerelease=minerva_is_pre_release(current_version))
        if latest_version and pkg_version.parse(current_version) < pkg_version.parse(latest_version):
            release_type = "pre-release" if minerva_is_pre_release(latest_version) else "version"
...
def print_banner(host: str, port: int) -> None:
    ...
    # Use minerva.utils.version to get the version info
    version_info = get_version_info()
    minerva_version = version_info["version"]
    package_name = version_info["package"]
    is_pre_release |= minerva_is_pre_release(minerva_version)  # Update pre-release status

    notice = build_version_notice(minerva_version, package_name)
    ...
    info_text = (
        ...
        "[bold][link=https://github.com/digitranslab/minerva]GitHub Repo[/link][/bold] :star2:"
    )
    telemetry_text = (
...
def run_minerva(host, port, log_level, options, app) -> None:
    """Run Minerva server on localhost."""
    if platform.system() == "Windows":
        import uvicorn
        ...
        from minerva.server import MinervaApplication

        server = MinervaApplication(app, options)
...
def superuser(
    ...
    log_level: str = typer.Option("error", help="Logging level.", envvar="MINERVA_LOG_LEVEL"),
) -> None:
    """Create a superuser."""
    ...
    async def _create_superuser():
        ...
        async with session_getter(db_service) as session:
            from minerva.services.auth.utils import create_super_user
...
# command to copy the minerva database from the cache to the current directory
# because now the database is stored per installation
@app.command()
def copy_db() -> None:
    """Copy the database files to the current directory.

    This function copies the 'minerva.db' and 'minerva-pre.db' files from the cache directory to the current
    directory.
    ...
    db_path = cache_dir / "minerva.db"
    pre_db_path = cache_dir / "minerva-pre.db"
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            if not superuser:
                ...
                return None
            from minerva.services.database.models.api_key import ApiKey, ApiKeyCreate
            from minerva.services.database.models.api_key.crud import create_api_key, delete_api_key
