...
from minerva.utils.util_strings import is_valid_database_url
...
class Settings(BaseSettings):
    # Define the default MINERVA_DIR
    config_dir: str | None = None
    # Define if minerva db should be saved in config dir or
    # in the minerva directory
    save_db_in_config_dir: bool = False
    """Define if minerva database should be saved in MINERVA_CONFIG_DIR or in the minerva directory
    (i.e. in the package directory)."""

    dev: bool = False
    """If True, Minerva will run in development mode."""
    database_url: str | None = None
    """Database URL for Minerva. If not provided, Minerva will use a SQLite database."""
    database_connection_retry: bool = False
    """If True, Minerva will retry to connect to the database if it fails."""
    pool_size: int = 10
    ...
    """If set to True, Minerva will expose Prometheus metrics."""
    prometheus_port: int = 9090
    """The port on which Minerva will expose Prometheus metrics. 9090 is the default port."""
    ...
    store_url: str | None = "https://api.minerva.store"
    download_webhook_url: str | None = "https://api.minerva.store/flows/trigger/ec611a61-8460-4438-b187-a4f65e5559d4"
    like_webhook_url: str | None = "https://api.minerva.store/flows/trigger/64275852-ec00-45c1-984e-3bff814732da"
    ...
    with the same name in case Minerva fails to retrieve the variable value."""
    ...
    user_agent: str = "minerva"
    """User agent for the API calls."""
    backend_only: bool = False
    """If set to True, Minerva will not serve the frontend."""

    # Telemetry
    do_not_track: bool = False
    """If set to True, Minerva will not track telemetry."""
    telemetry_base_url: str = "https://minerva.gateway.scarf.sh"
    transactions_storage_enabled: bool = True
    """If set to True, Minerva will track transactions between flows."""
    vertex_builds_storage_enabled: bool = True
    """If set to True, Minerva will keep track of each vertex builds (outputs) in the UI for any flow."""

    # Config
    host: str = "127.0.0.1"
    """The host on which Minerva will run."""
    port: int = 7860
    """The port on which Minerva will run."""
    workers: int = 1
    """The number of workers to run."""
    log_level: str = "critical"
    """The log level for Minerva."""
    log_file: str | None = "logs/minerva.log"
    """The path to log file for Minerva."""
    ...
    """If set to True, Minerva will open the browser on startup."""
    auto_saving: bool = True
    """If set to True, Minerva will auto save flows."""
    auto_saving_interval: int = 1000
    """The interval in ms at which Minerva will auto save flows."""
    ...
    """If set to False, Minerva will not enable the MCP server."""
    mcp_server_enable_progress_notifications: bool = False
    """If set to False, Minerva will not send progress notifications in the MCP server."""

    @field_validator("dev")
    @classmethod
    def set_dev(cls, value):
        from minerva.settings import set_dev
    ...
    def set_minerva_dir(cls, value):
        if not value:
            ...
            # Create a .minerva directory inside the cache directory
    ...
    def set_database_url(cls, value, info):
        ...
        logger.debug("No database_url provided, trying MINERVA_DATABASE_URL env variable")
        if minerva_database_url := os.getenv("MINERVA_DATABASE_URL"):
            value = minerva_database_url
            logger.debug("Using MINERVA_DATABASE_URL env variable.")
        else:
            logger.debug("No database_url env variable, using sqlite database")
            # Originally, we used sqlite:///./minerva.db
            ...
            from minerva.utils.version import is_pre_release as minerva_is_pre_release

            version = get_version_info()["version"]
            is_pre_release = minerva_is_pre_release(version)
            ...
            if info.data["save_db_in_config_dir"]:
                ...
                logger.debug(f"Saving database to minerva directory: {database_dir}")

            pre_db_file_name = "minerva-pre.db"
            db_file_name = "minerva.db"
    ...
    def set_components_path(cls, value):
        if os.getenv("MINERVA_COMPONENTS_PATH"):
            logger.debug("Adding MINERVA_COMPONENTS_PATH to components_path")
            minerva_component_path = os.getenv("MINERVA_COMPONENTS_PATH")
            if Path(minerva_component_path).exists() and minerva_component_path not in value:
                if isinstance(minerva_component_path, list):
                    for path in minerva_component_path:
                        if path not in value:
                            value.append(path)
                    logger.debug(f"Extending {minerva_component_path} to components_path")
                elif minerva_component_path not in value:
                    value.append(minerva_component_path)
                    logger.debug(f"Appending {minerva_component_path} to components_path")
    ...
    model_config = SettingsConfigDict(validate_assignment=True, extra="ignore", env_prefix="MINERVA_")
