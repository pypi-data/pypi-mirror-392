...
if TYPE_CHECKING:
    from minerva.services.store.schema import ListComponentResponse
...
# Get the latest released version of minerva (https://pypi.org/project/minerva/)
async def get_lf_version_from_pypi():
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get("https://pypi.org/pypi/minerva/json")
        if response.status_code != httpx.codes.OK:
            return None
        return response.json()["info"]["version"]
    except Exception:  # noqa: BLE001
        logger.opt(exception=True).debug("Error getting the latest version of minerva from PyPI")
        return None
