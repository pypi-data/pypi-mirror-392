...
from minerva.services.task.backends.anyio import AnyIOBackend
from minerva.services.task.utils import get_celery_worker_status

if TYPE_CHECKING:
    from minerva.services.settings.service import SettingsService
    from minerva.services.task.backends.base import TaskBackend


def check_celery_availability():
    try:
        from minerva.worker import celery_app
...
class TaskService(Service):
    ...
    def get_backend(self) -> TaskBackend:
        if self.use_celery:
            from minerva.services.task.backends.celery import CeleryBackend
