...
class AIAdapter:
    ...
    async def stream_response(self, message: str, file_references: List[str] = None) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        """
        print(f"🔥 stream_response方法被调用！消息: {message[:30]}...")
        # 重置思考状态
        ...
        try:
            print(f"🚀 开始流式响应，输入: {full_input[:100]}...")
            chunk_count = 0
            # 流式响应
            for chunk in self.agent.stream(
                ...
                chunk_count += 1
                print(f"📦 收到chunk #{chunk_count}: {type(chunk)}")

                # 处理流式数据块
                processed_chunk = self._process_stream_chunk(chunk)
                if processed_chunk:
                    print(f"✅ 处理chunk成功: {processed_chunk.get('type', 'unknown')}")
                    yield processed_chunk
                else:
                    print(f"⚪ chunk处理返回None")

            print(f"🏁 流式响应结束，共处理 {chunk_count} 个chunk")
            ...
            print(f"❌ stream_response 中发生错误: {e}")
            print(f"❌ 错误类型: {type(e)}")
            import traceback
            traceback.print_exc()
    ...
    def _process_stream_chunk(self, chunk) -> Optional[Dict]:
        ...
        print(f"🔍 chunk详情: type={type(chunk)}, len={len(chunk) if hasattr(chunk, '__len__') else 'N/A'}")
        if not isinstance(chunk, tuple) or len(chunk) < 3:
            print(f"⚠️ chunk格式不正确，跳过: {chunk}")
            return None

        namespace, stream_mode, data = chunk[:3]  # 只取前3个元素
        print(f"🔍 chunk结构: namespace={namespace}, stream_mode={stream_mode}, data_type={type(data)}")
        current_time = time.time()
        results = []

        if stream_mode == "messages":
            print(f"📋 处理messages模式")
                ...
                if hasattr(message, 'content_blocks'):
                    ...
                    for block in message.content_blocks:
                        ...
                        if not isinstance(block, dict):
                            print(f"⚠️ block类型错误: {type(block)}, 内容: {block}")
                            continue
                        ...
                        if block.get("type") == "tool_call_chunk":
                            ...
                            if not isinstance(tool_args, (dict, str)):
                                print(f"⚠️ tool_args类型错误: {type(tool_args)}, 转换为字符串")
                            ...
                            if tool_args:
                                buffer["args"] += str(tool_args)  # 确保转换为字符串
            ...
            if isinstance(data, dict):
                if "__interrupt__" in data:
                    ...
                    # 安全检查interrupt_data类型
                    if interrupt_data is None:
                        interrupt_data = {}
                    elif isinstance(interrupt_data, tuple):
                        print(f"🔧 interrupt_data是tuple，尝试提取内容: {interrupt_data}")
                        # 尝试从tuple中提取有效数据
                        if len(interrupt_data) > 0 and hasattr(interrupt_data[0], '__dict__'):
                            interrupt_data = interrupt_data[0].__dict__.get('value', {})
                        else:
                            interrupt_data = {}
                    elif not isinstance(interrupt_data, dict):
                        print(f"⚠️ interrupt_data类型错误: {type(interrupt_data)}, 内容: {interrupt_data}")
                        interrupt_data = {}
