...
from cli.config.subagent import defect_analyzer_subagent, code_fixer_subagent, fix_validator_subagent
...
from cli.midware.agent_memory import AgentMemoryMiddleware
from cli.midware.performance_monitor import PerformanceMonitorMiddleware
from cli.midware.layered_memory import LayeredMemoryMiddleware
from cli.midware.context_enhancement import ContextEnhancementMiddleware
from cli.midware.security import SecurityMiddleware
from cli.midware.logging import LoggingMiddleware
from cli.midware.memory_adapter import MemoryMiddlewareFactory
...
def create_agent_with_config(model, assistant_id: str, tools: list, memory_mode: str = "auto"):
    ...
    # 建中间件管道 (暂时移除SubAgentMiddleware以调试工具问题)
    agent_middleware = []
    ...
    # 临时使用简单的LangChain代理替代deepagents以避免SubAgentMiddleware问题
    try:
        ...
        print(f"✅ 使用简化代理架构创建成功")

    except Exception as e:
        print(f"❌ 创建代理失败: {e}")
        # 作为最后的备用方案，返回一个模拟的agent
        class MockAgent:
            def __init__(self):
                self.checkpointer = InMemorySaver()
                self.stream = self._mock_stream
                self.with_config = lambda x: self

            async def _mock_stream(self, inputs, config):
                yield {"type": "message", "content": "代理初始化失败，使用备用响应", "session_id": "mock"}
                yield {"type": "status", "content": "响应完成", "session_id": "mock", "metadata": {"state": "completed"}}
