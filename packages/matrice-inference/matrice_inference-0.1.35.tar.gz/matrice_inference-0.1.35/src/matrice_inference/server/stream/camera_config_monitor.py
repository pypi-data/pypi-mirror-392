...
class CameraConfigMonitor:
    ...
    def notify_refresh_completed(self, camera_configs: Dict[str, CameraConfig]) -> None:
        ...
        ALL cameras currently in the pipeline AND in the refresh event are marked as
        refresh-managed. This includes:
        - Cameras in the refresh event (added/updated)
        - Cameras in the pipeline but NOT in refresh event (implicitly removed)
        ...
        Args:
            camera_configs: Dictionary of camera_id -> CameraConfig from refresh event
        """
        try:
            # Get current cameras in the pipeline (before refresh reconciliation)
            current_cameras_in_pipeline = set(self.streaming_pipeline.camera_configs.keys())
            ...
            with self._refresh_lock:
                # Cameras in the refresh event
                refresh_event_cameras = set(camera_configs.keys())

                # Cameras that will be removed (in pipeline but not in refresh event)
                removed_cameras = list(current_cameras_in_pipeline - refresh_event_cameras)

                # Mark ALL relevant cameras as refresh-managed:
                # 1. Cameras in refresh event (explicit management)
                # 2. Cameras in pipeline but not in refresh (implicit removal - prevent re-add)
                all_cameras_to_manage = current_cameras_in_pipeline | refresh_event_cameras
                ...
                self.logger.warning(
                    ...
                    f"{len(current_cameras_in_pipeline)} currently in pipeline, "
                    f"{len(removed_cameras)} will be removed, "
            ...
            with self._hashes_lock:
                # Clear old hashes and build new cache from refresh event
            ...
            self.logger.info(
                f"✓ Refresh notification complete: updated hash cache for {len(camera_configs)} cameras, "
