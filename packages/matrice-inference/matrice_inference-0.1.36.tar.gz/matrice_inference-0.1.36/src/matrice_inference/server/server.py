    ...
    def _initialize_streaming_pipeline(self):
        ...
        try:
            ...
            if not app_deployment_id:
                ...
                # DISABLED: Initial camera fetching - system now waits for refresh events
                # Refresh events are the PRIMARY and ONLY source of camera configurations
                # All cameras will be managed exclusively through refresh events
                ...
                # Start with empty configs - wait for first refresh event to configure cameras
                camera_configs = {}
                logging.info(
                    "Starting pipeline with 0 cameras (waiting for refresh events). "
                    "Refresh listener is the PRIMARY source of camera configurations."
                )
