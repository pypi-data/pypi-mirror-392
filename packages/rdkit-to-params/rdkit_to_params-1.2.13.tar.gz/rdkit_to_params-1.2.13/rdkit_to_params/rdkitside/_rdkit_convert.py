...
class _RDKitCovertMixin(_RDKitPrepMixin):
    ...
    def _get_measurements(self, conf: Chem.Conformer, a: Chem.Atom, b: Chem.Atom, c: Chem.Atom, d: Chem.Atom) -> Any:
        ai = a.GetIdx()
        bi = b.GetIdx()
        ci = c.GetIdx()
        di = d.GetIdx()
    ...
    def _parse_atom(self, atom: Chem.Atom, pcharge_prop_name: str = '_GasteigerCharge') -> None:
        ...
        if atom.GetAtomicNum() == 0: # * (Chem.Atom) or R (Chem.QueryAtom)
            ...
            d = self._get_atom_descriptors(atom, pcharge_prop_name)  # dict of 'name', 'rtype': 'mtype', 'partial'
            self.ATOM.append(d)
    ...
    def _get_unseen_neighbors(self, atom: Chem.Atom, seen: List[Chem.Atom], nondummy: bool = True) -> List[Chem.Atom]:
    ...
    def _get_atom_descriptors(self, atom: Chem.Atom, pcharge_prop_name: str) -> Dict[str, Union[str, float]]:
    ...
    def write_conformers(self, filename: str) -> None:
        ...
        """
        assert self.mol.GetNumConformers() > 1, 'Molecule needs multiple conformers for `PDB_ROTAMERS'
