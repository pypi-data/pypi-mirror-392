...
class DummyMasker:
    ...
    def unmask(self):
        for dummy in self.dummies:
            assert dummy.HasProp('dummy'), 'The atoms have changed because of new protons: use `make_masked` and `unmask_without_proton`'
    ...
    def unmask_without_proton(cls, mol: Chem.Mol):
        ...
        for idx, atom in enumerate(mol.GetAtoms()):
            if atom.HasProp('dummy') and atom.GetBoolProp('dummy'):
                dummy = atom
                dummy.SetAtomicNum(0)
                hydrogen_idxs = [neigh.GetIdx() for neigh in dummy.GetNeighbors() if neigh.GetSymbol() == 'H']
