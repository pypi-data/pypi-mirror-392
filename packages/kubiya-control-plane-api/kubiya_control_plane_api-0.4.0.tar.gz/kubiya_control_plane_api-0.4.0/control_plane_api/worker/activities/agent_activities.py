    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 404:
                ...
                # Return success to allow workflow to complete gracefully
                print(f"⚠️  Execution not found (may have been deleted): {input.execution_id}\n")

                activity.logger.warning(
                    f"Execution not found - skipping status update",
                    extra={
                        "execution_id": input.execution_id,
                        "status": input.status,
                    }
                ...
                return {
                    ...
                    "message": f"Execution {input.execution_id} not found"
                }
