...
def create_team(
            ...
            for agent_id in team_data.configuration.member_ids:
                try:
                    ...
                    if result and result.data:
                        agent_runtime = result.data.get("runtime", "default")
                        agent_name = result.data.get("name", agent_id)
    ...
    response_team = TeamResponse(
        ...
        status=team.status,
        runtime=team.runtime or "default",  # Include runtime in response
        model_id=team.model_id,  # Include model_id in response
        ...
        environments=[],  # Environments will be loaded separately if needed
    )
        ...
        for team in teams:
            try:
                ...
                result.append(TeamWithAgentsResponse(
                    ...
                    skill_ids=skill_ids,
                    skills=skills,
                    environments=[],  # TODO: Load team environments from relationship
                ))
