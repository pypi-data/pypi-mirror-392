...
class Field(BaseField):
    ...
    def __init__(self, dtype: DataType, default: Optional[Any] = missing, name: Optional[str] = None) -> None:
...
class Struct(IRValue):
    ...
    def __setattrjit__(self, name: str, value: RuntimeNumeric) -> None:
        field = self.get_field(name)
        ir_name = field.ir_name(name)
        global_builder.get_ir_builder().create_emitasc_SetMemberOp(self.to_ir(), ir_name, field.to_ir(value))
