...
from ..core.enums import CubeFormat, Position, LayoutMode, BatchMode, IterateOrder, ScheduleType, MatmulConfigMode
...
class Matmul(IRValue):
    ...
    def __init__(self, a: MatmulType, b: MatmulType, c: MatmulType, bias: Optional[MatmulType] = None,
                 matmul_config: Optional[MatmulConfig] = None) -> None:
        ...
    ...
    def __init__(self, a: Optional[MatmulType] = None, b: Optional[MatmulType] = None, c: Optional[MatmulType] = None,
                 bias: Optional[MatmulType] = None, handle: Optional[IRHandle] = None,
        ...
        ir_type = builder.get_matmul_type(a.position, a.format, a.dtype.to_ir(), a.is_trans, a.layout,  # a
                                          ...
                                          bias_pos, bias_format, bias_type.to_ir())
    ...
    def iterate_batch(self, *args, **kwargs) -> None:
        ...
        @dispatcher.register(tensor=BaseTensor, batch_a=RuntimeInt, batch_b=RuntimeInt, en_sequential_write=RuntimeBool,
            ...
            matrix_stride_c=DefaultValued(RuntimeInt, 0), en_partial_sum=DefaultValued(RuntimeBool, False),
            en_atomic=DefaultValued(RuntimeInt, 0), sync=DefaultValued(RuntimeBool, True),
        ...
        def _(tensor: BaseTensor, batch_a: RuntimeInt, batch_b: RuntimeInt, en_sequential_write: RuntimeBool,
              ...
              en_partial_sum: RuntimeBool = False, en_atomic: RuntimeInt = 0, sync: RuntimeBool = True,
              wait_iterate_batch: Optional[RuntimeBool] = None):
        ...
        def _(tensor: BaseTensor, en_partial_sum: RuntimeBool, en_atomic: RuntimeInt, en_sequential_write: RuntimeBool,
            ...
            builder.create_asc_MatmulIterateBatchCubeOnlyOp(
                ...
                matrix_stride_a_value, matrix_stride_b_value, matrix_stride_c_value, sync_value)
...
def get_matmul_api_tiling(mm_cfg: MatmulConfig, l1_size: RuntimeInt, a_type: MatmulType, b_type: MatmulType, 
                            c_type: MatmulType, bias_type: Optional[MatmulType] = None) -> MatmulApiStaticTiling:
    builder = global_builder.get_ir_builder()
    if bias_type is not None:
        bias_pos = bias_type.position
        bias_format = bias_type.format
        bias_type = bias_type.dtype
    else:
        bias_pos = c_type.position
        bias_format = c_type.format
        bias_type = c_type.dtype
    ...
    builder.create_asc_MatmulGetMatmulApiTilingOp(builder.get_asc_MatmulApiStaticTilingType(),
                                                mm_cfg.to_ir(), _mat(l1_size, KT.int32).to_ir(), ir_type)


@overload
def get_basic_config(self, basic_m: int, basic_n: int, basic_k: int, intrinsics_limit: Optional[bool] = False,
                     batch_loop: Optional[bool] = False, bmm_mode: Optional[BatchMode] = 1) -> MatmulConfig:
    ...


@require_jit
def get_basic_config(self, basic_m: RuntimeInt, basic_n: RuntimeInt, basic_k: RuntimeInt,
    ...
    if bmm_mode is None:
        bmm_mode = 1
    check_type(bmm_mode, [1, 2, 3], "Params bmm_mode should be in [1, 2, 3]")
...
def get_special_basic_config(basic_m: RuntimeInt, basic_n: RuntimeInt, basic_k: RuntimeInt,
                             ...
                             batch_loop: RuntimeBool = False, bmm_mode: BatchMode = 1) -> MatmulConfig:
    if intrinsics_limit is None:
        intrinsics_limit = False
    if batch_loop is None:
        batch_loop = False
...
def get_normal_config(intrinsics_limit: Optional[bool] = False, batch_loop: Optional[bool] = False,
                      is_vec_nd2_nz: Optional[bool] = False, bmm_mode: Optional[BatchMode] = 1,
                      is_msg_reuse: Optional[bool] = True, iterate_order: Optional[IterateOrder] = 0,
                      schedule_type: Optional[ScheduleType] = 0, en_unit_flag: Optional[bool] = True) -> MatmulConfig:
    ...


@require_jit
def get_normal_config(intrinsics_limit: RuntimeBool = False, batch_loop: RuntimeBool = False,
                      is_vec_nd2_nz: RuntimeBool = False, bmm_mode: BatchMode = 1, is_msg_reuse: RuntimeBool = True,
                      iterate_order: IterateOrder = 0, schedule_type: ScheduleType = 0,
                      en_unit_flag: RuntimeBool = True) -> MatmulConfig:
    if intrinsics_limit is None:
        intrinsics_limit = False
    if batch_loop is None:
        batch_loop = False
    ...
    mm_config = MatmulConfig(intrinsics_check=intrinsics_limit, is_n_batch=batch_loop, vec_nd2nz=is_vec_nd2_nz,
                             batch_mode=bmm_mode, enable_reuse=is_msg_reuse, iterate_order=iterate_order,
                             schedule_type=schedule_type, unit_flag=en_unit_flag)
    return mm_config
...
def get_mdl_config(intrinsics_limit: RuntimeBool = False, batch_loop: RuntimeBool = False,
    ...
    if is_msg_reuse is None:
        is_msg_reuse = True
    if enable_ub_reuse is None:
        enable_ub_reuse = True
    if enable_l1_cache_ub is None:
        enable_l1_cache_ub = False
    if enable_mix_dual_master is None:
        enable_mix_dual_master = False
    if enable_kdim_reorder_load is None:
        enable_kdim_reorder_load = False
    mm_config = MatmulConfig(intrinsics_check=intrinsics_limit, is_n_batch=batch_loop,
                             do_mte2_preload=do_mte2_preload, vec_nd2nz=is_vec_nd2_nz, is_per_tensor=is_per_tensor,
                             has_anti_quant_offset=has_anti_quant_offset, unit_flag=en_unit_flag,
...
def get_special_mdl_config(intrinsics_limit: RuntimeBool = False, batch_loop: RuntimeBool = False,
    ...
    if has_anti_quant_offset is None:
        has_anti_quant_offset = False
    mm_config = MatmulConfig(intrinsics_check=intrinsics_limit, is_n_batch=batch_loop,
                             do_mte2_preload=do_mte2_pre_load, vec_nd2nz=is_vec_nd2_nz, is_per_tensor=is_per_tensor,
...
def get_ib_share_norm_config(intrinsics_limit: RuntimeBool = False, batch_loop: RuntimeBool = False,
                             is_vec_nd2_nz: RuntimeBool = False, bmm_mode: BatchMode = 1,
    ...
    if en_unit_flag is None:
        en_unit_flag = True
    mm_config = MatmulConfig(intrinsics_check=intrinsics_limit, is_n_batch=batch_loop, vec_nd2nz=is_vec_nd2_nz,
                             batch_mode=bmm_mode, enable_double_cache=is_double_cache, unit_flag=en_unit_flag)
    check_type(bmm_mode, [1, 2, 3], "Params bmm_mode should be in [1, 2, 3]")
    return mm_config
...
def get_mm_config(*args, **kwargs) -> MatmulConfig:
    ...
    enable_l1_cache_ub = False
    do_mte2_preload = 0
    iterate_order = IterateOrder.ORDER_M.value
    schedule_type = ScheduleType.INNER_PRODUCT.value
    enable_reuse = False
    ...
    return MatmulConfig(single_core_m=single_core_m, single_core_n=single_core_n, single_core_k=single_core_k,
                        ...
                        do_special_mdl=special_mdl, do_ibshare_norm=ib_share)
