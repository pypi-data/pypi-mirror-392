...
class Launcher:
    ...
    def run(self, kernel_binary: Optional[bytes], core_type: rt.CoreType, enable_debug: bool, function_name: str,
            user_args: Tuple[Any]) -> None:
        dry_run = os.environ.get('DRY_RUN')
        if dry_run:
            return
        if not isinstance(kernel_binary, bytes):
            raise RuntimeError("Compiled binary is required to launch the kernel")
        input_args = list(user_args)
        if enable_debug:
            input_args.append(np.zeros(utils.TOTAL_DUMP_SIZE, dtype=np.int8))
        kernel_args = self.expand_kernel_args(tuple(input_args))
        kernel = rt.register_device_binary_kernel(kernel_binary, rt.magic_elf_value(core_type))
        function = rt.register_function(kernel, function_name, mode=0)
        if self.options.core_num <= 0:
            raise ValueError("Core number should be large than 0")
        self.launch_kernel(function, kernel_args, enable_debug, function_name, core_type)
        rt.free_mem()
