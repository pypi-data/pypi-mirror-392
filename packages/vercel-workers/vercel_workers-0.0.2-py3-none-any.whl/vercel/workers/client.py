...
def _get_queue_token(explicit_token: str | None = None) -> str:
    """
    Resolve the token used to authenticate with the queue service.

    The TypeScript client prefers an explicit token and otherwise falls back
    to an OIDC token provided by the Vercel Functions environment.
    For now the Python client supports:
    ...
    msg = (
        "Failed to resolve queue token. Provide 'token' explicitly when calling send(), or set "
        "the VERCEL_QUEUE_TOKEN environment variable."
    )
...
def send(
    ...
    *,
    ...
    """
    Send a message to a Vercel Queue.

    This function mirrors the semantics of the TypeScript QueueClient.sendMessage method,
    adapted for Python and using httpx under the hood.
    ...
    Raises:
        RuntimeError: When authentication fails or an unexpected server error occurs.
        ValueError: When the API reports invalid parameters (HTTP 400).
        PermissionError: When authentication or authorisation fails (HTTP 401/403).
        httpx.HTTPStatusError: For other non-success HTTP responses.
    """
