...
def _get_queue_token(explicit_token: str | None = None) -> str:
    ...
    This helper is used by the synchronous ``send`` function, which cannot
    safely await the asynchronous OIDC helper.
    """
    if explicit_token:
    ...
    msg = (
        ...
        "or set the VERCEL_QUEUE_TOKEN environment variable.",
    )
