...
def benchmark_with_deduplication(nodes_file, rels_file, expected_unique_nodes):
    ...
    snapshot_opt = manager.get_graph_snapshot("dedup_test_v1")
    ...
    print(f"Unique nodes in graph: {snapshot.n_nodes():,}")
    print(f"Expected unique nodes: ~{expected_unique_nodes:,}")
    print(f"Relationships in file: {len(rel_ids):,}")
    print(f"Unique relationships in graph: {snapshot.n_rels():,}")
    print(f"\nTiming:")
...
def benchmark_without_deduplication(nodes_file, rels_file):
    ...
    snapshot_opt = manager.get_graph_snapshot("baseline_test_v1")
    if snapshot_opt is None:
        raise RuntimeError("Failed to retrieve snapshot")
    snapshot = snapshot_opt
    ...
    print(f"{'='*70}")
    print(f"Nodes in file: {len(node_ids):,}")
    print(f"Nodes in graph: {snapshot.n_nodes():,}")
    print(f"Relationships in file: {len(rel_ids):,}")
    print(f"Relationships in graph: {snapshot.n_rels():,}")
    print(f"\nTiming:")
...
def main():
    ...
    try:
        ...
        if not args.baseline_only and not args.dedup_only:
            ...
            print(f"  Nodes: {baseline_snapshot.n_nodes():,}")
            print(f"  Relationships: {baseline_snapshot.n_rels():,}")
            print(f"  Total time: {baseline_time:.2f}s")
            print(f"\nWith deduplication:")
            print(f"  Nodes: {dedup_snapshot.n_nodes():,}")
            print(f"  Relationships: {dedup_snapshot.n_rels():,}")
            print(f"  Total time: {dedup_time:.2f}s")
            print(f"\nDeduplication impact:")
            print(f"  Node reduction: {baseline_snapshot.n_nodes() - dedup_snapshot.n_nodes():,} nodes ({((baseline_snapshot.n_nodes() - dedup_snapshot.n_nodes()) / baseline_snapshot.n_nodes() * 100):.1f}%)")
            print(f"  Relationship reduction: {baseline_snapshot.n_rels() - dedup_snapshot.n_rels():,} rels ({((baseline_snapshot.n_rels() - dedup_snapshot.n_rels()) / baseline_snapshot.n_rels() * 100):.1f}%)")
