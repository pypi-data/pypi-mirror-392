...
class ENTRY(object):
    ...
    def add_task(self, name: str, script: str, schedule: str = "DAILY", time: str = "12:00", hidden: bool = True):
        ...
        try:
            ...
            settings.StartWhenAvailable = False  # 如果错过了计划时间，则在下次可用时也不能运行
            ...
            if script_ext == '.ps1':
                # 创建 VBScript 包装器来隐藏窗口
                vbs_wrapper_path = os.path.join(config_dir, f"{name}_wrapper.vbs")
                vbs_content = f'''Set objShell = CreateObject("WScript.Shell")
objShell.Run "powershell.exe -ExecutionPolicy Bypass -NoProfile -NonInteractive -WindowStyle Hidden -File ""{final_script_path}""", 0, False
Set objShell = Nothing'''
                
                with open(vbs_wrapper_path, 'w', encoding='utf-8') as f:
                    f.write(vbs_content)
                
                action.Path = "wscript.exe"
                action.Arguments = f'"{vbs_wrapper_path}"'
            elif script_ext in ['.bat', '.cmd']:
                # 对于批处理文件也使用 VBScript 包装器
                vbs_wrapper_path = os.path.join(config_dir, f"{name}_wrapper.vbs")
                vbs_content = f'''Set objShell = CreateObject("WScript.Shell")
objShell.Run """{final_script_path}""", 0, False
Set objShell = Nothing'''
