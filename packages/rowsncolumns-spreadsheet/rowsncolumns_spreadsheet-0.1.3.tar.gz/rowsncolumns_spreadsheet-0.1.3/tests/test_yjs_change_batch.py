...
def test_change_batch_registers_cell_xfs():
    ...
    assert len(cell_xfs._store) == 1
    style_id, format_value = next(iter(cell_xfs._store.items()))
    assert format_value["textFormat"]["bold"] is True

    sheet_rows = doc.get_map("sheetDataV2").get("1")
    row_entry = sheet_rows.get(0)
    values = row_entry["values"]
    cell = values.get(0)
    assert cell["uf"]["sid"] == style_id
    assert cell["ef"]["sid"] == style_id
