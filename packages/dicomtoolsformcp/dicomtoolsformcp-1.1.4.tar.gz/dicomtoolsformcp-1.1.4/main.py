...
class series_instance(BaseModel):
...
async def list_tools() -> list[Tool]:
    ...
    return [
        ...
        Tool(
            ...
            description="根据文件夹的series_instance_uid，如果没有分析结果，需要进行{Analysis_dicom_directory}工具上传分析，返回测量结果的url",
            inputSchema={
                "type": "object",
                "properties": {
                    "series_instance_uid": {
                        "type": "string",
                        "description": "DICOM序列的 SeriesInstanceUID，用于查询分析结果"
                    }
                },
...
async def call_tool(name: str, arguments: Dict[str, Any]) -> list[TextContent]:
    ...
    try:
        ...
        if name == "scan-dicom-directory":
            ...
            args = series_instance(**arguments)
            result = await get_result_tool(args.series_instance_uid)
        else:
