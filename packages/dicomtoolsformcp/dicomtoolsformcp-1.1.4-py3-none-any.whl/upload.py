...
#         ]
#     }
def get_result(SeriesInstanceUID):
    config = main()
    # --- 从 Excel 文件读取 study_uid ---
    cookie = "ls=" + config["cookie"]
    ...
    re = False
    for i in range(120):
        status,study_uid,series_uid,s_type= query_result(SeriesInstanceUID)
        print(f"查询状态: {status}, StudyInstanceUID: {SeriesInstanceUID}")
        if status is not None:
            if int(status) == 42:
                re = True
                print("查询成功")
...
if __name__ == "__main__":
    ...
    print(asyncio.run(get_result_tool('1.2.840.113564.345049290535.9692.637552042156233117.433089')))