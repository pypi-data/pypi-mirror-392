...
class DescribableConfig(Config):
    test_ref: NestedRef = dataclasses.field(default_factory=NestedRef)
...
class TestGet:
    def test_should_get_valid_reference(self):
        schema = Schema(TEST_SCHEMA)
        test_ref = schema.get("test_ref")
    ...
    def test_should_describe_config(self):
        ...
        assert data == {
            "test_ref": {
                "default": "",
                "type": "NestedRef",
                "description": "For more information use `config show --output test_ref`.",
                "values": {
                    "test_ref_value": {
                        "type": "str",
                        "description": "A nested property.",
