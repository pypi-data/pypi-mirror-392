...
class BluestacksElements:
    ...
    def __init__(self, controller):
        ...
        self.bluestacks_my_games_button: BluePyllElement = BluePyllElement(
            label="bluestacks_my_games_button",
            ele_type="button",
            og_window_size=self.controller.ref_window_size,
            path=files("bluepyll.assets").joinpath("bluestacks_my_games_button.png"),
            confidence=0.6,
            ele_txt="My games",
        )
...
class BluestacksController:
    ...
    def capture_screen(
        self, bluepyll_element: BluePyllElement | None = None
    ) -> bytes | None:
        """Capture the current BlueStacks screen using the appropriate strategy."""

        should_capture_loading_screen: bool = (
            self.bluestacks_state.current_state == BluestacksState.LOADING
        )
        loading_element_path = self.elements.bluestacks_loading_img.path
        if (
            bluepyll_element
            and bluepyll_element.path
            and bluepyll_element.path == loading_element_path
        ):
            should_capture_loading_screen = True

        if should_capture_loading_screen:
            return self._capture_loading_screen()

        if not self._adb_controller:
            logger.warning("Cannot capture screen - ADB controller is not initialized")
            return None

        return self._adb_controller.capture_screenshot()
