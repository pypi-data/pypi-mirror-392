...
class RegexDict(Dict):
    ...
    def __getattr__(self, attr) -> Dict | List | str | None:
        if not attr:
            return None
    ...
    def listA(self, url=None, start=None, end=None, before=None) -> list[Dict]:
        if not self["html"]:
            return List()
        return List(listA(self["html"], url, start, end, before))

    def listImg(self, url=None, start=None, end=None, before=None) -> list[Dict]:
        if not self["html"]:
            return List()
        return List(listImg(self["html"], url, start, end, before))

    def getElementByID(self, id, start=None, end=None, before=None) -> Dict:
        if not self["html"]:
            return Dict()
        ele = getElementByID(id, self["html"], start, end, before)
        return RegexDictNew(ele, root=self._rootNode, parent=self)

    def getElementByTag(self, tag, start=None, end=None, before=None) -> Dict:
        if not self["html"]:
            return Dict()
        ele = getElementByTag(tag, self["html"], start, end, before)
        return RegexDictNew(ele, root=self._rootNode, parent=self)

    def getElementByClass(self, className, start=None, end=None, before=None) -> Dict:
        if not self["html"]:
            return Dict()
        ele = getElementByClass(className, self["html"], start, end, before)
        return RegexDictNew(ele, root=self._rootNode, parent=self)

    def getElementsByTag(self, tag, start=None, end=None, before=None) -> list[Dict]:
        if not self["html"]:
            return List()
    ...
    def getElementsByClass(
        self, className, start=None, end=None, before=None
    ) -> list[Dict]:
        if not self["html"]:
            return List()
    ...
    def getElementByAttr(self, attr, value, start=None, end=None, before=None) -> Dict:
        if not self["html"]:
            return Dict()
    ...
    def getElement(
        self, tag, attr="class", value=None, start=None, end=None, before=None
    ) -> Dict:
        if not self["html"]:
            return Dict()
    ...
    def removeElement(
        ...
        if self["html"]:
            self["html"] = removeElement(
                tag, attr, value, self["html"], start, end, before
            )
        return self
    ...
    def getElementByText(
        self, text, tag=None, start=None, end=None, before=None
    ) -> Dict:
        if not self["html"]:
            return Dict()
    ...
    def getElementByReg(
        self, regex, tag=None, start=None, end=None, before=None
    ) -> Dict:
        if not self["html"]:
            return Dict()
    ...
    def getElementsByReg(
        self, regex, tag=None, html=None, start=None, end=None, before=None
    ) -> list[Dict]:
        if not html:
            html = self["html"]
        lst = List()
    ...
    def getParent(self, tag=None) -> Dict:
    ...
    def getNexts(self, tag=None) -> list[Dict]:
        if not self._rootNode:
            return List()
        html = self._rootNode["html"]
    ...
    def getPrevious(self, tag=None) -> list[Dict]:
        if not self._rootNode:
            return List()
        html = self._rootNode["html"]
    ...
    def getNext(self, tag=None) -> Dict:
        if not self._rootNode:
            return Dict()
        html = self._rootNode["html"]
    ...
    def getChild(self, tag=None, start=None, end=None, before=None) -> Dict | None:
        if not self["html"]:
            return None
    ...
    def getChildren(self, tag=None, start=None, end=None, before=None) -> list[Dict]:
        if not self["html"]:
            return List()
    ...
    def getText(self, separator="", tags=None) -> str:
    ...
    def createElement(self, tag, text="", **attrs) -> str:
        html = None
        if text == None:
            html = "<{} />".format(tag)
        else:
    ...
    def insertAfter(self, html):
        ...
        self._rootNode["html"] = h
        _updatePosition(
            self._rootNode, self._start, self._start + len(html), self._start
        )
    ...
    def nextText(self, end=None) -> str:
    ...
    def trimHtml(self) -> str:
        if not self["html"]:
            return None
        self["html"] = trimHtml(self["html"])
        return self["html"]

    def _convert2lst(self, eles, parent=None, s=None) -> list[Dict]:
        lst = List()
        if eles:
            for e in eles:
    ...
    def replaceReg(self, regex, new) -> str:
        if self["html"]:
            self["html"] = replaceReg(self["html"], regex, new)
        return self["html"]

    def getSectionByReg(
        self, regex, group=0, start=None, end=None, before=None
    ) -> str | None:
        return getOneByReg(self["html"], regex, group, start, end, before)

    def getSectionsByReg(
        self, regex, group=0, start=None, end=None, before=None
    ) -> list[str]:
        return getListByReg(self["html"], regex, group, start, end, before)

    def select_dic(self, value, start=None, end=None, before=None) -> Dict:
        dic = _select(self, value, start, end, before)
        if not dic:
            return Dict()
        return dic
    
    def select_str(self, value, start=None, end=None, before=None) -> str:
        s = _select(self, value, start, end, before)
        if not s:
            return ""
        return s
    
    def select(self, value, start=None, end=None, before=None) -> Dict | str | None:
        return _select(self, value, start, end, before)


    def selects_dic(self, value, start=None, end=None, before=None) -> list[Dict]:
        lst = self.selects(value, start, end, before)
        if not lst:
            return List()
        return lst

    def selects_str(self, value, start=None, end=None, before=None) -> list[str]:
        lst = self.selects(value, start, end, before)
        if not lst:
            return List()
        return lst

    def selects(self, value, start=None, end=None, before=None) -> list[Dict | str]:
        if value is None:
            return List()
        s = value.find("(")
        tp = None
...
def _getValue(_ele: RegexDict, attr) -> str | None:
    return _ele.__getattr__(attr)


def _select(_self: RegexDict, value, start=None, end=None, before=None) -> Dict | None:
    if value == None:
        return None
    s = value.find("(")
    tp = None
...
def _selectText(
    _ele: RegexDict, value, start=None, end=None, before=None
) -> list | str:
    v = value.strip()
    obj = []
    s = 0
    while True:
        ...
        if not p:
            ...
            if len(paras) == 1:
                ...
                ele = _ele.getElement(
                    tag, attr, value, start=start, end=end, before=before
                )
                if ele:
                    obj.append(_getValue(ele, _attr))
                else:
                    obj.append(None)
            else:
                for para in paras:
...
def RegexDictNew(dic, root, parent=None, s=None) -> Dict:
    if not dic:
        return Dict()
    ele = RegexDict(dic)
    _s = 0
