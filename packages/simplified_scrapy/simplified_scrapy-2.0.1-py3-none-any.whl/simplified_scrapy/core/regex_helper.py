...
def _getRegex(regex) -> re.Pattern:
    if not __regCache.get(regex):
        __regCache[regex] = re.compile(regex)
    return __regCache[regex]


def getSection(html, start=None, end=None, before=None) -> tuple[int, int]:
    if not html:
        return (0, 0)
    s = 0
    e = len(html)
...
def listA(html, baseUrl=None, start=None, end=None, before=None) -> list[Dict]:
    if not html:
        return []
                ...
                if baseUrl and url[:7] != "http://" and url[:8] != "https://":
                    ...
                    if absUrl != url:
                        ...
                        if d:
                            ...
                            dic[absUrl] = Dict(
                                {"url": absUrl, "title": title, "relativeUrl": url}
                            )
                else:
                    if url.rfind("/") < 9:
                        url += "/"
...
def getListByReg(html, regex, group=0, start=None, end=None, before=None) -> list[str]:
    if not html or not regex:
        return []
    section = getSection(html, start, end, before)
    s = section[0]
    e = section[1]
...
def regFindall(html, regex) -> list[str]:
    pattern = _getRegex(regex)
    return pattern.findall(html)


def regSearch(html, regex, group=0) -> str | None:
    pattern = _getRegex(regex)
...
def getOneByReg(html, regex, group=0, start=None, end=None, before=None) -> str | None:
    if not html or not regex:
        return None
    section = getSection(html, start, end, before)
    s = section[0]
...
def listImg(html, baseUrl=None, start=None, end=None, before=None) -> list[Dict] | None:
    if not html or html.find("<img") < 0:
        return []
    section = getSection(html, start, end, before)
    s = section[0]
    e = section[1]
                ...
                if baseUrl and url[:7] != "http://" and url[:8] != "https://":
                    ...
                    if absUrl != url:
                        ...
                        if d:
                            ...
                            dic[absUrl] = Dict(
                                {"url": absUrl, "alt": title, "relativeUrl": url}
                            )
...
def getElementsByTag(tag, html, start=None, end=None, before=None) -> list[Dict]:
    lst = []
    section = getSection(html, start, end, before)
...
def getElementByTag(tag, html, start=None, end=None, before=None) -> Dict:
...
def _checkSingleTag(tag, html=None, i=0) -> bool:
    if not html:
        return False
...
def _checkSingleTagByReg(pattern, html, tag) -> bool:
...
def _getElementByTag(
    tag, html, start=None, end=None, before=None
    ...
    if isinstance(tag, list):
        ...
        for t in tags:
            ...
            if tmp1 >= 0 and tmp2 >= 0:
                ...
                tag = t
                index = tmp2
    if isinstance(tag, list) or html.find("<" + tag) < 0:
        return None

    pattern = _getRegex("<" + _dealRegChar(tag) + r"([\s]+|)[^>]*?>")
...
def removeHtml(html, replace="", tags=None) -> str:
    if not html:
        return html
...
def replaceReg(html, regex, new, count=0) -> str:
    return re.sub(_getRegex(regex), new, html, count)


def trimHtml(html) -> str:
    if not html:
        return html
    html = html.strip()
    start = html.find(">")
    tmp = html.find("<")
    if start < 0 and tmp < 0:
        return html
    if tmp < 0:
        return html[start + 1 :]
    if start < 0:
...
def checkContains(html, reg) -> bool:
    return not not _getRegex(reg).search(html)


def _getTag(html, end, attr, start=None) -> str | None:
    if start == None:
...
def getElementsByClass(
    className, html, start=None, end=None, before=None
) -> list[Dict]:
    lst = []
    s = start
    h = html
...
def getElements(
    tag, attr="class", value=None, html=None, start=None, end=None, before=None
) -> list[Dict]:
    lst = []
    s = start
    h = html
...
def _getElement(
    ...
    if not attr or not value or not html or html.find(value) < 0:
        return None
    section = getSection(html, start, end, before)
    s = section[0]
    e = section[1]
...
def getElementByReg(
    regex, tag=None, html=None, start=None, end=None, before=None
) -> Dict | None:
    if not regex or not html:
        return None
    section = getSection(html, start, end, before)
    s = section[0]
    e = section[1]
    if s < 0 or e < s or s == e:
        return None
    h = html[s:e]
...
def getElementByText(
    text, tag=None, html=None, start=None, end=None, before=None
) -> Dict | None:
...
def getElementBy(attr, value, html, start=None, end=None, before=None)-> Dict | None:
    return getElementByAttr(attr, value, html, start, end, before)


def getElementByAttr(attr, value, html, start=None, end=None, before=None)-> Dict | None:
    return getElement(
        None, attr=attr, value=value, html=html, start=start, end=end, before=before
    )


def getElementByClass(className, html, start=None, end=None, before=None)-> Dict | None:
    return getElementByAttr("class", className, html, start, end, before)


def getParent(
    tag=None, attr=None, value=None, html=None, start=None, end=None, before=None
)-> Dict | None:
    ele = _getElement(tag, attr, value, html, start, end, before)
    return getParent4Ele(html, ele)


def getParent4Ele(html, e, tag=None)-> Dict | None:
    if e:
        start = e._start
...
def _getParentEnd(html, start, tag)-> int | None:
    end = start
    while True:
        if end < 0:
            return None
...
def _getParentStart(html, end, tag)-> tuple[int, str] | None:
    start = end
    s = end
    dicTag = Dict()
    if not tag:
...
def _getEnd(html, start)-> int | None:
    s = start
    startCount = 0
    while True:
        s = html.find("<", s)
...
def _getEnd4Tag(html, start, tag)-> int | None:
    if not tag:
...
def _getStart4Tag(html, end, tag)-> int | None:
    if not tag:
...
def _getStart(html, end)-> int | None:
    s = 0
    e = end
    endCount = 0
    while True:
...
def _checkSingle(html, start=None)-> bool:
    end = html.find(">", start)
    flag = html[end - 1 : end] == "/"
    if flag:
        return True
...
def getChildren(html, tag=None, start=None, end=None, before=None)-> list[Dict]:
    lst = []
    if tag:
...
def getChild(html, tag=None, start=None, end=None, before=None)-> Dict | None:
    if tag:
...
def _getNextTag(html, start)-> str | None:
...
def getNexts(attr, value, html, tag=None, start=None, end=None, before=None)-> list[Dict]:
    ele = _getElement(tag, attr, value, html, start, end, before)
    return getNexts4Ele(html, ele)


def getNexts4Ele(html, ele, tag=None)-> list[Dict]:
    if ele:
...
def getNext4Ele(html, ele, tag=None)-> Dict | None:
    if ele:
        start = ele._end
