...
class SimplifiedDoc(RegexDict):
    ...
    def _getStart(self, line, tag, tagLen)-> bool:
        i = line.find(tag)
        if i < 0:
            return False
        tmp = line[i + tagLen]
    ...
    def getSection(self, html=None, start=None, end=None, before=None)-> tuple[str, int, int]:
        return self._getSection(html, start, end, before)[0]

    def _getSection(self, html=None, start=None, end=None, before=None)-> tuple[str, int, int]:
        if html: html = preDealHtml(html)
        if (not html): html = self['html']
    ...
    def removeHtml(self, html, separator='', tags=None)-> str:
        return removeHtml(html, separator, tags)

    def trimHtml(self, html)-> str:
        return trimHtml(html)

    def absoluteUrl(self, baseUrl, url)-> str:
        return absoluteUrl(baseUrl, url)

    def getObjByModel(self, html, url=None, models=[{"Type": 3}], title=None)-> dict:
