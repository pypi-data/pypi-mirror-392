...
class AncDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        logging.debug(
            f"iter information: len(sampler) - {len(self.sampler)}, worker id - {self.wid}, num_workers - {num_workers}"
        )
        sampler_iter = iter(self.sampler)
        yield from self.get_generator(sampler_iter, num_workers)
    ...
    def get_generator(self, sampler_iter, num_workers):
        while True:
            ...
            if indices is EndOfIteration:
                logging.debug(
                    f"worker {self.wid} of {num_workers} get EndOfIteration from sampler"
                )
    ...
    def _add_batch_metadata(self, raw_batch):
        return {
            ...
            "sampler_indices_read_states": [
                (sampler.inner_epoch_count, sampler.end_idx)
                for sampler in self.sampler.samplers
            ],
        }
