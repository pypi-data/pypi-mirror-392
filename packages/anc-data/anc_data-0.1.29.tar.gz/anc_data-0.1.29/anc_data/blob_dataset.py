...
class BlobDataset:
    def __init__(
        self, obj_dir, metadata_file="metadata.json", obj_count=None, prefix=None
    ):
        assert (
            isinstance(obj_dir, list) and len(obj_dir) == 1
        ), "obj_dir should be a list of length 1"
        self.obj_dir = obj_dir[0]
        assert os.path.exists(
            self.obj_dir
        ), f"Object directory {self.obj_dir} not found"
        if os.path.exists(os.path.join(self.obj_dir, metadata_file)):
            with open(os.path.join(self.obj_dir, metadata_file), "r") as f:
                ...
                self.num_batches_per_subdir = metadata.get(
                    NUM_BATCHES_PER_SUBDIR_KEY, None
                )
                self.prefix = metadata.get(BLOB_FILENAME_PREFIX_KEY, None)
        else:
            assert (
                obj_count is not None
            ), "obj_count is required if metadata file is not provided"
    ...
    def get_obj_filename(self, idx):
        ...
        subdir_idx = (
            idx // self.num_batches_per_subdir if self.num_batches_per_subdir else ""
        )
    ...
    def __getitem__(self, idx):
        ...
        assert (
            0 <= idx < self.obj_count
        ), f"Object index {idx} out of range, expected range is [0, {self.obj_count})"
        # TODO: support other file formats
