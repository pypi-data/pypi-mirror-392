...
def _run_format_command(
    ...
    if not result.success:
        ...
        raise subprocess.CalledProcessError(
            result.returncode, actual_command, result.stderr
        )
...
def format_with_ruff(
    ...
    try:
        ...
        if not quiet:
            print(f"Error: ruff formatting failed with return code {e.returncode}")
        raise
    except FileNotFoundError as e:
        if not quiet:
            print(f"Error: uv or ruff not found in PATH: {e}")
        raise
    except Exception as e:
        if not quiet:
            print(f"Error occurred during ruff formatting: {e}")
        raise
...
def format_with_python_tools(
    ...
    try:
        ...
        if not quiet:
            print(f"Error: Python formatting failed with return code {e.returncode}")
        raise
    except Exception as e:
        if not quiet:
            print(f"Error occurred during Python formatting: {e}")
        raise
...
def generate_type_stubs(src_dir: str, typings_dir: str, env: dict, quiet: bool) -> None:
    ...
    try:
        ...
        if not quiet:
            print(f"Error generating type stubs: {e}")
