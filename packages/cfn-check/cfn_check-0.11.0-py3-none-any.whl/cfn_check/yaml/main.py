...
from cfn_check.yaml.error import UnsafeLoaderWarning, YAMLError  # NOQA

from cfn_check.yaml.tokens import *  # NOQA
from cfn_check.yaml.events import *  # NOQA
from cfn_check.yaml.nodes import *  # NOQA

from cfn_check.yaml.composer import Composer
from cfn_check.yaml.emitter import Emitter, RoundTripEmitter
from cfn_check.yaml.loader import BaseLoader, SafeLoader, Loader, RoundTripLoader  # NOQA
from cfn_check.yaml.dumper import BaseDumper, SafeDumper, Dumper, RoundTripDumper, Dumper as RuamelDumper  # NOQA
from cfn_check.yaml.compat import StringIO, BytesIO, with_metaclass, nprint, nprintf  # NOQA
from cfn_check.yaml.resolver import VersionedResolver, Resolver, BaseResolver  # NOQA
from cfn_check.yaml.representer import (
    BaseRepresenter,
    Representer,
    RoundTripRepresenter,
    SafeRepresenter,
)
from cfn_check.yaml.constructor import (
    BaseConstructor,
    SafeConstructor,
    Constructor,
    RoundTripConstructor,
)
from cfn_check.yaml.loader import Loader as RuamelLoader  # NOQA
from cfn_check.yaml.comments import CommentedMap, CommentedSeq, C_PRE
from cfn_check.yaml.docinfo import DocInfo, version, Version
from cfn_check.yaml.parser import Parser, RoundTripParserSC, RoundTripParser
from cfn_check.yaml.serializer import Serializer
from cfn_check.yaml.scanner import RoundTripScannerSC, RoundTripScanner, Scanner
from cfn_check.yaml.reader import Reader

from typing import List, Set, Dict, Tuple, Union, Any, Callable, Optional, Text, Type  # NOQA
from cfn_check.yaml.compat import StreamType, StreamTextType, VersionType  # NOQA
...
class YAML:
    def __init__(
        ...
        *,
        ...
        self.Resolver: Any = VersionedResolver
        ...
        if 'rt' in self.typ:
            ...
            self.Emitter = (
                Emitter if pure or CEmitter is None else CEmitter
            )
            self.Representer = SafeRepresenter
            self.Parser = Parser if pure or CParser is None else CParser
            self.Composer = Composer
            self.Constructor = SafeConstructor
        elif 'base' in self.typ:
            self.Emitter = Emitter
            self.Representer = BaseRepresenter
            self.Parser = Parser if pure or CParser is None else CParser
            self.Composer = Composer
            self.Constructor = BaseConstructor
            ...
            self.Emitter = RoundTripEmitter
            self.Serializer = Serializer
            self.Representer = RoundTripRepresenter
            self.Scanner = RoundTripScannerSC
            # no optimized rt-parser yet
            self.Parser = RoundTripParserSC
            self.Composer = Composer
            self.Constructor = RoundTripConstructor
        ...
        if setup_rt:
            ...
            self.Serializer = Serializer
            self.Representer = RoundTripRepresenter
            self.Scanner = RoundTripScanner
            # no optimized rt-parser yet
            self.Parser = RoundTripParser
            self.Composer = Composer
            self.Constructor = RoundTripConstructor
        del setup_rt
        self.stream = None
...
def add_implicit_resolver(
    ...
    if Loader:
        if hasattr(Loader, 'add_implicit_resolver'):
            ...
            Loader, (BaseLoader, SafeLoader, RuamelLoader, RoundTripLoader),
        ):
            Resolver.add_implicit_resolver(tag, regexp, first)
        else:
            raise NotImplementedError
    if Dumper:
        if hasattr(Dumper, 'add_implicit_resolver'):
            ...
            Dumper, (BaseDumper, SafeDumper, RuamelDumper, RoundTripDumper),
        ):
            Resolver.add_implicit_resolver(tag, regexp, first)
        else:
            raise NotImplementedError
...
def add_multi_constructor(
    ...
    if Loader is None:
        ...
        elif issubclass(Loader, RuamelLoader):
            Constructor.add_multi_constructor(tag_prefix, multi_constructor)
        elif issubclass(Loader, RoundTripLoader):
