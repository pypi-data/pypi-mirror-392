...
## Exceptions
from obzai.client.schemas.exceptions import ObzClientError
...
class ObzClient:
    ...
    def login(
        ...
        try:
            ...
            raise ObzClientError(f"Authentication failed") from e
    ...
    def init_project(
        ...
        """
        The method initializes a project at ObzAI dashboard. Correct project initialization 
        is required to log data. 
        
        Note! - If project_name points to an existing project, project initialization fails.

        Args:
            project_name: (string) Name of project where you want to log data.
            ml_task: (Enum type) One of possible Machine Learning Tasks: MLTask.Regression, MLTask.Classification, MLTask.Translation, MLTask.Segmentation
            index2name: (dict) Mapping of indices to prediction names, e.g. class names.
        
        Raises:
            ObzClientError when project initialization fails.
        """
        ...
        if not self.auth_service.authenticated:
            ...
            try:
                ...
                # (1.2) If not succesfull, raise error.
                logger.error("Error occured during authentication. Project not initialized.")
                raise ObzClientError("Error occured during authentication. Project not initialized.") from e
            
        # (2) Pass project initialization request to the ProjectService
        ...
        try:
            self.project_service.init_project(
            ...
            raise ObzClientError(f"Error occured during project initialization: {e}")
    ...
    def log_reference(self) -> None:
        """
        The method logs reference data from fitted Data Inspectors 
        provided during initialization the ObzAI Client.
        
        Raises:
            ObzClientError when logging reference data fails due to some reason.
        """
        ...
        try:
            reference_data = []
            for inspector in self._data_inspectors:
                ref_data = inspector.load_reference_data()
                reference_data.append(ref_data)
        except ObzClientError as e:
            logger.exception("Error occured during retrieving reference features from data inspector.")
            raise ObzClientError(f"Error occured during retrieving reference features from data inspector.") from e
        ...
        try:
            ...
            raise ObzClientError(f"Error occured during logging reference data: {e}")