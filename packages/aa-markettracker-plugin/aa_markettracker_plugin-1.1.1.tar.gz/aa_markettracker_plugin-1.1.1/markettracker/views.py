...
def item_search(request):
    ...
    qs = (EveType.objects
      ...
      .only("id", "name", "eve_group__eve_category__name"))
    ...
    return JsonResponse({"results": results, "pagination": {"more": start + page_size < total}})
...
def list_items_view(request):
    ...
    for tracked in tracked_items:
        ...
        # ⬇️ Nazwa kategorii / grupy z EveUniverse (bezpiecznie)
        cat_name = getattr(
        ...
        items_data.append({
            ...
            "category": cat_name,   # <<< dodane
            "group": grp_name,      # <<< opcjonalnie
        })
