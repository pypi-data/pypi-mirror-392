...
class MixClient:
    ...
    def create_model(
        ...
        if entrypoint:
            data["entrypoint"] = (None, entrypoint)
        ...
        for file_path in file_paths:
            ...
            if path.is_file():
                ...
                if "files" not in files:
                    files["files"] = []
                files["files"].append(
                    (os.path.basename(file_path), file_handle, "application/octet-stream")
                )
            elif path.is_dir():
                ...
                for root, _, filenames in os.walk(file_path):
                    for filename in filenames:
                        ...
                        if "files" not in files:
                            files["files"] = []
                        files["files"].append(
                            (relative_path, file_handle, "application/octet-stream")
                        )
    ...
    def get_model_run_logs(
        ...
        if run_number is not None:
            ...
            if not runs:
                return {"data": {"logs": "No runs found for this model"}}
            latest_run = runs[0]
