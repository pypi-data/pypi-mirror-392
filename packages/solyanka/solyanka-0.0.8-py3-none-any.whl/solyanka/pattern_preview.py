...
class PatternSample:
    ...
    def amount_display(self) -> str:
        return format_amount(self.amount, self.amount_format, self.currency)
        ...
        for info in patterns:
            ...
            while produced_for_pattern < per_pattern_limit and attempts < per_pattern_limit * 4:
                ...
                sample = PatternSample(
                    title=title_clean,
                    amount=amount,
                    amount_format=int(info.data.get("amount_format", 2)),
    ...
    payload = json.dumps(
        {
            ...
            "amount_range": info.data.get("amount_range"),
...
def generate_amount(pattern: Dict[str, Any], rng: random.Random) -> float:
    amount_range = pattern.get("amount_range") or {}
    amount_min = float(amount_range.get("min", 0.0))
    amount_max = float(amount_range.get("max", amount_min))
    if amount_min == amount_max:
        value = amount_min
    else:
        value = rng.uniform(amount_min, amount_max)
    amount_format = int(pattern.get("amount_format", 2))
    if amount_format >= 0:
        return round(value, amount_format)
    step = 10 ** abs(amount_format)
    return round(value / step) * step


def format_amount(amount: float, amount_format: int, currency: str) -> str:
    currency_clean = currency.strip().upper()
    if amount_format > 0:
        formatted = f"{amount:.{amount_format}f}"
    else:
...
def build_markdown_chunks(
    ...
    def _block_lines(group_label: str, group_samples: Sequence[PatternSample]) -> List[str]:
        block = [f"**{group_label}**", "", "| Title | Amount |", "| --- | --- |"]
        for sample in group_samples:
            title_cell = _escape_table_cell(sample.title)
            block.append(f"| {title_cell} | {sample.amount_display} |")
        block.append("")
        return block
