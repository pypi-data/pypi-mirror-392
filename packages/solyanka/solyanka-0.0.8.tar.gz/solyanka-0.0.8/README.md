...
| `amount_range`   | `{min, max}` floats describing the observed local-currency range.                                  |
| `amount_format`  | Rounding strategy: `n>0` decimals, `0` whole units, `n<0` powers of ten (e.g., `-2` rounds to 100s).|
...
| `refund_probability`        | Chance (0–1) that the generator emits a `CARD_REFUND` for this pattern.                 |
...
| `subscription_frequency_days` | Frequency for recurring charges (e.g., 30 for monthly subscriptions).                 |
...
- `global_constant: true` — reuse the same generated value across the statement (great for IDs).
...
- title: "Netflix.com"
  currency: "EUR"
  amount_range: {min: 13.49, max: 13.49}
  amount_format: 2
...
- title:
    ...
    params:
      code:
        ...
        global_constant: true
  currency: "USD"
  amount_range: {min: 70.0, max: 900.0}
  amount_format: 2
...
3. Choose realistic `amount_range`, `amount_format`, tags, and weights.
...
6. Run `pytest` to validate against `schema.json` before committing/publishing.
...
touched patterns, and posts a Markdown table comment back onto the PR so reviewers can eyeball
