...
_MAX_REQUESTS = 150  # Daily request limit for external service (increased from 80)
...
def _check_request_limit():
    ...
    if data["count"] >= _MAX_REQUESTS:
        ...
        raise RuntimeError(
            f"⚠️ Daily request limit exceeded!\n"
...
def _update_counter():
    ...
    if remaining <= 10:
        print(f"⚠️ Warning: {remaining} requests remaining today")
...
def query(text: str, mode: int = 1, temp: float = 0.7, max_length: int = 2048) -> str:
    """
    Query external service for information with two modes.
    ...
    --------
    str
        AI-generated response (code or explanation based on mode)
    ...
    - Daily limit: 150 requests (resets at midnight)
