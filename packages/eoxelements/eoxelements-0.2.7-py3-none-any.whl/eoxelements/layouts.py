...
class EOxMapWithControl(HBox):
    ...
    """
    def __init__(self, layers=None, config=None, map_layout=None, control_layout=None, **kwargs):
        ...
        map_layers = layers if layers is not None else []
        map_config = config if config is not None else {}
        self.map = EOxMap(
            element_id=map_id,
            layers=map_layers,
            config=map_config,
