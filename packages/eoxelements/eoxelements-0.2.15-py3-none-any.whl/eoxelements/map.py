...
function render(view) {{
  ...
  // TODO: fix config override in eox-map
  // if it is null, it should not override layers, etc. when set
  if (view.model.get(\"config\") !== null) map.config = view.model.get(\"config\");
  ...
  view.model.on(\"change:config\", () => {{ if (view.model.get(\"config\") !== null) map.config = view.model.get(\"config\"); }});
...
class EOxMap(anywidget.AnyWidget, DOMWidget):
    ...
    center = traitlets.List([0,0]).tag(sync=True)
