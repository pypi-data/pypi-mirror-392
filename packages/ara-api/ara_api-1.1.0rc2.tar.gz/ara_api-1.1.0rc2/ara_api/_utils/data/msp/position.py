...
@dataclass
class Position:
    grpc: position_grpc = field(default_factory=lambda: position_grpc())
    json: dict = field(default_factory=lambda: {"x": 0.0, "y": 0.0, "z": 0.0})

    def __repr__(self):
        return "Position(x={x}, y={y}, z={z})".format(
            x=self.grpc.data.x, y=self.grpc.data.y, z=self.grpc.data.z
        )

    @overload
    def sync(self, grpc: position_grpc) -> None: ...
    ...
    def sync(
        self, data: Union[position_grpc, Tuple[float, float, float], dict]
    ) -> None:
        """
        ...
        """
        if isinstance(data, position_grpc):
