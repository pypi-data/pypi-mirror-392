...
def postelast_lammps_optimize(work_dir, pdict):
    ...
    ### Convert LAMMPS output to extxyz_labeled
    task_dirs = [p for p in structure_paths if not Path(f"{p}/{FILE_FRAME_LABEL}").exists()]
    ...
    unlabel_paths = filter_dirs(structure_paths, has_files=[FILE_FRAME_unLABEL, FILE_FRAME_LABEL])
...
def postelast_lammps_singlepoint(work_dir, pdict):
    ...
    ### Convert LAMMPS output to extxyz_labeled
    task_dirs = [p for p in supercell_paths if not Path(f"{p}/{FILE_FRAME_LABEL}").exists()]
    for task_path in task_dirs:
        lmpdump2extxyz(
            lmpdump_file=f"{task_path}/output_md/conf_0.lmpdump",
            extxyz_file=f"{task_path}/{FILE_FRAME_LABEL}",
