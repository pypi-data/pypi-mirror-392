# Define functions related to phonon calculations.
"""
Notes:
...
from alff.util.key import FILE_PHONOPYwFORCES
...
def convert_phonopy2ase(struct_ph: PhonopyAtoms) -> Atoms:
    struct = Atoms(
        symbols=struct_ph.get_chemical_symbols(),
        scaled_positions=struct_ph.get_scaled_positions(),
        cell=struct_ph.get_cell(),
        pbc=True,
        masses=struct_ph.get_masses(),
    )
    return struct
...
def get_primitive_spglib(
    ...
    lattice, scaled_positions, numbers = prim_cell
    prim_struct = Atoms(
        cell=lattice,
...
def get_band_structure(work_dir, pdict):
    ...
    path_str = band_structure_arg.get("path_str", None)
    npoints = band_structure_arg.get("npoints", 61)

    ### Read phonon file
    phonon = phonopy.load(f"{work_dir}/{FILE_PHONOPYwFORCES}")
