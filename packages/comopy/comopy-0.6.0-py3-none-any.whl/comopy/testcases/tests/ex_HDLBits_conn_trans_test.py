# Tests for HDLBits examples, basics and vectors (@= only)
#

import pytest

import comopy.testcases.ex_HDLBits_conn as ex
...
def _translate(ModuleClass):
    check_verilog(ModuleClass, "ex_conn", COMOPY_PROJECT_PATH)


def test_conn_basics_verilog(project_path):
...
def test_conn_vectors_verilog(project_path):
    global COMOPY_PROJECT_PATH
    COMOPY_PROJECT_PATH = project_path
