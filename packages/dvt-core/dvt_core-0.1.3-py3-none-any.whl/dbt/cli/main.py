...
@click.pass_context
def compute(ctx):
    """Manage external compute clusters for DVT federated execution"""
    pass


@compute.command("register")
@click.argument("name")
@click.option("--type", "cluster_type", default="spark", help="Cluster type (spark)")
...
@click.option("--description", help="Cluster description")
@click.option("--replace", is_flag=True, help="Replace existing cluster with same name")
@click.pass_context
@p.project_dir
def compute_register(ctx, name, cluster_type, master, config_str, description, replace, **kwargs):
    """Register an external compute cluster"""
    ...
    if cluster_type == "spark" and "master" not in config:
        raise DbtUsageException("Spark clusters require --master option")
    ...
    click.echo(f"Registered compute cluster '{name}' (type: {cluster_type})")
    return True, True
...
def compute_list(ctx, **kwargs):
    ...
    if not clusters:
        click.echo("No compute clusters registered")
        return True, True

    click.echo("Registered compute clusters:")
    click.echo("")
    for cluster in clusters:
        click.echo(f"  {cluster.name} ({cluster.type})")
...
@click.pass_context
@p.project_dir
def compute_remove(ctx, name, **kwargs):
    """Remove a registered compute cluster"""
    from dbt.config.compute import ComputeRegistry
    from dbt.flags import get_flags
    ...
    click.echo(f"Removed compute cluster '{name}'")
    return True, True
