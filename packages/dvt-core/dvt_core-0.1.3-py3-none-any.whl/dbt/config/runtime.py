...
class RuntimeConfig(Project, Profile, AdapterRequiredConfig):
    ...
    def __post_init__(self):
        self.validate()
        # DVT: Initialize adapter registry from profile connections
        if hasattr(self, 'connections') and self.connections:
            self._init_adapter_registry()

    def _init_adapter_registry(self):
        """Initialize adapter registry from DVTProfile connections."""
        from dbt.adapters.factory import get_adapter, register_adapter
        from dbt.mp_context import get_mp_context

        self.adapters = {}
        for conn_name, credentials in self.connections.items():
            # Create a temporary config-like object for this connection
            ...
            register_adapter(temp_config, get_mp_context())
            adapter = get_adapter(temp_config)
            self.adapters[conn_name] = adapter
