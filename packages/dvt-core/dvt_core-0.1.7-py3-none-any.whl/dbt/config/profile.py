...
class Profile(HasCredentials):
    ...
    def pick_profile_name(
        ...
        def default_profiles_dir():
            ...
            # DVT uses .dvt/ directory, with fallback to .dbt/ for backward compatibility
            cwd = Path.cwd()
            home = Path.home()

            # Priority: 1. CWD with profiles.yml, 2. ~/.dvt/, 3. ~/.dbt/ (legacy)
            if (cwd / "profiles.yml").exists():
                return cwd
            elif (cwd / ".dvt" / "profiles.yml").exists():
                return cwd / ".dvt"
            elif (home / ".dvt" / "profiles.yml").exists():
                return home / ".dvt"
            elif (home / ".dbt" / "profiles.yml").exists():
                # Legacy fallback - warn user
                return home / ".dbt"
            else:
                # Default to .dvt/ for new installations
                return home / ".dvt"
