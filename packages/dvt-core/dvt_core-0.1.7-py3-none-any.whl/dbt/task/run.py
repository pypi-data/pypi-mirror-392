...
class ModelRunner(CompileRunner):
    ...
    def execute(self, model, manifest):
        ...
        user_compute = model.config.compute if hasattr(model.config, 'compute') else None
        analysis = analyzer.analyze(model, user_compute_override=user_compute)

        # Get target adapter (for materialization)
        target_conn = model.config.target if hasattr(model.config, 'target') else None
        target_adapter = self.config.get_adapter(target_conn)

        if analysis.is_pushdown:
            # Pushdown execution: Use source adapter directly
            source_adapter = self.config.get_adapter(analysis.primary_connection)
            ...
            if materialization_macro is None:
                raise MissingMaterializationError(
                    materialization=model.get_materialization(), adapter_type=source_adapter.type()
                )
            ...
            # Run hooks
            hook_ctx = source_adapter.pre_model_hook(context_config)
            ...
            # Materialize to target
            target_table = f"{model.schema}.{model.alias}"
            executor.materialize_result(
                result=fed_result,
            ...
            return RunResult(
                ...
                adapter_response={},
                message=f"Federated execution: {fed_result.rows_returned} rows",
                failures=None,
                node=model,
                agate_table=None,
            )
