...
def build_catalog_table(data) -> CatalogTable:
    ...
    # DVT: Connection metadata could be added to metadata in future enhancement
    # This would require extending TableMetadata schema in dbt_common
    # For now, core DVT functionality works without this visualization enhancement
        ...
        for table in self.values():
            ...
            if key in node_map:
                ...
                if selected_node_ids is None or unique_id in selected_node_ids:
                    # DVT: Add connection metadata for nodes
                    node = manifest.nodes.get(unique_id)
                    connection_name = None
                    if node and hasattr(node.config, 'target') and node.config.target:
                        connection_name = node.config.target

                    # Create updated metadata with connection info
                    updated_metadata = replace(
                        table.metadata,
                        comment=f"{table.metadata.comment or ''} [connection: {connection_name}]" if connection_name else table.metadata.comment
                    )
            ...
            for unique_id in unique_ids:
                if unique_id in sources:
                    ...
                    # DVT: Add connection metadata for sources
                    source = manifest.sources.get(unique_id)
                    connection_name = None
                    if source and hasattr(source, 'connection') and source.connection:
