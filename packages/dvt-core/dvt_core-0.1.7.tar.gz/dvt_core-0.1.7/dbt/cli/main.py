...
def target_test(ctx, target_name, profile, profiles_dir, project_dir, **kwargs):
    ...
    if not profile:
        click.echo("Error: Could not determine profile. Use --profile flag.")
        return False, False
    ...
    click.echo(f"Testing connection: {target_name} ({adapter_type})")
    ...
    if missing_fields:
        ...
        click.echo(f"    Type: {adapter_type}")
        ...
        if 'database' in target_config:
            click.echo(f"    Database: {target_config['database']}")
...
@target.command("test-all")
@click.option("--profile", help="Profile name (defaults to project profile)")
@click.pass_context
@p.profiles_dir
@p.project_dir
def target_test_all(ctx, profile, profiles_dir, project_dir, **kwargs):
    ...
    click.echo(f"Testing all connections in profile '{profile}'...\n")
    ...
    if all_passed:
        ...
        click.echo("✗ Some connection tests failed")
        return False, False
...
@click.option("--type", "adapter_type", required=True, help="Adapter type (postgres, snowflake, etc)")
...
@click.pass_context
@p.profiles_dir
def target_add(ctx, target_name, profile, adapter_type, host, port, user, password, database, schema, threads, set_default, profiles_dir, **kwargs):
    ...
    # Get or create connections dict (support both DVT and legacy format)
    if "connections" in profile_data:
        connections = profile_data["connections"]
    elif "outputs" in profile_data:
        # Convert legacy format to DVT format
        connections = profile_data.pop("outputs")
        profile_data["connections"] = connections
        if "target" in profile_data:
            profile_data["default_target"] = profile_data.pop("target")
    else:
        connections = {}
        profile_data["connections"] = connections
    ...
    # Add target to connections
    connections[target_name] = target_config

    # Set as default if requested
    if set_default:
        profile_data["default_target"] = target_name
...
def target_remove(ctx, target_name, profile, profiles_dir, **kwargs):
    ...
    # Remove the target
    del connections[target_name]

    # Check if this was the default target
    default_target = profile_data.get("default_target", profile_data.get("target"))
    if default_target == target_name:
        # Set new default to first available target
        if connections:
            new_default = list(connections.keys())[0]
            if "default_target" in profile_data:
                profile_data["default_target"] = new_default
            elif "target" in profile_data:
                profile_data["target"] = new_default
            click.echo(f"  Note: '{target_name}' was the default target, changed to '{new_default}'")
        else:
