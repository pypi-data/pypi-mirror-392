...
def guess_molecular_id(rdmols: list[Chem.Mol]) -> str | None:
    ...
    if num_id_properties == 0:
        ...
        if '_Name' in id_properties:
            ...
            # shorter id 
            _, id_property = sorted([(max([len(v) for v in U[k]]), k) for k in id_properties], reverse=True)[0]
            return id_property
...
def read_sdf(path:str | Path, 
    ...
    _args = [ (rdmol, name, std, props) for rdmol, name, props in zip(rdmols, names, _props) ]
