...
def breakup(parents: Any, maxChildren: int | None = None, verbose: bool = False) -> list:
...
def BRICS_fragmented(rdmol: Chem.Mol,
                     ...
                     max_atoms:int | None = None) -> list[Chem.Mol]:
...
def rigid_fragment_indices(rdmol: Chem.Mol) -> list[list[int]]:
    ...
    connecting_atom_indices = [b[0] for b in rotatable_bonds] + [b[1] for b in rotatable_bonds]
    if rotatable_bonds:
        indices = []
