...
class GFN2xTB:
    def __init__(self, molecule: Chem.Mol, ncores: int | None = None, xtb_exec: str | Path | None = None):
        """_summary_
        ...
        """
        assert isinstance(molecule, Chem.Mol), "molecule must be rdkit.Chem.Mol type"
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with open(test_geometry, 'w') as f:
                    f.write('\n'.join(h2o))
                proc = subprocess.run([self.xtb_exec, test_geometry, '--opt'],
                                      cwd=temp_dir,
                                      capture_output=True, 
                                      text=True,
                                      encoding='utf-8')
...
$ cp -r xtb-dist/share      /usr/local/ """)
    ...
    def create_regular_grid(self, surface_points, grid_spacing=0.6, padding=4.0, verbose: bool = False):
        """
    ...
    def to_cube(self, grid_dims, origin, grid_vectors, values) -> str:
        ...
        cube.append(f"{self.natoms:5d} {origin[0]:12.6f} {origin[1]:12.6f} {origin[2]:12.6f}")
        
        # Grid dimensions and vectors
        cube.append(f"{nx:5d} {grid_vectors[0][0]:12.6f} {grid_vectors[0][1]:12.6f} {grid_vectors[0][2]:12.6f}")
        cube.append(f"{ny:5d} {grid_vectors[1][0]:12.6f} {grid_vectors[1][1]:12.6f} {grid_vectors[1][2]:12.6f}")
        cube.append(f"{nz:5d} {grid_vectors[2][0]:12.6f} {grid_vectors[2][1]:12.6f} {grid_vectors[2][2]:12.6f}")
            
        # Atom coordinates
        for atomic_num, (x, y, z) in zip(self.numbers, self.positions):
            cube.append(f"{atomic_num:5d} {float(atomic_num):12.6f} {x:12.6f} {y:12.6f} {z:12.6f}")
    ...
    def esp_volumetric(self, 
        ...
        with tempfile.TemporaryDirectory() as temp_dir: # tmpdir is a string
            ...
            try:
                ...
                grid_points, grid_dims, origin, grid_vectors = self.create_regular_grid(surface_points, grid_spacing, padding)
                if verbose:
                    logger.info(f"Grid: {grid_dims[0]} x {grid_dims[1]} x {grid_dims[2]} points")
                grid_esp = GFN2xTB.interpolate_esp_to_grid(surface_points, grid_points, method, max_distance)
