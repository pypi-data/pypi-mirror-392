...
from .models.query import ChangeQueryParam, MetadataCommand, RefreshQueryOption
...
class PublicAPI(Protocol):
    ...
    async def refresh_query(self, ctx: Context, option: RefreshQueryOption) -> None:
        """
        ...
        Example - Refresh after marking item as favorite:
            async def mark_favorite(action_context: ActionContext):
                ...
                await api.refresh_query(ctx, RefreshQueryOption(preserve_selected_index=True))
        ...
        Example - Refresh after deleting item:
            async def delete_item(action_context: ActionContext):
                ...
                await api.refresh_query(ctx, RefreshQueryOption(preserve_selected_index=False))
        """
        ...
