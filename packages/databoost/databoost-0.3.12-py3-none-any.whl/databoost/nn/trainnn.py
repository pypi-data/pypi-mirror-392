...
def train_nn(
    ...
    last_test_loss : float
        Test loss of the final epoch.
    last_metric : float
        Metric value of the final epoch.
    ...
    if int(patience) != float(patience): # is fraction
    ...
    for epoch in range(epochs):
        ...
        if improved:
            ...
            best_epoch = metrics.index(metr_funct(metrics)) + 1
        ...
        if (epoch % max(1, epochs // verb_print) == 0 or epoch == epochs-1) and verbose != 0:
            print(f"Epoch {epoch+starting_done_epochs+1} - Test Loss: {round(test_loss, digits)} - "
                  f"Train Loss: {round(loss.item(), digits)} - Metric: {round(metr, digits)}")
    ...
    if plot:
        ...
        plt.figure(figsize=(8,4))
        plt.plot(metrics, color="blue", label="Metric")
    ...
    print(f"\nBest metric: {round(best_metric, digits)} at epoch {best_epoch}")
    ...
    print(f"Last model's metric: {round(last_metric, digits)}")
    
    if patience is not None:
        last_improve = abs(epochs_counter[0] - (metrics.index(metr_funct(metrics)) + 1))
        if not triggered:
