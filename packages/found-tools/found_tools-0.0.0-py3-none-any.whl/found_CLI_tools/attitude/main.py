...
from found_CLI_tools.attitude.transform import Attitude, DCM
...
def parse_args() -> Tuple[DCM, DCM, int]:  # pragma: no cover
    ...
    return (
        ...
        else Attitude(*args.local_attitude).to_dcm(),
        calibration_attitude,
        args.num_attitude_pairs,
    )
...
def output_result(
    ...
    def print_attitude(banner: str, attitude: Attitude, end=LF):
        if end is None:
            print(
                f"{banner:25}",
                f"RA: {attitude.ra:>7.3f}, DE: {attitude.de:>7.3f}, ROLL: {attitude.roll:>7.3f}",
            )
        else:
            print(
                f"{banner:25}",
    ...
    def print_distance_commands(loc_attitude: Attitude, ref_attitude: Attitude):
        generator_cmd = (
            f"Generator Command:{LF}tools.generator --position <Position> {SLASHLF}"
            + f"--orientation {loc_attitude.to_generator_format()} {SLASHLF}"
            + f"<camera_parameters> {SLASHLF}"
            + "--filename <generated_image_name>"
        )
        distance_cmd = (
            f"Distance Command:{LF}./build/bin/found distance {SLASHLF}"
    ...
    CALIBRATION_COMMAND = (
        f"Calibration Command:{LF}./build/bin/found calibration {SLASHLF}"
