...
async def get_full_storage_zone(
    region: str, zone: str, password: str, dir: str
) -> Generator[str, None, None]:
    url = f"https://{region_prefix(region)}storage.bunnycdn.com/{zone}/{dir}/"
    aclient = httpx.AsyncClient(headers=HEADERS)
    ...
    tasks = [get_full_storage_zone(region, zone, password, d) for d in directories]
...
async def diff_storage_zone(config: dict[str, Any]) -> list[tuple[str, str]]:
    """
    ...
    """
    remote = {
        f.replace(config["remote_path"] + "/", "", 1) if config["remote_path"] else f
        for f in await get_full_storage_zone(
            config["region"],
            config["storage_zone"],
            config["password"],
            config["remote_path"],
    ...
    for f in only_remote:
        if is_ignored(f, config.get("ignore_remote", [])):
...
def show_plan(file_status_list: list[tuple[str, str]], verbosity: int) -> None:
    ...
    for filename, status in file_status_list:
        ...
        if verbosity >= 1:
            ...
            elif status == "ignored" or status == "remote_ignored":
                if verbosity >= 2:
...
def execute_plan(
    ...
    for filename, status in file_status_list:
        match status:
            case "only_local":
                upload_file(config, filename, local_dir)
                upload_count += 1
            case "only_remote":
                if delete:
                    delete_remote_file(config, filename)
                delete_count += 1
            case "both":
                # TODO: check status using hash
                upload_file(config, filename, local_dir)
...
def _path_url(config, path):
    if config["remote_path"]:
        full_path = f"{config['remote_path']}/{path}"
    else:
        full_path = path
    zone = config["storage_zone"]
    return f"https://{region_prefix(config['region'])}storage.bunnycdn.com/{zone}/{full_path}"
...
def upload_file(
    ...
    with open(local_dir / path, "rb") as file:
        resp = client.put(
            url, headers=HEADERS | {"AccessKey": config["password"]}, data=file
        )
        # expect 201
...
def delete_remote_file(config: dict[str, Any], path: Path) -> None:
    """
    ...
    """
    url = _path_url(config, path)
    resp = client.delete(url, headers=HEADERS | {"AccessKey": config["password"]})
...
def status(
    ...
    config = load_config()
    start = time.time()
    plan = asyncio.run(diff_storage_zone(config))
    elapsed = time.time() - start
    debug(f"took {elapsed:.1f}s")
