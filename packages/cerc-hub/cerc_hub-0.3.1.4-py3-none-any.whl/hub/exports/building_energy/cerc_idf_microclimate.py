"""
CercIdfMicroclimate Idf exports one city or some buildings to idf format
...
from hub.exports.building_energy.idf_helper.idf_file_scheduleMC import IdfFileScheduleMC
...
from hub.exports.building_energy.idf_helper.idf_ems_sensor import IdfEmsSensor
from hub.exports.building_energy.idf_helper.idf_ems_actuator import IdfEmsActuator
from hub.exports.building_energy.idf_helper.idf_ems_program_calling_manager import IdfEmsProgramCallingManager
from hub.exports.building_energy.idf_helper.idf_ems_program import IdfEmsProgram

class CercIdfMicroclimate(IdfBase):
  """
  Exports city to IDF with additional microclimate-related objects
  """
  def __init__(self, city, output_path, idf_file_path, idd_file_path, epw_file_path, target_buildings=None, outputs=None):
    ...
    self._add_file_scheduleMC = IdfFileScheduleMC.add
    ...
    self._add_ems_sensor=IdfEmsSensor.add
    self._add_ems_actuator=IdfEmsActuator.add
    self._add_ems_program_calling_manager =IdfEmsProgramCallingManager.add
    self._add_ems_program=IdfEmsProgram.add
  ...
  def _export(self):
    calling_manager = False
    # Define empty lists to hold EMS variable names
    ems_sensor_names = []  # will store EMS sensor names
    ems_actuator_names = []  # will store EMS actuator names
    for building in self.city.buildings:
      is_target = building.name in self._target_buildings or building.name in self._adjacent_buildings

      # Objects related to Microclimate modeling
      if not calling_manager:
        calling_manager = True
        self._add_ems_program_calling_manager(self, "UpdateMC Temperature Each Hour","BeginZoneTimestepBeforeSetCurrentWeather","Update_MC_Temperature")

      if is_target:
        # Exporting building-level objects
        self._add_file_scheduleMC(self, "AvgBuildingTemp", 2, "MCdata.csv")
        self._add_ems_sensor(self, "AvgBuildingTemp_Sensor", "AvgBuildingTemp", "Schedule Value")
        self._add_ems_actuator(self, "AvgBuildingTemp_Act", "Environment", "Weather Data", "Outdoor Dry Bulb")
        ems_sensor_names.append("AvgBuildingTemp_Sensor")
        ems_actuator_names.append("AvgBuildingTemp_Act")

        # Exporting zone-level objects
        for storey, thermal_zone in enumerate(building.thermal_zones_from_internal_zones):
          for index, boundary in enumerate(thermal_zone.thermal_boundaries):
            srfName = f'Building_{building.name}_storey_{storey}_surface_{index}'
            srfType = boundary.parent_surface.type
            srfID = boundary.parent_surface.id
            if srfType in ["Roof",
                           "Wall"] and boundary.parent_surface.percentage_shared is not None and boundary.parent_surface.percentage_shared < 0.5:
              schedule_name = f"Surf{srfID}_Temp"
              sensor_name = f"Surf{srfID}_Sensor"
              actuator_name = f"Surf{srfID}_Ta_Act"
              actuator_unique_name = srfName
              column_number = int(srfID) + 2
              ems_sensor_names.append(sensor_name)
              ems_actuator_names.append(actuator_name)
              self._add_file_scheduleMC(self, schedule_name, column_number, "MCdata.csv")
              self._add_ems_sensor(self, sensor_name, schedule_name, "Schedule Value")
              self._add_ems_actuator(self, actuator_name, actuator_unique_name, "Surface","Outdoor Air Drybulb Temperature")
    ...
    # Write EMS program once for all buildings
    self._add_ems_program(self, "Update_MC_Temperature",ems_sensor_names, ems_actuator_names)
