...
class VSExpose:
    ...
    def _start_vscode(self, port: int = 8080, log_file: str = 'vscolab.log'):
        print("Starting VSCode server...",end='\r')
    ...
    def _start_ngrok(self, auth_token: Optional[str] = None, port: int = 8080, log_file: str = 'ngrok.log', silent: bool = False):
        print("Starting ngrok tunnel...",end='\r')
        if auth_token:
            print("Configuring ngrok auth token...",end='\r')
            run_command(f"ngrok authtoken {auth_token}")
            print("✅ ngrok auth token configured.",end='\r')
        if not silent:
            ...
            while True:
                ...
                if result.returncode == 0 and '"public_url":"' in result.stdout:
                    ...
                    if server_url_match:
                        ...
                        print(f" - Your Server: {server_url}")
                        break
                if time() - t > 15:
            ...
            while True:
                ...
                if password:
                    ...
                    break
                if time() - t > 15:
                    print("Timeout waiting for access password.")
                    break
