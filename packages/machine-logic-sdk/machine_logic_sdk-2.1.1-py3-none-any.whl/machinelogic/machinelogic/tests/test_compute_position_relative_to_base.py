...
from machinelogic.ivention.irobot import CartesianPose
    ...
    def _assert_poses_close(self, pose1: CartesianPose, pose2: CartesianPose) -> None:
        ...
        for i, (val1, val2) in enumerate(zip(pose1, pose2)):
            self.assertAlmostEqual(
                val1, val2, places=5, msg=f"Poses differ at index {i}: {val1} != {val2}"
            )
...
class MockReferenceFrameConfiguration(IReferenceFrameConfiguration):
    ...
    def __init__(
        self, uuid: str, name: str, default_position: CartesianPose, parent_id: str = ""
    ):
...
class MockReferenceFrame(IReferenceFrame):
    ...
    def __init__(
        ...
        self.__configuration = MockReferenceFrameConfiguration(
            uuid, name, default_position, parent_id
        )
...
class MockCalibrationFrame(ICalibrationFrame):
    ...
    def __init__(
        ...
        self.__configuration = MockCalibrationFrameConfiguration(
            uuid, name, default_value, calibrated_value
        )
...
class MockCartesianTarget(ICartesianTarget):
    ...
    def __init__(
        ...
        self.__configuration = MockCartesianTargetConfiguration(
            uuid, name, default_position, parent_id
        )
...
class TestComputePositionRelativeToBase(MatrixAssertionsMixin):
    ...
    def test_calibration_frame_with_no_calibrated_value_returns_default_value(
        ...
        calibration_frame = MockCalibrationFrame(
            self.calibration_frame_id, "test_calibration", default_value, None
        )
    ...
    def test_reference_frame_without_parent_raises_exception(self) -> None:
        reference_frame = MockReferenceFrame(
            self.child_frame_id, "test_ref", CARTESIAN_POSE, "non_existent_parent"
        )
        ...
        self.assertIn(
            "No asset found with id non_existent_parent", str(context.exception)
        )

    def test_cartesian_target_without_parent_raises_exception(self) -> None:
        cartesian_target = MockCartesianTarget(
            self.child_frame_id, "test_target", CARTESIAN_POSE, "non_existent_parent"
        )
    ...
    def test_reference_frame_with_calibration_frame_parent(self) -> None:
        ...
        child_frame = MockReferenceFrame(
            self.child_frame_id, "child_ref", child_pose, self.parent_frame_id
        )
    ...
    def test_reference_frame_with_reference_frame_parent(self) -> None:
        ...
        parent_frame = MockReferenceFrame(
            self.parent_frame_id, "parent_ref", parent_pose, self.base_frame_id
        )
    ...
    def test_cartesian_target_with_calibration_frame_parent(self) -> None:
        ...
        cartesian_target = MockCartesianTarget(
            self.child_frame_id, "test_target", target_pose, self.parent_frame_id
        )
    ...
    def test_identity_transform_composition(self) -> None:
        ...
        parent_frame = MockCalibrationFrame(
            self.parent_frame_id, "parent_calibration", IDENTITY_POSE, IDENTITY_POSE
        )

        child_frame = MockReferenceFrame(
            self.child_frame_id, "child_ref", IDENTITY_POSE, self.parent_frame_id
        )
    ...
    def test_translation_only_transform(self) -> None:
        ...
        child_frame = MockReferenceFrame(
            self.child_frame_id, "child_ref", child_translation, self.parent_frame_id
        )
    ...
    def test_multiple_asset_types_in_hierarchy(self) -> None:
        base_frame = MockCalibrationFrame(
            self.base_frame_id, "base_calibration", IDENTITY_POSE, PARENT_POSE
        )
        ...
        final_target = MockCartesianTarget(
            self.child_frame_id, "final_target", CHILD_POSE, "intermediate_frame"
        )
    ...
    def test_asset_not_found_in_mixed_asset_list(self) -> None:
        child_frame = MockReferenceFrame(
            self.child_frame_id, "child_ref", CHILD_POSE, "missing_parent"
        )

        other_frame = MockReferenceFrame(
            "other_id", "other_ref", PARENT_POSE, "some_other_parent"
        )
    ...
    def test_recursive_depth_handling(self) -> None:
        ...
        base_calibration = MockCalibrationFrame(
            "base", "base_cal", IDENTITY_POSE, [10.0, 10.0, 10.0, 0.0, 0.0, 0.0]
        )

        ref1 = MockReferenceFrame(
            "ref1", "ref1", [5.0, 5.0, 5.0, 0.0, 0.0, 0.0], "base"
        )

        ref2 = MockReferenceFrame(
            "ref2", "ref2", [2.0, 2.0, 2.0, 0.0, 0.0, 0.0], "ref1"
        )

        target = MockCartesianTarget(
            "target", "target", [1.0, 1.0, 1.0, 0.0, 0.0, 0.0], "ref2"
        )
