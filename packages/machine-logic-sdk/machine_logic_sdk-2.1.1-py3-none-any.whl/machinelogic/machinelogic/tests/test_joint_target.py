...
class TestJointTargetConfiguration(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.joint_target_config = JointTargetConfiguration(
            self.api_mock, self.uuid, self.name, self.joint_angles
        )
...
class TestJointTarget(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.name = "test_joint_target"
        self.joint_angles = [10.0, 20.0, 30.0, 40.0, 50.0, 60.0]
        self.joint_target = JointTarget(
            self.uuid, self.name, self.joint_angles, self.api_mock
        )

    def test_initialization_creates_configuration_with_correct_values(self) -> None:
        self.assertIsInstance(
            self.joint_target._configuration, JointTargetConfiguration
        )
        ...
        self.assertEqual(
            self.joint_target._configuration.joint_angles, self.joint_angles
        )
