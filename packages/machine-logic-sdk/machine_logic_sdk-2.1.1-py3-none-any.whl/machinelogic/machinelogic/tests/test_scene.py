...
class TestScene(unittest.TestCase):
    def setUp(self) -> None:
        ...
        def create_joint_target_json(
            name: str, joint_angles: List[float]
        ) -> JointTargetAsset:
            return {
                "id": "5678",
        ...
        def create_reference_frame_json(
            name: str, cartesian_pose: List[float], parent_id: str
        ) -> ReferenceFrameAsset:
            return {
                "id": "9876",
        ...
        def create_cartesian_target_json(
            name: str, cartesian_pose: List[float], parent_id: str
        ) -> ReferenceFrameAsset:
            return {
                "id": "4321",
        ...
        self.mock_calibration_frame1._configuration = (
            mock_calibration_frame_configuration1
        )

        self.mock_calibration_frame2 = MagicMock(spec=CalibrationFrame)
        self.mock_calibration_frame2._configuration = (
            mock_calibration_frame_configuration2
        )
    ...
    def test_initialize_assets_with_joint_target(self) -> None:
        # Arrange
        joint_target_json = self.create_joint_target_json(
            "target1", [10, 20, 30, 40, 50, 60]
        )
        ...
        self.assertEqual(
            initialized_joint_target_config.joint_angles, expected_joint_angles
        )

    def test_initialize_assets_with_reference_frame(self) -> None:
        # Arrange
        reference_frame_json = self.create_reference_frame_json(
            "ref_frame1", [10, 20, 30, 40, 50, 60], "parent_id_123"
        )
        ...
        self.assertEqual(
            initialized_reference_config.parent_reference_frame_id, expected_parent_id
        )
    ...
    def test_initialize_assets_with_mixed_asset_types(self) -> None:
        ...
        reference_frame_json = self.create_reference_frame_json(
            "ref_frame1", [100, 200, 300, 400, 500, 600], "parent_id_123"
        )
    ...
    @patch("machinelogic.machinelogic.scene.logging.warning")
    def test_get_joint_target_multiple_matches_expect_to_log_warning(
        self, mock_logger_warning: Any
    ) -> None:
        # Arrange
        mock_joint_target = MagicMock()
    ...
    def test_initialize_assets_with_cartesian_target(self) -> None:
        # Arrange
        cartesian_target_json = self.create_cartesian_target_json(
            "cartesian_target1", [10, 20, 30, 40, 50, 60], "parent_id_123"
        )
        ...
        initialized_cartesian_target_config = (
            initialized_cartesian_target._configuration
        )
    ...
    @patch("machinelogic.machinelogic.scene.logging.warning")
    def test_get_cartesian_target_multiple_matches_expect_to_log_warning(
        self, mock_logger_warning: Any
    ) -> None:
        # Arrange
