...
class TestCartesianTargetConfiguration(unittest.TestCase):
    ...
    def test_initialization_sets_properties_correctly(self) -> None:
        ...
        self.assertEqual(
            self.cartesian_target_config.default_position, self.default_position
        )
...
class TestCartesianTarget(MatrixAssertionsMixin):
    ...
    def test_initialization_creates_configuration_with_correct_values(self) -> None:
        self.assertIsInstance(
            self.cartesian_target._configuration, CartesianTargetConfiguration
        )
        ...
        self.assertEqual(
            self.cartesian_target._configuration.default_position, self.default_position
        )
    ...
    def test_get_cartesian_pose_returns_default_position_when_relative_to_is_roobt_base(
        ...
        self._assert_matrices_close(
            numpy.array(result), numpy.array([20.0, 40.0, 30.0, 40.0, 50.0, 60.0])
        )
    ...
    def test_set_asset_list_stores_assets(self) -> None:
        mock_assets: List[
            Union[IReferenceFrame, IJointTarget, ICalibrationFrame, ICartesianTarget]
        ] = [
