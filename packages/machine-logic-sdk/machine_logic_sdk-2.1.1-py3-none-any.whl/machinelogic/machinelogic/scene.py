...
def _find_asset_in_list_by_name(
    ...
    """
    matching_assets = [
        asset for asset in asset_list if asset._configuration.name == name
    ]
    if len(matching_assets) == 0:
        raise SceneException(exception_message.format(name))
    if len(matching_assets) > 1 and warning_message:
        logging.warning(
            warning_message.format(name, matching_assets[0]._configuration.uuid)
        )
...
def _create_joint_target(joint_target_json: JointTargetAsset, api: Api) -> JointTarget:
    ...
    joint_angles_deg = [
        (
            convert_angle(
                joint_angle["value"], joint_angle["unit"], UnitOfAngle.DEGREE
            )["value"]
...
def _create_reference_frame(
    reference_frame_json: ReferenceFrameAsset, api: Api
) -> ReferenceFrame:
...
def _create_cartesian_target(
    cartesian_target_json: CartesianTargetAsset, api: Api
) -> CartesianTarget:
