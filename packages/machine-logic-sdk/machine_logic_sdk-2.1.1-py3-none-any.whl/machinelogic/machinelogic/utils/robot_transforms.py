...
from machinelogic.ivention.types.robot_types import Matrix
...
def euler_to_rotation_matrix(
    ...
    """
    rotation_matrix_rad: Matrix = scipy_rotation.from_euler(
...
def euler_to_homogeneous_transform_matrix(
    ...
    """
    rotation_matrix_rad = euler_to_rotation_matrix(
    ...
    for row_index in range(3):
        ...
        for col_index in range(3):
            homogeneous_transform_matrix[row_index, col_index] = rotation_matrix_rad[
                row_index, col_index
            ]
...
def transform_position_between_ref_frames(
    ...
    """
    position_matrix = euler_to_homogeneous_transform_matrix(
        position, euler_angle_order=euler_angle_order
    )
    from_reference_matrix = euler_to_homogeneous_transform_matrix(
        from_reference_frame, euler_angle_order=euler_angle_order
    )
    to_reference_matrix = euler_to_homogeneous_transform_matrix(
        to_reference_frame, euler_angle_order=euler_angle_order
    )
    transform_matrix = (
        numpy.linalg.inv(to_reference_matrix) @ from_reference_matrix @ position_matrix
    )
