...
class CartesianTarget(ICartesianTarget):
    ...
    def __init__(
        ...
        self.__configuration = CartesianTargetConfiguration(
            api, uuid, name, default_position, parent_reference_frame_id
        )
        self._api = api
    ...
    def get_position(
        self, relative_to: Literal["robot_base", "parent"] = "parent"
    ) -> CartesianPose:
        if relative_to == "robot_base":
            ...
            raise SceneException(
                f"Invalid relative_to value: {relative_to}, expected 'robot_base' or 'parent'."
            )
