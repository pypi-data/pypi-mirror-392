...
class DensityFlow(nn.Module):
    ...
    def model1(self, xs):
        ...
        with pyro.plate('data'):
            ...
            if self.loss_func == 'negbinomial':
                ...
                #logits = (dispersion.log()-mu.log())#.clamp(min=-15, max=15)
                if self.use_zeroinflate:
    ...
    def predict(self, xs, us, perturbs_predict:list, perturbs_reference:list, library_sizes=None, show_progress=True):
        ...
        for pert in perturbs_predict:
            if pert in perturbs_reference:
                ...
                # factor effect of xs 
                dzs0 = self.get_cell_shift(zs, perturb_idx=pert_idx, perturb_us=us_i, show_progress=show_progress)
            
                # perturbation effect 
                ps = np.ones_like(us_i)
                if np.sum(np.abs(ps-us_i))>=1:
                    dzs = self.get_cell_shift(zs, perturb_idx=pert_idx, perturb_us=ps, show_progress=show_progress)          
                    dzs_total = dzs_total + dzs0 + dzs
                else:
                    dzs_total = dzs_total + dzs0
    ...
    def _count(self, log_mu, library_size=None):
        if self.loss_func == 'bernoulli':
            ...
            rate = log_mu.exp()
            #theta = dist.DirichletMultinomial(total_count=1, concentration=rate).mean
            z = torch.sum(rate, dim=1, keepdim=True)
            theta = rate / z
