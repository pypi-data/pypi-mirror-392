...
class DensityFlowNCB(nn.Module):
    def __init__(self,
                 input_size: int,
        ...
        print(f"🧬 DensityFlowNCB Initialized:")
    ...
    def model1(self, xs):
        pyro.module('DensityFlowNCB', self)
    ...
    def fit(self, xs, 
            ...
            use_jax: bool = True):
        """
        Train the DensityFlowNCB model.
        ...
        use_jax
            ...
            the Python script or Jupyter notebook. It is OK if it is used when runing DensityFlowNCB in the shell command.
        """
...
'''    def save(self, path):
    ...
    def load_model(cls, model_path: str):
        ...
        model = DensityFlowNCB(**checkpoint.get('model_config'))
...
EXAMPLE_RUN = (
    "example run: DensityFlowNCB --help"
)

def parse_args():
    parser = argparse.ArgumentParser(
        description="DensityFlowNCB\n{}".format(EXAMPLE_RUN))
...
def main():
    ...
    if args.save_model is not None:
        if args.save_model.endswith('gz'):
            DensityFlowNCB.save_model(df, args.save_model, compression=True)
        else:
            DensityFlowNCB.save_model(df, args.save_model)
