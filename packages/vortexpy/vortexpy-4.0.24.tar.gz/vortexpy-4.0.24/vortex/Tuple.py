...
class Tuple(Jsonable, _TupleToPlainJsonMixin, _TupleToSqlaJsonMixin):
    ...
    * Datetimes and ints are serialised and deserialsied
    ...
    def tupleClone(self, deep=False):
        clone = self.__class__()

        def cloneVal(anyVal: Any):
            if isinstance(anyVal, list):
                return [cloneVal(v) for v in anyVal]
            if isinstance(anyVal, Tuple):
                return anyVal.tupleClone()
            return anyVal
