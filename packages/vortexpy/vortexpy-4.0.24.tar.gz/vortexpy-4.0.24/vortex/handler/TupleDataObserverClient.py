...
class TupleDataObserverClient(TupleDataObservableCache):
    def __init__(
        ...
        VortexFactory.subscribeToVortexStatusChange(destVortexName).pipe(
            ops.filter(lambda online: online is True)
    ...
    def _sendUnsubscribeToServer(self, tupleSelector: TupleSelector):
        logger.debug("Sending unsubscribe for: %s", tupleSelector.name)
        payload = PayloadEnvelope(self._sendFilt.copy())
