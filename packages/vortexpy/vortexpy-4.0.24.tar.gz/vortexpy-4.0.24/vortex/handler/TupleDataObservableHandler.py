        ...
        **kwargs,
        ...
        if payloadEnvelope.filt.get("unsubscribe"):
            if tsStr in self._observerDataByTupleSelector:
                ...
                if observerDetails in observers:
                    observers.remove(observerDetails)
                    logger.debug(f"Processed unsubscribe for: {tsStr}")
                else:
                    logger.debug(
                        "Unsubscribe called for missing subscription: %s",
                        tsStr,
                    )

                if not self._observerDataByTupleSelector[tsStr].observers:
                    logger.debug(f"Last subscription unsubscribed for {tsStr}")
                    del self._observerDataByTupleSelector[tsStr]
            else:
                logger.debug(
                    "Unsubscribe called for non-existent tuple selector: %s",
                    tsStr,
                )
