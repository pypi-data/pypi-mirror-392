...
class ILController_RSST_AG:
    ...
    def __do_init_measurement(self):
        ...
        self.phase_shift = self.__get_phase_shift(ref_signals=signal,shift_signals=self.B_meas)
    ...
    def __postprocessing_measurement_data(self):
        U_B_meas_x = self.ss.readData(self.steps_iteration,"Bx")
        U_B_meas_y = self.ss.readData(self.steps_iteration,"By")
        ...
        U_meas_x = self.ss.readData(self.steps_iteration,"Ux")*self.ss.readInfoValue("Rohrer_voltage_factor")
        U_meas_y = self.ss.readData(self.steps_iteration,"Uy")*self.ss.readInfoValue("Rohrer_voltage_factor")
        self.U_meas = np.array([U_meas_x,U_meas_y])

        I_meas_x = self.ss.readData(self.steps_iteration,"Ix")*self.ss.readInfoValue("Rohrer_current_factor")
        I_meas_y = self.ss.readData(self.steps_iteration,"Iy")*self.ss.readInfoValue("Rohrer_current_factor")
