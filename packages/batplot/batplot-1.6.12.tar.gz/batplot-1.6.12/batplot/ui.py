...
def position_top_xlabel(ax, fig, tick_state: Dict[str, bool]):
    try:
        ...
        if on:
            ...
            # Apply manual offset (stored in points) if user nudged the duplicate title
            try:
                manual_pts = float(getattr(ax, '_top_xlabel_manual_offset_pts', 0.0) or 0.0)
            except Exception:
                manual_pts = 0.0
            dy_pts += manual_pts
...
def position_right_ylabel(ax, fig, tick_state: Dict[str, bool]):
    try:
        ...
        if on:
            ...
            # Apply manual offset (stored in points) if user nudged the duplicate title
            try:
                manual_pts = float(getattr(ax, '_right_ylabel_manual_offset_pts', 0.0) or 0.0)
            except Exception:
                manual_pts = 0.0
            dx_pts += manual_pts
