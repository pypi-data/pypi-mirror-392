# coding: utf-8

"""
    Fortanix SDKMS REST API

    This is a set of REST APIs for accessing the Fortanix Self-Defending Key Management System. This includes APIs for managing accounts, and for performing cryptographic and key management operations. 

    OpenAPI spec version: 1.0.0-20200608
    Contact: support@fortanix.com
    Generated by: https://github.com/swagger-api/swagger-codegen.git

    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at
    
        http://www.apache.org/licenses/LICENSE-2.0
    
    Unless required by applicable law or agreed to in writing, software
    distributed under the License is distributed on an "AS IS" BASIS,
    WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
    See the License for the specific language governing permissions and
    limitations under the License.
"""


from __future__ import absolute_import

import os
import sys
import unittest

import sdkms/v1
from sdkms/v1.rest import ApiException
from sdkms/v1.apis.groups_api import GroupsApi


class TestGroupsApi(unittest.TestCase):
    """ GroupsApi unit test stubs """

    def setUp(self):
        self.api = sdkms/v1.apis.groups_api.GroupsApi()

    def tearDown(self):
        pass

    def test_azure_vaults(self):
        """
        Test case for azure_vaults

        List all Azure Key Vaults available to back a group
        """
        pass

    def test_check_group_hmg(self):
        """
        Test case for check_group_hmg

        Check the HMG configuration of a group
        """
        pass

    def test_check_hmg(self):
        """
        Test case for check_hmg

        Check a HMG configuration
        """
        pass

    def test_create_group(self):
        """
        Test case for create_group

        Create new group
        """
        pass

    def test_delete_group(self):
        """
        Test case for delete_group

        Delete group
        """
        pass

    def test_get_group(self):
        """
        Test case for get_group

        Get a specific group
        """
        pass

    def test_get_groups(self):
        """
        Test case for get_groups

        Get all groups
        """
        pass

    def test_scan_hmg(self):
        """
        Test case for scan_hmg

        Scan all keys from the external source for a particular group
        """
        pass

    def test_update_group(self):
        """
        Test case for update_group

        Update group
        """
        pass


if __name__ == '__main__':
    unittest.main()
