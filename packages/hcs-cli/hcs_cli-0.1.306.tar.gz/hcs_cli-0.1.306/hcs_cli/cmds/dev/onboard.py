...
def start_onboard(mode: str):
    ...
    ret = run_cli("hcs api --post /deployment-orchestrator/v1/deployments", inherit_output=False, input=payload)
    _show_deployment_status(ret)
    _wait_for_deployment_complete()
...
def show():
    ...
    deployment = run_cli("hcs api /deployment-orchestrator/v1/deployments", output_json=True)
    if not deployment:
...
def get():
    ...
    return run_cli("hcs api /deployment-orchestrator/v1/deployments", output_json=True)
...
def delete(all: bool):
    ...
    deployment_id = _delete_express_onboarding_deployment()

    if all:
        _delete_all("pool")
        _delete_all("template")
        _delete_all("edge")
        _delete_all("provider")
        _delete_all("site")
        _delete_all_sso_configs()
        _delete_all("ad")

    _wait_for_deployment_deletion(deployment_id)

    click.echo("Reset complete.")


def _delete_express_onboarding_deployment():
    deployment = run_cli("hcs api /deployment-orchestrator/v1/deployments", output_json=True)
    if not deployment:
        click.echo("No express onboarding deployment found.")
        return
    ...
    try:
        run_cli("hcs api --delete /deployment-orchestrator/v1/resources", inherit_output=False, raise_on_error=True, log_error=False)
    except Exception as e:
        click.secho(error_details(e), fg="bright_black")
    click.echo("Deleting express onboarding deployment: " + deployment_id)
    try:
        run_cli("hcs api --delete /deployment-orchestrator/v1/deployments", inherit_output=False, raise_on_error=True, log_error=False)
    except Exception as e:
        click.secho(error_details(e), fg="bright_black")
    _show_deployment_status(deployment)
...
def _wait_for_deployment_complete():
    ...
    while True:
        sleep(60)
        deployment = run_cli("hcs api /deployment-orchestrator/v1/deployments", output_json=True, show_command=False, inherit_output=False)
        _show_deployment_status(deployment)
        elapsed = time() - start
        if elapsed > timeout:
            click.secho("Timeout waiting for express onboarding deployment to be deleted.", fg="bright_red")
        if deployment.get("deploymentStatus") in ["COMPLETED", "CANCELLED", "TIMEOUT"]:
            click.echo("Express onboarding deployment completed with status: " + deployment.get("deploymentStatus"))
            break


def _wait_for_deployment_deletion(deployment_id):
    ...
    while True:
        ...
        try:
            run_cli(
                "hcs api --delete /deployment-orchestrator/v1/deployments",
            ...
            click.secho(error_details(e), fg="bright_black")
            sleep(5)
        deployment = run_cli("hcs api /deployment-orchestrator/v1/deployments", output_json=True, show_command=False)
        if not deployment:
...
def _delete_all_sso_configs():
    sso_configs = run_cli("hcs api /admin/v1/sso-configurations --all-pages --ids", output_json=True)
    for id in sso_configs:
        run_cli(f"hcs api --delete /admin/v1/sso-configurations/{id}", inherit_output=False)


def _delete_all(entity_type: str):
    click.echo(f"Deleting all {entity_type}s...")
    items = run_cli(f"hcs {entity_type} list --ids", output_json=True)
    if not items:
        return

    for id in items:
        try:
            run_cli(f"hcs {entity_type} delete {id} -y", inherit_output=False)
        except Exception as e:
            click.secho(f"Error deleting {entity_type} {id}: {e}. Retry in 10 seconds...", fg="bright_red")
            sleep(10)
            run_cli(f"hcs {entity_type} delete {id} -y", inherit_output=True)
