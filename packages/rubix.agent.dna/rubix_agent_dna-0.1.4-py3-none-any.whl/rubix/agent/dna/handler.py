...
class RubixMessageHandler:
    ...
    #     """
    #     Execute an NFT using Rubix SDK. `payload` can be dict/list/etc.
    #     It will be JSON-encoded into `nft_data`.
    #     """
    #     load_dotenv()
    #     # Use the host node + mnemonic
    #     node = NodeClient(framework="host")
    #     DEFAULT_BASE_URL = node.get_base_url()


    #     MNEMONIC = os.getenv("HOST_MNEMONIC")
    #     if not MNEMONIC:
    #         raise RuntimeError("❌ HOST_MNEMONIC is missing in .env file!")

    #     client = RubixClient(node_url=DEFAULT_BASE_URL, timeout=300)
    #     signer = Signer(rubixClient=client, mnemonic=MNEMONIC)
    #     print("Signing DID:", signer.did)

    #     DEFAULT_DID = signer.did
    ...
    #         raise APIError(f"Rubix execute_nft call failed: {e}") from e
    ...
    async def handle_response_parts(
        ...
        *,
        ...
        """
        Parse remote parts, verify signatures, optionally write to NFT.
