...
from .helpers import env_exists, env_as_bool
...
class S3:
    ...
    def __init__(self, create_buckets: bool = True):
        ...
        self.skip_bucket_discovery = env_as_bool(
            os.getenv('MONGO_S3_SKIP_BUCKET_DISCOVERY'), False)
    ...
    def _make_azure_client(self) -> Any:
        ...
        if env_exists(azure_connection_string):
            ...
            try:
                ...
                if self.skip_bucket_discovery:
                    logging.info(
                        'Skipping Azure bucket discovery; assuming container "%s" exists.',
                        self.s3_bucket)
                    return azure_s3
                azure_buckets = [
    ...
    def _make_aws_client(self) -> Any:
        ...
        if env_exists(aws_access_key_id) and env_exists(aws_secret_access_key):
            ...
            try:
                ...
                if self.skip_bucket_discovery:
                    logging.info(
                        'Skipping AWS bucket discovery; assuming bucket "%s" exists.',
                        self.s3_bucket)
                    return aws_s3
                aws_buckets = [
                    bucket['Name']
    ...
    def _make_gcp_client(self) -> Any:
        ...
        if env_exists(google_application_credentials) and Path(
            ...
            try:
                ...
                if self.skip_bucket_discovery:
                    logging.info(
                        'Skipping GCP bucket discovery; assuming bucket "%s" exists.',
                        self.s3_bucket)
                    return gcp_s3
