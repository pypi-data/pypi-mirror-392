...
logger = logging.getLogger(
    "toolops.generation.test_case_generation.test_case_generation_utils.combination_modules.mandatory_param_testcase"
)


# This funcmethodtion generates a testcase with only mandatory paramters derived from all paramter testcase #
def mandatory_param_testcase(
    ...
    if "testcase_1" in data_generated_through_LLM:
        if len(data_generated_through_LLM["testcase_1"]) > 0:
            ...
            data_generated_through_LLM_copy = copy.deepcopy(
                data_generated_through_LLM["testcase_1"]
            )
            if "__requestBody__" in data_generated_through_LLM_copy:
                for key in list(
                    data_generated_through_LLM_copy["__requestBody__"].keys()
                ):
                    data_generated_through_LLM_copy[key] = (
                        data_generated_through_LLM_copy["__requestBody__"][key]
                    )
            ...
            if len(temp_mandatory_testcase) > 0:
                ...
                number_testcases_to_generate_more = (
                    number_testcases_to_generate_more - 1
                )
                count_testcases = count_testcases + 1
    logger.info(
        "One testcase generated with only mandatory parameters",
        extra={"details": {"testcase_2": data_generated_through_LLM["testcase_2"]}},
    )
    return (
        data_generated_through_LLM,
        number_testcases_to_generate_more,
