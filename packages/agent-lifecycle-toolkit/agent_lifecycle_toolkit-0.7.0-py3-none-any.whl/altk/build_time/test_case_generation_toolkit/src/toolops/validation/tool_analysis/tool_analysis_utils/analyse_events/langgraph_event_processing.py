...
def get_tool_name_args_from_event(first_event):
    ...
    try:
        ...
        logger.info(
            "Problem in parsing the first agentic flow execution event due to incorrect format",
            extra={
                "details": json.dumps({"first_event": first_event, "error": str(e)})
            },
        )
        pass
...
def get_mandatory_optional_parameters(test_scenarios):
    ...
    try:
        for ts in test_scenarios:
            ...
            input_params = [
                param_f.split("((")[0].strip()
                for param_f in ts.get("input").split("\n")
            ]
        ...
        logger.info(
            "Problem in getting mandatory parameters, list of all parameters due to issue in test scenarios",
            extra={"details": str(e)},
        )
        pass
...
def get_param_type_in_tool_input(tool_inputs, param_):
    ...
    if param_ in tool_inputs:
        if isinstance(tool_inputs.get(param_), str):
            param_type = "string"
        elif isinstance(tool_inputs.get(param_), list):
            param_type = "array"
        elif isinstance(tool_inputs.get(param_), int):
            param_type = "integer"
        elif isinstance(tool_inputs.get(param_), float):
            param_type = "number"
        elif isinstance(tool_inputs.get(param_), dict):
            param_type = "object"
    return param_type
...
def get_tool_inputs_from_agent_schema(tool_args_dict, tool_inputs=None):
    if not tool_inputs:
        tool_inputs = {}
    try:
...
def check_parameter_type_mismatch(test_scenario, agentic_flow_events):
    ...
    try:
        identified_tool_name, tool_args = get_tool_name_args_from_event(
            agentic_flow_events[0]
        )
        ...
        if test_case_tool_input is not None and test_case_tool_input != "":
            ...
            for inp_ in inputs_list:
                ...
                param_type = fix_param_type_variations(
                    inp_.split("((")[-1].split("))")[0].strip()
                )
                tool_input_param_type = get_param_type_in_tool_input(
                    tool_inputs, param_
                )
                    ...
                    type_mismatches.append(
                        {
                            ...
                            "tool_input_param_type": tool_input_param_type,
                        }
                    )
        if type_mismatches != []:
            error_taxonomy = "Incorrect tool inputs - Parameter Type Mismatch\n"
            parameter_type_mismatch_msg = (
                "Check tool input parameter type mismatch for : "
                + str(type_mismatches)
                + " , *** please modify the tool input processing code accordingly ***\n"
            )
    except Exception as e:
        logger.info(
            "Problem in checking parameter type mismatch errors due to issue in test scenario",
            extra={"details": str(e)},
        )
        pass
                ...
                if param_ in tool_call_inputs:
                    ...
                    if str(param_val) != tool_ip_par_val and len(param_) >= 1:
                        value_mismatches.append(
                            {
                                "parameter": param_,
                                "expected_param_value": param_val,
                                "tool_input_param_value": tool_ip_par_val,
                            }
                        )
        if value_mismatches != []:
            error_taxonomy = "Incorrect tool inputs - Parameter Value Mismatch\n"
            parameter_value_mismatch_msg = (
                "Check tool input parameter value mismatch for : "
                + str(value_mismatches)
                + " , *** please provide parameter descriptions,examples in the tool defintion, for the agent to correctly identify parameter values\n"
            )
    except Exception as e:
        logger.info(
            "Problem in checking parameter value mismatch errors due to issue in test scenario, agentic flow events",
            extra={"details": str(e)},
        )
        pass
    ...
    try:
        ...
        if tool_inputs != {}:
            if mandatory_params != []:
                ...
                if mis_man_params != []:
                    payload_check_msg += " Missing mandatory parameters - " + str(
                        mis_man_params
                    )
            if all_params != []:
                in_correct_params = []
                ...
                if in_correct_params != []:
                    payload_check_msg += " Incorrect tool parameters - " + str(
                        in_correct_params
                    )
        if payload_check_msg != "":
            error_taxonomy = "Incorrect tool input payload\n"
            final_payload_msg = (
                "Check tool input payload for : "
                + payload_check_msg
                + ", *** please modify tool input processing code accordingly, provide parameter descriptions in the tool defintion for the agent to correctly identify parameters ***\n"
            )
    except Exception as e:
        logger.info(
            "Problem in checking tool input payload errors due to issue in agentic flow events, parameter list",
            extra={"details": str(e)},
        )
        pass
...
def check_agent_tool_invocation_error(agentic_flow_events):
    ...
    try:
        ...
        if identified_tool_name != "" and len(agentic_flow_events) == 1:
            ...
            agent_tool_invocation_error = (
                "Agent could not invoke tool calling for the identified tool - "
                + identified_tool_name
                + " , *** please check the tool definition ***\n"
            )
    except Exception as e:
        logger.info(
            "Problem in checking agent tool calling error due to issue in agentic flow events ",
            extra={"details": str(e)},
        )
        pass
...
def check_agent_tool_inputs_issue(agentic_flow_events, tool_input_format=None):
    ...
    try:
        ...
        try:
            ...
            tool_input_format = json.loads(tool_args)
            if tool_input_format is not None:
                # TODO: add code to compare tool input format schema with received tool inputs
                pass
        except Exception:
            ...
            agent_tool_inputs_error = (
                "Agent could not invoke tool calling for the identified tool - "
                + identified_tool_name
                + " due to malformed tool input payload format , *** please modify the tool input processing code *** \n"
            )
            pass
    except Exception as e:
        logger.info(
            "Problem in checking agent tool input format error due to issue in agentic flow events ",
            extra={"details": str(e)},
        )
        pass
...
def check_recursive_tool_calling(agentic_flow_events, repetition_threshold=2):
    ...
    try:
        ...
        if max_tool_cal_count >= repetition_threshold:
            ...
            repeated_tool_calling = (
                "Agent invoked the tool repeatedly several times, tools invoked - "
                + str(tool_call_counter)
                + ", *** please modify the output returned in the tool definition accordingly, such that agent will not invoke the tool recursively ***\n"
            )
    except Exception as e:
        logger.info(
            "Problem in checking agent recursive tool calling error due to issue in agentic flow events ",
            extra={"details": str(e)},
        )
        pass
...
def check_llm_parsing_error(agentic_flow_events, tool_output_format=None):
    ...
    try:
        ...
        logger.info(
            "Problem in checking llm parsing error due to issue in agentic flow events ",
            extra={"details": str(e)},
        )
        pass
...
def check_llm_token_limit_error(agentic_flow_events):
    ...
    try:
        ...
        logger.info(
            "Problem in checking llm token limit error due to issue in agentic flow events ",
            extra={"details": str(e)},
        )
        pass
...
def check_correct_tool_identification(api_tool_name, agentic_flow_events):
    ...
    try:
        ...
        if api_tool_name != identified_tool_name:
            ...
            tool_identification_error = (
                "Agent identified incorrect tool - expected tool : "
                + api_tool_name
                + ", identified tool"
                + identified_tool_name
                + ", *** please modify or add the tool descriptions accordingly ***\n"
            )
    except Exception as e:
        logger.info(
            "Problem in checking correct tool identification error due to issue in agentic flow events ",
            extra={"details": str(e)},
        )
        pass
...
def get_tool_final_output(agentic_flow_events):
    ...
    try:
        ...
        for ae in agentic_flow_events:
            if "turn_event" in ae:
                ...
                if "tools" in tool_event:
                    final_act_event_output = (
                        tool_event.get("tools")
                        .get("messages")[0]
                        .get("kwargs")
                        .get("content")
                    )
        ...
        logger.info(
            "Problem in obtaining tool final output due to issue in agentic flow events ",
            extra={"details": str(e)},
        )
        pass
...
def check_tool_api_errors(test_scenario, agentic_flow_events):
    ...
    """
    error_taxonomy = ""
    tool_api_error_recommendation = ""
    try:
        api_res_code = test_scenario.get("api_testing_output", {}).get(
            "response_code", None
        )
        tool_final_output, tool_execution_status = get_tool_final_output(
            agentic_flow_events
        )
        if isinstance(tool_final_output, dict):
        ...
        if response_code in [401, 403]:
            ...
            tool_api_error_recommendation = (
                "please provide valid credentials for the tool\n"
            )
        elif response_code in [500, 501, 502, 503, 504]:
            error_taxonomy = "Tool back-end server issue\n"
            tool_api_error_recommendation = (
                "Tool server is down , try after some time\n"
            )
        elif response_code in [400, 422]:
            error_taxonomy = "Issue with the inputs provided to the tool\n"
            tool_api_error_recommendation = (
                "Tool input malformed, please provide tool inputs in correct format\n"
            )
        elif response_code in [404]:
            error_taxonomy = "No results found in the tool output\n"
            tool_api_error_recommendation = (
                "Please provide relevant tool input values\n"
            )
        else:
            pass
    except Exception as e:
        logger.info(
            "Problem in checking tool api errors due to issue in test scenario ",
            extra={"details": str(e)},
        )
        pass
    return error_taxonomy, tool_api_error_recommendation
...
def get_tool_test_case_status(test_scenario, agentic_flow_events, taxonomy):
    ...
    if taxonomy != "":
        ...
        taxonomy == "" and tool_res_code is not None and tool_res_code not in [200, 201]
    ):
        test_status = "Failed"
    return test_status
...
def identify_error_taxonomy(test_cases_with_tool_execution):
    ...
    """
    test_scenarios = test_cases_with_tool_execution
    ...
    for test_scenario in test_scenarios:
        ...
        if ts_nl_utterances is not None and ts_nl_utterances != [""]:
            for nl_utterance, tool_output in zip(  # noqa: B007
                ts_nl_utterances, tool_execution_responses
            ):
                ...
                logger.info(
                    "checking incorrect tool identification errors",
                    extra={"details": json.dumps({"tool": api_tool_name})},
                )
                tool_identification_taxonomy, tool_identification_check_msg = (
                    check_correct_tool_identification(
                        api_tool_name, agentic_flow_events
                    )
                )
                ...
                agent_tool_invocation_taxonomy, agent_tool_invocation_check_msg = (
                    check_agent_tool_invocation_error(agentic_flow_events)
                )
                ...
                agent_tool_inputs_taxonomy, agent_tool_inputs_check_msg = (
                    check_agent_tool_inputs_issue(agentic_flow_events)
                )
                if agent_tool_invocation_check_msg != "":
                ...
                logger.info(
                    "checking tool server api errors",
                    extra={"details": json.dumps({"tool": api_tool_name})},
                )
                tool_api_taxonomy, tool_api_recommendations_msg = check_tool_api_errors(
                    test_scenario, agentic_flow_events
                )
                if tool_api_taxonomy != "":
                    taxonomy += tool_api_taxonomy
                    recommendations += tool_api_recommendations_msg
                ...
                tool_payload_taxonomy, tool_payload_check_msg = check_tool_payload(
                    agentic_flow_events, mandatory_params, all_params
                )
                ...
                tool_payload_param_type_taxonomy, tool_payload_param_type_check_msg = (
                    check_parameter_type_mismatch(test_scenario, agentic_flow_events)
                )
                ...
                (
                    tool_payload_param_value_taxonomy,
                    tool_payload_param_value_check_msg,
                ) = check_parameter_value_mismatch(test_scenario, agentic_flow_events)
                ...
                recursive_tool_taxonomy, recursive_tool_calling = (
                    check_recursive_tool_calling(agentic_flow_events)
                )
                ...
                llm_parse_taxonomy, llm_parse_error = check_llm_parsing_error(
                    agentic_flow_events
                )
                if llm_parse_error != "":
                ...
                llm_token_limit_taxonomy, llm_token_limit_error = (
                    check_llm_token_limit_error(agentic_flow_events)
                )
                ...
                ts_status = get_tool_test_case_status(
                    test_scenario, agentic_flow_events, taxonomy
                )
