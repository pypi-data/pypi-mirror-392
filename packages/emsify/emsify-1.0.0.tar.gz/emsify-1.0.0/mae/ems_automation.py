...
class EmsAutomation:
    ...
    def upload_file(self, file_path: str, dropzone_sel: str, timeout: int = 10) -> bool:
        ...
        # Strategy 3: JS drag-drop
        try:
            with path.open('rb') as f:
                b64 = base64.b64encode(f.read()).decode()
            
            js = """
            var target=arguments[0],data=JSON.parse(arguments[1]);
            var dt=new DataTransfer();
            var bin=Uint8Array.from(atob(data.content),c=>c.charCodeAt(0));
            var file=new File([bin],data.name,{type:data.type});
            dt.items.add(file);
            ['dragenter','dragover','drop'].forEach(e=>target.dispatchEvent(new DragEvent(e,{bubbles:true,dataTransfer:dt})));
            return true;
            """
            file_data = json.dumps({'name': path.name, 'content': b64, 
                                   'type': self._get_mime(path)})
            self.dr.execute_script(js, dropzone, file_data)
            return True
        except Exception as e:
            self._handle_error(e, f'upload: {file_path}')
            return False
