...
def test_interact_timeout():
    """Test interact with custom timeout"""
    from mae.ems_automation import EmsAutomation
    ...
    timeout_used = None
    def mock_locate(sel, timeout, **kw):
        nonlocal timeout_used
        timeout_used = timeout
        return elem
    
    ems.locate = mock_locate
    
    result = ems.interact('//button', 'click', timeout=15)
    assert result is True
    assert elem.clicked
    assert timeout_used == 15
    print("[OK] interact respects custom timeout")
