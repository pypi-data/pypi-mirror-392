...
class EmsAutomation:
    ...
    def __init__(self, driver, log_path: Optional[Path] = None, verbose: bool = True):
        ...
        self._current_window = driver.current_window_handle
    ...
    def _check_window_switch(self):
        """Detect window switch and reset frame stack"""
        try:
            current = self.dr.current_window_handle
            if current != self._current_window:
                if self.verbose: print(f'[WINDOW] Switch detected, resetting frame stack')
    ...
    def _locate_shadow(self, sel: str, timeout: int) -> Optional[WebElement]:
        ...
        function findInShadow(root,sel){
            ...
            for(let h of hosts){
                if(h.shadowRoot){
                    el=findInShadow(h.shadowRoot,sel);
                    if(el)return el;
                }
            }
            return null;
        ...
        while time.time() - start < timeout:
            try:
                elem = self.dr.execute_script(js, sel)
                if elem: return elem
            except Exception as e:
                self._handle_error(e, f'shadow_locate: {sel}')
            time.sleep(0.3)
        return None
    
    # ==================== FRAME WALKING ====================
    def _walk_frames(self, sel: str, timeout: int, method: str, search_shadow: bool = False, **kwargs) -> Optional[WebElement]:
        ...
        for frame in self.dr.find_elements(By.CSS_SELECTOR, 'frame,iframe'):
            ...
            # Try Shadow DOM in this frame if search_shadow=True
            if not elem and search_shadow:
                elem = self._locate_shadow(sel, timeout)
            
            if elem: return elem
            
            # Recurse into nested frames
            elem = self._walk_frames(sel, timeout, method, search_shadow, **kwargs)
            if elem: return elem
    ...
    def locate(self, sel: str, timeout: int = 5, method: str = 'selenium', 
               ...
               default_first: bool = True, shadow: bool = False, 
               search_both: bool = False) -> Optional[WebElement]:
        """
        ...
        Args:
            ...
            shadow: Search ONLY Shadow DOM (CSS selectors only)
            search_both: Search regular DOM + Shadow DOM across frames
        
        Note: Leaves driver in found frame for next operation (optimization)
        """
        start = time.time()
        
        # Shadow-only search (original behavior)
        if shadow and not search_both:
            elem = self._locate_shadow(sel, timeout)
            duration = time.time() - start
            if elem: self._log_success(sel, 'shadow', duration)
            else: self._log_failure(sel, 'shadow', duration)
            return elem
        ...
        # A2) Try Shadow DOM in current context if search_both=True
        if not elem and search_both:
            elem = self._locate_shadow(sel, timeout)
        ...
        if default_first:
            ...
            # B2) Try Shadow DOM in default if search_both=True
            if not elem and search_both:
                elem = self._locate_shadow(sel, timeout)
        ...
        elem = self._walk_frames(sel, timeout, method, search_shadow=search_both, **kwargs)
    ...
    # ==================== INTERACTION METHODS ====================
    def click(self, sel: str, timeout: int = 5, force_js: bool = False, shadow: bool = False) -> bool:
        """Click element with fallback strategies"""
        elem = self.locate(sel, timeout, shadow=shadow)
    ...
    def send_keys(self, sel: str, text: str, timeout: int = 5, clear: bool = True, shadow: bool = False) -> bool:
    ...
    def interact(self, sel: str, cmd: str = 'click', value: Any = None, timeout: int = 5, **kwargs) -> Union[bool, str, WebElement, None]:
        """Universal interaction method - alternative to specific methods
        
        Commands: click, send_keys, clear, get_text, get_attr, is_displayed, 
                  is_enabled, hover, double_click, right_click, submit, locate
        
        Args:
            sel: Selector string
            cmd: Command to execute
            value: Value for commands that need it (send_keys, get_attr)
            timeout: Wait timeout in seconds (default: 5)
            **kwargs: Additional arguments passed to locate() (shadow, method, etc.)
        
        Examples:
            ems.interact('//button', 'click')
            ems.interact('id=username', 'send_keys', 'admin', timeout=10)
            text = ems.interact('//div', 'get_text')
            href = ems.interact('//a', 'get_attr', 'href', timeout=3)
        """
        elem = self.locate(sel, timeout=timeout, **kwargs)
        if not elem: return False if cmd not in ('get_text', 'get_attr', 'locate') else None
        
        try:
            if cmd == 'click': elem.click(); return True
            elif cmd == 'send_keys': elem.send_keys(value); return True
            elif cmd == 'clear': elem.clear(); return True
            elif cmd == 'submit': elem.submit(); return True
            elif cmd == 'hover': ActionChains(self.dr).move_to_element(elem).perform(); return True
            elif cmd == 'double_click': ActionChains(self.dr).double_click(elem).perform(); return True
            elif cmd == 'right_click': ActionChains(self.dr).context_click(elem).perform(); return True
            elif cmd == 'get_text': return elem.text
            elif cmd == 'get_attr': return elem.get_attribute(value)
            elif cmd == 'is_displayed': return elem.is_displayed()
            elif cmd == 'is_enabled': return elem.is_enabled()
            elif cmd == 'is_selected': return elem.is_selected()
            elif cmd == 'locate': return elem
            else: raise ValueError(f'Unknown command: {cmd}')
        except Exception as e:
            self._handle_error(e, f'interact({cmd}): {sel}')
            return False if cmd not in ('get_text', 'get_attr', 'locate') else None
    ...
    # ==================== CANVAS INTERACTION ====================
    def find_canvas(self, sel: str = 'canvas', timeout: int = 5) -> Optional[WebElement]:
        """Find canvas element in default or frames"""
        return self.locate(sel, timeout)
    
    def click_canvas(self, canvas_sel: str, x: int, y: int, timeout: int = 5) -> bool:
        """Click canvas at coordinates (x, y)"""
        canvas = self.locate(canvas_sel, timeout)
        if not canvas: return False
        try:
            ActionChains(self.dr).move_to_element_with_offset(canvas, x, y).click().perform()
            if self.verbose: print(f'[CANVAS] Clicked at ({x}, {y})')
            return True
        except Exception as e:
            self._handle_error(e, f'click_canvas: {canvas_sel} at ({x},{y})')
            return False
    
    def get_canvas_data(self, canvas_sel: str, timeout: int = 5) -> Optional[str]:
        """Get canvas image data as base64 PNG"""
        canvas = self.locate(canvas_sel, timeout)
        if not canvas: return None
        try:
            data = self.dr.execute_script('return arguments[0].toDataURL("image/png")', canvas)
            if self.verbose: print(f'[CANVAS] Captured image data')
            return data
        except Exception as e:
            self._handle_error(e, f'get_canvas_data: {canvas_sel}')
            return None
    
    def get_canvas_pixel(self, canvas_sel: str, x: int, y: int, timeout: int = 5) -> Optional[Tuple[int, int, int, int]]:
        """Get RGBA pixel value at (x, y)"""
        canvas = self.locate(canvas_sel, timeout)
        if not canvas: return None
        try:
            pixel = self.dr.execute_script("""
                const canvas = arguments[0];
                const ctx = canvas.getContext('2d');
                const data = ctx.getImageData(arguments[1], arguments[2], 1, 1).data;
                return [data[0], data[1], data[2], data[3]];
            """, canvas, x, y)
            return tuple(pixel) if pixel else None
        except Exception as e:
            self._handle_error(e, f'get_canvas_pixel: {canvas_sel} at ({x},{y})')
            return None
    
    # ==================== WINDOW MANAGEMENT ====================
    def switch_to_new_window(self):
        """Switch to newest window and reset frame tracking"""
        windows = self.dr.window_handles
        if len(windows) > 1:
            self.dr.switch_to.window(windows[-1])
            self._current_window = self.dr.current_window_handle
            self._frame_stack.clear()
            if self.verbose: print(f'[WINDOW] Switched to: {self.dr.title} (frame stack reset)')
