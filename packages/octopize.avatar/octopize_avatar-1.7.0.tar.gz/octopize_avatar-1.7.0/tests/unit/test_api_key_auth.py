...
class TestManagerApiKeyAuth:
    ...
    def test_manager_mutual_exclusivity_api_client_and_api_key(self) -> None:
        ...
        with pytest.raises(
            ValueError,
            match="Cannot provide both 'api_client' and 'api_key'",
        ):
            Manager(
                base_url="http://localhost:8000",
...
class TestApiKeyFormatting:
    ...
    def test_api_key_empty_string(self) -> None:
        """Verify empty string API key is handled."""
        # Empty string is falsy, so it shouldn't set the header
        client = ApiClient(
            base_url="http://localhost:8000",
            api_key="",
            verify_auth=False,
        ...
        # Empty string is falsy, so Authorization header shouldn't be set by api_key
