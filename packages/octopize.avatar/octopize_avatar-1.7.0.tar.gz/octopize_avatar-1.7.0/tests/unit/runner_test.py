...
class TestRunner:
    ...
    @classmethod
    def setup_class(cls):
        cls.manager = Manager("http://localhost:8000", api_client=FakeApiClient())
    ...
    def test_advise_parameters(self):
        manager = Manager("http://localhost:8000", api_client=FakeApiClient(tables=["test"]))
    ...
    def test_advise_parameters_multitable(self):
        manager = Manager(
            "http://localhost:8000", api_client=FakeApiClient(tables=["parent", "child"])
        )
        runner = manager.create_runner("test")
    ...
    def test_get_all_results(self):
        manager = Manager("http://localhost:8000", api_client=FakeApiClient(tables=["parent"]))
        runner = manager.create_runner("test")
        runner.add_table("parent", data=self.df_parent, primary_key="id", individual_level=True)
        runner.set_parameters("parent", k=1)
        runner.run()
