...
class DojoMultiTurnEnv(vf.ToolEnv):
    ...
    async def env_response(self, messages: Messages, state: State, **kwargs: Any) -> Tuple[Messages, State]:
        ...
        while not state.get("started", False):
            ...
            result = await self.engine.get_task_status(exec_id)
        ...
        await self.engine.submit_action(
            exec_id=exec_id,
        ...
        task_response = await self.engine.get_task_status(exec_id)
        ...
        try:
            ...
            await self.engine.submit_step_score(exec_id, task_response.step - 1, step_score)
            if self.verbose:
        ...
        image = await self.engine.get_image(screenshot_path)
    ...
    async def is_completed(self, messages: Messages, state: State, **kwargs: Any) -> bool:
        ...
        if state.get("step", 0) >= state.get("max_steps", 15):
            await self.engine.stop_task(exec_id, status=TaskStatus.COMPLETED)
