# MCP Atlassian INTSIG

Model Context Protocol (MCP) server for Atlassian products (Confluence and Jira). This integration supports both Confluence & Jira Cloud and Server/Data Center (extra support Confluence with INTSIG SSO) deployments.
...
|----------------|--------------------|-----------------------------|
| **Confluence** | INTSIG             | ✅ Fully Supported           |
...
#### A. INTSIG Personal Account (INTSIG Confluence)

1. Use your INTSIG personal account, such as <xxx_xx@intsig.net, pwd>

#### B. API Token Authentication (Cloud)

1. Go to https://id.atlassian.com/manage-profile/security/api-tokens
2. Click **Create API token**, name it
3. Copy the token immediately

#### C. Personal Access Token (Server/Data Center)

1. Go to your profile (avatar) → **Profile** → **Personal Access Tokens**
2. Click **Create token**, name it, set expiry
3. Copy the token immediately

#### D. OAuth 2.0 Authentication (Cloud)
...
5. Run setup wizard:
   ...
   docker run --rm -i \
     -p 8080:8080 \
     -v "${HOME}/.intsig-mcp-atlassian:/home/app/.intsig-mcp-atlassian" \
     registry.intsig.net/confluence-mcp/confluence-mcp:latest --oauth-setup -v
   ```
...
- **No Token Cache Volume:** The Docker volume mount for token storage (e.g., `-v "${HOME}/.intsig-mcp-atlassian:/home/app/.intsig-mcp-atlassian"`) is also not necessary if you are exclusively using the BYOT method, as no tokens are stored or managed by this server.
...
MCP Atlassian is distributed as a Docker image. This is the recommended way to run the server, especially for IDE integration. Ensure you have Docker or Podman installed.

```bash
# Pull Pre-built Image
docker pull registry.intsig.net/confluence-mcp/confluence-mcp:latest
```
...
>
> See the [.env.example](https://github.com/sooperset/intsig-mcp-atlassian/blob/main/.env.example) file for all available options.


### 📝 Configuration Examples

**For INTSIG Confluence:**
  ...
  "mcpServers": {
    "intsig-mcp-atlassian": {
      ...
      "args": [
        ...
        "registry.intsig.net/confluence-mcp/confluence-mcp:latest"
      ],
      "env": {
        "INTSIG_CONFLUENCE_URL": "https://doc.intsig.net",
        "INTSIG_CONFLUENCE_EMAIL": "xxx_xx@intsig.net",
        "INTSIG_CONFLUENCE_PWD": "your_intsig_pwd"
      }
    }
  }
}
  ...
  "mcpServers": {
    "intsig-mcp-atlassian": {
      ...
      "args": [
        ...
        "-i",
        "--env-file",
        "/path/to/your/intsig-mcp-atlassian.env",
        "registry.intsig.net/confluence-mcp/confluence-mcp:latest"
      ]
    }
  }
...
<details>
<summary>Passing Variables Directly</summary>
...
These examples show how to configure `intsig-mcp-atlassian` in your IDE (like Cursor or Claude Desktop) when using OAuth 2.0 for Atlassian Cloud.
  ...
  "mcpServers": {
    "intsig-mcp-atlassian": {
      ...
      "args": [
        ...
        "--rm",
        "-i",
        "-v", "<path_to_your_home>/.intsig-mcp-atlassian:/home/app/.intsig-mcp-atlassian",
        "-e", "JIRA_URL",
        "-e", "CONFLUENCE_URL",
...
>   - The volume mount (`-v .../.intsig-mcp-atlassian:/home/app/.intsig-mcp-atlassian`) is crucial for persisting the OAuth tokens obtained by the wizard, enabling automatic refresh.
...
>   - Token lifecycle (e.g., refreshing the token before it expires and restarting intsig-mcp-atlassian) is your responsibility, as the server will not refresh BYOT tokens.
  ...
  "mcpServers": {
    "intsig-mcp-atlassian": {
      ...
      "env": {
        ...
        "JIRA_USERNAME": "your.email@company.com",
        "JIRA_API_TOKEN": "your_jira_api_token",
        "JIRA_CUSTOM_HEADERS": "X-Forwarded-User=service-account,X-Company-Service=intsig-mcp-atlassian,X-Jira-Client=mcp-integration"
      }
    }
  }
}
  ...
  "mcpServers": {
    "intsig-mcp-atlassian": {
      ...
      "env": {
        "INTSIG_CONFLUENCE_URL": "https://doc.intsig.net",
        "INTSIG_CONFLUENCE_EMAIL": "xxx_xx@intsig.net",
        "INTSIG_CONFLUENCE_PWD": "your password"
      }
    }
  }
}
...
npx @modelcontextprotocol/inspector uvx intsig-mcp-atlassian ...

# For local development version
npx @modelcontextprotocol/inspector uv --directory /path/to/your/intsig-mcp-atlassian run intsig-mcp-atlassian ...
