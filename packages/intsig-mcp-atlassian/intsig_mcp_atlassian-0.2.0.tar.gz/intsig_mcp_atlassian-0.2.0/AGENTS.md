...
| Path | Purpose |
| --- | --- |
| `src/intsig_mcp_atlassian/` | Library source code (Python ≥ 3.10) |
...
```bash
# Running the server
uv run intsig-mcp-atlassian                 # Start server
uv run intsig-mcp-atlassian --oauth-setup   # OAuth wizard
uv run intsig-mcp-atlassian -v              # Verbose mode
