...
from intsig_mcp_atlassian.utils.lifecycle import _shutdown_event
        ...
        for transport in transports_to_test:
            with patch("asyncio.run") as mock_asyncio_run:
                with patch.dict("os.environ", {"TRANSPORT": transport}, clear=False):
                    with (
                        patch(
                            "intsig_mcp_atlassian.servers.main.AtlassianMCP"
                        ...
                        # Execute main
                        with patch("sys.argv", ["intsig-mcp-atlassian"]):
                            try:
                                main()
                            except SystemExit:
                                pass
...
class TestRegressionPrevention:
    ...
    def test_no_stdin_monitoring_in_codebase(self):
        ...
        from intsig_mcp_atlassian.utils import lifecycle
    ...
    def test_signal_handlers_are_setup(self):
        ...
        with patch("intsig_mcp_atlassian.setup_signal_handlers") as mock_setup:
            with patch("asyncio.run"):
                with patch("intsig_mcp_atlassian.servers.main.AtlassianMCP"):
                    with patch("sys.argv", ["intsig-mcp-atlassian"]):
                        try:
                            main()
                        except SystemExit:
