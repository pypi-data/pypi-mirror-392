...
def test_init_from_env():
    ...
    with (
        patch("intsig_mcp_atlassian.jira.config.JiraConfig.from_env") as mock_from_env,
        patch("intsig_mcp_atlassian.jira.client.Jira") as mock_jira,
...
def _test_get_paged(method: Literal["get", "post"]):
    ...
    with (
        patch(
            "intsig_mcp_atlassian.jira.client.Jira.get", new_callable=DeepcopyMock
        ) as mock_get,
        patch(
            "intsig_mcp_atlassian.jira.client.Jira.post", new_callable=DeepcopyMock
        ) as mock_post,
        patch("intsig_mcp_atlassian.jira.client.configure_ssl_verification"),
    ):
        config = JiraConfig(
...
def test_get_paged_without_cloud():
    ...
    with patch("intsig_mcp_atlassian.jira.client.configure_ssl_verification"):
        config = JiraConfig(
            url="https://jira.example.com",
            auth_type="pat",
...
def test_init_sets_proxies_and_no_proxy(monkeypatch):
    ...
    monkeypatch.setattr("intsig_mcp_atlassian.jira.client.Jira", lambda **kwargs: mock_jira)
    monkeypatch.setattr(
        "intsig_mcp_atlassian.jira.client.configure_ssl_verification", lambda **kwargs: None
    )
