...
from ..utils.intsig_sso import (
    perform_sso_login,
    set_basic_auth_header,
    wrap_session_with_reauth,
)
...
class ConfluenceClient:
    ...
    def __init__(self, config: ConfluenceConfig | None = None) -> None:
        ...
        if self.config.auth_type == "oauth":
        ...
        elif self.config.auth_type == "intsig_sso":
            logger.debug(
                f"Initializing Confluence client with INTSIG SSO + Basic auth. "
                f"URL: {self.config.url}, Username: {self.config.username}"
            )
            session = Session()
            # Set Basic header with username:password (password mapped via api_token)
            if not (self.config.username and self.config.api_token):
                raise ValueError("INTSIG SSO requires username and password via INTSIG_* envs")
            set_basic_auth_header(session, self.config.username, self.config.api_token)

            # Initialize Confluence with our prepared session
            self.confluence = Confluence(
                url=self.config.url,
                session=session,
                cloud=False,
            ...
            # Execute SSO flow to obtain cookies and establish session
            try:
                perform_sso_login(
                ...
                # Enable auto re-auth on 401/403
                wrap_session_with_reauth(
                    self.confluence._session,
                    self.config.url,
                    self.config.username,
                ...
                logger.error(f"INTSIG SSO login failed: {e}")
                raise

            logger.debug(
                f"Confluence client (INTSIG) initialized. Session headers (Authorization masked): "
