...
from intsig_mcp_atlassian.jira import JiraFetcher
...
from intsig_mcp_atlassian.utils.io import is_read_only_mode
from intsig_mcp_atlassian.utils.logging import mask_sensitive
from intsig_mcp_atlassian.utils.tools import get_enabled_tools, should_include_tool
...
async def main_lifespan(app: FastMCP[MainAppContext]) -> AsyncIterator[dict]:
    logger.info("Main INTSIG Atlassian MCP server lifespan starting...")
    ...
    try:
        ...
        logger.info("Main INTSIG Atlassian MCP server lifespan shutting down...")
        ...
        logger.info("Main INTSIG Atlassian MCP server lifespan shutdown complete.")
...
main_mcp = AtlassianMCP(name="INTSIG Atlassian MCP", lifespan=main_lifespan)
