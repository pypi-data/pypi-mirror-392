...
class PipelineOrchestrator:
    ...
    def __init__(self, config: PipelineConfig):
        ...
        self.read_tool = ReadTool(max_file_size=config.read_max_file_size, config=config)
    ...
    def search(
        ...
        Args:
            ...
            keywords: Extracted keywords from agent (primary_terms, search_terms, file_patterns)
        ...
        # 1. Use keywords provided by agent
        ...
        # Single-turn parallel search (optimized for speed)
        # Dynamic scaling based on system resources
        import os
        cpu_count = os.cpu_count() or 4
        MAX_TURNS = int(os.getenv("MAX_TURNS", "1"))  # Single turn for max speed
        # Scale parallel searches to match system capacity (16x CPU cores for I/O-bound grep)
        optimal_parallel = min(cpu_count * 16, int(os.getenv('MAX_PARALLEL_SEARCHES', '256')))
        MAX_PARALLEL_SEARCHES = optimal_parallel

        # Combine primary_terms and search_terms into single list for single-turn execution
        all_search_terms = (extracted_keywords.primary_terms + extracted_keywords.search_terms)[:MAX_PARALLEL_SEARCHES]

        all_candidates: List[CandidateMatch] = []
        search_start = time.time()
        
        # Execute all searches in parallel in a single batch
        with ThreadPoolExecutor(max_workers=MAX_PARALLEL_SEARCHES) as executor:
            futures = []
            # Submit all grep searches in parallel
            ...
            for tool_name, search_term, future in futures:
                try:
                    if tool_name == 'grep':
                        ...
                        all_candidates.extend(glob_candidates)
        ...
        # Send candidates to Python server for reranking with keywords
    ...
    def _deduplicate_candidates(self, candidates: List[CandidateMatch]) -> List[CandidateMatch]:
        ...
        Uses dict-based approach for O(n) time complexity while preserving insertion order.
        """
        seen = {}
    ...
    def search_in_file(
        ...
        Args:
            file_path: Path to the file
            query: Natural language search query
            keywords: Extracted keywords from agent (search_terms)
            context_lines: Number of context lines
        Returns:
            List of matching FileSpans
        """
        # Use keywords provided by agent
        search_terms = keywords.get("search_terms", [])
