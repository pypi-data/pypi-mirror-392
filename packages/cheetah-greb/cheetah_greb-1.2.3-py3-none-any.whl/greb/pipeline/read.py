...
from .base import FileSpan, CandidateMatch, PipelineConfig


class ReadTool:
    """Implements file reading with context windows and span extraction."""

    def __init__(self, max_file_size: Optional[int] = None, ignore_file: Optional[str] = None, config: Optional[PipelineConfig] = None):
        ...
        # Initialize LRU cache for file contents
        cache_size = config.file_cache_size if config else int(os.getenv('FILE_CACHE_SIZE', '128'))
    ...
    def _read_file_uncached(self, file_path: str, encoding: str = "utf-8") -> List[str]:
        """
        Read file contents without caching (internal method).
        This method is wrapped by lru_cache in __init__.
    ...
    def _get_file_lines(self, file_path: str, encoding: str = "utf-8") -> List[str]:
        """
        Get file lines with caching support.
        ...
        Returns:
            List of lines from the file
        """
        # Check if result is in cache by comparing cache info before and after
        cache_info_before = self._read_file_cached.cache_info()
        lines = self._read_file_cached(file_path, encoding)
        cache_info_after = self._read_file_cached.cache_info()
    ...
    def read_file(
        ...
        lines = self._get_file_lines(file_path, encoding)
        self.files_read += 1
    ...
    def read_spans_from_candidates(
        ...
        logger.info(f"File reading completed in {elapsed_time:.2f}ms: "
                   f"{len(spans)} spans from {len(unique_candidates)} candidates, "
    ...
    def _read_file_safe(self, file_path: str, encoding: str = "utf-8") -> Optional[List[str]]:
        """
        Safe file reading with error handling for parallel execution.
        ...
        Returns:
            List of lines or None if reading fails
        """
        try:
            return self._get_file_lines(file_path, encoding)
    ...
    def get_performance_metrics(self) -> Dict[str, int]:
        """
        Get performance metrics for file reading operations.
        
        Returns:
            Dictionary containing cache hits, misses, and files read
        """
    ...
    def read_spans_from_candidates_parallel(
        self,
        candidates: List[CandidateMatch],
        window_size: Optional[int] = None,
        max_spans: Optional[int] = None
        ...
        """
        Convert CandidateMatch objects to FileSpan objects with parallel file reading.
        
        This method uses ThreadPoolExecutor to read multiple files concurrently,
        significantly improving performance when dealing with many candidates.
        ...
        # Get max parallel file reads from config
        max_parallel = self.config.max_parallel_file_reads if self.config else int(os.getenv('MAX_PARALLEL_FILE_READS', '32'))
        ...
        # Use parallel file reading with ThreadPoolExecutor
        spans = []
        max_workers = min(len(unique_candidates), max_parallel)
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_candidate):
                try:
                    ...
                    # Skip files that can't be read
                    logger.debug(f"Failed to read candidate file: {e}")
                    continue
        
        # Log performance metrics
        elapsed_time = (time.time() - start_time) * 1000  # Convert to ms
        logger.info(f"Parallel file reading completed in {elapsed_time:.2f}ms: "
                   f"{len(spans)} spans from {len(unique_candidates)} candidates, "
                   f"cache hits: {self.cache_hits}, cache misses: {self.cache_misses}")
        
        # Return spans limited to max_spans
        return spans[:max_spans]
