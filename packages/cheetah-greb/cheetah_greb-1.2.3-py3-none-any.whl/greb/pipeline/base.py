...
class PipelineConfig(BaseModel):
    ...
    rerank_temperature: float = float(os.getenv("RERANK_TEMPERATURE", "0.0"))

    # Performance tuning configuration
    max_turns: int = int(os.getenv("MAX_TURNS", "1"))
    max_parallel_searches: int = int(os.getenv("MAX_PARALLEL_SEARCHES", "256"))
    max_parallel_file_reads: int = int(os.getenv("MAX_PARALLEL_FILE_READS", "32"))
    file_cache_size: int = int(os.getenv("FILE_CACHE_SIZE", "128"))
    rerank_timeout: int = int(os.getenv("RERANK_TIMEOUT", "10"))
    rerank_max_batch_size: int = int(os.getenv("RERANK_MAX_BATCH_SIZE", "100"))
