...
async def code_search(query: str, directory: str = ".", file_patterns: List[str] = None, max_results: int = 10) -> str:
    ...
    print(f"MCP: Using orchestrator pipeline search in {directory}", file=sys.stderr)
    ...
    orchestrator_config = PipelineConfig(
        ...
        max_grep_results=int(os.getenv("MAX_GREP_RESULTS", "1000")),
    ...
    # Create HTTP client for server calls
    ...
    # Add execution info
    formatted_results.append(f"\n**Search completed in:** {response.execution_time_ms:.2f}ms")

    if response.extracted_keywords:
