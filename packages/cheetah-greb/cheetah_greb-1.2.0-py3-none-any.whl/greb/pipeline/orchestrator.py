...
class PipelineOrchestrator:
    ...
    def search(
        ...
        for turn in range(1, MAX_TURNS):
            ...
            print(f"     [TIME] Turn {turn}: {len(turn_candidates)} candidates in {turn_time:.2f}ms")
        ...
        print(f"     [TIME] Deduplication: {len(unique_candidates)} unique from {len(all_candidates)} total in {dedup_time:.2f}ms")
        ...
        print(f"     [TIME] Read spans: {len(spans)} spans in {read_time:.2f}ms")
        # 7. Re-rank using Python server (passing candidates to /v1/rerank)
        rerank_start = time.time()

        # Convert candidates to format expected by rerank API
        rerank_candidates = []
        for candidate in unique_candidates:
            context_text = f"{candidate.context_before}\n{candidate.matched_text}\n{candidate.context_after}".strip()
            rerank_candidates.append({
                "path": candidate.path,
                "line_number": candidate.line_number,
                "content": candidate.matched_text,
        ...
        print(f"     [TIME] Server reranking: {len(rerank_data.get('results', []))} results in {rerank_time:.2f}ms")
