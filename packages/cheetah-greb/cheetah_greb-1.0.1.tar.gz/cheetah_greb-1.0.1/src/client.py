...
class GrebClient:
    ...
    def __init__(
        ...
        self.grep_tool = GrepTool(max_results=max_grep_results)
        self.glob_tool = GlobTool(max_results=50)
        self.read_tool = ReadTool(max_file_size=5048576)
    ...
    def search(
        ...
        # Step 2: Run grep searches LOCALLY on user's machine
        candidates = []
        ...
        for keyword in keywords[:3]:  # Use top 3 keywords
            try:
                grep_result = self.grep_tool.grep(
                    pattern=keyword,
                    path=search_dir,
                ...
                # Parse grep results and add to candidates
                if grep_result and grep_result.get("matches"):
                ...
                # Continue even if one keyword search fails
                print(f"Warning: grep search failed for '{keyword}': {e}")
                continue
        
        # Deduplicate candidates by path + line_number
        unique_candidates = {}
        for cand in candidates:
            key = (cand["path"], cand["line_number"])
            if key not in unique_candidates:
                unique_candidates[key] = cand
        
        candidates = list(unique_candidates.values())
        
        # Step 3: Send candidates to API for reranking
        ...
        response = self.client.post(
            "/v1/search",  # Same endpoint - server detects candidates and only reranks
    ...
    def _extract_keywords(self, query: str) -> List[str]:
        ...
        stop_words = {"find", "show", "get", "all", "the", "a", "an", "in", "for", "with", "and", "or"}
