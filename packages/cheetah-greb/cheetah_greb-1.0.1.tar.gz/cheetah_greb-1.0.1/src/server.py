...
class SearchRequest(BaseModel):
    """
    Search request - supports both modes:
    1. Legacy: directory + file_patterns (server searches - deprecated)
    2. New: candidates array (client searched locally, server only reranks)
    """
    query: str
    candidates: Optional[List[Dict[str, Any]]] = Field(
        default=None,
        description="Pre-searched candidates from local grep/glob searches"
    )
...
async def search_code(
    ...
    """
    Code search endpoint - supports two modes:
    1. NEW (recommended): Client sends pre-searched candidates, server only reranks
    2. LEGACY: Server searches and reranks (deprecated, requires file access)
    """
    global orchestrator
    ...
    try:
        ...
        # NEW MODE: Client sent pre-searched candidates - only rerank
        if request.candidates is not None:
            print(f"RERANK MODE: Reranking {len(request.candidates)} candidates for: {request.query}")
            
            # Import required classes
            from .pipeline.rerank import RerankTool
            from .pipeline.base import CandidateMatch, RankedResult
            
            # Convert candidates dict to CandidateMatch objects
            candidate_matches = []
            for cand in request.candidates:
                candidate_matches.append(CandidateMatch(
                    path=cand.get("path", ""),
                    line_number=cand.get("line_number", 0),
                    content=cand.get("content", ""),
                    context=cand.get("context", []),
                    score=cand.get("score", 0.0)
                ))
            
            # Rerank using Cerebras
            rerank_tool = orchestrator.rerank
            response = rerank_tool.rerank(
                query=request.query,
                candidates=candidate_matches,
                top_k=request.max_results or orchestrator.config.top_k_results
            )
            
            search_time = (time_module.time() - search_start) * 1000
            print(f"Reranking completed: {len(response.results)} results in {search_time:.2f}ms")
            
        else:
            # LEGACY MODE: Server searches (requires file access)
            print(f"LEGACY MODE: Server searching for: {request.query}")
            print(f"Directory: {request.directory or 'current'}")
            print(f"Max results: {request.max_results or 'default (config.top_k_results)'}")
            
            # Run blocking orchestrator.search() in thread pool
            ...
            search_time = (time_module.time() - search_start) * 1000
            print(f"Search completed: {len(response.results)} results in {search_time:.2f}ms")
