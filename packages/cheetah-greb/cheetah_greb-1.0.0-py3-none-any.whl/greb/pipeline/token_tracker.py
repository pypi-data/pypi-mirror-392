...
class TokenTracker:
    ...
    def extract_usage_from_response(self, response: Any, calculated_input_tokens: int = 0) -> TokenUsage:
        ...
        try:
            ...
            if hasattr(response, 'usage') and response.usage:
                ...
                # Use API tokens if available, but log calculated input for verification
                if calculated_input_tokens > 0:
                    print(f"Token verification: Calculated {calculated_input_tokens} input, API reports {api_prompt_tokens}")
    ...
    def track_reranking(self, response: Any, messages: list = None, span_data: list = None) -> TokenUsage:
        ...
        if span_data:
            ...
            print(f"   Grep results for re-ranking: {span_text_tokens} tokens from {len(span_data)} code spans")
