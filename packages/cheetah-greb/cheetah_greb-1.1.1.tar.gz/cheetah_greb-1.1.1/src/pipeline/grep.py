...
class GrepTool:
    ...
    def __init__(self, max_results: Optional[int] = None, ignore_file: Optional[str] = None):
        self.max_results = max_results or int(os.getenv('MAX_GREP_RESULTS', '1500'))
    ...
    def _find_rg_command(self) -> str:
        """Find ripgrep executable on the system."""
        # First try direct command execution
        rg_commands = ["rg", "ripgrep", "rg.exe"]
        ...
        try:
            lookup_cmd = ["where", "rg"] if os.name == "nt" else ["which", "rg"]
            result = subprocess.run(
                lookup_cmd,
                capture_output=True,
                text=True,
            ...
            if result.returncode == 0:
                # Get the first path from where output
                rg_paths = result.stdout.strip().split('\n')
                for rg_path in rg_paths:
                    rg_path = rg_path.strip()
                    if rg_path and os.path.exists(rg_path):
                        return rg_path
        except (subprocess.TimeoutExpired, FileNotFoundError, PermissionError):
            pass

        # Try to find rg in common installation locations on Windows
        if os.name == "nt":
            common_paths = [
                r"C:\ProgramData\chocolatey\bin\rg.exe",  # Chocolatey installation
                r"C:\Program Files\ripgrep\rg.exe",
                r"C:\Program Files (x86)\ripgrep\rg.exe",
                r"C:\Users\{}\AppData\Local\Programs\ripgrep\rg.exe".format(os.getenv("USERNAME", "")),
                # VS Code bundled ripgrep
                r"C:\Users\{}\AppData\Local\Programs\Microsoft VS Code\resources\app\node_modules\@vscode\ripgrep\bin\rg.exe".format(os.getenv("USERNAME", "")),
            ]
