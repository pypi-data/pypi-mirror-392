...
impl Layout {
    ...
    pub fn broadcast_as<S: Into<Shape>>(&self, shape: S) -> Result<Self> {
        ...
        for (&dst_dim, (&src_dim, &src_stride)) in
            shape.dims()[added_dims..].iter().zip(self.dims().iter().zip(self.stride()))
        {
    ...
    pub(crate) fn strided_blocks(&self) -> crate::StridedBlocks<'_> {
        ...
        if index_dims == 0 {
            crate::StridedBlocks::SingleBlock { start_offset: self.start_offset, len: block_len }
        } else {
