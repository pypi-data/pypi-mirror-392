...
class DataPreparation:
    ...
    def data_preprocessing(self, raw_df_text):
        ...
        raw_df_text['story_title'] = raw_df_text['story_title'].apply(
            preprocessor.convert_ascii_sentence)
        raw_df_text['story_title'] = raw_df_text['story_title'].apply(
    ...
    def __dataframe_creation_ner_tagset(self, cleaned_df_text):
        ...
        for idx_story, story in cleaned_df_text.iterrows():
            ...
            # do sentence segmentation using our TextPreprocessor
            ...
            for sentence in segmented_sentences:
                ...
                # preprocess sentence
                sentence = preprocessor.remove_whitespace_multiple(
                    sentence.strip())
                if sentence != '':
                    # predict pos tag untuk setiap w[i] pada sentence dengan pretrained HMM model
                    hmm_model = HiddenMarkovModelPOSTag()
                    predicted_sentence_hmm = hmm_model.predict(sentence)
                    token_tag_pairs = predicted_sentence_hmm.strip().split(' ')

                    for token_tag_pair in token_tag_pairs:
                        word, postag = token_tag_pair.split('/')
                        data.append([
                            story['story_title'], sentence_id, word, postag, ''
                        ])
                idx_sentences += 1
        # convert to dataframe
        df_nertagset = pd.DataFrame(
            data, columns=['StoryTitle', 'SentenceID', 'Word', 'POS Tag', 'Character Named Entity Tagset'])
        return df_nertagset
    ...
    def __dataframe_creation_chars_identification(self, cleaned_df_text):
        ...
        for story_id, story in cleaned_df_text.iterrows():
            ...
            for idx_sentence, sentence in enumerate(segmented_sentences):
                # append into dataframe
                data['id_sentence'].append(idx_sentence)
                data['focus_sentences'].append(sentence.strip())
