import re
import unicodedata
from balinese_nlp.textpreprocessor.stemming import Stemmer
...
class TextPreprocessor:
    ...
    def __init__(self):
        ...
        self.stemmer_model = Stemmer(TextPreprocessor._BALINESE_VOCAB)
    ...
    def balinese_sentences_segmentation(self, text):
        ...
        final_sentences = [s.strip()
                           for s in final_sentences if s != '' and isinstance(s, str)]
    ...
    def stem_text(self, text):
        text = self.remove_urls_and_link(text)
        text = self.remove_tab_characters(text)
        text = self.remove_special_punctuation(text)
        text = self.remove_emoji_pattern(text)
        text = self.remove_exclamation_words(text)
        text = self.remove_non_ascii_punctuation(text)
        text = self.convert_ascii_sentence(text)
        text = self.normalize_words(text)
        stemmed_tokens = [
            self.stemmer_model.stem_word(token) for token in self.balinese_word_tokenize(text)
        ]
