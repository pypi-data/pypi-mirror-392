...
def coerce_strenum_columns_to_str(df: DataFrame):
    """
    Convert StrEnum column labels to plain strings.
    ...
    ----------
    df : DataFrame
        DataFrame with StrEnum column labels to convert.
    ...
    """
    if not any(isinstance(col, StrEnum) for col in df.columns):
        return

    df.columns = [str(col) if isinstance(col, StrEnum) else col for col in df.columns]
    ...
    def write_parquet(
        ...
        **kwargs,
        ...
        if self.use_arro3:
            ...
            coerce_strenum_columns_to_str(df)
