"""
Adopted from Llama Index: LLMS-Ollama - A PyPI Package for Ollama API Interactions

Modifications made to include automatic header passing in all Ollama API requests.

This implementation allows for seamless interactions with the Ollama API while ensuring proper HTTP headers are included.
"""
...
class MLGymOllamaLLM(FunctionCallingLLM):
  base_url: str = Field(
  ...
  def client(self) -> Client:
    if self._client is None:
      self._client = Client(host=self.base_url, timeout=self.request_timeout,headers={ "api-key": self.api_key })
    return self._client
  ...
  def async_client(self) -> AsyncClient:
    if self._async_client is None:
      self._async_client = AsyncClient(
        host=self.base_url, timeout=self.request_timeout, headers={ "api-key": self.api_key }
      )
