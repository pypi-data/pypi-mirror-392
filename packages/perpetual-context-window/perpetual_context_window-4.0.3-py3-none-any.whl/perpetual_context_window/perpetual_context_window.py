...
class PerpetualContextWindow:
	...
	def __getWikipediaPage(self, url_path='', max_tokens=1000):
		try:
			result_text = ''
			...
			count_tokens = infinite_context.countTokens(string=result_text)
			if count_tokens > max_tokens: result_text = infinite_context.getSummaryText(text=result_text, max_tokens=max_tokens)
	...
	def fileInterpreter(self, file_path='', max_tokens=1000, main_page=None, language=None):
		try:
			...
			if file_path.lower().startswith('https://') and '.wikipedia.org/' in file_path.lower(): interpretation_of_the_file = self.__getWikipediaPage(url_path=file_path, max_tokens=max_tokens)
