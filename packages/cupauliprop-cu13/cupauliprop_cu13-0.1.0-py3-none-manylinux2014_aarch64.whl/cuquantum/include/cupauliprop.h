...
#define CUPAULIPROP_MINOR 1 //!< cuPauliProp minor version
...
typedef enum
{
  ...
  CUPAULIPROP_PAULI_Z = 2,  ///< Pauli-Z operator
  CUPAULIPROP_PAULI_Y = 3,  ///< Pauli-Y operator
...
typedef enum
{
  ...
  CUPAULIPROP_CLIFFORD_GATE_Z = 2,       ///< Pauli-Z gate
  CUPAULIPROP_CLIFFORD_GATE_Y = 3,       ///< Pauli-Y gate
  ...
  CUPAULIPROP_CLIFFORD_GATE_CZ = 8,      ///< CZ gate
  CUPAULIPROP_CLIFFORD_GATE_CY = 9,      ///< CY gate
  ...
  CUPAULIPROP_CLIFFORD_GATE_SQRTZ = 13,  ///< Sqrt Z gate
  CUPAULIPROP_CLIFFORD_GATE_SQRTY = 14,  ///< Sqrt Y gate
...
/**
 * \brief Sets the CUDA stream to be used for library operations.
 * 
 * \param[in] handle Library handle.
 * \param[in] stream CUDA stream to be used for library operations.
 * \return cupaulipropStatus_t 
 */
cupaulipropStatus_t cupaulipropSetStream(cupaulipropHandle_t handle,
                                         cudaStream_t stream);
/** \} end contextAPI */
 ...
 * \param[in] isSorted Whether or not the Pauli expansion is sorted.
    A sorted expansion has its Pauli operator terms sorted by the X and Z bits in ascending order (interpreted as big integers in little-endian representation).
...
/**
 * \brief Queries whether a Pauli operator expansion is sorted or not.
 * 
 * \param[in] handle Library handle.
 * \param[in] pauliExpansion Pauli operator expansion.
 * \param[out] isSorted Indicating whether the Pauli operator expansion is sorted.
 * Sortedness is defined as the Pauli strings being sorted in ascending order according to the little-endian representation of the big integers formed by the X and Z bits.
 * True (!= 0) if the Pauli operator expansion is sorted, false (0) otherwise.
 * \return cupaulipropStatus_t 
 */
cupaulipropStatus_t cupaulipropPauliExpansionIsSorted(const cupaulipropHandle_t handle,
                                                      const cupaulipropPauliExpansion_t pauliExpansion,
                                                      int32_t * isSorted);
 ...
 * \param[in] makeSorted Whether or not the output expansion is required to be sorted.
...
/**
 * \brief Prepares a Pauli expansion view for canonical sorting.
 * 
 * \details This function queries the workspace requirements for canonically sorting
 * a Pauli expansion view (by Pauli strings viewed as big integers in little-endian order).
 *
 * \param[in] handle Library handle.
 ...
 * \return cupaulipropStatus_t
 */
cupaulipropStatus_t cupaulipropPauliExpansionViewPrepareCanonicalSort(const cupaulipropHandle_t handle,
                                                                      const cupaulipropPauliExpansionView_t viewIn,
                                                                      int64_t maxWorkspaceSize,
                                                                      cupaulipropWorkspaceDescriptor_t workspace);

/**
 * \brief Sorts a Pauli expansion view canonically.
 * 
 * \details This function sorts a Pauli expansion view by Pauli strings
 * (viewed as big integers in little-endian order), writing the result to the output expansion.
 * 
 * \note This function is non-blocking if a stream is set on the library handle (via `cupaulipropSetStream`),
 * i.e. it will return immediately and the sorting will be performed asynchronously on the stream.
 * \note The storage location of both the input view and the output expansion must be GPU-accessible (i.e. `CUPAULIPROP_MEMSPACE_DEVICE`).
 *
 * \param[in] handle Library handle.
 * \param[in] viewIn Pauli expansion view to be sorted.
 ...
 * \return cupaulipropStatus_t 
 */
cupaulipropStatus_t cupaulipropPauliExpansionViewExecuteCanonicalSort(const cupaulipropHandle_t handle,
                                                                      const cupaulipropPauliExpansionView_t viewIn,
                                                                      cupaulipropPauliExpansion_t expansionOut,
                                                                      cupaulipropWorkspaceDescriptor_t workspace);
 ...
 *
 * \note This function is non-blocking if a stream is set on the library handle (via `cupaulipropSetStream`),
 * i.e. it will return immediately and the population will be performed asynchronously on the stream.
 *
 ...
 * 
 * \note This function is non-blocking if a stream is set on the library handle (via `cupaulipropSetStream`),
 * i.e. it will return immediately and the tracing will be performed asynchronously on the stream.
 ...
 * The output expansion will satisfy sortedness and uniqueness respectively if these flags have been set to true when creating or resetting the output expansion.
 * Otherwise they may or may not be satisfied.
 * Their state is queryable on the output expansion after this function call via `cupaulipropPauliExpansionGetSortedness()` and `cupaulipropPauliExpansionGetUniqueness()`.
 * \param[in] quantumOperator Quantum operator to be applied.
