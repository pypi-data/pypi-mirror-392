...
class AIJLAgentProxyHandler(WebSocketHandler):
    ...
    async def _handle_tool_call_request(self, data):
        ...
        try:
            ...
            frontend_interactive_tools = ["edit_cell", "rerun_all_cells", "insert_markdown_cell", "cell_execute", "open_tab"]
    ...
    async def _execute_local_tool(self, tool_name: str, tool_args: dict, request_id: str = None) -> str:
        ...
        if tool_name == "cell_execute":
            ...
            return await self.tool_executor.edit_file(
                tool_args.get("file_path", ""),
                tool_args.get("content", "")
            )

        elif tool_name == "list_dir":
            return await self.tool_executor.list_dir(tool_args.get("dir", "."))

        elif tool_name == "grep":
            return await self.tool_executor.grep(
                pattern=tool_args.get("pattern"),
                path=tool_args.get("path", "."),
                i=tool_args.get("i", False),
                A=tool_args.get("A"),
                B=tool_args.get("B"),
                C=tool_args.get("C"),
                ...
                head_limit=tool_args.get("head_limit"),
                multiline=tool_args.get("multiline", False),
            )
            ...
            return await self.tool_executor.open_tab(tool_args.get('file_path'), request_id)
        ...
        try:
            ...
            if msg_type == "user":
                ...
                await self._forward_to_remote_backend(msg)
                             ...
                             "edit_cell_result", "insert_markdown_cell_result", "rerun_all_cells_permission_response", "open_tab_result"]:
