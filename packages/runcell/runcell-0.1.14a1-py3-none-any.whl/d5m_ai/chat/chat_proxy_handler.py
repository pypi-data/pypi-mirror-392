...
from ..tools import execute_read_file_tool, execute_list_dir_tool, execute_grep_tool
    ...
    async def _handle_pending_tool_call(self, pending_data: Dict[str, Any], body_data: Dict[str, Any], user_token: str):
        ...
        for idx, tool_call in enumerate(tool_calls):
            ...
            if tool_name == 'read_file':
                tool_result = await execute_read_file_tool(parsed_args)
                sse_output = tool_result
            elif tool_name == 'list_dir':
                list_result = await execute_list_dir_tool(parsed_args)
                # For SSE event, send the list as-is (for frontend renderer)
                sse_output = list_result
                # For LLM messages, convert to JSON string
                tool_result = json.dumps(list_result)
            elif tool_name == 'grep':
                grep_result = await execute_grep_tool(parsed_args)
                # grep_result is already a JSON string from the tool
                tool_result = grep_result
                # For SSE event, parse and send the structured data
                try:
                    sse_output = json.loads(grep_result)
                except json.JSONDecodeError:
                    sse_output = grep_result
            else:
