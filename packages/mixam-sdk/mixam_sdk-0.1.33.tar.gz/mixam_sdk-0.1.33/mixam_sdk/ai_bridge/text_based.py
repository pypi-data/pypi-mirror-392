...
from mixam_sdk.utils.enum_json import enum_by_name_or_value
...
def text_based(cls: type[T]) -> type[T]:
    ...
    def _update_annotation(
        ...
        args = [
            arg
            if not isinstance(arg, BeforeValidator)
            else enum_by_name_or_value(new_annotation_concrete)
            for arg in args
        ]
...
def deduplicate_metadata(model_cls: type[T]) -> type[T]:
    for field in model_cls.model_fields.values():
        ...
        if not isinstance(md, tuple) or not md:
            continue
