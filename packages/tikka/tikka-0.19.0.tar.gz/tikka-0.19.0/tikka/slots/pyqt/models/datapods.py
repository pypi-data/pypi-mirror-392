...
class DataPodsTableModel(QAbstractTableModel):
    """
    DataPodsTableModel class that drives the population of tabular display
    """
    ...
    def __init__(self, application: Application):
        ...
        self.headers = [self._("Url")]

        self.column_types = [
            self.application.datapods.repository.COLUMN_URL,
        ]
        self.sort_order_types = [
            DataPodsRepositoryInterface.SORT_ORDER_ASCENDING,
            DataPodsRepositoryInterface.SORT_ORDER_DESCENDING,
        ]

        self.datapods: List[DataPod] = []
        self.init_data()
    ...
    def init_data(self):
        ...
        if pref_sort_column is None:
            sort_column = DataPodsRepositoryInterface.COLUMN_URL
        else:
        ...
        if pref_sort_order is None:
            sort_order = DataPodsRepositoryInterface.SORT_ORDER_ASCENDING
        else:
            sort_order = self.sort_order_types[int(pref_sort_order)]

        self.beginResetModel()
        self.datapods = self.application.datapods.repository.list(
    ...
    def sort(self, column: int, order: Optional[Qt.SortOrder] = None):
        ...
        if column > -1:
            self.application.repository.preferences.set(
                DATAPODS_TABLE_SORT_COLUMN_PREFERENCES_KEY, str(column)
            )
            self.application.repository.preferences.set(
                DATAPODS_TABLE_SORT_ORDER_PREFERENCES_KEY, str(order)
            )
        self.init_data()
    ...
    def rowCount(self, _: QModelIndex = QModelIndex()) -> int:
        ...
        :return:
        """
        count = self.application.datapods.repository.count()
        if count == 0:
            return 0
        if count <= len(self.datapods):
            return count

        return len(self.datapods)
    ...
    def data(self, index: QModelIndex, role: int = Qt.DisplayRole) -> QVariant:
        ...
        col = index.column()
        row = index.row()
        indexer = self.datapods[row]
        data = QVariant()
        if role == Qt.DisplayRole:
            if col == 0:
                data = QVariant(indexer.url)
