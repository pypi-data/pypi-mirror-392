...
from tikka.domains.entities.events import DataPodsEvent, IndexersEvent
from tikka.slots.pyqt.resources.gui.windows.indexer_add_rc import Ui_indexerAddDialog


class DataPodAddWindow(QDialog, Ui_indexerAddDialog):
    """
    DataPodAddWindow class
    """

    def __init__(self, application: Application, parent: Optional[QWidget] = None):
        """
        Init add data pod window
    ...
    def _on_test_button_clicked(self):
        ...
        datapod = self.application.datapods.network_test_and_get_datapod(url)
        if datapod is not None:
            self.blockValueLabel.setText(str(datapod.block))
    ...
    def on_accepted_button(self) -> None:
        ...
        """
        url = self.urlLineEdit.text()
        if self.application.datapods.get(url) is not None:
            return

        self.application.datapods.add(
            DataPod(
                url=self.urlLineEdit.text(),
        ...
        self.application.event_dispatcher.dispatch_event(
            DataPodsEvent(IndexersEvent.EVENT_TYPE_LIST_CHANGED)
        )
...
if __name__ == "__main__":
    ...
    DataPodAddWindow(application_).exec_()
