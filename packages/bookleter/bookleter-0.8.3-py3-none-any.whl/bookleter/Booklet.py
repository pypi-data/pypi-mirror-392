...
from PyPDF2 import (
    PdfWriter as PdfFileWriter,
    PdfReader as PdfFileReader,
    PageObject,
    PdfReader,
)
...
class Book:
    def __init__(
        ...
        self._validate_inputs(input_file_path, start_page_number, end_page_number, crop)
    ...
    def make_booklet(self):
        ...
        if self.end_page_number % 8 == 0:
            ...
            correct_pages_count = (
                (((self.end_page_number - self.start_page_number) + 1) // 8) + 1
            ) * 8
        ...
        base_page_size = inputpdf.pages[len(print_order) // 2].mediabox
        ...
        for number in print_order:
            ...
            if pg_number == -1:
                # it's a blank page
                page = PageObject.create_blank_page(pdf=inputpdf)
                page.mediabox = base_page_size
                final.add_page(page)
            else:
                # it's a page from the original pdf
                page = inputpdf.pages[pg_number - 1]
                # do the crops
                page.cropbox.lower_left = tuple(
                    [
                        a + b
                        for a, b in zip(
                            page.cropbox.lower_left,
                            (int(self.crop["left"]), int(self.crop["bottom"])),
                        )
                    ]
                )
                page.cropbox.upper_right = tuple(
                    [
                        a - b
                        for a, b in zip(
                            page.cropbox.upper_right,
                            (int(self.crop["right"]), int(self.crop["top"])),
                        )
                    ]
                )
                final.add_page(page)
        ...
        if self._get_pdf_pages_count(self.original_pdf_path) >= 8:
            for pg_number in test_print_order:
                test_file.add_page(inputpdf.pages[pg_number - 1])
    ...
    def _validate_inputs(
        self, input_file_path, start_page_number, end_page_number, crop
    ):
        if input_file_path == "":
        ...
        if int(end_page_number) > self._get_pdf_pages_count(input_file_path):
            raise ValueError(
                "End page number out of range\nYour book has only {} pages".format(
                    self._get_pdf_pages_count(input_file_path)
                )
            )
