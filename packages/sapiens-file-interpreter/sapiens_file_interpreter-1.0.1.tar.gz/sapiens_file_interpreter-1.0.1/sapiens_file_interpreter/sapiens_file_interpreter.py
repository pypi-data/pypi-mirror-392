...
class SapiensFileInterpreter:
	...
	def readFiles(self, path='', language=None):
		try:
			...
			total_size_in_bytes = self.__get_total_size_in_bytes(path_string=path)
			increment_value = max(1, total_size_in_bytes/16601529)
			self.__start_progress_increment(start_value=11, stop_value=75, increment_value=increment_value)
			text_of_the_interpretation = infinite_context_window.interpreter(path, max_tokens=1000000000, language=language)
