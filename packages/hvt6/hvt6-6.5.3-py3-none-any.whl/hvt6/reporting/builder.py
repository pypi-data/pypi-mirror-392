...
class ReportBuilder:
    ...
    def generate_comprehensive_pdf(
        ...
        try:
            # Lazy import - only load if PDF generation is requested
            try:
                from .pdf_generator import PDFReportGenerator
            except ImportError:
                raise ReportGenerationError(
                    "PDF generation requires optional dependencies. "
                    "Install with: pip install hvt6[pdf]"
                )
    ...
    def generate_excel(
        ...
        try:
            # Lazy import - only load if Excel generation is requested
            try:
                from .excel_generator import ExcelReportGenerator
            except ImportError:
                raise ReportGenerationError(
                    "Excel generation requires openpyxl. This should be installed by default. "
                    "Try: pip install --upgrade hvt6"
                )
