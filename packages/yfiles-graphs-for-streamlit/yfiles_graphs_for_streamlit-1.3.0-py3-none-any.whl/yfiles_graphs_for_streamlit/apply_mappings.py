from typing import Dict as TDict, List as TList, Union, Callable, Any, cast
import inspect
...
class MappingClass:
    ...
    def _get_wrapped_mapping_function(
            self,
            function: Union[MappingCallable, str, None],
            key: str):
        ...
        def _get_function_value(fn: Union[MappingCallable, str, None], index: int, element: TDict):
            if fn is None:
                return None
            ...
            if len(parameters) == 0:
                ...
                return cast(Callable[[int], Any], fn)(index)
            elif len(parameters) == 1:
                return cast(Callable[[dict], Any], fn)(element)
            else:
                return cast(Callable[[int, dict], Any], fn)(index, element)
        ...
        def wrapped(index: int, element: TDict):
            ...
            # convert dataclasses to js keyed dicts
            if isinstance(value, (LabelStyle, NodeStyle, EdgeStyle)):
                value = value.to_js()
    ...
    def _apply_mapping_and_change_value(
            self,
            key: str,
            mapping: Callable[[TList[TDict], TList[MappingCallable]], Any],
            *args,
            **kwargs
    ):
    ...
    @staticmethod
    def _apply_elements_mappings(elements: TList[TDict], mappings: TList[MappingCallable]):
        ...
        for index, element in enumerate(elements):
            for mapping in mappings:
                element = cast(Callable[[int, dict], Any], mapping)(index, element)
