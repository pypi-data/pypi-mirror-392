...
class CommonToolSet:
    ...
    def to_langchain(
        ...
        try:
            ...
            if adapter is not None:
                ...
                # 批量转换为 LangChain 格式
                return adapter.from_canonical(canonical_tools)
        except (ImportError, AttributeError, KeyError):
            pass
    ...
    def to_google_adk(
        ...
        # 回退到逐个转换（保持向后兼容）
        return [tool.to_google_adk() for tool in tools]
    ...
    def to_agentscope(
        ...
        tools = self.tools(prefix, modify_tool_name, filter_tools_by_name)
        return [tool.to_agentscope() for tool in tools]
    ...
    # def __getitem__(self, key: Union[int, str]) -> Tool:
    #     """支持通过索引或名称访问工具"""
    ...
    #     else:
    #         raise TypeError(
    #             f"Invalid key type: {type(key)}. Expected int or str."
    #         )
...
def tool(
    ...
    def decorator(func: Callable) -> Tool:
        ...
        def _func(*args, **kwargs):
            logger.debug(
                "invoke tool %s with arguments %s %s", name, args, kwargs
            )
            results = func(*args, **kwargs)
            logger.debug(
                "invoke tool %s finished with result %s", name, results
            )
...
def _convert_to_native(value: Any) -> Any:
    ...
    # 尝试转换为字典（处理 Google ADK Nestedobject）
...
def _build_args_schema(
    ...
    # OpenAPI 格式
    if "parameters" in meta or "request_body" in meta:
