...
class AgentRunServer:
    ...
    Example (默认 OpenAI 协议):
        >>> def invoke_agent(request: AgentRequest):
        ...     return "Hello, world!"
        >>>
        >>> server = AgentRunServer(invoke_agent=invoke_agent)
        >>> server.start(port=8000)
        # 可访问: POST http://localhost:8000/v1/chat/completions
    ...
    Example (自定义前缀):
        ...
        ... )
        >>> server.start(port=8000)
        # 可访问: POST http://localhost:8000/api/v1/chat/completions
    ...
    Example (集成到现有 FastAPI 应用):
        ...
        # 可访问: POST http://localhost:8000/agent/v1/chat/completions
    """
    ...
    def __init__(
        ...
        Args:
            invoke_agent: Agent 调用回调函数
                - 可以是同步或异步函数
                - 支持返回字符串、AgentResponse 或生成器

            protocols: 协议处理器列表
                - 默认使用 OpenAI 协议
                - 可以添加自定义协议

            prefix_overrides: 协议前缀覆盖
                - 格式: {协议类名: 前缀}
                - 例如: {"OpenAIProtocolHandler": "/api/v1"}
        """
