...
class ProcessWorkerPool:
    def __init__(self, config, snapshot_builder, resource_exporter):
        ...
        configured_threshold = max(0, getattr(config, 'WORKER_TIMEOUT_THRESHOLD', 0))
        if configured_threshold:
            self.worker_timeout_threshold = configured_threshold
        else:
            self.worker_timeout_threshold = max(1, math.ceil(self.worker_slots * 0.3))
